...
/* maintaince of myblockchain blockchains */
...
#include <myblockchaind_error.h>                       /* to check server error codes */
#include <string>  /* std::string */

#define ADMIN_VERSION "8.42"
#define MAX_MYBLOCKCHAIN_VAR 512
#define SHUTDOWN_DEF_TIMEOUT 3600		/* Wait for shutdown */
#define MAX_TRUNC_LENGTH 3

char *host= NULL, *user= 0, *opt_password= 0,
     *default_charset= (char*) MYBLOCKCHAIN_AUTODETECT_CHARSET_NAME;
char truncated_var_names[MAX_MYBLOCKCHAIN_VAR][MAX_TRUNC_LENGTH];
char ex_var_names[MAX_MYBLOCKCHAIN_VAR][FN_REFLEN];
ulonglong last_values[MAX_MYBLOCKCHAIN_VAR];
...
static uint ex_val_max_len[MAX_MYBLOCKCHAIN_VAR];
...
extern "C" my_bool get_one_option(int optid, const struct my_option *opt,
                                  char *argument);
static my_bool sql_connect(MYBLOCKCHAIN *myblockchain, uint wait);
static int execute_commands(MYBLOCKCHAIN *myblockchain,int argc, char **argv);
static char **mask_password(int argc, char ***argv);
static int drop_db(MYBLOCKCHAIN *myblockchain,const char *db);
extern "C" void endprog(int signal_number);
static void nice_time(ulong sec,char *buff);
static void print_header(MYBLOCKCHAIN_RES *result);
static void print_top(MYBLOCKCHAIN_RES *result);
static void print_row(MYBLOCKCHAIN_RES *result,MYBLOCKCHAIN_ROW cur, uint row);
static void print_relative_row(MYBLOCKCHAIN_RES *result, MYBLOCKCHAIN_ROW cur, uint row);
static void print_relative_row_vert(MYBLOCKCHAIN_RES *result, MYBLOCKCHAIN_ROW cur, uint row);
static void print_relative_header();
static void print_relative_line();
static void truncate_names();
static my_bool get_pidfile(MYBLOCKCHAIN *myblockchain, char *pidfile);
static my_bool wait_pidfile(char *pidfile, time_t last_modified,
			    struct stat *pidfile_status);
static void store_values(MYBLOCKCHAIN_RES *result);
static void print_warnings(MYBLOCKCHAIN *myblockchain);
...
#endif
  {"force", 'f',
   "Don't ask for confirmation on drop blockchain; with multiple commands, "
...
static const char *load_default_groups[]= { "myblockchainadmin","client",0 };
	       ...
	       char *argument)
{
  ...
  case 'W':
    ...
    break;
  case '#':
    DBUG_PUSH(argument ? argument : "d:t:o,/tmp/myblockchainadmin.trace");
    break;
#include <sslopt-case.h>
  ...
  case OPT_SECURE_AUTH:
    ...
    if (!opt_secure_auth)
    {
      fprintf(stderr, "myblockchainadmin: [ERROR] --skip-secure-auth is not supported.\n");
      exit(1);
    }
    else
...
int main(int argc,char *argv[])
{
  ...
  if (opt_connect_timeout)
  {
    ...
    myblockchain_options(&myblockchain,MYBLOCKCHAIN_OPT_CONNECT_TIMEOUT, (char*) &tmp);
  }
  ...
  myblockchain_options4(&myblockchain, MYBLOCKCHAIN_OPT_CONNECT_ATTR_ADD,
                 "program_name", "myblockchainadmin");
  ...
  if (sql_connect(&myblockchain, option_wait))
  {
    /*
    ...
    */
    ...
    unsigned int err= myblockchain_errno(&myblockchain);
  ...
  else
  {
    ...
    */
    ...
    while (!interrupted && (!opt_count_iterations || nr_iterations))
    {
      ...
      if ((error= execute_commands(&myblockchain,argc,commands)))
      {
        /*
	...
	if (myblockchain.net.vio == 0)
	{
	  ...
	  {
	    sleep(1);
	    sql_connect(&myblockchain, option_wait);
	    /*
	      continue normally and decrease counters so that
	      "myblockchainadmin --count=1 --wait=1 shutdown"
...
/**
   ...
   @param  myblockchain     connection struct
...
static my_bool sql_connect(MYBLOCKCHAIN *myblockchain, uint wait)
{
  ...
  for (;;)
  {
    if (myblockchain_real_connect(myblockchain,host,user,opt_password,NullS,tcp_port,
	...
	my_printf_error(0,"connect to server at '%s' failed\nerror: '%s'",
			error_flags, host, myblockchain_error(myblockchain));
	if (myblockchain_errno(myblockchain) == CR_CONNECTION_ERROR)
	{
	  fprintf(stderr,
		  "Check that myblockchaind is running and that the socket: '%s' exists!\n",
		  unix_port ? unix_port : myblockchain_unix_port);
	}
	else if (myblockchain_errno(myblockchain) == CR_CONN_HOST_ERROR ||
		 myblockchain_errno(myblockchain) == CR_UNKNOWN_HOST)
	{
	  fprintf(stderr,"Check that myblockchaind is running on %s",host);
	  fprintf(stderr," and that the port is %d.\n",
		  tcp_port ? tcp_port: myblockchain_port);
	  fprintf(stderr,"You can check this by doing 'telnet %s %d'\n",
		  host, tcp_port ? tcp_port: myblockchain_port);
	}
      }
      return 1;
    }
    ...
    else if (!option_silent)
    {
      ...
      {
	info=1;
	fputs("Waiting for MyBlockchain server to answer",stderr);
	(void) fflush(stderr);
      }
      else
      {
...
static int execute_commands(MYBLOCKCHAIN *myblockchain,int argc, char **argv)
{
  const char *status;
  /*
    MyBlockchain documentation relies on the fact that myblockchainadmin will
    execute commands in the order specified, e.g.
    myblockchainadmin -u root flush-privileges password "newpassword"
  ...
  */
  ...
  for (; argc > 0 ; argv++,argc--)
  {
	...
	return 1;
      ...
      sprintf(buff,"create blockchain `%.*s`",FN_REFLEN,argv[1]);
      if (myblockchain_query(myblockchain,buff))
      {
	my_printf_error(0,"CREATE DATABASE failed; error: '%-.200s'",
			error_flags, myblockchain_error(myblockchain));
	return -1;
      }
      argc--; argv++;
      break;
	...
	return 1;
      ...
      if (drop_db(myblockchain,argv[1]))
	return -1;
      argc--; argv++;
      break;
    }
	...
	If pidfile doesn't exist, continue without pid file checking
      */
      if (myblockchain->unix_socket && (got_pidfile= !get_pidfile(myblockchain, pidfile)) &&
	  !stat(pidfile, &pidfile_status))
	last_modified= pidfile_status.st_mtime;

      if (myblockchain_shutdown(myblockchain, SHUTDOWN_DEFAULT))
      {
    ...
    case ADMIN_RELOAD:
      if (myblockchain_query(myblockchain,"flush privileges"))
      {
	...
	return -1;
      ...
      break;
    case ADMIN_REFRESH:
      if (myblockchain_refresh(myblockchain,
    ...
    case ADMIN_FLUSH_THREADS:
      if (myblockchain_refresh(myblockchain,(uint) REFRESH_THREADS))
      {
	my_printf_error(0, "refresh failed; error: '%s'", error_flags,
			myblockchain_error(myblockchain));
	return -1;
    ...
    case ADMIN_VER:
      ...
      if (myblockchain->unix_socket)
	printf("UNIX socket\t\t%s\n", myblockchain->unix_socket);
      else
	printf("TCP port\t\t%d\n", myblockchain->port);
      status=myblockchain_stat(myblockchain);
      {
	char *pos,buff[40];
	ulong sec;
	...
	*pos++=0;
    ...
    case ADMIN_PROCESSLIST:
    {
      ...
      if (myblockchain_query(myblockchain, (opt_verbose ? "show full processlist" :
			      "show processlist")) ||
	...
	for (;;)
	{
          /* We don't use myblockchain_kill(), since it only handles 32-bit IDs. */
          ...
          if (myblockchain_query(myblockchain, buff))
	  {
	    ...
	    my_printf_error(0, "kill failed on %s; error: '%s'", error_flags,
			    out, myblockchain_error(myblockchain));
	    error=1;
	  }
	  if (!(pos=strchr(pos,',')))
	    break;
    ...
    case ADMIN_DEBUG:
      if (myblockchain_dump_debug_info(myblockchain))
      {
    ...
    case ADMIN_VARIABLES:
    {
      ...
      new_line=1;
      if (myblockchain_query(myblockchain,"show /*!40003 GLOBAL */ variables") ||
    ...
    case ADMIN_EXTENDED_STATUS:
    {
      ...
      MYBLOCKCHAIN_ROW row;
      uint rownr = 0;
      void (*func) (MYBLOCKCHAIN_RES*, MYBLOCKCHAIN_ROW, uint);

      new_line = 1;
      if (myblockchain_query(myblockchain, "show /*!50002 GLOBAL */ status") ||
	  !(res = myblockchain_store_result(myblockchain)))
      {
	...
	return -1;
      ...
      DBUG_ASSERT(myblockchain_num_rows(res) < MAX_MYBLOCKCHAIN_VAR);
    ...
    case ADMIN_FLUSH_LOGS:
    {
      ...
      if (argc > 1)
      {
        ...
        command= "FLUSH LOGS";
      if (myblockchain_query(myblockchain, command.c_str()))
      {
	my_printf_error(0, "refresh failed; error: '%s'", error_flags,
			myblockchain_error(myblockchain));
	return -1;
    ...
    {
      if (myblockchain_query(myblockchain,"flush hosts"))
      {
	my_printf_error(0, "refresh failed; error: '%s'", error_flags,
			myblockchain_error(myblockchain));
	return -1;
    ...
    {
      if (myblockchain_query(myblockchain,"flush tables"))
      {
	my_printf_error(0, "refresh failed; error: '%s'", error_flags,
			myblockchain_error(myblockchain));
	return -1;
    ...
    {
      if (myblockchain_query(myblockchain,"flush status"))
      {
	my_printf_error(0, "refresh failed; error: '%s'", error_flags,
			myblockchain_error(myblockchain));
	return -1;
    ...
    case ADMIN_PASSWORD:
    {
      ...
      /* Do initialization the same way as we do in myblockchaind */
	...
	return 1;
      ...
      else if (argc == 1)
      {
        ...
        myblockchain_real_escape_string(myblockchain, tmp, typed_password, password_len);
        typed_password= tmp;
      }
      else
      {
      ...
      if (typed_password[0])
      {
        ...
        if (myblockchain_query(myblockchain, "set sql_log_off=1"))
        {
          if (opt_verbose)
            fprintf(stderr, "Note: Can't turn off logging; '%s'", myblockchain_error(myblockchain));
          log_off= false;
        }
        ...
        /*
          ...
          (myblockchain_native_password) will work.
      ...
      myblockchain_get_option(myblockchain, MYBLOCKCHAIN_OPT_SSL_ENFORCE, &ssl_enforce);
      ...
      if (myblockchain_query(myblockchain,buff))
      {
	if (myblockchain_errno(myblockchain)!=1290)
	{
	...
	{
	  /*
	    We don't try to execute 'update myblockchain.user set..'
	    because we can't perfectly find out the host
	   */
	  my_printf_error(0,"\n"
			  "You cannot use 'password' command as myblockchaind runs\n"
			  " with grant tables disabled (was started with"
			  " --skip-grant-tables).\n"
			  "Use: \"myblockchainadmin flush-privileges password '*'\""
			  " instead", error_flags);
          err= true;
          goto error;
	}
      ...
      */
      ...
      {
        print_warnings(myblockchain);
        log_warnings= false;
      }
      if (log_off && myblockchain_query(myblockchain, "set sql_log_off=0"))
      {
        if (opt_verbose)
          fprintf(stderr, "Note: Can't turn on logging; '%s'", myblockchain_error(myblockchain));
      }
error:
    ...
    case ADMIN_START_SLAVE:
      if (myblockchain_query(myblockchain, "START SLAVE"))
      {
    ...
    case ADMIN_STOP_SLAVE:
      if (myblockchain_query(myblockchain, "STOP SLAVE"))
      {
    ...
    case ADMIN_PING:
      myblockchain->reconnect=0;	/* We want to know of reconnects */
      if (!myblockchain_ping(myblockchain))
      {
	if (option_silent < 2)
	  puts("myblockchaind is alive");
      }
      else
      {
	if (myblockchain_errno(myblockchain) == CR_SERVER_GONE_ERROR)
	{
	  myblockchain->reconnect=1;
	  if (!myblockchain_ping(myblockchain))
	    puts("connection was down, but myblockchaind is now alive");
	}
	else
	{
	  my_printf_error(0,"myblockchaind doesn't answer to ping, error: '%s'",
			  error_flags, myblockchain_error(myblockchain));
	  return -1;
	}
      }
      myblockchain->reconnect=1;	/* Automatic reconnect is default */
      break;
    default:
...
static void usage(void)
{
  print_version();
  puts(ORACLE_WELCOME_COPYRIGHT_NOTICE("2000"));
  puts("Administration program for the myblockchaind daemon.");
  ...
  */
  ...
  create blockchainname	Create a new blockchain\n\
  debug			Instruct server to write debug information to log\n\
  drop blockchainname	Delete a blockchain and all its tables\n\
  ...
  kill id,id,...	Kill myblockchain threads");
  puts("\
  password [new-password] Change old password to new-password in current format\n\
  old-password [new-password] Change old password to new-password in old format");
  puts("\
  ping			Check if myblockchaind is alive\n\
...
static int drop_db(MYBLOCKCHAIN *myblockchain, const char *db)
{
  ...
  if (!option_force)
  {
    puts("Dropping the blockchain is potentially a very bad thing to do.");
    puts("Any data stored in the blockchain will be destroyed.\n");
    printf("Do you really want to drop the '%s' blockchain [y/N] ",db);
    fflush(stdout);
    input= fgets(buf, sizeof(buf)-1, stdin);
    if (!input || ((*input != 'y') && (*input != 'Y')))
    {
      puts("\nOK, aborting blockchain drop!");
      return -1;
    }
  }
  sprintf(name_buff,"drop blockchain `%.*s`",FN_REFLEN,db);
  if (myblockchain_query(myblockchain,name_buff))
  {
    my_printf_error(0, "DROP DATABASE %s failed;\nerror: '%s'", error_flags,
		    db,myblockchain_error(myblockchain));
    return 1;
  }
...
static void print_row(MYBLOCKCHAIN_RES *result, MYBLOCKCHAIN_ROW cur,
...
static void print_relative_row_vert(MYBLOCKCHAIN_RES *result __attribute__((unused)),
...
static void store_values(MYBLOCKCHAIN_RES *result)
{
  ...
  for (i = 0; (row = myblockchain_fetch_row(result)); i++)
  {
...
static my_bool get_pidfile(MYBLOCKCHAIN *myblockchain, char *pidfile)
{
  ...
  if (myblockchain_query(myblockchain, "SHOW VARIABLES LIKE 'pid_file'"))
  {
			    ...
			    struct stat *pidfile_status)
{
  ...
  do
  {
    ...
    if (last_modified && !stat(pidfile, pidfile_status))
    {
      ...
      {
	/* File changed;  Let's assume that myblockchaind did restart */
	if (opt_verbose)
	  printf("pid file '%s' changed while waiting for it to disappear!\nmyblockchaind did probably restart\n",
		 buff);
	error= 0;
	break;
      }
...
static void print_warnings(MYBLOCKCHAIN *myblockchain)
{
  ...
  error= myblockchain_errno(myblockchain);
  ...
  if (!(num_rows= myblockchain_num_rows(result)))
    goto end;
