...
myblockchain_mutex_t LOCK_manager;
myblockchain_cond_t COND_manager;
...
extern "C" void *handle_manager(void *arg __attribute__((unused)))
{
  ...
  for (;;)
  {
    myblockchain_mutex_lock(&LOCK_manager);
	...
	set_timespec(&abstime, flush_time);
      ...
      while ((!error || error == EINTR) && !abort_manager)
        error= myblockchain_cond_timedwait(&COND_manager, &LOCK_manager, &abstime);
    }
    else
    {
      while ((!error || error == EINTR) && !abort_manager)
        error= myblockchain_cond_wait(&COND_manager, &LOCK_manager);
    }
    myblockchain_mutex_unlock(&LOCK_manager);
...
void start_handle_manager()
{
  ...
  if (flush_time && flush_time != ~(ulong) 0L)
  {
    ...
    int error;
    if ((error= myblockchain_thread_create(key_thread_handle_manager,
...
void stop_handle_manager()
{
  ...
  if (manager_thread_in_use)
  {
    ...
    myblockchain_cond_signal(&COND_manager);
  }
  myblockchain_mutex_unlock(&LOCK_manager);
  DBUG_VOID_RETURN;
}
