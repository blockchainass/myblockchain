      ...
      else
      {
          ...
          {       
            ...
            if (context->expecting(Item::FIELD_ITEM) &&
                ...
                (context->expecting_field_result(field->result_type()) ||
                 ...
                 ((type == MYBLOCKCHAIN_TYPE_TIME ||
                   type == MYBLOCKCHAIN_TYPE_TIME2 ||
                   type == MYBLOCKCHAIN_TYPE_DATE || 
                   type == MYBLOCKCHAIN_TYPE_NEWDATE || 
                   type == MYBLOCKCHAIN_TYPE_YEAR ||
                   type == MYBLOCKCHAIN_TYPE_DATETIME ||
                   type == MYBLOCKCHAIN_TYPE_DATETIME2)
                  ? (context->expecting_field_result(STRING_RESULT) ||
                     context->expecting_field_result(INT_RESULT))
                  : TRUE)) &&
                // Bit fields no yet supported in scan filter
                type != MYBLOCKCHAIN_TYPE_BIT &&
                /* Char(0) field is treated as Bit fields inside NDB
                   Hence not supported in scan filter */
                (!(type == MYBLOCKCHAIN_TYPE_STRING && field->pack_length() == 0)) &&
                // No BLOB support in scan filter
                type != MYBLOCKCHAIN_TYPE_TINY_BLOB &&
                type != MYBLOCKCHAIN_TYPE_MEDIUM_BLOB &&
                type != MYBLOCKCHAIN_TYPE_LONG_BLOB &&
                type != MYBLOCKCHAIN_TYPE_BLOB)
            {
        ...
        case Item::FUNC_ITEM:
        {
          ...
          case Item_func::LIKE_FUNC:
          {
             ...
             */
            context->expect(Item::FIELD_ITEM);
            context->expect_only_field_type(MYBLOCKCHAIN_TYPE_STRING);
            context->expect_field_type(MYBLOCKCHAIN_TYPE_VAR_STRING);
            context->expect_field_type(MYBLOCKCHAIN_TYPE_VARCHAR);
            context->expect_field_result(STRING_RESULT);
