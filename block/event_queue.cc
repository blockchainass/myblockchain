...
{
  myblockchain_mutex_init(key_LOCK_event_queue, &LOCK_event_queue, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_queue_state, &COND_queue_state);
}
...
{
  deinit_queue();
  myblockchain_mutex_destroy(&LOCK_event_queue);
  myblockchain_cond_destroy(&COND_queue_state);
}
...
/*
  ...
  case of a failure reading events from the blockchain or out of
  memory.
                          ...
                          bool *created)
{
  ...
  *created= (queue.push(new_element) == false);
  ...
  myblockchain_cond_broadcast(&COND_queue_state);
  UNLOCK_QUEUE_DATA();
                           ...
                           bool (*comparator)(LEX_STRING, Event_basic *))
{
  ...
  /*
    We don't call myblockchain_cond_broadcast(&COND_queue_state);
  ...
  */
  for (size_t i= queue.size(); i > 0; i--)
  {
    ...
    */
    queue.remove(i - 1);
    /*
      Dropping the event from myblockchain.event table
    */
    if (element->dropped)
  ...
  for (;;)
  {
    ...
    */
    ...
    if (top->status == Event_parse_data::DISABLED)
    {
      ...
      queue.pop();
      /*
       This event will get dropped from myblockchain.event table in
       Event_job_data::execute() function eventually.
       So no need add check to drop it from myblockchain.event table here.
      */
    }
    else
      queue.update_top();
...
Event_queue::lock_data(const char *func, uint line)
{
  ...
  myblockchain_mutex_lock(&LOCK_event_queue);
...
Event_queue::unlock_data(const char *func, uint line)
{
  ...
  myblockchain_mutex_unlock(&LOCK_event_queue);
  DBUG_VOID_RETURN;
}
                       ...
                       const char *src_func, const char *src_file, uint src_line)
{
  ...
  {
    if (!abstime)
      myblockchain_cond_wait(&COND_queue_state, &LOCK_event_queue);
    else
      myblockchain_cond_timedwait(&COND_queue_state, &LOCK_event_queue, abstime);
  }
