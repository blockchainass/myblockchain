...
#include "lock.h"       // myblockchain_unlock_tables
...
#include "sql_handler.h"               // myblockchain_ha_rm_tables
...
static int
myblockchain_prepare_create_table(THD *thd, HA_CREATE_INFO *create_info,
...
/**
  @brief Explain a path name by split it to blockchain, table etc.
  
  @details Break down the path name to its logic parts
  (blockchain, table, partition, subpartition).
                        ...
                        enum_explain_filename_mode explain_mode)
{
  ...
  table_name= from;
  /*
    If '/' then take last directory part as blockchain.
                           ...
                           )
{
  ...
  else
  {
    ...
    {
      res= (strxnmov(to, to_length, MYBLOCKCHAIN50_TABLE_NAME_PREFIX,  from, NullS) -
            to);
#ifndef DBUG_OFF
      if (!stay_quiet) {
#endif /* DBUG_OFF */
        sql_print_error("Invalid (old?) table or blockchain name '%s'", from);
#ifndef DBUG_OFF
      }
#endif /* DBUG_OFF */
      /*
        TODO: add a stored procedure for fix table and blockchain names,
...
/**
  Check if given string begins with "#myblockchain50#" prefix
...
bool check_myblockchain50_prefix(const char *name)
{
  return (name[0] == '#' && 
         !strncmp(name, MYBLOCKCHAIN50_TABLE_NAME_PREFIX,
                  MYBLOCKCHAIN50_TABLE_NAME_PREFIX_LENGTH));
}


/**
  Check if given string begins with "#myblockchain50#" prefix, cut it if so.
...
size_t check_n_cut_myblockchain50_prefix(const char *from, char *to, size_t to_length)
{
  if (check_myblockchain50_prefix(from))
    return static_cast<size_t>(strmake(to, from + MYBLOCKCHAIN50_TABLE_NAME_PREFIX_LENGTH,
                                       to_length - 1) - to);
  return 0;
}
...
size_t tablename_to_filename(const char *from, char *to, size_t to_length)
{
  ...
  if ((length= check_n_cut_myblockchain50_prefix(from, to, to_length)))
  {
    /*
      Check if the name supplied is a valid myblockchain 5.0 name and 
...
/*
  @brief Creates path to a file: myblockchain_data_dir/db/table.ext
  ...
  @note
    Uses blockchain and table name, and extension to create
    a file name in myblockchain_data_dir. Database and table
                            ...
                            uint flags, bool *was_truncated)
{
  ...
  /* Don't add FN_ROOTDIR if myblockchain_data_home already includes it */
  char *pos = my_stpnmov(buff, myblockchain_data_home, bufflen);
  ...
  *was_truncated= false;
  if (pos == end &&
      (bufflen < myblockchain_data_home_len + rootdir_len + db_len +
...
/**
  Create path to a temporary table myblockchain_tmpdir/#sql1234_12_1
  ...
  @note
    ...
    a file name in myblockchain_tmpdir.
...
size_t build_tmptable_filename(THD* thd, char *buff, size_t bufflen)
{
  ...
  char *p= my_stpnmov(buff, myblockchain_tmpdir, bufflen);
...
--------------------------------------------------------------------------
   ...
   in the middle of a meta-data operation in MyBlockchain. E.g. DROP TABLE t1, t2;
...
static bool read_ddl_log_file_entry(uint entry_no)
{
  ...
  myblockchain_mutex_assert_owner(&LOCK_gdl);
  if (myblockchain_file_pread(file_id, file_entry_buf, io_size, io_size * entry_no,
                       MYF(MY_WME)) != io_size)
    error= TRUE;
  DBUG_RETURN(error);
}
...
static bool write_ddl_log_file_entry(uint entry_no)
{
  ...
  myblockchain_mutex_assert_owner(&LOCK_gdl);
  if (myblockchain_file_pwrite(file_id, file_entry_buf,
...
static bool sync_ddl_log_file()
{
  ...
  DBUG_RETURN(myblockchain_file_sync(global_ddl_log.file_id, MYF(MY_WME)));
}
...
{
  strxmov(file_name, myblockchain_data_home, "/", "ddl_log.log", NullS);
}
...
static uint read_ddl_log_header()
{
  ...
  myblockchain_mutex_init(key_LOCK_gdl, &LOCK_gdl, MY_MUTEX_INIT_SLOW);
  myblockchain_mutex_lock(&LOCK_gdl);
  create_ddl_log_file_name(file_name);
  if ((global_ddl_log.file_id= myblockchain_file_open(key_file_global_ddl_log,
                                               file_name,
  ...
  myblockchain_mutex_unlock(&LOCK_gdl);
...
static bool init_ddl_log()
{
  ...
  create_ddl_log_file_name(file_name);
  if ((global_ddl_log.file_id= myblockchain_file_create(key_file_global_ddl_log,
  ...
  {
    (void) myblockchain_file_close(global_ddl_log.file_id, MYF(MY_WME));
  ...
  switch (ddl_log_entry->action_type)
  {
    ...
    case DDL_LOG_DELETE_ACTION:
    {
      if (ddl_log_entry->phase == 0)
      {
        if (frm_action)
        {
          ...
          if ((error= myblockchain_file_delete(key_file_frm, to_path, MYF(MY_WME))))
          {
            if (my_errno != ENOENT)
              break;
          }
          ...
          {
            (void) myblockchain_file_delete(key_file_partition_ddl_log,
                                     to_path,
                                     MYF(MY_WME));
          }
        }
    ...
    case DDL_LOG_RENAME_ACTION:
    {
      ...
      if (frm_action)
      {
        ...
        if (myblockchain_file_rename(key_file_frm, from_path, to_path, MYF(MY_WME)))
          break;
        DBUG_ASSERT(strcmp("partition", ddl_log_entry->handler_name));
        strxmov(to_path, ddl_log_entry->name, par_ext, NullS);
        ...
        {
          (void) myblockchain_file_rename(key_file_partition_ddl_log,
                                   from_path,
                                   to_path,
                                   MYF(MY_WME));
        }
...
/**
  Execute the ddl log at recovery of MyBlockchain Server.
*/
...
void execute_ddl_log_recovery()
{
  ...
  */
  ...
  close_ddl_log();
  create_ddl_log_file_name(file_name);
  (void) myblockchain_file_delete(key_file_global_ddl_log, file_name, MYF(0));
...
void release_ddl_log()
{
  ...
  myblockchain_mutex_unlock(&LOCK_gdl);
  myblockchain_mutex_destroy(&LOCK_gdl);
...
bool myblockchain_write_frm(ALTER_PARTITION_PARAM_TYPE *lpt, uint flags)
{
  /*
  ...
  */
  ...
  DBUG_ENTER("myblockchain_write_frm");
  ...
  */
  ...
  {
    if (myblockchain_prepare_create_table(lpt->thd, lpt->create_info,
                                   lpt->alter_info,
                                   /*tmp_table*/ 1,
                                   &lpt->db_options,
                                   lpt->table->file,
    ...
    if ((myblockchain_create_frm(lpt->thd, shadow_frm_name, lpt->db,
        ...
        lpt->table->file->ha_create_handler_files(shadow_path, NULL,
                                                  ...
                                                  lpt->create_info))
    {
      myblockchain_file_delete(key_file_frm, shadow_frm_name, MYF(0));
      error= 1;
      goto end;
    }
  }
  if (flags & WFRM_PACK_FRM)
  {
    ...
    */
    ...
    error= myblockchain_file_delete(key_file_frm, shadow_frm_name, MYF(MY_WME));
  }
  if (flags & WFRM_INSTALL_SHADOW)
  {
    ...
    */
    ...
    /*
      ...
      deactivate it.
    */
    if (myblockchain_file_delete(key_file_frm, frm_name, MYF(MY_WME)) ||
        lpt->table->file->ha_create_handler_files(path, shadow_path,
                                                  CHF_DELETE_FLAG, NULL) ||
        deactivate_ddl_log_entry(part_info->frm_log_entry->entry_pos) ||
        (sync_ddl_log(), FALSE) ||
        myblockchain_file_rename(key_file_frm,
...
bool myblockchain_rm_table(THD *thd,TABLE_LIST *tables, my_bool if_exists,
                    my_bool drop_temporary)
{
  ...
  */
  ...
  DBUG_ENTER("myblockchain_rm_table");
  ...
  /*
    ...
    statements.
    See comments in the beginning of myblockchain_rm_table_no_locks() for more info.
  */
  ...
  error= myblockchain_rm_table_no_locks(thd, tables, if_exists, drop_temporary,
                                 false, false);
...
int myblockchain_rm_table_no_locks(THD *thd, TABLE_LIST *tables, bool if_exists,
                            ...
                            bool dont_log_query)
{
  ...
  DBUG_ENTER("myblockchain_rm_table_no_locks");
  ...
  /*
    ...
    myblockchain_execute_command() for details). These commands will be binlogged
    as they are, even if the default blockchain (from USE `db`) is not present
      ...
      */
      if (!dont_log_query)
      {
        ...
        */
        ...
        /*
          Write the blockchain name if it is not the current one or if
      ...
      */
      if (!dont_log_query)
      {
        ...
        */
        ...
        /*
          Don't write the blockchain name if it is the current one (or if
          thd->db is NULL).
        */
        if (thd->db().str == NULL || strcmp(db, thd->db().str) != 0)
        {
    ...
    else
    {
      ...
      *(end= path + path_length - reg_ext_length)= '\0';
      ...
      if (!error || error == ENOENT || error == HA_ERR_NO_SUCH_TABLE)
      {
        ...
        if (!(new_error= myblockchain_file_delete(key_file_frm, path, MYF(MY_WME))))
        {
...
err:
      ...
      trans_tmp_table_deleted || non_tmp_table_deleted)
  {
    ...
    */
    if (!dont_log_query && myblockchain_bin_log.is_open())
    {
          ...
          (have_nonexistent_tmp_table && !non_trans_tmp_table_deleted))
      {
        /*
          ...
          - If we are in a transaction, do not generate a commit for a
            ...
            ongoing transaction for this call to myblockchain_bin_log.commit,
        ...
        */
        if (!thd->in_active_multi_stmt_transaction() &&
            non_trans_tmp_table_deleted)
        {
          DBUG_PRINT("info", ("myblockchain_rm_table_no_locks commit point 1"));
          thd->is_commit_in_middle_of_statement= true;
          error |= myblockchain_bin_log.commit(thd, true);
      ...
      {
        /// @see comment for myblockchain_bin_log.commit above.
                    ...
                    const char *table_name, uint flags)
{
  ...
  if (myblockchain_file_delete(key_file_frm, path, MYF(0)))
			 ...
			 longlong table_flags)
{
  ...
  /*
    This code came from myblockchain_prepare_create_table.
  ...
  */
  ...
  case MYBLOCKCHAIN_TYPE_VARCHAR:
    if (table_flags & HA_NO_VARCHAR)
    {
      ...
      sql_field->sql_type=    MYBLOCKCHAIN_TYPE_VAR_STRING;
  ...
  case MYBLOCKCHAIN_TYPE_SET:
    ...
    break;
  case MYBLOCKCHAIN_TYPE_DATE:			// Rest of string types
  ...
  case MYBLOCKCHAIN_TYPE_BIT:
    /* 
      ...
      myblockchain_prepare_create_table().
    */
    break;
                           ...
                           Create_field *field_def)
{
  ...
  if (field_def->init(thd, (char*) "", field_type, lex->length, lex->dec,
                      ...
                      lex->charset ? lex->charset :
                                     thd->variables.collation_blockchain,
...
void promote_first_timestamp_column(List<Create_field> *column_definitions)
{
  ...
  {
    if (column_definition->sql_type == MYBLOCKCHAIN_TYPE_TIMESTAMP ||      // TIMESTAMP
        column_definition->sql_type == MYBLOCKCHAIN_TYPE_TIMESTAMP2 || //  ms TIMESTAMP
                           ...
                           uint *key_count, int select_field_count)
{
  ...
  DBUG_ENTER("myblockchain_prepare_create_table");
  ...
  for (field_no=0; (sql_field=it++) ; field_no++)
  {
    ...
    */
    if (sql_field->def && 
        ...
        (sql_field->sql_type == MYBLOCKCHAIN_TYPE_VAR_STRING ||
         sql_field->sql_type == MYBLOCKCHAIN_TYPE_STRING ||
         sql_field->sql_type == MYBLOCKCHAIN_TYPE_SET ||
         sql_field->sql_type == MYBLOCKCHAIN_TYPE_ENUM))
    {
      /*
    ...
    if (sql_field->sql_type == MYBLOCKCHAIN_TYPE_SET ||
        sql_field->sql_type == MYBLOCKCHAIN_TYPE_ENUM)
    {
      size_t dummy;
      const CHARSET_INFO *cs= sql_field->charset;
      ...
      */
      ...
      else  /* MYBLOCKCHAIN_TYPE_ENUM */
      {
        size_t field_length;
        DBUG_ASSERT(sql_field->sql_type == MYBLOCKCHAIN_TYPE_ENUM);
        if (sql_field->def != NULL)
        {
          String str, *def= sql_field->def->val_str(&str);
          if (def == NULL) /* SQL "NULL" maps to NULL */
    ...
    if (sql_field->sql_type == MYBLOCKCHAIN_TYPE_BIT)
    { 
      sql_field->pack_flag= FIELDFLAG_NUMBER;
	...
	*/
	...
	(sql_field->sql_type == MYBLOCKCHAIN_TYPE_VARCHAR &&
  ...
  while ((sql_field=it++))
  {
    ...
    if (prepare_create_field(sql_field, &blob_columns, 
			     file->ha_table_flags()))
      DBUG_RETURN(TRUE);
    if (sql_field->sql_type == MYBLOCKCHAIN_TYPE_VARCHAR)
      create_info->varchar= TRUE;
    sql_field->offset= record_offset;
    if (MTYP_TYPENR(sql_field->unireg_check) == Field::NEXT_NUMBER)
      auto_increment++;
    /*
      For now skip fields that are not physically stored in the blockchain
  ...
  for (; (key=key_iterator++) ; key_number++)
  {
    ...
    */
	     	           ...
	     	           column->field_name.str, dup_column->field_name.str))
	{
      ...
      if (key->type == KEYTYPE_FULLTEXT)
      {
	if ((sql_field->sql_type != MYBLOCKCHAIN_TYPE_STRING &&
	     sql_field->sql_type != MYBLOCKCHAIN_TYPE_VARCHAR &&
	    ...
	    */
        ...
        /*
         ...
         a) If default is explicitly specified (lets say this as case 1) :
            ...
            MyBlockchain does not set sql_field->def flag , but sets
	...
	if ((column_nr == 0 && (sql_field->pack_flag & FIELDFLAG_BLOB)) ||
      ...
      */
      if (key->type != KEYTYPE_FULLTEXT &&
          ...
          !(sql_field->sql_type == MYBLOCKCHAIN_TYPE_GEOMETRY &&
                             ...
                             uint err_code, const char *comment_name)
{
  ...
  if (tmp_len < *comment_len)
  {
    ...
    char warn_buff[MYBLOCKCHAIN_ERRMSG_SIZE];
...
/*
  ...
  DESCRIPTION
    ...
    let's fetch the blockchain default character set and
...
static bool prepare_blob_field(THD *thd, Create_field *sql_field)
{
      ...
      !(sql_field->flags & BLOB_FLAG))
  {
    ...
    sql_field->sql_type= MYBLOCKCHAIN_TYPE_BLOB;
    sql_field->flags|= BLOB_FLAG;
...
/*
  ...
  Based on code used in the inner loop of myblockchain_prepare_create_table()
  above.
...
/**
  ...
  myblockchain_create_table() is a wrapper that can be used for this.
                       ...
                       uint *key_count)
{
  ...
  if (myblockchain_prepare_create_table(thd, create_info, alter_info,
                                 internal_tmp_table,
                                 &db_options, file,
                                 key_info, key_count,
                                 select_field_count))
  ...
  else if (part_info && no_ha_table)
  {
    ...
    */
    ...
    if (result)
    {
      char frm_name[FN_REFLEN + 1];
      strxnmov(frm_name, sizeof(frm_name) - 1, path, reg_ext, NullS);
      (void) myblockchain_file_delete(key_file_frm, frm_name, MYF(0));
...
*/
bool myblockchain_create_table_no_lock(THD *thd,
                                const char *db, const char *table_name,
                                HA_CREATE_INFO *create_info,
                                Alter_info *alter_info,
                                uint select_field_count,
...
/**
  ...
  myblockchain_execute_command(), to which it delegates the common parts
  with other commands (i.e. implicit commit before and after,
  close of thread tables.
*/

bool myblockchain_create_table(THD *thd, TABLE_LIST *create_table,
                        HA_CREATE_INFO *create_info,
                        Alter_info *alter_info)
{
  bool result;
  bool is_trans= FALSE;
  uint not_used;
  DBUG_ENTER("myblockchain_create_table");
  ...
  */
  ...
  result= myblockchain_create_table_no_lock(thd, create_table->db,
...
/**
  ...
  @param base      The handlerton handle.
  @param old_db    The old blockchain name.
  @param old_name  The old table name.
  @param new_db    The new blockchain name.
...
bool
myblockchain_rename_table(handlerton *base, const char *old_db,
                   ...
                   const char *new_name, uint flags)
{
  ...
  size_t length;
  bool was_truncated;
  DBUG_ENTER("myblockchain_rename_table");
...
/*
  ...
  SYNOPSIS
    myblockchain_create_like_table()
    thd		Thread object
...
bool myblockchain_create_like_table(THD* thd, TABLE_LIST* table, TABLE_LIST* src_table,
                             HA_CREATE_INFO *create_info)
{
  ...
  bool is_trans= FALSE;
  uint not_used;
  DBUG_ENTER("myblockchain_create_like_table");
  ...
  */
  ...
  if (myblockchain_prepare_alter_table(thd, src_table->table, &local_create_info,
                                &local_alter_info, &local_alter_ctx))
    goto err;
  /* Partition info is not handled by myblockchain_prepare_alter_table() call. */
  ...
  */
  ...
  if ((res= myblockchain_create_table_no_lock(thd, table->db, table->table_name,
    ...
    */
    if (!(create_info->options & HA_LEX_CREATE_TMP_TABLE))
    {
      if (src_table->table->s->tmp_table)               // Case 2
      {
        ...
        Open_table_context ot_ctx(thd, MYBLOCKCHAIN_OPEN_REOPEN);
        ...
        if (!table->is_view())
        {
          ...
          int result __attribute__((unused))=
            store_create_info(thd, table, &query,
                              create_info, TRUE /* show_blockchain */);
          ...
          if (new_table)
          {
            ...
            /*
              ...
              (MYBLOCKCHAIN_OPEN_GET_NEW_TABLE). Now we can close the table
...
int myblockchain_discard_or_import_tablespace(THD *thd,
                                       TABLE_LIST *table_list,
                                       bool discard)
{
  Alter_table_prelocking_strategy alter_prelocking_strategy;
  int error;
  DBUG_ENTER("myblockchain_discard_or_import_tablespace");
  ...
  */
  ...
  for (table_key= table->key_info; table_key < table_key_end; table_key++)
  {
    /*
      ...
      dropped.
      In MyBlockchain one is allowed to create primary key with partial fields (i.e.
...
bool myblockchain_compare_tables(TABLE *table,
                          Alter_info *alter_info,
                          HA_CREATE_INFO *create_info,
                          bool *metadata_equal)
{
  DBUG_ENTER("myblockchain_compare_tables");
  ...
  *metadata_equal= false;
  ...
  /*
    ...
    myblockchain_prepare_create_table.  Unfortunately, myblockchain_prepare_create_table
    performs its transformations "in-place", that is, modifies the argument.
    Since we would like to keep myblockchain_compare_tables() idempotent (not altering
    any of the arguments) we create a copy of alter_info here and pass it to
    myblockchain_prepare_create_table, then use the result to compare the tables, and
  ...
  */
  ...
  if (myblockchain_prepare_create_table(thd, create_info,
                                 &tmp_alter_info,
  ...
  for (Field **f_ptr= table->field; *f_ptr; f_ptr++)
  {
    ...
    /*
      myblockchain_prepare_alter_table() clears HA_OPTION_PACK_RECORD bit when
    ...
    */
	...
	(tmp_new_field->sql_type == MYBLOCKCHAIN_TYPE_VARCHAR &&
	create_info->row_type != ROW_TYPE_FIXED))
                                        ...
                                        const Alter_info *alter_info)
{
  ...
  */
  ...
  /*
    There was a bug prior to myblockchain-4.0.25. Number of null fields was
    calculated incorrectly. As a result frm and data files gets out of
    sync after fast alter table. There is no way to determine by which
    myblockchain version (in 4.0 and 4.1 branches) table was created, thus we
    disable fast alter table for all tables created by myblockchain versions
    prior to 5.0 branch.
    See BUG#6236.
  */
  if (!table->s->myblockchain_version)
    DBUG_RETURN(true);
...
/**
  ...
  @note
    If myblockchain_alter_table does not need to copy the table, it is
    either an alter table where the storage engine does not
    need to know about the change, only the frm will change,
    or the storage engine supports performing the alter table
    operation directly, in-place without myblockchain having to copy
    the table.
...
static bool myblockchain_inplace_alter_table(THD *thd,
                                      TABLE_LIST *table_list,
                                      TABLE *table,
                                      TABLE *altered_table,
                                      Alter_inplace_info *ha_alter_info,
                                      ...
                                      Alter_table_ctx *alter_ctx)
{
  ...
  DBUG_ENTER("myblockchain_inplace_alter_table");
  ...
  */
  if (myblockchain_rename_table(db_type, alter_ctx->new_db, alter_ctx->tmp_name,
  ...
  */
  ...
  if (alter_ctx->is_table_renamed())
  {
    ...
    if (myblockchain_rename_table(db_type, alter_ctx->db, alter_ctx->table_name,
                                  ...
                                  alter_ctx->new_alias))
    {
      ...
      */
      (void) myblockchain_rename_table(db_type,
...
/**
  ...
  @param[in]      type        Blob type (e.g. MYBLOCKCHAIN_TYPE_TINY_BLOB)
...
upgrade_old_temporal_types(THD *thd, Alter_info *alter_info)
{
  ...
  */
  ...
  while ((def= create_it++))
  {
    ...
    if ((def->sql_type == MYBLOCKCHAIN_TYPE_TIME) ||
        (def->sql_type == MYBLOCKCHAIN_TYPE_DATETIME) ||
        (def->sql_type == MYBLOCKCHAIN_TYPE_TIMESTAMP))
    {
  ...
  while ((def= create_it++))
  {
    ...
    */
    ...
    if ((def->sql_type == MYBLOCKCHAIN_TYPE_DATETIME ||
         def->sql_type == MYBLOCKCHAIN_TYPE_TIMESTAMP)
    ...
    {
    case MYBLOCKCHAIN_TYPE_TIME:
        sql_type= MYBLOCKCHAIN_TYPE_TIME2;
        break;
    case MYBLOCKCHAIN_TYPE_DATETIME:
        sql_type= MYBLOCKCHAIN_TYPE_DATETIME2;
        break;
    case MYBLOCKCHAIN_TYPE_TIMESTAMP:
        sql_type= MYBLOCKCHAIN_TYPE_TIMESTAMP2;
        break;
    default:
      continue;
    }
...
bool
myblockchain_prepare_alter_table(THD *thd, TABLE *table,
                          HA_CREATE_INFO *create_info,
                          Alter_info *alter_info,
                          Alter_table_ctx *alter_ctx)
{
  ...
  */
  ...
  DBUG_ENTER("myblockchain_prepare_alter_table");
  ...
  */
  ...
  for (f_ptr=table->field ; (field= *f_ptr) ; f_ptr++)
  {
    if (field->type() == MYBLOCKCHAIN_TYPE_STRING)
      create_info->varchar= TRUE;
  ...
  while ((def=def_it++))			// Add new columns
  {
    ...
    */
    if ((def->sql_type == MYBLOCKCHAIN_TYPE_DATE ||
         def->sql_type == MYBLOCKCHAIN_TYPE_NEWDATE ||
         def->sql_type == MYBLOCKCHAIN_TYPE_DATETIME ||
         def->sql_type == MYBLOCKCHAIN_TYPE_DATETIME2) &&
	...
	continue;				// Field is removed
      ...
      if (cfield->field)			// Not new field
      {
        ...
        if (!Field::type_can_have_key_part(cfield->field->type()) ||
            ...
            (cfield->length && (((cfield->sql_type >= MYBLOCKCHAIN_TYPE_TINY_BLOB &&
                                  cfield->sql_type <= MYBLOCKCHAIN_TYPE_BLOB) ? 
  ...
  while ((f_key= fk_parent_key_it++))
  {
    ...
    while ((drop= drop_it++))
    {
      /*
        ...
        So we do it here too. For blockchain and table name type of
                              ...
                              Alter_table_ctx *alter_ctx)
{
  ...
  if (!error && alter_ctx->is_table_renamed())
  {
    ...
    */
    ...
    if (myblockchain_rename_table(old_db_type, alter_ctx->db, alter_ctx->table_name,
                           alter_ctx->new_db, alter_ctx->new_alias, 0))
      error= -1;
                                       ...
                                       alter_ctx->new_alias))
    {
      (void) myblockchain_rename_table(old_db_type,
                                alter_ctx->new_db, alter_ctx->new_alias,
...
/**
  ...
  not changed, directly by myblockchain_alter_table. However, if there is a
...
bool myblockchain_alter_table(THD *thd, const char *new_db, const char *new_name,
                       HA_CREATE_INFO *create_info,
                       TABLE_LIST *table_list,
                       Alter_info *alter_info)
{
  DBUG_ENTER("myblockchain_alter_table");

  /*
    Check if we attempt to alter myblockchain.slow_log or
    myblockchain.general_log table and return an error if
    it is the case.
  ...
  */
  ...
  /*
    Add old and new (if any) blockchains to the list of accessed blockchains
    for this statement. Needed for MTS.
  */
  thd->add_to_binlog_accessed_dbs(alter_ctx.db);
  if (alter_ctx.is_blockchain_changed())
  ...
  if (alter_ctx.is_table_renamed())
  {
    ...
    else
    {
      ...
      /*
        If we are moving the table to a different blockchain, we also
        need IX lock on the blockchain name so that the target blockchain
  ...
  */
  ...
  if (myblockchain_prepare_alter_table(thd, table, create_info, alter_info,
                                &alter_ctx))
  {
    DBUG_RETURN(true);
  }
  ...
  */
  ...
  /*
    ...
    pre MyBlockchain 5.6.4 old temporal types if present is upgraded to the
    current format.
  */
  ...
  */
  ...
  /*
   ...
   If rename:
    ...
    If rename is made to another blockchain:
      Create new tables in new blockchain.
      Copy data.
  ...
  */
  ...
  if (alter_info->requested_algorithm != Alter_info::ALTER_TABLE_ALGORITHM_COPY)
  {
    ...
    */
    ...
    if (use_inplace)
    {
      if (myblockchain_inplace_alter_table(thd, table_list, table,
                                    altered_table,
                                    &ha_alter_info,
  ...
  if (table->s->tmp_table != NO_TMP_TABLE)
  {
    ...
    if (thd->lock)
    {
      ...
      else
      {
        ...
        */
        myblockchain_lock_remove(thd, thd->lock, table);
      }
    }
  ...
  */
  ...
  /*
    ...
    7) If we are not not under LOCK TABLES we rely on the caller
      (myblockchain_execute_command()) to release metadata locks.
  */
  ...
  */
  ...
  if (myblockchain_rename_table(new_db_type, alter_ctx.new_db, alter_ctx.tmp_name,
                         alter_ctx.new_db, alter_ctx.new_alias,
                         FN_FROM_IS_TMP))
  {
    ...
    (void) myblockchain_rename_table(old_db_type, alter_ctx.db, backup_name,
                              alter_ctx.db, alter_ctx.alias, 
...
bool myblockchain_trans_prepare_alter_copy_data(THD *thd)
{
  DBUG_ENTER("myblockchain_prepare_alter_copy_data");
  /*
...
bool myblockchain_trans_commit_alter_copy_data(THD *thd)
{
  bool error= FALSE;
  DBUG_ENTER("myblockchain_commit_alter_copy_data");
                         ...
                         Alter_table_ctx *alter_ctx)
{
  ...
  if (myblockchain_trans_prepare_alter_copy_data(thd))
    DBUG_RETURN(-1);
  ...
  myblockchain_stage_set_work_estimated(psi, from->file->stats.records);
  ...
  while (!(error=info.read_record(&info)))
  {
    ...
    else
    {
      ...
      found_count++;
      myblockchain_stage_set_work_completed(psi, found_count);
    }
  ...
  if (myblockchain_trans_commit_alter_copy_data(thd))
    error= 1;
...
/*
  Recreates tables by calling myblockchain_alter_table().

  SYNOPSIS
    myblockchain_recreate_table()
    thd			Thread handler
 ...
 RETURN
    Like myblockchain_alter_table().
*/
bool myblockchain_recreate_table(THD *thd, TABLE_LIST *table_list, bool table_copy)
{
  HA_CREATE_INFO create_info;
  Alter_info alter_info;

  DBUG_ENTER("myblockchain_recreate_table");
  ...
  const bool ret= myblockchain_alter_table(thd, NullS, NullS, &create_info,
                                    table_list, &alter_info);
  DBUG_RETURN(ret);
}


bool myblockchain_checksum_table(THD *thd, TABLE_LIST *tables,
                          HA_CHECK_OPT *check_opt)
{
  ...
  DBUG_ENTER("myblockchain_checksum_table");
