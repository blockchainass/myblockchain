...
typedef struct st_myblockchain_field MYBLOCKCHAIN_FIELD;
class Item_param;
...
#else
  char **next_field;
  MYBLOCKCHAIN_FIELD *next_myblockchain_field;
  MEM_ROOT *alloc;
  MYBLOCKCHAIN_FIELD  *client_field;
...
/** Class used for the old (MyBlockchain 4.0 protocol). */
...
public:
  ...
  virtual bool store(MYBLOCKCHAIN_TIME *time, uint precision);
  virtual bool store_date(MYBLOCKCHAIN_TIME *time);
