...
/*
   ...
   Shared libraries are not closed before myblockchaind exits;
...
static myblockchain_rwlock_t THR_LOCK_udf;
...
static void init_udf_psi_keys(void)
{
  ...
  myblockchain_rwlock_register(category, all_udf_rwlocks, count);

  count= array_elements(all_udf_memory);
  myblockchain_memory_register(category, all_udf_memory, count);
}
#endif

/*
  Read all predeclared functions from myblockchain.func and accept all that
  can be used.
*/
...
void udf_init()
{
  ...
  int error;
  DBUG_ENTER("ufd_init");
  char db[]= "myblockchain"; /* A subject to casednstr, can't be constant */
  ...
  myblockchain_rwlock_init(key_rwlock_THR_LOCK_udf, &THR_LOCK_udf);
  ...
  if (open_and_lock_tables(new_thd, &tables, MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT))
  {
    DBUG_PRINT("error",("Can't open udf table"));
    sql_print_error("Can't open the myblockchain.func table. Please "
  ...
  while (!(error= read_record_info.read_record(&read_record_info)))
  {
    ...
    */
    ...
    {
      sql_print_error("Invalid row in myblockchain.func table for function '%.64s'",
                      name.str);
      continue;
    }
  ...
  close_myblockchain_tables(new_thd);
  delete new_thd;
  DBUG_VOID_RETURN;
}
...
void udf_free()
{
  ...
  {
    initialized= 0;
    myblockchain_rwlock_destroy(&THR_LOCK_udf);
  }
  DBUG_VOID_RETURN;
}
...
void free_udf(udf_func *udf)
{
  ...
  myblockchain_rwlock_wrlock(&THR_LOCK_udf);
  if (!--udf->usage_count)
  {
    /*
  ...
  myblockchain_rwlock_unlock(&THR_LOCK_udf);
  DBUG_VOID_RETURN;
}
...
udf_func *find_udf(const char *name, size_t length,bool mark_used)
{
  ...
  if (mark_used)
    myblockchain_rwlock_wrlock(&THR_LOCK_udf);  /* Called during fix_fields */
  else
    myblockchain_rwlock_rdlock(&THR_LOCK_udf);  /* Called during parsing */
...
int myblockchain_create_function(THD *thd,udf_func *udf)
{
  int error;
  void *dl=0;
  bool new_dl=0;
  ...
  DBUG_ENTER("myblockchain_create_function");
  ...
  */
  ...
  tables.init_one_table("myblockchain", 5, "func", 4, "func", TL_WRITE);
  ...
  */
  ...
  /* create entry in myblockchain.func table */
  ...
  if (error)
  {
    ...
    my_error(ER_ERROR_ON_WRITE, MYF(0), "myblockchain.func", error,
...
int myblockchain_drop_function(THD *thd,const LEX_STRING *udf_name)
{
  TABLE *table;
  TABLE_LIST tables;
  udf_func *udf;
  ...
  bool save_binlog_row_based;
  int error= 1;
  DBUG_ENTER("myblockchain_drop_function");
