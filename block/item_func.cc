...
void Item_func::count_datetime_length(Item **item, uint nitems)
{
  unsigned_flag= 0;
  decimals= 0;
  if (field_type() != MYBLOCKCHAIN_TYPE_DATE)
  {
    for (uint i= 0; i < nitems; i++)
      set_if_bigger(decimals,
                    field_type() == MYBLOCKCHAIN_TYPE_TIME ?
...
void reject_geometry_args(uint arg_count, Item **args, Item_result_field *me)
{
  /*
    ...
    When a variable holds a MyBlockchain Geometry byte string, it is regarded as a
    string rather than a MYBLOCKCHAIN_TYPE_GEOMETRY, so here we can't catch an illegal
  ...
  */
  for (uint i= 0; i < arg_count; i++)
  {
    if (args[i]->result_type() != ROW_RESULT &&
        args[i]->field_type() == MYBLOCKCHAIN_TYPE_GEOMETRY)
    {
...
bool Item_func_numhybrid::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed == 1);
  switch (field_type())
  {
...
bool Item_func_numhybrid::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  switch (field_type())
  {
...
void Item_func_min_max::fix_length_and_dec()
{
  ...
  for (uint i=0 ; i < arg_count ; i++)
  {
    ...
    {
      datetime_found= TRUE;
      if (!datetime_item || args[i]->field_type() == MYBLOCKCHAIN_TYPE_DATETIME)
...
bool Item_func_min_max::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed == 1);
  if (compare_as_dates)
  {
...
bool Item_func_min_max::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  if (compare_as_dates)
  {
...
myblockchain> select least('11', '2'), least('11', '2')+0, concat(least(11,2));
+------------------+--------------------+---------------------+
...
void Item_func_find_in_set::fix_length_and_dec()
{
  ...
  if (args[0]->const_item() && args[1]->type() == FIELD_ITEM)
  {
    ...
    if (field->real_type() == MYBLOCKCHAIN_TYPE_SET)
    {
			...
			uint arg_count, Item **arguments)
{
  ...
  if (u_d->func_init)
  {
    char init_msg_buff[MYBLOCKCHAIN_ERRMSG_SIZE];
    char *to=num_buffer;
    for (uint i=0; i < arg_count; i++)
    {
      /*
...
class Interruptible_wait
{
  ...
  public:
    ...
    */
    ...
    /** The timed wait. */
    int wait(myblockchain_cond_t *, myblockchain_mutex_t *);
};
...
/**
  ...
  @retval return value from myblockchain_cond_timedwait
*/

int Interruptible_wait::wait(myblockchain_cond_t *cond, myblockchain_mutex_t *mutex)
{
  int error;
  ...
  while (1)
  {
    ...
    error= myblockchain_cond_timedwait(cond, mutex, &timeout);
...
{
  User_level_lock *ull;
  DBUG_ENTER("myblockchain_ull_cleanup");
...
{
  User_level_lock *ull;
  DBUG_ENTER("myblockchain_ull_set_explicit_lock_duration");
...
longlong Item_func_last_insert_id::val_int()
{
  ...
  if (arg_count)
  {
    ...
    null_value= args[0]->null_value;
    /*
      LAST_INSERT_ID(X) must affect the client's myblockchain_insert_id() as
...
myblockchain_mutex_t LOCK_item_func_sleep;
...
static void init_item_func_sleep_psi_keys()
{
  ...
  myblockchain_mutex_register("sql", item_func_sleep_mutexes, count);
}
#endif
...
void item_func_sleep_init()
{
  ...
  myblockchain_mutex_init(key_LOCK_item_func_sleep, &LOCK_item_func_sleep, MY_MUTEX_INIT_SLOW);
...
void item_func_sleep_free()
{
  if (item_func_sleep_inited)
  {
    ...
    myblockchain_mutex_destroy(&LOCK_item_func_sleep);
  }
}
...
longlong Item_func_sleep::val_int()
{
  ...
  */
  ...
  /*
    On 64-bit OSX myblockchain_cond_timedwait() waits forever
    if passed abstime time has already been exceeded by 
    the system time.
    When given a very short timeout (< 10 mcs) just return 
    immediately.
    We assume that the lines between this test and the call 
    to myblockchain_cond_timedwait() will be executed in less than 0.00001 sec.
  */
  if (timeout < 0.00001)
    return 0;
  ...
  myblockchain_cond_init(key_item_func_sleep_cond, &cond);
  myblockchain_mutex_lock(&LOCK_item_func_sleep);
  ...
  thd_wait_end(thd);
  myblockchain_mutex_unlock(&LOCK_item_func_sleep);
  thd->EXIT_COND(NULL);
...
void user_var_entry::lock()
{
  ...
  myblockchain_mutex_lock(&m_owner->LOCK_thd_data);
}

void user_var_entry::unlock()
{
  DBUG_ASSERT(m_owner != NULL);
  myblockchain_mutex_unlock(&m_owner->LOCK_thd_data);
}
    ...
    */
    ...
    if (var_entry == NULL)
      goto err;
           ...
           myblockchain_bin_log.is_query_in_union(thd, var_entry->used_query_id))
  {
    /* 
...
bool Item_user_var_as_out_param::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  const CHARSET_INFO *cs= thd->lex->exchange->cs ?
    thd->lex->exchange->cs : thd->variables.collation_blockchain;
...
enum_field_types Item_func_get_system_var::field_type() const
{
  switch (var->show_type())
  {
    ...
    default:
      my_error(ER_VAR_CANT_BE_READ, MYF(0), var->name.str);
      return MYBLOCKCHAIN_TYPE_VARCHAR;              // keep the compiler happy
  }
}
...
bool Item_func_sp::itemize(Parse_context *pc, Item **res)
{
  ...
  if (m_name->m_db.str == NULL) // use the default blockchain name
  {
    /* Cannot match the function since no blockchain is selected */
    if (thd->db().str == NULL)
    {
...
{
  if (sp_result_field->type() == MYBLOCKCHAIN_TYPE_JSON)
  {
    if (execute())
    {
      return true;
