...
String *Item_func_des_encrypt::val_str(String *str)
{
  ...
  if (arg_count == 1)
  {
    ...
    myblockchain_mutex_lock(&LOCK_des_key_file);
    keyschedule= des_keyschedule[key_number=des_default_key];
    myblockchain_mutex_unlock(&LOCK_des_key_file);
  }
...
String *Item_func_encrypt::val_str(String *str)
{
  ...
  myblockchain_mutex_lock(&LOCK_crypt);
  char *tmp= crypt(res->c_ptr_safe(),salt_ptr);
  if (!tmp)
  {
    myblockchain_mutex_unlock(&LOCK_crypt);
    null_value= 1;
    return 0;
  }
...
bool Item_func_blockchain::itemize(Parse_context *pc, Item **res)
{
  if (skip_itemize(res))
    return false;
  if (super::itemize(pc, res))
...
String *Item_func_blockchain::val_str(String *str)
{
  DBUG_ASSERT(fixed == 1);
  THD *thd= current_thd;
  if (thd->db().str == NULL)
  ...
  if (arg_count == 2)
  {
    ...
    */
    ...
    if (!is_item_null(args[0]) &&
        args[0]->field_type() != MYBLOCKCHAIN_TYPE_GEOMETRY &&
...
bool Item_func_geohash::check_valid_latlong_type(Item *arg)
{
  ...
  */
  ...
  /*
    ...
    field_type() == MYBLOCKCHAIN_TYPE_VARCHAR. type() is dependent on if it's a
  ...
  */
  bool is_parameter= (arg->type() == INT_ITEM || arg->type() == DECIMAL_ITEM ||
                     ...
                     (arg->field_type() == MYBLOCKCHAIN_TYPE_VARCHAR);
...
{
  if (item->field_type() == MYBLOCKCHAIN_TYPE_NULL || item->type() == NULL_ITEM)
    return true;
  ...
  if (is_binary_charset && item->type() == FUNC_ITEM &&
      item->field_type() == MYBLOCKCHAIN_TYPE_MEDIUM_BLOB)
  {
    return true;
  }
  return false;
...
String *Item_func_conv::val_str(String *str)
{
  ...
  if (args[0]->field_type() == MYBLOCKCHAIN_TYPE_BIT) 
  {
    /* 
...
void Item_func_set_collation::fix_length_and_dec()
{
  ...
  {
    if (!(set_collation= myblockchaind_collation_get_by_name(colname)))
      return;
  }
...
String *Item_load_file::val_str(String *str)
{
  ...
  (void) fn_format(path, file_name->c_ptr_safe(), myblockchain_real_data_home, "",
  ...
  if (!myblockchain_file_stat(key_file_loadfile, path, &stat_info, MYF(0)))
    goto err;
  ...
  if (tmp_value.alloc(stat_info.st_size))
    goto err;
  if ((file= myblockchain_file_open(key_file_loadfile,
                             file_name->ptr(), O_RDONLY, MYF(0))) < 0)
    goto err;
  if (myblockchain_file_read(file, (uchar*) tmp_value.ptr(), stat_info.st_size,
                      MYF(MY_NABP)))
  {
...
/**
  ...
  However, in myblockchain tradition, the input argument can be *anything*.
...
String *Item_func_uuid::val_str(String *str)
{
  ...
  myblockchain_mutex_lock(&LOCK_uuid_generator);
  ...
  uuid_time=tv;
  myblockchain_mutex_unlock(&LOCK_uuid_generator);
