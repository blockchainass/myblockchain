              ...
              ha_rows *returned_rows)
{
  ...
  */
  ...
  MYBLOCKCHAIN_FILESORT_START(const_cast<char*>(table->s->db.str),
  ...
  */
  ...
  if (open_cached_file(&chunk_file,myblockchain_tmpdir,TEMP_PREFIX,
  ...
  else
  {
	...
	open_cached_file(outfile,myblockchain_tmpdir,TEMP_PREFIX,READ_RECORD_BUFFER,
			  MYF(MY_WME)))
      goto err;
  ...
  *returned_rows= num_rows;
  ...
  MYBLOCKCHAIN_FILESORT_DONE(error, num_rows);
  DBUG_RETURN(error);
} /* filesort */
           ...
           IO_CACHE *chunk_file, IO_CACHE *tempfile)
{
  ...
  if (!my_b_inited(tempfile) &&
      open_cached_file(tempfile, myblockchain_tmpdir, TEMP_PREFIX, DISK_BUFFER_SIZE,
                       MYF(MY_WME)))
       ...
       sort_field++)
  {
	...
	if (field->is_null())
	{
	  ...
	  *to++=1;
      ...
      if (sort_field->field_type == MYBLOCKCHAIN_TYPE_JSON)
      {
    ...
    {						// Item
      ...
      case INT_RESULT:
	{
          longlong value= item->field_type() == MYBLOCKCHAIN_TYPE_TIME ?
                    ...
                    size_t *p_num_chunks, IO_CACHE *t_file)
{
  ...
  *p_num_chunks= num_chunks;
  ...
  if (flush_io_cache(t_file) ||
      open_cached_file(&t_file2,myblockchain_tmpdir,TEMP_PREFIX,DISK_BUFFER_SIZE,
                       MYF(MY_WME)))
    DBUG_RETURN(1);				/* purecov: inspected */
                    ...
                    Sort_param *param)
{
  ...
  if ((count= min(merge_chunk->max_keys(), merge_chunk->rowcount())))
  {
    ...
    if (myblockchain_file_pread(fromfile->file,
  ...
  *use_hash= false;
  ...
  for (; s_length-- ; sortorder++)
  {
    ...
    if (sortorder->field)
    {
      ...
      if (sortorder->field_type == MYBLOCKCHAIN_TYPE_JSON)
        *use_hash= true;
    }
    else
    {
                           ...
                           uint *ppackable_length)
{
  ...
  *plength= *ppackable_length= 0;
  ...
  for (pfield= ptabfield; (field= *pfield) ; pfield++)
  {
    ...
    if (field->maybe_null() ||
        field_type == MYBLOCKCHAIN_TYPE_STRING ||
        field_type == MYBLOCKCHAIN_TYPE_VARCHAR ||
        field_type == MYBLOCKCHAIN_TYPE_VAR_STRING)
