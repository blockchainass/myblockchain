...
frm_type_enum dd_frm_type(THD *thd, char *path, enum legacy_db_type *dbt)
{
  ...
  *dbt= DB_TYPE_UNKNOWN;
  ...
  if ((file= myblockchain_file_open(key_file_frm, path, O_RDONLY | O_SHARE, MYF(0))) < 0)
    DBUG_RETURN(FRMTYPE_ERROR);
  error= myblockchain_file_read(file, (uchar*) header, sizeof(header), MYF(MY_NABP));
...
/**
  ...
  @param[in]   table_name   Table blockchain.
...
/*
  ...
  @param  db    Name of the blockchain to which the table belongs to.
