...
/**
  Daemonize myblockchaind.

  This function does sysv style of daemonization of myblockchaind.

  @return - returns write end of the pipe file descriptor
            which is used to notify the parent to exit.
*/
int myblockchaind::runtime::myblockchaind_daemonize()
{
  int pipe_fd[2];
  if (pipe(pipe_fd) < 0)
    return -1;
  ...
  if (pid != 0)
  {
    ...
    {
      fprintf(stderr, "Initialization of myblockchaind failed: %d\n", waitstatus);
      exit(MYBLOCKCHAIND_ABORT_EXIT);
    }
    _exit(MYBLOCKCHAIND_SUCCESS_EXIT);
  }
  else
  {
...
*/
void myblockchaind::runtime::signal_parent(int pipe_write_fd, char status)
{
  if (pipe_write_fd != -1)
  {
