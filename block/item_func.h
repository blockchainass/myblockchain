...
public:
  ...
  */
  ...
  bool get_arg0_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzy_date)
  {
    return (null_value=args[0]->get_date(ltime, fuzzy_date));
  }
  inline bool get_arg0_time(MYBLOCKCHAIN_TIME *ltime)
  {
  ...
  */
  ...
  bool has_timestamp_args()
  {
    ...
    for (uint i= 0; i < arg_count; i++)
    {
      if (args[i]->type() == Item::FIELD_ITEM &&
          args[i]->field_type() == MYBLOCKCHAIN_TYPE_TIMESTAMP)
        return TRUE;
    }
    return FALSE;
  }
  ...
  bool has_date_args()
  {
    ...
    {
      if (args[i]->type() == Item::FIELD_ITEM &&
          (args[i]->field_type() == MYBLOCKCHAIN_TYPE_DATE ||
           args[i]->field_type() == MYBLOCKCHAIN_TYPE_DATETIME))
        return TRUE;
    }
    return FALSE;
  }
  ...
  bool has_time_args()
  {
    ...
    {
      if (args[i]->type() == Item::FIELD_ITEM &&
          (args[i]->field_type() == MYBLOCKCHAIN_TYPE_TIME ||
           args[i]->field_type() == MYBLOCKCHAIN_TYPE_DATETIME))
        return TRUE;
    }
    return FALSE;
...
public:
  ...
  */
  ...
  /**
     @brief Performs the operation that this functions implements when the
     result type is MYBLOCKCHAIN_TYPE_DATE or MYBLOCKCHAIN_TYPE_DATETIME.

     @return The result of the operation.
  */
  virtual bool date_op(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)= 0;
  virtual bool time_op(MYBLOCKCHAIN_TIME *ltime)= 0;
...
public:
  ...
  bool date_op(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
  { DBUG_ASSERT(0); return 0; }
  bool time_op(MYBLOCKCHAIN_TIME *ltime)
  { DBUG_ASSERT(0); return 0; }
};
...
public:
  ...
  enum Item_result result_type () const
  {
    /*
      ...
      - field_type is MYBLOCKCHAIN_TYPE_VARSTRING, MYBLOCKCHAIN_TYPE_DATETIME
        or MYBLOCKCHAIN_TYPE_DATE.
...
void myblockchain_ull_cleanup(THD *thd);
void myblockchain_ull_set_explicit_lock_duration(THD *thd);
