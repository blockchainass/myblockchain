...
String *Item_func_geometry_from_wkb::val_str(String *str)
{
  ...
  /*
    ...
    MyBlockchain standard (little) endian. Note that users can pass via client
    any WKB/Geometry byte string, including those of big endianess.
  */
  if (args[0]->field_type() == MYBLOCKCHAIN_TYPE_GEOMETRY)
  {
      ...
      *geometry= collection;
           ...
           itr != data_array->End(); ++itr)
      {
        ...
        else
        {
          ...
          {
            // Make the Gis_polygon suitable for MyBlockchain GIS code.
...
String *Item_func_point::val_str(String *str)
{
  ...
  */
  if ((null_value= (args[0]->field_type() == MYBLOCKCHAIN_TYPE_GEOMETRY ||
                    args[1]->field_type() == MYBLOCKCHAIN_TYPE_GEOMETRY)))
  {
    my_error(ER_WRONG_ARGUMENTS, MYF(0), func_name());
    return error_str();
  }
