...
/* Function items used by myblockchain */
...
protected:
  ...
  /**
    Get "native" temporal value as MYBLOCKCHAIN_TIME
    @param[out] ltime       The value is stored here.
    @param[in]  fuzzy_date  Date flags.
    @retval     false       On success.
    @retval     true        On error.
  */
  virtual bool val_datetime(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzy_date)= 0;
...
public:
  ...
  const CHARSET_INFO *charset_for_protocol() const
  {
    ...
    */
    DBUG_ASSERT(fixed == 1);
    return cached_field_type == MYBLOCKCHAIN_TYPE_STRING ?
...
public:
  ...
  { }
  enum_field_types field_type() const { return MYBLOCKCHAIN_TYPE_DATE; }
  bool get_time(MYBLOCKCHAIN_TIME *ltime)
  {
  ...
  virtual bool get_date(MYBLOCKCHAIN_TIME *res, my_time_flags_t fuzzy_date)= 0;
};
...
public:
  ...
  enum_field_types field_type() const { return MYBLOCKCHAIN_TYPE_DATETIME; }
  double val_real() { return val_real_from_decimal(); }
  String *val_str(String *str)
  {
...
public:
  ...
  Item_time_func(const POS &pos, Item *a, Item *b, Item *c)
    :Item_temporal_func(pos, a, b ,c)
  {}
  enum_field_types field_type() const { return MYBLOCKCHAIN_TYPE_TIME; }
  double val_real() { return val_real_from_decimal(); }
  my_decimal *val_decimal(my_decimal *decimal_value)
  {
    DBUG_ASSERT(fixed == 1);
  ...
  bool get_date(MYBLOCKCHAIN_TIME *res, my_time_flags_t fuzzy_date)
  {
  ...
  virtual bool get_time(MYBLOCKCHAIN_TIME *res)= 0;
};


/**
  Cache for MYBLOCKCHAIN_TIME value with various representations.
  
  - MYBLOCKCHAIN_TIME representation (time) is initialized during set_XXX().
...
*/
class MYBLOCKCHAIN_TIME_cache
{
  MYBLOCKCHAIN_TIME time;                              ///< MYBLOCKCHAIN_TIME representation
  ...
  /**
    Cache string representation from the cached MYBLOCKCHAIN_TIME representation.
  ...
  */
  void reset()
  {
    time.time_type= MYBLOCKCHAIN_TIMESTAMP_NONE;
    time_packed= 0;
    reset_string();
    dec= 0;
  }
  /**
    Store MYBLOCKCHAIN_TIME representation into the given MYBLOCKCHAIN_TIME variable.
  */
  void get_TIME(MYBLOCKCHAIN_TIME *ltime) const
  {
...
public:
  ...
  */
  void set_date(MYBLOCKCHAIN_TIME *ltime);
  /**
    Set time and time_packed from a TIME value.
  */
  void set_time(MYBLOCKCHAIN_TIME *ltime, uint8 dec_arg);
  /**
    Set time and time_packed from a DATETIME value.
  */
  void set_datetime(MYBLOCKCHAIN_TIME *ltime, uint8 dec_arg);
  /**
  ...
  */
  ...
  /**
    Test if cached value is equal to another MYBLOCKCHAIN_TIME_cache value.
  */
  bool eq(const MYBLOCKCHAIN_TIME_cache &tm) const
  {
  ...
  */
  ...
  /**
    Store MYBLOCKCHAIN_TIME representation into the given date/datetime variable
    checking date flags.
  */
  bool get_date(MYBLOCKCHAIN_TIME *ltime, uint fuzzyflags) const;
  /**
    Store MYBLOCKCHAIN_TIME representation into the given time variable.
  */
  bool get_time(MYBLOCKCHAIN_TIME *ltime) const
  {
    get_TIME(ltime);
    return false;
  }
  /**
    Return pointer to MYBLOCKCHAIN_TIME representation.
  */
  MYBLOCKCHAIN_TIME *get_TIME_ptr()
  {
    DBUG_ASSERT(time.time_type != MYBLOCKCHAIN_TIMESTAMP_NONE);
    return &time;
  }
...
{
  MYBLOCKCHAIN_TIME_cache cached_time;
public:
  /**
    Constructor for Item_date_literal.
    @param ltime  DATE value.
  */
  Item_date_literal(MYBLOCKCHAIN_TIME *ltime)
  {
  ...
  bool get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzy_date)
  {
    DBUG_ASSERT(fixed);
...
public:
  ...
  */
  Item_time_literal(MYBLOCKCHAIN_TIME *ltime, uint dec_arg)
  {
...
public:
  /**
    ...
    @param dec_arg  number of fractional digits in ltime.
  */
  Item_datetime_literal(MYBLOCKCHAIN_TIME *ltime, uint dec_arg)
  {
    decimals= MY_MIN(dec_arg, DATETIME_MAX_DECIMALS);
...
class Item_func_curtime :public Item_time_func
{
  ...
  MYBLOCKCHAIN_TIME_cache cached_time; // Initialized in fix_length_and_dec
protected:
...
{
  String value;
  bool get_date_internal(MYBLOCKCHAIN_TIME *res, my_time_flags_t fuzzy_date);
  bool get_time_internal(MYBLOCKCHAIN_TIME *res);
protected:
  bool val_datetime(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzy_date);
...
class Item_func_add_time :public Item_temporal_hybrid_func
{
  const bool is_date;
  int sign;
  bool val_datetime(MYBLOCKCHAIN_TIME *time, my_time_flags_t fuzzy_date);
public:
