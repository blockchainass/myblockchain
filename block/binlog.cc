...
#include <myblockchain/psi/myblockchain_transaction.h>
#include "xa.h"
...
MYBLOCKCHAIN_BIN_LOG myblockchain_bin_log(&sync_binlog_period, WRITE_CACHE);
...
{
public:
  Mutex_sentry(myblockchain_mutex_t *mutex)
    : m_mutex(mutex)
  {
    if (m_mutex)
  ...
  void try_to_attach_to(THD *thd)
  {
    ...
    */
    if (MAX_SESSION_ATTACH_TRIES == i)
    {
      ...
      _exit(MYBLOCKCHAIND_FAILURE_EXIT);
    }
  }
...
binlog_trans_log_savepos(THD *thd, my_off_t *pos)
{
  ...
  DBUG_ASSERT(myblockchain_bin_log.is_open());
...
/*
  ...
  conceptually, binlog initialization (now mostly done in MYBLOCKCHAIN_BIN_LOG::open)
...
static int binlog_deinit(void *p)
{
  ...
  if (tc_log == &myblockchain_bin_log)
    tc_log= NULL;
  binlog_hton= NULL;
  return 0;
}
...
/**
  ...
  @retval true Error.
*/
bool MYBLOCKCHAIN_BIN_LOG::write_gtid(THD *thd, binlog_cache_data *cache_data,
                               Binlog_event_writer *writer)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::write_gtid");
  ...
  */
  ...
  Logical_clock& clock= myblockchain_bin_log.max_committed_transaction;
...
int MYBLOCKCHAIN_BIN_LOG::gtid_end_transaction(THD *thd)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::gtid_end_transaction");
    ...
    else
    {
      ...
      */
      ...
      if (cache_data->write_event(thd, &qinfo) ||
          myblockchain_bin_log.commit(thd, true))
        DBUG_RETURN(1);
    }
  }
...
binlog_cache_data::flush(THD *thd, my_off_t *bytes_written, bool *wrote_xid)
{
  ...
  */
  ...
  if (flags.finalized)
  {
    ...
    trn_ctx->sequence_number= myblockchain_bin_log.transaction_counter.step();
    /*
    ...
    */
    ...
    /*
      ...
      correct.
    */
    Binlog_event_writer writer(myblockchain_bin_log.get_log_file());
    ...
    if (!error)
      error= myblockchain_bin_log.write_gtid(thd, this, &writer);
    if (!error)
      error= myblockchain_bin_log.write_cache(thd, this, &writer);
  ...
  */
  if (ending_trans(thd, all))
  {
    if (has_incident())
    {
      ...
      error= myblockchain_bin_log.write_incident(thd, true/*need_lock_log=true*/,
                                          err_msg);
    }
    reset();
  }
...
static int binlog_prepare(handlerton *hton, THD *thd, bool all)
{
  ...
  DBUG_RETURN(all && is_loggable_xa_prepare(thd) ?
              myblockchain_bin_log.commit(thd, true) : 0);
}
...
inline int do_binlog_xa_commit_rollback(THD *thd, XID *xid, bool commit)
{
  ...
  return myblockchain_bin_log.write_event(&qinfo);
}
...
inline int binlog_xa_commit_or_rollback(THD *thd, XID *xid, bool commit)
{
  ...
  if (!(error= do_binlog_xa_commit_rollback(thd, xid, commit)))
  {
    ...
    */
    ...
    if (commit)
      (void) myblockchain_bin_log.commit(thd, true);
    else
      (void) myblockchain_bin_log.rollback(thd, true);
    if (cache_mngr)
...
/**
  ...
  MYBLOCKCHAIN_BIN_LOG::commit function.

  @see MYBLOCKCHAIN_BIN_LOG::commit
...
/**
  ...
  MYBLOCKCHAIN_BIN_LOG::rollback is called in that case.

  @todo Refactor code to introduce a <code>MYBLOCKCHAIN_BIN_LOG::rollback(THD
...
static int binlog_rollback(handlerton *hton, THD *thd, bool all)
{
  ...
  if (thd->lex->sql_command == SQLCOM_ROLLBACK_TO_SAVEPOINT)
    error= myblockchain_bin_log.rollback(thd, all);
  DBUG_RETURN(error);
}
...
bool
Stage_manager::enroll_for(StageID stage, THD *thd, myblockchain_mutex_t *stage_mutex)
{
  ...
  */
  if (stage_mutex)
    myblockchain_mutex_unlock(stage_mutex);
  ...
  */
  if (!leader)
  {
    myblockchain_mutex_lock(&m_lock_done);
#ifndef DBUG_OFF
    /*
    ...
    */
    ...
    if (leader_await_preempt_status)
      myblockchain_cond_signal(&m_cond_preempt);
#endif
    while (thd->get_transaction()->m_flags.pending)
      myblockchain_cond_wait(&m_cond_done, &m_lock_done);
    myblockchain_mutex_unlock(&m_lock_done);
  }
  return leader;
}
...
void Stage_manager::signal_done(THD *queue)
{
  ...
  myblockchain_mutex_unlock(&m_lock_done);
  myblockchain_cond_broadcast(&m_cond_done);
}
...
void Stage_manager::clear_preempt_status(THD *head)
{
  ...
  while(!head->get_transaction()->m_flags.ready_preempt)
  {
    ...
    myblockchain_cond_wait(&m_cond_preempt, &m_lock_done);
  }
...
/**
  ...
  @see MYBLOCKCHAIN_BIN_LOG::ordered_commit
  @see ha_commit_low
  @see ha_rollback_low
...
int MYBLOCKCHAIN_BIN_LOG::rollback(THD *thd, bool all)
{
  int error= 0;
  bool stuff_logged= false;
  binlog_cache_mngr *cache_mngr= thd_get_cache_mngr(thd);

  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::rollback(THD *thd, bool all)");
...
/**
  ...
  savepoint names when using myblockchainbinlog or in the slave SQL thread.
...
static int binlog_savepoint_set(handlerton *hton, THD *thd, void *sv)
{
  ...
  /* 
    ...
    log.
  */
  if (!(error= myblockchain_bin_log.write_event(&qinfo)))
...
static int binlog_savepoint_rollback(handlerton *hton, THD *thd, void *sv)
{
  ...
  */
  if (trans_cannot_safely_rollback(thd))
  {
    ...
    Query_log_event qinfo(thd, log_query.c_ptr_safe(), log_query.length(),
                          TRUE, FALSE, TRUE, errcode);
    DBUG_RETURN(myblockchain_bin_log.write_event(&qinfo));
  }
...
/**
   ...
   convertor.
   Called from @c purge_error_message(), @c MYBLOCKCHAIN_BIN_LOG::reset_logs()
...
public:
  ...
  virtual void operator()(THD *thd)
  {
    ...
    myblockchain_mutex_unlock(&thd->LOCK_thd_data);
  }
private:
...
int check_binlog_magic(IO_CACHE* log, const char** errmsg)
{
  ...
  {
    *errmsg = "Binlog has bad magic number;  It's not a binary log file that can be used by this version of MyBlockchain";
    return 1;
  }
  return 0;
}
...
File open_binlog_file(IO_CACHE *log, const char *log_file_name, const char **errmsg)
{
  ...
  if ((file= myblockchain_file_open(key_file_binlog,
...
bool purge_master_logs(THD* thd, const char* to_log)
{
  ...
  if (!myblockchain_bin_log.is_open())
  {
    my_ok(thd);
    return FALSE;
  }

  myblockchain_bin_log.make_log_name(search_file_name, to_log);
  return purge_error_message(thd,
                             myblockchain_bin_log.purge_logs(search_file_name, false,
...
bool purge_master_logs_before_date(THD* thd, time_t purge_time)
{
  ...
  return purge_error_message(thd,
                             myblockchain_bin_log.purge_logs_before_date(purge_time,
                                                                  false));
}
#endif /* EMBEDDED_LIBRARY */
...
int query_error_code(THD *thd, bool not_killed)
{
  ...
  {
    error= thd->is_error() ? thd->get_stmt_da()->myblockchain_errno() : 0;
...
static bool copy_file(IO_CACHE *from, IO_CACHE *to, my_off_t offset)
{
  ...
  myblockchain_file_seek(from->file, offset, MY_SEEK_SET, MYF(0));
  while(TRUE)
  {
    if ((bytes_read= (int) myblockchain_file_read(from->file, io_buf, sizeof(io_buf),
                                           MYF(MY_WME)))
        < 0)
      goto err;
    ...
    if (!bytes_read)
      break;                                    // end of file
    if (myblockchain_file_write(to->file, io_buf, bytes_read, MYF(MY_WME | MY_NABP)))
      goto err;
  }
       ...
       block_len -= min(block_len, max_event_size))
  {
    ...
    if (lf_info->wrote_create_file)
    {
      Append_block_log_event a(lf_info->thd, lf_info->thd->db().str, buffer,
                               ...
                               lf_info->log_delayed);
      if (myblockchain_bin_log.write_event(&a))
        DBUG_RETURN(1);
    }
    else
    {
      Begin_load_query_log_event b(lf_info->thd, lf_info->thd->db().str,
                                   ...
                                   min(block_len, max_event_size),
                                   lf_info->log_delayed);
      if (myblockchain_bin_log.write_event(&b))
        DBUG_RETURN(1);
...
bool show_binlog_events(THD *thd, MYBLOCKCHAIN_BIN_LOG *binary_log)
{
  ...
  Format_description_log_event *description_event= new
    Format_description_log_event(3); /* MyBlockchain 4.0 by default */
  ...
  if (binary_log->is_open())
  {
    ...
    myblockchain_mutex_t *log_lock = binary_log->get_log_lock();
    Log_event* ev;
    ...
    */
    ev= Log_event::read_log_event(&log, (myblockchain_mutex_t*)0, description_event,
    ...
    for (event_count = 0;
         (ev = Log_event::read_log_event(&log, (myblockchain_mutex_t*) 0,
                                         description_event,
                                         opt_master_verify_checksum)); )
    {
      if (ev->get_type_code() == binary_log::FORMAT_DESCRIPTION_EVENT)
...
err:
  ...
  {
    end_io_cache(&log);
    myblockchain_file_close(file, MYF(MY_WME));
  }
...
/**
  ...
  @retval TRUE failure
*/
bool myblockchain_show_binlog_events(THD* thd)
{
  List<Item> field_list;
  DBUG_ENTER("myblockchain_show_binlog_events");
  ...
  */
  ...
  DBUG_RETURN(show_binlog_events(thd, &myblockchain_bin_log));
}

#endif /* HAVE_REPLICATION */


MYBLOCKCHAIN_BIN_LOG::MYBLOCKCHAIN_BIN_LOG(uint *sync_period,
...
void MYBLOCKCHAIN_BIN_LOG::cleanup()
{
  DBUG_ENTER("cleanup");
  if (inited)
  {
    ...
    myblockchain_mutex_destroy(&LOCK_log);
    myblockchain_mutex_destroy(&LOCK_index);
    myblockchain_mutex_destroy(&LOCK_commit);
    myblockchain_mutex_destroy(&LOCK_sync);
    myblockchain_mutex_destroy(&LOCK_binlog_end_pos);
    myblockchain_mutex_destroy(&LOCK_xids);
    myblockchain_cond_destroy(&update_cond);
    myblockchain_cond_destroy(&m_prep_xids_cond);
...
void MYBLOCKCHAIN_BIN_LOG::init_pthread_objects()
{
  DBUG_ASSERT(inited == 0);
  inited= 1;
  myblockchain_mutex_init(m_key_LOCK_log, &LOCK_log, MY_MUTEX_INIT_SLOW);
  myblockchain_mutex_init(m_key_LOCK_index, &LOCK_index, MY_MUTEX_INIT_SLOW);
  myblockchain_mutex_init(m_key_LOCK_commit, &LOCK_commit, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(m_key_LOCK_sync, &LOCK_sync, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(m_key_LOCK_binlog_end_pos, &LOCK_binlog_end_pos,
                   MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(m_key_LOCK_xids, &LOCK_xids, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(m_key_update_cond, &update_cond);
  myblockchain_cond_init(m_key_prep_xids_cond, &m_prep_xids_cond);
  stage_manager.init(
#ifdef HAVE_PSI_INTERFACE
...
int MYBLOCKCHAIN_BIN_LOG::generate_new_name(char *new_name, const char *log_name)
{
  fn_format(new_name, log_name, myblockchain_data_home, "", 4);
...
*/
const char *MYBLOCKCHAIN_BIN_LOG::generate_name(const char *log_name,
                                         const char *suffix,
                                         char *buff)
{
...
bool MYBLOCKCHAIN_BIN_LOG::init_and_set_log_file_name(const char *log_name,
                                               const char *new_name)
{
...
bool MYBLOCKCHAIN_BIN_LOG::open(
#ifdef HAVE_PSI_INTERFACE
                     ...
                     const char *new_name)
{
  ...
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::open");

  write_error= 0;

  if (!(name= my_strdup(key_memory_MYBLOCKCHAIN_LOG_name,
                        log_name, MYF(MY_WME))))
  {
  ...
  if ((file= myblockchain_file_open(log_file_key,
                             log_file_name, open_flags,
                             MYF(MY_WME))) < 0)
    goto err;

  if ((pos= myblockchain_file_tell(file, MYF(MY_WME))) == MY_FILEPOS_ERROR)
  {
    if (my_errno == ESPIPE)
      pos= 0;
    else
...
err:
  if (binlog_error_action == ABORT_SERVER)
  {
    ...
    sql_print_error("Could not open %s for logging (error %d). "
                    ...
                    "of the MyBlockchain server process. To turn it on "
                    "again: fix the cause, shutdown the MyBlockchain "
...
bool MYBLOCKCHAIN_BIN_LOG::open_index_file(const char *index_file_name_arg,
                                    const char *log_name, bool need_lock_index)
{
  ...
  */
  ...
  fn_format(index_file_name, index_file_name_arg, myblockchain_data_home,
            ".index", opt);

  if (set_crash_safe_index_file_name(index_file_name_arg))
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::set_crash_safe_index_file_name failed.");
    return TRUE;
  }

  /*
    We need move crash_safe_index_file to index_file if the index_file
    does not exist and crash_safe_index_file exists when myblockchaind server
    restarts.
  */
  ...
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::open_index_file failed to "
                    "move crash_safe_index_file to index file.");
    return TRUE;
  }

  if ((index_file_nr= myblockchain_file_open(m_key_file_log_index,
                                      index_file_name,
                                      O_RDWR | O_CREAT | O_BINARY,
                                      MYF(MY_WME))) < 0 ||
       myblockchain_file_sync(index_file_nr, MYF(MY_WME)) ||
       init_io_cache(&index_file, index_file_nr,
                     IO_SIZE, READ_CACHE,
                     myblockchain_file_seek(index_file_nr, 0L, MY_SEEK_END, MYF(0)),
      ...
      DBUG_EVALUATE_IF("fault_injection_openning_index", 1, 0))
  {
    /*
      ...
      TODO: file creation should be done with myblockchain_file_create()
      not myblockchain_file_open().
    */
    if (index_file_nr >= 0)
      myblockchain_file_close(index_file_nr, MYF(0));
    return TRUE;
  }
  ...
  */
  ...
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::open_index_file failed to sync the index "
                    "file.");
    return TRUE;
  }
#endif
...
bool MYBLOCKCHAIN_BIN_LOG::find_first_log_not_in_gtid_set(char *binlog_file_name,
                                                   const Gtid_set *gtid_set,
                                                   Gtid *first_gtid,
                                                   const char **errmsg)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::gtid_read_start_binlog");
  /*
  ...
  */
  ...
  myblockchain_mutex_lock(&LOCK_index);
  ...
  myblockchain_mutex_unlock(&LOCK_index);
...
bool MYBLOCKCHAIN_BIN_LOG::init_gtid_sets(Gtid_set *all_gtids, Gtid_set *lost_gtids,
                                   bool verify_checksum, bool need_lock,
                                   Transaction_boundary_parser *trx_parser,
                                   Gtid *gtid_partial_trx,
                                   bool is_server_starting)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::init_gtid_sets");
  ...
  */
  if (need_lock)
  {
    ...
    if (all_gtids != NULL)
      myblockchain_mutex_lock(&LOCK_log);
    myblockchain_mutex_lock(&LOCK_index);
  ...
  {
    if (all_gtids != NULL)
      myblockchain_mutex_assert_owner(&LOCK_log);
    myblockchain_mutex_assert_owner(&LOCK_index);
  ...
  */
  ...
  if (need_lock)
  {
    ...
    myblockchain_mutex_unlock(&LOCK_index);
    if (all_gtids != NULL)
      myblockchain_mutex_unlock(&LOCK_log);
  }
  filename_list.clear();
  DBUG_PRINT("info", ("returning %d", error));
  DBUG_RETURN(error != 0 ? true : false);
...
bool MYBLOCKCHAIN_BIN_LOG::open_binlog(const char *log_name,
                                const char *new_name,
                                ulong max_size_arg,
                                ...
                                Format_description_log_event *extra_description_event)
{
  ...
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::open_binlog(const char *, ...)");
  DBUG_PRINT("enter",("base filename: %s", log_name));

  myblockchain_mutex_assert_owner(get_log_lock());

  if (init_and_set_log_file_name(log_name, new_name))
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::open failed to generate new file name.");
    DBUG_RETURN(1);
  }
      ...
      DBUG_EVALUATE_IF("fault_injection_registering_index", 1, 0))
  {
    /**
      ...
      Perhaps we might need the code below in MYBLOCKCHAIN_BIN_LOG::cleanup
    ...
    */
    ...
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::open failed to sync the index file.");
    DBUG_RETURN(1);
  }
  ...
  */
  ...
  /*
    ...
    See further comments in the myblockchaind.
    /Alfranio
  */
  if (current_thd)
  {
  ...
  else // !(current_thd)
  {
    /*
      ...
      During server's startup at myblockchaind_main(), from the binary/relay log
  ...
  if (flush_io_cache(&log_file) ||
      myblockchain_file_sync(log_file.file, MYF(MY_WME)))
    goto err;
...
err:
  ...
  if (binlog_error_action == ABORT_SERVER)
  {
    ...
    sql_print_error("Could not use %s for logging (error %d). "
                    "Turning logging off for the whole duration of the MyBlockchain "
                    "server process. To turn it on again: fix the cause, "
                    "shutdown the MyBlockchain server and restart it.", name, errno);
  DBUG_RETURN(1);
}
...
/**
  ...
  @retval -1 error
*/
int MYBLOCKCHAIN_BIN_LOG::move_crash_safe_index_file_to_index_file(bool need_lock_index)
{
  int error= 0;
  File fd= -1;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::move_crash_safe_index_file_to_index_file");
  ...
  {
    end_io_cache(&index_file);
    if (myblockchain_file_close(index_file.file, MYF(0)) < 0)
    {
      error= -1;
      sql_print_error("MYBLOCKCHAIN_BIN_LOG::move_crash_safe_index_file_to_index_file "
                      "failed to close the index file.");
      goto err;
    }
    myblockchain_file_delete(key_file_binlog_index, index_file_name, MYF(MY_WME));
  }
  ...
  if ((fd= myblockchain_file_open(key_file_binlog_index,
                           index_file_name,
                           O_RDWR | O_CREAT | O_BINARY,
                           MYF(MY_WME))) < 0 ||
           myblockchain_file_sync(fd, MYF(MY_WME)) ||
           init_io_cache(&index_file, fd, IO_SIZE, READ_CACHE,
                         myblockchain_file_seek(fd, 0L, MY_SEEK_END, MYF(0)),
                                         0, MYF(MY_WME | MY_WAIT_IF_FULL)))
  {
    error= -1;
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::move_crash_safe_index_file_to_index_file "
...
/**
  ...
  @retval
    -1   error
*/
int MYBLOCKCHAIN_BIN_LOG::add_log_to_index(uchar* log_name,
                                    size_t log_name_len, bool need_lock_index)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::add_log_to_index");

  if (open_crash_safe_index_file())
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::add_log_to_index failed to "
  ...
  if (my_b_write(&crash_safe_index_file, log_name, log_name_len) ||
      ...
      myblockchain_file_sync(crash_safe_index_file.file, MYF(MY_WME)))
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::add_log_to_index failed to "
...
int MYBLOCKCHAIN_BIN_LOG::get_current_log(LOG_INFO* linfo)
{
  myblockchain_mutex_lock(&LOCK_log);
...
int MYBLOCKCHAIN_BIN_LOG::raw_get_current_log(LOG_INFO* linfo)
{
...
bool MYBLOCKCHAIN_BIN_LOG::check_write_error(THD *thd)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::check_write_error");
  ...
  switch (thd->get_stmt_da()->myblockchain_errno())
  {
...
void MYBLOCKCHAIN_BIN_LOG::set_write_error(THD *thd, bool is_transactional)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::set_write_error");
...
int MYBLOCKCHAIN_BIN_LOG::find_log_pos(LOG_INFO *linfo, const char *log_name,
                                bool need_lock_index)
{
  int error= 0;
...
*/
int MYBLOCKCHAIN_BIN_LOG::find_next_log(LOG_INFO* linfo, bool need_lock_index)
{
  int error= 0;
  size_t length;
...
*/
int MYBLOCKCHAIN_BIN_LOG::find_next_relay_log(char log_name[FN_REFLEN+1])
{
  LOG_INFO info;
  int error;
...
/**
  ...
  @retval
    1   error
*/
bool MYBLOCKCHAIN_BIN_LOG::reset_logs(THD* thd, bool delete_only)
{
  LOG_INFO linfo;
  bool error=0;
  int err;
...
/**
  ...
  @retval
    1   error
*/
int MYBLOCKCHAIN_BIN_LOG::set_crash_safe_index_file_name(const char *base_file_name)
{
  int error= 0;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::set_crash_safe_index_file_name");
  if (fn_format(crash_safe_index_file_name, base_file_name, myblockchain_data_home,
...
/**
  ...
  @retval
    1   error
*/
int MYBLOCKCHAIN_BIN_LOG::open_crash_safe_index_file()
{
  int error= 0;
  File file= -1;

  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::open_crash_safe_index_file");
  ...
  if (!my_b_inited(&crash_safe_index_file))
  {
    ...
    {
      error= 1;
      sql_print_error("MYBLOCKCHAIN_BIN_LOG::open_crash_safe_index_file failed "
...
/**
  ...
  @retval
    1   error
*/
int MYBLOCKCHAIN_BIN_LOG::close_crash_safe_index_file()
{
  int error= 0;

  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::close_crash_safe_index_file");
...
int MYBLOCKCHAIN_BIN_LOG::purge_first_log(Relay_log_info* rli, bool included)
{
  int error;
  ...
  myblockchain_mutex_assert_owner(&rli->data_lock);
  ...
  */
  ...
  myblockchain_mutex_lock(&rli->log_space_lock);
  ...
  myblockchain_mutex_unlock(&rli->log_space_lock);
  ...
  /*
    ...
    context switches
  */
  myblockchain_cond_broadcast(&rli->log_space_cond);
...
*/
int MYBLOCKCHAIN_BIN_LOG::remove_logs_from_index(LOG_INFO* log_info, bool need_update_threads)
{
  if (open_crash_safe_index_file())
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::remove_logs_from_index failed to "
                    "open the crash safe index file.");
    goto err;
  }
...
/**
  ...
  @retval
    ...
    LOG_INFO_FATAL              if any other than ENOENT error from
                                myblockchain_file_stat() or myblockchain_file_delete()
*/

int MYBLOCKCHAIN_BIN_LOG::purge_logs(const char *to_log,
                              bool included,
                              bool need_lock_index,
                              ...
                              bool auto_purge)
{
  ...
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_logs was called with file %s not "
  ...
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_logs failed to sync the index file.");
    goto err;
  }
  ...
  */
  ...
  while ((strcmp(to_log,log_info.log_file_name) || (exit_loop=included)))
  {
    ...
    {
      sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_logs failed to copy %s to register file.",
                      log_info.log_file_name);
      goto err;
    }
  ...
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_logs failed to flush register file.");
    goto err;
  }
  ...
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_logs failed to update the index file");
    goto err;
  }
...
err:
  ...
  if (is_inited_purge_index_file() &&
    ...
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_logs failed to process registered files"
...
int MYBLOCKCHAIN_BIN_LOG::set_purge_index_file_name(const char *base_file_name)
{
  int error= 0;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::set_purge_index_file_name");
  if (fn_format(purge_index_file_name, base_file_name, myblockchain_data_home,
                ".~rec~", MYF(MY_UNPACK_FILENAME | MY_SAFE_PATH |
                              MY_REPLACE_EXT)) == NULL)
  {
    error= 1;
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::set_purge_index_file_name failed to set "
                      "file name.");
  }
  DBUG_RETURN(error);
}

int MYBLOCKCHAIN_BIN_LOG::open_purge_index_file(bool destroy)
{
  int error= 0;
  File file= -1;

  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::open_purge_index_file");
  ...
  if (!my_b_inited(&purge_index_file))
  {
    ...
    {
      error= 1;
      sql_print_error("MYBLOCKCHAIN_BIN_LOG::open_purge_index_file failed to open register "
                      " file.");
    }
  }
  DBUG_RETURN(error);
...
int MYBLOCKCHAIN_BIN_LOG::close_purge_index_file()
{
  int error= 0;

  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::close_purge_index_file");
...
bool MYBLOCKCHAIN_BIN_LOG::is_inited_purge_index_file()
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::is_inited_purge_index_file");
  DBUG_RETURN (my_b_inited(&purge_index_file));
}

int MYBLOCKCHAIN_BIN_LOG::sync_purge_index_file()
{
  int error= 0;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::sync_purge_index_file");
...
int MYBLOCKCHAIN_BIN_LOG::register_purge_index_entry(const char *entry)
{
  int error= 0;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::register_purge_index_entry");
...
int MYBLOCKCHAIN_BIN_LOG::register_create_index_entry(const char *entry)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::register_create_index_entry");
  DBUG_RETURN(register_purge_index_entry(entry));
}

int MYBLOCKCHAIN_BIN_LOG::purge_index_entry(THD *thd, ulonglong *decrease_log_space,
                                     bool need_lock_index)
{
  MY_STAT s;
  int error= 0;
  ...
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG:purge_index_entry");

  DBUG_ASSERT(my_b_inited(&purge_index_file));

  if ((error=reinit_io_cache(&purge_index_file, READ_CACHE, 0, 0, 0)))
  {
    sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_index_entry failed to reinit register file "
                    "for read");
    goto err;
  }
  ...
  for (;;)
  {
                          ...
                          FN_REFLEN)) <= 1)
    {
      if (purge_index_file.error)
      {
        ...
        sql_print_error("MYBLOCKCHAIN_BIN_LOG::purge_index_entry error %d reading from "
                        "register file.", error);
        goto err;
      }
    ...
    if (!myblockchain_file_stat(m_key_file_log, log_info.log_file_name, &s, MYF(0)))
    {
      if (my_errno == ENOENT) 
      {
        /*
        ...
        */
        ...
        sql_print_information("Failed to execute myblockchain_file_stat on file '%s'",
			      log_info.log_file_name);
        my_errno= 0;
      }
      else
    ...
    else
    {
                               ...
                               need_lock_index)))
      {
        ...
        if (!myblockchain_file_delete(key_file_binlog, log_info.log_file_name, MYF(0)))
        {
...
int MYBLOCKCHAIN_BIN_LOG::purge_logs_before_date(time_t purge_time, bool auto_purge)
{
  int error;
  ...
  while (!(log_is_active= is_active(log_info.log_file_name)))
  {
    ...
    if (!myblockchain_file_stat(m_key_file_log,
  ...
  if (log_is_in_use)
  {
    ...
    {
      if (myblockchain_file_stat(m_key_file_log, log_info.log_file_name,
                          &stat_area, MYF(0)))
      {
        if (stat_area.st_mtime < purge_time)
...
void MYBLOCKCHAIN_BIN_LOG::make_log_name(char* buf, const char* log_ident)
{
...
bool MYBLOCKCHAIN_BIN_LOG::is_active(const char *log_file_name_arg)
{
  return !strcmp(log_file_name, log_file_name_arg);
}


void MYBLOCKCHAIN_BIN_LOG::inc_prep_xids(THD *thd)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::inc_prep_xids");
#ifndef DBUG_OFF
...
void MYBLOCKCHAIN_BIN_LOG::dec_prep_xids(THD *thd)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::dec_prep_xids");
  ...
  if (result == 1)
  {
    myblockchain_mutex_lock(&LOCK_xids);
    myblockchain_cond_signal(&m_prep_xids_cond);
    myblockchain_mutex_unlock(&LOCK_xids);
  }
  DBUG_VOID_RETURN;
}
...
int MYBLOCKCHAIN_BIN_LOG::new_file(Format_description_log_event *extra_description_event)
{
...
/*
  @retval
    nonzero - error
*/
int MYBLOCKCHAIN_BIN_LOG::new_file_without_locking(Format_description_log_event *extra_description_event)
{
...
*/
int MYBLOCKCHAIN_BIN_LOG::new_file_impl(bool need_lock_log, Format_description_log_event *extra_description_event)
{
  int error= 0;
  bool close_on_error= false;
  char new_name[FN_REFLEN], *new_name_ptr, *old_name, *file_to_open;

  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::new_file_impl");
  if (!is_open())
  {
  ...
  while (get_prep_xids() > 0)
    myblockchain_cond_wait(&m_prep_xids_cond, &LOCK_xids);
  myblockchain_mutex_unlock(&LOCK_xids);
...
/**
  ...
  @retval true error
*/
bool MYBLOCKCHAIN_BIN_LOG::after_append_to_relay_log(Master_info *mi)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::after_append_to_relay_log");
  DBUG_PRINT("info",("max_size: %lu",max_size));

  // Check pre-conditions
  myblockchain_mutex_assert_owner(&LOCK_log);
  myblockchain_mutex_assert_owner(&mi->data_lock);
  DBUG_ASSERT(is_relay_log);
...
bool MYBLOCKCHAIN_BIN_LOG::append_event(Log_event* ev, Master_info *mi)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::append");
...
bool MYBLOCKCHAIN_BIN_LOG::append_buffer(const char* buf, uint len, Master_info *mi)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::append_buffer");
...
bool MYBLOCKCHAIN_BIN_LOG::flush_and_sync(const bool force)
{
  myblockchain_mutex_assert_owner(&LOCK_log);
...
void MYBLOCKCHAIN_BIN_LOG::start_union_events(THD *thd, query_id_t query_id_param)
{
...
void MYBLOCKCHAIN_BIN_LOG::stop_union_events(THD *thd)
{
  DBUG_ASSERT(thd->binlog_evt_union.do_union);
  thd->binlog_evt_union.do_union= FALSE;
}

bool MYBLOCKCHAIN_BIN_LOG::is_query_in_union(THD *thd, query_id_t query_id_param)
{
 ...
 */
void MYBLOCKCHAIN_BIN_LOG::update_thd_next_event_pos(THD* thd)
{
  if (likely(thd != NULL))
  {
...
*/
int
MYBLOCKCHAIN_BIN_LOG::flush_and_set_pending_rows_event(THD *thd,
                                                Rows_log_event* event,
                                                bool is_transactional)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::flush_and_set_pending_rows_event(event)");
  DBUG_ASSERT(myblockchain_bin_log.is_open());
...
bool MYBLOCKCHAIN_BIN_LOG::write_event(Log_event *event_info)
{
  THD *thd= event_info->thd;
  bool error= 1;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::write_event(Log_event *)");
...
*/
int MYBLOCKCHAIN_BIN_LOG::rotate(bool force_rotate, bool* check_purge)
{
  int error= 0;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::rotate");
...
/**
  ...
  @retval
    nonzero - error in rotating routine.
*/
void MYBLOCKCHAIN_BIN_LOG::purge()
{
#ifdef HAVE_REPLICATION
  if (expire_logs_days)
  {
...
/**
  ...
  @retval
    nonzero - error in rotating routine.
*/
int MYBLOCKCHAIN_BIN_LOG::rotate_and_purge(THD* thd, bool force_rotate)
{
  int error= 0;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::rotate_and_purge");
...
uint MYBLOCKCHAIN_BIN_LOG::next_file_id()
{
  uint res;
  myblockchain_mutex_lock(&LOCK_log);
  res = file_id++;
...
/**
  ...
  @see MYBLOCKCHAIN_BIN_LOG::write_cache
*/
bool MYBLOCKCHAIN_BIN_LOG::do_write_cache(IO_CACHE *cache, Binlog_event_writer *writer)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::do_write_cache");
...
/**
  ...
  @retval true success
*/
bool MYBLOCKCHAIN_BIN_LOG::write_incident(Incident_log_event *ev, bool need_lock_log,
                                   const char* err_msg, bool do_flush_and_sync)
{
  uint error= 0;
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::write_incident");
  DBUG_ASSERT(err_msg);
...
/**
  ...
  @retval
    1    success
*/
bool MYBLOCKCHAIN_BIN_LOG::write_incident(THD *thd, bool need_lock_log,
                                   const char* err_msg,
...
*/
bool MYBLOCKCHAIN_BIN_LOG::write_cache(THD *thd, binlog_cache_data *cache_data,
                                Binlog_event_writer *writer)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::write_cache(THD *, binlog_cache_data *, bool)");
...
int MYBLOCKCHAIN_BIN_LOG::wait_for_update_relay_log(THD* thd, const struct timespec *timeout)
{
  int ret= 0;
  ...
  if (!timeout)
    myblockchain_cond_wait(&update_cond, &LOCK_log);
  else
    ret= myblockchain_cond_timedwait(&update_cond, &LOCK_log,
...
int MYBLOCKCHAIN_BIN_LOG::wait_for_update_bin_log(THD* thd,
                                           const struct timespec *timeout)
{
  ...
  if (!timeout)
    myblockchain_cond_wait(&update_cond, &LOCK_binlog_end_pos);
  else
    ret= myblockchain_cond_timedwait(&update_cond, &LOCK_binlog_end_pos,
                              const_cast<struct timespec *>(timeout));
  DBUG_RETURN(ret);
}
...
void MYBLOCKCHAIN_BIN_LOG::close(uint exiting)
{					// One can't set log_type here!
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::close");
  ...
  if (log_state.atomic_get() == LOG_OPENED)
  {
    ...
    if (log_file.type == WRITE_CACHE)
    {
      ...
      my_off_t org_position= myblockchain_file_tell(log_file.file, MYF(0));
      uchar flags= 0;            // clearing LOG_EVENT_BINLOG_IN_USE_F
      myblockchain_file_pwrite(log_file.file, &flags, 1, offset, MYF(0));
      /*
        Restore position so that anything we have in the IO_cache is written
        to the correct position.
        We need the seek here, as myblockchain_file_pwrite() is not guaranteed to keep the
        original position on system that doesn't support pwrite().
      */
      myblockchain_file_seek(log_file.file, org_position, MY_SEEK_SET, MYF(0));
    }
    ...
    if (log_state.atomic_get() == LOG_OPENED)
    {
      ...
      if (myblockchain_file_sync(log_file.file, MYF(MY_WME)) && ! write_error)
      {
        char errbuf[MYSYS_STRERROR_SIZE];
        write_error= 1;
      ...
      if (myblockchain_file_close(log_file.file, MYF(MY_WME)) && ! write_error)
      {
        char errbuf[MYSYS_STRERROR_SIZE];
        write_error= 1;
        sql_print_error(ER_DEFAULT(ER_ERROR_ON_WRITE), name, errno,
  ...
  */
  ...
  {
    end_io_cache(&index_file);
    if (myblockchain_file_close(index_file.file, MYF(0)) < 0 && ! write_error)
    {
      char errbuf[MYSYS_STRERROR_SIZE];
      write_error= 1;
...
void MYBLOCKCHAIN_BIN_LOG::set_max_size(ulong max_size_arg)
{
  /*
  ...
  */
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::set_max_size");
  myblockchain_mutex_lock(&LOCK_log);
  if (is_open())
    max_size= max_size_arg;
  myblockchain_mutex_unlock(&LOCK_log);
...
int MYBLOCKCHAIN_BIN_LOG::open_binlog(const char *opt_name)
{
  LOG_INFO log_info;
  int      error= 1;
  ...
  */
...

  {
    ...
    /*
      ...
      may have crashed. In that case, we need to call MYBLOCKCHAIN_BIN_LOG::recover
      to:
    ...
    */
    ...
    if (valid_pos > 0)
    {
      if ((file= myblockchain_file_open(key_file_binlog, log_name,
      ...
      my_off_t offset= BIN_LOG_HEADER_SIZE + FLAGS_OFFSET;
      uchar flags= 0;
      if (myblockchain_file_pwrite(file, &flags, 1, offset, MYF(0)) != 1)
      {
...
void MYBLOCKCHAIN_BIN_LOG::close()
{
}
...
/*
  ...
  @retval 1    error
*/
int MYBLOCKCHAIN_BIN_LOG::prepare(THD *thd, bool all)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::prepare");
...
*/
TC_LOG::enum_result MYBLOCKCHAIN_BIN_LOG::commit(THD *thd, bool all)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::commit");
...
TC_LOG::enum_result MYBLOCKCHAIN_BIN_LOG::write_binlog_and_commit_engine(THD *thd,
                                                                  bool all)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::write_binlog_and_commit_engine");
  binlog_cache_mngr *cache_mngr= thd_get_cache_mngr(thd);
  Transaction_ctx *trn_ctx= thd->get_transaction();
...
MYBLOCKCHAIN_BIN_LOG::flush_thread_caches(THD *thd)
{
  binlog_cache_mngr *cache_mngr= thd_get_cache_mngr(thd);
  my_off_t bytes= 0;
...
int
MYBLOCKCHAIN_BIN_LOG::process_flush_stage_queue(my_off_t *total_bytes_var,
                                         bool *rotate_var,
                                         THD **out_queue_var)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::process_flush_stage_queue");
  #ifndef DBUG_OFF
...
*/
void MYBLOCKCHAIN_BIN_LOG::update_max_committed(THD *thd)
{
  Transaction_ctx *trn_ctx= thd->get_transaction();
...
void
MYBLOCKCHAIN_BIN_LOG::process_commit_stage_queue(THD *thd, THD *first)
{
  myblockchain_mutex_assert_owner(&LOCK_commit);
...
void
MYBLOCKCHAIN_BIN_LOG::process_after_commit_stage_queue(THD *thd, THD *first)
{
  Thread_excursion excursion(thd);
  for (THD *head= first; head; head= head->next_to_commit)
  {
...
bool
MYBLOCKCHAIN_BIN_LOG::change_stage(THD *thd,
                            Stage_manager::StageID stage, THD *queue,
                            myblockchain_mutex_t *leave_mutex,
                            myblockchain_mutex_t *enter_mutex)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::change_stage");
  ...
  */
  ...
  myblockchain_mutex_lock(enter_mutex);
  DBUG_RETURN(false);
}
...
int
MYBLOCKCHAIN_BIN_LOG::flush_cache_to_file(my_off_t *end_pos_var)
{
...
*/
std::pair<bool, bool>
MYBLOCKCHAIN_BIN_LOG::sync_binlog_file(bool force)
{
  bool synced= false;
  ...
  {
    sync_counter= 0;
    if (myblockchain_file_sync(log_file.file, MYF(MY_WME)))
 ...
 */
int
MYBLOCKCHAIN_BIN_LOG::finish_commit(THD *thd)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::finish_commit");
 ...
 */
int MYBLOCKCHAIN_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit)
{
  DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::ordered_commit");
  int flush_error= 0;
  my_off_t total_bytes= 0;
  ...
  */
  ...
  if (opt_binlog_order_commits)
  {
    ...
    */
    ...
    myblockchain_mutex_unlock(&LOCK_commit);
    /*
  ...
  {
    myblockchain_mutex_unlock(&LOCK_sync);
    if (flush_error == 0)
...
/**
  MYBLOCKCHAIND server recovers from last crashed binlog.
  ...
  @retval
    1                  error
*/
int MYBLOCKCHAIN_BIN_LOG::recover(IO_CACHE *log, Format_description_log_event *fdle,
                            my_off_t *valid_pos)
{
  Log_event  *ev;
  HASH xids;
...
err1:
  sql_print_error("Crash recovery failed. Either correct the problem "
                  ...
                  "or delete (or rename) binary log and start myblockchaind with "
...
int THD::binlog_setup_trx_data()
{
  ...
  if (!cache_mngr ||
      open_cached_file(&cache_mngr->stmt_cache.cache_log, myblockchain_tmpdir,
                       LOG_PREFIX, binlog_stmt_cache_size, MYF(MY_WME)) ||
      open_cached_file(&cache_mngr->trx_cache.cache_log, myblockchain_tmpdir,
                                ...
                                bool binlog_rows_query)
{
  ...
  /* Pre-conditions */
  DBUG_ASSERT(is_current_stmt_binlog_format_row() && myblockchain_bin_log.is_open());
...
THD::add_to_binlog_accessed_dbs(const char *db_param)
{
  char *after_db;
  /*
    binlog_accessed_db_names list is to maintain the blockchain
...
int THD::decide_logging_format(TABLE_LIST *tables)
{
  ...
  */
  if (myblockchain_bin_log.is_open() && (variables.option_bits & OPTION_BIN_LOG) &&
        ...
        !binlog_filter->db_ok(m_db.str)))
  {
    ...
    */
    ...
    /**
      ...
      'local' to a MyBlockchain instance.
    ...
    */
        ...
        lex->sql_command != SQLCOM_END /* rows-event applying by slave */)
    {
      /*
        ...
        In case the number of blockchains exceeds MAX_DBS_IN_EVENT_MTS maximum
  ...
  else
  {
    DBUG_PRINT("info", ("decision: no logging since "
                        "myblockchain_bin_log.is_open() = %d "
                        "and (options & OPTION_BIN_LOG) = 0x%llx "
                        "and binlog_format = %lu "
                        "and binlog_filter->db_ok(db) = %d",
                        myblockchain_bin_log.is_open(),
...
bool THD::is_ddl_gtid_compatible(bool handle_error, bool handle_nonerror)
{
  ...
  if ((variables.option_bits & OPTION_BIN_LOG) == 0 ||
      myblockchain_bin_log.is_open() == false)
    DBUG_RETURN(true);
                                       ...
                                       const uchar* extra_row_info)
{
  ...
  */
                                      ...
                                      extra_row_info))
  {
    ...
    */
    if (unlikely(
        myblockchain_bin_log.flush_and_set_pending_rows_event(this, ev,
                                                       is_transactional)))
    {
      delete ev;
      DBUG_RETURN(NULL);
...
int THD::binlog_flush_pending_rows_event(bool stmt_end, bool is_transactional)
{
  ...
  */
  ...
  if (Rows_log_event *pending= binlog_get_pending_rows_event(is_transactional))
  {
    ...
    error= myblockchain_bin_log.flush_and_set_pending_rows_event(this, 0,
                                                          is_transactional);
  }
...
{
  char buf[MYBLOCKCHAIN_ERRMSG_SIZE * 2];
                      ...
                      bool suppress_use, int errcode)
{
  ...
  DBUG_ASSERT(query_arg && myblockchain_bin_log.is_open());
  ...
  /*
    If we are not in prelocked mode, myblockchain_unlock_tables() will be
  ...
  */
  ...
  case THD::STMT_QUERY_TYPE:
    /*
      The MYBLOCKCHAIN_BIN_LOG::write() function will set the STMT_END_F flag and
    ...
    */
    {
       ...
       */
      int error= myblockchain_bin_log.write_event(&qinfo);
      binlog_table_maps= 0;
      DBUG_RETURN(error);
    }
    break;
...
#endif /* !defined(MYBLOCKCHAIN_CLIENT) */

struct st_myblockchain_storage_engine binlog_storage_engine=
{ MYBLOCKCHAIN_HANDLERTON_INTERFACE_VERSION };

/** @} */

myblockchain_declare_plugin(binlog)
{
