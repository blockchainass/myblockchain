...
#include "myblockchain/psi/myblockchain_sp.h"
...
static const
TABLE_FIELD_TYPE proc_table_fields[MYBLOCKCHAIN_PROC_FIELD_COUNT] =
{
  {
  ...
  },
  {
    ...
    "'NO_KEY_OPTIONS','NO_TABLE_OPTIONS','NO_FIELD_OPTIONS','MYBLOCKCHAIN323','MYBLOCKCHAIN40',"
...
static const TABLE_FIELD_DEF
  proc_table_def= {MYBLOCKCHAIN_PROC_FIELD_COUNT, proc_table_fields};
                                         ...
                                         TABLE *proc_tbl)
{
  ...
  if (load_charset(thd->mem_root,
                   proc_tbl->field[MYBLOCKCHAIN_PROC_FIELD_CHARACTER_SET_CLIENT],
                   thd->variables.character_set_client,
                   &client_cs))
  {
    sql_print_warning("Stored routine '%s'.'%s': invalid value "
                      "in column myblockchain.proc.character_set_client.",
                      db_name,
                      sr_name);
  ...
  if (load_collation(thd->mem_root,
                     proc_tbl->field[MYBLOCKCHAIN_PROC_FIELD_COLLATION_CONNECTION],
                     thd->variables.collation_connection,
                     &connection_cl))
  {
    sql_print_warning("Stored routine '%s'.'%s': invalid value "
                      "in column myblockchain.proc.collation_connection.",
                      db_name,
                      sr_name);
  ...
  if (load_collation(thd->mem_root,
                     proc_tbl->field[MYBLOCKCHAIN_PROC_FIELD_DB_COLLATION],
                     NULL,
                     &db_cl))
  {
    sql_print_warning("Stored routine '%s'.'%s': invalid value "
                      "in column myblockchain.proc.db_collation.",
                      db_name,
                      sr_name);
  ...
  /*
    If we failed to retrieve the blockchain collation, load the default one
    from the disk.
  */
...
/**
  Report failure to validate the myblockchain.proc table definition.
...
void Proc_table_intact::report_error(uint code, const char *fmt, ...)
{
  ...
  if (code)
    ...
    my_error(ER_CANNOT_LOAD_FROM_TABLE_V2, MYF(0), "myblockchain", "proc");
...
/**
  Open the myblockchain.proc table for read.

  @param thd     Thread context
  @param backup  Pointer to Open_tables_state instance where information about
                 currently open tables will be saved, and from which will be
                 restored when we will end work with myblockchain.proc.

  @retval
    0	Error
  @retval
    \#	Pointer to TABLE object of myblockchain.proc
*/
...
TABLE *open_proc_table_for_read(THD *thd, Open_tables_backup *backup)
{
  ...
  table.init_one_table("myblockchain", 5, "proc", 4, "proc", TL_READ);
...
/**
  Open the myblockchain.proc table for update.
...
static TABLE *open_proc_table_for_update(THD *thd)
{
  ...
  table_list.init_one_table("myblockchain", 5, "proc", 4, "proc", TL_WRITE);
...
/**
  Find row in open myblockchain.proc table representing stored routine.

  @param thd    Thread context
  @param type   Type of routine to find (function or procedure)
  @param name   Name of routine
  @param table  TABLE object for open myblockchain.proc table.
...
/**
  Find routine definition in myblockchain.proc table and create corresponding
...
db_find_routine(THD *thd, enum_sp_type type, sp_name *name, sp_head **sphp)
{
  ...
  *sphp= 0;                                     // In case of errors
  ...
  if (table->s->fields < MYBLOCKCHAIN_PROC_FIELD_COUNT)
  {
  ...
  if ((ptr= get_field(thd->mem_root,
		      table->field[MYBLOCKCHAIN_PROC_FIELD_ACCESS])) == NULL)
  {
    ret= SP_GET_FIELD_FAILED;
    goto done;
  }
  ...
  if ((ptr= get_field(thd->mem_root,
		      table->field[MYBLOCKCHAIN_PROC_FIELD_DETERMINISTIC])) == NULL)
  {
    ret= SP_GET_FIELD_FAILED;
    goto done;
  }
  ...
  if ((ptr= get_field(thd->mem_root,
		      table->field[MYBLOCKCHAIN_PROC_FIELD_SECURITY_TYPE])) == NULL)
  {
    ret= SP_GET_FIELD_FAILED;
    goto done;
  }
  ...
  if ((params= get_field(thd->mem_root,
			 table->field[MYBLOCKCHAIN_PROC_FIELD_PARAM_LIST])) == NULL)
  {
    params= "";
  }
  ...
  if (type == SP_TYPE_PROCEDURE)
    returns= "";
			       ...
			       table->field[MYBLOCKCHAIN_PROC_FIELD_RETURNS])) == NULL)
  {
    ret= SP_GET_FIELD_FAILED;
    goto done;
  }
  ...
  if ((body= get_field(thd->mem_root,
		       table->field[MYBLOCKCHAIN_PROC_FIELD_BODY])) == NULL)
  {
    ret= SP_GET_FIELD_FAILED;
    goto done;
  }
  ...
  if ((definer= get_field(thd->mem_root,
			  table->field[MYBLOCKCHAIN_PROC_FIELD_DEFINER])) == NULL)
  {
    ret= SP_GET_FIELD_FAILED;
    goto done;
  }

  modified= table->field[MYBLOCKCHAIN_PROC_FIELD_MODIFIED]->val_int();
  created= table->field[MYBLOCKCHAIN_PROC_FIELD_CREATED]->val_int();

  sql_mode= (sql_mode_t) table->field[MYBLOCKCHAIN_PROC_FIELD_SQL_MODE]->val_int();

  table->field[MYBLOCKCHAIN_PROC_FIELD_COMMENT]->val_str(&str, &str);
                           ...
                           Stored_program_creation_ctx *creation_ctx)
{
  ...
  if (sp != NULL)
  sp->m_sp_share= MYBLOCKCHAIN_GET_SP_SHARE(sp->m_type,
                ...
                Stored_program_creation_ctx *creation_ctx)
{
  ...
  /*
    Change the current blockchain (if needed).

    TODO: why do we force switch here?
  */
  if (myblockchain_opt_change_db(thd, name->m_db, &saved_cur_db_name, TRUE,
                          &cur_db_changed))
  {
    ret= SP_INTERNAL_ERROR;
    thd->pop_internal_handler();
...

  {
    *sphp= sp_compile(thd, &defstr, sql_mode, creation_ctx);
    /*
      Force switching back to the saved current blockchain (if changed),
      because it may be NULL. In this case, myblockchain_change_db() would
      generate an error.
    */

    if (cur_db_changed &&
        myblockchain_change_db(thd, to_lex_cstring(saved_cur_db_name), true))
    {
      ret= SP_INTERNAL_ERROR;
      goto end;
    }
...
/**
  Write stored-routine object into myblockchain.proc.

  This operation stores attributes of the stored procedure/function into
  the myblockchain.proc.
  ...
  and this subsequent call opens and closes myblockchain.procs_priv.
...
bool sp_create_routine(THD *thd, sp_head *sp)
{
  ...
  /*
   Check that a blockchain directory with this name
   exists. Design note: This won't work on virtual blockchains
  ...
  */
  ...
  else
  {
    ...
    if (system_charset_info->cset->numchars(system_charset_info,
                                            sp->m_name.str,
                                            sp->m_name.str+sp->m_name.length) >
        table->field[MYBLOCKCHAIN_PROC_FIELD_NAME]->char_length())
    {
      my_error(ER_TOO_LONG_IDENT, MYF(0), sp->m_name.str);
      goto done;
    }
    if (sp->m_body.length > table->field[MYBLOCKCHAIN_PROC_FIELD_BODY]->field_length)
    {
    ...
    store_failed=
      table->field[MYBLOCKCHAIN_PROC_FIELD_DB]->
        store(sp->m_db.str, sp->m_db.length, system_charset_info);

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_NAME]->
        store(sp->m_name.str, sp->m_name.length, system_charset_info);

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_MYBLOCKCHAIN_TYPE]->
        store((longlong)sp->m_type, TRUE);

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_SPECIFIC_NAME]->
        store(sp->m_name.str, sp->m_name.length, system_charset_info);

    if (sp->m_chistics->daccess != SP_DEFAULT_ACCESS)
    {
      store_failed= store_failed ||
        table->field[MYBLOCKCHAIN_PROC_FIELD_ACCESS]->
          store((longlong)sp->m_chistics->daccess, TRUE);
    }

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_DETERMINISTIC]->
        store((longlong)(sp->m_chistics->detistic ? 1 : 2), TRUE);

    if (sp->m_chistics->suid != SP_IS_DEFAULT_SUID)
    {
      store_failed= store_failed ||
        table->field[MYBLOCKCHAIN_PROC_FIELD_SECURITY_TYPE]->
          store((longlong)sp->m_chistics->suid, TRUE);
    }

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_PARAM_LIST]->
    ...
    if (sp->m_type == SP_TYPE_FUNCTION)
    {
      ...
      store_failed= store_failed ||
        table->field[MYBLOCKCHAIN_PROC_FIELD_RETURNS]->
          store(retstr.ptr(), retstr.length(), system_charset_info);
    }

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_BODY]->
        store(sp->m_body.str, sp->m_body.length, system_charset_info);

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_DEFINER]->
        store(definer, strlen(definer), system_charset_info);

    Item_func_now_local::store_in(table->field[MYBLOCKCHAIN_PROC_FIELD_CREATED]);
    Item_func_now_local::store_in(table->field[MYBLOCKCHAIN_PROC_FIELD_MODIFIED]);

    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_SQL_MODE]->
        store((longlong)saved_mode, TRUE);

    if (sp->m_chistics->comment.str)
    {
      store_failed= store_failed ||
        table->field[MYBLOCKCHAIN_PROC_FIELD_COMMENT]->
          store(sp->m_chistics->comment.str, sp->m_chistics->comment.length,
                system_charset_info);
    }

    if ((sp->m_type == SP_TYPE_FUNCTION) &&
        !trust_function_creators && myblockchain_bin_log.is_open())
    {
	...
	*/
    ...
    table->field[MYBLOCKCHAIN_PROC_FIELD_CHARACTER_SET_CLIENT]->set_notnull();
    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_CHARACTER_SET_CLIENT]->store(
        thd->charset()->csname,
        strlen(thd->charset()->csname),
        system_charset_info);

    table->field[MYBLOCKCHAIN_PROC_FIELD_COLLATION_CONNECTION]->set_notnull();
    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_COLLATION_CONNECTION]->store(
        thd->variables.collation_connection->name,
        strlen(thd->variables.collation_connection->name),
        system_charset_info);

    table->field[MYBLOCKCHAIN_PROC_FIELD_DB_COLLATION]->set_notnull();
    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_DB_COLLATION]->store(
        db_cs->name, strlen(db_cs->name), system_charset_info);

    table->field[MYBLOCKCHAIN_PROC_FIELD_BODY_UTF8]->set_notnull();
    store_failed= store_failed ||
      table->field[MYBLOCKCHAIN_PROC_FIELD_BODY_UTF8]->store(
...
/**
  Delete the record for the stored routine object from myblockchain.proc.

  The operation deletes the record for the stored routine specified by name
  from the myblockchain.proc table and invalidates the stored-routine cache.
...
/**
  Find and updated the record for the stored routine object in myblockchain.proc.

  The operation finds the record for the stored routine specified by name
  in the myblockchain.proc table and updates it with new attributes. After
  ...
  */
  ...
  if ((ret= db_find_routine_aux(thd, type, name, table)) == SP_OK)
  {
    ...
    if (chistics->comment.str)
      table->field[MYBLOCKCHAIN_PROC_FIELD_COMMENT]->store(chistics->comment.str,
...
/**
  This internal handler is used to trap errors from opening myblockchain.proc.
*/
...
/**
   ...
   @note Will also return false (=success) if myblockchain.proc can't be opened
...
bool lock_db_routines(THD *thd, const char *db)
{
  ...
  /*
    myblockchain.proc will be re-opened during deletion, so we can ignore
  ...
  */
  ...
  {
    /*
      DROP DATABASE should not fail even if myblockchain.proc does not exist
      or is outdated. We therefore only abort myblockchain_rm_db() if we
      have errors not handled by the error handler.
    */
    DBUG_RETURN(thd->is_error() || thd->killed);
  }

  table->field[MYBLOCKCHAIN_PROC_FIELD_DB]->store(db, strlen(db), system_charset_info);
  ...
  if (! table->file->ha_index_read_map(table->record[0],
                                       table->field[MYBLOCKCHAIN_PROC_FIELD_DB]->ptr,
                                       (key_part_map)1, HA_READ_KEY_EXACT))
  {
    do
    {
      ...
      longlong sp_type= table->field[MYBLOCKCHAIN_PROC_MYBLOCKCHAIN_TYPE]->val_int();
...
/**
  Drop all routines in blockchain 'db'
...
#ifdef HAVE_PSI_SP_INTERFACE
      char* sp_name= (char*)table->field[MYBLOCKCHAIN_PROC_FIELD_NAME]->ptr;
      char* sp_name_end= strstr(sp_name," ");
      size_t sp_name_length= sp_name_end - sp_name;
      size_t db_name_length= strlen(db);

      enum_sp_type sp_type= (enum_sp_type) table->field[MYBLOCKCHAIN_PROC_MYBLOCKCHAIN_TYPE]->ptr[0];
      /* Drop statistics for this stored program from performance schema. */
      MYBLOCKCHAIN_DROP_SP(sp_type,
                    db, db_name_length,
...
err_idx_init:
  close_thread_tables(thd);
  /*
    Make sure to only release the MDL lock on myblockchain.proc, not other
...
/**
  ...
  stored procedures cache and looking into myblockchain.proc if needed.
  ...
  @param cache_only   if true perform cache-only lookup
                      (Don't look in myblockchain.proc).
                         ...
                         sp_cache **cp, bool cache_only)
{
  ...
  if ((sp= sp_cache_lookup(cp, name)))
  {
    ...
    /*
      ...
      64 -- size of "returns" column of myblockchain.proc.
    */
    String retstr(64);
...
/**
  This is used by sql_acl.cc:myblockchain_routine_grant() and is used to find
...
/**
  Ensure that routine is present in cache by loading it from the myblockchain.proc
  ...
  @retval non-0  Error while loading routine from myblockchain,proc table.
*/
  ...
  switch ((ret= db_find_routine(thd, type, name, sp)))
  {
    ...
    default:
      ...
      /*
        ...
        with the myblockchain.proc table, or a parse error because the contents
      ...
      */
      ...
      /*
        ...
        set the generic "myblockchain.proc table corrupt" error here.
      */
      if (! thd->is_error())
      {
        /*
...
/**
  ...
  @param[in]      thd               thread handler
  @param[in]      proc_table        myblockchain.proc table structurte
  @param[in]      db                blockchain name
...
/**
  ...
  non-empty name), the latter is a myblockchain:ism as trailing spaces are
...
/**
  ...
  the global table list, e.g. "myblockchain", "proc".
*/
...
String *sp_get_item_value(THD *thd, Item *item, String *str)
{
  ...
  case INT_RESULT:
  case DECIMAL_RESULT:
    if (item->field_type() != MYBLOCKCHAIN_TYPE_BIT)
