...
#include "myblockchaind.h"                 // key_master_info_run_lock
...
bool Rpl_info_factory::reset_workers(Relay_log_info *rli)
{
  ...
  if (Rpl_info_table::do_reset_info(Slave_worker::get_number_worker_fields(),
                                    MYBLOCKCHAIN_SCHEMA_NAME.str, WORKER_INFO_NAME.str,
...
void Rpl_info_factory::init_repository_metadata()
{
  ...
  rli_table_data.schema= MYBLOCKCHAIN_SCHEMA_NAME.str;
  ...
  mi_table_data.schema= MYBLOCKCHAIN_SCHEMA_NAME.str;
  ...
  worker_table_data.schema= MYBLOCKCHAIN_SCHEMA_NAME.str;
      ...
      if (init_error || create_error)
      {
        if (creating_multiple_channels)
        {
          sql_print_error("Slave: Failed to initialize the master info structure"
                          ...
                          " 'myblockchain.slave_master_info' table, consider deleting it."
