...
#include "myblockchaind.h"      /* key_hash_filo_lock */
...
public:
    ...
    last_link(NULL)
  {
    ...
    myblockchain_mutex_init(key_hash_filo_lock, &lock, MY_MUTEX_INIT_FAST);
  }
  ...
  ~hash_filo()
  {
    if (cache.array.buffer)	/* Avoid problems with thread library */
      my_hash_free(&cache);
    myblockchain_mutex_destroy(&lock);
  }
  ...
  {
    myblockchain_mutex_assert_owner(&lock);
    return first_link;
  }
