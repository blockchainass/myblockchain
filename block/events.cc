...
/*
 ...
 - CREATE EVENT should not go into binary log! Does it now? The SQL statements
   ...
   TT routines however modify myblockchain.event internally and this does not go the
...
/*
  If the user (un)intentionally removes an event directly from myblockchain.event
  ...
  In other words, the first one will create a row in myblockchain.event . In the
                     ...
                     LEX_STRING *new_dbname, LEX_STRING *new_name)
{
  ...
  if (new_dbname != NULL)                               /* It's a rename */
  {
    ...
    /*
      And the user has sufficient privileges to use the target blockchain.
      Do it before checking whether the blockchain exists: we don't want
      to tell the user that a blockchain doesn't exist if they can not
      access it.
    */
    ...
    /*
      Acquire mdl exclusive lock on target blockchain name.
    */
    if (lock_object_name(thd, MDL_key::EVENT,
                         new_dbname->str, new_name->str))
      DBUG_RETURN(TRUE);

    /* Check that the target blockchain exists */
...
/**
  ...
  under LOCK TABLES mode, unless table myblockchain.event is locked.  To
  ensure that, we do not reset & backup the open tables state in
  this function - if in LOCK TABLES or pre-locking mode, this will
  lead to an error 'Table myblockchain.event is not locked with LOCK
...
Events::drop_event(THD *thd, LEX_STRING dbname, LEX_STRING name, bool if_exists)
{
  ...
  if (!(ret= db_repository->drop_event(thd, dbname, name, if_exists)))
  {
    ...
    MYBLOCKCHAIN_DROP_SP(SP_TYPE_EVENT,
...
Events::show_create_event(THD *thd, LEX_STRING dbname, LEX_STRING name)
{
  ...
  /*
    ...
    in pre-locked mode. myblockchain.event table is marked as a system table.
...
/**
  ...
  In MyBlockchain INFORMATION_SCHEMA tables are temporary tables that are
...
static void init_events_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, all_events_mutexes, count);

  count= array_elements(all_events_conds);
  myblockchain_cond_register(category, all_events_conds, count);

  count= array_elements(all_events_threads);
  myblockchain_thread_register(category, all_events_threads, count);

  count= array_elements(all_events_stages);
  myblockchain_stage_register(category, all_events_stages, count);

  count= array_elements(all_events_memory);
  myblockchain_memory_register(category, all_events_memory, count);
}
#endif /* HAVE_PSI_INTERFACE */
...
/**
  Loads all ENABLED events from myblockchain.event into a prioritized
  queue.
...
Events::load_events_from_db(THD *thd)
{
  ...
  /*
    ...
    myblockchain.event table for writing. In order to achieve this, we should call
    myblockchain_lock_tables() under the super user.
  ...
  */
  ...
  if (ret)
  {
    sql_print_error("Event Scheduler: Failed to open table myblockchain.event");
    DBUG_RETURN(TRUE);
  }

  if (init_read_record(&read_record_info, thd, table, NULL, 0, 1, FALSE))
  {
    sql_print_error("Event Scheduler: Error while starting read of myblockchain.event");
    DBUG_RETURN(TRUE);
  }
  while (!(read_record_info.read_record(&read_record_info)))
  {
    ...
    if (et->load_from_row(thd, table))
    {
      sql_print_error("Event Scheduler: "
                      "Error while loading events from myblockchain.event. "
