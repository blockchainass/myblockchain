...
/**
  ...
  Locking functions for myblockchain.
  ...
  - For each SQL statement myblockchain_lock_tables() is called for all involved
    tables.
    - myblockchain_lock_tables() will call
      table_handler->external_lock(thd,locktype) for each table.
      This is followed by a call to thr_multi_lock() for all tables.

  - When statement is done, we call myblockchain_unlock_tables().
    This will call thr_multi_unlock() followed by
    table_handler->external_lock(thd, F_UNLCK) for each table.

  - Note that myblockchain_unlock_tables() may be called several times as
    MyBlockchain in some cases can free some tables earlier than others.
  ...
  - LOCK TABLE will call myblockchain_lock_tables() for all tables.
    myblockchain_lock_tables() will call
    table_handler->external_lock(thd,locktype) for each table.
    This is followed by a call to thr_multi_lock() for all tables.
  ...
  - When calling UNLOCK TABLES we call myblockchain_unlock_tables() for all
  ...
  we are forced to use myblockchain_lock_merge.
*/
...
static MYBLOCKCHAIN_LOCK *get_lock_data(THD *thd, TABLE **table_ptr, size_t count,
                                 uint flags);
...
static int thr_lock_errno_to_myblockchain[]=
{ 0, ER_LOCK_ABORTED, ER_LOCK_WAIT_TIMEOUT, ER_LOCK_DEADLOCK };

/**
  Perform semantic checks for myblockchain_lock_tables.
  ...
  for (i=0 ; i<count; i++)
  {
    ...
    */
    if (t->s->table_category == TABLE_CATEGORY_LOG &&
        (flags & MYBLOCKCHAIN_LOCK_LOG_TABLE) == 0 &&
        !log_table_write_query)
    {
      /*
    ...
    */
    if (!(flags & MYBLOCKCHAIN_LOCK_IGNORE_GLOBAL_READ_ONLY) && !t->s->tmp_table)
    {
      if (t->reginfo.lock_type >= TL_WRITE_ALLOW_WRITE &&
...
/**
  ...
  @param myblockchain_lock Lock structures to reset.
...
static void reset_lock_data(MYBLOCKCHAIN_LOCK *sql_lock)
{
...
static void reset_lock_data_and_free(MYBLOCKCHAIN_LOCK **myblockchain_lock)
{
  reset_lock_data(*myblockchain_lock);
  my_free(*myblockchain_lock);
  *myblockchain_lock= 0;
}
...
/**
   ...
   @param count        The number of tables to lock.
   @param flags        Options:
                 MYBLOCKCHAIN_LOCK_IGNORE_GLOBAL_READ_ONLY Ignore SET GLOBAL READ_ONLY
                 MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT          Use maximum timeout value.

   @retval  A lock structure pointer on success.
   @retval  NULL if an error or if wait on a lock was killed.
*/

MYBLOCKCHAIN_LOCK *myblockchain_lock_tables(THD *thd, TABLE **tables, size_t count, uint flags)
{
  int rc;
  MYBLOCKCHAIN_LOCK *sql_lock;
  ulong timeout= (flags & MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT) ?
    LONG_TIMEOUT : thd->variables.lock_wait_timeout;

  DBUG_ENTER("myblockchain_lock_tables");
  ...
  rc= thr_lock_errno_to_myblockchain[(int) thr_multi_lock(sql_lock->locks +
                                                   sql_lock->lock_count,
                                                   sql_lock->lock_count,
                                                   &thd->lock_info, timeout,
                                                   thd->mysys_var)];

  DBUG_EXECUTE_IF("myblockchain_lock_tables_kill_query",
  ...
  if (!(flags & MYBLOCKCHAIN_OPEN_IGNORE_KILLED) && thd->killed)
  {
    ...
    if (sql_lock)
    {
      myblockchain_unlock_tables(thd, sql_lock);
      sql_lock= 0;
    }
  }
...
{
  DBUG_ENTER("myblockchain_unlock_tables");
...
/**
  Unlock some of the tables locked by myblockchain_lock_tables.
...
{
  uint i,found;
  DBUG_ENTER("myblockchain_unlock_read_tables");
  ...
  if (locked)
  {
    ...
    for (i=0; i < locked->table_count; i++)
    {
      if (locked->table[i] == table)
      {
        ...
        /* Unlock the table. */
        myblockchain_unlock_some_tables(thd, &table, /* table count */ 1);
...
{
  MYBLOCKCHAIN_LOCK *locked;
  DBUG_ENTER("myblockchain_lock_abort");
...
{
  MYBLOCKCHAIN_LOCK *locked;
  DBUG_ENTER("myblockchain_lock_abort_for_thread");
...
MYBLOCKCHAIN_LOCK *myblockchain_lock_merge(MYBLOCKCHAIN_LOCK *a,MYBLOCKCHAIN_LOCK *b)
{
  ...
  DBUG_ENTER("myblockchain_lock_merge");

  if (!(sql_lock= (MYBLOCKCHAIN_LOCK*)
	my_malloc(key_memory_MYBLOCKCHAIN_LOCK,
                  sizeof(*sql_lock)+
