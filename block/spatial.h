...
protected:
  ...
  /*
    ...
    only by MyBlockchain. There are some operations that only work for one type and
  ...
  /*
    Whether the polygon's data is in WKB form, as is so in MyBlockchain, or it's in
...
public:
  /**
    ...
    bo: byte order, 1 for little endian(ndr), 0 for big endian(xdr); Currently
        it must be always wkb_ndr since it is MyBlockchain's portable geometry format.
...
///        Boost Geometry, or to be only used in MyBlockchain code.
template <typename T>
Gis_wkb_vector<T>::
Gis_wkb_vector(const void *ptr, size_t nbytes, const Flags_t &flags,
...
/*
  ...
  Boost Range concept doesn't forbid so. But inside MyBlockchain we should
...
/**
   ...
   Thus in myblockchain before using the returned geometry, also inside the
