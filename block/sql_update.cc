...
#include "probes_myblockchain.h"             // MYBLOCKCHAIN_UPDATE_START
...
bool myblockchain_update_prepare_table(THD *thd, SELECT_LEX *select)
{
...
bool myblockchain_update(THD *thd,
                  List<Item> &fields,
                  List<Item> &values,
                  ha_rows limit,
...
{
  DBUG_ENTER("myblockchain_update");
  ...
  if (myblockchain_prepare_update(thd, update_table_ref, &covering_keys_for_cond,
                           values))
    DBUG_RETURN(1);
      ...
      */
      ...
      else
      {
        ...
        */
        ...
        IO_CACHE tempfile;
        if (open_cached_file(&tempfile, myblockchain_tmpdir,TEMP_PREFIX,
    ...
    while (true)
    {
      ...
      if (fill_record_n_invoke_before_triggers(thd, fields, values,
        ...
        if (!records_are_comparable(table) || compare_records(table))
        {
          ...
          /*
            In order to keep MyBlockchain legacy behavior, we do this update *after*
...
*/
bool myblockchain_prepare_update(THD *thd, const TABLE_LIST *update_table_ref,
                          key_map *covering_keys_for_cond,
                          List<Item> &update_value_list)
{
  ...
  DBUG_ENTER("myblockchain_prepare_update");
...
/*
  ...
  SYNOPSIS
    myblockchain_multi_update_prepare()
    thd         thread handler
...
int Sql_cmd_update::myblockchain_multi_update_prepare(THD *thd)
{
  LEX *lex= thd->lex;
  ...
  DBUG_ENTER("myblockchain_multi_update_prepare");
  ...
  */
  if (original_multiupdate &&
      open_tables_for_query(thd, table_list,
                            ...
                            MYBLOCKCHAIN_OPEN_FORCE_SHARED_MDL : 0))
    DBUG_RETURN(true);
...
bool myblockchain_multi_update(THD *thd,
                        List<Item> *fields,
                        List<Item> *values,
                        enum enum_duplicates handle_duplicates,
...
{
  bool res;
  DBUG_ENTER("myblockchain_multi_update");
    ...
    do
    {
      ...
      */
      ...
      /*
        ...
        table to be updated was created by myblockchain 4.1. Deny this.
      */
                                             ...
                                             bool *switch_to_multitable)
{
  ...
  */
  ...
  MYBLOCKCHAIN_UPDATE_START(const_cast<char*>(thd->query().str));

  // Need to open to check for multi-update
  if (open_tables_for_query(thd, all_tables, 0) ||
      myblockchain_update_prepare_table(thd, select_lex) ||
  ...
  if (!all_tables->is_multiple_tables())
  {
    ...
    const bool res= myblockchain_update(thd, select_lex->item_list,
                                 update_value_list,
    ...
    MYBLOCKCHAIN_UPDATE_DONE(res, found, updated);
    if (res)
      return true;
    else
    {
  ...
  else
  {
    ...
    MYBLOCKCHAIN_UPDATE_DONE(true, 0, 0);
...
bool Sql_cmd_update::execute_multi_table_update(THD *thd)
{
  ...
  /*
    ...
    the myblockchain_multi_update_prepare() function will reset it below.
  */
  const bool have_table_map_for_update= thd->table_map_for_update;
#endif

  res= myblockchain_multi_update_prepare(thd);
...
#endif
  {
    ...
    MYBLOCKCHAIN_MULTI_UPDATE_START(const_cast<char*>(thd->query().str));
    res= myblockchain_multi_update(thd,
    ...
    if (result_obj)
    {
      MYBLOCKCHAIN_MULTI_UPDATE_DONE(res, result_obj->num_found(),
    ...
    {
      MYBLOCKCHAIN_MULTI_UPDATE_DONE(1, 0, 0);
    }
  }
  return res;
}
...
bool Sql_cmd_update::prepared_statement_test(THD *thd)
{
  ...
  {
    int res= myblockchain_test_update(thd);
    /* myblockchain_test_update returns 2 if we need to switch to multi-update */
    if (res == 2)
