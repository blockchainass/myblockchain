...
#include "myblockchaind.h"                      // max_connections
...
myblockchain_mutex_t Per_thread_connection_handler::LOCK_thread_cache;
myblockchain_cond_t Per_thread_connection_handler::COND_thread_cache;
myblockchain_cond_t Per_thread_connection_handler::COND_flush_thread_cache;
...
void Per_thread_connection_handler::init()
{
  ...
  myblockchain_mutex_register("sql", all_per_thread_mutexes, count);

  count= array_elements(all_per_thread_conds);
  myblockchain_cond_register("sql", all_per_thread_conds, count);
#endif

  myblockchain_mutex_init(key_LOCK_thread_cache, &LOCK_thread_cache,
                   MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_thread_cache, &COND_thread_cache);
  myblockchain_cond_init(key_COND_flush_thread_cache, &COND_flush_thread_cache);
...
void Per_thread_connection_handler::destroy()
{
  if (waiting_channel_info_list != NULL)
  {
    ...
    waiting_channel_info_list= NULL;
    myblockchain_mutex_destroy(&LOCK_thread_cache);
    myblockchain_cond_destroy(&COND_thread_cache);
    myblockchain_cond_destroy(&COND_flush_thread_cache);
  }
}
...
Channel_info* Per_thread_connection_handler::block_until_new_connection()
{
  ...
  myblockchain_mutex_lock(&LOCK_thread_cache);
      ...
      !kill_blocked_pthreads_flag)
  {
    ...
    */
    ...
    while (!abort_loop && !wake_pthread && !kill_blocked_pthreads_flag)
      myblockchain_cond_wait(&COND_thread_cache, &LOCK_thread_cache);
    blocked_pthread_count--;

    if (kill_blocked_pthreads_flag)
      myblockchain_cond_signal(&COND_flush_thread_cache);
  ...
  myblockchain_mutex_unlock(&LOCK_thread_cache);
  return new_conn;
}
...
extern "C" void *handle_connection(void *arg)
{
  ...
  for (;;)
  {
    ...
    myblockchain_thread_set_psi_id(thd->thread_id());
    myblockchain_thread_set_psi_THD(thd);
    myblockchain_socket_set_thread_owner(
      thd->get_protocol_classic()->get_vio()->myblockchain_socket);
...
void Per_thread_connection_handler::kill_blocked_pthreads()
{
  ...
  {
    myblockchain_cond_broadcast(&COND_thread_cache);
    myblockchain_cond_wait(&COND_flush_thread_cache, &LOCK_thread_cache);
  }
  kill_blocked_pthreads_flag--;
                                                  ...
                                                  Channel_info* channel_info)
{
  ...
  if (Per_thread_connection_handler::blocked_pthread_count > wake_pthread)
  {
    ...
    wake_pthread++;
    myblockchain_cond_signal(&COND_thread_cache);
    res= false;
  }
  myblockchain_mutex_unlock(&LOCK_thread_cache);
...
bool Per_thread_connection_handler::add_connection(Channel_info* channel_info)
{
  ...
  */
  ...
  error= myblockchain_thread_create(key_thread_one_connection, &id,
                             &connection_attrib,
                             handle_connection,
                             (void*) channel_info);
#ifndef DBUG_OFF
