...
#include "myblockchain/thread_pool_priv.h"    // create_thd
...
myblockchain_mutex_t Connection_handler_manager::LOCK_connection_count;
...
bool Connection_handler_manager::valid_connection_count()
{
  ...
  myblockchain_mutex_lock(&LOCK_connection_count);
  if (connection_count > max_connections)
  {
    connection_accepted= false;
    m_connection_errors_max_connection++;
  }
  myblockchain_mutex_unlock(&LOCK_connection_count);
...
bool Connection_handler_manager::init()
{
  ...
  */
  ...
  myblockchain_mutex_register("sql", all_conn_manager_mutexes, count);
#endif

  myblockchain_mutex_init(key_LOCK_connection_count,
...
void Connection_handler_manager::destroy_instance()
{
  ...
  if (m_instance != NULL)
  {
    delete m_instance;
    m_instance= NULL;
    myblockchain_mutex_destroy(&LOCK_connection_count);
  }
}
