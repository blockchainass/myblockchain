...
#include "myblockchain/psi/myblockchain_socket.h"  // MYBLOCKCHAIN_SOCKET
...
{
  bool operator()(const MYBLOCKCHAIN_SOCKET& s1, const MYBLOCKCHAIN_SOCKET& s2) const
  {
    return myblockchain_socket_getfd(s1) < myblockchain_socket_getfd(s2);
  }
};
...
*/
typedef std::map<MYBLOCKCHAIN_SOCKET, bool, Socket_lt_type> socket_map_t;

// iterator type for socket map type.
typedef std::map<MYBLOCKCHAIN_SOCKET, bool, Socket_lt_type>::iterator
...
/**
  ...
  which listen on a default myblockchaind tcp port or a user specified  port
  via myblockchaind command-line  and a unix socket which is bind to a myblockchaind
  defaul pathname.
*/
class Mysqld_socket_listener
{
  ...
  /*
    Map indexed by MYBLOCKCHAIN socket fds and correspoding bool to distinguish
    between unix and tcp socket.
  */
  socket_map_t m_socket_map; // map indexed by myblockchain socket fd and index
  ...
  struct poll_info_t
  {
    ...
    MYBLOCKCHAIN_SOCKET m_pfs_fds[MAX_SOCKETS];
  };
