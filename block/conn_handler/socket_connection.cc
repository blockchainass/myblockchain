...
#include "myblockchaind.h"                     // key_socket_tcpip
...
void net_before_header_psi(struct st_net *net, void *user_data, size_t /* unused: count */)
{
  ...
  if (thd->m_server_idle)
  {
    ...
    */
    MYBLOCKCHAIN_SOCKET_SET_STATE(net->vio->myblockchain_socket, PSI_SOCKET_STATE_IDLE);
    MYBLOCKCHAIN_START_IDLE_WAIT(thd->m_idle_psi, &thd->m_idle_state);
  }
}
...
void net_after_header_psi(struct st_net *net, void *user_data, size_t /* unused: count */, my_bool rc)
{
  ...
  if (thd->m_server_idle)
  {
    ...
    */
    MYBLOCKCHAIN_END_IDLE_WAIT(thd->m_idle_psi);
    ...
    */
    MYBLOCKCHAIN_SOCKET_SET_STATE(net->vio->myblockchain_socket, PSI_SOCKET_STATE_ACTIVE);
...
static void init_net_server_extension(THD *thd)
{
  ...
  /* Activate this private extension for the myblockchaind server. */
...
class Channel_info_local_socket : public Channel_info
{
  ...
  MYBLOCKCHAIN_SOCKET m_connect_sock;

protected:
  virtual Vio* create_and_init_vio() const
  {
    return myblockchain_socket_vio_new(m_connect_sock, VIO_TYPE_SOCKET, VIO_LOCALHOST);
  }
...
public:
  ...
  */
  Channel_info_local_socket(MYBLOCKCHAIN_SOCKET connect_socket)
                                            ...
                                            bool senderror)
  {
    ...
    myblockchain_socket_shutdown(m_connect_sock, SHUT_RDWR);
    myblockchain_socket_close(m_connect_sock);
  }
};
...
protected:
  virtual Vio* create_and_init_vio() const
  {
    return myblockchain_socket_vio_new(m_connect_sock, VIO_TYPE_TCPIP, 0);
  }
...
public:
  /**
    ...
    @param connect_socket set connect socket descriptor.
  */
  Channel_info_tcpip_socket(MYBLOCKCHAIN_SOCKET connect_socket)
  : m_connect_sock(connect_socket)
  { }
...
class TCP_socket
{
  ...
  MYBLOCKCHAIN_SOCKET create_socket(const struct addrinfo *addrinfo_list,
                             int addr_family,
                             struct addrinfo **use_addrinfo)
  {
         ...
         cur_ai= cur_ai->ai_next)
    {
      ...
      MYBLOCKCHAIN_SOCKET sock= myblockchain_socket_socket(key_socket_tcpip,
                                             cur_ai->ai_family,
                                             cur_ai->ai_socktype,
                                             cur_ai->ai_protocol);
      ...
      if (myblockchain_socket_getfd(sock) == INVALID_SOCKET)
      {
    ...
    return MYBLOCKCHAIN_INVALID_SOCKET;
  }
...
public:
  ...
  */
  ...
  /**
    ...
    @retval   valid socket if successful else MYBLOCKCHAIN_INVALID_SOCKET on failure.
  */
  MYBLOCKCHAIN_SOCKET get_listener_socket()
  {
    struct addrinfo *ai;
      ...
      */
      ...
      if (!getaddrinfo(ipv6_all_addresses, port_buf, &hints, &ai))
      {
        ...
        */
        ...
        MYBLOCKCHAIN_SOCKET s= myblockchain_socket_socket(0, AF_INET6, SOCK_STREAM, 0);
        ipv6_available= myblockchain_socket_getfd(s) != INVALID_SOCKET;
        myblockchain_socket_close(s);
      }
      if (ipv6_available)
      {
    ...
    */
    ...
    struct addrinfo *a;
    MYBLOCKCHAIN_SOCKET listener_socket= create_socket(ai, AF_INET, &a);

    if (myblockchain_socket_getfd(listener_socket) == INVALID_SOCKET)
    ...
    myblockchain_socket_set_thread_owner(listener_socket);

#ifndef _WIN32
    /*
      We should not use SO_REUSEADDR on windows as this would enable a
      user to open two myblockchaind servers with the same TCP/IP port.
    */
    {
      int option_flag= 1;
      (void) myblockchain_socket_setsockopt(listener_socket, SOL_SOCKET, SO_REUSEADDR,
    ...
    if (a->ai_family == AF_INET6)
    {
      ...
      if (myblockchain_socket_setsockopt(listener_socket, IPPROTO_IPV6, IPV6_V6ONLY,
                                  (char *) &option_flag, sizeof (option_flag)))
      {
    ...
    */
    ...
    {
      if (((ret= myblockchain_socket_bind(listener_socket, a->ai_addr, a->ai_addrlen)) >= 0 ) ||
          (socket_errno != SOCKET_EADDRINUSE) ||
          (waited >= m_port_timeout))
        break;
      sql_print_information("Retrying bind on TCP/IP port %u", myblockchaind_port);
    ...
    if (ret < 0)
    {
      ...
      sql_print_error("Can't start server: Bind on TCP/IP port: %s",
                      strerror(errno));
      sql_print_error("Do you already have another myblockchaind server running on port: %d ?",m_tcp_port);
      myblockchain_socket_close(listener_socket);
      return MYBLOCKCHAIN_INVALID_SOCKET;
    }

    if (myblockchain_socket_listen(listener_socket, static_cast<int>(m_backlog)) < 0)
    {
    ...
    (void) myblockchain_sock_set_nonblocking(listener_socket);
#endif
...
class Unix_socket
{
  ...
  /**
    Create a lockfile which contains the pid of the myblockchaind instance started
...
public:
  ...
  */
  MYBLOCKCHAIN_SOCKET get_listener_socket()
  {
    ...
    MYBLOCKCHAIN_SOCKET listener_socket= myblockchain_socket_socket(key_socket_unix, AF_UNIX,
                                                      SOCK_STREAM, 0);

    if (myblockchain_socket_getfd(listener_socket) < 0)
    {
    ...
    // bind
    umask(0);
    if (myblockchain_socket_bind(listener_socket,
                          reinterpret_cast<struct sockaddr *> (&UNIXaddr),
                          sizeof(UNIXaddr)) < 0)
    {
      sql_print_error("Can't start server : Bind on unix socket: %s",
                      strerror(errno));
      sql_print_error("Do you already have another myblockchaind server running on socket: %s ?",
                      m_unix_sockname.c_str());
      myblockchain_socket_close(listener_socket);
      return MYBLOCKCHAIN_INVALID_SOCKET;
    }
    ...
    // listen
    if (myblockchain_socket_listen(listener_socket, (int)m_backlog) < 0)
...
bool Mysqld_socket_listener::setup_listener()
{
  ...
  if (m_tcp_port)
  {
    ...
    MYBLOCKCHAIN_SOCKET myblockchain_socket= tcp_socket.get_listener_socket();
    if (myblockchain_socket.fd == INVALID_SOCKET)
      return true;

    m_socket_map.insert(std::pair<MYBLOCKCHAIN_SOCKET,bool>(myblockchain_socket, false));
  }
#if defined(HAVE_SYS_UN_H)
  ...
  if (m_unix_sockname != "")
  {
    ...
    MYBLOCKCHAIN_SOCKET myblockchain_socket= unix_socket.get_listener_socket();
    if (myblockchain_socket.fd == INVALID_SOCKET)
      return true;

    m_socket_map.insert(std::pair<MYBLOCKCHAIN_SOCKET,bool>(myblockchain_socket, true));
  ...
  {
    MYBLOCKCHAIN_SOCKET listen_socket= sock_map_iter->first;
    myblockchain_socket_set_thread_owner(listen_socket);
#ifdef HAVE_POLL
    m_poll_info.m_fds[count].fd= myblockchain_socket_getfd(listen_socket);
    m_poll_info.m_fds[count].events= POLLIN;
    m_poll_info.m_pfs_fds[count]= listen_socket;
    count++;
#else  // HAVE_POLL
    FD_SET(myblockchain_socket_getfd(listen_socket), &m_select_info.m_client_fds);
    if ((uint) myblockchain_socket_getfd(listen_socket) > m_select_info.m_max_used_connection)
      m_select_info.m_max_used_connection= myblockchain_socket_getfd(listen_socket);
#endif // HAVE_POLL
  }
  return false;
}
...
Channel_info* Mysqld_socket_listener::listen_for_connection_event()
{
  ...
  if (retval < 0 && socket_errno != SOCKET_EINTR)
  {
    ...
    */
    ...
    if (!select_errors++ && !abort_loop)
      sql_print_error("myblockchaind: Got error %d from select",socket_errno);
  }
  ...
  MYBLOCKCHAIN_SOCKET listen_sock= MYBLOCKCHAIN_INVALID_SOCKET;
  ...
  for (socket_map_iterator_t sock_map_iter=  m_socket_map.begin();
       sock_map_iter != m_socket_map.end(); ++sock_map_iter)
  {
    if (FD_ISSET(myblockchain_socket_getfd(sock_map_iter->first),
  ...
  MYBLOCKCHAIN_SOCKET connect_sock;
  struct sockaddr_storage cAddr;
  for (uint retry= 0; retry < MAX_ACCEPT_RETRY; retry++)
  {
    socket_len_t length= sizeof(struct sockaddr_storage);
    connect_sock= myblockchain_socket_accept(key_socket_client_connection, listen_sock,
                                      (struct sockaddr *)(&cAddr), &length);
    if (myblockchain_socket_getfd(connect_sock) != INVALID_SOCKET ||
...
#ifdef __APPLE__
  if (myblockchain_socket_getfd(connect_sock) >= FD_SETSIZE)
  {
    sql_print_warning("File Descriptor %d exceedeed FD_SETSIZE=%d",
                      myblockchain_socket_getfd(connect_sock), FD_SETSIZE);
    connection_errors_internal++;
    (void) myblockchain_socket_close(connect_sock);
    return NULL;
  }
#endif
  ...
  if (!is_unix_socket)
  {
    ...
    request_init(&req, RQ_DAEMON, m_libwrap_name, RQ_FILE,
                 myblockchain_socket_getfd(connect_sock), NULL);
    fromhost(&req);
    ...
    if (!hosts_access(&req))
    {
      ...
      */
      ...
      myblockchain_socket_shutdown(listen_sock, SHUT_RDWR);
      myblockchain_socket_close(listen_sock);
      /*
  ...
  {
    (void) myblockchain_socket_shutdown(connect_sock, SHUT_RDWR);
    (void) myblockchain_socket_close(connect_sock);
    connection_errors_internal++;
    return NULL;
  }
...
void Mysqld_socket_listener::close_listener()
{
  for (socket_map_iterator_t sock_map_iter=  m_socket_map.begin();
       sock_map_iter != m_socket_map.end(); ++sock_map_iter)
  {
    (void) myblockchain_socket_shutdown(sock_map_iter->first,SHUT_RDWR);
    (void) myblockchain_socket_close(sock_map_iter->first);
  }
