                              ...
                              enum_field_types type, bool send_error)
{
  ...
  case MYBLOCKCHAIN_TYPE_NEWDATE:
    if (!str_to_datetime(cs, str, length, &ltime, flags, &status) &&
        ltime.time_type == MYBLOCKCHAIN_TIMESTAMP_DATE && !status.warnings)
      item= new (thd->mem_root) Item_date_literal(&ltime);
    break;
  case MYBLOCKCHAIN_TYPE_DATETIME:
    if (!str_to_datetime(cs, str, length, &ltime, flags, &status) &&
        ltime.time_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME && !status.warnings)
  ...
  case MYBLOCKCHAIN_TYPE_TIME:
    if (!str_to_time(cs, str, length, &ltime, 0, &status) &&
        ltime.time_type == MYBLOCKCHAIN_TIMESTAMP_TIME && !status.warnings)
  ...
  {
    const char *typestr=
      (type == MYBLOCKCHAIN_TYPE_DATE) ? "DATE" :
      (type == MYBLOCKCHAIN_TYPE_TIME) ? "TIME" : "DATETIME";
