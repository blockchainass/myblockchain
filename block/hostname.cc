...
void Host_errors::aggregate(const Host_errors *errors)
{
  ...
  m_default_blockchain+= errors->m_default_blockchain;
...
{
  myblockchain_mutex_lock(&hostname_cache->lock);
}

void hostname_cache_unlock()
{
  myblockchain_mutex_unlock(&hostname_cache->lock);
}
...
/**
  ...
  The function does not set/report MyBlockchain server error in case of failure.
