...
set_field_to_null_with_conversions(Field *field, bool no_conversions)
{
  ...
  /*
    ...
    no special value.
  */
  if (field->type() == MYBLOCKCHAIN_TYPE_TIMESTAMP &&
...
{
  if (to->type() == MYBLOCKCHAIN_TYPE_NULL)
  {
    to_null_ptr=0;				// For easy debugging
    to_ptr=0;
    m_do_copy= do_skip;
  ...
  if (from->maybe_null())
  {
    ...
    {
      if (m_to_field->type() == MYBLOCKCHAIN_TYPE_TIMESTAMP)
  ...
  {
    if (to->real_type() == MYBLOCKCHAIN_TYPE_JSON &&
        from->real_type() == MYBLOCKCHAIN_TYPE_JSON)
...
Copy_field::get_copy_func(Field *to,Field *from)
{
  ...
  {
    if (to->real_type() == MYBLOCKCHAIN_TYPE_BIT ||
        from->real_type() == MYBLOCKCHAIN_TYPE_BIT)
      return do_field_int;
    ...
    if (from->result_type() == STRING_RESULT)
    {
      ...
      /*
        ...
        repairs data.
      */
      if (from->type() == MYBLOCKCHAIN_TYPE_VAR_STRING && !from->has_charset() &&
          to->type() == MYBLOCKCHAIN_TYPE_VARCHAR && !to->has_charset())
      ...
      */
      if (to->real_type() != from->real_type() ||
          ...
          (((to->table->in_use->variables.sql_mode &
           ...
           to->type() == MYBLOCKCHAIN_TYPE_DATE) ||
           to->type() == MYBLOCKCHAIN_TYPE_DATETIME))
      {
	if (from->real_type() == MYBLOCKCHAIN_TYPE_ENUM ||
	    from->real_type() == MYBLOCKCHAIN_TYPE_SET)
	  if (to->result_type() != STRING_RESULT)
	    return do_field_int;		// Convert SET to number
	return do_field_string;
      }
      if (to->real_type() == MYBLOCKCHAIN_TYPE_ENUM ||
	  to->real_type() == MYBLOCKCHAIN_TYPE_SET)
      {
	if (!to->eq_def(from))
        {
          if (from->real_type() == MYBLOCKCHAIN_TYPE_ENUM &&
              to->real_type() == MYBLOCKCHAIN_TYPE_ENUM)
	...
	return do_field_string;
      else if (to->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR)
      {
      ...
      else if (m_to_length > m_from_length)
      {
        if (to->charset() == &my_charset_bin)
          ...
          return do_expand_string;
             ...
             !compatible_db_low_byte_first)
    {
      if (to->real_type() == MYBLOCKCHAIN_TYPE_DECIMAL ||
...
{
  return (to->type() == MYBLOCKCHAIN_TYPE_BLOB || to->type() == MYBLOCKCHAIN_TYPE_GEOMETRY);
}
...
type_conversion_status field_conv(Field *to,Field *from)
{
  ...
  if ((to_type == MYBLOCKCHAIN_TYPE_JSON) && (from_type == MYBLOCKCHAIN_TYPE_JSON))
  {
  ...
  {
    if (to->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR &&
        from->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR)
    {
	...
	to->real_type() != MYBLOCKCHAIN_TYPE_ENUM &&
	to->real_type() != MYBLOCKCHAIN_TYPE_SET &&
        to->real_type() != MYBLOCKCHAIN_TYPE_BIT &&
        (!to->is_temporal_with_time() ||
         to->decimals() == from->decimals()) &&
        (to->real_type() != MYBLOCKCHAIN_TYPE_NEWDECIMAL ||
        ...
        (!(to->table->in_use->variables.sql_mode &
         ...
         (to->type() != MYBLOCKCHAIN_TYPE_DATE &&
          ...
          (!to->table->in_use->variables.explicit_defaults_for_timestamp ||
           to->type() != MYBLOCKCHAIN_TYPE_TIMESTAMP))) &&
         (from->real_type() != MYBLOCKCHAIN_TYPE_VARCHAR))
    {						// Identical fields
  ...
  if (to->type() == MYBLOCKCHAIN_TYPE_BLOB)
    ...
    */
    if (to->table->copy_blobs ||
        (!blob->value.is_alloced() &&
         from->real_type() != MYBLOCKCHAIN_TYPE_STRING &&
         from->real_type() != MYBLOCKCHAIN_TYPE_VARCHAR))
      blob->value.copy();
  ...
  else if (from->is_temporal() && to->result_type() == INT_RESULT)
  {
    MYBLOCKCHAIN_TIME ltime;
    longlong nr;
    if (from->type() == MYBLOCKCHAIN_TYPE_TIME)
    {
      from->get_time(&ltime);
  ...
  else if (from_type == MYBLOCKCHAIN_TYPE_JSON &&
           (to_type == MYBLOCKCHAIN_TYPE_TINY ||
            to_type == MYBLOCKCHAIN_TYPE_SHORT ||
            to_type == MYBLOCKCHAIN_TYPE_INT24 ||
            to_type == MYBLOCKCHAIN_TYPE_LONG ||
            to_type == MYBLOCKCHAIN_TYPE_LONGLONG))
  {
    return to->store(from->val_int(), from->flags & UNSIGNED_FLAG);
  }
  else if (from_type == MYBLOCKCHAIN_TYPE_JSON &&
           to_type == MYBLOCKCHAIN_TYPE_NEWDECIMAL)
  {
    my_decimal buff;
    return to->store_decimal(from->val_decimal(&buff));
  }
  else if (from_type == MYBLOCKCHAIN_TYPE_JSON &&
           (to_type == MYBLOCKCHAIN_TYPE_FLOAT ||
            to_type == MYBLOCKCHAIN_TYPE_DOUBLE))
  {
           ...
           to->is_temporal())
  {
    ...
    return to->store_time(&ltime);
            ...
            (to->result_type() == STRING_RESULT ||
             (from->real_type() != MYBLOCKCHAIN_TYPE_ENUM &&
              from->real_type() != MYBLOCKCHAIN_TYPE_SET))) ||
           to->type() == MYBLOCKCHAIN_TYPE_DECIMAL)
  {
    char buff[MAX_FIELD_WIDTH];
