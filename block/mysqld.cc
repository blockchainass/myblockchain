...
#include "myblockchaind_daemon.h"
...
#include "sql_test.h"     // myblockchain_print_status
...
#include "myblockchaind_suffix.h"
#include "mysys_err.h"
#include "events.h"
#include "sql_audit.h"
...
#include <myblockchain/psi/myblockchain_idle.h>
#include <myblockchain/psi/myblockchain_socket.h>
#include <myblockchain/psi/myblockchain_memory.h>
#include <myblockchain/psi/myblockchain_statement.h>

#include "myblockchain_com_server.h"
#include "keycaches.h"
...
static char *myblockchaind_user, *myblockchaind_chroot;
...
static char compiled_default_collation_name[]= MYBLOCKCHAIN_DEFAULT_COLLATION_NAME;
...
#if MYBLOCKCHAIN_VERSION_ID >= 50800
#error "show_compatibility_56 is to be removed in MyBlockchain 5.8"
#else
/*
...
#endif /* MYBLOCKCHAIN_VERSION_ID >= 50800 */
...
myblockchain_mutex_t LOCK_default_password_lifetime;

#if defined(ENABLED_DEBUG_SYNC)
MYBLOCKCHAIN_PLUGIN_IMPORT uint    opt_debug_sync_timeout= 0;
#endif /* defined(ENABLED_DEBUG_SYNC) */
my_bool opt_old_style_user_limits= 0, trust_function_creators= 0;
my_bool check_proxy_users= 0, myblockchain_native_password_proxy_users= 0, sha256_password_proxy_users= 0;
/*
...
uint myblockchaind_port, test_flags, select_errors, dropping_tables, ha_open_options;
uint myblockchaind_port_timeout;
...
char myblockchain_home[FN_REFLEN], pidfile_name[FN_REFLEN], system_time_zone[30];
char default_logfile_name[FN_REFLEN];
char *default_tz_name;
static char errorlog_filename_buff[FN_REFLEN];
const char *log_error_dest;
char glob_hostname[FN_REFLEN];
char myblockchain_real_data_home[FN_REFLEN],
     lc_messages_dir[FN_REFLEN], reg_ext[FN_EXTLEN],
     myblockchain_charsets_dir[FN_REFLEN],
     *opt_init_file, *opt_tc_log_file;
char *lc_messages_dir_ptr;
char myblockchain_unpacked_real_data_home[FN_REFLEN];
size_t myblockchain_unpacked_real_data_home_len;
size_t myblockchain_real_data_home_len, myblockchain_data_home_len= 1;
uint reg_ext_length;
...
char *myblockchain_data_home= const_cast<char*>(".");
const char *myblockchain_real_data_home_ptr= myblockchain_real_data_home;
char server_version[SERVER_VERSION_LENGTH];
char *myblockchaind_unix_port, *opt_myblockchain_tmpdir;
...
MY_TMPDIR myblockchain_tmpdir_list;
MY_BITMAP temp_pool;
...
*/
myblockchain_mutex_t LOCK_prepared_stmt_count;
...
/*
 The below two locks are introudced as guards (second mutex) for
  ...
  for more details
*/
myblockchain_mutex_t LOCK_sql_slave_skip_counter;
myblockchain_mutex_t LOCK_slave_net_timeout;
myblockchain_mutex_t LOCK_log_throttle_qni;
myblockchain_mutex_t LOCK_offline_mode;
#ifdef HAVE_OPENSSL
myblockchain_mutex_t LOCK_des_key_file;
#endif
myblockchain_rwlock_t LOCK_sys_init_connect, LOCK_sys_init_slave;
myblockchain_rwlock_t LOCK_system_variables_hash;
my_thread_handle signal_thread_id;
my_thread_attr_t connection_attrib;
myblockchain_mutex_t LOCK_server_started;
myblockchain_cond_t COND_server_started;
myblockchain_mutex_t LOCK_reset_gtid_table;
myblockchain_mutex_t LOCK_compress_gtid_table;
myblockchain_cond_t COND_compress_gtid_table;
#if !defined (EMBEDDED_LIBRARY) && !defined(_WIN32)
myblockchain_mutex_t LOCK_socket_listener_active;
myblockchain_cond_t COND_socket_listener_active;
myblockchain_mutex_t LOCK_start_signal_handler;
myblockchain_cond_t COND_start_signal_handler;
#endif

bool myblockchaind_server_started= false;
...
char *myblockchain_home_ptr, *pidfile_name_ptr;
...
Connection_acceptor<Mysqld_socket_listener> *myblockchaind_socket_acceptor= NULL;
#ifdef _WIN32
...
{
  myblockchain_mutex_lock(&LOCK_sql_rand);
  ulong tmp=(ulong) (my_rnd(&sql_rand) * 0xffffffff); /* make all bits random */
  myblockchain_mutex_unlock(&LOCK_sql_rand);
  return tmp;
}
...
void Buffered_log::print()
{
  ...
  */
  ...
  case INFORMATION_LEVEL:
    /*
      Messages printed as "information" still end up in the myblockchaind *error* log,
...
/**
  ...
  warnings and errors inside an already running myblockchaind server,
  e.g. when a character set or collation is requested for the very first time
  and its initialization does not go well for some reasons.

  Note: At early myblockchaind initialization stage,
...
static myblockchain_mutex_t LOCK_handler_count;
static myblockchain_cond_t COND_handler_count;
...
#ifndef EMBEDDED_LIBRARY
bool myblockchaind_embedded=0;
#else
bool myblockchaind_embedded=1;
#endif
...
static int myblockchain_init_variables(void);
static int get_options(int *argc_ptr, char ***argv_ptr);
static void add_terminator(vector<my_option> *options);
extern "C" my_bool myblockchaind_get_one_option(int, const struct my_option *, char *);
...
static void myblockchaind_exit(int exit_code) __attribute__((noreturn));
...
public:
  ...
  virtual void operator()(THD *killing_thd)
  {
    ...
    myblockchain_mutex_lock(&killing_thd->LOCK_thd_data);
    killing_thd->killed= THD::KILL_CONNECTION;
    MYBLOCKCHAIN_CALLBACK(Connection_handler_manager::event_functions,
                   post_kill_notification, (killing_thd));
    if (killing_thd->mysys_var)
    {
      myblockchain_mutex_lock(&killing_thd->LOCK_current_cond);
      killing_thd->mysys_var->abort= 1;
      if (killing_thd->current_cond)
      {
        myblockchain_mutex_lock(killing_thd->current_mutex);
        myblockchain_cond_broadcast(killing_thd->current_cond);
        myblockchain_mutex_unlock(killing_thd->current_mutex);
      }
      myblockchain_mutex_unlock(&killing_thd->LOCK_current_cond);
    }
    myblockchain_mutex_unlock(&killing_thd->LOCK_thd_data);
  }
};
...
static void close_connections(void)
{
  ...
  // Close listeners.
  if (myblockchaind_socket_acceptor != NULL)
    myblockchaind_socket_acceptor->close_listener();
#ifdef _WIN32
...
void kill_myblockchain(void)
{
  DBUG_ENTER("kill_myblockchain");
...
extern "C" void unireg_abort(int exit_code)
{
  ...
  if (opt_daemonize)
  {
    myblockchaind::runtime::signal_parent(pipe_write_fd,0);
  }
#endif

  clean_up(!opt_help && (exit_code || !opt_bootstrap)); /* purecov: inspected */
  DBUG_PRINT("quit",("done with cleanup in unireg_abort"));
  myblockchaind_exit(exit_code);
}

static void myblockchaind_exit(int exit_code)
{
  DBUG_ASSERT(exit_code >= MYBLOCKCHAIND_SUCCESS_EXIT
              && exit_code <= MYBLOCKCHAIND_FAILURE_EXIT);
  log_syslog_exit();
  myblockchain_audit_finalize();
...
{
  delete myblockchaind_socket_acceptor;
  myblockchaind_socket_acceptor= NULL;
...
void clean_up(bool print_message)
{
  ...
  */
  ...
  free_tmpdir(&myblockchain_tmpdir_list);
  ...
  myblockchain_client_plugin_deinit();
  finish_client_errs();
...
{
  myblockchain_mutex_destroy(&LOCK_log_throttle_qni);
  myblockchain_mutex_destroy(&LOCK_status);
  myblockchain_mutex_destroy(&LOCK_manager);
  myblockchain_mutex_destroy(&LOCK_crypt);
  myblockchain_mutex_destroy(&LOCK_user_conn);
#ifdef HAVE_OPENSSL
  myblockchain_mutex_destroy(&LOCK_des_key_file);
#endif
  myblockchain_mutex_destroy(&LOCK_msr_map);
  myblockchain_rwlock_destroy(&LOCK_sys_init_connect);
  myblockchain_rwlock_destroy(&LOCK_sys_init_slave);
  myblockchain_mutex_destroy(&LOCK_global_system_variables);
  myblockchain_rwlock_destroy(&LOCK_system_variables_hash);
  myblockchain_mutex_destroy(&LOCK_uuid_generator);
  myblockchain_mutex_destroy(&LOCK_sql_rand);
  myblockchain_mutex_destroy(&LOCK_prepared_stmt_count);
  myblockchain_mutex_destroy(&LOCK_sql_slave_skip_counter);
  myblockchain_mutex_destroy(&LOCK_slave_net_timeout);
  myblockchain_mutex_destroy(&LOCK_error_messages);
  myblockchain_mutex_destroy(&LOCK_offline_mode);
  myblockchain_mutex_destroy(&LOCK_default_password_lifetime);
  myblockchain_cond_destroy(&COND_manager);
#ifdef _WIN32
  myblockchain_cond_destroy(&COND_handler_count);
  myblockchain_mutex_destroy(&LOCK_handler_count);
#endif
#ifndef _WIN32
  myblockchain_cond_destroy(&COND_socket_listener_active);
  myblockchain_mutex_destroy(&LOCK_socket_listener_active);
  myblockchain_cond_destroy(&COND_start_signal_handler);
  myblockchain_mutex_destroy(&LOCK_start_signal_handler);
#endif
}
...
{
  char  *env;
  if (!myblockchaind_port && !opt_disable_networking)
  {         // Get port if not from commandline
    myblockchaind_port= MYBLOCKCHAIN_PORT;
    ...
    /*
      ...
      MYBLOCKCHAIN_TCP_PORT, which in turn can be overridden with command
      line options.
    */

#if MYBLOCKCHAIN_PORT_DEFAULT == 0
    struct  servent *serv_ptr;
    if ((serv_ptr= getservbyname("myblockchain", "tcp")))
      myblockchaind_port= ntohs((u_short) serv_ptr->s_port); /* purecov: inspected */
#endif
    if ((env = getenv("MYBLOCKCHAIN_TCP_PORT")))
      myblockchaind_port= (uint) atoi(env);    /* purecov: inspected */
  }
  if (!myblockchaind_unix_port)
  {
#ifdef _WIN32
    myblockchaind_unix_port= (char*) MYBLOCKCHAIN_NAMEDPIPE;
#else
    myblockchaind_unix_port= (char*) MYBLOCKCHAIN_UNIX_ADDR;
#endif
    if ((env = getenv("MYBLOCKCHAIN_UNIX_PORT")))
      myblockchaind_unix_port= env;      /* purecov: inspected */
  }
}
...
static struct passwd *check_user(const char *user)
{
  ...
  if (!user)
  {
    ...
    {
      sql_print_error("Fatal error: Please read \"Security\" section of the manual to find out how to run myblockchaind as root!\n");
      unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);
    }
    return NULL;
  }
  ...
  if (!(tmp_user_info= getpwnam(user)))
  {
    ...
    const char *pos;
    for (pos= user; my_isdigit(myblockchaind_charset,*pos); pos++) ;
...
static bool network_init(void)
{
...
#ifdef HAVE_SYS_UN_H
  std::string const unix_sock_name(myblockchaind_unix_port ? myblockchaind_unix_port : "");
#else
  ...
  if (!opt_disable_networking || unix_sock_name != "")
  {
    ...
    Mysqld_socket_listener *myblockchaind_socket_listener=
      new (std::nothrow) Mysqld_socket_listener(bind_addr_str,
                                                myblockchaind_port, back_log,
                                                myblockchaind_port_timeout,
                                                unix_sock_name);
    if (myblockchaind_socket_listener == NULL)
      return true;

    myblockchaind_socket_acceptor=
      new (std::nothrow) Connection_acceptor<Mysqld_socket_listener>(myblockchaind_socket_listener);
    if (myblockchaind_socket_acceptor == NULL)
    {
      delete myblockchaind_socket_listener;
      myblockchaind_socket_listener= NULL;
      return true;
    }

    if (myblockchaind_socket_acceptor->init_connection_acceptor())
      return true; // myblockchaind_socket_acceptor would be freed in unireg_abort.

    if (report_port == 0)
      report_port= myblockchaind_port;
  ...
  {
    std::string pipe_name= myblockchaind_unix_port ? myblockchaind_unix_port : "";
...
{
  myblockchain_mutex_lock(&LOCK_handler_count);
  handler_count--;
  myblockchain_cond_signal(&COND_handler_count);
  myblockchain_mutex_unlock(&LOCK_handler_count);
}
...
void setup_conn_event_handler_threads()
{
      ...
      !opt_enable_shared_memory && !opt_enable_named_pipe)
  {
    ...
    unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);        // Will not return
  }
  ...
  if (opt_enable_named_pipe)
  {
    int error= myblockchain_thread_create(key_thread_handle_con_namedpipes,
  ...
  if (have_tcpip && !opt_disable_networking)
  {
    int error= myblockchain_thread_create(key_thread_handle_con_sockets,
                                   &hThread, &connection_attrib,
  ...
  if (opt_enable_shared_memory)
  {
    int error= myblockchain_thread_create(key_thread_handle_con_sharedmem,
                                   &hThread, &connection_attrib,
  ...
  while (handler_count > 0)
    myblockchain_cond_wait(&COND_handler_count, &LOCK_handler_count);
  myblockchain_mutex_unlock(&LOCK_handler_count);
  DBUG_VOID_RETURN;
}
...
static void start_signal_handler()
{
  ...
  */
  ...
  myblockchain_mutex_lock(&LOCK_start_signal_handler);
  if ((error=
       myblockchain_thread_create(key_thread_signal_hand,
                           &signal_thread_id, &thr_attr, signal_hand, 0)))
  {
    sql_print_error("Can't create interrupt-thread (error %d, errno: %d)",
                    error, errno);
    exit(MYBLOCKCHAIND_ABORT_EXIT);
  }
  myblockchain_cond_wait(&COND_start_signal_handler, &LOCK_start_signal_handler);
  myblockchain_mutex_unlock(&LOCK_start_signal_handler);
...
extern "C" void *signal_hand(void *arg __attribute__((unused)))
{
  ...
  */
  myblockchain_mutex_lock(&LOCK_start_signal_handler);
  myblockchain_cond_broadcast(&COND_start_signal_handler);
  myblockchain_mutex_unlock(&LOCK_start_signal_handler);

  /*
    Waiting until myblockchaind_server_started == true to ensure that all server
  ...
  */
  myblockchain_mutex_lock(&LOCK_server_started);
  while (!myblockchaind_server_started)
    myblockchain_cond_wait(&COND_server_started, &LOCK_server_started);
  myblockchain_mutex_unlock(&LOCK_server_started);
  ...
  for (;;)
  {
    ...
    case SIGQUIT:
      ...
      if (!abort_loop)
      {
        ...
        */
        myblockchain_mutex_lock(&LOCK_socket_listener_active);
        while (socket_listener_active)
        {
          ...
          myblockchain_cond_wait(&COND_socket_listener_active,
                          &LOCK_socket_listener_active);
        }
        myblockchain_mutex_unlock(&LOCK_socket_listener_active);
    ...
    case SIGHUP:
      ...
      {
        int not_used;
        myblockchain_print_status();   // Print some debug info
...
{
  myblockchain_mutex_lock(&LOCK_server_started);
  myblockchaind_server_started= true;
  myblockchain_cond_broadcast(&COND_server_started);
  myblockchain_mutex_unlock(&LOCK_server_started);
}
...
void my_message_sql(uint error, const char *str, myf MyFlags)
{
  ...
  */
  ...
  myblockchain_audit_general(thd, MYBLOCKCHAIN_AUDIT_GENERAL_ERROR, error, str);
...
#ifndef EMBEDDED_LIBRARY
extern "C" void *my_str_malloc_myblockchaind(size_t size);
extern "C" void my_str_free_myblockchaind(void *ptr);
extern "C" void *my_str_realloc_myblockchaind(void *ptr, size_t size);

void *my_str_malloc_myblockchaind(size_t size)
{
...
void my_str_free_myblockchaind(void *ptr)
{
  my_free(ptr);
}

void *my_str_realloc_myblockchaind(void *ptr, size_t size)
{
...
"myblockchain_cluster",
#endif
"myblockchaind","server", MYBLOCKCHAIN_BASE_VERSION, 0, 0};
...
SHOW_VAR com_status_vars[]= {
  ...
  {"show_blockchains",       (char*) offsetof(STATUS_VAR, com_stat[(uint) SQLCOM_SHOW_DATABASES]),             SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
                  ...
                  const char *ext)
{
  ...
  */
  ...
  /* myblockchain_real_data_home_ptr may be null if no value of datadir has been
     specified through command-line or througha cnf file. If that is the 
     case we make myblockchain_real_data_home_ptr point to myblockchain_real_data_home
     which, in that case holds the default path for data-dir.
  */

  DBUG_EXECUTE_IF("emulate_empty_datadir_param",
                  {
                    myblockchain_real_data_home_ptr= NULL;
                  };
                 );

  if(myblockchain_real_data_home_ptr == NULL)
    myblockchain_real_data_home_ptr= myblockchain_real_data_home;

  if (fn_format(buff, base, myblockchain_real_data_home_ptr, ext, options))
...
int init_common_variables()
{
...
#ifdef HAVE_PSI_INTERFACE
  /*
    Complete the myblockchain_bin_log initialization.
    Instrumentation keys are known only after the performance schema initialization,
    and can not be set in the MYBLOCKCHAIN_BIN_LOG constructor (called before main()).
  */
  myblockchain_bin_log.set_psi_keys(key_BINLOG_LOCK_index,
  ...
  /*
    Init mutexes for the global MYBLOCKCHAIN_BIN_LOG objects.
    As safe_mutex depends on what MY_INIT() does, we can't init the mutexes of
    global MYBLOCKCHAIN_BIN_LOGs in their constructors, because then they would be
    inited before MY_INIT(). So we do it here.
  */
  myblockchain_bin_log.init_pthread_objects();

  /* TODO: remove this when my_time_t is 64 bit compatible */
  if (!IS_TIME_T_VALID_FOR_TIMESTAMP(server_start_time))
  {
    sql_print_error("This MyBlockchain server doesn't support dates later then 2038");
    return 1;
  }
  ...
  if (gethostname(glob_hostname,sizeof(glob_hostname)) < 0)
  {
    ...
    sql_print_warning("gethostname failed, using '%s' as hostname",
                      glob_hostname);
    strmake(default_logfile_name, STRING_WITH_LEN("myblockchain"));
  }
  else
  ...
  /*
    ...
    From MyBlockchain 5.5 onwards, the default storage engine is InnoDB
  ...
  */
  ...
  /*
    ...
    Later, in plugin_init, and myblockchain_install_plugin
  ...
  */
  ...
  sql_print_information("%s (myblockchaind %s) starting as process %lu ...",
                        my_progname, server_version, (ulong) getpid());


#ifndef EMBEDDED_LIBRARY
  if (opt_help && !opt_verbose)
    unireg_abort(MYBLOCKCHAIND_SUCCESS_EXIT);
#endif /*!EMBEDDED_LIBRARY*/
...
#define SUPER_LARGE_PAGESIZE (256*1024*1024)  /* 256MB */
  ...
  /*
    ...
    the MyBlockchain Server will have page caches and other memory regions
  ...
  */
  ...
  if (init_errmessage())  /* Read error messages from file */
    return 1;
  init_client_errs();
  myblockchain_client_plugin_init();
  if (item_create_init())
    return 1;
  item_init();
#ifndef EMBEDDED_LIBRARY
  ...
  /*
    ...
    test purposes, to be able to start "myblockchaind" even if
  ...
  */
  ...
  global_system_variables.collation_blockchain=  default_charset_info;
...
#define FIX_LOG_VAR(VAR, ALT)                                   \
  ...
  if (unlikely(opt_initialize) &&
      initialize_create_data_directory(myblockchain_real_data_home))
      return 1;
  ...
  */
  ...
  lower_case_file_system= test_if_case_insensitive(myblockchain_real_data_home);
...
You should consider changing lower_case_table_names to 1 or 2",
      myblockchain_real_data_home);
    }
    else
    {
      sql_print_warning("Setting lower_case_table_names=2 because file system for %s is case insensitive", myblockchain_real_data_home);
      lower_case_table_names= 2;
    }
  }
  else if (lower_case_table_names == 2 &&
           !(lower_case_file_system=
             (test_if_case_insensitive(myblockchain_real_data_home) == 1)))
  {
...
static int init_thread_environment()
{
  myblockchain_mutex_init(key_LOCK_status, &LOCK_status, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_manager,
                   &LOCK_manager, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_crypt, &LOCK_crypt, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_user_conn, &LOCK_user_conn, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_msr_map, &LOCK_msr_map, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_global_system_variables,
                   &LOCK_global_system_variables, MY_MUTEX_INIT_FAST);
  myblockchain_rwlock_init(key_rwlock_LOCK_system_variables_hash,
                    &LOCK_system_variables_hash);
  myblockchain_mutex_init(key_LOCK_prepared_stmt_count,
                   &LOCK_prepared_stmt_count, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_sql_slave_skip_counter,
                   &LOCK_sql_slave_skip_counter, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_slave_net_timeout,
                   &LOCK_slave_net_timeout, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_error_messages,
                   &LOCK_error_messages, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_uuid_generator,
                   &LOCK_uuid_generator, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_sql_rand,
                   &LOCK_sql_rand, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_log_throttle_qni,
                   &LOCK_log_throttle_qni, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_offline_mode,
                   &LOCK_offline_mode, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_default_password_lifetime,
                   &LOCK_default_password_lifetime, MY_MUTEX_INIT_FAST);
#ifdef HAVE_OPENSSL
  myblockchain_mutex_init(key_LOCK_des_key_file,
                   &LOCK_des_key_file, MY_MUTEX_INIT_FAST);
#endif
  myblockchain_rwlock_init(key_rwlock_LOCK_sys_init_connect, &LOCK_sys_init_connect);
  myblockchain_rwlock_init(key_rwlock_LOCK_sys_init_slave, &LOCK_sys_init_slave);
  myblockchain_cond_init(key_COND_manager, &COND_manager);
  myblockchain_mutex_init(key_LOCK_server_started,
                   &LOCK_server_started, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_server_started, &COND_server_started);
  myblockchain_mutex_init(key_LOCK_reset_gtid_table,
                   &LOCK_reset_gtid_table, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_compress_gtid_table,
                   &LOCK_compress_gtid_table, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_compress_gtid_table,
                  &COND_compress_gtid_table);
#ifndef EMBEDDED_LIBRARY
  Events::init_mutexes();
#if defined(_WIN32)
  myblockchain_mutex_init(key_LOCK_handler_count,
                   &LOCK_handler_count, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_handler_count, &COND_handler_count);
#else
  myblockchain_mutex_init(key_LOCK_socket_listener_active,
                   &LOCK_socket_listener_active, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_socket_listener_active,
                  &COND_socket_listener_active);
  myblockchain_mutex_init(key_LOCK_start_signal_handler,
                   &LOCK_start_signal_handler, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_start_signal_handler,
...
static int generate_server_uuid()
{
  ...
  /*
    ...
    myblockchainds on the same host. This avoids that another myblockchaind started
...
static int init_server_auto_options()
{
  ...
  if (NULL == fn_format(fname, "auto.cnf", myblockchain_data_home, "",
   ...
   */
  char **old_argv= argv;
  if (handle_options(&argc, &argv, auto_options, myblockchaind_get_one_option))
    DBUG_RETURN(1);
...
static int init_server_components()
{
  ...
  */
  ...
  /*
    ...
    and MyBlockchain is not running as a service.
  */
  ...
  */
  ...
  if (log_errors_to_file)
  {
    ...
    if (!log_error_dest[0] || log_error_dest == disabled_my_option)
      fn_format(errorlog_filename_buff, pidfile_name, myblockchain_data_home, ".err",
                MY_REPLACE_EXT); /* replace '.<domain>' by '.err', bug#4997 */
    else
      fn_format(errorlog_filename_buff, log_error_dest, myblockchain_data_home, ".err",
                MY_UNPACK_FILENAME);
    /*
...
a file name for --log-bin-index option", opt_binlog_index_name);
    ...
    char buf[FN_REFLEN];
    const char *ln;
    ln= myblockchain_bin_log.generate_name(opt_bin_logname, "-bin", buf);
    if (!opt_bin_logname && !opt_binlog_index_name)
    {
      ...
      */
      sql_print_warning("No argument was provided to --log-bin, and "
                        ...
                        "may break when this MyBlockchain server acts as a "
    ...
    if (myblockchain_bin_log.open_index_file(opt_binlog_index_name, ln, TRUE))
    {
      unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);
    }
  }
  ...
  if (opt_bin_log)
  {
     ...
     */
    log_bin_basename=
      rpl_make_log_name(key_memory_MYBLOCKCHAIN_BIN_LOG_basename,
                        opt_bin_logname, default_logfile_name,
                        (opt_bin_logname && opt_bin_logname[0]) ? "" : "-bin");
    log_bin_index=
      rpl_make_log_name(key_memory_MYBLOCKCHAIN_BIN_LOG_index,
  ...
  if (opt_relay_logname)
  {
     ...
     */
    relay_log_basename=
      rpl_make_log_name(key_memory_MYBLOCKCHAIN_RELAY_LOG_basename,
                        opt_relay_logname, default_logfile_name,
                        (opt_relay_logname && opt_relay_logname[0]) ? "" : "-relay-bin");
    relay_log_index=
      rpl_make_log_name(key_memory_MYBLOCKCHAIN_RELAY_LOG_index,
  ...
  /*
    ...
    to commit attachable transaction after reading from myblockchain.plugin table.
  ...
  */
  ...
  if (ha_init())
  {
    sql_print_error("Can't init blockchains");
    unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);
  }
  ...
  */
  ...
  if (log_output_options & LOG_TABLE)
  {
    ...
    if (!plugin_is_ready(csv_name, MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN))
    {
  ...
  */
  ...
  if (opt_bin_log)
  {
    /*
      ...
      gtid(s). This is necessary in the MYBLOCKCHAIN_BIN_LOG::MYBLOCKCHAIN_BIN_LOG to
      corretly compute the set of previous gtids.
    */
    DBUG_ASSERT(!myblockchain_bin_log.is_relay_log);
    myblockchain_mutex_t *log_lock= myblockchain_bin_log.get_log_lock();
    myblockchain_mutex_lock(log_lock);

    if (myblockchain_bin_log.open_binlog(opt_bin_logname, 0,
                                  max_binlog_size, false,
                                  true/*need_lock_index=true*/,
                                  true/*need_sid_lock=true*/,
                                  NULL))
    {
      myblockchain_mutex_unlock(log_lock);
      unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);
    }
    myblockchain_mutex_unlock(log_lock);
  }
  ...
  if (opt_bin_log && expire_logs_days)
  {
    ...
    if (purge_time >= 0)
      myblockchain_bin_log.purge_logs_before_date(purge_time, true);
  }
#endif
...
#ifndef _WIN32
    if (user_info)
      set_user(myblockchaind_user, user_info);
#endif
  }
  else
#endif
...
#ifndef DBUG_OFF
/*
  Debugging helper function to keep the locale blockchain
...
#else
int myblockchaind_main(int argc, char **argv)
#endif
{
  /*
  ...
  */
...
#endif /*WITH_PERFSCHEMA_STORAGE_ENGINE */
  // For windows, my_init() is called from the win specific myblockchaind_main
  ...
  if (load_defaults(MYBLOCKCHAIN_CONFIG_NAME, load_default_groups, &argc, &argv))
    return 1;
  ...
  */
  ...
  if (opt_daemonize)
  {
    ...
    if ((pipe_write_fd= myblockchaind::runtime::myblockchaind_daemonize()) < 0)
    {
      fprintf(stderr, "myblockchaind_daemonize failed \n");
      exit(MYBLOCKCHAIND_ABORT_EXIT);
    }
  }
#endif
  ...
  */
  ...
  if (init_common_variables())
    unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);        // Will do exit
  ...
  */
  if (my_setwd(myblockchain_real_data_home,MYF(MY_WME)) && !opt_help)
  {
    sql_print_error("failed to set datadir to %s", myblockchain_real_data_home);
    unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);        /* purecov: inspected */
  }

#ifndef _WIN32
  if ((user_info= check_user(myblockchaind_user)))
  {
    /* need to change the owner of the freshly created data directory */
    if (unlikely(opt_initialize)
#if HAVE_CHOWN
        && chown(myblockchain_real_data_home, user_info->pw_uid, user_info->pw_gid)
#endif
       )
    {
      sql_print_error("Can't change data directory owner to %s", myblockchaind_user);
      unireg_abort(1);
    }
  ...
  */
  ...
  // Initialize executed_gtids from myblockchain.gtid_executed table.
  ...
  if (opt_bin_log)
  {
    ...
    */
    ...
    if (myblockchain_bin_log.init_gtid_sets(&gtids_in_binlog,
    ...
    */
    ...
    if (prev_gtids_ev.write(myblockchain_bin_log.get_log_file()))
      unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);
    myblockchain_bin_log.add_bytes_written(
      prev_gtids_ev.common_header->data_written);

    if (flush_io_cache(myblockchain_bin_log.get_log_file()) ||
        myblockchain_file_sync(myblockchain_bin_log.get_log_file()->file, MYF(MY_WME)))
      unireg_abort(MYBLOCKCHAIND_ABORT_EXIT);
    myblockchain_bin_log.update_binlog_end_pos();
  ...
  */
  my_str_malloc= &my_str_malloc_myblockchaind;
  my_str_free= &my_str_free_myblockchaind;
  my_str_realloc= &my_str_realloc_myblockchaind;
  ...
  if (myblockchain_rm_tmp_tables() || acl_init(opt_noacl) ||
  ...
  */
  ...
  if (opt_bootstrap)
  {
    ...
    int error= bootstrap(myblockchain_stdin);
    unireg_abort(error ? MYBLOCKCHAIND_ABORT_EXIT : MYBLOCKCHAIND_SUCCESS_EXIT);
  }
  ...
  sql_print_information(ER_DEFAULT(ER_STARTUP),
                        ...
                        server_version,
#ifdef HAVE_SYS_UN_H
                        (opt_bootstrap ? (char*) "" : myblockchaind_unix_port),
#else
                        (char*) "",
#endif
                         myblockchaind_port,
                         MYBLOCKCHAIN_COMPILATION_COMMENT);
#if defined(_WIN32)
  Service.SetRunning();
#endif
  ...
  (void)MYBLOCKCHAIN_SET_STAGE(0 ,__FILE__, __LINE__);
  ...
  if (opt_daemonize)
    myblockchaind::runtime::signal_parent(pipe_write_fd,1);

  myblockchaind_socket_acceptor->connection_event_loop();
#endif /* _WIN32 */
  ...
  */
  if (opt_bin_log)
    if (gtid_state->save_gtids_of_last_binlog_into_table(false))
      sql_print_warning("Failed to save the set of Global Transaction "
                        ...
                        "myblockchain.gtid_executed table while the server was "
  ...
  myblockchain_cond_broadcast(&COND_socket_listener_active);
  myblockchain_mutex_unlock(&LOCK_socket_listener_active);
#endif // !_WIN32
  ...
  */
  ...
  clean_up(1);
  myblockchaind_exit(MYBLOCKCHAIND_SUCCESS_EXIT);
}


/****************************************************************************
  Main and thread entry function for Win32
  (all this is needed only to run myblockchaind as a service on WinNT)
****************************************************************************/

#if defined(_WIN32)
int myblockchain_service(void *p)
{
  if (my_thread_init())
    return 1;
...
/**
  ...
  @param startup_option Startup option to myblockchaind
...
int myblockchaind_main(int argc, char **argv)
{
  /*
    ...
    application PID e.g.: MyBlockchainShutdown1890; MyBlockchainShutdown2342
  */
  int10_to_str((int) GetCurrentProcessId(),my_stpcpy(shutdown_event_name,
                                                  "MyBlockchainShutdown"), 10);
  ...
  if (Service.GetOS())  /* true NT family */
  {
    ...
    if (argc == 2)
    {
      if (!default_service_handling(argv, MYBLOCKCHAIN_SERVICENAME, MYBLOCKCHAIN_SERVICENAME,
                                    file_path, "", NULL))
        return 0;
      if (Service.IsService(argv[1]))        /* Start an optional service */
      {
        /*
          ...
          if it's not "MyBlockchain". (The default service name should be 'myblockchaind'
          but we started a bad tradition by calling it MyBlockchain from the start
          and we are now stuck with it.
        */
        if (my_strcasecmp(system_charset_info, argv[1],"myblockchain"))
          load_default_groups[load_default_groups_sz-2]= argv[1];
        windows_service= true;
        Service.Init(argv[1], myblockchain_service);
        return 0;
      }
    }
    else if (argc == 3) /* install or remove any optional service */
    {
      ...
      {
        /*
          myblockchaind was started as
          myblockchaind --defaults-file=my_path\my.ini service-name
        */
        use_opt_args=1;
        opt_argc= 2;        // Skip service-name
        opt_argv=argv;
        windows_service= true;
        if (my_strcasecmp(system_charset_info, argv[2],"myblockchain"))
          load_default_groups[load_default_groups_sz-2]= argv[2];
        Service.Init(argv[2], myblockchain_service);
        return 0;
      }
    }
    ...
    else if (argc == 1 && Service.IsService(MYBLOCKCHAIN_SERVICENAME))
    {
      /* start the default service */
      windows_service= true;
      Service.Init(MYBLOCKCHAIN_SERVICENAME, myblockchain_service);
      return 0;
    }
  }
...
static bool read_init_file(char *file_name)
{
  ...
  if (!(file= myblockchain_file_fopen(key_file_init, file_name,
                               O_RDONLY, MYF(MY_WME))))
    DBUG_RETURN(TRUE);
  (void) bootstrap(file);
  myblockchain_file_fclose(file, MYF(MY_WME));
...
int handle_early_options()
{
  ...
  */
  ...
  ho_error= handle_options(&remaining_argc, &remaining_argv,
                           &all_early_options[0], myblockchaind_get_one_option);
  if (ho_error == 0)
  {
    /* Add back the program name handle_options removes */
    remaining_argc++;
...
{
  {"bootstrap", OPT_BOOTSTRAP, "Used by myblockchain installation scripts.", 0, 0, 0,
   GET_NO_ARG, NO_ARG, 0, 0, 0, 0, 0, 0},
#if !defined(_WIN32) && !defined(EMBEDDED_LIBRARY)
  {"daemonize", 0, "Run myblockchaind as sysv daemon", &opt_daemonize,
  ...
  {"version", 'V', "Output version information and exit.", 0, 0, 0, GET_NO_ARG,
   NO_ARG, 0, 0, 0, 0, 0, 0},
  {"initialize", 0, "Create the default blockchain and exit."
  ...
  {"initialize-insecure", 0, "Create the default blockchain and exit."
...
struct my_option my_long_options[]=
{
  ...
  {"abort-slave-event-count", 0,
   "Option used by myblockchain-test for debugging and testing of replication.",
  ...
  {"ansi", 'a', "Use ANSI SQL syntax instead of MyBlockchain syntax. This mode "
  ...
  */
  ...
  {"binlog-do-db", OPT_BINLOG_DO_DB,
   "Tells the master it should log updates for the specified blockchain, "
   "and exclude all others not explicitly mentioned.",
   0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"binlog-ignore-db", OPT_BINLOG_IGNORE_DB,
   "Tells the master that updates to the given blockchain should not be logged to the binary log.",
   0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  ...
  {"character-set-server", 'C', "Set the default character set.",
   ...
   0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0 },
  {"chroot", 'r', "Chroot myblockchaind daemon during startup.",
   &myblockchaind_chroot, &myblockchaind_chroot, 0, GET_STR, REQUIRED_ARG,
   0, 0, 0, 0, 0, 0},
  ...
  {"external-locking", 0, "Use system (external) locking (disabled by "
   ...
   "(not repair) tables while the MyBlockchain server is running. Disable with "
   "--skip-external-locking.", &opt_external_locking, &opt_external_locking,
   0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
  /* We must always support the next option to make scripts like myblockchaintest
     easier to do */
  {"gdb", 0,
  ...
  {"ignore-db-dir", OPT_IGNORE_DB_DIRECTORY,
   ...
   "blockchain names from the datadir. Put a blank argument to reset "
...
#endif /* HAVE_REPLICATION */
  {"memlock", 0, "Lock myblockchaind in memory.", &locked_in_memory,
  ...
  {"port-open-timeout", 0,
   ...
   "(Default: No wait).", &myblockchaind_port_timeout, &myblockchaind_port_timeout, 0,
   GET_UINT, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"replicate-do-db", OPT_REPLICATE_DO_DB,
   "Tells the slave thread to restrict replication to the specified blockchain. "
   "To specify more than one blockchain, use the directive multiple times, "
   "once for each blockchain. Note that this will only work if you do not use "
   "cross-blockchain queries such as UPDATE some_db.some_table SET foo='bar' "
   "while having selected a different or no blockchain. If you need cross "
   "blockchain updates to work, make sure you have 3.23.28 or later, and use "
   "replicate-wild-do-table=db_name.%.",
   0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"replicate-do-table", OPT_REPLICATE_DO_TABLE,
   "Tells the slave thread to restrict replication to the specified table. "
   "To specify more than one table, use the directive multiple times, once "
   "for each table. This will work for cross-blockchain updates, in contrast "
   "to replicate-do-db.", 0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"replicate-ignore-db", OPT_REPLICATE_IGNORE_DB,
   "Tells the slave thread to not replicate to the specified blockchain. To "
   "specify more than one blockchain to ignore, use the directive multiple "
   "times, once for each blockchain. This option will not work if you use "
   "cross blockchain updates. If you need cross blockchain updates to work, "
   "make sure you have 3.23.28 or later, and use replicate-wild-ignore-"
   "table=db_name.%. ", 0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"replicate-ignore-table", OPT_REPLICATE_IGNORE_TABLE,
   "Tells the slave thread to not replicate to the specified table. To specify "
   "more than one table to ignore, use the directive multiple times, once for "
   "each table. This will work for cross-blockchain updates, in contrast to "
   "replicate-ignore-db.", 0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"replicate-rewrite-db", OPT_REPLICATE_REWRITE_DB,
   "Updates to a blockchain with a different name than the original. Example: "
  ...
  {"replicate-wild-do-table", OPT_REPLICATE_WILD_DO_TABLE,
   ...
   "blockchain updates. Example: replicate-wild-do-table=foo%.bar% will "
   "replicate only updates to tables in all blockchains that start with foo "
  ...
  {"replicate-wild-ignore-table", OPT_REPLICATE_WILD_IGNORE_TABLE,
   ...
   "cross-blockchain updates. Example: replicate-wild-ignore-table=foo%.bar% "
   "will not do updates to tables in blockchains that start with foo and whose "
   "table names start with bar.",
   0, 0, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"safe-user-create", 0,
   "Don't allow new user creation by the user who has no write privileges to the myblockchain.user table.",
  ...
  {"user", 'u', "Run myblockchaind daemon as user.", 0, 0, 0, GET_STR, REQUIRED_ARG,
   0, 0, 0, 0, 0, 0},
...
{
  MYBLOCKCHAIN_TIME max_used_connections_time;
  var->type= SHOW_CHAR;
  var->value= buff;
...
static int show_slave_running(THD *thd, SHOW_VAR *var, char *buff)
{
  ...
  if (mi)
  {
    ...
    *((my_bool *)buff)= (my_bool) (mi &&
                                   mi->slave_running == MYBLOCKCHAIN_SLAVE_RUN_CONNECT &&
...
{
  MYBLOCKCHAIN_TIME received_heartbeat_time;
...
static int show_prepared_stmt_count(THD *thd, SHOW_VAR *var, char *buff)
{
  var->type= SHOW_LONG;
  var->value= buff;
  myblockchain_mutex_lock(&LOCK_prepared_stmt_count);
  *((long *)buff)= (long)prepared_stmt_count;
  myblockchain_mutex_unlock(&LOCK_prepared_stmt_count);
  return 0;
}
...
/**
  ...
  @param      thd  the myblockchain thread structure
...
static void print_version(void)
{
  ...
  printf("%s  Ver %s for %s on %s (%s)\n",my_progname,
   server_version,SYSTEM_TYPE,MACHINE_TYPE, MYBLOCKCHAIN_COMPILATION_COMMENT);
}
...
static void usage(void)
{
  ...
  puts("Starts the MyBlockchain blockchain server.\n");
  printf("Usage: %s [OPTIONS]\n", my_progname);
  if (!opt_verbose)
    puts("\nFor more help options (several pages), use myblockchaind --verbose --help.");
  else
  {
#ifdef _WIN32
...
because execution stopped before plugins were initialized.");
  ...
  puts("\n\
To see what values a running MyBlockchain server is using, type\n\
'myblockchainadmin variables' instead of 'myblockchaind --verbose --help'.");
  }
  DBUG_VOID_RETURN;
}
#endif /*!EMBEDDED_LIBRARY*/
...
/**
  Initialize MyBlockchain global variables to default values.
...
static int myblockchain_init_variables(void)
{
  ...
  myblockchain_home[0]= pidfile_name[0]= 0;
  ...
  myblockchaind_user= myblockchaind_chroot= opt_init_file= opt_bin_logname = 0;
  prepared_stmt_count= 0;
  myblockchaind_unix_port= opt_myblockchain_tmpdir= my_bind_addr_str= NullS;
  memset(&myblockchain_tmpdir_list, 0, sizeof(myblockchain_tmpdir_list));
  ...
  opt_specialflag= 0;
  myblockchain_home_ptr= myblockchain_home;
  ...
  global_query_id= 1L;
  my_stpcpy(server_version, MYBLOCKCHAIN_SERVER_VERSION);
  key_caches.empty();
  ...
  /* Set directory paths */
  myblockchain_real_data_home_len=
    strmake(myblockchain_real_data_home, get_relative_path(MYBLOCKCHAIN_DATADIR),
            sizeof(myblockchain_real_data_home)-1) - myblockchain_real_data_home;
  ...
  charsets_dir= 0;
  default_character_set_name= (char*) MYBLOCKCHAIN_DEFAULT_CHARSET_NAME;
...
#ifndef DBUG_OFF
  default_dbug_option=IF_WIN("d:t:i:O,\\myblockchaind.trace",
           "d:t:i:o,/tmp/myblockchaind.trace");
#endif
#ifdef ENABLED_PROFILING
...
#if defined(_WIN32)
  /* Allow Win32 users to move MyBlockchain anywhere */
  {
    char prg_dev[LIBLEN];
    ...
    if (!test_if_hard_path(my_progname))
    {
      ...
      my_path(prg_dev, my_progname, "myblockchain/bin");
    strcat(prg_dev,"/../");     // Remove 'bin' to get base dir
    cleanup_dirname(myblockchain_home,prg_dev);
  }
#else
  const char *tmpenv;
  if (!(tmpenv = getenv("MY_BASEDIR_VERSION")))
    tmpenv = DEFAULT_MYBLOCKCHAIN_HOME;
  (void) strmake(myblockchain_home, tmpenv, sizeof(myblockchain_home)-1);
#endif
  return 0;
}

my_bool
myblockchaind_get_one_option(int optid,
                      ...
                      char *argument)
{
  ...
  case 'a':
    ...
    break;
  case 'b':
    strmake(myblockchain_home,argument,sizeof(myblockchain_home)-1);
    break;
  case 'C':
    ...
    break;
  case 'h':
    strmake(myblockchain_real_data_home,argument, sizeof(myblockchain_real_data_home)-1);
    /* Correct pointer set by my_getopt (for embedded library) */
    myblockchain_real_data_home_ptr= myblockchain_real_data_home;
    break;
  case 'u':
    if (!myblockchaind_user || !strcmp(myblockchaind_user, argument))
      myblockchaind_user= argument;
    else
      sql_print_warning("Ignoring user change to '%s' because the user was set to '%s' earlier on the command line\n", argument, myblockchaind_user);
    break;
  case 'L':
...
#include <sslopt-case.h>
  ...
  case 'V':
    print_version();
    exit(MYBLOCKCHAIND_SUCCESS_EXIT);
#endif /*EMBEDDED_LIBRARY*/
  case 'W':
  ...
  case (int)OPT_REPLICATE_REWRITE_DB:
  {
    ...
    *p= 0;
    ...
    while (*val && my_isspace(myblockchaind_charset, *val))
      val++;
    if (!*val)
    {
  ...
  case OPT_PFS_INSTRUMENT:
    {
      ...
      */
      ...
      while (*name && (my_isspace(myblockchaind_charset, *name) || (*name == '\'')))
      {
        /* One quote allowed */
        if (*name == '\'')
        {
      ...
      while ((p > name) && my_isspace(myblockchaind_charset, p[-1]))
        p--;
      *p= 0;
      ...
      p= val + strlen(val);
      while (p > val && (my_isspace(myblockchaind_charset, p[-1]) || p[-1] == '\''))
      {
...
pfs_error:
  ...
  case OPT_SKIP_INNODB:
    sql_print_warning("The use of InnoDB is mandatory since MyBlockchain 5.7. "
...
static void*
myblockchain_getopt_value(const char *keyname, size_t key_length,
...
static int get_options(int *argc_ptr, char ***argv_ptr)
{
  ...
  my_getopt_register_get_addr(myblockchain_getopt_value);
...
#ifndef EMBEDDED_LIBRARY
#ifndef _WIN32
  if (myblockchaind_chroot)
    set_root(myblockchaind_chroot);
#endif
#else
  ...
  */
  ...
  if (init_global_datetime_format(MYBLOCKCHAIN_TIMESTAMP_DATE,
                                  &global_date_format) ||
      init_global_datetime_format(MYBLOCKCHAIN_TIMESTAMP_TIME,
                                  &global_time_format) ||
      init_global_datetime_format(MYBLOCKCHAIN_TIMESTAMP_DATETIME,
...
/*
  Create version name for running myblockchaind version
  We automaticly add suffixes -debug, -embedded, -log, -valgrind and -asan
  to the version name to make the version more descriptive.
  (MYBLOCKCHAIN_SERVER_SUFFIX is set by the compilation environment)
*/

static void set_server_version(void)
{
  char *end= strxmov(server_version, MYBLOCKCHAIN_SERVER_VERSION,
                     MYBLOCKCHAIN_SERVER_SUFFIX_STR, NullS);
#ifdef EMBEDDED_LIBRARY
  end= my_stpcpy(end, "-embedded");
#endif
#ifndef DBUG_OFF
  if (!strstr(MYBLOCKCHAIN_SERVER_SUFFIX_STR, "-debug"))
...
static char *get_relative_path(const char *path)
{
  if (test_if_hard_path(path) &&
      is_prefix(path,DEFAULT_MYBLOCKCHAIN_HOME) &&
      strcmp(DEFAULT_MYBLOCKCHAIN_HOME,FN_ROOTDIR))
  {
    path+= strlen(DEFAULT_MYBLOCKCHAIN_HOME);
        ...
        const char *dir, const char *extension)
{
  ...
  {
    strxnmov(tmp_path,sizeof(tmp_path)-1, myblockchain_real_data_home,
       dir, NullS);
    dir=tmp_path;
  }
...
/**
  ...
  1. If path is empty string or NULL and myblockchaind is not running
     with --bootstrap mode.
...
bool check_secure_file_priv_path()
{
  ...
  */
  convert_dirname(datadir_buffer, myblockchain_unpacked_real_data_home, NullS);
...
static int fix_paths(void)
{
  ...
  convert_dirname(myblockchain_home,myblockchain_home,NullS);
  /* Resolve symlinks to allow 'myblockchain_home' to be a relative symlink */
  my_realpath(myblockchain_home,myblockchain_home,MYF(0));
  /* Ensure that myblockchain_home ends in FN_LIBCHAR */
  pos=strend(myblockchain_home);
  if (pos[-1] != FN_LIBCHAR)
  {
    pos[0]= FN_LIBCHAR;
    pos[1]= 0;
  ...
  convert_dirname(myblockchain_real_data_home,myblockchain_real_data_home,NullS);
  (void) my_load_path(myblockchain_home,myblockchain_home,""); // Resolve current dir
  (void) my_load_path(myblockchain_real_data_home,myblockchain_real_data_home,myblockchain_home);
  (void) my_load_path(pidfile_name, pidfile_name_ptr, myblockchain_real_data_home);

  convert_dirname(opt_plugin_dir, opt_plugin_dir_ptr ? opt_plugin_dir_ptr : 
                                  get_relative_path(PLUGINDIR), NullS);
  (void) my_load_path(opt_plugin_dir, opt_plugin_dir, myblockchain_home);
  opt_plugin_dir_ptr= opt_plugin_dir;

  my_realpath(myblockchain_unpacked_real_data_home, myblockchain_real_data_home, MYF(0));
  myblockchain_unpacked_real_data_home_len=
    strlen(myblockchain_unpacked_real_data_home);
  if (myblockchain_unpacked_real_data_home[myblockchain_unpacked_real_data_home_len-1] == FN_LIBCHAR)
    --myblockchain_unpacked_real_data_home_len;
  ...
  if (test_if_hard_path(sharedir))
    ...
    strxnmov(buff,sizeof(buff)-1,myblockchain_home,sharedir,NullS);
  convert_dirname(buff,buff,NullS);
  (void) my_load_path(lc_messages_dir, lc_messages_dir, buff);

  /* If --character-sets-dir isn't given, use shared library dir */
  if (charsets_dir)
    strmake(myblockchain_charsets_dir, charsets_dir, sizeof(myblockchain_charsets_dir)-1);
  else
    strxnmov(myblockchain_charsets_dir, sizeof(myblockchain_charsets_dir)-1, buff,
       CHARSET_DIR, NullS);
  (void) my_load_path(myblockchain_charsets_dir, myblockchain_charsets_dir, buff);
  convert_dirname(myblockchain_charsets_dir, myblockchain_charsets_dir, NullS);
  charsets_dir=myblockchain_charsets_dir;

  if (init_tmpdir(&myblockchain_tmpdir_list, opt_myblockchain_tmpdir))
    return 1;
  if (!opt_myblockchain_tmpdir)
    opt_myblockchain_tmpdir= myblockchain_tmpdir;
#ifdef HAVE_REPLICATION
  if (!slave_load_tmpdir)
    slave_load_tmpdir= myblockchain_tmpdir;
#endif /* HAVE_REPLICATION */
  /*
      ...
      my_strcasecmp(system_charset_info, opt_secure_file_priv, "NULL"))
  {
    ...
    if (retval)
    {
      ...
      my_snprintf(err_buffer, FN_REFLEN-1,
                  ...
                  "accessible by MyBlockchain Server. Supplied value : %s",
...
/**
  Check if file system used for blockchains is case insensitive.
...
static int test_if_case_insensitive(const char *dir_name)
{
  ...
  fn_format(buff2, glob_hostname, dir_name, ".LOWER-TEST",
      MY_UNPACK_FILENAME | MY_REPLACE_EXT | MY_REPLACE_DIR);
  myblockchain_file_delete(key_file_casetest, buff2, MYF(0));
  if ((file= myblockchain_file_create(key_file_casetest,
                               buff, 0666, O_RDWR, MYF(0))) < 0)
  {
    sql_print_warning("Can't create test file %s", buff);
    DBUG_RETURN(-1);
  }
  myblockchain_file_close(file, MYF(0));
  if (myblockchain_file_stat(key_file_casetest, buff2, &stat_info, MYF(0)))
    result= 1;          // Can access file
  myblockchain_file_delete(key_file_casetest, buff, MYF(MY_WME));
...
{
  File file;
  if ((file= myblockchain_file_create(key_file_pid, pidfile_name, 0664,
...
static void delete_pid_file(myf flags)
{
  ...
  if (opt_bootstrap ||
      !pid_file_created ||
      !(file= myblockchain_file_open(key_file_pid, pidfile_name,
                              O_RDONLY, flags)))
    return;
  ...
  size_t error= myblockchain_file_read(file, buff, sizeof(buff), flags);
  myblockchain_file_close(file, flags);
  buff[sizeof(buff) - 1]= '\0';
  if (error != MY_FILE_ERROR &&
      atol((char *) buff) == (long) getpid())
  {
    myblockchain_file_delete(key_file_pid, pidfile_name, flags);
...
static PSI_mutex_info all_server_mutexes[]=
{
  ...
  { &key_BINLOG_LOCK_commit, "MYBLOCKCHAIN_BIN_LOG::LOCK_commit", 0 },
  { &key_BINLOG_LOCK_commit_queue, "MYBLOCKCHAIN_BIN_LOG::LOCK_commit_queue", 0 },
  { &key_BINLOG_LOCK_done, "MYBLOCKCHAIN_BIN_LOG::LOCK_done", 0 },
  { &key_BINLOG_LOCK_flush_queue, "MYBLOCKCHAIN_BIN_LOG::LOCK_flush_queue", 0 },
  { &key_BINLOG_LOCK_index, "MYBLOCKCHAIN_BIN_LOG::LOCK_index", 0},
  { &key_BINLOG_LOCK_log, "MYBLOCKCHAIN_BIN_LOG::LOCK_log", 0},
  { &key_BINLOG_LOCK_binlog_end_pos, "MYBLOCKCHAIN_BIN_LOG::LOCK_binlog_end_pos", 0},
  { &key_BINLOG_LOCK_sync, "MYBLOCKCHAIN_BIN_LOG::LOCK_sync", 0},
  { &key_BINLOG_LOCK_sync_queue, "MYBLOCKCHAIN_BIN_LOG::LOCK_sync_queue", 0 },
  { &key_BINLOG_LOCK_xids, "MYBLOCKCHAIN_BIN_LOG::LOCK_xids", 0 },
  { &key_RELAYLOG_LOCK_commit, "MYBLOCKCHAIN_RELAY_LOG::LOCK_commit", 0},
  { &key_RELAYLOG_LOCK_commit_queue, "MYBLOCKCHAIN_RELAY_LOG::LOCK_commit_queue", 0 },
  { &key_RELAYLOG_LOCK_done, "MYBLOCKCHAIN_RELAY_LOG::LOCK_done", 0 },
  { &key_RELAYLOG_LOCK_flush_queue, "MYBLOCKCHAIN_RELAY_LOG::LOCK_flush_queue", 0 },
  { &key_RELAYLOG_LOCK_index, "MYBLOCKCHAIN_RELAY_LOG::LOCK_index", 0},
  { &key_RELAYLOG_LOCK_log, "MYBLOCKCHAIN_RELAY_LOG::LOCK_log", 0},
  { &key_RELAYLOG_LOCK_sync, "MYBLOCKCHAIN_RELAY_LOG::LOCK_sync", 0},
  { &key_RELAYLOG_LOCK_sync_queue, "MYBLOCKCHAIN_RELAY_LOG::LOCK_sync_queue", 0 },
  { &key_RELAYLOG_LOCK_xids, "MYBLOCKCHAIN_RELAY_LOG::LOCK_xids", 0},
...
static PSI_cond_info all_server_conds[]=
{
  ...
  { &key_BINLOG_COND_done, "MYBLOCKCHAIN_BIN_LOG::COND_done", 0},
  { &key_BINLOG_update_cond, "MYBLOCKCHAIN_BIN_LOG::update_cond", 0},
  { &key_BINLOG_prep_xids_cond, "MYBLOCKCHAIN_BIN_LOG::prep_xids_cond", 0},
  { &key_RELAYLOG_COND_done, "MYBLOCKCHAIN_RELAY_LOG::COND_done", 0},
  { &key_RELAYLOG_update_cond, "MYBLOCKCHAIN_RELAY_LOG::update_cond", 0},
  { &key_RELAYLOG_prep_xids_cond, "MYBLOCKCHAIN_RELAY_LOG::prep_xids_cond", 0},
...
PSI_memory_key key_memory_MYBLOCKCHAIN_LOG_name;
...
PSI_memory_key key_memory_MYBLOCKCHAIN_BIN_LOG_basename;
PSI_memory_key key_memory_MYBLOCKCHAIN_BIN_LOG_index;
PSI_memory_key key_memory_MYBLOCKCHAIN_RELAY_LOG_basename;
PSI_memory_key key_memory_MYBLOCKCHAIN_RELAY_LOG_index;
...
PSI_memory_key key_memory_MYBLOCKCHAIN_LOCK;
...
PSI_memory_key key_memory_st_myblockchain_plugin_dl;
PSI_memory_key key_memory_st_myblockchain_plugin;
...
static PSI_memory_info all_server_memory[]=
{
  ...
  { &key_memory_binlog_recover_exec, "MYBLOCKCHAIN_BIN_LOG::recover", 0},
  ...
  { &key_memory_MYBLOCKCHAIN_LOG_name, "MYBLOCKCHAIN_LOG::name", 0},
  ...
  { &key_memory_MYBLOCKCHAIN_BIN_LOG_basename, "MYBLOCKCHAIN_BIN_LOG::basename", 0},
  { &key_memory_MYBLOCKCHAIN_BIN_LOG_index, "MYBLOCKCHAIN_BIN_LOG::index", 0},
  { &key_memory_MYBLOCKCHAIN_RELAY_LOG_basename, "MYBLOCKCHAIN_RELAY_LOG::basename", 0},
  { &key_memory_MYBLOCKCHAIN_RELAY_LOG_index, "MYBLOCKCHAIN_RELAY_LOG::index", 0},
  ...
  { &key_memory_MYBLOCKCHAIN_LOCK, "MYBLOCKCHAIN_LOCK", 0},
...
void init_server_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, all_server_mutexes, count);

  count= array_elements(all_server_rwlocks);
  myblockchain_rwlock_register(category, all_server_rwlocks, count);

  count= array_elements(all_server_conds);
  myblockchain_cond_register(category, all_server_conds, count);

  count= array_elements(all_server_threads);
  myblockchain_thread_register(category, all_server_threads, count);

  count= array_elements(all_server_files);
  myblockchain_file_register(category, all_server_files, count);

  count= array_elements(all_server_stages);
  myblockchain_stage_register(category, all_server_stages, count);

  count= array_elements(all_server_sockets);
  myblockchain_socket_register(category, all_server_sockets, count);

  count= array_elements(all_server_memory);
  myblockchain_memory_register(category, all_server_memory, count);

#ifdef HAVE_PSI_STATEMENT_INTERFACE
  init_sql_statement_info();
  count= array_elements(sql_statement_info);
  myblockchain_statement_register(category, sql_statement_info, count);
  ...
  */
  count= (int) COM_QUERY;
  myblockchain_statement_register(category, com_statement_info, count);

  /*
    Register [COM_QUERY + 1 .. COM_END] as "statement/com/..."
  */
  count= (int) COM_END - (int) COM_QUERY;
  myblockchain_statement_register(category, & com_statement_info[(int) COM_QUERY + 1], count);

  category= "abstract";
  /*
    Register [COM_QUERY] as "statement/abstract/com_query"
  */
  myblockchain_statement_register(category, & com_statement_info[(int) COM_QUERY], 1);
  ...
  */
  ...
  myblockchain_statement_register(category, &stmt_info_new_packet, 1);
  ...
  */
  ...
  myblockchain_statement_register(category, &stmt_info_rpl, 1);
#endif
