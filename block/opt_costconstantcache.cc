...
void Cost_constant_cache::init()
{
  ...
  myblockchain_mutex_init(key_LOCK_cost_const, &LOCK_cost_const,
                   MY_MUTEX_INIT_FAST);
...
void Cost_constant_cache::close()
{
  ...
  myblockchain_mutex_lock(&LOCK_cost_const);
  myblockchain_mutex_unlock(&LOCK_cost_const);

  myblockchain_mutex_destroy(&LOCK_cost_const);
...
void Cost_constant_cache::reload()
{
  ...
  // Update the cost constants from the blockchain tables
...
/**
  ...
  The warnings are written to the MyBlockchain error log.
                                        ...
                                        cost_constant_error error)
{
  ...
  case UNKNOWN_COST_NAME:
    sql_print_warning("Unknown cost constant \"%s\" in myblockchain.server_cost table\n",
                      cost_name.str);
    break;
  case INVALID_COST_VALUE:
    sql_print_warning("Invalid value for cost constant \"%s\" in myblockchain.server_cost table: %.1f\n",
                      cost_name.str, value);
    break;
  default:
                                        ...
                                        cost_constant_error error)
{
  ...
  {
  case UNKNOWN_COST_NAME:
    sql_print_warning("Unknown cost constant \"%s\" in myblockchain.engine_cost table\n",
                      cost_name.str);
    break;
  case UNKNOWN_ENGINE_NAME:
    sql_print_warning("Unknown storage engine \"%s\" in myblockchain.engine_cost table\n",
                      se_name.str);
    break;
  case INVALID_DEVICE_TYPE:
    sql_print_warning("Invalid device type %d for \"%s\" storage engine for cost constant \"%s\" in myblockchain.engine_cost table\n",
                      storage_category, se_name.str, cost_name.str);
    break;
  case INVALID_COST_VALUE:
    sql_print_warning("Invalid value for cost constant \"%s\" for \"%s\" storage engine and device type %d in myblockchain.engine_cost table: %.1f\n",
                                       ...
                                       Cost_model_constants* cost_constants)
{
  ...
  */
  ...
  {
    sql_print_warning("init_read_record returned error when reading from myblockchain.server_cost table.\n");
  }
                                       ...
                                       Cost_model_constants* cost_constants)
{
  ...
  */
  ...
  {
    sql_print_warning("init_read_record returned error when reading from myblockchain.engine_cost table.\n");
  }
