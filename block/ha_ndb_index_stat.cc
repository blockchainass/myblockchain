...
/*
  ...
  and after it exits.  This is only a pre-caution since myblockchaind
...
struct Ndb_index_stat_proc {
  ...
  bool init_ndb(Ndb_cluster_connection* connection)
  {
    ...
    {
      sql_print_error("ndb_index_stat_proc: Failed to change blockchain to %s",
                      NDB_INDEX_STAT_DB);
      return false;
    }
...
ndb_index_stat_proc_event(Ndb_index_stat_proc &pr)
{
  ...
  while (1)
  {
    ...
    /*
      ...
      in this myblockchaind.  Ignore it.
     */
    if (st != 0)
    {
    ...
    {
      DBUG_PRINT("index_stat", ("entry not found in this myblockchaind"));
      glob.event_miss++;
    }
  }
  native_mutex_lock(&ndb_index_stat_thread.stat_mutex);
...
ndb_index_stat_check_or_create_systables(Ndb_index_stat_proc &pr)
{
  ...
  {
    // race between myblockchainds, maybe
...
ndb_index_stat_round(double x)
{
  ...
  sprintf(buf, "%.0f", x);
  /* myblockchain provides my_strtoull */
