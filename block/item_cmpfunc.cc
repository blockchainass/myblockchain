    ...
    */
         ...
         rc == TYPE_NOTE_TIME_TRUNCATED)) // TS-TODO
    {
      ...
      */
      ...
      if (0 == field_cmp)
      {
        Item *tmp= field->type() == MYBLOCKCHAIN_TYPE_TIME ?
#define OLD_CMP
#ifdef OLD_CMP
...
/**
  Parse date provided in a string to a MYBLOCKCHAIN_TIME.

  @param[in]   thd        Thread handle
  @param[in]   str        A string to convert
  @param[in]   warn_type  Type of the timestamp for issuing the warning
  @param[in]   warn_name  Field name for issuing the warning
  @param[out]  l_time     The MYBLOCKCHAIN_TIME objects is initialized.

  Parses a date provided in the string str into a MYBLOCKCHAIN_TIME object. If the
...
bool get_myblockchain_time_from_str(THD *thd, String *str, timestamp_type warn_type, 
                             const char *warn_name, MYBLOCKCHAIN_TIME *l_time)
{
  bool value;
  ...
  if (!str_to_datetime(str, l_time, flags, &status) &&
      (l_time->time_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME ||
       l_time->time_type == MYBLOCKCHAIN_TIMESTAMP_DATE))
    /*
  ...
  {
    value= TRUE;
    status.warnings= MYBLOCKCHAIN_TIME_WARN_TRUNCATED;  /* force warning */
  }
                                   ...
                                   const char *warn_name, bool *error_arg)
{
  ...
  *error_arg= get_myblockchain_time_from_str(thd, str, warn_type, warn_name, &l_time);
                                         ...
                                         ulonglong *const_value)
{
  ...
  */
  ...
  {
    ulonglong value;
    if (str_arg->field_type() == MYBLOCKCHAIN_TYPE_TIME)
    {
    ...
    else
    {
      ...
      timestamp_type t_type= (date_arg->field_type() == MYBLOCKCHAIN_TYPE_DATE ?
               ...
               Item *warn_item, bool *is_null)
{
  ...
  DBUG_ASSERT(item->field_type() == MYBLOCKCHAIN_TYPE_TIME || 
              item->field_type() == MYBLOCKCHAIN_TYPE_NULL);
                                 ...
                                 Item_result type)
{
  ...
  if (type != ROW_RESULT &&
      (((*a)->result_type() == STRING_RESULT &&
        (*a)->field_type() == MYBLOCKCHAIN_TYPE_JSON) ||
       ((*b)->result_type() == STRING_RESULT &&
        (*b)->field_type() == MYBLOCKCHAIN_TYPE_JSON)))
  {
  ...
  if (can_compare_as_dates(*a, *b, &const_value))
  {
    ...
    if (const_value != (ulonglong)-1)
    {
      ...
      */
      Item_cache_datetime *cache= new Item_cache_datetime(MYBLOCKCHAIN_TYPE_DATETIME);
      /* Mark the cache as non-const to prevent re-caching. */
      cache->set_used_tables(1);
      if (!(*a)->is_temporal_with_date())
      {
  ...
  else if (type == STRING_RESULT && (*a)->field_type() == MYBLOCKCHAIN_TYPE_TIME &&
           (*b)->field_type() == MYBLOCKCHAIN_TYPE_TIME)
  {
...
bool Arg_comparator::try_year_cmp_func(Item_result type)
{
  ...
  bool a_is_year= (*a)->field_type() == MYBLOCKCHAIN_TYPE_YEAR;
  bool b_is_year= (*b)->field_type() == MYBLOCKCHAIN_TYPE_YEAR;
  ...
  */
  if (str)
  {
    ...
    timestamp_type t_type= f_type ==
      MYBLOCKCHAIN_TYPE_DATE ? MYBLOCKCHAIN_TIMESTAMP_DATE : MYBLOCKCHAIN_TIMESTAMP_DATETIME;
                         ...
                         Json_wrapper *result, Json_scalar_holder **scalar)
{
  ...
  */
  if ((arg->field_type() != MYBLOCKCHAIN_TYPE_JSON) &&
      (arg->field_type() != MYBLOCKCHAIN_TYPE_GEOMETRY))
  {
    /*
...
int Arg_comparator::compare_time_packed()
{
  /*
    ...
    DBUG_ASSERT((*a)->field_type() == MYBLOCKCHAIN_TYPE_TIME);
    DBUG_ASSERT((*b)->field_type() == MYBLOCKCHAIN_TYPE_TIME);
  ...
  */
  if (cmp_type == STRING_RESULT)
  {
    for (i= 0; i < 3; i++)
    {
      ...
      if (args[i]->field_type() == MYBLOCKCHAIN_TYPE_TIME)
        time_items_found++;
    }
  }
    ...
    if (field_item->field->can_be_compared_as_longlong())
    {
      ...
      */
          ...
          args[2]->is_temporal())
      {
        ...
        */
        if (field_item->field_type() == MYBLOCKCHAIN_TYPE_TIME)
          compare_as_temporal_times= true;
...
bool Item_func_ifnull::date_op(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed == 1);
...
bool Item_func_ifnull::time_op(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
...
bool Item_func_if::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed == 1);
  Item *arg= args[0]->val_bool() ? args[1] : args[2];
...
bool Item_func_if::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  Item *arg= args[0]->val_bool() ? args[1] : args[2];
...
bool Item_func_case::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed == 1);
  char buff[MAX_FIELD_WIDTH];
  String dummy_str(buff, sizeof(buff), default_charset());
...
bool Item_func_case::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  char buff[MAX_FIELD_WIDTH];
  String dummy_str(buff, sizeof(buff), default_charset());
...
bool Item_func_coalesce::date_op(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed == 1);
  for (uint i= 0; i < arg_count; i++)
  {
...
bool Item_func_coalesce::time_op(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  for (uint i= 0; i < arg_count; i++)
  {
        ...
        for (arg= args, arg_end= args + arg_count; arg != arg_end ; arg++)
        {
          ...
          else if (itm->is_temporal_with_date())
          {
            ...
            */
            if (!date_arg)
              date_arg= itm;
            else if (itm->field_type() == MYBLOCKCHAIN_TYPE_DATETIME)
            {
              date_arg= itm;
...
/**
  ...
  @todo
    ...
    To be able to do this, we would however first have to extend the MyBlockchain
...
Item_field* Item_equal::get_subst_item(const Item_field *field)
{
  ...
  /*
    ...
    JOIN::optimize() but from e.g. myblockchain_delete() or myblockchain_update().
