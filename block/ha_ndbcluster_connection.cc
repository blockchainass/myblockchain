                   ...
                   uint recv_thread_activation_threshold)
{
  ...
  const char myblockchaind_name[]= "myblockchaind";
#else
  const char myblockchaind_name[]= "libmyblockchaind";
#endif
  int res;
    ...
    for (uint i= 1; i < g_pool_alloc; i++)
    {
      ...
      }
      {
        ...
        my_snprintf(buf, sizeof(buf), "%s --server-id=%lu (connection %u)",
                    myblockchaind_name, server_id, i+1);
...
static ST_FIELD_INFO ndb_transid_myblockchain_connection_map_fields_info[] =
{
  {
    "myblockchain_connection_id",
  ...
  { 0, 0, MYBLOCKCHAIN_TYPE_NULL, 0, 0, "", SKIP_OPEN_TABLE }
};

#include <myblockchain/innodb_priv.h>

static
int
ndb_transid_myblockchain_connection_map_fill_table(THD* thd, TABLE_LIST* tables,
                                            Item*)
{
  DBUG_ENTER("ndb_transid_myblockchain_connection_map_fill_table");
...
static
int
ndb_transid_myblockchain_connection_map_init(void *p)
{
  DBUG_ENTER("ndb_transid_myblockchain_connection_map_init");
  ST_SCHEMA_TABLE* schema = reinterpret_cast<ST_SCHEMA_TABLE*>(p);
  schema->fields_info = ndb_transid_myblockchain_connection_map_fields_info;
  schema->fill_table = ndb_transid_myblockchain_connection_map_fill_table;
  DBUG_RETURN(0);
}

static
int
ndb_transid_myblockchain_connection_map_deinit(void *p)
{
  DBUG_ENTER("ndb_transid_myblockchain_connection_map_deinit");
  DBUG_RETURN(0);
}
...
struct st_myblockchain_plugin i_s_ndb_transid_myblockchain_connection_map_plugin =
{
  MYBLOCKCHAIN_INFORMATION_SCHEMA_PLUGIN,
  &i_s_info,
  "ndb_transid_myblockchain_connection_map",
  "Oracle Corporation",
  "Map between myblockchain connection id and ndb transaction id",
  PLUGIN_LICENSE_GPL,
  ndb_transid_myblockchain_connection_map_init,
  ndb_transid_myblockchain_connection_map_deinit,
  0x0001,
  NULL,
  NULL,
  NULL,
