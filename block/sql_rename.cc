...
#include "sql_view.h"             // myblockchain_frm_type, myblockchain_rename_view
#include "lock.h"       // MYBLOCKCHAIN_OPEN_SKIP_TEMPORARY
...
bool myblockchain_rename_tables(THD *thd, TABLE_LIST *table_list, bool silent)
{
  bool error= 1;
  bool binlog_error= 0;
  ...
  DBUG_ENTER("myblockchain_rename_tables");
  ...
  */
  ...
  myblockchain_ha_rm_tables(thd, table_list);
...
/*
  ...
  SYNPOSIS
    do_rename()
      ...
      new_db            The blockchain to which the table to be moved to
    ...
    case FRMTYPE_TABLE:
      {
        ...
        if (!(rc= myblockchain_rename_table(hton, ren_table->db, old_alias,
                                     new_db, new_alias, 0)))
        {
                                             ...
                                             new_db, new_alias)))
          {
            ...
            */
            (void) myblockchain_rename_table(hton, new_db, new_alias,
    ...
    case FRMTYPE_VIEW:
      ...
      */
      if (thd->lex->sql_command != SQLCOM_ALTER_DB_UPGRADE &&
        ...
        rc= myblockchain_rename_view(thd, new_db, new_alias, ren_table);
      break;
    default:
