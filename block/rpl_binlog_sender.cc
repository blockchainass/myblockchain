...
void Binlog_sender::init()
{
  ...
  /*
    ...
    - Set the BINLOG_DUMP_NON_BLOCK flag.
      This is official, documented, not used by any myblockchain
      client, but used by some external users.

    - Set server_id=0.
      This is unofficial, undocumented, and used by
      myblockchainbinlog -R since the beginning of time.

    When myblockchainbinlog --stop-never is used, it sets a 'fake'
    ...
    running slave, or with any other instance of myblockchainbinlog
    ...
    As of MyBlockchain 5.6.20 and 5.7.5, myblockchainbinlog redundantly sets
    ...
    behavior. That will allow myblockchainbinlog to use server_id=0
    always. That has the advantage that myblockchainbinlog
...
void Binlog_sender::run()
{
  ...
  while (!has_error() && !m_thd->killed)
  {
    /*
      ...
      The main issue here are some dependencies on myblockchainbinlog, that should be
    ...
    */
    ...
    int error= myblockchain_bin_log.find_next_log(&m_linfo, 1);
    if (unlikely(error))
    {
  ...
  /*
    ...
    reconnect anymore.
  */
  myblockchain_mutex_lock(&m_thd->LOCK_thd_data);
  bool was_killed_by_duplicate_slave_uuid= m_thd->duplicate_slave_uuid;
  myblockchain_mutex_unlock(&m_thd->LOCK_thd_data);
...
inline my_off_t Binlog_sender::get_binlog_end_pos(IO_CACHE *log_cache)
{
  ...
  do
  {
    myblockchain_bin_log.lock_binlog_end_pos();
    end_pos= myblockchain_bin_log.get_binlog_end_pos();
    myblockchain_bin_log.unlock_binlog_end_pos();

    if (unlikely(!myblockchain_bin_log.is_active(m_linfo.log_file_name)))
    {
    ...
    DBUG_PRINT("info", ("Active file is %s", myblockchain_bin_log.get_log_fname()));
...
int Binlog_sender::wait_new_events(my_off_t log_pos)
{
  ...
  myblockchain_bin_log.lock_binlog_end_pos();
  m_thd->ENTER_COND(myblockchain_bin_log.get_log_cond(),
                    myblockchain_bin_log.get_binlog_end_pos_lock(),
                    &stage_master_has_sent_all_binlog_to_slave,
                    &old_stage);

  if (myblockchain_bin_log.get_binlog_end_pos() <= log_pos &&
      myblockchain_bin_log.is_active(m_linfo.log_file_name))
  {
...
inline int Binlog_sender::wait_with_heartbeat(my_off_t log_pos)
{
  ...
  do
  {
    ...
    ret= myblockchain_bin_log.wait_for_update_bin_log(m_thd, &ts);
...
{
  return myblockchain_bin_log.wait_for_update_bin_log(m_thd, NULL);
}
...
int Binlog_sender::check_start_file()
{
  ...
  if (m_start_file[0] != '\0')
  {
    myblockchain_bin_log.make_log_name(index_entry_name, m_start_file);
  ...
  else if (m_using_gtid_protocol)
  {
    ...
    */
    ...
    global_sid_lock->unlock();
    Gtid first_gtid= {0, 0};
    if (myblockchain_bin_log.find_first_log_not_in_gtid_set(index_entry_name,
                                                     m_exclude_gtid,
                                                     &first_gtid,
                                                     &errmsg))
    {
  ...
  */
  ...
  if (myblockchain_bin_log.find_log_pos(&m_linfo, name_ptr, true))
  {
