    ...
    while ((item = it++))
    {
      ...
      case INT_RESULT:
        ...
        if (item->type() == Item::FIELD_ITEM &&
            ((Item_field*) item)->field->type() == MYBLOCKCHAIN_TYPE_LONGLONG &&
...
bool test_if_number(NUM_INFO *info, const char *str, uint str_len)
{
  ...
  /*
    MyBlockchain removes any endspaces of a string, so we must take care only of
...
bool get_ev_num_info(EV_NUM_INFO *ev_info, NUM_INFO *info, const char *num)
{
  ...
  else		// ulonglong is as big as bigint in MyBlockchain
  {
