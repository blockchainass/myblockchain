                                 ...
                                 Stored_program_creation_ctx **ctx)
{
                   ...
                   &client_cs))
  {
    sql_print_warning("Event '%s'.'%s': invalid value "
                      "in column myblockchain.event.character_set_client.",
                      db_name,
                      event_name);
  ...
  {
    sql_print_warning("Event '%s'.'%s': invalid value "
                      "in column myblockchain.event.collation_connection.",
                      db_name,
                      event_name);
  ...
  {
    sql_print_warning("Event '%s'.'%s': invalid value "
                      "in column myblockchain.event.db_collation.",
                      db_name,
                      event_name);
  ...
  /*
    If we failed to resolve the blockchain collation, load the default one
    from the disk.
  */
...
/**
  Load an event's body from a row from myblockchain.event.
...
static
my_time_t
add_interval(MYBLOCKCHAIN_TIME *ltime, const Time_zone *time_zone,
                   ...
                   int i_value, interval_type i_type)
{
  ...
  MYBLOCKCHAIN_TIME local_start;
  MYBLOCKCHAIN_TIME local_now;
  ...
  */
  ...
  if (!starts_null && !ends_null)
  {
    /*
      ...
      If last_executed is set then increase with m_expression. The new MYBLOCKCHAIN_TIME is
...
/*
  Set the internal last_executed MYBLOCKCHAIN_TIME struct to now. NOW is the
...
/*
  ...
  RETURN VALUE
    0                       OK
    EVEX_MICROSECOND_UNSUP  Error (for now if myblockchain.event has been
...
Event_job_data::execute(THD *thd, bool drop)
{
  ...
  /*
    MyBlockchain parser currently assumes that current blockchain is either
    present in THD or all names in all statements are fully specified.
    And yet not fully specified names inside stored programs must be 
    be supported, even if the current blockchain is not set:
    CREATE PROCEDURE db1.p1() BEGIN CREATE TABLE t1; END//
    -- in this example t1 should be always created in db1 and the statement
    must parse even if there is no current blockchain.

    To support this feature and still address the parser limitation,
    we need to set the current blockchain here.
    We don't have to call myblockchain_change_db, since the checks performed
    in it are unnecessary for the purpose of parsing, and
    myblockchain_change_db will be invoked anyway later, to activate the
    procedure blockchain before it's executed.
  */
  ...
  */
...

  {
    ...
    sphead->m_sp_share= MYBLOCKCHAIN_GET_SP_SHARE(SP_TYPE_EVENT,
    ...
    */
    ...
    else
    {
      ...
      /*
        ...
        the myblockchain.event table for writing. In order to achieve this, we
        should call myblockchain_lock_tables() under the super-user.
