...
#include "myblockchaind.h"                    // opt_relay_logname
...
public:
  ...
  public:
    ...
    void init(
      ...
      myblockchain_mutex_init(key_LOCK_queue, &m_lock, MY_MUTEX_INIT_FAST);
    }

    void deinit() {
      myblockchain_mutex_destroy(&m_lock);
    }
  ...
  private:
    void lock() { myblockchain_mutex_lock(&m_lock); }
    void unlock() { myblockchain_mutex_unlock(&m_lock); }
...
public:
  ...
  {
    myblockchain_mutex_init(key_LOCK_done, &m_lock_done, MY_MUTEX_INIT_FAST);
    myblockchain_cond_init(key_COND_done, &m_cond_done);
#ifndef DBUG_OFF
    /* reuse key_COND_done 'cos a new PSI object would be wasteful in !DBUG_OFF */
    myblockchain_cond_init(key_COND_done, &m_cond_preempt);
#endif
  ...
  void deinit()
  {
    for (size_t i = 0 ; i < STAGE_COUNTER ; ++i)
      m_queue[i].deinit();
    myblockchain_cond_destroy(&m_cond_done);
    myblockchain_mutex_destroy(&m_lock_done);
  }
   ...
   */
  bool enroll_for(StageID stage, THD *first, myblockchain_mutex_t *stage_mutex);
...
private:
  ...
  myblockchain_cond_t m_cond_done;

  /** Mutex used for the condition variable above */
  myblockchain_mutex_t m_lock_done;
#ifndef DBUG_OFF
  ...
  myblockchain_cond_t m_cond_preempt;
#endif
};
...
/* bitmap to MYBLOCKCHAIN_BIN_LOG::close() */
...
class MYBLOCKCHAIN_BIN_LOG: public TC_LOG
{
  enum enum_log_state { LOG_OPENED, LOG_CLOSED, LOG_TO_BE_OPENED };

  /* LOCK_log is inited by init_pthread_objects() */
  myblockchain_mutex_t LOCK_log;
  char *name;
  char log_file_name[FN_REFLEN];
  char db[NAME_LEN + 1];
  ...
  myblockchain_mutex_t LOCK_index;
  myblockchain_mutex_t LOCK_commit;
  myblockchain_mutex_t LOCK_sync;
  myblockchain_mutex_t LOCK_binlog_end_pos;
  myblockchain_mutex_t LOCK_xids;
  myblockchain_cond_t update_cond;
  ...
  */
  ...
  myblockchain_cond_t m_prep_xids_cond;
...
public:
  ...
  */
  ...
  MYBLOCKCHAIN_BIN_LOG(uint *sync_period,
                enum cache_type io_cache_type_arg);
  /*
    note that there's no destructor ~MYBLOCKCHAIN_BIN_LOG() !
  ...
  */
  ...
  /**
    ...
    - If the binary log is disabled for this thread, the GTID is
      inserted directly into the myblockchain.gtid_executed table and the
    ...
    - If the binary log is enabled for this thread, an empty
      ...
      the GTID is added to the myblockchain.gtid_executed table on the next
      binlog rotation.
    ...
    statement updated the blockchain.
...
private:
  ...
  bool change_stage(THD *thd, Stage_manager::StageID stage,
                    THD* queue, myblockchain_mutex_t *leave,
                    myblockchain_mutex_t *enter);
...
public:
  ...
  void signal_update()
  {
    DBUG_ENTER("MYBLOCKCHAIN_BIN_LOG::signal_update");
    signal_cnt++;
    myblockchain_cond_broadcast(&update_cond);
    DBUG_VOID_RETURN;
  }
...
public:
  ...
  */
  ...
  inline myblockchain_mutex_t* get_log_lock() { return &LOCK_log; }
  inline myblockchain_cond_t* get_log_cond() { return &update_cond; }
  inline IO_CACHE* get_log_file() { return &log_file; }

  inline void lock_index() { myblockchain_mutex_lock(&LOCK_index);}
  inline void unlock_index() { myblockchain_mutex_unlock(&LOCK_index);}
  ...
  */
  ...
  {
    myblockchain_mutex_assert_not_owner(&LOCK_log);
    myblockchain_mutex_assert_owner(&LOCK_binlog_end_pos);
    return binlog_end_pos;
  }
  myblockchain_mutex_t* get_binlog_end_pos_lock() { return &LOCK_binlog_end_pos; }
  void lock_binlog_end_pos() { myblockchain_mutex_lock(&LOCK_binlog_end_pos); }
  void unlock_binlog_end_pos() { myblockchain_mutex_unlock(&LOCK_binlog_end_pos); }
};
...
extern MYBLOCKCHAIN_PLUGIN_IMPORT MYBLOCKCHAIN_BIN_LOG myblockchain_bin_log;
