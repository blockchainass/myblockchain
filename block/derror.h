...
public:
  ...
  const char *lookup(int myblockchain_errno);

#ifdef EXTRA_CODE_FOR_UNIT_TESTING
  bool replace_msg(int myblockchain_errno, const char *new_msg)
  {
    ...
    for (int i= 0; i < num_sections; i++)
    {
      ...
      {
        errmsgs[myblockchain_errno - errmsg_section_start[i] + offset]= new_msg;
        return false;
      }
