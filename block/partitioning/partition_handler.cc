...
void partitioning_init()
{
  ...
  myblockchain_memory_register("sql", all_partitioning_memory, count);
  count= array_elements(all_partitioning_mutex);
  myblockchain_mutex_register("sql", all_partitioning_mutex, count);
#endif
}
...
Partition_share::~Partition_share()
{
  if (auto_inc_mutex)
  {
    myblockchain_mutex_destroy(auto_inc_mutex);
...
bool Partition_share::init_auto_inc_mutex(TABLE_SHARE *table_share)
{
...
#endif /* DBUG_OFF */
  auto_inc_mutex= static_cast<myblockchain_mutex_t*>(my_malloc(
  ...
  myblockchain_mutex_init(key_partition_auto_inc_mutex,
                   auto_inc_mutex,
                   MY_MUTEX_INIT_FAST);
  DBUG_RETURN(false);
}
...
{
  DBUG_ASSERT(auto_inc_mutex);
  myblockchain_mutex_assert_owner(auto_inc_mutex);
  /*
...
bool Partition_share::populate_partition_name_hash(partition_info *part_info)
{
  ...
  */
  ...
  {
    myblockchain_mutex_assert_owner(&part_info->table->s->LOCK_ha_data);
  }
#endif
  if (partition_name_hash_initialized)
  {
...
/**
  ...
  @param old_data  The old record in MyBlockchain Row Format.
  @param new_data  The new record in MyBlockchain Row Format.
...
int Partition_helper::ph_update_row(const uchar *old_data, uchar *new_data)
{
  ...
  */
  ...
  /*
    ...
    index)
    myblockchain_update does not set table->next_number_field, so we use
  ...
  else if (error == HA_ERR_ROW_IN_WRONG_PARTITION)
  {
    ...
    */
    ...
    if (m_err_rec)
    {
      ...
      max_length= (MYBLOCKCHAIN_ERRMSG_SIZE - strlen(ER(ER_ROW_IN_WRONG_PARTITION)));
      ...
      if (m_part_info->is_sub_partitioned())
      {
        ...
        do
        {
          ...
          /*
            ...
            myblockchain_prepare_alter_table. Which may have set data/index_file_name
...
/**
  ...
  @param[out] buf  buffer that should be filled with record in MyBlockchain format.
...
/**
  ...
  index_read_map starts a new index scan using a start key. The MyBlockchain Server
  will check the end key on its own. Thus to function properly the
  partitioned handler need to ensure that it delivers records in the sort
  order of the MyBlockchain Server.
  ...
  @param[out] buf          Read row in MyBlockchain Row Format
...
/**
  ...
  @param[out] buf  Row returned in MyBlockchain Row Format.
*/
