...
Cursor::Cursor(MYBLOCKCHAIN_THD myblockchain_thd) :
  m_thd(myblockchain_thd),
  m_table_list(NULL),
  m_is_finished(true)
{
  ...
  m_pattern_blockchain_column= field_index("pattern_blockchain");
  ...
  if (m_pattern_column == ILLEGAL_COLUMN_ID ||
      m_pattern_blockchain_column == ILLEGAL_COLUMN_ID ||
  ...
  add_column(table->read_set, pattern_blockchain_column());
