...
#include "sql_parse.h"           // myblockchain_parse

#include "pfs_file_provider.h"
#include "myblockchain/psi/myblockchain_file.h"

static MYBLOCKCHAIN_FILE *bootstrap_file= NULL;
...
char *myblockchain_file_fgets_fn(char *buffer, size_t size, MYBLOCKCHAIN_FILE* input, int *error)
{
  char *line= myblockchain_file_fgets(buffer, static_cast<int>(size), input);
  if (error)
...
{
  is_allocated= false;
  if (!(m_input= myblockchain_file_fopen(key_file_init, file_name,
    O_RDONLY, MYF(MY_WME))))
    return;
  m_fgets_fn= myblockchain_file_fgets_fn;
  is_allocated= true;
}
...
void File_command_iterator::end(void)
{
  if (is_allocated)
  {
    myblockchain_file_fclose(m_input, MYF(0));
    is_allocated= false;
    m_input= NULL;
  }
}
...
static void handle_bootstrap_impl(THD *thd)
{
  ...
  File_command_iterator file_iter(bootstrap_file, myblockchain_file_fgets_fn);
  ...
  */
  ...
  for ( ; ; )
  {
    ...
    */
    ...
    {
      /*
        myblockchain_parse() may have set a successful error status for the previous
...
int bootstrap(MYBLOCKCHAIN_FILE *file)
{
  ...
  int error= myblockchain_thread_create(key_thread_bootstrap,
