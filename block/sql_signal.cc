...
*/
#define MAX_MYBLOCKCHAIN_ERRNO UINT_MAX16
                                    ...
                                    int sqlcode)
{
  if (set_level_code)
  {
    ...
    cond->m_myblockchain_errno= sqlcode;
  }
...
int Sql_cmd_common_signal::eval_signal_informations(THD *thd, Sql_condition *cond)
{
  ...
  */
  ...
  set= m_set_signal_information->m_item[CIN_MYBLOCKCHAIN_ERRNO];
  if (set != NULL)
  {
    if (set->is_null())
    {
      thd->raise_error_printf(ER_WRONG_VALUE_FOR_VAR,
                              "MYBLOCKCHAIN_ERRNO", "NULL");
      goto end;
    }
    longlong code= set->val_int();
    if ((code <= 0) || (code > MAX_MYBLOCKCHAIN_ERRNO))
    {
      str= set->val_str(& str_value);
      thd->raise_error_printf(ER_WRONG_VALUE_FOR_VAR,
                              "MYBLOCKCHAIN_ERRNO", str->c_ptr_safe());
      goto end;
    }
    cond->m_myblockchain_errno= (int) code;
  }
...
bool Sql_cmd_signal::execute(THD *thd)
{
  ...
  */
  ...
  Sql_condition *raised= thd->raise_condition(cond.myblockchain_errno(),
...
bool Sql_cmd_resignal::execute(THD *thd)
{
  ...
  if (m_cond)
  {
    ...
    Sql_condition signaled_err(thd->mem_root,
                               frame->sql_condition->myblockchain_errno(),
    ...
    if (!eval_signal_informations(thd, &signaled_err))
    {
      ...
      raised= thd->raise_condition(signaled_err.myblockchain_errno(),
    ...
    */
    ...
    if (!eval_signal_informations(thd, raised))
    {
      if (raised->severity() == Sql_condition::SL_ERROR)
        da->set_error_status(raised->myblockchain_errno(),
                             raised->message_text(),
  ...
  */
  ...
  if (da->is_error())
    frame->handler_da.set_error_status(da->myblockchain_errno(),
                                       da->message_text(),
