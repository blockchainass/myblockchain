...
#include "myblockchain_time.h"         // MYBLOCKCHAIN_TIME
...
/**
  ...
  MyBlockchain representation of in-memory JSON objects used by the JSON type
...
public:
  /**
    Json values in MyBlockchain comprises the stand set of JSON values plus a
    MyBlockchain specific set. A Json _number_ type is subdivided into _int_,
    _uint_, _double_ and _decimal_.

    MyBlockchain also adds four built-in date/time values: _date_, _time_,
    _datetime_ and _timestamp_.  An additional _opaque_ value can
    store any other MyBlockchain type.
...
/**
  ...
  represent MyBlockchain extensions).
*/
...
/**
  Represents a MyBlockchain decimal number, type J_DECIMAL.
*/
...
public:
  ...
  */
  ...
  /**
    Get a pointer to the MyBlockchain decimal held by this object. Ownership
...
/**
  Represents a MyBlockchain double JSON scalar (an extension of the ECMA
...
/**
  Represents a MyBlockchain integer (64 bits signed) JSON scalar (an extension
...
/**
  Represents a MyBlockchain integer (64 bits unsigned) JSON scalar (an extension
...
/**
  Represents a MyBlockchain date/time value (DATE, TIME, DATETIME or
...
private:
  MYBLOCKCHAIN_TIME m_t;                //!< holds the date/time value
  enum_field_types m_field_type; //!< identifies which type of date/time

public:
  /**
    Constructs a object to hold a MyBlockchain date/time value.

    @param[in] t   the time/value
    @param[in] ft  the field type: must be one of MYBLOCKCHAIN_TYPE_TIME,
                   MYBLOCKCHAIN_TYPE_DATE, MYBLOCKCHAIN_TYPE_DATETIME or
                   MYBLOCKCHAIN_TYPE_TIMESTAMP.
  */
  Json_datetime(const MYBLOCKCHAIN_TIME &t, enum_field_types ft)
  ...
  /**
    ...
    @return the pointer
  */
  const MYBLOCKCHAIN_TIME *value() const { return &m_t; }

  /**
    Return what kind of date/time value this object holds.
    @return One of MYBLOCKCHAIN_TYPE_TIME, MYBLOCKCHAIN_TYPE_DATE, MYBLOCKCHAIN_TYPE_DATETIME
            or MYBLOCKCHAIN_TYPE_TIMESTAMP.
  */
...
/**
  Represents a MyBlockchain value opaquely, i.e. the Json DOM can not
  ...
  64 encoded string tagged with the MyBlockchain type with this syntax:
...
public:
  /**
    An opaque MyBlockchain value.

    @param[in] mytype  the MyBlockchain type of the value
  ...
  */
  ...
  /**
    @return the MyBlockchain type of the value
  */
...
public:
  ...
  */
  ...
  /**
    Return the MYBLOCKCHAIN type of the opaque value, see ::type(). Valid for
  ...
  */
  ...
  /**
    Get the MyBlockchain representation of a JSON decimal value.
  ...
  */
  void get_datetime(MYBLOCKCHAIN_TIME *t) const;
  ...
  */
   ...
   */
  bool coerce_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate,
  ...
  */
  bool coerce_time(MYBLOCKCHAIN_TIME *ltime, const char *msgnam) const;
