...
/**
  ...
  query plans without accessing myblockchaind internal classes (JOIN_TAB, QEP_TAB, 
  etc.) directly.

  AQP maps join execution plans, as represented by myblockchaind internals, to a set 
  ...
  that the MyBlockchain server would execute as part of its nested loop join 
  ...
  execution plan that may be executed in the engine rather than in myblockchaind. By 
  using the AQP rather than the myblockchaind internals directly, the coupling between
  the engine and myblockchaind is reduced.
...
namespace AQP
{
  ...
  */
  ...
  private:
    /**
      This class is implemented in terms of this myblockchaind internal class.
     */
