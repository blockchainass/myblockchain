...
#include "myblockchain_com.h" /* MYBLOCKCHAIN_ERRMSG_SIZE */
...
public:
  ...
  */
  ...
  /**
    Get the MYBLOCKCHAIN_ERRNO of this condition.
    @return the sql error number condition item.
  */
  uint myblockchain_errno() const
  { return m_myblockchain_errno; }
...
private:
  ...
  */
  ...
  /**
    ...
    @param mem_root          Memory root to use for the condition items
                             of this condition.
    @param myblockchain_errno       MYBLOCKCHAIN_ERRNO
...
private:
  ...
  /** MyBlockchain extension, MYBLOCKCHAIN_ERRNO condition item. */
  uint m_myblockchain_errno;
...
class ErrConvString
{
  char err_buffer[MYBLOCKCHAIN_ERRMSG_SIZE];
  size_t buf_length;
public:
  ...
  ErrConvString(const struct st_myblockchain_time *ltime, uint dec);
...
public:
  ...
  */
  ...
  /**
    ...
    @param myblockchain_errno      SQL-condition error number
  */
  void set_error_status(uint myblockchain_errno);
...
private:
  ...
  */
  char m_message_text[MYBLOCKCHAIN_ERRMSG_SIZE];
...
bool myblockchaind_show_warnings(THD *thd, ulong levels_to_show);
