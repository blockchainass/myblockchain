...
#include "myblockchain/thread_pool_priv.h"  // inc_thread_created
    ...
    unit_test(false)
{
  ...
  myblockchain_mutex_register("sql", all_thd_manager_mutexes, count);

  count= array_elements(all_thd_manager_conds);
  myblockchain_cond_register("sql", all_thd_manager_conds, count);
#endif

  myblockchain_mutex_init(key_LOCK_thd_list, &LOCK_thd_list,
                   MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_thd_remove,
                   &LOCK_thd_remove, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_thread_ids,
                   &LOCK_thread_ids, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_thd_list, &COND_thd_list);
...
Global_THD_manager::~Global_THD_manager()
{
  ...
  myblockchain_mutex_destroy(&LOCK_thd_list);
  myblockchain_mutex_destroy(&LOCK_thd_remove);
  myblockchain_mutex_destroy(&LOCK_thread_ids);
  myblockchain_cond_destroy(&COND_thd_list);
}
...
void Global_THD_manager::add_thd(THD *thd)
{
  ...
  myblockchain_mutex_lock(&LOCK_thd_list);
  ...
  myblockchain_mutex_unlock(&LOCK_thd_list);
}
...
void Global_THD_manager::remove_thd(THD *thd)
{
  ...
  myblockchain_mutex_lock(&LOCK_thd_remove);
  myblockchain_mutex_lock(&LOCK_thd_list);
  ...
  */
  ...
  myblockchain_mutex_unlock(&LOCK_thd_remove);
  myblockchain_cond_broadcast(&COND_thd_list);
  myblockchain_mutex_unlock(&LOCK_thd_list);
}
...
void Global_THD_manager::wait_till_no_thd()
{
  ...
  {
    myblockchain_cond_wait(&COND_thd_list, &LOCK_thd_list);
...
{
  myblockchain_mutex_lock(&Global_THD_manager::get_instance()->LOCK_thd_list);
}
...
void thd_unlock_thread_count(THD *)
{
  ...
  myblockchain_cond_broadcast(&thd_manager->COND_thd_list);
  myblockchain_mutex_unlock(&thd_manager->LOCK_thd_list);
}
