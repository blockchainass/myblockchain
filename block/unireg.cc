...
/*
  ...
  SYNOPSIS
    myblockchain_create_frm()
    thd			Thread handler
    file_name		Path for file (including blockchain and .frm)
    db                  Name of blockchain
    table               Name of table
...
bool myblockchain_create_frm(THD *thd, const char *file_name,
		      ...
		      handler *db_file)
{
  ...
  DBUG_ENTER("myblockchain_create_frm");
  ...
  */
  ...
  if (myblockchain_file_pwrite(file, fileinfo, 64, 0L, MYF_RW) ||
      myblockchain_file_pwrite(file, keybuff, key_info_length,
                        (ulong) uint2korr(fileinfo+6), MYF_RW))
    goto err;
  myblockchain_file_seek(file,
  ...
  if (myblockchain_file_write(file, (const uchar*)buff, 2, MYF(MY_NABP)) ||
      myblockchain_file_write(file, (const uchar*)create_info->connect_string.str,
               create_info->connect_string.length, MYF(MY_NABP)))
      goto err;

  int2store(buff, static_cast<uint16>(str_db_type.length));
  if (myblockchain_file_write(file, (const uchar*)buff, 2, MYF(MY_NABP)) ||
      myblockchain_file_write(file, (const uchar*)str_db_type.str,
  ...
  if (part_info)
  {
    ...
    if (myblockchain_file_write(file, (const uchar*)buff, 4, MYF_RW) ||
        myblockchain_file_write(file, (const uchar*)part_info->part_info_string,
                 part_info->part_info_len + 1, MYF_RW) ||
        myblockchain_file_write(file, (const uchar*)&auto_partitioned, 1, MYF_RW))
      goto err;
  }
  else
  {
    memset(buff, 0, 6);
    if (myblockchain_file_write(file, (uchar*) buff, 6, MYF_RW))
      goto err;
  }
  for (i= 0; i < keys; i++)
  {
    if (key_info[i].parser_name)
    {
      if (myblockchain_file_write(file, (const uchar*)key_info[i].parser_name->str,
  ...
  if (forminfo[46] == (uchar)255)
  {
    ...
    if (myblockchain_file_write(file, comment_length_buff, 2, MYF(MY_NABP)) ||
        myblockchain_file_write(file, (uchar*) create_info->comment.str,
  ...
  /* "Format section" with additional table and column properties */
  {
    ...
    *ptr= 0; /* tablespace string terminating zero */
    ...
    if (myblockchain_file_write(file, format_section_buff,
  ...
  /* Write out the COMPRESS table attribute */
  {
    ...
    if (myblockchain_file_write(file, length_buff, 2, MYF(MY_NABP)) ||
        myblockchain_file_write(file, (uchar*) create_info->compress.str,
                         create_info->compress.length, MYF(MY_NABP)))
      goto err;
  }

  myblockchain_file_seek(file, filepos, MY_SEEK_SET, MYF(0));
  if (myblockchain_file_write(file, forminfo, 288, MYF_RW) ||
      myblockchain_file_write(file, screen_buff, info_length, MYF_RW) ||
  ...
  if (opt_sync_frm && !(create_info->options & HA_LEX_CREATE_TMP_TABLE) &&
      (myblockchain_file_sync(file, MYF(MY_WME)) ||
...
err3:
  myblockchain_file_delete(key_file_frm, file_name, MYF(0));
  DBUG_RETURN(1);
} /* myblockchain_create_frm */
...
/**
  ...
  @param path          Name of file (including blockchain, without .frm)
                     ...
                     bool no_ha_table)
{
  ...
  if (myblockchain_create_frm(thd, frm_name, db, table_name, create_info,
                       create_fields, keys, key_info, file))

    DBUG_RETURN(1);

  // Make sure myblockchain_create_frm din't remove extension
...
err_handler_frm:
  myblockchain_file_delete(key_file_frm, frm_name, MYF(0));
  DBUG_RETURN(1);
} /* rea_create_table */
                        ...
                        ulong data_offset, handler *file)
{
  ...
  while ((field=it++))
  {
    ...
    */
    if (field->sql_type == MYBLOCKCHAIN_TYPE_TIMESTAMP &&
    ...
    /**
      ...
      in myblockchain_prepare_create_table. So the offset is messed up by
  ...
  /* Hack to avoid bugs with small static rows in MyBlockchain */
                        ...
                        ulong data_offset)
{
  ...
  while ((field=it++))
  {
    ...
    if (field->sql_type == MYBLOCKCHAIN_TYPE_GEOMETRY)
    {
      buff[11]= 0;
    ...
    if (myblockchain_file_write(file, buff, FCOMP, MYF_RW))
      DBUG_RETURN(1);
  }

	/* Write fieldnames */
  buff[0]=(uchar) NAMES_SEP_CHAR;
  if (myblockchain_file_write(file, buff, 1, MYF_RW))
    DBUG_RETURN(1);
  i=0;
  it.rewind();
  while ((field=it++))
  {
    ...
    *pos++=NAMES_SEP_CHAR;
    if (i == create_fields.elements-1)
      *pos++=0;
    if (myblockchain_file_write(file, buff, (size_t) (pos-(char*) buff), MYF_RW))
      DBUG_RETURN(1);
    i++;
  }
  ...
  if (int_count)
  {
    ...
    if (myblockchain_file_write(file, (uchar*) tmp.ptr(), tmp.length(), MYF_RW))
      DBUG_RETURN(1);
  }
  if (comment_length)
  {
    ...
    while ((field=it++))
    {
      if (field->comment.length)
        if (myblockchain_file_write(file, (uchar*) field->comment.str,
                        ...
                        uchar *rec_pos, uchar *null_pos, uint *null_count)
{
  ...
  if (field->sql_type == MYBLOCKCHAIN_TYPE_BIT && !f_bit_as_char(field->pack_flag))
  ...
  else if (regfield->real_type() == MYBLOCKCHAIN_TYPE_ENUM &&
                           ...
                           handler *handler)
{
  ...
  */
  ...
  error= myblockchain_file_write(file, buff, (size_t) reclength, MYF_RW) != 0;
