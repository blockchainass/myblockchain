...
#include "probes_myblockchain.h" // MYBLOCKCHAIN_QUERY_EXEC_START
...
/*
  ...
  * Each statement executed in SP is binlogged separately, according to rules
  ...
  When we call myblockchain_bin_log.write() for an SP statement, thd->user_var_events
  ...
  2) We need to empty thd->user_var_events after we have wrote a function
     ...
     myblockchain_bin_log.write() function)
                                           ...
                                           bool open_tables)
{
  ...
  */
  ...
  if (!error || !thd->is_error() ||
      (thd->get_stmt_da()->myblockchain_errno() != ER_CANT_REOPEN_TABLE &&
       thd->get_stmt_da()->myblockchain_errno() != ER_NO_SUCH_TABLE &&
       thd->get_stmt_da()->myblockchain_errno() != ER_UPDATE_TABLE_USED))
  ...
  while (true)
  {
    ...
    */
    if (stmt_reprepare_observer &&
        !thd->is_fatal_error &&
        !thd->killed &&
        thd->get_stmt_da()->myblockchain_errno() == ER_NEED_REPREPARE &&
        reprepare_attempt++ < 3)
    {
...
bool sp_instr_stmt::execute(THD *thd, uint *nextp)
{
  ...
  MYBLOCKCHAIN_SET_STATEMENT_TEXT(thd->m_statement_psi, m_query.str, m_query.length);
  ...
  */
  ...
  if (query_cache.send_result_to_client(thd, thd->query()) <= 0)
  {
    ...
    /*
      With the current setup, a subst_spvars() and a myblockchain_rewrite_query()
...
{
  MYBLOCKCHAIN_QUERY_EXEC_START(const_cast<char*>(thd->query().str),
                         thd->thread_id(),
  ...
  bool rc= myblockchain_execute_command(thd);

  thd->lex->set_sp_current_parsing_ctx(NULL);
  thd->lex->sphead= NULL;
  thd->m_statement_psi= statement_psi_saved;

  MYBLOCKCHAIN_QUERY_EXEC_DONE(rc);
