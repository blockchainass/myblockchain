...
#include "probes_myblockchain.h"             // MYBLOCKCHAIN_INSERT_START
...
/**
  ...
  @todo check_insert_fields() should be refactored as follows:
        ...
        - Rearrange the call to check_insert_fields() from
          myblockchain_prepare_insert() so that the value_count is known also when
...
/**
  ...
  @note Like implementations of other DDL/DML in MyBlockchain, this function
  relies on the caller to close the thread tables. This is done in the
  end of dispatch_command().
*/

bool Sql_cmd_insert::myblockchain_insert(THD *thd,TABLE_LIST *table_list)
{
  DBUG_ENTER("myblockchain_insert");
  ...
  */
  ...
  if (myblockchain_prepare_insert(thd, table_list, values, false))
	...
	if (error <= 0)
	  ...
	  /*
	    ...
	    MyISAM, then thd->net.last_error/errno will be set to
            "disk full"... and the myblockchain_file_pwrite() will wait until free
...
bool
Sql_cmd_insert_base::myblockchain_prepare_insert_check_table(THD *thd,
                                                      TABLE_LIST *table_list,
                                                      List<Item> &fields,
                                                      bool select_insert)
{
  DBUG_ENTER("myblockchain_prepare_insert_check_table");
...
bool Sql_cmd_insert_base::myblockchain_prepare_insert(THD *thd, TABLE_LIST *table_list,
                                               List_item *values,
                                               bool select_insert)
{
  DBUG_ENTER("myblockchain_prepare_insert");
  ...
  */
  ...
  if (myblockchain_prepare_insert_check_table(thd, table_list, insert_field_list,
                                       select_insert))
    DBUG_RETURN(true);
  ...
  else if (thd->stmt_arena->is_stmt_prepare())
  {
    ...
    */
    ...
    /*
      ...
      myblockchain_prepare_insert() is called from
      myblockchain_insert_select_prepare_tester(), when the values list (aka the
...
int write_record(THD *thd, TABLE *table, COPY_INFO *info, COPY_INFO *update)
{
  ...
  if (duplicate_handling == DUP_REPLACE || duplicate_handling == DUP_UPDATE)
  {
	...
	DBUG_ASSERT(table->insert_values != NULL);
        ...
        if (!records_are_comparable(table) || compare_records(table))
        {
          ...
          /*
            ...
            next SELECT LAST_INSERT_ID() or myblockchain_insert_id().
	...
	/*
	  ...
	  InnoDB do not function in the defined way if we allow MyBlockchain
                                           ...
                                           TABLE_LIST *table_list)
{
  ...
  for (Field **field=entry->field ; *field ; field++)
  {
    if (!bitmap_is_set(write_set, (*field)->field_index) &&
        ((*field)->flags & NO_DEFAULT_VALUE_FLAG) &&
        ((*field)->real_type() != MYBLOCKCHAIN_TYPE_ENUM))
    {
      bool view= false;
      if (table_list)
      {
...
/*
  ...
  SYNOPSIS
    myblockchain_insert_select_prepare()
    thd         thread handler
...
bool Sql_cmd_insert_select::myblockchain_insert_select_prepare(THD *thd)
{
  LEX *lex= thd->lex;
  SELECT_LEX *select_lex= lex->select_lex;
  DBUG_ENTER("myblockchain_insert_select_prepare");

  /*
    SELECT_LEX do not belong to INSERT statement, so we can't add WHERE
    clause if table is VIEW
  */
  if (myblockchain_prepare_insert(thd, lex->query_tables, NULL, true))
    DBUG_RETURN(true);
  ...
  if (error)
  {
        ...
        myblockchain_bin_log.is_open())
    {
      ...
      */
      ...
      /* Don't write the blockchain name if it is the current one. */
...
bool Query_result_insert::send_eof()
{
  ...
  if (!error && thd->is_error())
    error= thd->get_stmt_da()->myblockchain_errno();
...
/**
  ...
  @param create_table  [in]     Pointer to TABLE_LIST object providing blockchain
  ...
  @param lock          [out]    Pointer to the MYBLOCKCHAIN_LOCK object for table
                                      ...
                                      List<Item> *items)
{
  ...
  */
  {
    if (!myblockchain_create_table_no_lock(thd, create_table->db,
...
int Query_result_create::prepare2()
{
  ...
  MYBLOCKCHAIN_LOCK *extra_lock= NULL;
  /*
  ...
  /*
    myblockchain_lock_tables() below should never fail with request to reopen table
    since it won't wait for the table lock (we have exclusive metadata lock on
    the table) and thus can't get aborted.
  */
  if (! (extra_lock= myblockchain_lock_tables(thd, &table, 1, 0)) ||
        hooks.postlock(&table, 1))
  {
    if (extra_lock)
    {
      myblockchain_unlock_tables(thd, extra_lock);
      extra_lock= 0;
    }
...
int Query_result_create::binlog_show_create_table(TABLE **tables, uint count)
{
  ...
  */
  ...
  if (myblockchain_bin_log.is_open())
  {
    ...
    result |= myblockchain_bin_log.commit(thd, true);
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    if (m_plock)
    {
      myblockchain_unlock_tables(thd, *m_plock);
      *m_plock= NULL;
      m_plock= NULL;
    }
  }
...
bool Sql_cmd_insert::execute(THD *thd)
{
  ...
  MYBLOCKCHAIN_INSERT_START(const_cast<char*>(thd->query().str));
  res= myblockchain_insert(thd, all_tables);
  MYBLOCKCHAIN_INSERT_DONE(res, (ulong) thd->get_row_count_func());
  ...
  */
  ...
  DBUG_EXECUTE_IF("after_myblockchain_insert",
                  {
                    const char act[]=
                      "now "
...
bool Sql_cmd_insert_select::execute(THD *thd)
{
  ...
  */
  ...
  if (!(res= open_tables_for_query(thd, all_tables, 0)))
  {
    ...
    MYBLOCKCHAIN_INSERT_SELECT_START(const_cast<char*>(thd->query().str));
    ...
    res= myblockchain_insert_select_prepare(thd);
    ...
    MYBLOCKCHAIN_INSERT_SELECT_DONE(res, (ulong) thd->get_row_count_func());
...
{
  LEX *lex= thd->lex;
  return myblockchain_test_insert(thd, lex->query_tables);
}
