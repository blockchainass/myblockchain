...
#include "lock.h"                            // myblockchain_lock_abort_for_thread
...
#include "sql_audit.h"                       // myblockchain_audit_release
#include "sql_base.h"                        // close_temporary_tables
#include "sql_callback.h"                    // MYBLOCKCHAIN_CALLBACK
#include "sql_handler.h"                     // myblockchain_ha_cleanup
...
#include "myblockchain/psi/myblockchain_ps.h"
...
THD::Attachable_trx::~Attachable_trx()
{
  ...
  // Remove the attachable transaction from InnoDB myblockchain_trx_list.
...
/**
  ...
  @retval         Max number of connections for MyBlockchain Server
*/
...
extern "C" int myblockchain_tmpfile(const char *prefix)
{
  char filename[FN_REFLEN];
  File fd = create_temp_file(filename, myblockchain_tmpdir, prefix,
#ifdef _WIN32
...
extern "C"
const char *set_thd_proc_info(MYBLOCKCHAIN_THD thd_arg, const char *info,
                      ...
                      const unsigned int calling_line)
{
  ...
  if (new_stage != NULL)
  {
    ...
    m_stage_progress_psi= MYBLOCKCHAIN_SET_STAGE(m_current_stage_key, calling_file, calling_line);
  }
  else
  {
...
extern "C"
void thd_enter_cond(void *opaque_thd, myblockchain_cond_t *cond, myblockchain_mutex_t *mutex,
                           ...
                           size_t max_query_len)
{
  ...
  */
  ...
  len= my_snprintf(header, sizeof(header),
                   "MyBlockchain thread id %u, OS thread handle %lu, query id %lu",
  ...
  myblockchain_mutex_lock(&thd->LOCK_thd_query);
  ...
  myblockchain_mutex_unlock(&thd->LOCK_thd_query);
...
/**
  ...
  the following warnings during DROP TABLE:
    ...
    - A trigger associated with the table does not have DEFINER (One of the
      MyBlockchain specifics now is that triggers are loaded for the table being
   ...
   duplicate_slave_uuid(false)
{
  ...
  // Must be reset to handle error with THD's created for init of myblockchaind
  lex->thd= NULL;
  ...
  query_id= 0;
  query_name_consts= 0;
  db_charset= global_system_variables.collation_blockchain;
...
#endif
#ifndef EMBEDDED_LIBRARY
  myblockchain_audit_init_thd(this);
  net.vio=0;
#endif
  ...
  active_vio = 0;
  myblockchain_mutex_init(key_LOCK_thd_data, &LOCK_thd_data, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_thd_query, &LOCK_thd_query, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_thd_sysvar, &LOCK_thd_sysvar, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_query_plan, &LOCK_query_plan, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_current_cond, &LOCK_current_cond,
                   MY_MUTEX_INIT_FAST);
...
{
  va_list args;
  char ebuff[MYBLOCKCHAIN_ERRMSG_SIZE];
                                    ...
                                    const char* msg)
{
  ...
  if (msg == NULL)
    msg= ER(sql_errno);
  if (sqlstate == NULL)
   sqlstate= myblockchain_errno_to_sqlstate(sql_errno);
...
{
  *xid = *(MYBLOCKCHAIN_XID *) thd->get_transaction()->xid_state()->get_xid();
}
...
/*
  ...
  This has to be called once before we call myblockchain_parse.
...
void THD::cleanup(void)
{
  ...
  */
  ...
  /* Protects user_vars. */
  myblockchain_mutex_lock(&LOCK_thd_data);
  my_hash_free(&user_vars);
  myblockchain_mutex_unlock(&LOCK_thd_data);
...
void THD::release_resources()
{
  ...
  */
  ...
  myblockchain_mutex_lock(&LOCK_thd_data);
  myblockchain_mutex_lock(&LOCK_query_plan);
  ...
  myblockchain_mutex_unlock(&LOCK_query_plan);
  myblockchain_mutex_unlock(&LOCK_thd_data);
  myblockchain_mutex_lock(&LOCK_thd_query);
  myblockchain_mutex_unlock(&LOCK_thd_query);
  ...
  */
  ...
  myblockchain_audit_release(this);
  ...
  myblockchain_audit_free_thd(this);
#endif
...
THD::~THD()
{
  ...
  myblockchain_mutex_destroy(&LOCK_query_plan);
  myblockchain_mutex_destroy(&LOCK_thd_data);
  myblockchain_mutex_destroy(&LOCK_thd_query);
  myblockchain_mutex_destroy(&LOCK_thd_sysvar);
  myblockchain_mutex_destroy(&LOCK_current_cond);
#ifndef DBUG_OFF
...
void THD::awake(THD::killed_state state_to_set)
{
  ...
  THD_CHECK_SENTRY(this);
  myblockchain_mutex_assert_owner(&LOCK_thd_data);
  ...
  */
  ...
  if (mysys_var)
  {
    myblockchain_mutex_lock(&LOCK_current_cond);
    ...
    */
    if (current_cond && current_mutex)
    {
      ...
      myblockchain_mutex_lock(current_mutex);
      myblockchain_cond_broadcast(current_cond);
      myblockchain_mutex_unlock(current_mutex);
    }
    myblockchain_mutex_unlock(&LOCK_current_cond);
  }
  DBUG_VOID_RETURN;
}
                             ...
                             bool needs_thr_lock_abort)
{
  ...
  {
    myblockchain_mutex_lock(&in_use->LOCK_thd_data);
         ...
         thd_table= thd_table->next)
    {
      ...
      */
      if (!thd_table->needs_reopen())
        myblockchain_lock_abort_for_thread(this, thd_table);
    }
    myblockchain_mutex_unlock(&in_use->LOCK_thd_data);
  }
}
...
bool THD::store_globals()
{
  ...
  */
  ...
  /*
    Let myblockchaind define the thread id (not mysys)
...
int THD::send_explain_fields(Query_result *result)
{
  ...
  field_list.push_back(new Item_return_int("id",3, MYBLOCKCHAIN_TYPE_LONGLONG));
...
bool Query_result_send::send_data(List<Item> &items)
{
  ...
  /*
    We may be passing the control from myblockchaind to the client: release the
...
void Query_result_to_file::send_error(uint errcode,const char *err)
{
  ...
  if (file > 0)
  {
    ...
    myblockchain_file_delete(key_select_to_file, path, MYF(0));
    file= -1;
  }
}
...
bool Query_result_to_file::send_eof()
{
  ...
  if (myblockchain_file_close(file, MYF(MY_WME)) || thd->is_error())
    error= true;
			...
			IO_CACHE *cache)
{
  ...
  {
    strxnmov(path, FN_REFLEN-1, myblockchain_real_data_home,
             thd->db().str ? thd->db().str : "",
             NullS);
    (void) fn_format(path, exchange->file_name, path, "", option);
  }
  else
    (void) fn_format(path, exchange->file_name, myblockchain_real_data_home, "", option);
  ...
  if ((file= myblockchain_file_create(key_select_to_file,
	  ...
	  if (use_mb(res_charset))
	  {
	    ...
	    if ((l=my_ismbchar(res_charset, pos, end)))
	    {
          ...
          /*
            ...
            myblockchain> create table t1 (a varchar(128)) character set big5;
            myblockchain> LOAD DATA INFILE 'dump.txt' INTO TABLE t1;
...
void Prepared_statement_map::reset()
{
  ...
  if (st_hash.records > 0)
  {
    ...
    for (uint i=0 ; i < st_hash.records ; i++)
    {
      ...
      MYBLOCKCHAIN_DESTROY_PS(stmt->get_PS_prepared_stmt());
    }
#endif
    myblockchain_mutex_lock(&LOCK_prepared_stmt_count);
...
/****************************************************************************
  ...
  access to myblockchain.proc table to find definitions of stored routines.
****************************************************************************/
...
*/
extern "C" int thd_killed(const MYBLOCKCHAIN_THD thd)
{
  return(thd->killed);
}
...
*/
extern "C" void thd_set_kill_status(const MYBLOCKCHAIN_THD thd)
{
...
/**
  ...
  @return thread id
*/
extern "C" unsigned long thd_get_thread_id(const MYBLOCKCHAIN_THD thd)
{
...
*/
extern "C" int thd_allow_batch(MYBLOCKCHAIN_THD thd)
{
...
enum_tx_isolation thd_get_trx_isolation(const MYBLOCKCHAIN_THD thd)
{
	return thd->tx_isolation;
}

#ifdef INNODB_COMPATIBILITY_HOOKS
extern "C" const struct charset_info_st *thd_charset(MYBLOCKCHAIN_THD thd)
{
  return(thd->charset());
}
...
/**
  ...
  @param thd   The MyBlockchain internal thread pointer
...
*/
extern "C" LEX_CSTRING thd_query_unsafe(MYBLOCKCHAIN_THD thd)
{
...
*/
extern "C" size_t thd_query_safe(MYBLOCKCHAIN_THD thd, char *buf, size_t buflen)
{
  myblockchain_mutex_lock(&thd->LOCK_thd_query);
...
extern "C" int thd_slave_thread(const MYBLOCKCHAIN_THD thd)
{
  return(thd->slave_thread);
}

extern "C" int thd_non_transactional_update(const MYBLOCKCHAIN_THD thd)
{
  return thd->get_transaction()->has_modified_non_trans_table(
    Transaction_ctx::SESSION);
}

extern "C" int thd_binlog_format(const MYBLOCKCHAIN_THD thd)
{
  if (myblockchain_bin_log.is_open() && (thd->variables.option_bits & OPTION_BIN_LOG))
    return (int) thd->variables.binlog_format;
  else
    return BINLOG_FORMAT_UNSPEC;
}

extern "C" void thd_mark_transaction_to_rollback(MYBLOCKCHAIN_THD thd, int all)
{
...
extern "C" bool thd_binlog_filter_ok(const MYBLOCKCHAIN_THD thd)
{
  return binlog_filter->db_ok(thd->db().str);
}

extern "C" bool thd_sqlcom_can_generate_row_events(const MYBLOCKCHAIN_THD thd)
{
  return sqlcom_can_generate_row_events(thd);
}

extern "C" enum durability_properties thd_get_durability_property(const MYBLOCKCHAIN_THD thd)
{
...
extern "C" void thd_get_autoinc(const MYBLOCKCHAIN_THD thd, ulong* off, ulong* inc)
{
...
*/
extern "C" bool thd_is_strict_mode(const MYBLOCKCHAIN_THD thd)
{
...
#ifndef EMBEDDED_LIBRARY
extern "C" void thd_pool_wait_begin(MYBLOCKCHAIN_THD thd, int wait_type);
extern "C" void thd_pool_wait_end(MYBLOCKCHAIN_THD thd);

/*
  Interface for MyBlockchain Server, plugins and storage engines to report
...
*/
extern "C" void thd_wait_begin(MYBLOCKCHAIN_THD thd, int wait_type)
{
  MYBLOCKCHAIN_CALLBACK(Connection_handler_manager::event_functions,
...
*/
extern "C" void thd_wait_end(MYBLOCKCHAIN_THD thd)
{
  MYBLOCKCHAIN_CALLBACK(Connection_handler_manager::event_functions,
                 thd_wait_end, (thd));
}
                                    ...
                                    uint new_state)
{
  ...
  if ((backup->option_bits & OPTION_BIN_LOG) &&
       ...
       !is_current_stmt_binlog_format_row())
    myblockchain_bin_log.start_union_events(this, this->query_id);
...
void THD::restore_sub_statement_state(Sub_statement_state *backup)
{
  ...
  */
  ...
  if ((variables.option_bits & OPTION_BIN_LOG) && is_update_query(lex->sql_command) &&
       !is_current_stmt_binlog_format_row())
    myblockchain_bin_log.stop_union_events(this);
...
void THD::set_sent_row_count(ha_rows count)
{
  ...
  MYBLOCKCHAIN_SET_STATEMENT_ROWS_SENT(m_statement_psi, m_sent_row_count);
}

void THD::set_examined_row_count(ha_rows count)
{
  m_examined_row_count= count;
  MYBLOCKCHAIN_SET_STATEMENT_ROWS_EXAMINED(m_statement_psi, m_examined_row_count);
}
  ...
  if (locked_tables_mode == LTM_LOCK_TABLES)
  {
    ...
    */
    if (handler_tables_hash.records)
      myblockchain_ha_set_explicit_lock_duration(this);
    if (ull_hash.records)
      myblockchain_ull_set_explicit_lock_duration(this);
  }
  locked_tables_mode= LTM_NONE;
}
...
void THD::set_currently_executing_gtid_for_slave_thread()
{
  /*
    ...
    - From a normal client thread that is executing output from
      myblockchainbinlog when myblockchainbinlog is processing an old binlog file
...
{
  myblockchain_mutex_assert_owner(&LOCK_user_conn);
...
{
  if (current_thd != thd)
    myblockchain_mutex_assert_owner(&thd->LOCK_query_plan);
}
#endif
...
/**
  Push an error message into MyBlockchain diagnostic area with line
  ...
  switch (da->status())
  {
    case Diagnostics_area::DA_ERROR:
      ...
      error= m_protocol->send_error(
              da->myblockchain_errno(), da->message_text(), da->returned_sqlstate());
          break;
