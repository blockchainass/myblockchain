...
class File_query_log
{
  ...
  : m_log_type(log_type), name(NULL), write_error(false), log_open(false)
  {
    ...
    myblockchain_mutex_init(key_LOG_LOCK_log, &LOCK_log, MY_MUTEX_INIT_SLOW);
...
{
  File_query_log myblockchain_general_log;
  File_query_log myblockchain_slow_log;
...
private:
  Log_to_file_event_handler()
    : myblockchain_general_log(QUERY_LOG_GENERAL),
    myblockchain_slow_log(QUERY_LOG_SLOW)
  { }

  /** Close slow and general log files. */
  void cleanup()
  {
    myblockchain_general_log.close();
    myblockchain_slow_log.close();
  }
  ...
  File_query_log *get_query_log(enum_log_table_type log_type)
  {
    if (log_type == QUERY_LOG_SLOW)
      return &myblockchain_slow_log;
    DBUG_ASSERT(log_type == QUERY_LOG_GENERAL);
    return &myblockchain_general_log;
  }
...
class Query_logger
{
  ...
  */
  myblockchain_rwlock_t LOCK_logger;
...
public:
  ...
  */
  void init()
  {
    ...
    myblockchain_rwlock_init(key_rwlock_LOCK_logger, &LOCK_logger);
  }
...
#ifdef MYBLOCKCHAIN_SERVER // Security_context not defined otherwise.
...
private:
  ...
  */
  myblockchain_mutex_t *LOCK_log_throttle;
...
public:
  ...
  /**
    ...
    @param msg           use this template containing %lu as only non-literal
  */
  Slow_log_throttle(ulong *threshold, myblockchain_mutex_t *lock, ulong window_usecs,
...
/**
   ...
   to its numeric value on the platform this myblockchaind was compiled for.
