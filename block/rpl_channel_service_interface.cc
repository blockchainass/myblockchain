                  ...
                  int wait_for_connection)
{
  ...
  if (wait_for_connection && (thread_mask & SLAVE_IO) && !error)
  {
    ...
    */
    ...
    {
      myblockchain_cond_wait(&mi->start_cond, &mi->run_lock);
    }
    myblockchain_mutex_unlock(&mi->run_lock);

    while (mi->slave_running != MYBLOCKCHAIN_SLAVE_RUN_CONNECT)
    {
      //If there is such a state change then there was an error on connection
      if (mi->slave_running == MYBLOCKCHAIN_SLAVE_NOT_RUN)
      {
  ...
  if (mi->rli != NULL)
  {
    ...
    if (num_workers > 1)
    {
      *appliers_id=
      ...
      {
        myblockchain_mutex_lock(&mi->rli->workers.at(i)->info_thd_lock);
        *appliers_id_pointer= mi->rli->workers.at(i)->info_thd->thread_id();
        myblockchain_mutex_unlock(&mi->rli->workers.at(i)->info_thd_lock);
      }
