...
enum {
  ...
  /* MyBlockchain Cluster 6.3 added master_bind */
...
Master_info::Master_info(
   ...
   port(MYBLOCKCHAIN_PORT), connect_retry(DEFAULT_CONNECT_RETRY),
...
bool Master_info::read_info(Rpl_info_handler *from)
{
  ...
  */
  ...
  if (from->get_info(&temp_master_log_pos,
      ...
      from->get_info((int *) &port, (int) MYBLOCKCHAIN_PORT) ||
  ...
  */
  ...
  if (ssl)
    sql_print_warning("SSL information in the master info file "
                      "are ignored because this MyBlockchain slave was "
