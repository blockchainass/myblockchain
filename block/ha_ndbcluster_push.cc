                           ...
                           const AQP::Table_access* table)
{
   ...
   *     FIELD_ITEMs as set up by the MyBlockchain optimizer.
  ...
  if (!ndbcluster_is_lookup_operation(table->get_access_type()))
  {
     ...
     * myblockchaind would be confused since the rows are not grouped on t1 values.
  ...
  else
  {
    ...
    for (uint i= 0; i < key_fields; i++, key_part++)
    {
      ...
      if (item->const_item())
      {
        ...
        const uchar* const ptr= (field->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR)
