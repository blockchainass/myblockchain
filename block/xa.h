...
#include "myblockchaind.h"           // server_id
#include "sql_cmd.h"
...
#define MYBLOCKCHAIN_XID_PREFIX "MyBlockchainXid"
#define XIDDATASIZE MYBLOCKCHAIN_XIDDATASIZE
class XID_STATE;
...
{
private:
  static const uint MYBLOCKCHAIN_XID_PREFIX_LEN= 8; // must be a multiple of 8
  static const uint MYBLOCKCHAIN_XID_OFFSET= MYBLOCKCHAIN_XID_PREFIX_LEN + sizeof(server_id);
  static const uint MYBLOCKCHAIN_XID_GTRID_LEN= MYBLOCKCHAIN_XID_OFFSET + sizeof(my_xid);
...
public:
  ...
  {
    if (gtrid_length == static_cast<long>(MYBLOCKCHAIN_XID_GTRID_LEN) &&
        bqual_length == 0 &&
        !memcmp(data, MYBLOCKCHAIN_XID_PREFIX, MYBLOCKCHAIN_XID_PREFIX_LEN))
    {
      my_xid tmp;
      memcpy(&tmp, data + MYBLOCKCHAIN_XID_OFFSET, sizeof(tmp));
      return tmp;
    }
    return 0;
  }
...
private:
  ...
  {
    formatID= 1;
    memcpy(data, MYBLOCKCHAIN_XID_PREFIX, MYBLOCKCHAIN_XID_PREFIX_LEN);
    memcpy(data + MYBLOCKCHAIN_XID_PREFIX_LEN, &server_id, sizeof(server_id));
    memcpy(data + MYBLOCKCHAIN_XID_OFFSET, &xid, sizeof(xid));
    gtrid_length= MYBLOCKCHAIN_XID_GTRID_LEN;
    bqual_length= 0;
  }
...
public:
  ...
  */
  ...
  /**
    ...
    to a myblockchain user in case when there is an uncommitted work for XA transaction.
