...
int sys_var_init()
{
  ...
  if (myblockchain_add_sys_var_chain(all_sys_vars.first))
    goto error;
...
/**
  ...
  @param chain     variables are linked into chain for myblockchain_add_sys_var_chain()
  ...
  @param comment   shown in myblockchaind --help, @sa my_option::comment
  ...
  is_os_charset(FALSE)
{
  /*
    ...
    for myblockchaind, we enforce here that no short option is present
...
bool sys_var::update(THD *thd, set_var *var)
{
  ...
  else
  {
    ...
    if (!show_compatibility_56)
    {
      ...
      myblockchain_mutex_lock(&thd->LOCK_thd_sysvar);
      locked= true;
    }
    ...
    if (locked)
      myblockchain_mutex_unlock(&thd->LOCK_thd_sysvar);
...
int myblockchain_add_sys_var_chain(sys_var *first)
{
  sys_var *var;
...
/*
  ...
  SYNOPSIS
    myblockchain_del_sys_var_chain()
...
int myblockchain_del_sys_var_chain(sys_var *first)
{
  int result= 0;
                        ...
                        bool strict)
{
  ...
  if (show_var_array)
  {
    ...
    show_var_array->push_back(st_myblockchain_show_var());
  }
...
/*****************************************************************************
  Functions to handle SET myblockchain_internal_variable=const_expr
*****************************************************************************/
