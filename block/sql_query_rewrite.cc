...
void invoke_pre_parse_rewrite_plugins(THD *thd)
{
  ...
  myblockchain_audit_notify(thd, MYBLOCKCHAIN_AUDIT_PARSE_CLASS, MYBLOCKCHAIN_AUDIT_PREPARSE,
                     &flags, //flags
                     thd->query().str, // query
...
{
  if (is_audit_plugin_class_active(thd, MYBLOCKCHAIN_AUDIT_PARSE_CLASS))
...
bool invoke_post_parse_rewrite_plugins(THD *thd, my_bool is_prepared)
{
  ...
  */
  ...
  myblockchain_audit_notify(thd, MYBLOCKCHAIN_AUDIT_PARSE_CLASS, MYBLOCKCHAIN_AUDIT_POSTPARSE,
                     &flags,
                     original_query.str,
  ...
  if (plugin_da->current_statement_cond_count() != 0)
  {
    ...
    */
    ...
    if (plugin_da->is_error())
      stmt_da->set_error_status(plugin_da->myblockchain_errno(),
