...
bool Sql_cmd_delete::myblockchain_delete(THD *thd, ha_rows limit)
{
  DBUG_ENTER("myblockchain_delete");
  ...
  if (myblockchain_prepare_delete(thd))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  /*
    ...
    HA_EXTRA_NO_IGNORE_DUP_KEY flag should be removed from myblockchain_delete(),
  ...
  */
  if (!using_limit && const_cond_result &&
      ...
      ((!thd->is_current_stmt_binlog_format_row() ||   /* not ROW binlog-format */
        ...
        !myblockchain_bin_log.is_open()) &&                   /* binary log is not opened */
      ...
      if (!qep_tab.skip_record(thd, &skip_record) && !skip_record)
      {
        ...
        else
        {
          ...
          /*
            ...
            was not sensible, because then MyBlockchain would not roll back the
...
bool Sql_cmd_delete::myblockchain_prepare_delete(THD *thd)
{
  DBUG_ENTER("myblockchain_prepare_delete");
...
int Sql_cmd_delete_multi::myblockchain_multi_delete_prepare(THD *thd,
                                                     uint *table_count)
{
  DBUG_ENTER("myblockchain_multi_delete_prepare");
...
bool Sql_cmd_delete::execute(THD *thd)
{
  ...
  MYBLOCKCHAIN_DELETE_START(const_cast<char*>(thd->query().str));
  bool res = myblockchain_delete(thd, unit->select_limit_cnt);
  MYBLOCKCHAIN_DELETE_DONE(res, (ulong) thd->get_row_count_func());
...
bool Sql_cmd_delete_multi::execute(THD *thd)
{
  ...
  MYBLOCKCHAIN_MULTI_DELETE_START(const_cast<char*>(thd->query().str));
  if (myblockchain_multi_delete_prepare(thd, &del_table_count))
  {
    MYBLOCKCHAIN_MULTI_DELETE_DONE(1, 0);
    return true;
  }
      ...
      (del_result= new Query_result_delete(aux_tables, del_table_count)))
  {
    ...
    MYBLOCKCHAIN_MULTI_DELETE_DONE(res, del_result->num_deleted());
    delete del_result;
  }
  else
  {
