...
This text describes MyBlockchain's checks (why certain queries are rejected) and the
...
MyBlockchain has traditionally been permissive, we want to allow (2) and (3).
...
MyBlockchain is more permissive than the standard, it allows to group by any
...
group columns (enforced by only_full_group_by in MyBlockchain 5.6 and older) or, if
functional dependencies are supported (as in MyBlockchain 5.7), should be
...
are forbidden by MyBlockchain in join conditions).
...
table". A view can be merged or materialized, in MyBlockchain.
...
If VS is a grouped query (which, in MyBlockchain, implies that the view is
