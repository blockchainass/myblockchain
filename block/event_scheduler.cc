...
/*
  Performs pre- myblockchain_thread_create() initialisation of THD. Do this
...
Event_worker_thread::run(THD *thd, Event_queue_element_for_exec *event)
{
  ...
  thd->m_statement_psi= MYBLOCKCHAIN_START_STATEMENT(& state,
...
Event_scheduler::Event_scheduler(Event_queue *queue_arg)
  ...
  started_events(0)
{
  myblockchain_mutex_init(key_event_scheduler_LOCK_scheduler_state,
                   &LOCK_scheduler_state, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_event_scheduler_COND_state, &COND_state);
}
...
Event_scheduler::~Event_scheduler()
{
  ...
  myblockchain_mutex_destroy(&LOCK_scheduler_state);
  myblockchain_cond_destroy(&COND_state);
}
...
Event_scheduler::start(int *err_no)
{
  ...
  /*
    ...
    In particular, this is needed to be able to lock the myblockchain.event table
  ...
  */
  ...
  if ((*err_no= myblockchain_thread_create(key_thread_event_scheduler,
...
Event_scheduler::run(THD *thd)
{
  ...
  */
  ...
  myblockchain_cond_broadcast(&COND_state);
  UNLOCK_DATA();
...
Event_scheduler::execute_top(Event_queue_element_for_exec *event_name)
{
  ...
  */
  /* Major failure */
  if ((res= myblockchain_thread_create(key_thread_event_worker,
...
Event_scheduler::stop()
{
  ...
  do {
    ...
    */
    ...
    /* Lock from delete */
    myblockchain_mutex_lock(&scheduler_thd->LOCK_thd_data);
    /* This will wake up the thread if it waits on Queue's conditional */
    sql_print_information("Event Scheduler: Killing the scheduler thread, "
                          "thread id %u",
                          scheduler_thd->thread_id());
    scheduler_thd->awake(THD::KILL_CONNECTION);
    myblockchain_mutex_unlock(&scheduler_thd->LOCK_thd_data);
...
Event_scheduler::lock_data(const char *func, uint line)
{
  ...
  myblockchain_mutex_lock(&LOCK_scheduler_state);
...
Event_scheduler::unlock_data(const char *func, uint line)
{
  ...
  myblockchain_mutex_unlock(&LOCK_scheduler_state);
  DBUG_VOID_RETURN;
}


/*
  Wrapper for myblockchain_cond_wait/timedwait

  SYNOPSIS
    Event_scheduler::cond_wait()
      thd     Thread (Could be NULL during shutdown procedure)
      abstime If not null then call myblockchain_cond_timedwait()
                           ...
                           const char *src_func, const char *src_file, uint src_line)
{
  ...
  DBUG_PRINT("info", ("myblockchain_cond_%swait", abstime? "timed":""));
  if (!abstime)
    myblockchain_cond_wait(&COND_state, &LOCK_scheduler_state);
  else
    myblockchain_cond_timedwait(&COND_state, &LOCK_scheduler_state, abstime);
  if (thd)
  {
    /*
