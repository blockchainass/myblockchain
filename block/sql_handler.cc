...
/* TODO:
  ...
  compute List<Item> field_list not in myblockchain_ha_read
  but in myblockchain_ha_open, and then store it in TABLE structure.

  The problem here is that myblockchain_parse calls free_item to free all the
  items allocated at the end of every query. The workaround would to
  keep two item lists per THD - normal free_list and handler_items.
  The second is to be freeed only on thread end. myblockchain_ha_open should
...
static bool myblockchain_ha_open_table(THD *thd, TABLE_LIST *table);

/*
  Get hash key and hash key length.

  SYNOPSIS
    myblockchain_ha_hash_get_key()
...
static const char *myblockchain_ha_hash_get_key(TABLE_LIST *tables,
                                         size_t *key_len_p,
...
static void myblockchain_ha_hash_free(TABLE_LIST *tables)
{
  my_free(tables);
}
...
static void myblockchain_ha_close_table(THD *thd, TABLE_LIST *tables)
{
  ...
  if (! my_hash_inited(&thd->handler_tables_hash))
  {
    ...
    */
    if (my_hash_init(&thd->handler_tables_hash, &my_charset_latin1,
                     ...
                     (my_hash_get_key) myblockchain_ha_hash_get_key,
                     (my_hash_free_key) myblockchain_ha_hash_free, 0,
  ...
  */
  ...
  if (open_temporary_tables(thd, hash_tables) ||
      check_table_access(thd, SELECT_ACL, hash_tables, FALSE, UINT_MAX,
                         FALSE) ||
      myblockchain_ha_open_table(thd, hash_tables))
...
static bool myblockchain_ha_open_table(THD *thd, TABLE_LIST *hash_tables)
{
  ...
  DBUG_ENTER("myblockchain_ha_open_table");
...
bool Sql_cmd_handler_close::execute(THD *thd)
{
  ...
  if ((hash_tables= (TABLE_LIST*) my_hash_search(&thd->handler_tables_hash,
                                                 ...
                                                 strlen(tables->alias) + 1)))
  {
    myblockchain_ha_close_table(thd, hash_tables);
    my_hash_delete(&thd->handler_tables_hash, (uchar*) hash_tables);
  }
  else
  {
...
retry:
                                                 ...
                                                 strlen(tables->alias) + 1)))
  {
    ...
    if (!table)
    {
      ...
      */
      if (myblockchain_ha_open_table(thd, hash_tables))
      {
  ...
  */
  ...
  backup_open_tables= thd->open_tables;
  /* Always a one-element list, see myblockchain_ha_open(). */
  DBUG_ASSERT(hash_tables->table->next == NULL ||
              hash_tables->table->s->tmp_table);
  /*
    myblockchain_lock_tables() needs thd->open_tables to be set correctly to
  ...
  */
  ...
  lock= myblockchain_lock_tables(thd, &thd->open_tables, 1, 0);

  thd->pop_internal_handler();
  /*
    In 5.1 and earlier, myblockchain_lock_tables() could replace the TABLE
  ...
  */
  ...
  if (!lock)
    goto err1; // myblockchain_lock_tables() printed error message already
  ...
  */
  ...
  for (num_rows=0; num_rows < select_limit_cnt; )
  {
    ...
    if (error)
    {
      ...
      {
        sql_print_error("myblockchain_ha_read: Got error %d when reading table '%s'",
...
static TABLE_LIST *myblockchain_ha_find(THD *thd, TABLE_LIST *tables)
{
  TABLE_LIST *hash_tables, *head= NULL, *first= tables;
  DBUG_ENTER("myblockchain_ha_find");
...
void myblockchain_ha_rm_tables(THD *thd, TABLE_LIST *tables)
{
  TABLE_LIST *hash_tables, *next;
  DBUG_ENTER("myblockchain_ha_rm_tables");

  DBUG_ASSERT(tables);

  hash_tables= myblockchain_ha_find(thd, tables);
...
void myblockchain_ha_flush_tables(THD *thd, TABLE_LIST *all_tables)
{
  DBUG_ENTER("myblockchain_ha_flush_tables");

  for (TABLE_LIST *table_list= all_tables; table_list;
       table_list= table_list->next_global)
  {
    TABLE_LIST *hash_tables= myblockchain_ha_find(thd, table_list);
...
void myblockchain_ha_flush(THD *thd)
{
  TABLE_LIST *hash_tables;
  DBUG_ENTER("myblockchain_ha_flush");
...
/**
  ...
  myblockchain_ha_rm_tables() is that it is not always feasible (e.g. in
  ...
  @param thd Thread identifier.
*/
void myblockchain_ha_rm_temporary_tables(THD *thd)
{
  DBUG_ENTER("myblockchain_ha_rm_temporary_tables");
  ...
  while (tmp_handler_tables)
  {
    ...
    myblockchain_ha_close_table(thd, tmp_handler_tables);
...
void myblockchain_ha_cleanup(THD *thd)
{
  TABLE_LIST *hash_tables;
  DBUG_ENTER("myblockchain_ha_cleanup");
...
void myblockchain_ha_set_explicit_lock_duration(THD *thd)
{
  TABLE_LIST *hash_tables;
  DBUG_ENTER("myblockchain_ha_set_explicit_lock_duration");
