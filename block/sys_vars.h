...
#include "myblockchain/plugin.h"         // enum_myblockchain_show_type
#include "item.h"                 // Item
...
*/
template
  <typename T, ulong ARGT, enum enum_myblockchain_show_type SHOWT, bool SIGNED>
...
public:
  ...
  bool do_check(THD *thd, set_var *var)
  {
    ...
    if (SIGNED) /* target variable has signed type */
    {
      ...
      {
        /*
          Input value is such a large positive number that MyBlockchain used an
...
public:
  ...
  /**
    ...
    alias will be used in myblockchaind --help to show the default value.
  ...
  */
  ...
  /**
    ...
    this function from myblockchaind.cc:myblockchaind_get_one_option.
...
/**
  ...
  Class specific constructor arguments:
    int plugin_type_arg (for example MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN)

  Backing store: plugin_ref

  @note
  these variables don't support command-line equivalents, any such
  command-line options should be added manually to my_long_options in myblockchaind.cc
*/
...
public:
  ...
  bool do_check(THD *thd, set_var *var)
  {
    ...
    if (plugin_type == MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN)
...
public:
      ...
      name_offset(name_off)
  {
    ...
    /*
      ...
      to my_long_options in myblockchaind.cc
    */
    DBUG_ASSERT(getopt.id == -1);
  ...
  bool global_update(THD *thd, set_var *var)
  {
    ...
    */
    ...
    // Rotate
    {
      ...
      if (myblockchain_bin_log.rotate(true, &dont_care))
        goto err;
    }
