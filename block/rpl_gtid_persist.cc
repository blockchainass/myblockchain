...
#include "sql_base.h"         // MYBLOCKCHAIN_OPEN_IGNORE_GLOBAL_READ_LOCK
...
const LEX_STRING Gtid_table_access_context::DB_NAME= {C_STRING_WITH_LEN("myblockchain")};
...
THD *Gtid_table_access_context::create_thd()
{
  ...
  /*
    ...
    both lex_start() and myblockchain_reset_thd_for_next_command.
  */
  lex_start(thd);
...
void Gtid_table_access_context::before_open(THD* thd)
{
  ...
  */
  m_flags= (MYBLOCKCHAIN_OPEN_IGNORE_GLOBAL_READ_LOCK |
...
int Gtid_table_persistor::save(THD *thd, const Gtid *gtid)
{
  ...
  if (error == 0 && gtid_executed_compression_period != 0)
  {
    ...
    {
      myblockchain_mutex_lock(&LOCK_compress_gtid_table);
      should_compress= true;
      myblockchain_cond_signal(&COND_compress_gtid_table);
      myblockchain_mutex_unlock(&LOCK_compress_gtid_table);
    }
  }
                                                         ...
                                                         bool &is_complete)
{
  ...
  myblockchain_mutex_lock(&LOCK_reset_gtid_table);
  if (table_access_ctx.init(&thd, &table, true))
  {
    error= 1;
    goto end;
  ...
  */
  ...
  table_access_ctx.deinit(thd, table, 0 != error, true);
  myblockchain_mutex_unlock(&LOCK_reset_gtid_table);
...
extern "C" void *compress_gtid_table(void *p_thd)
{
  ...
  for (;;)
  {
    ...
    while(!(should_compress || terminate_compress_thread))
      myblockchain_cond_wait(&COND_compress_gtid_table, &LOCK_compress_gtid_table);
...
#endif
      (error= myblockchain_thread_create(key_thread_compress_gtid_table,
