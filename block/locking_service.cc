...
int acquire_locking_service_locks(MYBLOCKCHAIN_THD opaque_thd,
...
int release_locking_service_locks(MYBLOCKCHAIN_THD opaque_thd,
                                  const char *lock_namespace)
{
  // Check that namespace length is acceptable
  if (check_lock_name(lock_namespace))
...
/*
  ...
  in service_locking.h as UDFs are built with MYBLOCKCHAIN_DYNAMIC_PLUGIN
