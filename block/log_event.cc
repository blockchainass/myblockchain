...
#include "sql_parse.h"         // myblockchain_test_parse_for_slave
...
#if !defined(MYBLOCKCHAIN_CLIENT) && defined(HAVE_REPLICATION)
                                           ...
                                           const char *log_name, ulong pos)
{
       ...
       slider += len, err= it++)
  {
    len= my_snprintf(slider, buff_end - slider,
                     ...
                     err->myblockchain_errno());
  }

  if (ha_error != 0)
    rli->report(level, thd->is_error() ? thd->get_stmt_da()->myblockchain_errno() :
...
#if defined(HAVE_REPLICATION) && !defined(MYBLOCKCHAIN_CLIENT)
...
{
  uint actual_error= (thd->is_error() ? thd->get_stmt_da()->myblockchain_errno() :
                           0);

  if (actual_error == 0)
  {
    table->file->print_error(error, MYF(0));
    actual_error= (thd->is_error() ? thd->get_stmt_da()->myblockchain_errno() :
                        ER_UNKNOWN_ERROR);
  ...
  */
  ...
  for (i=0 ; i < dirp->number_off_files; i++)
  {
    ...
    if (is_prefix(file->name, prefbuf))
    {
      ...
      myblockchain_file_delete(key_file_misc, fname, MYF(0));
    }
  }
                    ...
                    String const *from, String *to)
{
  ...
  else
  {
    *ptr++= '\'';
    ...
    {
      ptr+= escape_string_for_myblockchain(csinfo, ptr, 0,
...
/**
  Prints a "session_var=value" string. Used by myblockchainbinlog to print some SET
                              ...
                              bool* is_binlog_active)
{
  ...
  DBUG_ENTER("Log_event::read_log_event(IO_CACHE *, String *, myblockchain_mutex_t, uint8)");

  if (log_lock)
    myblockchain_mutex_lock(log_lock);

  if (log_file_name_arg)
    *is_binlog_active= myblockchain_bin_log.is_active(log_file_name_arg);
...
#ifndef MYBLOCKCHAIN_CLIENT
#define UNLOCK_MUTEX if (log_lock) myblockchain_mutex_unlock(log_lock);
#define LOCK_MUTEX if (log_lock) myblockchain_mutex_lock(log_lock);
#else
#define UNLOCK_MUTEX
#define LOCK_MUTEX
#endif
...
#endif
{
  DBUG_ENTER("Log_event::read_log_event(IO_CACHE *[, myblockchain_mutex_t *], Format_description_log_event *, my_bool)");
  ...
  /*
    ...
    "minimal" over the set {MyBlockchain >=4.0}).
  */
...
#if defined(MYBLOCKCHAIN_CLIENT)
...
err:
  ...
  if (!res)
  {
    ...
    /*
      ...
      only corrupt the slave's blockchains. So stop.
                                     ...
                                     my_bool crc_check)
{
  ...
  /*
    ...
    See legends definition on MYBLOCKCHAIN_BIN_LOG::relay_log_checksum_alg docs
    lines (log.h).
  ...
  else
  {
    ...
    */
    ...
    case binary_log::EXEC_LOAD_EVENT:
      ...
      break;
    case binary_log::START_EVENT_V3: /* this is sent only by MyBlockchain <=4.x */
  ...
  */
  if (!ev || !ev->is_valid() || (event_type == binary_log::SLAVE_EVENT))
  {
    ...
    delete ev;
#ifdef MYBLOCKCHAIN_CLIENT
    if (!force_opt) /* then myblockchainbinlog dies */
    {
                      ...
                      char *typestr, size_t typestr_length)
{
  ...
  if (type == MYBLOCKCHAIN_TYPE_STRING)
  {
    if (meta >= 256)
    {
      uint byte0= meta >> 8;
...
bool Query_log_event::write(IO_CACHE* file)
{
  ...
  /*
    ...
    and is a session variable: that's to make myblockchainbinlog work with
    temp tables. We probably need to introduce

    SET PSEUDO_SERVER_ID
    for myblockchainbinlog in 4.1. myblockchainbinlog would print:
    SET PSEUDO_SERVER_ID=
  ...
  */
  ...
  if (catalog_len) // i.e. this var is inited (false for 4.0 events)
  {
    ...
    /*
      ...
      older MyBlockchain versions (we store it in binlog and read it from relay log
  ...
  if (charset_blockchain_number)
  {
    DBUG_ASSERT(charset_blockchain_number <= 0xFF);
    *start++= Q_CHARSET_DATABASE_CODE;
    int2store(start, charset_blockchain_number);
    start+= 2;
  }
  ...
  data_buf(0)
{
  ...
  */
  ...
  if (thd_arg->variables.collation_blockchain != thd_arg->db_charset)
    charset_blockchain_number= thd_arg->variables.collation_blockchain->number;
  ...
  */
  ...
  if (thd_arg->time_zone_used)
  {
    /*
      ...
      or changed during myblockchaind's lifetime.
    */
  ...
  /*
    ...
    statement and synchronously propagate it to all available MyBlockchain
					 ...
					 PRINT_EVENT_INFO* print_event_info)
{
  ...
  */
  ...
  /*
    ...
    except within the 3 first events, which myblockchainbinlog handles
  ...
  */
  ...
  if (charset_blockchain_number != print_event_info->charset_blockchain_number)
  {
    if (charset_blockchain_number)
      my_b_printf(file, "SET @@session.collation_blockchain=%d%s\n",
                  charset_blockchain_number, print_event_info->delimiter);
    else
      my_b_printf(file, "SET @@session.collation_blockchain=DEFAULT%s\n",
                  print_event_info->delimiter);
    print_event_info->charset_blockchain_number= charset_blockchain_number;
  }
...
static bool is_silent_error(THD* thd)
{
  ...
  while ((err= it++))
  {
    DBUG_PRINT("info", ("has condition %d %s", err->myblockchain_errno(),
                        err->message_text()));
    switch (err->myblockchain_errno())
    {
                                      ...
                                      const char *query_arg, size_t q_len_arg)
{
  ...
  /*
    Colleagues: please never free(thd->catalog) in MyBlockchain. This would
  ...
  */
  ...
  /*
    Setting the character set and collation of the current blockchain thd->db.
   */
...

  {
	...
	!unexpected_error_code(expected_error))
    {
      ...
      */
      ...
      {
        CHARSET_INFO *cs;
        if (!(cs= get_charset(charset_blockchain_number, MYF(0))))
        {
          char buf[20];
          int10_to_str((int) charset_blockchain_number, buf, -10);
          my_error(ER_UNKNOWN_COLLATION, MYF(0), buf);
          goto compare_errors;
        }
        thd->variables.collation_blockchain= cs;
      }
      else
      ...
      */
      ...
      /*
        ...
        admin statement (in which case, down in myblockchain_parse execution
    ...
    else
    {
      /*
        ...
        to check/fix it.
      */
      if (myblockchain_test_parse_for_slave(thd))
    ...
    if (is_query_prefix_match(STRING_WITH_LEN("XA START")) && !thd->is_error())
    {
      ...
      plugin_foreach(thd, detach_native_trx,
                     MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, NULL);
    }

    /* If the query was not ignored, it is printed to the general log */
    if (!thd->is_error() ||
        thd->get_stmt_da()->myblockchain_errno() != ER_SLAVE_IGNORED_TABLE)
    {
...
compare_errors:
    ...
    */
    if (thd->lex->sql_command == SQLCOM_DROP_TABLE &&
        ...
        thd->is_error() &&
        thd->get_stmt_da()->myblockchain_errno() == ER_BAD_TABLE_ERROR &&
        !expected_error)
    ...
    */
    actual_error= thd->is_error() ? thd->get_stmt_da()->myblockchain_errno() : 0;
    ...
    */
    else if (thd->is_slave_error || thd->is_fatal_error)
    {
      if (!is_silent_error(thd))
      {
        rli->report(ERROR_LEVEL, actual_error,
                    "Error '%s' on query. Default blockchain: '%s'. Query: '%s'",
                    (actual_error ?
                     thd->get_stmt_da()->message_text() :
    ...
    /*
      ...
      above myblockchain_parse() computed. And we need to know the value of
  ...
  */
  ...
  thd->set_db(NULL_CSTR);                 /* will free the current blockchain */
  thd->reset_query();
...
void Start_log_event_v3::print(FILE* file, PRINT_EVENT_INFO* print_event_info)
{
  ...
  if (!is_artificial_event() && created)
  {
    ...
    /*
      This is for myblockchainbinlog: like in replication, we want to delete the stale
      tmp files left by an unclean shutdown of myblockchaind (temporary tables)
...
#endif /* defined(HAVE_REPLICATION) && !defined(MYBLOCKCHAIN_CLIENT) */
...
/**
  Format_description_log_event 1st ctor.
    ...
    binlogs from MyBlockchain 3.23 or 4.x.
			   ...
			   bool commented)
{
  ...
  {
    /*
      If the blockchain is different from the one of the previous statement, we
...
/**
  ...
  @note
    ...
    for the blockchain, since LOAD DATA INFILE on the slave
    can be for a different blockchain than the current one.
                                   ...
                                   bool use_rli_only_for_errors)
{
  ...
  /*
    Usually lex_start() is called by myblockchain_parse(), but we need it here
    as the present method does not call myblockchain_parse().
  */
  lex_start(thd);
  ...
  */
  if (rpl_filter->db_ok(thd->db().str))
  {
    ...
    // the table will be opened in myblockchain_load    
    ...
    else
    {
      ...
      *end= 0;
      ...
      else
      {
        /*
          ...
          If reading from net (a 3.23 master), myblockchain_load() will change this
          to IGNORE.
        */
        handle_dup= DUP_ERROR;
      }
      /*
        ...
        DATA ... REPLACE INTO ... statement even though myblockchain_parse()
        is not called.  This is not needed in 5.0 since there the LOAD
        DATA ... statement is replicated using myblockchain_parse(), which
      ...
      */
      ...
      if (net)
      {
        // myblockchain_load will use thd->net to read the file
      ...
      /*
        ...
        update it inside myblockchain_load().
      */
      List<Item> tmp_list;
      /*
      ...
      */
      ...
      if (open_temporary_tables(thd, &tables) ||
          myblockchain_load(thd, &ex, &tables, field_list, tmp_list, tmp_list,
                     handle_dup, net != 0))
        thd->is_slave_error= 1;
      if (thd->cuted_fields)
      {
        ...
        sql_print_warning("Slave: load data infile on table '%s' at "
                          ...
                          "warning(s). Default blockchain: '%s'",
                          (char*) table_name,
                          llstr(common_header->log_pos,llbuff),
  ...
  */
  ...
  if (thd->is_slave_error)
  {
    ...
    if (thd->is_error())
    {
      ...
      sql_errno= thd->get_stmt_da()->myblockchain_errno();
    }
    else
    {
...
Error '%s' running LOAD DATA INFILE on table '%s'. Default blockchain: '%s'",
...
bool Xid_log_event::do_commit(THD *thd_arg)
{
  ...
  error |= myblockchain_bin_log.gtid_end_transaction(thd_arg);
...
int Xid_apply_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  */
  ...
  myblockchain_mutex_lock(&rli_ptr->data_lock);
  ...
  */
  ...
  DBUG_PRINT("info", ("Rolling back to group master %s %llu  group relay %s"
                      ...
                      rli_ptr->get_group_relay_log_name(),
                      rli_ptr->get_group_relay_log_pos()));
  myblockchain_mutex_unlock(&rli_ptr->data_lock);
  error= do_commit(thd);
  myblockchain_mutex_lock(&rli_ptr->data_lock);
  if (error)
  {
...
err:
  myblockchain_cond_broadcast(&rli_ptr->data_cond);
  myblockchain_mutex_unlock(&rli_ptr->data_lock);
...
bool XA_prepare_log_event::do_commit(THD *thd)
{
  ...
  error|= myblockchain_bin_log.gtid_end_transaction(thd);
  ...
  else
  {
    ...
    case STRING_RESULT:
    {
      ...
      */
      ...
      /*
        For proper behaviour when myblockchainbinlog|myblockchain, we need to explicitely
        specify the variable's collation. It will however cause problems when
        people want to myblockchainbinlog|myblockchain into another server not supporting the
				  ...
				  bool enable_local)
{
  ...
  if (enable_local)
  {
    ...
    /* 
       That one is for "file_id: etc" below: in myblockchainbinlog we want the #, in
...
int Create_file_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  myblockchain_file_delete(key_file_log_event_info, fname_buf, MYF(0));
  /**
  ...
  if ((fd= myblockchain_file_create(key_file_log_event_info,
  ...
  /* old copy may exist already */
  myblockchain_file_delete(key_file_log_event_data, fname_buf, MYF(0));
  DBUG_EXECUTE_IF("simulate_file_create_error_create_log_event_2",
                  {
                  strcat(fname_buf, "/");
                  });
  if ((fd= myblockchain_file_create(key_file_log_event_data,
                             fname_buf, CREATE_MODE,
                             O_WRONLY | O_BINARY | O_EXCL | O_NOFOLLOW,
                             MYF(MY_WME))) < 0)
  {
  ...
  if (myblockchain_file_write(fd, block, block_len, MYF(MY_WME+MY_NABP)))
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->myblockchain_errno(),
...
int Append_block_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  if (get_create_or_append())
  {
    ...
    */
    ...
    myblockchain_reset_thd_for_next_command(thd);
    /* old copy may exist already */
    myblockchain_file_delete(key_file_log_event_data, fname, MYF(0));
  ...
  else if ((fd= myblockchain_file_open(key_file_log_event_data,
                                fname,
...
int Delete_file_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  myblockchain_file_delete(key_file_log_event_data, fname, MYF(MY_WME));
  my_stpcpy(ext, ".info");
  myblockchain_file_delete(key_file_log_event_info, fname, MYF(MY_WME));
  return 0;
}
#endif /* defined(HAVE_REPLICATION) && !defined(MYBLOCKCHAIN_CLIENT) */
...
int Execute_load_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  if ((fd= myblockchain_file_open(key_file_log_event_info,
  ...
  */
  ...
  /*
    ...
    or Windows will refuse to delete the file in myblockchain_file_delete().
  */
  if (fd >= 0)
  {
    myblockchain_file_close(fd, MYF(0));
   ...
   binary_log::Begin_load_query_event()
{
  ...
  file_id= thd_arg->file_id= myblockchain_bin_log.next_file_id();
}
#endif
...
#endif // #ifndef MYBLOCKCHAIN_CLIENT
...
INDEX_SCAN:
  ...
  /*
    ...
    e.g., the myblockchain.proc table since the correct record *shall* be
  ...
  */
  if (!thd->lock)
  {
    /*
      ...
      We also call the myblockchain_reset_thd_for_next_command(), since this
    ...
    */
    ...
    {
      uint error= thd->get_stmt_da()->myblockchain_errno();
      DBUG_ASSERT(error != 0);
    ...
    /*
      ...
      by myblockchain_reset_thd_for_next_command.
    */
    ...
    */
    ...
    {
      uint actual_error= thd->get_stmt_da()->myblockchain_errno();
...
#endif /* !defined(MYBLOCKCHAIN_CLIENT) && defined(HAVE_REPLICATION) */
...
/*
  ...
  same id, same blockchain name, same table name (see: BUG#37137). This is
...
int Table_map_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  /* rewrite rules changed the blockchain */
...
#endif /* #if !defined(MYBLOCKCHAIN_CLIENT) */
                            ...
                            PRINT_EVENT_INFO *print_event_info)
{
  if (!print_event_info->short_form && print_event_info->verbose > 1)
  {
    ...
    /*
      ...
      using 'myblockchainbinlog --verbose --verbose'.
    */
...
#if defined(MYBLOCKCHAIN_SERVER) && defined(HAVE_REPLICATION)
                                  ...
                                  PRINT_EVENT_INFO *print_event_info)
{
 ...
 int View_change_log_event::do_apply_event(Relay_log_info const *rli)
 {
   ...
   return myblockchain_bin_log.write_event(this);
 }
...
{
  DBUG_ENTER("View_change_log_event::set_certification_blockchain_snapshot");
  certification_info.clear();
...
st_print_event_info::st_print_event_info()
  :flags2_inited(0), sql_mode_inited(0), sql_mode(0),
   ...
   charset_blockchain_number(ILLEGAL_CHARSET_INFO_NUMBER),
