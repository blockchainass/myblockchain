...
static void init_all_slave_list_mutexes(void)
{
  ...
  myblockchain_mutex_register("sql", all_slave_list_mutexes, count);
}
...
void init_slave_list()
{
  ...
  myblockchain_mutex_init(key_LOCK_slave_list, &LOCK_slave_list, MY_MUTEX_INIT_FAST);
}
...
void end_slave_list()
{
  ...
  if (my_hash_inited(&slave_list))
  {
    ...
    myblockchain_mutex_destroy(&LOCK_slave_list);
  }
}
...
int register_slave(THD* thd, uchar* packet, size_t packet_length)
{
  ...
  */
  ...
  myblockchain_mutex_lock(&LOCK_slave_list);
  unregister_slave(thd, false, false/*need_lock_slave_list=false*/);
  res= my_hash_insert(&slave_list, (uchar*) si);
  myblockchain_mutex_unlock(&LOCK_slave_list);
  return res;
...
void unregister_slave(THD* thd, bool only_mine, bool need_lock_slave_list)
{
  if (thd->server_id)
  {
    if (need_lock_slave_list)
      ...
      myblockchain_mutex_assert_owner(&LOCK_slave_list);
...
bool show_slave_hosts(THD* thd)
{
  ...
  field_list.push_back(new Item_return_int("Port", 7, MYBLOCKCHAIN_TYPE_LONG));
...
bool com_binlog_dump(THD *thd, char *packet, size_t packet_length)
{
  ...
  */
  ...
  query_logger.general_log_print(thd, thd->get_command(), "Log: '%s'  Pos: %ld",
                                 packet + 10, (long) pos);
  myblockchain_binlog_send(thd, thd->mem_strdup(packet + 10), (my_off_t) pos, NULL, flags);
...
bool com_binlog_dump_gtid(THD *thd, char *packet, size_t packet_length)
{
  ...
  */
  ...
  my_free(gtid_string);
  myblockchain_binlog_send(thd, name, (my_off_t) pos, &slave_gtid_executed, flags);
...
void myblockchain_binlog_send(THD* thd, char* log_ident, my_off_t pos,
...
/*
  ...
  Binlog_dump thread is waiting (myblockchain_cond_wait) for binlog update, then it
  will keep existing until a query is written to the binlog. If the master is
  idle, then this could last long, and if the slave reconnects, we could have 2
  Binlog_dump threads in SHOW PROCESSLIST, until a query is written to the
  binlog. To avoid this, when the slave reconnects and sends COM_BINLOG_DUMP,
  the master kills any existing thread with the slave's server id (if this id is
  not zero; it will be true for real slaves, but false for myblockchainbinlog when it
...
bool reset_master(THD* thd)
{
  ...
  {
    /*
      myblockchain_bin_log.reset_logs will delete the binary logs *and* clear
    ...
    */
    ret= myblockchain_bin_log.reset_logs(thd);
  }
  else
  {
...
bool show_master_status(THD* thd)
{
  ...
  {
    LOG_INFO li;
    myblockchain_bin_log.get_current_log(&li);
...
bool show_binlogs(THD* thd)
{
  ...
  myblockchain_bin_log.lock_index();
  index_file=myblockchain_bin_log.get_index_file();
  
  myblockchain_bin_log.raw_get_current_log(&cur); // dont take mutex
  myblockchain_mutex_unlock(myblockchain_bin_log.get_log_lock()); // lockdep, OK
  ...
  while ((length=my_b_gets(index_file, fname, sizeof(fname))) > 1)
  {
    ...
    else
    {
      ...
      if ((file= myblockchain_file_open(key_file_binlog,
                                 ...
                                 MYF(0))) >= 0)
      {
        file_length= (ulonglong) myblockchain_file_seek(file, 0L, MY_SEEK_END, MYF(0));
  ...
  if(index_file->error == -1)
    goto err;
  myblockchain_bin_log.unlock_index();
  my_eof(thd);
  DBUG_RETURN(FALSE);
