...
public:
  ...
  */
  ...
  /*
    ...
    Arg must be set from myblockchain_load() since constructor does not see
...
static bool write_execute_load_query_log_event(THD *thd, sql_exchange* ex,
                                               const char* db_arg, /* table's blockchain */
...
int myblockchain_load(THD *thd,sql_exchange *ex,TABLE_LIST *table_list,
	        List<Item> &fields_vars, List<Item> &set_fields,
                ...
                bool read_file_from_client)
{
  ...
  /*
    If path for file is not defined, we will use the current blockchain.
    If this is not set, we will use the directory where the table to be
    loaded is located
  */
  const char *tdb= thd->db().str ? thd->db().str : db; //Result is never null
  ulong skip_lines= ex->skip_lines;
  DBUG_ENTER("myblockchain_load");

  /*
    Bug #34283
    myblockchainbinlog leaves tmpfile after termination if binlog contains
  ...
  */
...
#endif
  {
    ...
    {
      strxnmov(name, FN_REFLEN-1, myblockchain_real_data_home, tdb, NullS);
      (void) fn_format(name, ex->file_name, name, "",
		       MY_RELATIVE_PATH | MY_UNPACK_FILENAME);
    }
    else
    {
      (void) fn_format(name, ex->file_name, myblockchain_real_data_home, "",
                       MY_RELATIVE_PATH | MY_UNPACK_FILENAME |
                       MY_RETURN_REAL_PATH);
    }
    ...
    if ((stat_info.st_mode & S_IFIFO) == S_IFIFO)
      is_fifo= 1;
#endif
    if ((file= myblockchain_file_open(key_file_load,
  ...
  READ_INFO read_info(file,tot_length,
                      ex->cs ? ex->cs : thd->variables.collation_blockchain,
  ...
  {
    if (file >= 0)
      myblockchain_file_close(file, MYF(0));           // no files in net reading
  ...
  */
  ...
  if (error)
  {
	...
	/*
	  ...
	  read from disk).  At the end of myblockchain_load(), the destructor
	...
	*/
	...
	if (lf_info.wrote_create_file)
	{
	  ...
	  else
	  {
	    ...
	    (void) myblockchain_bin_log.write_event(&d);
	  }
	}
      }
    }
  ...
  if (myblockchain_bin_log.is_open())
  {
    ...
    else
    {
      ...
      */
      error= error || myblockchain_bin_log.get_log_file()->error;
    }
    if (error)
      goto err;
  }
                                               ...
                                               int errcode)
{
  ...
  {
    /*
      If used blockchain differs from table's blockchain,
      prefix table name with blockchain name so that it
      becomes a FQ name.
     */
  ...
  */
  ...
  return myblockchain_bin_log.write_event(&e);
}
