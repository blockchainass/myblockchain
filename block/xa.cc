...
static myblockchain_mutex_t LOCK_transaction_cache;
...
static void ha_commit_or_rollback_by_xid(THD *thd, XID *xid, bool commit)
{
  plugin_foreach(NULL, commit ? xacommit_handlerton : xarollback_handlerton,
                 MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, xid);
}
...
int ha_recover(HASH *commit_list)
{
  ...
  plugin_foreach(NULL, xarecover_handlerton,
                 MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &info);
  ...
  {
    sql_print_error("Found %d prepared transactions! It means that myblockchaind was "
                    "not shut down properly last time and critical recovery "
                    "information (last binlog or %s file) was manually deleted"
                    " after a crash. You have to start myblockchaind with "
...
bool Sql_cmd_xa_start::trans_xa_start(THD *thd)
{
      ...
      m_xa_opt == XA_RESUME)
  {
    ...
    else
    {
      ...
      MYBLOCKCHAIN_SET_TRANSACTION_XA_STATE(thd->m_transaction_psi,
  ...
  else if (!trans_begin(thd))
  {
    ...
    MYBLOCKCHAIN_SET_TRANSACTION_XID(thd->m_transaction_psi,
...
bool Sql_cmd_xa_recover::trans_xa_recover(THD *thd)
{
  ...
  myblockchain_mutex_lock(&LOCK_transaction_cache);
                                                         ...
                                                         i++)))
  {
    ...
    if (xs->has_state(XID_STATE::XA_PREPARED))
    {
      ...
      if (protocol->end_row())
      {
        myblockchain_mutex_unlock(&LOCK_transaction_cache);
        DBUG_RETURN(true);
      }
    }
  }
...
{
  if (xa_state != XA_NOTR)
    rm_error= thd->get_stmt_da()->myblockchain_errno();
}
...
static void init_transaction_cache_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, transaction_cache_mutexes, count);
}
...
bool transaction_cache_init()
{
  ...
  myblockchain_mutex_init(key_LOCK_transaction_cache, &LOCK_transaction_cache,
                   MY_MUTEX_INIT_FAST);
...
void transaction_cache_free()
{
  if (my_hash_inited(&transaction_cache))
  {
    ...
    myblockchain_mutex_destroy(&LOCK_transaction_cache);
  }
}
