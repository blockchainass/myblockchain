...
Mutex_cond_array::~Mutex_cond_array()
{
  ...
  for (int i= 0; i <= max_index; i++)
  {
    ...
    if (mutex_cond)
    {
      myblockchain_mutex_destroy(&mutex_cond->mutex);
      myblockchain_cond_destroy(&mutex_cond->cond);
      my_free(mutex_cond);
    }
  }
  global_lock->unlock();
...
enum_return_status Mutex_cond_array::ensure_index(int n)
{
  ...
  if (n > max_index)
  {
    ...
    for (int i= max_index + 1; i <= n; i++)
    {
      ...
      if (mutex_cond == NULL)
        goto error;
      myblockchain_mutex_init(key_gtid_ensure_index_mutex, &mutex_cond->mutex, NULL);
      myblockchain_cond_init(key_gtid_ensure_index_cond, &mutex_cond->cond);
