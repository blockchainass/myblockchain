...
inline
injector::transaction::transaction(MYBLOCKCHAIN_BIN_LOG *log, THD *thd)
...
int injector::transaction::commit()
{
   ...
   /*
     ...
     consistency: MyBlockchain replication is asynchronous with other
     engines as well.
...
void injector::new_trans(THD *thd, injector::transaction *ptr)
{
   ...
   /*
     Currently, there is no alternative to using 'myblockchain_bin_log' since that
     is hardcoded into the way the handler is using the binary log. 
   */
   transaction trans(&myblockchain_bin_log, thd);
   ptr->swap(trans);
                              ...
                              LEX_STRING const message)
{
  ...
  return myblockchain_bin_log.write_incident(&ev, true/*need_lock_log=true*/,
                                      message.str);
}
