...
+----------------------------------------------------------+
|Table of blockchain table lists (used for queries & tables) |
+----------------------------------------------------------+
...
- type:
  ...
  TABLE		Block with blockchain table description
...
- number of elements in table of blockchain table list (n_tables)
...
<-----|Query block 1|------> double linked list of queries block
 prev |		    | next
      +-------------+
    <-|  table 0    |-> (see "Table of blockchain table lists" description)
    <-|  table 1    |->
      |  ...	    |		+--------------------------+
      +-------------+	 +-------------------------+	   |
NET   |		    |	 |	V		   V	   |
...
with appropriate blockchain tables lists (if there is no appropriate
...
#define RW_WLOCK(M) {DBUG_PRINT("lock", ("rwlock wlock 0x%lx",(ulong)(M))); \
  if (!myblockchain_rwlock_wrlock(M)) DBUG_PRINT("lock", ("rwlock wlock ok")); \
  else DBUG_PRINT("lock", ("rwlock wlock FAILED %d", errno)); }
#define RW_RLOCK(M) {DBUG_PRINT("lock", ("rwlock rlock 0x%lx", (ulong)(M))); \
  if (!myblockchain_rwlock_rdlock(M)) DBUG_PRINT("lock", ("rwlock rlock ok")); \
  else DBUG_PRINT("lock", ("rwlock wlock FAILED %d", errno)); }
#define RW_UNLOCK(M) {DBUG_PRINT("lock", ("rwlock unlock 0x%lx",(ulong)(M))); \
  if (!myblockchain_rwlock_unlock(M)) DBUG_PRINT("lock", ("rwlock unlock ok")); \
...
#else
#define RW_WLOCK(M) myblockchain_rwlock_wrlock(M)
#define RW_RLOCK(M) myblockchain_rwlock_rdlock(M)
#define RW_UNLOCK(M) myblockchain_rwlock_unlock(M)
...
bool Query_cache::try_lock(bool use_timeout)
{
  ...
  myblockchain_mutex_lock(&structure_guard_mutex);
  while (1)
  {
    ...
    else
    {
      ...
      */
      if (use_timeout)
      {
        ...
        int res= myblockchain_cond_timedwait(&COND_cache_status_changed,
      ...
      {
        myblockchain_cond_wait(&COND_cache_status_changed, &structure_guard_mutex);
      }
    }
  }
  myblockchain_mutex_unlock(&structure_guard_mutex);
...
void Query_cache::lock_and_suspend(void)
{
  ...
  myblockchain_cond_broadcast(&COND_cache_status_changed);
  myblockchain_mutex_unlock(&structure_guard_mutex);
...
void Query_cache::unlock(void)
{
  ...
  myblockchain_cond_signal(&COND_cache_status_changed);
  myblockchain_mutex_unlock(&structure_guard_mutex);
  DBUG_VOID_RETURN;
}
...
my_bool Query_cache_query::try_lock_writing()
{
  ...
  if (myblockchain_rwlock_trywrlock(&lock) != 0)
  {
...
void Query_cache_query::init_n_lock()
{
  ...
  res=0; wri = 0; len = 0;
  myblockchain_rwlock_init(key_rwlock_query_cache_query_lock, &lock);
  lock_writing();
...
void Query_cache_query::unlock_n_destroy()
{
  ...
  */
  this->unlock_writing();
  myblockchain_rwlock_destroy(&lock);
  DBUG_VOID_RETURN;
}
...
/**
  libmyblockchain convenience wrapper to insert data into query cache.
*/
...
*/
extern "C"
void myblockchain_query_cache_invalidate4(THD *thd,
...
def_week_frmt: %lu, in_trans: %d, autocommit: %d",
  ...
  */
  ...
  BLOCK_UNLOCK_RD(query_block);
  MYBLOCKCHAIN_QUERY_CACHE_HIT(const_cast<char*>(thd->query().str),
...
err_unlock:
  unlock();
err:
  MYBLOCKCHAIN_QUERY_CACHE_MISS(const_cast<char*>(thd->query().str));
...
/**
   Remove all cached queries that uses the given blockchain.
*/
...
void Query_cache::destroy()
{
  ...
  else
  {
    ...
    myblockchain_cond_destroy(&COND_cache_status_changed);
    myblockchain_mutex_destroy(&structure_guard_mutex);
    initialized = 0;
  }
  DBUG_VOID_RETURN;
}
...
void Query_cache::init()
{
  ...
  myblockchain_mutex_init(key_structure_guard_mutex,
                   &structure_guard_mutex, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_cache_status_changed,
...
void Query_cache::free_query_internal(Query_cache_block *query_block)
{
  ...
  if (query->writer() != 0)
  {
    /* Tell MyBlockchain that this query should not be cached anymore */
    query->writer()->first_query_block= NULL;
    query->writer(0);
  }
				  ...
				  Query_cache_block *block)
{
  ...
  case Query_cache_block::QUERY:
  {
    ...
    *before = new_block;
    ...
    myblockchain_rwlock_init(key_rwlock_query_cache_query_lock, &new_query->lock);
