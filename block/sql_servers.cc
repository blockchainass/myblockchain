...
static MEM_ROOT mem;
static myblockchain_rwlock_t THR_LOCK_servers;

/**
   This enum describes the structure of the myblockchain.servers table.
*/
...
static void init_servers_cache_psi_keys(void)
{
  ...
  myblockchain_rwlock_register(category, all_servers_cache_rwlocks, count);

  count= array_elements(all_servers_cache_memory);
  myblockchain_memory_register(category, all_servers_cache_memory, count);
}
...
/*
  ...
  table in the 'myblockchain' blockchain.
...
bool servers_init(bool dont_read_servers_table)
{
  ...
  /* init the mutex */
  if (myblockchain_rwlock_init(key_rwlock_THR_LOCK_servers, &THR_LOCK_servers))
    DBUG_RETURN(TRUE);
...
/*
  ...
  SYNOPSIS
    servers_load()
      thd     Current thread
      tables  List containing open "myblockchain.servers"
...
bool servers_reload(THD *thd)
{
  ...
  myblockchain_rwlock_wrlock(&THR_LOCK_servers);

  tables[0].init_one_table("myblockchain", 5, "servers", 7, "servers", TL_READ);
  ...
  myblockchain_rwlock_unlock(&THR_LOCK_servers);
  DBUG_RETURN(return_val);
}
...
/**
   ...
   a new server into the myblockchain.servers table.
...
/**
   ...
   an existing server in the myblockchain.servers table.
...
bool Sql_cmd_common_server::check_and_open_table(THD *thd)
{
  ...
  TABLE_LIST tables;
  tables.init_one_table("myblockchain", 5, "servers", 7, "servers", TL_WRITE);

  table= open_ltable(thd, &tables, TL_WRITE, MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT);
  return (table == NULL);
}
...
void servers_free(bool end)
{
  ...
  myblockchain_rwlock_destroy(&THR_LOCK_servers);
  free_root(&mem,MYF(0));
                                   ...
                                   FOREIGN_SERVER *buff)
{
  ...
  DBUG_PRINT("info", ("locking servers_cache"));
  myblockchain_rwlock_rdlock(&THR_LOCK_servers);
