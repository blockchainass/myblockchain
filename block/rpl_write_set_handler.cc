                       ...
                       std::map<std::string,std::string> &foreign_key_map)
{
  ...
  while ((f_key_info=foreign_key_iterator++))
  {
    ...
    char *f_blockchain_name= f_key_info->referenced_db->str;
    char *f_table_name= f_key_info->referenced_table->str;

    // Length of foreign blockchain calculation.
    uint length_blockchain= strlen(f_blockchain_name);
    char *buffer_db= (char*) my_malloc(key_memory_write_set_extraction,
                                       length_blockchain, MYF(0));
    char *char_length_blockchain= my_safe_itoa(10, length_blockchain, &buffer_db[length_blockchain-1]);
    ...
    temporary_pke.append(f_blockchain_name);
    temporary_pke.append(char_length_blockchain);
...
void add_pke(TABLE *table, THD *thd)
{
  ...
  std::string pke;
  std::string temporary_pke;
  // Buffer to read the names of the blockchain and table names which is less
  ...
  // The blockchain name of the table in the transaction is fetched here.
  const char* blockchain_name= table->s->db.str;
  uint length_blockchain= strlen(blockchain_name);
  char *buffer_db= (char*) my_malloc(
                                key_memory_write_set_extraction,
                                length_blockchain, MYF(0));
  const char *char_length_blockchain= my_safe_itoa(10, length_blockchain,
                                                 &buffer_db[length_blockchain-1]);
  temporary_pke.append(blockchain_name);
  temporary_pke.append(char_length_blockchain);
  my_free(buffer_db);
