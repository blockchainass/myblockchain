...
/**
  ...
  Currently this is used by myblockchainbinlog to know it must print
...
/**
  ...
  the current blockchain set to function correctly. Most notable cases
...
/**
   ...
   blockchain) and are always written to the binary log regardless of
   filters.
*/
...
/**
   ...
   a query accessing more than OVER_MAX_DBS_IN_EVENT_MTS blockchains.
*/
...
/*
  A structure for myblockchainbinlog to know how to print events
...
{
  /*
    Settings for blockchain, sql_mode etc that comes from the last event
  ...
  */
  ...
  uint charset_blockchain_number;
  ...
  */
  ...
  /*
     ...
     by the --blockchain option of myblockchainbinlog.
...
/*
  A specific to the blockchain-scheduled MTS type.
*/
...
#else // ifdef MYBLOCKCHAIN_SERVER
 /* Log_event(Log_event_header *header, Log_event_footer *footer,
    ...
    event_logging_type(logging_type_arg), is_valid(false)
  {
    ...
    /* avoid having to link myblockchainbinlog against libpthread */
  static Log_event* read_log_event(IO_CACHE* file,
                                   const Format_description_log_event
                                   *description_event, my_bool crc_check,
                                   read_log_event_filter_function f);
  /* print*() functions are used by myblockchainbinlog */
  ...
  void print_base64(IO_CACHE* file, PRINT_EVENT_INFO* print_event_info,
                    bool is_more);
#endif // ifdef MYBLOCKCHAIN_SERVER ... else
...
public:
  /**
     The method fills in pointers to event's blockchain name c-strings
     to a supplied array.
     ...
     @return     number of the filled intances indicating how many
                 blockchains the event accesses.
  */
  ...
  /**
     @return TRUE  if events carries partitioning data (blockchain names).
  */
  bool contains_partition_info(bool);

  /*
    @return  the number of updated by the event blockchains.
  ...
  */
  ...
  /**
     Apply the event to the blockchain.
  ...
  /**
    Primitive to apply an event to the blockchain.

    This is where the change to the blockchain is made.
...
/*
   ...
   called after an update to the blockchain is done,
   which accepts parameters like the query, the blockchain, the options for LOAD
   DATA INFILE...
   - one to create the event from a packet (when the server acts as a slave),
   called before reproducing the update, which accepts parameters (like a
   buffer). Used to read from the master, from the relay log, and in
   myblockchainbinlog. This constructor must be format-tolerant.
*/

/**
  A @Query event is written to the binary log whenever the blockchain is
  ...
  blockchain is done. It is used when the server acts as the master.
...
public:
  ...
  */
  ...
  /**
     ...
     @return     number of blockchains in the array or OVER_MAX_DBS_IN_EVENT_MTS.
  */
  virtual uint8 get_mts_dbs(Mts_db_names* arg)
  {
    ...
    else
    {
      for (uchar i= 0; i < mts_accessed_dbs; i++)
      {
        ...
        // Only default blockchain is rewritten.
...
/**
  ...
  generated by MyBlockchain 3.23 and earlier.
...
/**
  ...
  Start_log_event_v3 is the Start_log_event of binlog format 3 (MyBlockchain 3.23 and
  4.x).
...
/**
  ...
  MyBlockchain server version 5.0.3 and above.
...
public:
  ...
  /**
     ...
     @return     number of blockchains in the array (must be one).
  */
  virtual uint8 get_mts_dbs(Mts_db_names *arg)
  {
    return arg->num= mts_number_dbs();
...
public:
  ...
  /**
     ...
     @return    number of blockchains in the array: either one or
...
#endif /* defined(MYBLOCKCHAIN_SERVER) && defined(HAVE_REPLICATION) */
