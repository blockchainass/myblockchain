...
/**
  ...
  BY (SQL Standard) or from HAVING and GROUP BY (MyBlockchain
  extension).
*/
static inline bool
...
/**
   ...
   merge-able view never has GROUP BY. @see myblockchain_register_view().
*/
...
/**
  @details
  Created mostly for myblockchain_prepare_table(). Important
...
bool Item::get_date_from_string(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t flags)
{
  char buff[MAX_DATE_STRING_REP_LENGTH];
  String tmp(buff, sizeof(buff), &my_charset_bin), *res;
  if (!(res= val_str(&tmp)))
  {
    set_zero_time(ltime, MYBLOCKCHAIN_TIMESTAMP_DATETIME);
    return true;
  }
...
bool Item::get_date_from_real(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t flags)
{
...
bool Item::get_date_from_decimal(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t flags)
{
...
bool Item::get_date_from_int(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t flags)
{
...
bool Item::get_date_from_time(MYBLOCKCHAIN_TIME *ltime)
{
...
bool Item::get_date_from_numeric(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  switch (result_type())
  {
  case REAL_RESULT:
...
/**
  Get the value of the function as a MYBLOCKCHAIN_TIME structure.
  As a extra convenience the time structure is reset on error!
*/

bool Item::get_date_from_non_temporal(MYBLOCKCHAIN_TIME *ltime,
...
bool Item::get_time_from_string(MYBLOCKCHAIN_TIME *ltime)
{
  char buff[MAX_DATE_STRING_REP_LENGTH];
  String tmp(buff, sizeof(buff), &my_charset_bin), *res;
  if (!(res= val_str(&tmp)))
  {
    set_zero_time(ltime, MYBLOCKCHAIN_TIMESTAMP_TIME);
    return true;
  }
...
bool Item::get_time_from_real(MYBLOCKCHAIN_TIME *ltime)
{
  double value= val_real();
  if (null_value)
  {
...
bool Item::get_time_from_decimal(MYBLOCKCHAIN_TIME *ltime)
{
  my_decimal buf, *decimal= val_decimal(&buf);
  if (null_value)
  {
...
bool Item::get_time_from_int(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(!is_temporal());
  longlong value= val_int();
  if (null_value)
...
bool Item::get_time_from_date(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
...
bool Item::get_time_from_datetime(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  if (get_date(ltime, TIME_FUZZY_DATE))
    return true;
...
bool Item::get_time_from_numeric(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(!is_temporal());
  switch (result_type())
  {
...
bool Item::get_time_from_non_temporal(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(!is_temporal());
  switch (result_type())
  {
...
bool Item::is_blob_field() const
{
  ...
  return (type == MYBLOCKCHAIN_TYPE_BLOB || type == MYBLOCKCHAIN_TYPE_GEOMETRY ||
...
bool Item_sp_variable::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed);
  Item *it= this_item();
...
bool Item_sp_variable::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed);
  Item *it= this_item();
...
bool Item_name_const::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed);
...
bool Item_name_const::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed);
...
/**
  ...
  Ensures that field, table, and blockchain names will live as long as
   ...
   have_privileges(0), any_privileges(false)
{
  /*
    ...
    in myblockchain_multi_update_prepare() or due to wildcard expansion in stored
    procedures).
  */
  {
    if (db_name)
...
{
  DBUG_ASSERT(fixed);
  DBUG_ASSERT(field_type() == MYBLOCKCHAIN_TYPE_JSON);
...
bool Item_field::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
...
bool Item_field::get_date_result(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  if ((null_value=result_field->is_null()) ||
...
bool Item_field::get_time(MYBLOCKCHAIN_TIME *ltime)
{
...
Item_param::Item_param(const POS &pos, uint pos_in_query_arg) : super(pos),
  ...
  param_type(MYBLOCKCHAIN_TYPE_VARCHAR),
  ...
  m_out_param_info(NULL)
{
  ...
  /* 
    ...
    before myblockchain_stmt_execute(), so we assuming that it can be NULL until
    value is set.
  */
  maybe_null= 1;
...
/**
  Set parameter value from MYBLOCKCHAIN_TIME value.
...
*/
void Item_param::set_time(MYBLOCKCHAIN_TIME *tm, timestamp_type time_type,
...
{ 
  ...
  if (check_datetime_range(&value.time))
  {
    make_truncated_value_warning(ErrConvString(&value.time,
                                 ...
                                 time_type);
    set_zero_time(&value.time, MYBLOCKCHAIN_TIMESTAMP_ERROR);
  }
  ...
  */
  if (str_value.length() + length > current_thd->variables.max_allowed_packet)
  {
    my_message(ER_UNKNOWN_ERROR,
               ...
               "myblockchain_send_long_data() is longer than "
...
bool Item_param::get_time(MYBLOCKCHAIN_TIME *res)
{
  if (state == TIME_VALUE)
  {
    *res= value.time;
...
bool Item_param::get_date(MYBLOCKCHAIN_TIME *res, my_time_flags_t fuzzydate)
{
  if (state == TIME_VALUE)
  {
    *res= value.time;
...
/**
  ...
  OUT-parameters, such as stored routine name, blockchain, stored routine
...
Item_copy *Item_copy::create (Item *item)
{
  ...
  {
    case STRING_RESULT:
      if (item->field_type() == MYBLOCKCHAIN_TYPE_JSON)
...
bool Item_copy_string::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  return get_date_from_string(ltime, fuzzydate);
}

bool Item_copy_string::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  return get_time_from_string(ltime);
}
...
bool Item_copy_json::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  if (null_value)
    return true;
  ...
  if (!result && ltime->time_type == MYBLOCKCHAIN_TIMESTAMP_TIME)
  {
...
bool Item_copy_json::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  if (null_value)
    return true;
...
type_conversion_status Item_copy_json::save_in_field(Field *field, bool no_conversions)
{
  ...
  if (field->type() == MYBLOCKCHAIN_TYPE_JSON)
  {
...
bool Item_ref_null_helper::get_date(MYBLOCKCHAIN_TIME *ltime,
                                    my_time_flags_t fuzzydate)
{
...
/**
  ...
  blockchain) name as the one being searched for.
...
static Item** find_field_in_group_list(Item *find_item, ORDER *group_list)
{
  ...
  {
    /* Convert blockchain to lower case for comparison */
  ...
  for (ORDER *cur_group= group_list ; cur_group ; cur_group= cur_group->next)
  {
    if ((*(cur_group->item))->real_item()->type() == Item::FIELD_ITEM)
    {
      ...
      if (cur_field->table_name && table_name)
      {
        ...
        {
          /* If field_name is also qualified by a blockchain name. */
          if (strcmp(cur_field->db_name, db_name))
            /* Same field names, different blockchains. */
            return NULL;
          ++cur_match_degree;
        }
      }
...
{
  enum_field_types f_type= MYBLOCKCHAIN_TYPE_VAR_STRING;
  if (max_length >= 16777216)
    f_type= MYBLOCKCHAIN_TYPE_LONG_BLOB;
  else if (max_length >= 65536)
    f_type= MYBLOCKCHAIN_TYPE_MEDIUM_BLOB;
  return f_type;
}
...
enum_field_types Item::field_type() const
{
  ...
  case INT_RESULT:     return MYBLOCKCHAIN_TYPE_LONGLONG;
  case DECIMAL_RESULT: return MYBLOCKCHAIN_TYPE_NEWDECIMAL;
  case REAL_RESULT:    return MYBLOCKCHAIN_TYPE_DOUBLE;
  case ROW_RESULT:
  default:
    DBUG_ASSERT(0);
    return MYBLOCKCHAIN_TYPE_VARCHAR;
  }
}
...
Field *Item::make_string_field(TABLE *table)
{
  ...
  if (field_type() == MYBLOCKCHAIN_TYPE_JSON)
  ...
  /* Item_type_holder holds the exact type, do not change it */
      ...
      (type() != Item::TYPE_HOLDER || field_type() != MYBLOCKCHAIN_TYPE_STRING))
  ...
  else
  {
    ...
    */
    if (max_length == 0 && marker == 4 && maybe_null &&
        field_type() == MYBLOCKCHAIN_TYPE_VAR_STRING && type() != Item::TYPE_HOLDER)
      field= new Field_varstring(max_length, maybe_null, item_name.ptr(),
                                 table->s, collation.collation);
    else
  ...
  if (result_type() == STRING_RESULT)
  {
        ...
        typ == SUBSELECT_ITEM)
    {
      ...
      if (ft == MYBLOCKCHAIN_TYPE_JSON)
      {
        ...
        if (field->type() == MYBLOCKCHAIN_TYPE_NEWDECIMAL)
        {
          my_decimal decimal_value;
        ...
        if (field->type() == MYBLOCKCHAIN_TYPE_INT24 ||
            field->type() == MYBLOCKCHAIN_TYPE_TINY  ||
            field->type() == MYBLOCKCHAIN_TYPE_SHORT ||
            field->type() == MYBLOCKCHAIN_TYPE_LONG  ||
            field->type() == MYBLOCKCHAIN_TYPE_LONGLONG)
        {
          longlong nr=val_int();
          if (null_value)
            return set_field_to_null_with_conversions(field, no_conversions);
        ...
        if (field->type() == MYBLOCKCHAIN_TYPE_FLOAT ||
                 field->type() == MYBLOCKCHAIN_TYPE_DOUBLE)
        {
          double nr= val_real();
          if (null_value)
            return set_field_to_null_with_conversions(field, no_conversions);
    ...
    type_conversion_status error=
      field->store(result->ptr(),result->length(),
                   field->type() == MYBLOCKCHAIN_TYPE_JSON ? result->charset() : cs);
    str_value.set_quick(0, 0, cs);
...
Item *Item_time_with_ref::clone_item()
{
  ...
  /*
    ...
    parameter markers.
  */
  return new Item_temporal(MYBLOCKCHAIN_TYPE_TIME, ref->item_name,
...
Item *Item_datetime_with_ref::clone_item()
{
  ...
  /*
    ...
    parameter markers.
  */
  return new Item_temporal(MYBLOCKCHAIN_TYPE_DATETIME, ref->item_name,
...
public:
  ...
  enum_field_types field_type() const { return MYBLOCKCHAIN_TYPE_JSON; }
  ...
  */
  ...
  bool get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
  {
    return m_value.coerce_date(ltime, fuzzydate, item_name.ptr());
  }
  bool get_time(MYBLOCKCHAIN_TIME *ltime)
  {
    return m_value.coerce_time(ltime, item_name.ptr());
  }
  Item *clone_item()
...
bool Item::send(Protocol *protocol, String *buffer)
{
  ...
  case MYBLOCKCHAIN_TYPE_TIMESTAMP:
  {
    ...
    if (!null_value)
      result= (f_type == MYBLOCKCHAIN_TYPE_DATE) ? protocol->store_date(&tm) :
...
bool Item_ref::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
...
bool Item_direct_ref::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
...
Item_default_value::save_in_field(Field *field_arg, bool no_conversions)
{
  if (!arg)
  {
    if (field_arg->flags & NO_DEFAULT_VALUE_FLAG &&
        field_arg->real_type() != MYBLOCKCHAIN_TYPE_ENUM)
    {
      if (field_arg->reset())
      {
...
void resolve_const_item(THD *thd, Item **ref, Item *comp_item)
{
  ...
  case STRING_RESULT:
  {
    ...
    {
      enum_field_types type= item->field_type() == MYBLOCKCHAIN_TYPE_TIMESTAMP ?
                              MYBLOCKCHAIN_TYPE_DATETIME : item->field_type();
...
int stored_field_cmp_to_item(THD *thd, Field *field, Item *item)
{
  Item_result res_type=item_cmp_type(field->result_type(),
				     item->result_type());
  if (field->type() == MYBLOCKCHAIN_TYPE_TIME &&
      item->field_type() == MYBLOCKCHAIN_TYPE_TIME)
  {
  ...
  if (field->is_temporal_with_date() && item->is_temporal())
  {
    /*
      ...
      Only non-temporal data types go though get_myblockchain_time_from_str()
  ...
  if (res_type == STRING_RESULT)
  {
    ...
    */
    ...
    {
      enum_myblockchain_timestamp_type type=
        field_type_to_timestamp_type(field->type());
      const char *field_name= field->field_name;
      MYBLOCKCHAIN_TIME field_time, item_time;
      get_myblockchain_time_from_str(thd, field_result, type, field_name, &field_time);
      get_myblockchain_time_from_str(thd, item_result, type, field_name,  &item_time);
...
bool Item_cache_datetime::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  if ((value_cached || str_value_cached) && null_value)
    return true;

  if (str_value_cached) // TS-TODO: reuse MYBLOCKCHAIN_TIME_cache eventually.
    return get_date_from_string(ltime, fuzzydate);
...
bool Item_cache_datetime::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  if ((value_cached || str_value_cached) && null_value)
    return true;
...
longlong Item_cache_datetime::val_date_temporal()
{
  ...
  if ((!value_cached && !cache_value_int()) || null_value)
    return 0;
  if (cached_field_type == MYBLOCKCHAIN_TYPE_TIME)
  {
...
Item_cache_json::Item_cache_json()
  : Item_cache(MYBLOCKCHAIN_TYPE_JSON), m_value(new Json_wrapper())
{}
...
bool Item_cache_json::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  Json_wrapper wr;
...
bool Item_cache_json::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  Json_wrapper wr;
...
Item_cache_str::save_in_field(Field *field, bool no_conversions)
{
  ...
  const type_conversion_status res= Item_cache::save_in_field(field,
                                                              no_conversions);
  if (is_varbinary && field->type() == MYBLOCKCHAIN_TYPE_STRING && value != NULL &&
  ...
  :Item(thd, item), enum_set_typelib(0), fld_type(get_real_type(item))
{
  ...
  if (item->field_type() == MYBLOCKCHAIN_TYPE_GEOMETRY)
...
/**
  ...
  @return
    Item_result (type of internal MyBlockchain expression result)
*/
  ...
  case FIELD_ITEM:
  {
    ...
    */
    ...
    if (field->is_created_from_null_item)
      return MYBLOCKCHAIN_TYPE_NULL;
    /* work around about varchar type field detection */
    if (type == MYBLOCKCHAIN_TYPE_STRING && field->type() == MYBLOCKCHAIN_TYPE_VAR_STRING)
      return MYBLOCKCHAIN_TYPE_VAR_STRING;
    return type;
  }
  case SUM_FUNC_ITEM:
  {
    ...
    if (((Item_func *) item)->functype() == Item_func::GUSERVAR_FUNC)
    {
      ...
      */
      ...
      case STRING_RESULT:
        return MYBLOCKCHAIN_TYPE_VAR_STRING;
      case INT_RESULT:
        return MYBLOCKCHAIN_TYPE_LONGLONG;
      case REAL_RESULT:
        return MYBLOCKCHAIN_TYPE_DOUBLE;
      case DECIMAL_RESULT:
        return MYBLOCKCHAIN_TYPE_NEWDECIMAL;
      case ROW_RESULT:
      default:
        DBUG_ASSERT(0);
        return MYBLOCKCHAIN_TYPE_VAR_STRING;
...
bool Item_type_holder::join_types(THD *thd, Item *item)
{
  ...
  case STRING_RESULT:
  {
    ...
    */
    if (fld_type == MYBLOCKCHAIN_TYPE_GEOMETRY &&
        geometry_type != item->get_geometry_type())
    {
      geometry_type= Field::GEOM_GEOMETRY;
    }
    ...
    if (decimals != NOT_FIXED_DEC)
    {
      ...
      */
          ...
          item->decimals != decimals_orig)
      {
        ...
        if (fld_type == MYBLOCKCHAIN_TYPE_FLOAT && max_length > FLT_DIG + 2)
        {
          max_length= MAX_FLOAT_STR_LENGTH;
          decimals= NOT_FIXED_DEC;
        } 
        else if (fld_type == MYBLOCKCHAIN_TYPE_DOUBLE && max_length > DBL_DIG + 2)
        {
      ...
      max_length= (fld_type == MYBLOCKCHAIN_TYPE_FLOAT) ? FLT_DIG+6 : DBL_DIG+7;
    break;
  }
  default:
...
{
  if (fld_type == MYBLOCKCHAIN_TYPE_ENUM ||
      fld_type == MYBLOCKCHAIN_TYPE_SET)
  {
    ...
    */
    if (enum_set_typelib)
    {
      DBUG_ASSERT(get_real_type(item) == MYBLOCKCHAIN_TYPE_NULL);
    }
    else
    {
      ...
      DBUG_ASSERT((get_real_type(item) == MYBLOCKCHAIN_TYPE_ENUM ||
                   get_real_type(item) == MYBLOCKCHAIN_TYPE_SET) &&
                  field_enum->typelib);
