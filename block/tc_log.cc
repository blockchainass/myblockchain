...
int TC_LOG_MMAP::open(const char *opt_name)
{
  ...
  fn_format(logname,opt_name,myblockchain_data_home,"",MY_UNPACK_FILENAME);
  if ((fd= myblockchain_file_open(key_file_tclog, logname, O_RDWR, MYF(0))) < 0)
  {
    if (my_errno != ENOENT)
      goto err;
    if (using_heuristic_recover())
      return 1;
    if ((fd= myblockchain_file_create(key_file_tclog, logname, CREATE_MODE,
                               O_RDWR, MYF(MY_WME))) < 0)
      goto err;
    inited=1;
    file_length= opt_tc_log_size;
    if (myblockchain_file_chsize(fd, file_length, 0, MYF(MY_WME)))
      goto err;
  }
  else
  {
    ...
    file_length= myblockchain_file_seek(fd, 0L, MY_SEEK_END, MYF(MY_WME+MY_FAE));
  ...
  for (pg=pages, i=0; i < npages; i++, pg++)
  {
    ...
    pg->waiters=0;
    pg->state=PS_POOL;
    myblockchain_cond_init(key_PAGE_cond, &pg->cond);
  ...
  myblockchain_mutex_init(key_LOCK_tc, &LOCK_tc, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_active, &COND_active);
  myblockchain_cond_init(key_COND_pool, &COND_pool);
...
void TC_LOG_MMAP::overflow()
{
  ...
  */
  ...
  myblockchain_cond_wait(&COND_pool, &LOCK_tc);
...
/**
  Record that transaction XID is committed on the persistent storage.
   ...
   If tc_log == MYBLOCKCHAIN_BIN_LOG then tc_log writes transaction to binlog and
...
{
  myblockchain_mutex_lock(&LOCK_tc);

  while (true)
  {
    /* If active page is full - just wait... */
    while (unlikely(active && active->free == 0))
      myblockchain_cond_wait(&COND_active, &LOCK_tc);
  ...
  {                                          // somebody's syncing. let's wait
    ...
    {
      if (p->waiters == 0)
        myblockchain_cond_broadcast(&COND_pool);    // in case somebody's waiting
      myblockchain_mutex_unlock(&LOCK_tc);
  ...
  myblockchain_cond_broadcast(&COND_active);        // in case somebody's waiting
  myblockchain_mutex_unlock(&LOCK_tc);
  err= sync();
...
bool TC_LOG_MMAP::sync()
{
  ...
  *pool_last_ptr= syncing;
  ...
  myblockchain_cond_broadcast(&syncing->cond);

  /* Mark syncing slot as free and wake-up new syncer. */
  syncing= NULL;
  if (active)
    myblockchain_cond_signal(&active->cond);
...
void TC_LOG_MMAP::unlog(ulong cookie, my_xid xid)
{
  ...
  *x= 0;
  ...
  if (p->waiters == 0)                 // the page is in pool and ready to rock
    myblockchain_cond_broadcast(&COND_pool);  // ping ... for overflow()
  myblockchain_mutex_unlock(&LOCK_tc);
}
...
void TC_LOG_MMAP::close()
{
  ...
  switch (inited) {
  case 6:
    myblockchain_mutex_destroy(&LOCK_tc);
    myblockchain_cond_destroy(&COND_pool);
  case 5:
    data[0]='A'; // garble the first (signature) byte, in case myblockchain_file_delete fails
  case 4:
    for (i=0; i < npages; i++)
    {
      if (pages[i].ptr == 0)
        break;
      myblockchain_cond_destroy(&pages[i].cond);
    }
  case 3:
    my_free(pages);
  case 2:
  ...
  if (inited>=5) // cannot do in the switch because of Windows
    myblockchain_file_delete(key_file_tclog, logname, MYF(MY_WME));
  inited=0;
}
...
err1:
  sql_print_error("Crash recovery failed. Either correct the problem "
                  ...
                  "or delete tc log and start myblockchaind with "
...
bool TC_LOG::using_heuristic_recover()
{
  ...
  sql_print_information("Please restart myblockchaind without --tc-heuristic-recover");
  return true;
}
