...
thd_timer_create(void)
{
  ...
  myblockchain_mutex_init(key_thd_timer_mutex, &thd_timer->mutex, MY_MUTEX_INIT_FAST);
  thd_timer->destroy= 0;
  ...
  myblockchain_mutex_destroy(&thd_timer->mutex);
  my_free(thd_timer);
...
timer_callback(my_timer_t *timer)
{
  ...
  myblockchain_mutex_lock(&thd_timer->mutex);
  destroy= timer_notify(thd_timer);
  myblockchain_mutex_unlock(&thd_timer->mutex);
