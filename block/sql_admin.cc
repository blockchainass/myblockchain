			      ...
			      HA_CHECK_OPT *check_opt)
{
  ...
  const char **ext;
  MY_STAT stat_info;
  Open_table_context ot_ctx(thd, (MYBLOCKCHAIN_OPEN_IGNORE_FLUSH |
                                  MYBLOCKCHAIN_OPEN_HAS_MDL_LOCK |
  ...
  if (!(table= table_list->table))
  {
    ...
    /*
      ...
      on it that is left from myblockchain_admin_table()'s attempt to 
    ...
    */
    ...
    /*
      Attempt to do full-blown table open in myblockchain_admin_table() has failed.
  ...
  */
  ...
  if (!myblockchain_file_stat(key_file_misc, from, &stat_info, MYF(0)))
  ...
  {
    /*
      Table was successfully open in myblockchain_admin_table(). Now we need
  ...
  /*
    ...
    in both cases when table was successfully open in myblockchain_admin_table()
  ...
  */
  ...
  if (myblockchain_file_rename(key_file_misc, tmp, from, MYF(MY_WME)))
  {
    error= send_check_errmsg(thd, table_list, "repair",
...
*/
static bool myblockchain_admin_table(THD* thd, TABLE_LIST* tables,
                              HA_CHECK_OPT* check_opt,
                              ...
                              int (view_operator_func)(THD *, TABLE_LIST*))
{
  ...
  DBUG_ENTER("myblockchain_admin_table");
      ...
      if (!thd->locked_tables_mode && repair_table_use_frm)
      {
        ...
        */
        ...
        if (tmp_da.is_error())
        {
          ...
          thd->get_stmt_da()->set_error_status(tmp_da.myblockchain_errno(),
                                               tmp_da.message_text(),
        ...
        */
        ...
        if (alter_info->flags & Alter_info::ALTER_ADMIN_PARTITION)
        {
          ...
          if (set_part_state(alter_info,
                             ...
                             true))
          {
            char buff[FN_REFLEN + MYBLOCKCHAIN_ERRMSG_SIZE];
            size_t length;
    ...
    */
    if (!table->table)
    {
      ...
      if (thd->get_stmt_da()->is_error() &&
          table_not_corrupt_error(thd->get_stmt_da()->myblockchain_errno()))
        result_code= HA_ADMIN_FAILED;
      else
        ...
        !(check_opt->sql_flags & TT_USEFRM))
    {
           ...
           HA_ADMIN_NEEDS_ALTER))
      {
        ...
        */
        ...
        result_code= myblockchain_recreate_table(thd, table, false);
        reenable_binlog(thd);
        /*
          myblockchain_recreate_table() can push OK or ERROR.
    ...
    case HA_ADMIN_TRY_ALTER:
    {
      ...
      */
      ...
      result_code= (open_temporary_tables(thd, table) ||
                    myblockchain_recreate_table(thd, table, false));
      reenable_binlog(thd);
      /*
        myblockchain_recreate_table() can push OK or ERROR.
        Clear 'OK' status. If there is an error, keep it:
      ...
      */
      ...
      protocol->store(table_name, system_charset_info);
      protocol->store(operator_name, system_charset_info);
      if (result_code) // either myblockchain_recreate_table or analyze failed
      {
...
/*
  ...
  SYNOPSIS
    myblockchain_assign_to_keycache()
    thd		Thread object
...
bool myblockchain_assign_to_keycache(THD* thd, TABLE_LIST* tables,
			     LEX_STRING *key_cache_name)
{
  HA_CHECK_OPT check_opt;
  KEY_CACHE *key_cache;
  DBUG_ENTER("myblockchain_assign_to_keycache");
  ...
  DBUG_RETURN(myblockchain_admin_table(thd, tables, &check_opt,
...
bool myblockchain_preload_keys(THD* thd, TABLE_LIST* tables)
{
  DBUG_ENTER("myblockchain_preload_keys");
  /*
    We cannot allow concurrent inserts. The storage engine reads
    directly from the index file, bypassing the cache. It could read
    outdated information if parallel inserts into cache blocks happen.
  */
  DBUG_RETURN(myblockchain_admin_table(thd, tables, 0,
...
bool Sql_cmd_analyze_table::execute(THD *thd)
{
  ...
  res= myblockchain_admin_table(thd, first_table, &thd->lex->check_opt,
...
bool Sql_cmd_check_table::execute(THD *thd)
{
  ...
  res= myblockchain_admin_table(thd, first_table, &thd->lex->check_opt, "check",
...
bool Sql_cmd_optimize_table::execute(THD *thd)
{
  ...
  res= (specialflag & SPECIAL_NO_NEW_FUNC) ?
    myblockchain_recreate_table(thd, first_table, true) :
    myblockchain_admin_table(thd, first_table, &thd->lex->check_opt,
...
bool Sql_cmd_repair_table::execute(THD *thd)
{
  ...
  if (check_table_access(thd, SELECT_ACL | INSERT_ACL, first_table,
    ...
    goto error; /* purecov: inspected */
  thd->enable_slow_log= opt_log_slow_admin_statements;
  res= myblockchain_admin_table(thd, first_table, &thd->lex->check_opt, "repair",
                         TL_WRITE, 1,
