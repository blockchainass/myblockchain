...
/* Function with list blockchains, tables or fields */
...
#include "sp.h"                             // MYBLOCKCHAIN_PROC_FIELD_DB
...
#include "sql_view.h"                       // myblockchain_make_view
                            ...
                            void *arg)
{
  ...
  struct st_myblockchain_plugin *plug= plugin_decl(plugin);
...
int fill_plugins(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  if (plugin_foreach_with_mask(thd, show_plugins, MYBLOCKCHAIN_ANY_PLUGIN,
...
bool myblockchaind_show_privileges(THD *thd)
{
  List<Item> field_list;
  Protocol *protocol= thd->get_protocol();
  DBUG_ENTER("myblockchaind_show_privileges");
...
/*
  ...
  SYNOPSIS
    ...
    db                  blockchain name to set in TABLE_LIST structure
    path                path to blockchain
    wild                filter for found files
    dir                 read blockchains in path if TRUE, read .frm files in
                        blockchain otherwise
  ...
  for (i=0 ; i < dirp->number_off_files  ; i++)
  {
    ...
    {                                           /* Return blockchains */
      /*
        ...
        This covers '.' and '..' and other cases like e.g. '.myblockchaingui'.
        Note that since 5.1 blockchain directory names can't start with a
...
class Show_create_error_handler : public Internal_error_handler
{
  ...
  char m_view_access_denied_message[MYBLOCKCHAIN_ERRMSG_SIZE];
  ...
  */
  const char* get_view_access_denied_message()
  {
    if (!m_view_access_denied_message_ptr)
    {
      ...
      my_snprintf(m_view_access_denied_message, MYBLOCKCHAIN_ERRMSG_SIZE,
...
bool
myblockchaind_show_create(THD *thd, TABLE_LIST *table_list)
{
  Protocol *protocol= thd->get_protocol();
  char buff[2048];
  ...
  List<Item> field_list;
  bool error= TRUE;
  DBUG_ENTER("myblockchaind_show_create");
...

  {
    ...
    */
    ...
    bool open_error= open_tables(thd, &table_list, &counter,
                                 MYBLOCKCHAIN_OPEN_FORCE_SHARED_HIGH_PRIO_MDL);
...
bool myblockchaind_show_create_db(THD *thd, char *dbname,
                           HA_CREATE_INFO *create_info)
{
  ...
  Protocol *protocol=thd->get_protocol();
  DBUG_ENTER("myblockchain_show_create_db");
...
/****************************************************************************
  Return only fields for API myblockchain_list_fields
  Use "show table wildcard" in myblockchain instead of this
****************************************************************************/

void
myblockchaind_list_fields(THD *thd, TABLE_LIST *table_list, const char *wild)
{
  DBUG_ENTER("myblockchaind_list_fields");
                                 ...
                                 bool quoted)
{
  ...
  const bool has_default=
    (field_type != FIELD_TYPE_BLOB &&
     ...
     !((thd->variables.sql_mode & (MODE_MYBLOCKCHAIN323 | MODE_MYBLOCKCHAIN40))
       && has_now_default));
  ...
  if (has_default)
  {
    ...
    {                                             // Not null by default
      ...
      if (field_type == MYBLOCKCHAIN_TYPE_BIT)
      {
...
/*
  ...
  SYNOPSIS
    ...
    show_blockchain     If true, then print the blockchain before the table
                      name. The blockchain name is only printed in the event
                      that it is different from the current blockchain.
                      If false, then do not print the blockchain before
                      the table name.
...
int store_create_info(THD *thd, TABLE_LIST *table_list, String *packet,
                      HA_CREATE_INFO *create_info_arg, bool show_blockchain)
{
  List<Item> field_list;
  ...
  bool foreign_db_mode=  (thd->variables.sql_mode & (MODE_POSTGRESQL |
                                                     ...
                                                     MODE_MAXDB |
                                                     MODE_ANSI)) != 0;
  bool limited_myblockchain_mode= (thd->variables.sql_mode & (MODE_NO_FIELD_OPTIONS |
                                                       MODE_MYBLOCKCHAIN323 |
                                                       MODE_MYBLOCKCHAIN40)) != 0;
  ...
  /*
    Print the blockchain before the table name if told to do that. The
    blockchain name is only printed in the event that it is different
    from the current blockchain.  The main reason for doing this is to
  ...
  for (ptr=table->field ; (field= *ptr); ptr++)
  {
    ...
    */
    if (thd->variables.show_old_temporals &&
        (field_type == MYBLOCKCHAIN_TYPE_TIME || field_type == MYBLOCKCHAIN_TYPE_DATETIME ||
         field_type == MYBLOCKCHAIN_TYPE_TIMESTAMP))
    ...
    else if (field->type() == MYBLOCKCHAIN_TYPE_TIMESTAMP)
    {
      /*
  ...
  */
  ...
  if (!(thd->variables.sql_mode & MODE_NO_TABLE_OPTIONS) && !foreign_db_mode)
  {
    ...
    */
    ...
    {
      if (thd->variables.sql_mode & (MODE_MYBLOCKCHAIN323 | MODE_MYBLOCKCHAIN40))
    ...
    */
	...
	!(thd->variables.sql_mode & MODE_MYBLOCKCHAIN323) &&
	!(thd->variables.sql_mode & MODE_MYBLOCKCHAIN40))
    {
      /*
        IF   check_create_info
...
static void store_key_options(THD *thd, String *packet, TABLE *table,
                              KEY *key_info)
{
  bool limited_myblockchain_mode= (thd->variables.sql_mode &
                            (MODE_NO_FIELD_OPTIONS | MODE_MYBLOCKCHAIN323 |
                             MODE_MYBLOCKCHAIN40)) != 0;
  bool foreign_db_mode=  (thd->variables.sql_mode & (MODE_POSTGRESQL |
                                                     MODE_ORACLE |
                                                     MODE_MSSQL |
  ...
  if (!(thd->variables.sql_mode & MODE_NO_KEY_OPTIONS) &&
      !limited_myblockchain_mode && !foreign_db_mode)
  {
...
view_store_create_info(THD *thd, TABLE_LIST *table, String *buff)
{
  ...
  if (!thd->db().str || strcmp(thd->db().str, table->view_db.str))
    /*
      print compact view name if the view belongs to the current blockchain
    */
...
/**
  This class implements callback function used by myblockchaind_list_processes() to
...
public:
  ...
  virtual void operator()(THD *inspect_thd)
  {
    ...
    /* DB */
    myblockchain_mutex_lock(&inspect_thd->LOCK_thd_data);
    ...
    myblockchain_mutex_unlock(&inspect_thd->LOCK_thd_data);

    /* INFO */
    myblockchain_mutex_lock(&inspect_thd->LOCK_thd_query);
    ...
    myblockchain_mutex_unlock(&inspect_thd->LOCK_thd_query);
...
void myblockchaind_list_processes(THD *thd,const char *user, bool verbose)
{
  Item *field;
  List<Item> field_list;
  ...
  size_t max_query_length= (verbose ? thd->variables.max_allowed_packet :
                            PROCESS_LIST_WIDTH);
  Protocol *protocol= thd->get_protocol();
  DBUG_ENTER("myblockchaind_list_processes");
  ...
  field_list.push_back(field= new Item_return_int("Time",7, MYBLOCKCHAIN_TYPE_LONG));
...
class Show_var_cmp :
  public std::binary_function<const st_myblockchain_show_var &,
                              const st_myblockchain_show_var &, bool>
{
public:
  bool operator()(const st_myblockchain_show_var &var1,
                  const st_myblockchain_show_var &var2)
  {
...
static inline bool is_show_undef(const st_myblockchain_show_var &var)
{
  ...
  for (; list->name; list++)
  {
    ...
    if (res == 0)
    {
      ...
      for (; list->type == SHOW_FUNC; list= &tmp)
        ((myblockchain_show_var_func)(list->value))(thd, &tmp, value);
...
/*
  ...
  NOTE
    ...
    initialization in the myblockchaind startup.
*/
  ...
  for (; variables->name; variables++)
  {
    ...
    */
    for (var=variables; var->type == SHOW_FUNC; var= &tmp)
      ((myblockchain_show_var_func)(var->value))(thd, &tmp, buff);
                      ...
                      LOOKUP_FIELD_VALUES *lookup_field_vals)
{
      ...
      item_func->functype() == Item_func::EQUAL_FUNC)
  {
    ...
    /* Lookup value is blockchain name */
...
/**
  ...
  @details This function calculates lookup value(blockchain name, table name)
...
/**
  @brief   Calculate lookup values(blockchain name, table name)

  @details This function calculates lookup values(blockchain name, table name)
                 ...
                 bool *with_i_schema, MEM_ROOT *tmp_mem_root)
{
  ...
  *with_i_schema= 0;
  if (lookup_field_vals->wild_db_value)
  {
    ...
    */
    ...
    return (find_files(thd, files, NullS, myblockchain_data_home,
  ...
  /*
    ...
    We don't do this for blockchain names longer than the maximum
    name length.
  */
  if (lookup_field_vals->db_value.str)
  {
    ...
    {
      /*
        Impossible value for a blockchain name,
  ...
  /*
    Create list of existing blockchains. It is used in case
  ...
  *with_i_schema= 1;
  return (find_files(thd, files, NullS,
                     myblockchain_data_home, NullS, 1, tmp_mem_root) != FIND_FILES_OK);
}
...
int schema_tables_add(THD *thd, List<LEX_STRING> *files, const char *wild)
{
  ...
  if (plugin_foreach(thd, add_schema_table,
                     MYBLOCKCHAIN_INFORMATION_SCHEMA_PLUGIN, &add_data))
      DBUG_RETURN(1);
...
/**
  ...
  @param[in]      thd                   thread handler
  @param[in]      table_names           List of table names in blockchain
  @param[in]      lex                   pointer to LEX struct
  @param[in]      lookup_field_vals     pointer to LOOKUP_FIELD_VALUE struct
  @param[in]      with_i_schema         TRUE means that we add I_S tables to list
  @param[in]      db_name               blockchain name
                     ...
                     MEM_ROOT *tmp_mem_root)
{
  ...
  */
  ...
  {
    /*
      Downgrade errors about problems with blockchain directory to
      warnings if this is not a 'SHOW' command.  Another thread
      may have dropped blockchain, and we may still have a name
      for that directory.
    */
                          ...
                          bool can_deadlock)
{
  ...
  */
  ...
  /*
    Since make_table_list() might change blockchain and table name passed
  ...
  */
  ...
  if (!result)
    result= open_tables_for_query(thd, table_list,
                                  MYBLOCKCHAIN_OPEN_IGNORE_FLUSH |
                                  MYBLOCKCHAIN_OPEN_FORCE_SHARED_HIGH_PRIO_MDL |
                                  (can_deadlock ?
                                   MYBLOCKCHAIN_OPEN_FAIL_ON_MDL_CONFLICT : 0));
  ...
  */
  if (!is_show_fields_or_keys && result && thd->is_error() &&
      thd->get_stmt_da()->myblockchain_errno() == ER_NO_SUCH_TABLE)
  {
    /*
                                      ...
                                      bool can_deadlock)
{
  ...
  if (lower_case_table_names)
  {
    /*
      ...
      cache subsystems require normalized (lowercased) blockchain and table
      names as input.
    */
  ...
  */
  ...
  myblockchain_mutex_lock(&LOCK_open);
  ...
  if (share->is_view)
  {
    if (myblockchain_make_view(thd, share, &table_list, true))
      goto end_share;
...
end_unlock:
  myblockchain_mutex_unlock(&LOCK_open);
...
/**
  ...
  The potential SQL conditions are:
    ...
    - ER_TRG_NO_DEFINER -- this warning is thrown when we're loading a
      trigger created/imported in/from the version of MyBlockchain, which does not
...
int fill_schema_schemata(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  */
  ...
  /*
    If we have lookup db value we should check that the blockchain exists
  */
     ...
     !with_i_schema)
  {
    ...
    path_len= build_table_filename(path, sizeof(path) - 1,
                                   lookup_field_vals.db_value.str, "", "", 0);
    path[path_len-1]= 0;
    if (!myblockchain_file_stat(key_file_misc, path, &stat_info, MYF(0)))
      DBUG_RETURN(0);
  }
    ...
    */
    ...
    if (thd->is_error())
    {
      push_warning(thd, Sql_condition::SL_WARNING,
                   thd->get_stmt_da()->myblockchain_errno(),
                       ...
                       uint offset)
{
  ...
  */
  ...
  /*
    DATA_TYPE column:
    MyBlockchain column type has the following format:
  ...
  */
  ...
  is_blob= (field->type() == MYBLOCKCHAIN_TYPE_BLOB);
  if (field->has_charset() || is_blob ||
      field->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR ||  // For varbinary type
      field->real_type() == MYBLOCKCHAIN_TYPE_STRING)     // For binary type
  {
...
/**
  ...
  @param[in]      thd                   thread handler
  @param[in]      table                 I_S table
  @param[in]      proc_table            'myblockchain.proc' table
                         ...
                         const char *sp_user)
{
  ...
  get_field(thd->mem_root, proc_table->field[MYBLOCKCHAIN_PROC_FIELD_DB], &sp_db);
  get_field(thd->mem_root, proc_table->field[MYBLOCKCHAIN_PROC_FIELD_NAME], &sp_name);
  get_field(thd->mem_root,proc_table->field[MYBLOCKCHAIN_PROC_FIELD_DEFINER],&definer);
  routine_type= (enum_sp_type) proc_table->field[MYBLOCKCHAIN_PROC_MYBLOCKCHAIN_TYPE]->val_int();
  ...
  params.length(0);
  get_field(thd->mem_root, proc_table->field[MYBLOCKCHAIN_PROC_FIELD_PARAM_LIST],
            &params);
  returns.length(0);
  if (routine_type == SP_TYPE_FUNCTION)
    get_field(thd->mem_root, proc_table->field[MYBLOCKCHAIN_PROC_FIELD_RETURNS],
              &returns);

  sp= sp_load_for_information_schema(thd, proc_table, &sp_db, &sp_name,
                                     (sql_mode_t) proc_table->
                                     field[MYBLOCKCHAIN_PROC_FIELD_SQL_MODE]->val_int(),
                                     routine_type,
                                     returns.c_ptr_safe(),
                                     params.c_ptr_safe(),
                                     &free_sp_head);
  ...
  if (sp)
  {
    ...
    if (routine_type == SP_TYPE_FUNCTION)
    {
      ...
      get_field(thd->mem_root, proc_table->field[MYBLOCKCHAIN_PROC_MYBLOCKCHAIN_TYPE],
                &tmp_string);
                       ...
                       const char *wild, bool full_access, const char *sp_user)
{
  ...
  proc_table->field[MYBLOCKCHAIN_PROC_FIELD_DB]->val_str(&sp_db);
  proc_table->field[MYBLOCKCHAIN_PROC_FIELD_NAME]->val_str(&sp_name);
  proc_table->field[MYBLOCKCHAIN_PROC_FIELD_DEFINER]->val_str(&definer);

  enum_sp_type sp_type=
    (enum_sp_type) proc_table->field[MYBLOCKCHAIN_PROC_MYBLOCKCHAIN_TYPE]->val_int();
      ...
      (sql_command_flags[lex->sql_command] & CF_STATUS_COMMAND) == 0)
  {
    ...
    {
      int enum_idx= (int) proc_table->field[MYBLOCKCHAIN_PROC_FIELD_ACCESS]->val_int();
      table->field[IS_ROUTINES_ROUTINE_NAME]->store(sp_name.ptr(),
                                                    sp_name.length(), cs);

      copy_field_as_string(table->field[IS_ROUTINES_SPECIFIC_NAME],
                           proc_table->field[MYBLOCKCHAIN_PROC_FIELD_SPECIFIC_NAME]);
      ...
      copy_field_as_string(table->field[IS_ROUTINES_ROUTINE_TYPE],
                           proc_table->field[MYBLOCKCHAIN_PROC_MYBLOCKCHAIN_TYPE]);

      if (sp_type == SP_TYPE_FUNCTION)
      {
        sp_head *sp;
        bool free_sp_head;
        proc_table->field[MYBLOCKCHAIN_PROC_FIELD_RETURNS]->val_str(&returns);
        sp= sp_load_for_information_schema(thd, proc_table, &sp_db, &sp_name,
                                           (sql_mode_t) proc_table->
                                           field[MYBLOCKCHAIN_PROC_FIELD_SQL_MODE]->
                                           val_int(),
                                           SP_TYPE_FUNCTION,
                                           returns.c_ptr_safe(),
                                           "", &free_sp_head);
      ...
      if (full_access)
      {
        copy_field_as_string(table->field[IS_ROUTINES_ROUTINE_DEFINITION],
                             proc_table->field[MYBLOCKCHAIN_PROC_FIELD_BODY_UTF8]);
      ...
      copy_field_as_string(table->field[IS_ROUTINES_IS_DETERMINISTIC],
                           proc_table->field[MYBLOCKCHAIN_PROC_FIELD_DETERMINISTIC]);
...
int fill_schema_proc(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  proc_tables.db= (char*) "myblockchain";
...
#ifndef EMBEDDED_LIBRARY
/*
  Loads an event from myblockchain.event and copies it's data to a row of
  I_S.EVENTS
  ...
  Synopsis
    copy_event_to_schema_table()
      ...
      event_table The event table to use for loading (myblockchain.event).
...
copy_event_to_schema_table(THD *thd, TABLE *sch_table, TABLE *event_table)
{
  ...
  {
    my_error(ER_CANNOT_LOAD_FROM_TABLE_V2, MYF(0), "myblockchain", "event");
    DBUG_RETURN(1);
  }
...
int fill_status(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  */
  ...
  all_status_vars.push_back(st_myblockchain_show_var());
...
ST_SCHEMA_TABLE *find_schema_table(THD *thd, const char* table_name)
{
  ...
  if (plugin_foreach(thd, find_schema_table_in_plugin, 
                     MYBLOCKCHAIN_INFORMATION_SCHEMA_PLUGIN, &schema_table_a))
...
/**
  ...
  @note
    For MYBLOCKCHAIN_TYPE_DECIMAL fields only, the field_length member has encoded
...
TABLE *create_schema_table(THD *thd, TABLE_LIST *table_list)
{
  ...
  for (; fields_info->field_name; fields_info++)
  {
    ...
    case MYBLOCKCHAIN_TYPE_DATETIME:
    {
      ...
      if (fields_info->field_type == MYBLOCKCHAIN_TYPE_TIMESTAMP ||
          fields_info->field_type == MYBLOCKCHAIN_TYPE_DATETIME)
    ...
    default:
      ...
      DBUG_ASSERT(fields_info->field_type == MYBLOCKCHAIN_TYPE_STRING);
...
int myblockchain_schema_table(THD *thd, LEX *lex, TABLE_LIST *table_list)
{
  TABLE *table;
  DBUG_ENTER("myblockchain_schema_table");
		       ...
		       enum enum_schema_tables schema_table_idx)
{
  ...
  DBUG_PRINT("enter", ("myblockchain_schema_select: %s", schema_table->table_name));
  /*
                          ...
                          QEP_TAB *qep_tab)
{
  ...
  /*
    ...
    not other blockchain directories). Filling schema tables is done before
  ...
  */
  ...
  if (tmp_da.is_error())
  {
    da->set_error_status(tmp_da.myblockchain_errno(),
                         tmp_da.message_text(),
                         tmp_da.returned_sqlstate());
    da->push_warning(thd,
                     tmp_da.myblockchain_errno(),
                     tmp_da.returned_sqlstate(),
...
int hton_fill_schema_table(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  plugin_foreach(thd, run_hton_fill_schema_table,
                 MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &args);
...
{
  {"CATALOG_NAME", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"SCHEMA_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Database",
   SKIP_OPEN_TABLE},
  {"DEFAULT_CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"DEFAULT_COLLATION_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"SQL_PATH", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};


ST_FIELD_INFO tables_fields_info[]=
{
  {"TABLE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Name",
   SKIP_OPEN_TABLE},
  {"TABLE_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"ENGINE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Engine", OPEN_FRM_ONLY},
  {"VERSION", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Version", OPEN_FRM_ONLY},
  {"ROW_FORMAT", 10, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Row_format", OPEN_FULL_TABLE},
  {"TABLE_ROWS", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Rows", OPEN_FULL_TABLE},
  {"AVG_ROW_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Avg_row_length", OPEN_FULL_TABLE},
  {"DATA_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Data_length", OPEN_FULL_TABLE},
  {"MAX_DATA_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Max_data_length", OPEN_FULL_TABLE},
  {"INDEX_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Index_length", OPEN_FULL_TABLE},
  {"DATA_FREE", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Data_free", OPEN_FULL_TABLE},
  {"AUTO_INCREMENT", MY_INT64_NUM_DECIMAL_DIGITS , MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Auto_increment", OPEN_FULL_TABLE},
  {"CREATE_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Create_time", OPEN_FULL_TABLE},
  {"UPDATE_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Update_time", OPEN_FULL_TABLE},
  {"CHECK_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Check_time", OPEN_FULL_TABLE},
  {"TABLE_COLLATION", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Collation",
   OPEN_FRM_ONLY},
  {"CHECKSUM", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Checksum", OPEN_FULL_TABLE},
  {"CREATE_OPTIONS", 255, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Create_options",
   OPEN_FRM_ONLY},
  {"TABLE_COMMENT", TABLE_COMMENT_MAXLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 
...
{
  {"TABLE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"COLUMN_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Field",
   OPEN_FRM_ONLY},
  {"ORDINAL_POSITION", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   MY_I_S_UNSIGNED, 0, OPEN_FRM_ONLY},
  {"COLUMN_DEFAULT", MAX_FIELD_VARCHARLENGTH, MYBLOCKCHAIN_TYPE_STRING, 0,
   1, "Default", OPEN_FRM_ONLY},
  {"IS_NULLABLE", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Null", OPEN_FRM_ONLY},
  {"DATA_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"CHARACTER_MAXIMUM_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FRM_ONLY},
  {"CHARACTER_OCTET_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS , MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FRM_ONLY},
  {"NUMERIC_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FRM_ONLY},
  {"NUMERIC_SCALE", MY_INT64_NUM_DECIMAL_DIGITS , MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FRM_ONLY},
  {"DATETIME_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS , MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
  {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FRM_ONLY},
  {"COLLATION_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Collation",
   OPEN_FRM_ONLY},
  {"COLUMN_TYPE", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Type", OPEN_FRM_ONLY},
  {"COLUMN_KEY", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Key", OPEN_FRM_ONLY},
  {"EXTRA", 30, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Extra", OPEN_FRM_ONLY},
  {"PRIVILEGES", 80, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Privileges", OPEN_FRM_ONLY},
  {"COLUMN_COMMENT", COLUMN_COMMENT_MAXLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 
   "Comment", OPEN_FRM_ONLY},
  {"GENERATION_EXPRESSION", GENERATED_COLUMN_EXPRESSION_MAXLEN, MYBLOCKCHAIN_TYPE_STRING,
...
{
  {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Charset",
   SKIP_OPEN_TABLE},
  {"DEFAULT_COLLATE_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0,
   "Default collation", SKIP_OPEN_TABLE},
  {"DESCRIPTION", 60, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Description",
   SKIP_OPEN_TABLE},
  {"MAXLEN", 3, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "Maxlen", SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"COLLATION_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Collation",
   SKIP_OPEN_TABLE},
  {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Charset",
   SKIP_OPEN_TABLE},
  {"ID", MY_INT32_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "Id",
   SKIP_OPEN_TABLE},
  {"IS_DEFAULT", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Default", SKIP_OPEN_TABLE},
  {"IS_COMPILED", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Compiled", SKIP_OPEN_TABLE},
  {"SORTLEN", 3, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "Sortlen", SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"ENGINE", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Engine", SKIP_OPEN_TABLE},
  {"SUPPORT", 8, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Support", SKIP_OPEN_TABLE},
  {"COMMENT", 80, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Comment", SKIP_OPEN_TABLE},
  {"TRANSACTIONS", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Transactions", SKIP_OPEN_TABLE},
  {"XA", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "XA", SKIP_OPEN_TABLE},
  {"SAVEPOINTS", 3 ,MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Savepoints", SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"EVENT_CATALOG", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"EVENT_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Db",
   SKIP_OPEN_TABLE},
  {"EVENT_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Name",
   SKIP_OPEN_TABLE},
  {"DEFINER", 77, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Definer", SKIP_OPEN_TABLE},
  {"TIME_ZONE", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Time zone", SKIP_OPEN_TABLE},
  {"EVENT_BODY", 8, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"EVENT_DEFINITION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"EVENT_TYPE", 9, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Type", SKIP_OPEN_TABLE},
  {"EXECUTE_AT", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Execute at", SKIP_OPEN_TABLE},
  {"INTERVAL_VALUE", 256, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Interval value",
   SKIP_OPEN_TABLE},
  {"INTERVAL_FIELD", 18, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Interval field",
   SKIP_OPEN_TABLE},
  {"SQL_MODE", 32*256, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"STARTS", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Starts", SKIP_OPEN_TABLE},
  {"ENDS", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Ends", SKIP_OPEN_TABLE},
  {"STATUS", 18, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Status", SKIP_OPEN_TABLE},
  {"ON_COMPLETION", 12, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"CREATED", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 0, 0, SKIP_OPEN_TABLE},
  {"LAST_ALTERED", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 0, 0, SKIP_OPEN_TABLE},
  {"LAST_EXECUTED", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, 0, SKIP_OPEN_TABLE},
  {"EVENT_COMMENT", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"ORIGINATOR", 10, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "Originator", SKIP_OPEN_TABLE},
  {"CHARACTER_SET_CLIENT", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0,
   "character_set_client", SKIP_OPEN_TABLE},
  {"COLLATION_CONNECTION", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0,
   "collation_connection", SKIP_OPEN_TABLE},
  {"DATABASE_COLLATION", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0,
...
{
  {"COLLATION_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"SPECIFIC_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"ROUTINE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"ROUTINE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Db",
   SKIP_OPEN_TABLE},
  {"ROUTINE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Name",
   SKIP_OPEN_TABLE},
  {"ROUTINE_TYPE", 9, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Type", SKIP_OPEN_TABLE},
  {"DATA_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"CHARACTER_MAXIMUM_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"CHARACTER_OCTET_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"NUMERIC_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
  {"NUMERIC_SCALE", 21 , MYBLOCKCHAIN_TYPE_LONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"DATETIME_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS , MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
  {"CHARACTER_SET_NAME", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"COLLATION_NAME", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"DTD_IDENTIFIER", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"ROUTINE_BODY", 8, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"ROUTINE_DEFINITION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"EXTERNAL_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"EXTERNAL_LANGUAGE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   SKIP_OPEN_TABLE},
  {"PARAMETER_STYLE", 8, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"IS_DETERMINISTIC", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"SQL_DATA_ACCESS", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"SQL_PATH", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"SECURITY_TYPE", 7, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Security_type",
   SKIP_OPEN_TABLE},
  {"CREATED", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 0, "Created", SKIP_OPEN_TABLE},
  {"LAST_ALTERED", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 0, "Modified", SKIP_OPEN_TABLE},
  {"SQL_MODE", 32*256, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"ROUTINE_COMMENT", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Comment",
   SKIP_OPEN_TABLE},
  {"DEFINER", 77, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Definer", SKIP_OPEN_TABLE},
  {"CHARACTER_SET_CLIENT", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0,
   "character_set_client", SKIP_OPEN_TABLE},
...
ST_FIELD_INFO stat_fields_info[]=
{
  {"TABLE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Table", OPEN_FRM_ONLY},
  {"NON_UNIQUE", 1, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "Non_unique", OPEN_FRM_ONLY},
  {"INDEX_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"INDEX_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Key_name",
   OPEN_FRM_ONLY},
  {"SEQ_IN_INDEX", 2, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "Seq_in_index", OPEN_FRM_ONLY},
  {"COLUMN_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Column_name",
   OPEN_FRM_ONLY},
  {"COLLATION", 1, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Collation", OPEN_FRM_ONLY},
  {"CARDINALITY", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1,
   "Cardinality", OPEN_FULL_TABLE},
  {"SUB_PART", 3, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, "Sub_part", OPEN_FRM_ONLY},
  {"PACKED", 10, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Packed", OPEN_FRM_ONLY},
  {"NULLABLE", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Null", OPEN_FRM_ONLY},
  {"INDEX_TYPE", 16, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Index_type", OPEN_FULL_TABLE},
  {"COMMENT", 16, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Comment", OPEN_FRM_ONLY},
  {"INDEX_COMMENT", INDEX_COMMENT_MAXLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 
...
ST_FIELD_INFO view_fields_info[]=
{
  ...
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"VIEW_DEFINITION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"CHECK_OPTION", 8, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"IS_UPDATABLE", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"DEFINER", 77, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"SECURITY_TYPE", 7, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"CHARACTER_SET_CLIENT", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FRM_ONLY},
  {"COLLATION_CONNECTION", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FRM_ONLY},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"GRANTEE", 81, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"PRIVILEGE_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"IS_GRANTABLE", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
ST_FIELD_INFO table_privileges_fields_info[]=
{
  ...
  {"TABLE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"PRIVILEGE_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"IS_GRANTABLE", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
ST_FIELD_INFO column_privileges_fields_info[]=
{
  ...
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"COLUMN_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"PRIVILEGE_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"IS_GRANTABLE", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"CONSTRAINT_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"CONSTRAINT_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {"CONSTRAINT_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"CONSTRAINT_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
ST_FIELD_INFO key_column_usage_fields_info[]=
{
  ...
  {"CONSTRAINT_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {"TABLE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"COLUMN_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"ORDINAL_POSITION", 10 ,MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, 0, OPEN_FULL_TABLE},
  {"POSITION_IN_UNIQUE_CONSTRAINT", 10 ,MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0,
   OPEN_FULL_TABLE},
  {"REFERENCED_TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FULL_TABLE},
  {"REFERENCED_TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FULL_TABLE},
  {"REFERENCED_COLUMN_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FULL_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
ST_FIELD_INFO table_names_fields_info[]=
{
  {"TABLE_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_SCHEMA",NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Tables_in_",
   SKIP_OPEN_TABLE},
  {"TABLE_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Table_type",
   OPEN_FRM_ONLY},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"Database", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Database",
   SKIP_OPEN_TABLE},
  {"Table",NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Table", SKIP_OPEN_TABLE},
  {"In_use", 1, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "In_use", SKIP_OPEN_TABLE},
  {"Name_locked", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, "Name_locked", SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"TRIGGER_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"TRIGGER_SCHEMA",NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"TRIGGER_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Trigger",
   OPEN_FRM_ONLY},
  {"EVENT_MANIPULATION", 6, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Event", OPEN_FRM_ONLY},
  {"EVENT_OBJECT_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FRM_ONLY},
  {"EVENT_OBJECT_SCHEMA",NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FRM_ONLY},
  {"EVENT_OBJECT_TABLE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Table",
   OPEN_FRM_ONLY},
  {"ACTION_ORDER", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, 0, OPEN_FRM_ONLY},
  {"ACTION_CONDITION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FRM_ONLY},
  {"ACTION_STATEMENT", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Statement",
   OPEN_FRM_ONLY},
  {"ACTION_ORIENTATION", 9, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"ACTION_TIMING", 6, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Timing", OPEN_FRM_ONLY},
  {"ACTION_REFERENCE_OLD_TABLE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FRM_ONLY},
  {"ACTION_REFERENCE_NEW_TABLE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FRM_ONLY},
  {"ACTION_REFERENCE_OLD_ROW", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  {"ACTION_REFERENCE_NEW_ROW", 3, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
  /*
    Set field_length to the value of 2 for field type MYBLOCKCHAIN_TYPE_DATETIME.
    ...
    the value of MYBLOCKCHAIN_TYPE_DATETIME is stored in the Field.
  */
  {"CREATED", 2, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Created", OPEN_FRM_ONLY},
  {"SQL_MODE", 32*256, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "sql_mode", OPEN_FRM_ONLY},
  {"DEFINER", 77, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Definer", OPEN_FRM_ONLY},
  {"CHARACTER_SET_CLIENT", MY_CS_NAME_SIZE, MYBLOCKCHAIN_TYPE_STRING, 0, 0,
...
ST_FIELD_INFO partitions_fields_info[]=
{
  ...
  {"TABLE_SCHEMA",NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"PARTITION_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"SUBPARTITION_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FULL_TABLE},
  {"PARTITION_ORDINAL_POSITION", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
  {"SUBPARTITION_ORDINAL_POSITION", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
  {"PARTITION_METHOD", 18, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"SUBPARTITION_METHOD", 12, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"PARTITION_EXPRESSION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"SUBPARTITION_EXPRESSION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FULL_TABLE},
  {"PARTITION_DESCRIPTION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"TABLE_ROWS", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0,
   OPEN_FULL_TABLE},
  {"AVG_ROW_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0,
   OPEN_FULL_TABLE},
  {"DATA_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0,
   OPEN_FULL_TABLE},
  {"MAX_DATA_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
  {"INDEX_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0,
   OPEN_FULL_TABLE},
  {"DATA_FREE", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0,
   OPEN_FULL_TABLE},
  {"CREATE_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, 0, OPEN_FULL_TABLE},
  {"UPDATE_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, 0, OPEN_FULL_TABLE},
  {"CHECK_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, 0, OPEN_FULL_TABLE},
  {"CHECKSUM", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
  {"PARTITION_COMMENT", 80, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"NODEGROUP", 12 , MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"TABLESPACE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   OPEN_FULL_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"VARIABLE_NAME", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Variable_name",
   SKIP_OPEN_TABLE},
  {"VARIABLE_VALUE", 1024, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Value", SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"ID", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, "Id", SKIP_OPEN_TABLE},
  {"USER", USERNAME_CHAR_LENGTH, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "User", SKIP_OPEN_TABLE},
  {"HOST", LIST_PROCESS_HOST_LEN,  MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Host",
   SKIP_OPEN_TABLE},
  {"DB", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Db", SKIP_OPEN_TABLE},
  {"COMMAND", 16, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Command", SKIP_OPEN_TABLE},
  {"TIME", 7, MYBLOCKCHAIN_TYPE_LONG, 0, 0, "Time", SKIP_OPEN_TABLE},
  {"STATE", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "State", SKIP_OPEN_TABLE},
  {"INFO", PROCESS_LIST_INFO_WIDTH, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Info",
   SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"PLUGIN_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Name",
   SKIP_OPEN_TABLE},
  {"PLUGIN_VERSION", 20, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"PLUGIN_STATUS", 10, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Status", SKIP_OPEN_TABLE},
  {"PLUGIN_TYPE", 80, MYBLOCKCHAIN_TYPE_STRING, 0, 0, "Type", SKIP_OPEN_TABLE},
  {"PLUGIN_TYPE_VERSION", 20, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"PLUGIN_LIBRARY", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Library",
   SKIP_OPEN_TABLE},
  {"PLUGIN_LIBRARY_VERSION", 20, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"PLUGIN_AUTHOR", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"PLUGIN_DESCRIPTION", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"PLUGIN_LICENSE", 80, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "License", SKIP_OPEN_TABLE},
  {"LOAD_OPTION", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};

ST_FIELD_INFO files_fields_info[]=
{
  {"FILE_ID", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, 0, SKIP_OPEN_TABLE},
  {"FILE_NAME", FN_REFLEN_SE, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"FILE_TYPE", 20, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLESPACE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   SKIP_OPEN_TABLE},
  {"TABLE_CATALOG", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLE_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"LOGFILE_GROUP_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0,
   SKIP_OPEN_TABLE},
  {"LOGFILE_GROUP_NUMBER", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"ENGINE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"FULLTEXT_KEYS", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {"DELETED_ROWS", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"UPDATE_COUNT", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"FREE_EXTENTS", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"TOTAL_EXTENTS", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"EXTENT_SIZE", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 0, 0, SKIP_OPEN_TABLE},
  {"INITIAL_SIZE", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
  {"MAXIMUM_SIZE", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
  {"AUTOEXTEND_SIZE", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
  {"CREATION_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, 0, SKIP_OPEN_TABLE},
  {"LAST_UPDATE_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, 0, SKIP_OPEN_TABLE},
  {"LAST_ACCESS_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, 0, SKIP_OPEN_TABLE},
  {"RECOVER_TIME", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"TRANSACTION_COUNTER", 4, MYBLOCKCHAIN_TYPE_LONGLONG, 0, 1, 0, SKIP_OPEN_TABLE},
  {"VERSION", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Version", SKIP_OPEN_TABLE},
  {"ROW_FORMAT", 10, MYBLOCKCHAIN_TYPE_STRING, 0, 1, "Row_format", SKIP_OPEN_TABLE},
  {"TABLE_ROWS", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Rows", SKIP_OPEN_TABLE},
  {"AVG_ROW_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Avg_row_length", SKIP_OPEN_TABLE},
  {"DATA_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
  ...
  {"INDEX_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Index_length", SKIP_OPEN_TABLE},
  {"DATA_FREE", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Data_free", SKIP_OPEN_TABLE},
  {"CREATE_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Create_time", SKIP_OPEN_TABLE},
  {"UPDATE_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Update_time", SKIP_OPEN_TABLE},
  {"CHECK_TIME", 0, MYBLOCKCHAIN_TYPE_DATETIME, 0, 1, "Check_time", SKIP_OPEN_TABLE},
  {"CHECKSUM", 21 , MYBLOCKCHAIN_TYPE_LONGLONG, 0, 
   (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Checksum", SKIP_OPEN_TABLE},
  {"STATUS", 20, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"EXTRA", 255, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
ST_FIELD_INFO referential_constraints_fields_info[]=
{
  ...
  {"CONSTRAINT_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {"UNIQUE_CONSTRAINT_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {"UNIQUE_CONSTRAINT_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {"UNIQUE_CONSTRAINT_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0,
   MY_I_S_MAYBE_NULL, 0, OPEN_FULL_TABLE},
  {"MATCH_OPTION", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"UPDATE_RULE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"DELETE_RULE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"REFERENCED_TABLE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
{
  {"SPECIFIC_CATALOG", FN_REFLEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"SPECIFIC_SCHEMA", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   OPEN_FULL_TABLE},
  {"SPECIFIC_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"ORDINAL_POSITION", 21 , MYBLOCKCHAIN_TYPE_LONG, 0, 0, 0, OPEN_FULL_TABLE},
  {"PARAMETER_MODE", 5, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"PARAMETER_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"DATA_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"CHARACTER_MAXIMUM_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONG, 0, 1, 0, OPEN_FULL_TABLE},
  {"CHARACTER_OCTET_LENGTH", 21 , MYBLOCKCHAIN_TYPE_LONG, 0, 1, 0, OPEN_FULL_TABLE},
  {"NUMERIC_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS, MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
  {"NUMERIC_SCALE", 21 , MYBLOCKCHAIN_TYPE_LONG, 0, 1, 0, OPEN_FULL_TABLE},
  {"DATETIME_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS , MYBLOCKCHAIN_TYPE_LONGLONG,
   0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
  {"CHARACTER_SET_NAME", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"COLLATION_NAME", 64, MYBLOCKCHAIN_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
  {"DTD_IDENTIFIER", 65535, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {"ROUTINE_TYPE", 9, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE}
};


ST_FIELD_INFO tablespaces_fields_info[]=
{
  {"TABLESPACE_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0,
   SKIP_OPEN_TABLE},
  {"ENGINE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
  {"TABLESPACE_TYPE", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, MY_I_S_MAYBE_NULL,
   0, SKIP_OPEN_TABLE},
  {"LOGFILE_GROUP_NAME", NAME_CHAR_LEN, MYBLOCKCHAIN_TYPE_STRING, 0, MY_I_S_MAYBE_NULL,
   0, SKIP_OPEN_TABLE},
  {"EXTENT_SIZE", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
  {"AUTOEXTEND_SIZE", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
  {"MAXIMUM_SIZE", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
  {"NODEGROUP_ID", 21, MYBLOCKCHAIN_TYPE_LONGLONG, 0,
   MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
  {"TABLESPACE_COMMENT", 2048, MYBLOCKCHAIN_TYPE_STRING, 0, MY_I_S_MAYBE_NULL, 0,
   SKIP_OPEN_TABLE},
  {0, 0, MYBLOCKCHAIN_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}
};
...
static bool show_create_trigger_impl(THD *thd, Trigger *trigger)
{
  ...
  fields.push_back(new Item_temporal(MYBLOCKCHAIN_TYPE_TIMESTAMP,
                                     Name_string("Created",
                                                 sizeof("created")-1),
                                     0, 0));
