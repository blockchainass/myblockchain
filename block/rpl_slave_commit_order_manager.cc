...
{
  myblockchain_mutex_init(key_commit_order_manager_mutex, &m_mutex, NULL);
  for (uint32 i= 0; i < worker_numbers; i++)
  {
    myblockchain_cond_init(key_commit_order_manager_cond, &m_workers[i].cond);
                                                  ...
                                                  bool all)
{
  ...
  */
      ...
      m_workers[worker->id].status == OCS_WAIT)
  {
    ...
    myblockchain_cond_t *cond= &m_workers[worker->id].cond;
    ...
    while (queue_front() != worker->id)
    {
      ...
      myblockchain_cond_wait(cond, &m_mutex);
    }
...
void Commit_order_manager::unregister_trx(Slave_worker *worker)
{
  ...
  if (m_workers[worker->id].status == OCS_SIGNAL)
  {
    ...
    queue_pop();
    if (!queue_empty())
      myblockchain_cond_signal(&m_workers[queue_front()].cond);
...
void Commit_order_manager::report_deadlock(Slave_worker *worker)
{
  ...
  myblockchain_cond_signal(&m_workers[worker->id].cond);
  myblockchain_mutex_unlock(&m_mutex);
  DBUG_VOID_RETURN;
}
