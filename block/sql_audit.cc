...
struct st_myblockchain_event_generic
{
  unsigned int event_class;
  const void *event;
};

unsigned long myblockchain_global_audit_mask[MYBLOCKCHAIN_AUDIT_CLASS_MASK_SIZE];

static myblockchain_mutex_t LOCK_audit_mask;
...
/**
  MYBLOCKCHAIN_AUDIT_GENERAL_CLASS handler
...
{
  myblockchain_event_general event;
  ...
  event.general_sql_command= va_arg(ap, MYBLOCKCHAIN_LEX_STRING);
  event.general_host= va_arg(ap, MYBLOCKCHAIN_LEX_STRING);
  event.general_external_user= va_arg(ap, MYBLOCKCHAIN_LEX_STRING);
  event.general_ip= va_arg(ap, MYBLOCKCHAIN_LEX_STRING);
  event_class_dispatch(thd, MYBLOCKCHAIN_AUDIT_GENERAL_CLASS, &event);
}


static void connection_class_handler(THD *thd, uint event_subclass, va_list ap)
{
  myblockchain_event_connection event;
  ...
  event.blockchain= va_arg(ap, const char *);
  event.blockchain_length= va_arg(ap, unsigned int);
  event.connection_type = va_arg(ap, int);
  event_class_dispatch(thd, MYBLOCKCHAIN_AUDIT_CONNECTION_CLASS, &event);
}

static void parse_class_handler(THD *thd, uint event_subclass, va_list ap)
{
  myblockchain_event_parse event;
  event.event_subclass= event_subclass;
  ...
  event_class_dispatch(thd, MYBLOCKCHAIN_AUDIT_PARSE_CLASS, &event);
}
...
static my_bool acquire_plugins(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  unsigned long event_class_mask[MYBLOCKCHAIN_AUDIT_CLASS_MASK_SIZE];
  st_myblockchain_audit *data= plugin_data<st_myblockchain_audit*>(plugin);
...
*/
void myblockchain_audit_acquire_plugins(THD *thd, uint event_class)
{
  unsigned long event_class_mask[MYBLOCKCHAIN_AUDIT_CLASS_MASK_SIZE];
  DBUG_ENTER("myblockchain_audit_acquire_plugins");
  set_audit_mask(event_class_mask, event_class);
  if (thd && !check_audit_mask(myblockchain_global_audit_mask, event_class_mask) &&
      check_audit_mask(&thd->audit_class_mask[0], event_class_mask))
  {
    plugin_foreach(thd, acquire_plugins, MYBLOCKCHAIN_AUDIT_PLUGIN, &event_class);
...
void myblockchain_audit_notify(THD *thd, uint event_class, uint event_subtype, ...)
{
  va_list ap;
  audit_handler_t *handlers= audit_handlers + event_class;
  DBUG_ASSERT(event_class < audit_handlers_count);
  myblockchain_audit_acquire_plugins(thd, event_class);
...
void myblockchain_audit_release(THD *thd)
{
  ...
  {
    st_myblockchain_audit *data= plugin_data<st_myblockchain_audit*>(*plugins);
  ...
  thd->audit_class_mask.resize(MYBLOCKCHAIN_AUDIT_CLASS_MASK_SIZE);
}
...
void myblockchain_audit_init_thd(THD *thd)
{
  thd->audit_class_mask.clear();
  thd->audit_class_mask.resize(MYBLOCKCHAIN_AUDIT_CLASS_MASK_SIZE);
}
...
void myblockchain_audit_free_thd(THD *thd)
{
  myblockchain_audit_release(thd);
...
static void init_audit_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, all_audit_mutexes, count);
}
...
void myblockchain_audit_initialize()
{
#ifdef HAVE_PSI_INTERFACE
  init_audit_psi_keys();
#endif

  myblockchain_mutex_init(key_LOCK_audit_mask, &LOCK_audit_mask, MY_MUTEX_INIT_FAST);
  memset(myblockchain_global_audit_mask, 0, sizeof(myblockchain_global_audit_mask));
}
...
void myblockchain_audit_finalize()
{
  myblockchain_mutex_destroy(&LOCK_audit_mask);
}
...
{
  st_myblockchain_audit *data= (st_myblockchain_audit*) plugin->plugin->info;
  ...
  myblockchain_mutex_lock(&LOCK_audit_mask);
  add_audit_mask(myblockchain_global_audit_mask, data->class_mask);
  myblockchain_mutex_unlock(&LOCK_audit_mask);
...
int finalize_audit_plugin(st_plugin_int *plugin)
{
  ...
  /*
    ...
    lock on myblockchain.plugin.
  */
  myblockchain_mutex_lock(&LOCK_audit_mask);
  plugin_foreach(current_thd, calc_class_mask, MYBLOCKCHAIN_AUDIT_PLUGIN,
                 &event_class_mask);

  /* Set the global audit mask */
  memmove(myblockchain_global_audit_mask, event_class_mask, sizeof(event_class_mask));
  myblockchain_mutex_unlock(&LOCK_audit_mask);
...
{
  const struct st_myblockchain_event_generic *event_generic=
    (const struct st_myblockchain_event_generic *) arg;
  unsigned long event_class_mask[MYBLOCKCHAIN_AUDIT_CLASS_MASK_SIZE];
  st_myblockchain_audit *data= plugin_data<st_myblockchain_audit*>(plugin);
...
static void event_class_dispatch(THD *thd, unsigned int event_class,
                                 const void *event)
{
  struct st_myblockchain_event_generic event_generic;
  ...
  */
  if (unlikely(!thd))
  {
    plugin_foreach(thd, plugins_dispatch, MYBLOCKCHAIN_AUDIT_PLUGIN, &event_generic);
  }
  else
  {
    plugin_ref *plugins, *plugins_last;
...
{
  return (myblockchain_global_audit_mask[0] & MYBLOCKCHAIN_AUDIT_GENERAL_CLASSMASK);
}
                                  ...
                                  unsigned int event_class)
{
  unsigned long event_class_mask[MYBLOCKCHAIN_AUDIT_CLASS_MASK_SIZE];
  set_audit_mask(event_class_mask, event_class);
  return !check_audit_mask(myblockchain_global_audit_mask, event_class_mask);
}
