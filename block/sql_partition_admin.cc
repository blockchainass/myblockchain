...
#include "sql_table.h"                      // myblockchain_alter_table, etc.
...
{
  /* Moved from myblockchain_execute_command */
  LEX *lex= thd->lex;
                                         ...
                                         partition_element *part_elem)
{
  ...
  if (myblockchain_prepare_alter_table(thd, part_table, &part_create_info,
  ...
  */
  if (myblockchain_compare_tables(table, &part_alter_info, &part_create_info,
                           &metadata_equal))
  {
    my_error(ER_TABLES_DIFFERENT_METADATA, MYF(0));
    DBUG_RETURN(TRUE);
  ...
  exchange_partition(THD *thd, TABLE_LIST *table_list, Alter_info *alter_info)
{
  ...
  uint table_counter;
  bool error= TRUE;
  DBUG_ENTER("myblockchain_exchange_partition");
  DBUG_ASSERT(alter_info->flags & Alter_info::ALTER_EXCHANGE_PARTITION);
