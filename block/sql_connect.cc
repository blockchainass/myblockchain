...
#include "probes_myblockchain.h"               // MYBLOCKCHAIN_CONNECTION_START
#include "auth_common.h"                // SUPER_ACL
#include "hostname.h"                   // Host_errors
#include "log.h"                        // sql_print_information
#include "myblockchaind.h"                     // LOCK_user_conn
#include "sql_audit.h"                  // MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_CONNECT
                            ...
                            const USER_RESOURCES *mqh)
{
  ...
  myblockchain_mutex_lock(&LOCK_user_conn);
  ...
  myblockchain_mutex_unlock(&LOCK_user_conn);
  return return_val;
...
/*
  Check for maximum allowable user connections, if the myblockchaind server is
...
bool thd_prepare_connection(THD *thd)
{
  ...
  MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_CONNECT(thd);
  if (rc)
    return rc;

  MYBLOCKCHAIN_CONNECTION_START(thd->thread_id(),
...
void close_connection(THD *thd, uint sql_errno)
{
  ...
  MYBLOCKCHAIN_CONNECTION_DONE((int) sql_errno, thd->thread_id());

  if (MYBLOCKCHAIN_CONNECTION_DONE_ENABLED())
  {
    sleep(0); /* Workaround to avoid tailcall optimisation */
  }
  MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_DISCONNECT(thd, sql_errno);
  DBUG_VOID_RETURN;
}
