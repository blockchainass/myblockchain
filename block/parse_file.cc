...
#include <my_sys.h>
#include <my_dir.h>
#include "myblockchaind.h"                           // reg_ext
...
/* from sql_db.cc */
extern long myblockchain_rm_arc_files(THD *thd, MY_DIR *dirp, const char *org_path);
			   ...
			   uchar* base, File_option *parameters)
{
  ...
  path[path_end]='~';
  path[path_end+1]= '\0';
  if ((handler= myblockchain_file_create(key_file_fileparser,
  ...
  if (opt_sync_frm) {
    if (myblockchain_file_sync(handler, MYF(MY_WME)))
      goto err_w_file;
  }

  if (myblockchain_file_close(handler, MYF(MY_WME)))
  {
    DBUG_RETURN(TRUE);
  }
...

  {
    ...
    path[path_end]='~';
    if (myblockchain_file_rename(key_file_fileparser, path, path_to, MYF(MY_WME)))
    {
      DBUG_RETURN(TRUE);
    }
  }
...
err_w_cache:
  end_io_cache(&file);
err_w_file:
  myblockchain_file_close(handler, MYF(MY_WME));
  DBUG_RETURN(TRUE);
}
                              ...
                              const char *new_db, const char *new_name)
{
  ...
  if (myblockchain_file_rename(key_file_frm, old_path, new_path, MYF(MY_WME)))
    return 1;
  ...
  { // remove obsolete 'arc' directory and files if any
    ...
    if ((new_dirp = my_dir(arc_path, MYF(MY_DONT_SORT))))
    {
      ...
      (void) myblockchain_rm_arc_files(thd, new_dirp, arc_path);
    }
  }
  return 0;
}
		  ...
		  bool bad_format_errors)
{
  ...
  if (!myblockchain_file_stat(key_file_fileparser,
  ...
  if ((file= myblockchain_file_open(key_file_fileparser, file_name->str,
                             O_RDONLY | O_SHARE, MYF(MY_WME))) < 0)
  {
    DBUG_RETURN(0);
  }
  
  if ((len= myblockchain_file_read(file, (uchar *)buff,
