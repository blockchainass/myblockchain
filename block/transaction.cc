...
bool trans_begin(THD *thd, uint flags)
{
  ...
  */
  ...
  DBUG_ASSERT(!((flags & MYBLOCKCHAIN_START_TRANS_OPT_READ_ONLY) &&
                (flags & MYBLOCKCHAIN_START_TRANS_OPT_READ_WRITE)));
  if (flags & MYBLOCKCHAIN_START_TRANS_OPT_READ_ONLY)
  {
    thd->tx_read_only= true;
    if (tst)
      tst->set_read_flags(thd, TX_READ_ONLY);
  }
  else if (flags & MYBLOCKCHAIN_START_TRANS_OPT_READ_WRITE)
  {
    /*
  ...
  if (flags & MYBLOCKCHAIN_START_TRANS_OPT_WITH_CONS_SNAPSHOT)
  {
    if (tst)
...
bool trans_rollback_to_savepoint(THD *thd, LEX_STRING name)
{
  ...
  */
  bool mdl_can_safely_rollback_to_savepoint=
                (!(myblockchain_bin_log.is_open() && thd->variables.sql_log_bin) ||
