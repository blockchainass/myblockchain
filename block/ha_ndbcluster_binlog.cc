...
#if MYBLOCKCHAIN_VERSION_ID > 50600
#include "rpl_slave.h"
#else
#include "slave.h"
#include "log_event.h"
...
/*
  ...
  myblockchain servers, and between myblockchain server and the binlog
*/
  ...
  for (unsigned int i= 0; i < table->s->fields; i++) 
  {
    ...
    DBUG_PRINT("info",
               ("[%d] \"%s\"(0x%lx:%s%s%s%s%s%s) type: %d  pack_length: %d  "
                ...
                (int) f->null_offset()));
    if (f->type() == MYBLOCKCHAIN_TYPE_BIT)
    {
      Field_bit *g= (Field_bit*) f;
      DBUG_PRINT("MYBLOCKCHAIN_TYPE_BIT",("field_length: %d  bit_ptr: 0x%lx[+%d] "
                                   "bit_ofs: %d  bit_len: %u",
                      ...
                      const int *no_print_error)
{
  ...
  */
  (void)myblockchaind.raw_run_query(buf, (end - buf),
                             no_print_error);
}
...
/*
  called in myblockchain_show_binlog_events and reset_logs to make sure we wait for
...
/*
 Called from MYBLOCKCHAIN_BIN_LOG::reset_logs in log.cc when binlog is emptied
*/
static int ndbcluster_reset_logs(THD *thd)
{
  ...
  /*
    Wait for all events originating from this myblockchain server has
    reached the binlog before continuing to reset
  */
  ndbcluster_binlog_wait(thd);

  /*
    Truncate myblockchain.ndb_binlog_index table, if table does not
    exist ignore the error as it is a "consistent" behavior
  */
  Ndb_local_connection myblockchaind(thd);
  const bool ignore_no_such_table = true;
  if(myblockchaind.truncate_table(STRING_WITH_LEN("myblockchain"),
...
/*
  Called from MYBLOCKCHAIN_BIN_LOG::purge_logs in log.cc when the binlog "file"
  is removed
*/
...
ndbcluster_binlog_index_purge_file(THD *passed_thd, const char *file)
{
  ...
  /*
    delete rows from myblockchain.ndb_binlog_index table for the given
    filename, if table does not exist ignore the error as it
    is a "consistent" behavior
  */
  Ndb_local_connection myblockchaind(my_thd);
  const bool ignore_no_such_table = true;
  if(myblockchaind.delete_rows(STRING_WITH_LEN("myblockchain"),
                        STRING_WITH_LEN("ndb_binlog_index"),
                        ignore_no_such_table,
                        "File='", file, "'", NULL))
  {
...
Ndb_dist_priv_util::priv_tables_are_in_ndb(THD* thd)
{
  ...
  if (ndb->setDatabaseName(dist_priv.blockchain()) != 0)
    DBUG_RETURN(false);
  ...
  while((table_name= dist_priv.iter_next_table()))
  {
    ...
    else if (distributed)
    {
      sql_print_error("NDB: Inconsistency detected in distributed "
                      ...
                      dist_priv.blockchain(), table_name);
      DBUG_RETURN(false);
    }
  }
...
/*
  ndbcluster_binlog_log_query
   ...
   - called by MyBlockchain Server in places where no other handlerton
     function exists which can be used to notify about changes
   - used by ndbcluster to detect when
     -- blockchains are created or altered
                            ...
                            const char *db, const char *table_name)
{
  ...
  case LOGCOM_CREATE_DB:
    DBUG_PRINT("info", ("New blockchain '%s' created", db));
    type= SOT_CREATE_DB;
    break;

  case LOGCOM_ALTER_DB:
    DBUG_PRINT("info", ("The blockchain '%s' was altered", db));
    type= SOT_ALTER_DB;
    break;
  ...
  case LOGCOM_ACL_NOTIFY:
    ...
    /*
      ...
      blockchain. Since ndbcluster_log_schema_op does not allow
      ...
      to the constant string "myblockchain" which should work in most cases.
...
static void ndbcluster_reset_slave(THD *thd)
{
  ...
  /*
    delete all rows from myblockchain.ndb_apply_status table
...
{
  return ndb_open_tables__is_table_open(STRING_WITH_LEN("myblockchain"),
                         ...
                         const char* create_options)
{
  ...
  /*
    Check if table exists in MyBlockchain "dictionary"(i.e on disk)
  ...
  */
  {
    ...
    if (my_delete(path, MYF(0)) == 0)
    {
      ...
      */
      ...
      /* Flush myblockchain.ndb_apply_status table, ignore all errors */
      (void)myblockchaind.flush_table(db, db_length,
                               table, table_length);
    }
  }

  const bool create_if_not_exists = true;
  const bool res = myblockchaind.create_sys_table(db, db_length,
                                           table, table_length,
...
void clean_away_stray_files(THD *thd)
{
  ...
  // Populate list of blockchains
  List<LEX_STRING> db_names;
  if (find_files(thd, &db_names, NULL,
                 myblockchain_data_home, NULL, 1, NULL) != FIND_FILES_OK)
  {
    thd->clear_error();
    DBUG_PRINT("info", ("Failed to find blockchains"));
    DBUG_VOID_RETURN;
  }
  ...
  while ((db_name= it++))
  {
    DBUG_PRINT("info", ("Found blockchain %s", db_name->str));
    if (strcmp(NDB_REP_DB, db_name->str)) /* Skip system blockchain */
    {

      sql_print_information("NDB: Cleaning stray tables from blockchain '%s'",
                            db_name->str);
...
/*
  Ndb has no representation of the blockchain schema objects.
  The myblockchain.ndb_schema table contains the latest schema operations
  done via a myblockchaind, and thus reflects blockchains created/dropped/altered
  while a myblockchaind was disconnected.  This function tries to recover
  the correct state w.r.t created blockchains using the information in
  that table.


*/
static int ndbcluster_find_all_blockchains(THD *thd)
{
  Ndb *ndb= check_ndb_in_thd(thd);
  Thd_ndb *thd_ndb= get_thd_ndb(thd);
  ...
  DBUG_ENTER("ndbcluster_find_all_blockchains");
  ...
  /*
    ...
    scanning.
  */
  if (!thd_ndb->has_required_global_schema_lock("ndbcluster_find_all_blockchains"))
    DBUG_RETURN(1);
  ...
  while (1)
  {
	...
	/* find query */
        ...
        int blockchain_exists= !my_access(name, F_OK);
        if (native_strncasecmp("CREATE", query, 6) == 0)
        {
          /* Database should exist */
          if (!blockchain_exists)
          {
            /* create missing blockchain */
            sql_print_information("NDB: Discovered missing blockchain '%s'", db);
        ...
        else if (native_strncasecmp("DROP", query, 4) == 0)
        {
          ...
          if (blockchain_exists)
          {
            /* drop missing blockchain */
            sql_print_information("NDB: Discovered remaining blockchain '%s'", db);
          }
        }
      }
    }
  ...
  error:
    ...
    if (ndb_error.status == NdbError::TemporaryError && !thd->killed)
    {
      if (retries--)
      {
        sql_print_warning("NDB: ndbcluster_find_all_blockchains retry: %u - %s",
                          ndb_error.code,
                          ndb_error.message);
    ...
    if (!thd->killed)
    {
      sql_print_error("NDB: ndbcluster_find_all_blockchains fail: %u - %s",
                      ndb_error.code,
                      ndb_error.message);
    }
    ...
    for (uint i= 0 ; i < list.count ; i++)
    {
          ...
          elmt.state != NDBOBJ::StateBuilding)
      {
        sql_print_information("NDB: skipping setup table %s.%s, in state %d",
                              elmt.blockchain, elmt.name, elmt.state);
        skipped++;
        continue;
      }

      ndb->setDatabaseName(elmt.blockchain);
      ...
      char *end= key +
        build_table_filename(key, sizeof(key) - 1, elmt.blockchain, "", "", 0);
      if (my_access(key, F_OK))
      {
        /* no such blockchain defined, skip table */
        continue;
      }
      ...
      if (discover)
      {
        /* ToDo 4.1 blockchain needs to be created if missing */
        if (ndb_create_table_from_engine(thd, elmt.blockchain, elmt.name))
        {
...
ndb_binlog_setup(THD *thd)
{
  ...
  */
  ...
  if (ndbcluster_find_all_blockchains(thd))
  {
    return false;
  }
                             ...
                             const char *new_db, const char *new_table_name)
{
  ...
  /* Check that the blockchain name will fit within limits */
  ...
  {
  case SOT_DROP_TABLE:
    /* drop blockchain command, do not log at drop table */
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      if (! thd->slave_thread)
      {
        /* Schema change originating from this MyBlockchainD, check SQL_LOG_BIN
         * variable and pass 'setting' to all logging MyBlockchainDs via AnyValue  
         */
...
err:
  ...
  if (ndb_error)
    push_warning_printf(thd, Sql_condition::SL_WARNING,
                        ...
                        ndb_error->code,
                        ndb_error->message,
                        "Could not log query '%s' on other myblockchaind's");
  ...
  /*
    Wait for other myblockchaind's to acknowledge the table operation
  */
...
public:
  ...
  void init(Ndb_cluster_connection* cluster_connection)
  {
    ...
    // the NDB binlog thread handles events on the myblockchain.ndb_schema table
    uint node_id, i= 0;
...
class Ndb_schema_event_handler {
  ...
  write_schema_op_to_binlog(THD *thd, Ndb_schema_op *schema)
  {
    ...
    if (!ndb_binlog_running)
    {
      // This myblockchaind is not writing a binlog
      return;
    }
    ...
    */
    ...
    if (queryServerId)
    {
      ...
      */
      ...
      {
        /* This MyBlockchainD does not log slave updates */
        return;
      }
    }
    else
  ...
  */
  ...
  void
  myblockchaind_close_cached_table(const char* db_name, const char* table_name) const
  {
    DBUG_ENTER("myblockchaind_close_cached_table");
  ...
  void
  myblockchaind_write_frm_from_ndb(const char* db_name,
                            const char* table_name) const
  {
    DBUG_ENTER("myblockchaind_write_frm_from_ndb");
    Thd_ndb *thd_ndb= get_thd_ndb(m_thd);
    Ndb *ndb= thd_ndb->ndb;
    Ndb_table_guard ndbtab_g(ndb->getDictionary(), table_name);
    const NDBTAB *ndbtab= ndbtab_g.get_table();
  ...
  check_if_local_tables_in_db(const char *dbname) const
  {
    ...
    /*
      ...
      to drop or create any blockchain. When determining if a blockchain
      should be dropped on participating myblockchaind it will thus need
      full permissions also when listing the tables in the blockchain.
      ...
      files in the blockchain(without checking any grants).

      Without full permission no tables would  be returned for blockchains
  ...
  void handle_clear_slock(Ndb_schema_op* schema)
  {
    ...
    {
      /* Noone waiting for this schema op in this myblockchaind */
      if (opt_ndb_extra_logging > 19)
  ...
  handle_offline_alter_table_commit(Ndb_schema_op* schema)
  {
    ...
    myblockchaind_close_cached_table(schema->db, schema->name);
  ...
  handle_online_alter_table_prepare(Ndb_schema_op* schema)
  {
    ...
    if (schema->node_id != own_nodeid())
    {
      ...
      if (!is_local_table(schema->db, schema->name))
      {
        myblockchaind_write_frm_from_ndb(schema->db, schema->name);
      }
    }
  }
  ...
  handle_drop_table(Ndb_schema_op* schema)
  {
    ...
    {
      /* Table is not a NDB table in this myblockchaind -> leave it */
  ...
  */
  ...
  handle_drop_db(Ndb_schema_op* schema)
  {
    ...
    {
      /* Tables exists as a local table, print error and leave it */
      sql_print_error("NDB Binlog: Skipping drop blockchain '%s' since "
  ...
  handle_schema_op(Ndb_schema_op* schema)
  {
    DBUG_ENTER("handle_schema_op");
    {
      ...
      if ((schema->db[0] == 0) && (schema->name[0] == 0))
      {
         ...
         *   that this myblockchaind does not know about.
      ...
      case SOT_RENAME_TABLE_NEW:
        /*
          Only very old MyBlockchain Server connected to the cluster may
                             ...
                             TABLE **ndb_binlog_index)
{
  ...
  const uint flags =
    MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT; /* Wait for lock "infinitely" */
...
int ndbcluster_binlog_start()
{
  ...
  if (::server_id == 0)
  {
    sql_print_warning("NDB: server id set to zero - changes logged to "
                      ...
                      "another server id by slave myblockchainds");
  }
                        ...
                        int push_warning)
{
  ...
  if (dict->createEvent(my_event)) // Add event to blockchain
  {
    if (dict->getNdbError().classification != NdbError::SchemaObjectExists)
    {
      ...
      */
      ...
      sql_print_error("NDB Binlog: Unable to create event in blockchain. "
                            ...
                            const NDBTAB *ndbtab, const char *event_name)
{
  ...
  */
  ...
  while (1)
  {
    ...
    {
      // set injector_ndb blockchain/schema from table internal name
                      ...
                      const char *tabname)
{
  ...
  /*
    ...
    differently for different myblockchaind's.  So we drop both
  */
  for (uint i= 0; i < 2; i++)
  {
    NDBDICT *dict= ndb->getDictionary();
        ...
        dict->getNdbError().code != 1419)
    {
      ...
      sql_print_error("NDB Binlog: Unable to drop event in blockchain. "
                      "Event: %s Error Code: %d Message: %s",
                      event_name.c_ptr(),
                      dict->getNdbError().code,
                      dict->getNdbError().message);
      ...
      if (share && share->op &&
          ...
          dict->getNdbError().myblockchain_code != HA_ERR_NO_CONNECTION)
      {
        DBUG_ASSERT(FALSE);
        DBUG_RETURN(-1);
      }
  ...
  */
      ...
      */
      ...
      if (!get_binlog_full(share) && table->s->primary_key != MAX_KEY)
        ...
        n= 1; /*
                ...
                since the myblockchain server does not handle the hidden primary
                key
              */
  ...
  case NDBEVENT::TE_UPDATE:
                        ...
                        table->s->db.str, table->s->table_name.str));
    {
      ...
      {
        /*
          myblockchain server cannot handle the ndb hidden key and
...
restart_cluster_failure:
  ...
  */
  ...
  /*
    wait for myblockchain server to start (so that the binlog is started
  ...
  */
  ...
  while (do_incident && ndb_binlog_running)
  {
    ...
    */
    ...
    {
      LOG_INFO log_info;
      myblockchain_bin_log.get_current_log(&log_info);
    ...
    */
    LEX_STRING const msg[2]=
      {
        { C_STRING_WITH_LEN("myblockchaind startup")    },
                        ...
                        (uint)(schema_gci)));
    {
      ...
      else if (ndb_latest_applied_binlog_epoch > 0)
      {
        sql_print_warning("NDB Binlog: cluster has reconnected. "
                          "Changes to the blockchain that occured while "
