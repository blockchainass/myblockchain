...
typedef struct st_db_worker_hash_entry
{
  ...
  Slave_worker *worker;
  /*
    The number of transaction pending on this blockchain.
    This should only be modified under the lock slave_worker_hash_lock.
   */
  long usage;
  /*
    The list of temp tables belonging to @ db blockchain is
...
public:
  ...
  */
  ...
  myblockchain_mutex_t jobs_lock; // mutex for the jobs queue
  myblockchain_cond_t  jobs_cond; // condition variable for the jobs queue
  ...
  */
  void set_rli_description_event(Format_description_log_event *fdle)
  {
    ...
    if (fdle)
    {
      myblockchain_mutex_assert_owner(&jobs_lock);
