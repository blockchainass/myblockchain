...
#include "probes_myblockchain.h"             // MYBLOCKCHAIN_HANDLER_WRLOCK_START
...
/**
  @def MYBLOCKCHAIN_TABLE_IO_WAIT
...
*/
  ...
  #define MYBLOCKCHAIN_TABLE_IO_WAIT(OP, INDEX, RESULT, PAYLOAD)     \
    {                                                         \
      if (m_psi != NULL)                                      \
      {                                                       \
...
/**
  @def MYBLOCKCHAIN_TABLE_LOCK_WAIT
  Instrumentation helper for table io_waits.
  @param OP the table operation to be performed
  @param FLAGS per table operation flags.
  @param PAYLOAD the code to instrument.
  @sa MYBLOCKCHAIN_END_TABLE_WAIT.
*/
#ifdef HAVE_PSI_TABLE_INTERFACE
  #define MYBLOCKCHAIN_TABLE_LOCK_WAIT(OP, FLAGS, PAYLOAD)    \
    {                                                  \
      if (m_psi != NULL)                               \
      {                                                \
...
/**
  Database name that hold most of myblockchaind system tables.
  Current code assumes that, there exists only some
  specific "blockchain name" designated as system blockchain.
*/
const char* myblockchaind_system_blockchain= "myblockchain";

// System tables that belong to myblockchaind_system_blockchain.
st_system_tablename myblockchaind_system_tables[]= {
  {myblockchaind_system_blockchain, "db"},
  {myblockchaind_system_blockchain, "user"},
  {myblockchaind_system_blockchain, "host"},
  {myblockchaind_system_blockchain, "func"},
  {myblockchaind_system_blockchain, "proc"},
  {myblockchaind_system_blockchain, "event"},
  {myblockchaind_system_blockchain, "plugin"},
  {myblockchaind_system_blockchain, "servers"},
  {myblockchaind_system_blockchain, "procs_priv"},
  {myblockchaind_system_blockchain, "tables_priv"},
  {myblockchaind_system_blockchain, "proxies_priv"},
  {myblockchaind_system_blockchain, "columns_priv"},
  {myblockchaind_system_blockchain, "time_zone"},
  {myblockchaind_system_blockchain, "time_zone_name"},
  {myblockchaind_system_blockchain, "time_zone_leap_second"},
  {myblockchaind_system_blockchain, "time_zone_transition"},
  {myblockchaind_system_blockchain, "time_zone_transition_type"},
  {myblockchaind_system_blockchain, "help_category"},
  {myblockchaind_system_blockchain, "help_keyword"},
  {myblockchaind_system_blockchain, "help_relation"},
  {myblockchaind_system_blockchain, "help_topic"},
  {(const char *)NULL, (const char *)NULL} /* This must be at the end */
};

/**
  This static pointer holds list of system blockchains from SQL layer and
  various SE's. The required memory is allocated once, and never freed.
*/
static const char **known_system_blockchains= NULL;
static const char **ha_known_system_blockchains();

// Called for each SE to get SE specific system blockchain.
static my_bool system_blockchains_handlerton(THD *unused, plugin_ref plugin,
                                           void *arg);
...
{
  return plugin_lock_by_name(thd, name, MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN);
}
...
/**
  Use other blockchain handler if blockchainhandler is not compiled in.
*/
handlerton *ha_checktype(THD *thd, enum legacy_db_type blockchain_type,
                          bool no_substitute, bool report_error)
{
  handlerton *hton= ha_resolve_by_legacy_type(thd, blockchain_type);
...
int ha_init()
{
  ...
  */
  ...
  /*
    Initialize system blockchain name cache.
    This cache is used to do a quick check if a given
    db.tablename is a system table.
  */
  known_system_blockchains= ha_known_system_blockchains();
...
{
  handlerton *hton= plugin_data<handlerton*>(plugin);
  if (hton->state == SHOW_OPTION_YES && hton->drop_blockchain)
    hton->drop_blockchain(hton, (char *)path);
  return FALSE;
}


void ha_drop_blockchain(char* path)
{
  plugin_foreach(NULL, dropdb_handlerton, MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, path);
}
...
{
  plugin_foreach(thd, closecon_handlerton, MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, 0);
}
...
{
  plugin_foreach(thd, kill_handlerton, MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, 0);
}
...
/**
  ...
  In each client connection, MyBlockchain maintains two transactional
  states:
  ...
  from the times when MyBlockchain supported BerkeleyDB storage engine.

  First of all, it should be said that in BerkeleyDB auto-commit
  mode auto-commits operations that are atomic to the storage
  engine itself, such as a write of a record, and are too
  high-granular to be atomic from the application perspective
  (MyBlockchain). One SQL statement could involve many BerkeleyDB
  auto-committed operations and thus BerkeleyDB auto-commit was of
  little use to MyBlockchain.
  ...
  Since MyBlockchain supports PSEA (pluggable storage engine
  ...
  engine transaction.
  For example, imagine that some day MyBlockchain supports unique
  constraint checks deferred till the end of statement. In such
  case a commit in one of the engines may yield ER_DUP_KEY,
  and MyBlockchain should be able to gracefully abort statement
  ...
  MyBlockchain is prefixed with an implicit normal transaction commit
...
*/
      ...
      ht_arg->db_type != DB_TYPE_BINLOG)
  {
    ...
    thd->m_transaction_psi= MYBLOCKCHAIN_START_TRANSACTION(&thd->m_transaction_state,
...
*/
  ...
  {
    MYBLOCKCHAIN_COMMIT_TRANSACTION(thd->m_transaction_psi);
  ...
  if (run_after_commit && thd->get_transaction()->m_flags.run_hooks)
  {
    /*
       ...
       TODO: Investigate if this can be refactored so that there is
             ...
             MYBLOCKCHAIN_LOG_BIN::finish_commit).
    */
    if (!error)
...
int ha_rollback_trans(THD *thd, bool all)
{
  ...
  */
  ...
  {
    MYBLOCKCHAIN_ROLLBACK_TRANSACTION(thd->m_transaction_psi);
    thd->m_transaction_psi= NULL;
  }
#endif
...
/**
  @details
  This function should be called when MyBlockchain sends rows of a SELECT result set
  or the EOF mark to the client. It releases a possible adaptive hash index
  S-latch held by thd in InnoDB and also releases a possible InnoDB query
  FIFO ticket to enter InnoDB. To save CPU time, InnoDB allows a thd to
  keep them over several calls of the InnoDB handler interface when a join
  is executed. But when we let the control to pass to the client they have
  to be released because if the application program uses myblockchain_use_result(),
  it may deadlock on the S-latch if the application on another connection
  performs another SQL query. In MyBlockchain-4.1 this is even more important because
...
#ifdef HAVE_PSI_TRANSACTION_INTERFACE
  if (thd->m_transaction_psi != NULL)
    MYBLOCKCHAIN_INC_TRANSACTION_ROLLBACK_TO_SAVEPOINT(thd->m_transaction_psi, 1);
#endif
...
int ha_savepoint(THD *thd, SAVEPOINT *sv)
{
  ...
  */
  ...
  if (!error && thd->m_transaction_psi != NULL)
    MYBLOCKCHAIN_INC_TRANSACTION_SAVEPOINTS(thd->m_transaction_psi, 1);
#endif
...
#ifdef HAVE_PSI_TRANSACTION_INTERFACE
  if (thd->m_transaction_psi != NULL)
    MYBLOCKCHAIN_INC_TRANSACTION_RELEASE_SAVEPOINT(thd->m_transaction_psi, 1);
#endif
  DBUG_RETURN(error);
}
...
int ha_start_consistent_snapshot(THD *thd)
{
  ...
  plugin_foreach(thd, snapshot_handlerton, MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &warn);
  ...
  */
  if (warn)
    push_warning(thd, Sql_condition::SL_WARNING, ER_UNKNOWN_ERROR,
                 "This MyBlockchain server does not support any "
...
/**
  ...
  @details Lower case db name and table name path parts for
           ...
           Filesystem path prefix (myblockchain_data_home or tmpdir)
           is left intact.
                                   ...
                                   char *tmp_path)
{
  ...
  for (i= 0; i <= myblockchain_tmpdir_list.max; i++)
  {
    if (is_prefix(path, myblockchain_tmpdir_list.list[i]))
      return path;
  }
  ...
  /*
    we only should turn into lowercase blockchain/table part
    so start the process after homedirectory
  */
  my_casedn_str(files_charset_info, tmp_path + myblockchain_data_home_len);
  return tmp_path;
}
...
int handler::ha_rnd_next(uchar *buf)
{
  ...
  MYBLOCKCHAIN_TABLE_IO_WAIT(PSI_TABLE_FETCH_ROW, MAX_KEY, result,
    { result= rnd_next(buf); })
                               ...
                               enum ha_rkey_function find_flag)
{
  ...
  MYBLOCKCHAIN_TABLE_IO_WAIT(PSI_TABLE_FETCH_ROW, active_index, result,
                                   ...
                                   enum ha_rkey_function find_flag)
{
  ...
  MYBLOCKCHAIN_TABLE_IO_WAIT(PSI_TABLE_FETCH_ROW, index, result,
...
int handler::update_auto_increment()
{
  ...
  */
  ...
  if (append)
  {
    ...
    if (myblockchain_bin_log.is_open() && !thd->is_current_stmt_binlog_format_row())
...
/** @brief
  MyBlockchain signal that it changed the column bitmap
  ...
  USAGE
    ...
    rnd_init() call is made as after this, MyBlockchain will not use the bitmap
                                 ...
                                 ulonglong *nb_reserved_values)
{
  ...
  {						// Autoincrement at key-start
    ...
    /*
      MyBlockchain implicitely assumes such method does locking (as MyBlockchain decides to
...
void print_keydup_error(TABLE *table, KEY *key, const char *msg, myf errflag)
{
  ...
  else
  {
    ...
    size_t max_length= MYBLOCKCHAIN_ERRMSG_SIZE - strlen(msg);
...
{
  ulong myblockchain_version= table->s->myblockchain_version;

  if (myblockchain_version < 50124)
  {
...
int handler::check_old_types()
{
  ...
  {
    if (table->s->myblockchain_version == 0) // prior to MyBlockchain 5.0
    {
      /* check for bad DECIMAL field */
      if ((*field)->type() == MYBLOCKCHAIN_TYPE_NEWDECIMAL) // TODO: error? MYBLOCKCHAIN_TYPE_DECIMAL?
      {
        return HA_ADMIN_NEEDS_ALTER;
      }
      if ((*field)->type() == MYBLOCKCHAIN_TYPE_VAR_STRING)
      {
        return HA_ADMIN_NEEDS_ALTER;
      }
    }
    if ((*field)->type() == MYBLOCKCHAIN_TYPE_YEAR && (*field)->field_length == 2)
    ...
    {
      if (((*field)->real_type() == MYBLOCKCHAIN_TYPE_TIME) ||
          ((*field)->real_type() == MYBLOCKCHAIN_TYPE_DATETIME) ||
          ((*field)->real_type() == MYBLOCKCHAIN_TYPE_TIMESTAMP))
        return HA_ADMIN_NEEDS_ALTER;
    }
  }
  return 0;
...
static bool update_frm_version(TABLE *table)
{
  ...
  /*
    ...
    temporary tables.
  */
  if (table->s->myblockchain_version == MYBLOCKCHAIN_VERSION_ID)
    DBUG_RETURN(0);

  strxmov(path, table->s->normalized_path.str, reg_ext, NullS);

  if ((file= myblockchain_file_open(key_file_frm,
                             path, O_RDWR|O_BINARY, MYF(MY_WME))) >= 0)
  {
    uchar version[4];

    int4store(version, MYBLOCKCHAIN_VERSION_ID);

    if ((result= myblockchain_file_pwrite(file, (uchar*) version, 4, 51L, MYF_RW)))
      goto err;

    table->s->myblockchain_version= MYBLOCKCHAIN_VERSION_ID;
  }
err:
  if (file >= 0)
    (void) myblockchain_file_close(file, MYF(MY_WME));
  DBUG_RETURN(result);
}
...
int handler::delete_table(const char *name)
{
  ...
  for (const char **ext=bas_ext(); *ext ; ext++)
  {
    ...
    if (myblockchain_file_delete_with_symlink(key_file_misc, buff, MYF(0)))
    {
      if (my_errno != ENOENT)
      {
        /*
...
/**
  Initiates table-file and calls appropriate blockchain-creator.
                                        ...
                                        const char *table_name)
{
  ...
  bool is_system_blockchain= false;
  const char **names;
  st_system_tablename *systab;

  // Check if we have a system blockchain name in the command.
  DBUG_ASSERT(known_system_blockchains != NULL);
  names= known_system_blockchains;
  while (names && *names)
  {
    if (strcmp(*names, db) == 0)
    {
      ...
      is_system_blockchain= true;
      break;
    }
    names++;
  }
  if (!is_system_blockchain)
    DBUG_RETURN(true); // It's a user table name.

  // Check if this is SQL layer system tables.
  systab= myblockchaind_system_tables;
  ...
  plugin_foreach(NULL, check_engine_system_table_handlerton,
                 MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &check_params);
...
/*
  Prepare list of all known system blockchain names
  current we just have 'myblockchain' as system blockchain name.

  Later ndbcluster, innodb SE's can define some new blockchain
  name which can store system tables specific to SE.
*/
const char** ha_known_system_blockchains(void)
{
  list<const char*> found_blockchains;
  const char **blockchains, **blockchain;

  // Get myblockchaind system blockchain name.
  found_blockchains.push_back((char*) myblockchaind_system_blockchain);

  // Get system blockchain names from every specific storage engine.
  plugin_foreach(NULL, system_blockchains_handlerton,
                 MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &found_blockchains);

  blockchains= (const char **) my_once_alloc(sizeof(char *)*
                                     (found_blockchains.size()+1),
                                     MYF(MY_WME | MY_FAE));
  DBUG_ASSERT(blockchains != NULL);

  list<const char*>::iterator it;
  blockchain= blockchains;
  for (it= found_blockchains.begin(); it != found_blockchains.end(); it++)
    *blockchain++= *it;
  *blockchain= 0; // Last element.

  return blockchains;
}

/**
  @brief Fetch system blockchain name specific to SE.

  @details This function is invoked by plugin_foreach() from
           ha_known_system_blockchains(), for each storage engine.
*/
static my_bool system_blockchains_handlerton(THD *unused, plugin_ref plugin,
                                           void *arg)
{
  list<const char*> *found_blockchains= (list<const char*> *) arg;
  const char *db;

  handlerton *hton= plugin_data<handlerton*>(plugin);
  if (hton->system_blockchain)
  {
    db= hton->system_blockchain();
    if (db)
      found_blockchains->push_back(db);
  }
...
static my_bool binlog_func_foreach(THD *thd, binlog_func_st *bfn)
{
  ...
  plugin_foreach(thd, binlog_func_list,
                 MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &hton_list);
                         ...
                         const char *db, const char *table_name)
{
  ...
  if (hton == 0)
    plugin_foreach(thd, binlog_log_query_handlerton,
                   MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &b);
  else
...
/**
  Returns a list of all known extensions.
    ...
    We have to recreate the extension map if myblockchaind is restarted (for example
    within libmyblockchaind)
...
TYPELIB* ha_known_exts()
{
  ...
  plugin_foreach(NULL, exts_handlerton,
                 MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &found_exts);
...
bool ha_show_status(THD *thd, handlerton *db_type, enum ha_stat_type stat)
{
  ...
  if (db_type == NULL)
  {
    result= plugin_foreach(thd, showstat_handlerton,
                           MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, &stat);
  }
  else
  {
    if (db_type->state != SHOW_OPTION_YES)
...
/*
  ...
  - The blockchain the table resides in shall be binlogged (binlog_*_db rules)
  - table is not myblockchain.event
*/
...
static int write_locked_table_maps(THD *thd)
{
  ...
  {
    MYBLOCKCHAIN_LOCK *locks[2];
    locks[0]= thd->extra_lock;
    locks[1]= thd->lock;
    for (uint i= 0 ; i < sizeof(locks)/sizeof(*locks) ; ++i )
    {
      MYBLOCKCHAIN_LOCK const *const lock= locks[i];
      if (lock == NULL)
        continue;
...
int handler::ha_external_lock(THD *thd, int lock_type)
{
  ...
  */
  ...
  if (MYBLOCKCHAIN_HANDLER_RDLOCK_START_ENABLED() && lock_type == F_RDLCK)
  {
    MYBLOCKCHAIN_HANDLER_RDLOCK_START(table_share->db.str,
                               table_share->table_name.str);
  }
  else if (MYBLOCKCHAIN_HANDLER_WRLOCK_START_ENABLED() && lock_type == F_WRLCK)
  {
    MYBLOCKCHAIN_HANDLER_WRLOCK_START(table_share->db.str,
                               table_share->table_name.str);
  }
  else if (MYBLOCKCHAIN_HANDLER_UNLOCK_START_ENABLED() && lock_type == F_UNLCK)
  {
    MYBLOCKCHAIN_HANDLER_UNLOCK_START(table_share->db.str,
                               table_share->table_name.str);
  }

  ha_statistic_increment(&SSV::ha_external_lock_count);

  MYBLOCKCHAIN_TABLE_LOCK_WAIT(PSI_TABLE_EXTERNAL_LOCK, lock_type,
  ...
  */
  ...
  if (MYBLOCKCHAIN_HANDLER_RDLOCK_DONE_ENABLED() && lock_type == F_RDLCK)
  {
    MYBLOCKCHAIN_HANDLER_RDLOCK_DONE(error);
  }
  else if (MYBLOCKCHAIN_HANDLER_WRLOCK_DONE_ENABLED() && lock_type == F_WRLCK)
  {
    MYBLOCKCHAIN_HANDLER_WRLOCK_DONE(error);
  }
  else if (MYBLOCKCHAIN_HANDLER_UNLOCK_DONE_ENABLED() && lock_type == F_UNLCK)
  {
    MYBLOCKCHAIN_HANDLER_UNLOCK_DONE(error);
  }
  DBUG_RETURN(error);
}
...
int handler::ha_write_row(uchar *buf)
{
  ...
  MYBLOCKCHAIN_INSERT_ROW_START(table_share->db.str, table_share->table_name.str);
  mark_trx_read_write();
  ...
  MYBLOCKCHAIN_TABLE_IO_WAIT(PSI_TABLE_WRITE_ROW, MAX_KEY, error,
    { error= write_row(buf); })

  MYBLOCKCHAIN_INSERT_ROW_DONE(error);
  if (unlikely(error))
    DBUG_RETURN(error);
...
int handler::ha_update_row(const uchar *old_data, uchar *new_data)
{
  ...
  MYBLOCKCHAIN_UPDATE_ROW_START(table_share->db.str, table_share->table_name.str);
  mark_trx_read_write();
  ...
  MYBLOCKCHAIN_TABLE_IO_WAIT(PSI_TABLE_UPDATE_ROW, active_index, error,
    { error= update_row(old_data, new_data);})

  MYBLOCKCHAIN_UPDATE_ROW_DONE(error);
  if (unlikely(error))
    DBUG_RETURN(error);
...
int handler::ha_delete_row(const uchar *buf)
{
  ...
  */
  ...
  MYBLOCKCHAIN_DELETE_ROW_START(table_share->db.str, table_share->table_name.str);
  mark_trx_read_write();

  MYBLOCKCHAIN_TABLE_IO_WAIT(PSI_TABLE_DELETE_ROW, active_index, error,
    { error= delete_row(buf);})

  MYBLOCKCHAIN_DELETE_ROW_DONE(error);
  if (unlikely(error))
    return error;
...
Handler_share *handler::get_ha_share_ptr()
{
  ...
  if (table_share->tmp_table == NO_TMP_TABLE)
    myblockchain_mutex_assert_owner(&table_share->LOCK_ha_data);
#endif
...
void handler::lock_shared_ha_data()
{
  ...
  if (table_share->tmp_table == NO_TMP_TABLE)
    myblockchain_mutex_lock(&table_share->LOCK_ha_data);
}
...
void handler::unlock_shared_ha_data()
{
  DBUG_ASSERT(table_share);
  if (table_share->tmp_table == NO_TMP_TABLE)
    myblockchain_mutex_unlock(&table_share->LOCK_ha_data);
}
                                           ...
                                           blob_len_ptr *blob_len_ptr_array)
{
  ...
  for (Field **vfield= table->vfield; *vfield; vfield++)
  {
    ...
    if ((*vfield)->is_virtual_gcol() && (*vfield)->type() == MYBLOCKCHAIN_TYPE_BLOB)
    {
...
/**
   ...
   @param db_name        Name of blockchain containing the table
...
/*
  ...
  @param thd        Thread handle
  @param db_name    name of blockchain
