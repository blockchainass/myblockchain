...
#include "auth_common.h" // SUPER_ACL, MYBLOCKCHAIN_DB_FIELD_COUNT, myblockchain_db_table_fields
...
static bool
myblockchain_event_fill_row(THD *thd,
                     TABLE *table,
                     Event_parse_data *et,
                     sp_head *sp,
                     sql_mode_t sql_mode,
                     my_bool is_update)
{
  ...
  DBUG_ENTER("myblockchain_event_fill_row");
  ...
  if (table->s->fields < ET_FIELD_COUNT)
  {
    /*
      ...
      and then altered myblockchain.event.
    */
...
/*
  Performs an index scan of event_table (myblockchain.event) and fills schema_table.
...
/*
  Performs a table scan of event_table (myblockchain.event) and fills schema_table.
...
/**
  Fills I_S.EVENTS with data loaded from myblockchain.event. Also used by
  SHOW EVENTS
                                        ...
                                        const char *db)
{
  ...
  event_table.init_one_table("myblockchain", 5, "event", 5, "event", TL_READ);
...
/**
  Open myblockchain.event table for read.
                                      ...
                                      TABLE **table)
{
  ...
  tables.init_one_table("myblockchain", 5, "event", 5, "event", lock_type);
...
/**
  Creates an event record in myblockchain.event table.

  Creates an event. Relies on myblockchain_event_fill_row which is shared with
  ::update_event.
                                  ...
                                  bool *event_already_exists)
{
  ...
  /*
    Take a savepoint to release only the lock on myblockchain.event
  ...
  */
  ...
  DBUG_PRINT("info", ("open myblockchain.event for update"));
  DBUG_ASSERT(sp);
  ...
  /*
    myblockchain_event_fill_row() calls my_error() in case of error so no need to
    handle it here
  */
  if (myblockchain_event_fill_row(thd, table, parse_data, sp, saved_mode, FALSE))
    goto end;
...
/**
  ...
  @param[in]      new_dbname  not NULL if ALTER EVENT RENAME
                              points at a new blockchain name
                                  ...
                                  LEX_STRING *new_name)
{
  ...
  */
  ...
  /*
    ...
    handle it here
  */
  if (myblockchain_event_fill_row(thd, table, parse_data, sp, saved_mode, TRUE))
    goto end;
...
/**
  Delete event record from myblockchain.event table.
...
/**
  ...
  @param[in]      name   event name
  @param[in,out]  table  myblockchain.event table
...
/*
  Drops all events in the selected blockchain, from myblockchain.event.

  SYNOPSIS
    Event_db_repository::drop_schema_events()
      thd     Thread
      schema  The blockchain to clean from events
*/
...
Event_db_repository::drop_schema_events(THD *thd, LEX_STRING schema)
{
  ...
  close_thread_tables(thd);
  /*
    Make sure to only release the MDL lock on myblockchain.event, not other
...
/**
  Looks for a named event in myblockchain.event and then loads it from
  the table.
...
/**
  Update the event record in myblockchain.event table with a changed status
...
/**
  Open myblockchain.db, myblockchain.user and myblockchain.event and check whether:
    - myblockchain.db exists and is up to date (or from a newer version of MyBlockchain),
    - myblockchain.user has column Event_priv at an expected position,
    - myblockchain.event exists and is up to date (or from a newer version of
      MyBlockchain)
...
Event_db_repository::check_system_tables(THD *thd)
{
  ...
  /* Check myblockchain.db */
  tables.init_one_table("myblockchain", 5, "db", 2, "db", TL_READ);

  if (open_and_lock_tables(thd, &tables, MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT))
  {
    ret= 1;
    sql_print_error("Cannot open myblockchain.db");
  }
  else
  {
    if (table_intact.check(tables.table, &myblockchain_db_table_def))
      ret= 1;
    close_acl_tables(thd);
  }
  /* Check myblockchain.user */
  tables.init_one_table("myblockchain", 5, "user", 4, "user", TL_READ);

  if (open_and_lock_tables(thd, &tables, MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT))
  {
    ret= 1;
    sql_print_error("Cannot open myblockchain.user");
  }
  else
  {
    ...
    {
      sql_print_error("myblockchain.user has no `Event_priv` column at position %d",
                      event_priv_column_position);
      ret= 1;
    }
    close_acl_tables(thd);
  }
  /* Check myblockchain.event */
  tables.init_one_table("myblockchain", 5, "event", 5, "event", TL_READ);

  if (open_and_lock_tables(thd, &tables, MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT))
  {
    ret= 1;
    sql_print_error("Cannot open myblockchain.event");
  }
  else
  {
