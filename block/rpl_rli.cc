   ...
   thd_tx_priority(0)
{
  ...
  if (!rli_fake)
  {
    myblockchain_mutex_init(key_relay_log_info_log_space_lock,
                     &log_space_lock, MY_MUTEX_INIT_FAST);
    myblockchain_cond_init(key_relay_log_info_log_space_cond, &log_space_cond);
    myblockchain_mutex_init(key_mutex_slave_parallel_pend_jobs, &pending_jobs_lock,
                     MY_MUTEX_INIT_FAST);
    myblockchain_cond_init(key_cond_slave_parallel_pend_jobs, &pending_jobs_cond);
    myblockchain_mutex_init(key_mutex_slave_parallel_worker_count, &exit_count_lock,
                   MY_MUTEX_INIT_FAST);
    myblockchain_mutex_init(key_mts_temp_table_LOCK, &mts_temp_table_LOCK,
                     MY_MUTEX_INIT_FAST);
    myblockchain_mutex_init(key_mts_gaq_LOCK, &mts_gaq_LOCK,
                     MY_MUTEX_INIT_FAST);
...
Relay_log_info::~Relay_log_info()
{
  ...
  if(!rli_fake)
  {
    ...
    myblockchain_mutex_destroy(&log_space_lock);
    myblockchain_cond_destroy(&log_space_cond);
    myblockchain_mutex_destroy(&pending_jobs_lock);
    myblockchain_cond_destroy(&pending_jobs_cond);
    myblockchain_mutex_destroy(&exit_count_lock);
    myblockchain_mutex_destroy(&mts_temp_table_LOCK);
    myblockchain_mutex_destroy(&mts_gaq_LOCK);
                                               ...
                                               bool need_data_lock)
{
  ...
  */
  ...
  {
    if (need_data_lock)
      myblockchain_mutex_lock(&data_lock);
    else
      myblockchain_mutex_assert_owner(&data_lock);
    last_master_timestamp= new_ts;
    if (need_data_lock)
      myblockchain_mutex_unlock(&data_lock);
  }
}
...
static inline int add_relay_log(Relay_log_info* rli,LOG_INFO* linfo)
{
  ...
  if (!myblockchain_file_stat(key_file_relaylog,
                                       ...
                                       bool look_for_description_event)
{
  ...
  *errmsg=0;
  ...
  static char errmsg_buff[MYBLOCKCHAIN_ERRMSG_SIZE + FN_REFLEN];
  myblockchain_mutex_t *log_lock= relay_log.get_log_lock();
  ...
  */
  ...
  if (cur_log_fd >= 0)
  {
    ...
    myblockchain_file_close(cur_log_fd, MYF(MY_WME));
    cur_log_fd = -1;
  }
...
err:
  ...
  */
  ...
  myblockchain_cond_broadcast(&data_cond);
...
{
  myblockchain_mutex_lock(&err_lock);
  ...
  myblockchain_mutex_unlock(&err_lock);
}
  ...
  */
         ...
         slave_running)
  {
    ...
    */
    ...
    /*
      We are going to myblockchain_cond_(timed)wait(); if the SQL thread stops it
      will wake us up.
    */
    ...
    {
      /*
        Note that myblockchain_cond_timedwait checks for the timeout
        before for the condition ; i.e. it returns ETIMEDOUT
        if the system time equals or exceeds the time specified by abstime
        before the condition variable is signaled or broadcast, _or_ if
        the absolute time specified by abstime has already passed at the time
        of the call.
        For that reason, myblockchain_cond_timedwait will do the "timeoutting" job
        even if its condition is always immediately signaled (case of a loaded
        master).
      */
      error= myblockchain_cond_timedwait(&data_cond, &data_lock, &abstime);
    }
    else
      myblockchain_cond_wait(&data_cond, &data_lock);
    thd_wait_end(thd);
  ...
  if (channel[0])
  {
   ...
   /* adding a "-" */
    ...
    /*
      ...
      as it was defined as utf8 in the myblockchain.slaveinfo tables.
    */
...
/**
  ...
  @retval TRUE failure
*/
bool myblockchain_show_relaylog_events(THD* thd)
{

  Master_info *mi =0;
  List<Item> field_list;
  bool res;
  DBUG_ENTER("myblockchain_show_relaylog_events");
...
int Relay_log_info::rli_init_info()
{
  ...
  */
  /*
    ...
    relay_log.max_size (and myblockchain_bin_log.max_size).
  */
  {
...
int Relay_log_info::flush_info(const bool force)
{
  ...
  /* 
    ...
    dinamically set.
  */
  myblockchain_mutex_lock(&mts_temp_table_LOCK);
  ...
  myblockchain_mutex_unlock(&mts_temp_table_LOCK);
  DBUG_RETURN(0);
