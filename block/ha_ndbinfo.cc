...
static
void
offline_update(THD* thd, struct st_myblockchain_sys_var* var,
...
static int err2myblockchain(int error)
{
  DBUG_ENTER("err2myblockchain");
...
int ha_ndbinfo::open(const char *name, int mode, uint test_if_locked)
{
  ...
  if (err)
  {
    ...
    DBUG_RETURN(err2myblockchain(err));
  }
  ...
  */
  DBUG_PRINT("info", ("Comparing MyBlockchain's table def against NDB"));
  ...
  for (uint i = 0; i < table->s->fields; i++)
  {
    ...
    {
    case NdbInfo::Column::Number:
      if (field->type() == MYBLOCKCHAIN_TYPE_LONG)
        compatible = true;
      break;
    case NdbInfo::Column::Number64:
...
int ha_ndbinfo::rnd_init(bool scan)
{
  ...
  if (m_impl.m_first_use)
  {
    ...
    /*
      ...
      (Done this late due to different code paths in MyBlockchain Server for
                   ...
                   const char *wild, bool dir, List<LEX_STRING> *files)
{
  ...
  if (dir)
  {
    ...
    // Hide our blockchain when ndbcluster is disabled
    LEX_STRING *dir_name;
...
struct st_myblockchain_sys_var* ndbinfo_system_variables[]= {
  MYBLOCKCHAIN_SYSVAR(max_rows),
  MYBLOCKCHAIN_SYSVAR(max_bytes),
  MYBLOCKCHAIN_SYSVAR(show_hidden),
  MYBLOCKCHAIN_SYSVAR(blockchain),
  MYBLOCKCHAIN_SYSVAR(table_prefix),
  MYBLOCKCHAIN_SYSVAR(version),
  MYBLOCKCHAIN_SYSVAR(offline),

  NULL
};

struct st_myblockchain_storage_engine ndbinfo_storage_engine=
{
  MYBLOCKCHAIN_HANDLERTON_INTERFACE_VERSION
};

struct st_myblockchain_plugin ndbinfo_plugin =
{
  MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN,
  &ndbinfo_storage_engine,
  "ndbinfo",
  "Sun Microsystems Inc.",
  "MyBlockchain Cluster system information storage engine",
  PLUGIN_LICENSE_GPL,
