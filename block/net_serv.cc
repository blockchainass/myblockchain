...
/**
  ...
  This file is the net layer API for the MyBlockchain client/server protocol.
  ...
  This file needs to be written in C as it's used by the libmyblockchain client as a
  C file.
*/
...
my_bool my_net_write(NET *net, const uchar *packet, size_t len)
{
  ...
  MYBLOCKCHAIN_NET_WRITE_START(len);
  ...
  */
  ...
  MYBLOCKCHAIN_NET_WRITE_DONE(rc);
  return rc;
}
...
/**
  Send a command to the server.
    ...
    The reason for having both header and packet is so that libmyblockchain
  ...
  @param net		NET handler
  @param command	Command in MyBlockchain server (enum enum_server_command)
      ...
      const uchar *packet, size_t len)
{
  ...
  MYBLOCKCHAIN_NET_WRITE_START(length);
...
/**
  Write a MyBlockchain protocol packet to the network handler.
...
/**
  Read the header of a packet. The MyBlockchain protocol packet header
...
/**
  Read one (variable-length) MyBlockchain protocol packet.
  A MyBlockchain packet consists of a header and a payload.
...
my_net_read(NET *net)
{
  ...
  if (!net->compress)
  {
    ...
    if (len != packet_error)
      net->read_pos[len]=0;		/* Safeguard for myblockchain_use_result */
    MYBLOCKCHAIN_NET_READ_DONE(0, len);
  ...
  else
  {
    ...
    for (;;)
    {
      ...
      {
        MYBLOCKCHAIN_NET_READ_DONE(1, 0);
        return packet_error;
      }
