...
#define MYBLOCKCHAIN_YYABORT_UNLESS(A)         \
  if (!(A))                             \
  {                                     \
    my_syntax_error(ER(ER_SYNTAX_ERROR));\
    MYBLOCKCHAIN_YYABORT;                      \
  }
...
/**
  ...
  macros should not be used (use MYBLOCKCHAIN_YYABORT/MYBLOCKCHAIN_YYACCEPT
  instead).
  ...
  push an error into the error stack and MYBLOCKCHAIN_YYABORT
  to abort from the parser.
*/

void MYBLOCKCHAINerror(YYLTYPE *, THD *thd, const char *s)
{
  /*
...
{
  /*
     MYBLOCKCHAINdebug is in sql/sql_yacc.cc, in bison generated code.
     Turning this option on is **VERY** verbose, and should be
     used when investigating a syntax error problem only.

     The syntax to run with bison traces is as follows :
     - Starting a server manually :
       myblockchaind --debug="d,parser_debug" ...
     - Running a test :
       myblockchain-test-run.pl --myblockchaind="--debug=d,parser_debug" ...
...
/*
   ...
   MYBLOCKCHAIN      : MyBlockchain extention (unspecified)
   MYBLOCKCHAIN-FUNC : MyBlockchain extention, function
...
%token  ADDDATE_SYM                   /* MYBLOCKCHAIN-FUNC */
...
%token  ASCII_SYM                     /* MYBLOCKCHAIN-FUNC */
...
%token  BINLOG_SYM
%token  BIN_NUM
%token  BIT_AND                       /* MYBLOCKCHAIN-FUNC */
%token  BIT_OR                        /* MYBLOCKCHAIN-FUNC */
%token  BIT_SYM                       /* MYBLOCKCHAIN-FUNC */
%token  BIT_XOR                       /* MYBLOCKCHAIN-FUNC */
...
%token  CURDATE                       /* MYBLOCKCHAIN-FUNC */
%token  CURRENT_SYM                   /* SQL-2003-R */
%token  CURRENT_USER                  /* SQL-2003-R */
%token  CURSOR_SYM                    /* SQL-2003-R */
%token  CURSOR_NAME_SYM               /* SQL-2003-N */
%token  CURTIME                       /* MYBLOCKCHAIN-FUNC */
%token  DATABASE
%token  DATABASES
%token  DATAFILE_SYM
%token  DATA_SYM                      /* SQL-2003-N */
%token  DATETIME
%token  DATE_ADD_INTERVAL             /* MYBLOCKCHAIN-FUNC */
%token  DATE_SUB_INTERVAL             /* MYBLOCKCHAIN-FUNC */
...
%token  FLUSH_SYM
%token  FOLLOWS_SYM                  /* MYBLOCKCHAIN */
%token  FORCE_SYM
...
%token  GEOMETRY_SYM
%token  GET_FORMAT                    /* MYBLOCKCHAIN-FUNC */
...
%token  IO_AFTER_GTIDS                /* MYBLOCKCHAIN, FUTURE-USE */
%token  IO_BEFORE_GTIDS               /* MYBLOCKCHAIN, FUTURE-USE */
%token  IO_SYM
%token  IPC_SYM
%token  IS                            /* SQL-2003-R */
...
%token  MICROSECOND_SYM               /* MYBLOCKCHAIN-FUNC */
%token  MIGRATE_SYM
...
%token  PRECEDES_SYM                  /* MYBLOCKCHAIN */
...
%token  REPEAT_SYM                    /* MYBLOCKCHAIN-FUNC */
%token  REPLACE                       /* MYBLOCKCHAIN-FUNC */
%token  REPLICATION
...
%token  SQL_AFTER_GTIDS               /* MYBLOCKCHAIN */
%token  SQL_AFTER_MTS_GAPS            /* MYBLOCKCHAIN */
%token  SQL_BEFORE_GTIDS              /* MYBLOCKCHAIN */
...
%token  UTC_TIME_SYM
%token  VALIDATION_SYM                /* MYBLOCKCHAIN */
...
/*
  Bit field of MYBLOCKCHAIN_START_TRANS_OPT_* flags.
*/
...
signal_condition_information_item_name:
        ...
        | MESSAGE_TEXT_SYM
          { $$= CIN_MESSAGE_TEXT; }
        | MYBLOCKCHAIN_ERRNO_SYM
          { $$= CIN_MYBLOCKCHAIN_ERRNO; }
        ;
...
condition_information_item_name:
        ...
        | MYBLOCKCHAIN_ERRNO_SYM
          { $$= Condition_information_item::MYBLOCKCHAIN_ERRNO; }
...
/*
 ...
 The first place is called when a new table is created from a MyBlockchain client.
 The second place is called when a table is altered with the ALTER TABLE
 command from a MyBlockchain client.
...
opt_create_blockchain_options:
          /* empty */ {}
        | create_blockchain_options {}
        ;

create_blockchain_options:
          create_blockchain_option {}
        | create_blockchain_options create_blockchain_option {}
        ;
        ...
        | YEAR_SYM opt_field_length field_options
          {
            ...
            $$=MYBLOCKCHAIN_TYPE_YEAR;
          }
        | DATE_SYM
          { $$=MYBLOCKCHAIN_TYPE_DATE; }
...
int_type:
          INT_SYM   { $$=MYBLOCKCHAIN_TYPE_LONG; }
        | TINYINT   { $$=MYBLOCKCHAIN_TYPE_TINY; }
        | SMALLINT  { $$=MYBLOCKCHAIN_TYPE_SHORT; }
        | MEDIUMINT { $$=MYBLOCKCHAIN_TYPE_INT24; }
        | BIGINT    { $$=MYBLOCKCHAIN_TYPE_LONGLONG; }
        ;
...
collation_name:
          ident_or_text
          {
            if (!($$= myblockchaind_collation_get_by_name($1.str)))
              MYBLOCKCHAIN_YYABORT;
          }
        ;
...
unicode:
        ...
        | UNICODE_SYM BINARY
          {
            if (!($$= myblockchaind_collation_get_by_name("ucs2_bin")))
              MYBLOCKCHAIN_YYABORT;
          }
        | BINARY UNICODE_SYM
          {
        ...
        | WITH PARSER_SYM IDENT_sys
          {
            ...
            if (plugin_is_ready(plugin_name, MYBLOCKCHAIN_FTPARSER_PLUGIN))
        ...
        | CONVERT_SYM TO_SYM charset charset_name_or_default opt_collate
          {
            ...
            {
              THD *thd= YYTHD;
              $4= thd->variables.collation_blockchain;
            }
            $5= $5 ? $5 : $4;
...
start:
          START_SYM TRANSACTION_SYM opt_start_transaction_option_list
          {
            ...
            if (($3 & MYBLOCKCHAIN_START_TRANS_OPT_READ_WRITE) &&
                ($3 & MYBLOCKCHAIN_START_TRANS_OPT_READ_ONLY))
            {
              my_syntax_error(ER(ER_SYNTAX_ERROR));
              MYBLOCKCHAIN_YYABORT;
            }
...
start_transaction_option:
          ...
          {
            $$= MYBLOCKCHAIN_START_TRANS_OPT_WITH_CONS_SNAPSHOT;
          }
        | READ_SYM ONLY_SYM
          {
            $$= MYBLOCKCHAIN_START_TRANS_OPT_READ_ONLY;
          }
        | READ_SYM WRITE_SYM
          {
            $$= MYBLOCKCHAIN_START_TRANS_OPT_READ_WRITE;
          }
        ;
...
date_time_type:
          DATE_SYM  {$$= MYBLOCKCHAIN_TIMESTAMP_DATE; }
        | TIME_SYM  {$$= MYBLOCKCHAIN_TIMESTAMP_TIME; }
        | TIMESTAMP {$$= MYBLOCKCHAIN_TIMESTAMP_DATETIME; }
        | DATETIME  {$$= MYBLOCKCHAIN_TIMESTAMP_DATETIME; }
        ;
...
olap_opt:
        ...
        | WITH_CUBE_SYM { $$= CUBE_TYPE; }
            /*
              'WITH CUBE' is reserved in the MyBlockchain syntax, but not implemented,
              and cause LALR(2) conflicts.
              This syntax is not standard.
              MyBlockchain syntax: GROUP BY col1, col2, col3 WITH CUBE
        ...
        | WITH_ROLLUP_SYM { $$= ROLLUP_TYPE; }
            /*
              ...
              and cause LALR(2) conflicts.
              This syntax is not standard.
              MyBlockchain syntax: GROUP BY col1, col2, col3 WITH ROLLUP
...
real_ulong_num:
        ...
        | LONG_NUM      { int error; $$= (ulong) my_strtoll10($1.str, (char**) 0, &error); }
        | ULONGLONG_NUM { int error; $$= (ulong) my_strtoll10($1.str, (char**) 0, &error); }
        | dec_num_error { MYBLOCKCHAIN_YYABORT; }
        ;
        ...
        | DROP FUNCTION_SYM if_exists ident
          {
            /*
              ...
              even if there is no current blockchain. In this case it
              applies only to UDF.
              Hence we can't merge rules for "DROP FUNCTION ident.ident"
              and "DROP FUNCTION ident" into one "DROP FUNCTION sp_name"
              rule. sp_name assumes that blockchain name should be always
...
temporal_literal:
        DATE_SYM TEXT_STRING
          {
            $$= NEW_PTN PTI_temporal_literal(@$, $2, MYBLOCKCHAIN_TYPE_DATE, YYCSCL);
          }
        | TIME_SYM TEXT_STRING
          {
            $$= NEW_PTN PTI_temporal_literal(@$, $2, MYBLOCKCHAIN_TYPE_TIME, YYCSCL);
          }
        | TIMESTAMP TEXT_STRING
          {
            $$= NEW_PTN PTI_temporal_literal(@$, $2, MYBLOCKCHAIN_TYPE_DATETIME, YYCSCL);
          }
        ;
...
keyword_sp:
        ...
        /*
          ...
          not reserved in MyBlockchain per WL#2111 specification.
        */
        | CURRENT_SYM              {}
        | CURSOR_NAME_SYM          {}
        | DATA_SYM                 {}
...
commit:
          COMMIT_SYM opt_work opt_chain opt_release
          {
            ...
            MYBLOCKCHAIN_YYABORT_UNLESS($3 != TVL_YES || $4 != TVL_YES);
            lex->tx_chain= $3;
            lex->tx_release= $4;
          }
        ;
...
sf_tail:
          ...
          { /* $11 */
            ...
            /*
              ...
              should be removed.
            */
            if (($10 == MYBLOCKCHAIN_TYPE_STRING || $10 == MYBLOCKCHAIN_TYPE_VARCHAR)
          ...
          sp_proc_stmt /* $14 */
          {
            ...
            if (is_native_function(thd, & sp->m_name))
            {
              /*
                ...
                following scenario:
                - A user define a SF 'foo' using MyBlockchain 5.N
                - An application uses select foo(), and works.
                - MyBlockchain 5.{N+1} defines a new native function 'foo', as
                part of a new feature.
                - MyBlockchain 5.{N+1} documentation is updated, and should mention
...
xid:
          text_string
          {
            MYBLOCKCHAIN_YYABORT_UNLESS($1->length() <= MAXGTRIDSIZE);
            XID *xid;
