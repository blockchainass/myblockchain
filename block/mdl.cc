...
#include <myblockchain/psi/myblockchain_mdl.h>
...
static void init_mdl_psi_keys(void)
{
  ...
  myblockchain_mutex_register("sql", all_mdl_mutexes, count);

  count= array_elements(all_mdl_rwlocks);
  myblockchain_rwlock_register("sql", all_mdl_rwlocks, count);

  count= array_elements(all_mdl_conds);
  myblockchain_cond_register("sql", all_mdl_conds, count);

  count= array_elements(all_mdl_memory);
  myblockchain_memory_register("sql", all_mdl_memory, count);
...
void MDL_key::init_psi_keys()
{
  ...
  for (i= 0; i<count; i++)
  {
    /* myblockchain_stage_register wants an array of pointers, registering 1 by 1. */
    info= & MDL_key::m_namespace_to_wait_state_name[i];
    myblockchain_stage_register("sql", &info, 1);
  }
}
#endif
...
public:
  ...
  */
  myblockchain_prlock_t m_rwlock;
  ...
  */
  ...
  {
    myblockchain_prlock_assert_write_owner(&m_rwlock);
    /*
...
public:
  ...
  */
  ...
  {
    myblockchain_prlock_init(key_MDL_lock_rwlock, &m_rwlock);
  }
  ...
  ~MDL_lock()
  {
    myblockchain_prlock_destroy(&m_rwlock);
  }
                                   ...
                                   int32 *unused_locks)
{
  ...
  */
  myblockchain_prlock_wrlock(&lock->m_rwlock);
  ...
  if (lock->m_fast_path_state & MDL_lock::IS_DESTROYED)
  {
    ...
    */
    myblockchain_prlock_unlock(&lock->m_rwlock);
    lf_hash_search_unpin(pins);
    *unused_locks= m_unused_lock_objects;
    return;
  }
...
{
  myblockchain_prlock_init(key_MDL_context_LOCK_waiting_for, &m_LOCK_waiting_for);
}
...
void MDL_context::destroy()
{
  ...
  myblockchain_prlock_destroy(&m_LOCK_waiting_for);
  if (m_pins)
...
/**
  ...
  @param  db             Name of blockchain to which the object belongs
...
{
  myblockchain_mdl_destroy(ticket->m_psi);
  ticket->m_psi= NULL;
...
MDL_wait::MDL_wait()
  :m_wait_status(EMPTY)
{
  myblockchain_mutex_init(key_MDL_wait_LOCK_wait_status, &m_LOCK_wait_status, NULL);
  myblockchain_cond_init(key_MDL_wait_COND_wait_status, &m_COND_wait_status);
}
...
MDL_wait::~MDL_wait()
{
  myblockchain_mutex_destroy(&m_LOCK_wait_status);
  myblockchain_cond_destroy(&m_COND_wait_status);
}
...
bool MDL_wait::set_status(enum_wait_status status_arg)
{
  ...
  myblockchain_mutex_lock(&m_LOCK_wait_status);
  if (m_wait_status == EMPTY)
  {
    was_occupied= FALSE;
    m_wait_status= status_arg;
    myblockchain_cond_signal(&m_COND_wait_status);
  }
  myblockchain_mutex_unlock(&m_LOCK_wait_status);
  return was_occupied;
}
                     ...
                     const PSI_stage_info *wait_state_name)
{
  ...
  {
    wait_result= myblockchain_cond_timedwait(&m_COND_wait_status, &m_LOCK_wait_status,
                                      abs_timeout);
  }
  thd_wait_end(NULL);
                             ...
                             MDL_ticket *ticket)
{
  ...
  myblockchain_prlock_wrlock(&m_rwlock);
  ...
  */
  ...
  myblockchain_prlock_unlock(&m_rwlock);
...
bool MDL_lock::has_pending_conflicting_lock(enum_mdl_type type)
{
  ...
  myblockchain_prlock_rdlock(&m_rwlock);
                                   ...
                                   MDL_ticket **out_ticket)
{
  ...
  */
  ...
  ticket->m_psi= myblockchain_mdl_create(ticket, key,
                                  mdl_request->type,
                                  mdl_request->duration,
                                  MDL_wait::EMPTY,
  ...
  */
  ...
  if (! force_slow)
  {
    ...
    */
    ...
    myblockchain_mdl_set_status(ticket->m_psi, MDL_wait::GRANTED);
    return FALSE;
  }
...
MDL_context::clone_ticket(MDL_request *mdl_request)
{
  ...
  */
  ...
  if (mdl_request->ticket->m_is_fast_path)
  {
    ...
    */
    ...
    /*
      ...
      invariant [INV1].
    */
    myblockchain_prlock_wrlock(&ticket->m_lock->m_rwlock);
    ticket->m_lock->fast_path_state_add(unobtrusive_lock_increment);
    myblockchain_prlock_unlock(&ticket->m_lock->m_rwlock);
    ticket->m_is_fast_path= true;
  }
  else
  {
  ...
  DBUG_ASSERT(ticket->m_psi == NULL);
  ticket->m_psi= myblockchain_mdl_create(ticket,
                                  &mdl_request->key,
                                  mdl_request->type,
                                  mdl_request->duration,
                                  MDL_wait::GRANTED,
...
bool MDL_context::visit_subgraph(MDL_wait_for_graph_visitor *gvisitor)
{
  ...
  myblockchain_prlock_rdlock(&m_LOCK_waiting_for);

  if (m_waiting_for)
    result= m_waiting_for->accept_visitor(gvisitor);

  myblockchain_prlock_unlock(&m_LOCK_waiting_for);
...
void MDL_ticket::downgrade_lock(enum_mdl_type new_type)
{
  ...
  */
  ...
  myblockchain_prlock_wrlock(&m_lock->m_rwlock);
  /*
  ...
  */
  ...
  myblockchain_prlock_unlock(&m_lock->m_rwlock);
}
...
/**
  ...
  @param db            Name of the blockchain
...
retry:
  ...
  myblockchain_prlock_rdlock(&lock->m_rwlock);
