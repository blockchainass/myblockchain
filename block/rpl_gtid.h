...
#include "myblockchaind.h"             // key_rwlock_global_sid_lock
#include "table.h"
#endif
...
/**
  ...
  or myblockchainbinlog.  There is no common error reporting mechanism, so we
  have to duplicate the error message (write it out in the source file
  for myblockchainbinlog, write it in share/errmsg-utf8.txt for the server).

  @param MYBLOCKCHAINBINLOG_ERROR arguments to myblockchainbinlog's 'error'
...
#define BINLOG_ERROR(MYBLOCKCHAINBINLOG_ERROR, SERVER_ERROR) error MYBLOCKCHAINBINLOG_ERROR
#else
#define BINLOG_ERROR(MYBLOCKCHAINBINLOG_ERROR, SERVER_ERROR) my_error SERVER_ERROR
#endif
...
/*
  ...
  /// myblockchain_bin_log.get_log_lock() held.
  GTID_MODE_LOCK_LOG
*/
};
/**
...
/**
  This has the functionality of myblockchain_rwlock_t, with two differences:
...
public:
...
#endif
#if defined(HAVE_PSI_INTERFACE)
    myblockchain_rwlock_init(psi_key, &rwlock);
#else
    myblockchain_rwlock_init(0, &rwlock);
#endif
  }
  /// Destroy this Checkable_lock.
  ~Checkable_rwlock()
  {
    myblockchain_rwlock_destroy(&rwlock);
  }

  /// Acquire the read lock.
  inline void rdlock()
  {
    myblockchain_rwlock_rdlock(&rwlock);
    assert_no_wrlock();
#ifndef DBUG_OFF
    if (dbug_trace)
  ...
  inline void wrlock()
  {
    myblockchain_rwlock_wrlock(&rwlock);
    assert_no_lock();
#ifndef DBUG_OFF
    if (dbug_trace)
  ...
  inline void unlock()
  {
    ...
    is_write_lock= false;
#endif
    myblockchain_rwlock_unlock(&rwlock);
  }
  /**
...
public:
  ...
  */
  ...
  {
    assert_not_owner(n);
    myblockchain_mutex_lock(&get_mutex_cond(n)->mutex);
  }
  /// Unlock the n'th mutex.
  inline void unlock(int n) const
  {
    assert_owner(n);
    myblockchain_mutex_unlock(&get_mutex_cond(n)->mutex);
  }
  /// Broadcast the n'th condition.
  inline void broadcast(int n) const
  {
    myblockchain_cond_broadcast(&get_mutex_cond(n)->cond);
  }
  /**
  ...
  */
  ...
  {
#ifndef DBUG_OFF
    myblockchain_mutex_assert_owner(&get_mutex_cond(n)->mutex);
#endif
  }
  /**
  ...
  */
  ...
  {
#ifndef DBUG_OFF
    myblockchain_mutex_assert_not_owner(&get_mutex_cond(n)->mutex);
#endif
  }
  /**
  ...
  */
  inline void wait(const THD* thd, int n) const
  {
    ...
    {
      myblockchain_mutex_assert_owner(&mutex_cond->mutex);
      myblockchain_cond_wait(&mutex_cond->cond, &mutex_cond->mutex);
      myblockchain_mutex_assert_owner(&mutex_cond->mutex);
    }
    DBUG_VOID_RETURN;
  }
  ...
  */
  inline int wait(const THD* thd, int sidno, struct timespec* abstime) const
  {
    ...
    {
      myblockchain_mutex_assert_owner(&mutex_cond->mutex);
      error= myblockchain_cond_timedwait(&mutex_cond->cond,
...
public:
  ...
  */
  ...
  {
    if (sid_lock != NULL)
      myblockchain_mutex_lock(&free_intervals_mutex);
    add_interval_memory_lock_taken(n_intervals, intervals_param);
    if (sid_lock != NULL)
      myblockchain_mutex_unlock(&free_intervals_mutex);
  }
...
private:
  ...
  */
  myblockchain_mutex_t free_intervals_mutex;
  /**
  ...
  */
  ...
  public:
    ...
    void lock_if_not_locked()
    {
      ...
      {
        myblockchain_mutex_lock(&gtid_set->free_intervals_mutex);
        locked= true;
      }
    }
    ...
    void unlock_if_locked()
    {
      if (gtid_set->sid_lock && locked)
      {
        myblockchain_mutex_unlock(&gtid_set->free_intervals_mutex);
        locked= false;
      }
    }
  ...
  {
    if (sid_lock != NULL)
      myblockchain_mutex_assert_owner(&free_intervals_mutex);
  }
