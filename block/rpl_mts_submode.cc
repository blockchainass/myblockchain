...
*/
int
Mts_submode_blockchain::schedule_next_event(Relay_log_info *rli, Log_event *ev)
{
  /*nothing to do here*/
  return 0;
}
...
*/
void
Mts_submode_blockchain::attach_temp_tables(THD *thd, const Relay_log_info* rli,
                                         Query_log_event* ev)
{
  int i, parts;
  DBUG_ENTER("Mts_submode_blockchain::attach_temp_tables");
...
int
Mts_submode_blockchain::wait_for_workers_to_finish(Relay_log_info *rli,
                                                 Slave_worker *ignore)
{
  uint ret= 0;
  ...
  DBUG_ENTER("Mts_submode_blockchain::wait_for_workers_to_finish");
 ...
 */
void
Mts_submode_blockchain::detach_temp_tables(THD *thd, const Relay_log_info* rli,
                                         Query_log_event *ev)
{
  int i, parts;
  DBUG_ENTER("Mts_submode_blockchain::detach_temp_tables");
...
/**
  Logic to get least occupied worker when the sql mts_submode= blockchain
  @param
 ...
 */
Slave_worker *
Mts_submode_blockchain::get_least_occupied_worker(Relay_log_info *rli,
                                                ...
                                                Log_event *ev)
{
 long usage= LONG_MAX;
  ...
  DBUG_ENTER("Mts_submode_blockchain::get_least_occupied_worker");
                            ...
                            longlong lwm_estimate_arg)
{
  ...
  */
      ...
      !clock_leq(last_committed_arg, get_lwm_timestamp(rli, true)))
  {
    ...
    do
    {
      myblockchain_cond_wait(&rli->logical_clock_cond, &rli->mts_gaq_LOCK);
    }
                                       ...
                                       Query_log_event * ev)
{
  ...
  DBUG_ASSERT(!thd->temporary_tables);
  myblockchain_mutex_lock(&c_rli->mts_temp_table_LOCK);
  if (!(table= c_rli->info_thd->temporary_tables))
  {
    myblockchain_mutex_unlock(&c_rli->mts_temp_table_LOCK);
    DBUG_VOID_RETURN;
  }
