...
#include "sql_base.h"                   /* MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT */
...
{
  init (get_field(mem, table->field[MYBLOCKCHAIN_PROXIES_PRIV_HOST]),
        get_field(mem, table->field[MYBLOCKCHAIN_PROXIES_PRIV_USER]),
        get_field(mem, table->field[MYBLOCKCHAIN_PROXIES_PRIV_PROXIED_HOST]),
        get_field(mem, table->field[MYBLOCKCHAIN_PROXIES_PRIV_PROXIED_USER]),
                  table->field[MYBLOCKCHAIN_PROXIES_PRIV_WITH_GRANT]->val_int() != 0);
}
                         ...
                         const LEX_CSTRING &proxied_user)
{
  ...
  if (table->field[MYBLOCKCHAIN_PROXIES_PRIV_HOST]->store(host.str,
                                                   host.length,
                                                   system_charset_info))
    DBUG_RETURN(TRUE);
  if (table->field[MYBLOCKCHAIN_PROXIES_PRIV_USER]->store(user.str,
                                                   user.length,
                                                   system_charset_info))
    DBUG_RETURN(TRUE);
  if (table->field[MYBLOCKCHAIN_PROXIES_PRIV_PROXIED_HOST]->store(proxied_host.str,
                                                           proxied_host.length,
                                                           system_charset_info))
    DBUG_RETURN(TRUE);
  if (table->field[MYBLOCKCHAIN_PROXIES_PRIV_PROXIED_USER]->store(proxied_user.str,
                                                           proxied_user.length,
                                                           system_charset_info))
    DBUG_RETURN(TRUE);
                                 ...
                                 bool with_grant)
{
  ...
  if (table->field[MYBLOCKCHAIN_PROXIES_PRIV_WITH_GRANT]->store(with_grant ? 1 : 0,
                                                         TRUE))
    DBUG_RETURN(TRUE);
                                  ...
                                  const char *grantor)
{
  ...
  if (store_with_grant(table, with_grant))
    DBUG_RETURN(TRUE);
  if (table->field[MYBLOCKCHAIN_PROXIES_PRIV_GRANTOR]->store(grantor, 
                                                      strlen(grantor),
                                                      system_charset_info))
    DBUG_RETURN(TRUE);
              ...
              const char *user, const char *db, my_bool db_is_pattern)
{
  ...
  */
  ...
  /*
    Check if there are some access rights for blockchain and user
  */
                 ...
                 char *ip, const char *db)
{
  ...
  {
    /*
      here if myblockchaind's been started with --skip-grant-tables option.
    */
    sctx->skip_grants();
    DBUG_RETURN(FALSE);
  }
  ...
  /*
     Find acl entry in user blockchain.
...
/**
  ...
  Note that it works only for native and "old" myblockchain authentication built-in
  plugins.
...
/*
  ...
  load privilege information for them from tables in the 'myblockchain' blockchain.
...
my_bool acl_init(bool dont_read_acl_tables)
{
  ...
  */
  native_password_plugin= my_plugin_lock_by_name(0,
           native_password_plugin_name, MYBLOCKCHAIN_AUTHENTICATION_PLUGIN);
...
/*
  ...
  SYNOPSIS
    acl_load()
      thd     Current thread
      tables  List containing open "myblockchain.host", "myblockchain.user",
              "myblockchain.db" and "myblockchain.proxies_priv" tables in that order.
...
static my_bool acl_load(THD *thd, TABLE_LIST *tables)
{
  ...
  /*
    Prepare reading from the myblockchain.user table
  */
  ...
  acl_users->clear();
  /*
   We need to check whether we are working with old blockchain layout. This
   might be the case for instance when we are running myblockchain_upgrade.
  */
  ...
  while (!(read_rec_errcode= read_record_info.read_record(&read_record_info)))
  {
    ...
    /* Reading record from myblockchain.user */
    ACL_USER user;
    ...
    */
    ...
    {
      sql_print_error("Fatal error: myblockchain.user table is damaged. "
                      "Please run myblockchain_upgrade.");
      goto end;
    }
          ...
          */
                            ...
                            native_password_plugin_name.str) == 0))
          {
            ...
            if (native_plugin)
            {
              ...
              st_myblockchain_auth *auth=
                (st_myblockchain_auth *) plugin_decl(native_plugin)->info;
  ...
  } // END while reading records from the myblockchain.user table
  ...
  if (super_users_with_empty_plugin)
  {
    sql_print_warning("Some of the user accounts with SUPER privileges were "
                      "disabled because of empty myblockchain.user.plugin value. "
                      "If you are upgrading from MyBlockchain 5.6 to MyBlockchain 5.7 it "
    ...
    sql_print_warning("1. Stop the server and restart it with "
                      "--skip-grant-tables.");
    sql_print_warning("2. Run myblockchain_upgrade.");
    sql_print_warning("3. Restart the server with the parameters you "
                      "normally use.");
    sql_print_warning("For complete instructions on how to upgrade MyBlockchain "
                      "to a new version please see the 'Upgrading MyBlockchain' "
                      "section from the MyBlockchain manual");
  }

  /*
    Prepare reading from the myblockchain.db table
  */
  ...
  while (!(read_rec_errcode= read_record_info.read_record(&read_record_info)))
  {
    /* Reading record in myblockchain.db */
    ACL_DB db;
    db.host.update_hostname(get_field(&global_acl_memory, 
                            table->field[MYBLOCKCHAIN_DB_FIELD_HOST]));
    db.db=get_field(&global_acl_memory, table->field[MYBLOCKCHAIN_DB_FIELD_DB]);
    if (!db.db)
    {
      sql_print_warning("Found an entry in the 'db' table with empty blockchain name; Skipped");
      continue;
    }
    db.user=get_field(&global_acl_memory, table->field[MYBLOCKCHAIN_DB_FIELD_USER]);
    ...
    if (lower_case_table_names)
    {
      ...
      */
      ...
      {
        sql_print_warning("'db' entry '%s %s@%s' had blockchain in mixed "
  ...
  } // END reading records from myblockchain.db tables
  ...
  /* Prepare to read records from the myblockchain.proxies_priv table */
  ...
  if (tables[2].table)
  {
    ...
    while (!(read_rec_errcode= read_record_info.read_record(&read_record_info)))
    {
      /* Reading record in myblockchain.proxies_priv */
    ...
    } // END reading records from the myblockchain.proxies_priv table
  ...
  {
    sql_print_error("Missing system table myblockchain.proxies_priv; "
                    "please run myblockchain_upgrade to create it");
  }
...
my_bool acl_reload(THD *thd)
{
  ...
  */
  tables[0].init_one_table(C_STRING_WITH_LEN("myblockchain"),
...
/**
  @brief Initialize structures responsible for table/column-level privilege
   checking and load information for them from tables in the 'myblockchain' blockchain.
...
/**
  ...
  @param tables List containing open "myblockchain.tables_priv" and
    "myblockchain.columns_priv" tables.
...
my_bool grant_reload(THD *thd)
{
  ...
  /*
    Reload will work in the following manner:-
                   ...
                   /               \                     |
    myblockchain.procs_priv table        Server Startup         |
        is missing                      \                |
  ...
  */
  ...
  {
    sql_print_warning("Table 'myblockchain.procs_priv' does not exist. "
                      "Please run myblockchain_upgrade.");
