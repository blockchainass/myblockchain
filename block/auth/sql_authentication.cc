...
#include <myblockchain/plugin_validate_password.h> /* validate_password plugin */
...
/****************************************************************************
   ...
   MyBlockchain authentication plugins.
****************************************************************************/

LEX_CSTRING native_password_plugin_name= {
  C_STRING_WITH_LEN("myblockchain_native_password")
};
...
Rsa_authentication_keys::get_key_file_path(char *key, String *key_file_path)
{
  ...
  {
    key_file_path->append(myblockchain_real_data_home, strlen(myblockchain_real_data_home));
                                       ...
                                       char **key_text_buffer)
{
  ...
  */
  ...
  else
  {
    ...
    if (!(*key_ptr))
    {
      char error_buf[MYBLOCKCHAIN_ERRMSG_SIZE];
      ERR_error_string_n(ERR_get_error(), error_buf, MYBLOCKCHAIN_ERRMSG_SIZE);
...
/**
  Only the plugins that are known to use the myblockchain.user table 
                                       ...
                                       const uchar *data, uint data_len)
{
  ...
  const char *client_auth_plugin=
    ((st_myblockchain_auth *) (plugin_decl(mpvio->plugin)->info))->client_auth_plugin;
...
{
  myblockchain_mutex_lock(&acl_cache->lock);
  if (allow_all_hosts)
  {
    
    myblockchain_mutex_unlock(&acl_cache->lock);
    return 0;
  }
                      ...
                      MEM_ROOT *mem)
{
  ...
  user->salt_len= 0;
  user->password_last_changed.time_type= MYBLOCKCHAIN_TIMESTAMP_ERROR;
...
/**
   Finds acl entry in user blockchain for authentication purposes.
...
static bool parse_com_change_user_packet(MPVIO_EXT *mpvio, size_t packet_length)
{
  ...
  */
  ...
  db+= passwd_len + 1;
  /*
    Database name is always NUL-terminated, so in case of empty blockchain
  ...
  */
  ...
  /* Convert blockchain and user names to utf8 */
  ...
  if (!initialized)
  {
    // if myblockchaind's been started with --skip-grant-tables option
...
/**
  Get a string formatted according to the 4.1 version of the MyBlockchain protocol.
...
/**
  Get a string formatted according to the 4.0 version of the MyBlockchain protocol.
...
skip_to_ssl:
  ...
  */
  ...
  /*
    Copy and convert the user and blockchain names to the character set used
    by the server. Since 4.1 all blockchain names are stored in UTF-8. Also,
...
/**
  ...
  it escapes the plugin data, if it starts with a myblockchain protocol packet byte.
*/
static int server_mpvio_write_packet(MYBLOCKCHAIN_PLUGIN_VIO *param,
                                   const uchar *packet, int packet_len)
{
  ...
  /* 
    Reset cached_client_reply if not an old client doing myblockchain_change_user, 
...
*/
static int server_mpvio_read_packet(MYBLOCKCHAIN_PLUGIN_VIO *param, uchar **buf)
{
  MPVIO_EXT *mpvio= (MPVIO_EXT *) param;
  Protocol_classic *protocol = mpvio->protocol;
  size_t pkt_len;
...
/**
  ...
  connection
*/
static void server_mpvio_info(MYBLOCKCHAIN_PLUGIN_VIO *vio,
                        ...
                        MPVIO_EXT *mpvio)
{
  ...
  if (plugin)
  {
    st_myblockchain_auth *auth= (st_myblockchain_auth *) plugin_decl(plugin)->info;
...
check_password_lifetime(THD *thd, const ACL_USER *acl_user)
{
  ...
  if (likely(acl_user != NULL) && !acl_user->password_expired &&
      acl_user->password_last_changed.time_type != MYBLOCKCHAIN_TIMESTAMP_ERROR
      && auth_plugin_is_built_in(acl_user->plugin.str)
      && (acl_user->use_default_password_lifetime ||
      acl_user->password_lifetime))
  {
    MYBLOCKCHAIN_TIME cur_time, password_change_by;
    Interval interval;
...
acl_authenticate(THD *thd, enum_server_command command)
{
  ...
  */
  ...
  if (initialized) // if not --skip-grant-tables
  {
    ...
    {
      myblockchain_mutex_lock(&LOCK_offline_mode);
      bool tmp_offline_mode= MY_TEST(offline_mode);
      myblockchain_mutex_unlock(&LOCK_offline_mode);
  ...
  /*
    This is the default access rights for the current blockchain.  It's
    set to 0 here because we don't have an active blockchain yet (and we
    may not have an active blockchain to set.
  */
  sctx->set_db_access(0);

  /* Change a blockchain if necessary */
  if (mpvio.db.length)
  {
    if (myblockchain_change_db(thd, to_lex_cstring(mpvio.db), false))
    {
      /* myblockchain_change_db() has pushed the error message. */
      release_user_connection(thd);
      Host_errors errors;
      errors.m_default_blockchain= 1;
      inc_host_errors(mpvio.ip, &errors);
      DBUG_RETURN(1);
    }
  }
...
/**
  MyBlockchain Server Password Authentication Plugin

  In the MyBlockchain authentication protocol:
  1. the server sends the random scramble to the client
  2. client sends the encrypted password back to the server
  3. the server checks the password.
*/
static int native_password_authenticate(MYBLOCKCHAIN_PLUGIN_VIO *vio,
                                        MYBLOCKCHAIN_SERVER_AUTH_INFO *info)
{
  uchar *pkt;
  int pkt_len;
  MPVIO_EXT *mpvio= (MPVIO_EXT *) vio;
  ...
  */
  ...
  if (myblockchain_native_password_proxy_users)
  {
    *info->authenticated_as= PROXY_FLAG;
	DBUG_PRINT("info", ("myblockchain_native_authentication_proxy_users is enabled, setting authenticated_as to NULL"));
  }
...
/**
  Interface for querying the MYBLOCKCHAIN_PUBLIC_VIO about encryption state.
 
*/

int my_vio_is_encrypted(MYBLOCKCHAIN_PLUGIN_VIO *vio)
{
  MPVIO_EXT *mpvio= (MPVIO_EXT *) vio;
...
int init_sha256_password_handler(MYBLOCKCHAIN_PLUGIN plugin_ref)
{
...
void static inline auth_save_scramble(MYBLOCKCHAIN_PLUGIN_VIO *vio, const char *scramble)
{
  MPVIO_EXT *mpvio= (MPVIO_EXT *) vio;
...
static int sha256_password_authenticate(MYBLOCKCHAIN_PLUGIN_VIO *vio,
                                        MYBLOCKCHAIN_SERVER_AUTH_INFO *info)
{
  uchar *pkt;
  int pkt_len;
...
http://dev.myblockchain.com/doc/internals/en/connection-phase-packets.html#packet-Protocol::HandshakeV10
  ...
  */
  ...
  if (result == 0)
  {
    ...
    {
      *info->authenticated_as= PROXY_FLAG;
       DBUG_PRINT("info", ("myblockchain_native_authentication_proxy_users is enabled \
						   , setting authenticated_as to NULL"));
    }
    DBUG_RETURN(CR_OK);
  }
...
#if !defined(HAVE_YASSL)
static MYBLOCKCHAIN_SYSVAR_STR(private_key_path, auth_rsa_private_key_path,
        PLUGIN_VAR_READONLY,
        "A fully qualified path to the private RSA key used for authentication",
        NULL, NULL, AUTH_DEFAULT_RSA_PRIVATE_KEY);
static MYBLOCKCHAIN_SYSVAR_STR(public_key_path, auth_rsa_public_key_path,
        PLUGIN_VAR_READONLY,
        "A fully qualified path to the public RSA key used for authentication",
        NULL, NULL, AUTH_DEFAULT_RSA_PUBLIC_KEY);
static MYBLOCKCHAIN_SYSVAR_BOOL(auto_generate_rsa_keys, auth_rsa_auto_generate_rsa_keys,
...
static struct st_myblockchain_sys_var* sha256_password_sysvars[]= {
  MYBLOCKCHAIN_SYSVAR(private_key_path),
  MYBLOCKCHAIN_SYSVAR(public_key_path),
  MYBLOCKCHAIN_SYSVAR(auto_generate_rsa_keys),
  0
};
  ...
  if (opt_auto_generate_certs == true)
  {
    ...
    */
    ...
    else
    {
       ...
       */
      File_creator fcr;
      Sql_string_t ca_name= "MyBlockchain_Server_";
      Sql_string_t server_name= "MyBlockchain_Server_";
      Sql_string_t client_name= "MyBlockchain_Server_";

      ca_name.append(MYBLOCKCHAIN_SERVER_VERSION);
      ca_name.append("_Auto_Generated_CA_Certificate");
      server_name.append(MYBLOCKCHAIN_SERVER_VERSION);
      server_name.append("_Auto_Generated_Server_Certificate");
      client_name.append(MYBLOCKCHAIN_SERVER_VERSION);
      ...
      */
      ...
      {
        ca_name.clear();
        ca_name.append("MyBlockchain_Server_Auto_Generated_CA_Certificate");
        server_name.clear();
        server_name.append("MyBlockchain_Server_Auto_Generated_Server_Certificate");
        client_name.clear();
        client_name.append("MyBlockchain_Server_Auto_Generated_Client_Certificate");
      }
...
static struct st_myblockchain_auth native_password_handler=
{
...
#if defined(HAVE_OPENSSL)
static struct st_myblockchain_auth sha256_password_handler=
{
  MYBLOCKCHAIN_AUTHENTICATION_INTERFACE_VERSION,
...
myblockchain_declare_plugin(myblockchain_password)
{
  MYBLOCKCHAIN_AUTHENTICATION_PLUGIN,                  /* type constant    */
  &native_password_handler,                     /* type descriptor  */
  native_password_plugin_name.str,              /* Name             */
  "R.J.Silk, Sergei Golubchik",                 /* Author           */
  "Native MyBlockchain authentication",                /* Description      */
