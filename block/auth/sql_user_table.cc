...
static const
TABLE_FIELD_TYPE myblockchain_db_table_fields[MYBLOCKCHAIN_DB_FIELD_COUNT] = {
  {
...
const TABLE_FIELD_DEF
  myblockchain_db_table_def= {MYBLOCKCHAIN_DB_FIELD_COUNT, myblockchain_db_table_fields};
...
ulong get_access(TABLE *form, uint fieldnr, uint *next_field)
{
  ...
  for (pos=form->field+fieldnr, bit=1;
       *pos && (*pos)->real_type() == MYBLOCKCHAIN_TYPE_ENUM &&
...
/**
  ...
  @note We assume that we have only read from the tables so commit
        can't fail. @sa close_myblockchain_tables().
...
bool acl_end_trans_and_close_tables(THD *thd, bool rollback_transaction)
{
  ...
  /*
    ...
    end of myblockchain_execute_command() in the presence of error, leaving on-disk
...
/**
  Update SSL properties in myblockchain.user table.

  @param thd          Thread context
  @param tables       table points to myblockchain.user table.
...
void update_ssl_properties(THD *thd, TABLE *table)
{
  ...
  {
    case SSL_TYPE_ANY:
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_TYPE]->store(STRING_WITH_LEN("ANY"),
                                      &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_CIPHER]->
        store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_X509_ISSUER]->store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_X509_SUBJECT]->store("", 0, &my_charset_latin1);
      break;
    case SSL_TYPE_X509:
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_TYPE]->store(STRING_WITH_LEN("X509"),
                                      &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_CIPHER]->
        store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_X509_ISSUER]->store("", 0, &my_charset_latin1);
    ...
    case SSL_TYPE_SPECIFIED:
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_TYPE]->store(STRING_WITH_LEN("SPECIFIED"),
                                      &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_CIPHER]->store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_X509_ISSUER]->store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_X509_SUBJECT]->store("", 0, &my_charset_latin1);
      if (lex->ssl_cipher)
        table->field[MYBLOCKCHAIN_USER_FIELD_SSL_CIPHER]->store(lex->ssl_cipher,
                                strlen(lex->ssl_cipher), system_charset_info);
      if (lex->x509_issuer)
        table->field[MYBLOCKCHAIN_USER_FIELD_X509_ISSUER]->store(lex->x509_issuer,
                                strlen(lex->x509_issuer), system_charset_info);
      if (lex->x509_subject)
        table->field[MYBLOCKCHAIN_USER_FIELD_X509_SUBJECT]->store(lex->x509_subject,
                                strlen(lex->x509_subject), system_charset_info);
      break;
    case SSL_TYPE_NOT_SPECIFIED:
      break;
    case SSL_TYPE_NONE:
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_TYPE]->store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_SSL_CIPHER]->store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_X509_ISSUER]->store("", 0, &my_charset_latin1);
      table->field[MYBLOCKCHAIN_USER_FIELD_X509_SUBJECT]->store("", 0, &my_charset_latin1);
      break;
...
/**
  Update user resources in myblockchain.user table.
...
void update_user_resource(TABLE *table, USER_RESOURCES *mqh)
{
  if (mqh->specified_limits & USER_RESOURCES::QUERIES_PER_HOUR)
    table->field[MYBLOCKCHAIN_USER_FIELD_MAX_QUESTIONS]->
      store((longlong) mqh->questions, TRUE);
  if (mqh->specified_limits & USER_RESOURCES::UPDATES_PER_HOUR)
    table->field[MYBLOCKCHAIN_USER_FIELD_MAX_UPDATES]->
      store((longlong) mqh->updates, TRUE);
  if (mqh->specified_limits & USER_RESOURCES::CONNECTIONS_PER_HOUR)
    table->field[MYBLOCKCHAIN_USER_FIELD_MAX_CONNECTIONS]->
      store((longlong) mqh->conn_per_hour, TRUE);
  if (table->s->fields >= 36 &&
      (mqh->specified_limits & USER_RESOURCES::USER_CONNECTIONS))
    table->field[MYBLOCKCHAIN_USER_FIELD_MAX_USER_CONNECTIONS]->
                       ...
                       bool can_create_user, ulong what_to_replace)
{
  ...
  myblockchain_mutex_assert_owner(&acl_cache->lock);
  ...
  table->field[MYBLOCKCHAIN_USER_FIELD_HOST]->store(combo->host.str,combo->host.length,
                                             system_charset_info);
  table->field[MYBLOCKCHAIN_USER_FIELD_USER]->store(combo->user.str,combo->user.length,
                                             system_charset_info);
  ...
  else // if (table->file->ha_index_read_idx_map [..]
  {
    ...
    old_plugin.str=
      get_field(thd->mem_root, table->field[MYBLOCKCHAIN_USER_FIELD_PLUGIN]);
      ...
      (what_to_replace & DEFAULT_AUTH_ATTR && !old_row_exists))
  {
    ...
    {
      table->field[MYBLOCKCHAIN_USER_FIELD_PLUGIN]->
        store(combo->plugin.str, combo->plugin.length, system_charset_info);
      table->field[MYBLOCKCHAIN_USER_FIELD_PLUGIN]->set_notnull();
      table->field[MYBLOCKCHAIN_USER_FIELD_AUTHENTICATION_STRING]->
        store(combo->auth.str, combo->auth.length, &my_charset_utf8_bin);
      table->field[MYBLOCKCHAIN_USER_FIELD_AUTHENTICATION_STRING]->set_notnull();
    }
    else
    {
      my_error(ER_BAD_FIELD_ERROR, MYF(0), "plugin", "myblockchain.user");
      DBUG_RETURN(-1);
    }
    ...
    */
    if (table->s->fields > MYBLOCKCHAIN_USER_FIELD_PASSWORD_LAST_CHANGED)
    {
      if (builtin_plugin)
      {
        /*
        ...
        */
        ...
        table->field[MYBLOCKCHAIN_USER_FIELD_PASSWORD_LAST_CHANGED]->store_timestamp
            (&password_change_timestamp);
        table->field[MYBLOCKCHAIN_USER_FIELD_PASSWORD_LAST_CHANGED]->set_notnull();
      }
    }
    else
    {
      my_error(ER_BAD_FIELD_ERROR, MYF(0), "password_last_changed", "myblockchain.user");
      DBUG_RETURN(-1);
    }
    /* if we have a password supplied we update the expiration field */
    if (table->s->fields > MYBLOCKCHAIN_USER_FIELD_PASSWORD_EXPIRED)
    {
      if (auth_plugin_supports_expiration(combo->plugin.str))
        table->field[MYBLOCKCHAIN_USER_FIELD_PASSWORD_EXPIRED]->store("N",
                                       1, system_charset_info);
    }
    else
    {
      my_error(ER_BAD_FIELD_ERROR, MYF(0), "password_expired", "myblockchain.user");
      DBUG_RETURN(-1);
    }
  }
  ...
  if (what_to_replace & ACCESS_RIGHTS_ATTR)
  {
    ...
    for (tmp_field= table->field+2, priv = SELECT_ACL;
         *tmp_field && (*tmp_field)->real_type() == MYBLOCKCHAIN_TYPE_ENUM &&
  ...
  rights= get_access(table, MYBLOCKCHAIN_USER_FIELD_SELECT_PRIV, &next_field);
  ...
  if (what_to_replace & PASSWORD_EXPIRE_ATTR)
  {
    ...
    */
    if (combo->alter_status.update_password_expired_column)
    {
      if (table->s->fields > MYBLOCKCHAIN_USER_FIELD_PASSWORD_EXPIRED)
      {
        table->field[MYBLOCKCHAIN_USER_FIELD_PASSWORD_EXPIRED]->
    ...
    */
    if (table->s->fields > MYBLOCKCHAIN_USER_FIELD_PASSWORD_LIFETIME &&
        !combo->alter_status.update_password_expired_column)
    {
      if (!combo->alter_status.use_default_password_lifetime)
      {
        table->field[MYBLOCKCHAIN_USER_FIELD_PASSWORD_LIFETIME]->
          store((longlong) combo->alter_status.expire_after_days, TRUE);
        table->field[MYBLOCKCHAIN_USER_FIELD_PASSWORD_LIFETIME]->set_notnull();
      }
      else
        table->field[MYBLOCKCHAIN_USER_FIELD_PASSWORD_LIFETIME]->set_null();
    }
  }
  ...
  if (what_to_replace & ACCOUNT_LOCK_ATTR)
  {
    ...
    {
      if (table->s->fields > MYBLOCKCHAIN_USER_FIELD_ACCOUNT_LOCKED)
      {
        /*
          Update the field for a new row and for the row that exists and the
          update was enforced (ACCOUNT [UNLOCK|LOCK]).
        */
        table->field[MYBLOCKCHAIN_USER_FIELD_ACCOUNT_LOCKED]->
                      store(combo->alter_status.account_locked ? "Y" : "N", 1,
                            system_charset_info);
      }
      else
      {
        my_error(ER_BAD_FIELD_ERROR, MYF(0), "account_locked", "myblockchain.user");
        DBUG_RETURN(-1);
      }
    }
  }
  ...
  {   
    /*
      ...
      use myblockchaind even if he doesn't have any privileges in the user table!
    */
  ...
  if (!error)
  {
    /*
       ...
       structure to MYBLOCKCHAIN_TIME format, to store it in cache.
    */
    MYBLOCKCHAIN_TIME password_change_time;
    ...
    if (builtin_plugin && (update_password || !old_row_exists))
      ...
      password_change_time.time_type= MYBLOCKCHAIN_TIMESTAMP_ERROR;
...
/*
  change grants in the myblockchain.db table
*/
                        ...
                        bool revoke_grant)
{
  ...
  */
  ...
  if (old_row_exists)
  {
    ...
    if (revoke_grant)
    {
      /* column rights are already fixed in myblockchain_table_grant */
...
int open_grant_tables(THD *thd, TABLE_LIST *tables, bool *transactional_tables)
{
  ...
  *transactional_tables= false;
  ...
  tables->init_one_table(C_STRING_WITH_LEN("myblockchain"),
                         C_STRING_WITH_LEN("user"), "user", TL_WRITE);
  (tables+1)->init_one_table(C_STRING_WITH_LEN("myblockchain"),
                             C_STRING_WITH_LEN("db"), "db", TL_WRITE);
  (tables+2)->init_one_table(C_STRING_WITH_LEN("myblockchain"),
                             C_STRING_WITH_LEN("tables_priv"),
                             "tables_priv", TL_WRITE);
  (tables+3)->init_one_table(C_STRING_WITH_LEN("myblockchain"),
                             C_STRING_WITH_LEN("columns_priv"),
                             "columns_priv", TL_WRITE);
  (tables+4)->init_one_table(C_STRING_WITH_LEN("myblockchain"),
                             C_STRING_WITH_LEN("procs_priv"),
                             "procs_priv", TL_WRITE);
  (tables+5)->init_one_table(C_STRING_WITH_LEN("myblockchain"),
  ...
  /*
    ...
    some kind of updates to the myblockchain.% tables.
  */
                       ...
                       LEX_USER *user_from, LEX_USER *user_to)
{
  ...
  table->use_all_columns();
  if (! table_no) // myblockchain.user table
  {
    /*
