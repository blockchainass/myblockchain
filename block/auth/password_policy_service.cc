...
#include <myblockchaind_error.h> /* To get ER_NOT_VALID_PASSWORD */
...
int my_validate_password_policy(const char *password, unsigned int password_len)
{
  ...
  plugin= my_plugin_lock_by_name(0, validate_password_plugin,
                                 MYBLOCKCHAIN_VALIDATE_PASSWORD_PLUGIN);
  if (plugin)
  {
    st_myblockchain_validate_password *password_validate=
                      (st_myblockchain_validate_password *) plugin_decl(plugin)->info;
...
int my_calculate_password_strength(const char *password, unsigned int password_len)
{
  ...
  if (plugin)
  {
    st_myblockchain_validate_password *password_strength=
                      (st_myblockchain_validate_password *) plugin_decl(plugin)->info;
