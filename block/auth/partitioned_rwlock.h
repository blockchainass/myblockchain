...
public:
  ...
  */
  ...
  {
    m_parts= parts;
    if (!(m_locks_array= new (std::nothrow) myblockchain_rwlock_t[m_parts]))
      return true;
    for (uint i= 0 ; i < m_parts; ++i)
      myblockchain_rwlock_init(psi_key, &m_locks_array[i]);
    return false;
  }
  void destroy()
  {
    for (uint i= 0 ; i < m_parts; ++i)
      myblockchain_rwlock_destroy(&m_locks_array[i]);
    delete [] m_locks_array;
  }
  void wrlock()
  {
    for (uint i= 0 ; i < m_parts; ++i)
      myblockchain_rwlock_wrlock(&m_locks_array[i]);
  }
  void wrunlock()
  {
    for (uint i= 0 ; i < m_parts; ++i)
      myblockchain_rwlock_unlock(&m_locks_array[i]);
  }
  void rdlock(uint thread_id)
  {
    myblockchain_rwlock_rdlock(&m_locks_array[thread_id%m_parts]);
  }
  /*
  ...
  */
  ...
  {
    myblockchain_rwlock_unlock(&m_locks_array[thread_id%m_parts]);
  }

private:
  myblockchain_rwlock_t* m_locks_array;
  uint m_parts;
