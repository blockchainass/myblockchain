                          ...
                          const char *new_password, size_t new_password_len)
{
                     ...
                     sctx->priv_host().str)))
  {
    ...
    if (check_access(thd, UPDATE_ACL, "myblockchain", NULL, NULL, 1, 0))
      return(1);
  }
  if (!thd->slave_thread &&
...
bool myblockchain_show_create_user(THD *thd, LEX_USER *user_name)
{
  int error= 0;
  ACL_USER *acl_user;
  LEX *lex= thd->lex;
  ...
  DBUG_ENTER("myblockchain_show_create_user");
  ...
  myblockchain_rewrite_create_alter_user(thd, &sql_text);
...
/**
   ...
   3. Identify what all fields needs to be updated in myblockchain.user
  ...
  @param is_privileged_user     Whether caller has CREATE_USER_ACL
                                or UPDATE_ACL over myblockchain.*
      ...
      (what_to_set & PLUGIN_ATTR))
  {
    ...
    if (!(auth->authentication_flags & AUTH_FLAG_USES_INTERNAL_STORAGE))
    {
      if (thd->lex->sql_command == SQLCOM_SET_OPTION)
      {
        ...
        */
        char warning_buffer[MYBLOCKCHAIN_ERRMSG_SIZE];
        ...
        warning_buffer[MYBLOCKCHAIN_ERRMSG_SIZE-1]= '\0';
                     ...
                     char *new_password)
{
  ...
  tables.init_one_table("myblockchain", 5, "user", 4, "user", TL_WRITE);
...
bool myblockchain_create_user(THD *thd, List <LEX_USER> &list, bool if_not_exists)
{
  ...
  DBUG_ENTER("myblockchain_create_user");
  ...
  */
  ...
  if (some_users_created || if_not_exists)
  {
    ...
    rlb->mem_free();
    myblockchain_rewrite_create_alter_user(thd, rlb);
...
bool myblockchain_drop_user(THD *thd, List <LEX_USER> &list, bool if_exists)
{
  ...
  bool transactional_tables;
  bool rollback_whole_statement= false;
  DBUG_ENTER("myblockchain_drop_user");
...
bool myblockchain_rename_user(THD *thd, List <LEX_USER> &list)
{
  ...
  bool transactional_tables;
  bool rollback_whole_statement= false;
  DBUG_ENTER("myblockchain_rename_user");
...
bool myblockchain_alter_user(THD *thd, List <LEX_USER> &list, bool if_exists)
{
  ...
  DBUG_ENTER("myblockchain_alter_user");
  ...
  */
  ...
  while ((tmp_user_from= user_list++))
  {
    ...
    */
    ...
    /* update the myblockchain.user table */
