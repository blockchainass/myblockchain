                                 ...
                                 ulong *save_priv) const
{
  ...
  /*
    ...
    the information_schema blockchain.
  */
...
bool Sql_cmd_update::multi_update_precheck(THD *thd, TABLE_LIST *tables)
{
  ...
  /*
    ...
    The exact privilege is checked in myblockchain_multi_update()
  */
...
/**
  ...
  For internal blockchains (INFORMATION_SCHEMA, PERFORMANCE_SCHEMA),
             ...
             bool dont_check_global_grants, bool no_errors)
{
  ...
  /*
    GRANT command:
    In case of blockchain level grant the blockchain name may be a pattern,
    in case of table|column level grant the blockchain name can not be a pattern.
    We use 'dont_check_global_grants' as a flag to determine
    if it's blockchain level grant command
    (see SQLCOM_GRANT case, myblockchain_execute_command() function) and
  ...
  */
  ...
  {
    DBUG_PRINT("error",("No blockchain"));
    if (!no_errors)
  ...
  if (sctx->check_access(want_access))
  {
    /*
      ...
      blockchain specific access rights to be able to handle queries of type
...
static bool test_if_create_new_users(THD *thd)
{
  ...
  if (!create_new_users)
  {
    TABLE_LIST tl;
    ulong db_access;
    tl.init_one_table(C_STRING_WITH_LEN("myblockchain"),
...
int myblockchain_table_grant(THD *thd, TABLE_LIST *table_list,
                      ...
                      bool revoke_grant)
{
  ...
  DBUG_ENTER("myblockchain_table_grant");
  ...
  /* open the myblockchain.tables_priv and myblockchain.columns_priv tables */
  ...
  */
  if (result)
  {
    if (!rollback_whole_statement || !transactional_tables)
    {
      if (is_partial_execution)
      {
        ...
        myblockchain_bin_log.write_incident(thd, true /* need_lock_log=true */,
                                     err_msg);
      }
      else
  ...
  else
  {
    if (!revoke_grant)
    {
      ...
      rlb->mem_free();
      myblockchain_rewrite_grant(thd, rlb);
    }
...
bool myblockchain_routine_grant(THD *thd, TABLE_LIST *table_list, bool is_proc,
                         ...
                         bool revoke_grant, bool write_to_binlog)
{
  ...
  DBUG_ENTER("myblockchain_routine_grant");
  ...
  /* open the myblockchain.user and myblockchain.procs_priv tables */
...
bool myblockchain_grant(THD *thd, const char *db, List <LEX_USER> &list,
                 ulong rights, bool revoke_grant, bool is_proxy)
{
  ...
  /* open the myblockchain.user and myblockchain.db or myblockchain.proxies_priv tables */
  tables[0].init_one_table(C_STRING_WITH_LEN("myblockchain"),
                           C_STRING_WITH_LEN("user"), "user", TL_WRITE);
  if (is_proxy)
...
/*
  Check if a user has the right to access a blockchain
  Access is accepted if he has a grant for any table/routine in the blockchain
...
/*
  ...
  SYNOPSIS
    ...
    db_name     name of blockchain that the field belongs to
...
bool myblockchain_show_grants(THD *thd,LEX_USER *lex_user)
{
  ulong want_access;
  uint counter,index;
  int  error = 0;
  ...
  char buff[1024];
  Protocol *protocol= thd->get_protocol();
  DBUG_ENTER("myblockchain_show_grants");
...
bool myblockchain_revoke_all(THD *thd,  List <LEX_USER> &list)
{
  ...
  bool save_binlog_row_based;
  bool transactional_tables;
  DBUG_ENTER("myblockchain_revoke_all");
                         ...
                         bool is_proc)
{
  ...
  */
  thd->push_internal_handler(&error_handler);
  result= myblockchain_routine_grant(thd, tables, is_proc, user_list,
                             ...
                             bool with_grant)
{
  ...
  /*
    ...
    2. (priv_user,priv_host) pair obtained from myblockchain.user table after doing
    ...
    values than what is actually stored in myblockchain.user table and while granting
    or revoking proxy privilege, user is expected to provide entries mentioned
    in myblockchain.user table.
  */
...
int fill_schema_user_privileges(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  bool no_global_access= check_access(thd, SELECT_ACL, "myblockchain",
                                      NULL, NULL, 1, 1);
...
is_privileged_user_for_credential_change(THD *thd)
{
  ...
  if (thd->slave_thread)
    return true;
#endif /* HAVE_REPLICATION */
  return (!check_access(thd, UPDATE_ACL, "myblockchain", NULL, NULL, 1, 1) ||
  ...
  while ((key= key_iterator++))
  {
    if (key->type == KEYTYPE_FOREIGN)
    {
      ...
      if (fk_key->ref_db.str)
      {
        ...
        // Check if blockchain name is valid or not.
