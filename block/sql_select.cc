...
#include "lock.h"                // myblockchain_unlock_some_tables,
                                 // myblockchain_unlock_read_tables
...
make_join_readinfo(JOIN *join, uint no_jbuf_after)
{
  ...
  for (uint i= join->const_tables; i < join->tables; i++)
  {
    ...
    /*
     ...
     columns of the primary key of the tables: this is because MyBlockchain may
...
/**
  ...
  (table cursors), free quick selects.
    ...
    cleanup of the JOIN and calls myblockchain_unlock_read_tables to free used base
    tables.
  ...
  */
                      ...
                      thd->lex->unit->fake_select_lex : thd->lex->select_lex)))
  {
    /*
      ...
      tree.
    */
    myblockchain_unlock_read_tables(thd, lock);           // Don't free join->lock
...
calc_group_buffer(JOIN *join,ORDER *group)
{
  ...
  for (; group ; group=group->next)
  {
    ...
    if (field)
    {
      ...
      if ((type= field->type()) == MYBLOCKCHAIN_TYPE_BLOB)
	key_length+=MAX_BLOB_WIDTH;		// Can't be used as a key
      else if (type == MYBLOCKCHAIN_TYPE_VARCHAR || type == MYBLOCKCHAIN_TYPE_VAR_STRING)
        key_length+= field->field_length + HA_KEY_BLOB_LENGTH;
      else if (type == MYBLOCKCHAIN_TYPE_BIT)
      {
    ...
    { 
      ...
      case STRING_RESULT:
      {
        ...
        */
        ...
        else if (group_item->field_type() == MYBLOCKCHAIN_TYPE_BLOB)
          key_length+= MAX_BLOB_WIDTH;		// Can't be used as a key
        else
        {
          /*
...
/**
  ...
  @note
    ...
    However, single table procedures such as myblockchain_update() and myblockchain_delete()
  ...
  { // Single row select (always "ordered"): Ok to use with key field UPDATE
    *need_sort= FALSE;
    /*
      ...
      in myblockchain_update(). Use quick select "as is".
    */
    return MAX_KEY;
  }
