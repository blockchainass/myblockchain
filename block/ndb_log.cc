...
// need to pass in a MYBLOCKCHAIN_PLUGIN pointer. Presumably 'my_plugin_log_service'
// can be extended with a my_log_message(level, prefix, message, ...) function
#include "log.h"


/*
  Print message to MyBlockchain Server's error log(s)
              ...
              const char* prefix, const char* fmt, va_list args)
{
  ...
  // Print message to MyBlockchain error log
  switch (loglevel)
  {
