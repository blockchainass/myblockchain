...
*/
static void
adjust_time_range_with_warn(MYBLOCKCHAIN_TIME *ltime, uint8 decimals)
{
...
/*
  Convert seconds to MYBLOCKCHAIN_TIME value with overflow checking.

  SYNOPSIS:
    sec_to_time()
    seconds          number of seconds
    ltime            output MYBLOCKCHAIN_TIME value

  DESCRIPTION
    If the 'seconds' argument is inside MYBLOCKCHAIN_TIME data range, convert it to a
    corresponding value.
...
static bool sec_to_time(lldiv_t seconds, MYBLOCKCHAIN_TIME *ltime)
{
  int warning= 0;
...
/**
  Extract datetime value to MYBLOCKCHAIN_TIME struct from string value
...
static bool extract_date_time(Date_time_format *format,
                              const char *val, size_t length, MYBLOCKCHAIN_TIME *l_time,
...
bool make_date_time(Date_time_format *format, MYBLOCKCHAIN_TIME *l_time,
  ...
  for (; ptr != end ; ptr++)
  {
    ...
    else
    {
      ...
      case 'b':
        ...
        break;
      case 'W':
        if (type == MYBLOCKCHAIN_TIMESTAMP_TIME || !(l_time->month || l_time->year))
          return 1;
      ...
      case 'a':
        ...
        break;
      case 'D':
	if (type == MYBLOCKCHAIN_TIMESTAMP_TIME)
	  return 1;
...
my_decimal *Item_temporal_hybrid_func::val_decimal(my_decimal *decimal_value)
{
  ...
  else if (cached_field_type == MYBLOCKCHAIN_TYPE_DATETIME)
...
bool Item_temporal_hybrid_func::get_date(MYBLOCKCHAIN_TIME *ltime,
...
bool Item_temporal_hybrid_func::get_time(MYBLOCKCHAIN_TIME *ltime)
{
...
String *Item_temporal_hybrid_func::val_str_ascii(String *str)
{
  ...
  if (val_datetime(&ltime, TIME_FUZZY_DATE) ||
      (null_value= my_TIME_to_str(&ltime, str,
                                  cached_field_type == MYBLOCKCHAIN_TYPE_STRING ?
                                  (ltime.second_part ?
  ...
  DBUG_ASSERT((cached_field_type == MYBLOCKCHAIN_TYPE_TIME &&
               ltime.time_type == MYBLOCKCHAIN_TIMESTAMP_TIME) ||
              (cached_field_type == MYBLOCKCHAIN_TYPE_DATE &&
               ltime.time_type == MYBLOCKCHAIN_TIMESTAMP_DATE) ||
              (cached_field_type == MYBLOCKCHAIN_TYPE_DATETIME &&
               ltime.time_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME) ||
               cached_field_type == MYBLOCKCHAIN_TYPE_STRING ||
               ltime.time_type == MYBLOCKCHAIN_TIMESTAMP_NONE);
  return str;
}
...
enum_monotonicity_info Item_func_to_days::get_monotonicity_info() const
{
  ...
  {
    if (args[0]->field_type() == MYBLOCKCHAIN_TYPE_DATE)
      return MONOTONIC_STRICT_INCREASING_NOT_NULL;
    if (args[0]->field_type() == MYBLOCKCHAIN_TYPE_DATETIME)
...
enum_monotonicity_info Item_func_to_seconds::get_monotonicity_info() const
{
  ...
  {
    if (args[0]->field_type() == MYBLOCKCHAIN_TYPE_DATE ||
        args[0]->field_type() == MYBLOCKCHAIN_TYPE_DATETIME)
      return MONOTONIC_STRICT_INCREASING_NOT_NULL;
  }
  return NON_MONOTONIC;
}
...
{
  if (args[0]->type() == Item::FIELD_ITEM &&
      (args[0]->field_type() == MYBLOCKCHAIN_TYPE_DATE ||
       args[0]->field_type() == MYBLOCKCHAIN_TYPE_DATETIME))
...
{
  if (args[0]->type() == Item::FIELD_ITEM &&
      (args[0]->field_type() == MYBLOCKCHAIN_TYPE_TIMESTAMP))
    return MONOTONIC_INCREASING;
  return NON_MONOTONIC;
}
...
bool Item_func_from_days::get_date(MYBLOCKCHAIN_TIME *ltime,
                                   my_time_flags_t fuzzy_date)
{
  longlong value=args[0]->val_int();
  if ((null_value=args[0]->null_value))
    return 1;
  memset(ltime, 0, sizeof(MYBLOCKCHAIN_TIME));
...
void MYBLOCKCHAIN_TIME_cache::set_time(MYBLOCKCHAIN_TIME *ltime, uint8 dec_arg)
{
  DBUG_ASSERT(ltime->time_type == MYBLOCKCHAIN_TIMESTAMP_TIME);
  time= *ltime;
...
void MYBLOCKCHAIN_TIME_cache::set_date(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(ltime->time_type == MYBLOCKCHAIN_TIMESTAMP_DATE);
  time= *ltime;
...
void MYBLOCKCHAIN_TIME_cache::set_datetime(MYBLOCKCHAIN_TIME *ltime, uint8 dec_arg)
{
  DBUG_ASSERT(ltime->time_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME);
  time= *ltime;
...
void MYBLOCKCHAIN_TIME_cache::set_datetime(struct timeval tv, uint8 dec_arg,
                                    Time_zone *tz)
{
...
void MYBLOCKCHAIN_TIME_cache::set_date(struct timeval tv, Time_zone *tz)
{
  tz->gmt_sec_to_TIME(&time, (my_time_t) tv.tv_sec);
  time.time_type= MYBLOCKCHAIN_TIMESTAMP_DATE;
...
void MYBLOCKCHAIN_TIME_cache::set_time(struct timeval tv, uint8 dec_arg,
                                Time_zone *tz)
{
  tz->gmt_sec_to_TIME(&time, tv);
...
void MYBLOCKCHAIN_TIME_cache::cache_string()
{
  DBUG_ASSERT(time.time_type != MYBLOCKCHAIN_TIMESTAMP_NONE);
  if (string_length == 0)
    string_length= my_TIME_to_str(&time, string_buff, decimals());
}


const char *MYBLOCKCHAIN_TIME_cache::cptr()
{
  cache_string();
  return string_buff;
}
...
bool MYBLOCKCHAIN_TIME_cache::get_date(MYBLOCKCHAIN_TIME *ltime,
...
String *MYBLOCKCHAIN_TIME_cache::val_str(String *str)
{
  cache_string();
...
/**
    Converts current time in my_time_t to MYBLOCKCHAIN_TIME represenatation for local
    time zone. Defines time zone (local) used for whole SYSDATE function.
*/
bool Item_func_sysdate_local::get_date(MYBLOCKCHAIN_TIME *now_time,
                                       my_time_flags_t fuzzy_date
...
bool Item_func_sec_to_time::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  ...
  if (my_decimal2lldiv_t(0, val, &seconds))
  {
    ...
    make_truncated_value_warning(ErrConvString(val), MYBLOCKCHAIN_TIMESTAMP_TIME);
    return false;
  }
...
String *Item_func_date_format::val_str(String *str)
{
  ...
  if (!make_date_time(&date_time_format, &l_time,
                      is_time_format ? MYBLOCKCHAIN_TIMESTAMP_TIME :
                                       MYBLOCKCHAIN_TIMESTAMP_DATE,
                      str))
    return str;
...
bool Item_func_from_unixtime::get_date(MYBLOCKCHAIN_TIME *ltime,
				       my_time_flags_t fuzzy_date
                                       __attribute__((unused)))
{
  lldiv_t lld;
...
bool Item_func_convert_tz::get_date(MYBLOCKCHAIN_TIME *ltime,
                                    my_time_flags_t fuzzy_date
                                    __attribute__((unused)))
{
...
void Item_date_add_interval::fix_length_and_dec()
{
  ...
  /*
    ...
    - If first arg is a MYBLOCKCHAIN_TYPE_DATETIME result is MYBLOCKCHAIN_TYPE_DATETIME
    - If first arg is a MYBLOCKCHAIN_TYPE_DATE and the interval type uses hours,
      minutes or seconds then type is MYBLOCKCHAIN_TYPE_DATETIME.
    - Otherwise the result is MYBLOCKCHAIN_TYPE_STRING
      (This is because you can't know if the string contains a DATE, MYBLOCKCHAIN_TIME or
      DATETIME argument)
  */
  ...
  if (arg0_field_type == MYBLOCKCHAIN_TYPE_DATETIME ||
      arg0_field_type == MYBLOCKCHAIN_TYPE_TIMESTAMP)
  {
    uint8 dec= MY_MAX(args[0]->datetime_precision(), interval_dec);
    fix_length_and_dec_and_charset_datetime(MAX_DATETIME_WIDTH, dec);
    cached_field_type= MYBLOCKCHAIN_TYPE_DATETIME;
  }
  else if (arg0_field_type == MYBLOCKCHAIN_TYPE_DATE)
  {
  ...
  else if (arg0_field_type == MYBLOCKCHAIN_TYPE_TIME)
  {
    uint8 dec= MY_MAX(args[0]->time_precision(), interval_dec);
    cached_field_type= MYBLOCKCHAIN_TYPE_TIME;
...
bool Item_date_add_interval::get_date_internal(MYBLOCKCHAIN_TIME *ltime,
                                               my_time_flags_t fuzzy_date)
{
  Interval interval;
  ...
  */
  if (cached_field_type == MYBLOCKCHAIN_TYPE_DATE &&
      ltime->time_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME)
...
bool Item_date_add_interval::get_time_internal(MYBLOCKCHAIN_TIME *ltime)
{
  Interval interval;
...
bool Item_date_add_interval::val_datetime(MYBLOCKCHAIN_TIME *ltime,
                                          my_time_flags_t fuzzy_date)
{
  if (cached_field_type != MYBLOCKCHAIN_TYPE_TIME)
...
bool Item_datetime_typecast::get_date(MYBLOCKCHAIN_TIME *ltime,
                                      my_time_flags_t fuzzy_date)
{
  if ((null_value= args[0]->get_date(ltime, fuzzy_date | TIME_NO_DATE_FRAC_WARN)))
    return true;
  DBUG_ASSERT(ltime->time_type != MYBLOCKCHAIN_TIMESTAMP_TIME);
  ltime->time_type= MYBLOCKCHAIN_TIMESTAMP_DATETIME; // In case it was DATE
  int warnings= 0;
...
bool Item_time_typecast::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  if (get_arg0_time(ltime))
    return true;
  my_time_round(ltime, decimals);
  /*
    For MYBLOCKCHAIN_TIMESTAMP_TIME value we can have non-zero day part,
    which we should not lose.
  */
  if (ltime->time_type != MYBLOCKCHAIN_TIMESTAMP_TIME)
    datetime_to_time(ltime);
  return false;
}
...
bool Item_date_typecast::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzy_date)
{
...
bool Item_func_makedate::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzy_date)
{
  DBUG_ASSERT(fixed == 1);
...
void Item_func_add_time::fix_length_and_dec()
{
  /*
    ...
    - If first arg is a MYBLOCKCHAIN_TYPE_DATETIME or MYBLOCKCHAIN_TYPE_TIMESTAMP 
      result is MYBLOCKCHAIN_TYPE_DATETIME
    - If first arg is a MYBLOCKCHAIN_TYPE_TIME result is MYBLOCKCHAIN_TYPE_TIME
    - Otherwise the result is MYBLOCKCHAIN_TYPE_STRING
    
    TODO: perhaps it should also return MYBLOCKCHAIN_TYPE_DATETIME
    when the first argument is MYBLOCKCHAIN_TYPE_DATE.
  */
  if (args[0]->field_type() == MYBLOCKCHAIN_TYPE_TIME && !is_date)
  {
    cached_field_type= MYBLOCKCHAIN_TYPE_TIME;
...
bool Item_func_add_time::val_datetime(MYBLOCKCHAIN_TIME *time,
                                      my_time_flags_t fuzzy_date)
{
  DBUG_ASSERT(fixed == 1);
  MYBLOCKCHAIN_TIME l_time1, l_time2;
  bool is_time= 0;
  ...
  null_value=0;
  if (cached_field_type == MYBLOCKCHAIN_TYPE_DATETIME)  // TIMESTAMP function
  {
    if (get_arg0_date(&l_time1, fuzzy_date) || 
        args[1]->get_time(&l_time2) ||
        l_time1.time_type == MYBLOCKCHAIN_TIMESTAMP_TIME || 
        l_time2.time_type != MYBLOCKCHAIN_TIMESTAMP_TIME)
      goto null_date;
  }
  else                                // ADDTIME function
  {
    if (args[0]->get_time(&l_time1) || 
        args[1]->get_time(&l_time2) ||
        l_time2.time_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME)
      goto null_date;
    is_time= (l_time1.time_type == MYBLOCKCHAIN_TIMESTAMP_TIME);
  }
  if (l_time1.neg != l_time2.neg)
    l_sign= -l_sign;
  
  memset(time, 0, sizeof(MYBLOCKCHAIN_TIME));
  ...
  */
  ...
  if (!is_time)
  {
    ...
    time->time_type= MYBLOCKCHAIN_TIMESTAMP_DATETIME;
  ...
  time->time_type= MYBLOCKCHAIN_TIMESTAMP_TIME;
  time->hour+= days*24;
...
bool Item_func_timediff::get_time(MYBLOCKCHAIN_TIME *l_time3)
{
  DBUG_ASSERT(fixed == 1);
  longlong seconds;
  long microseconds;
  ...
  if ((args[0]->is_temporal_with_date() &&
       args[1]->field_type() == MYBLOCKCHAIN_TYPE_TIME) ||
      (args[1]->is_temporal_with_date() &&
       args[0]->field_type() == MYBLOCKCHAIN_TYPE_TIME))
  ...
  /*
    For MYBLOCKCHAIN_TIMESTAMP_TIME only:
      If first argument was negative and diff between arguments
      is non-zero we need to swap sign to get proper result.
  */
  if (l_time1.neg && (seconds || microseconds))
...
bool Item_func_maketime::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  bool overflow= 0;
  ...
  make_truncated_value_warning(ErrConvString(buf, len), MYBLOCKCHAIN_TIMESTAMP_TIME);
  return false;
}
...
{
  MYBLOCKCHAIN_TIME ltime1, ltime2;
  longlong seconds;
  long microseconds;
  long months= 0;
  int neg= 1;
  ...
  case INTERVAL_MICROSECOND:
    /*
      In MyBlockchain difference between any two valid datetime values
...
void Item_func_str_to_date::fix_from_format(const char *format, size_t length)
{
  ...
  for (; val != end && val != end; val++)
  {
	...
	date_part_used= 1;
      if (date_part_used && frac_second_used)
      {
        ...
        */
        cached_timestamp_type= MYBLOCKCHAIN_TIMESTAMP_DATETIME;
        cached_field_type= MYBLOCKCHAIN_TYPE_DATETIME; 
        fix_length_and_dec_and_charset_datetime(MAX_DATETIME_WIDTH,
  ...
  {
    cached_timestamp_type= MYBLOCKCHAIN_TIMESTAMP_TIME;
    cached_field_type= MYBLOCKCHAIN_TYPE_TIME;
  ...
  {
    cached_timestamp_type= MYBLOCKCHAIN_TIMESTAMP_DATE;
    cached_field_type= MYBLOCKCHAIN_TYPE_DATE; 
    fix_length_and_dec_and_charset_datetime(MAX_DATE_WIDTH, 0);
  }
}
...
bool Item_func_str_to_date::val_datetime(MYBLOCKCHAIN_TIME *ltime,
                                         my_time_flags_t fuzzy_date)
{
  Date_time_format date_time_format;
  ...
  if (cached_timestamp_type == MYBLOCKCHAIN_TIMESTAMP_TIME && ltime->day)
  {
    /*
...
bool Item_func_last_day::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzy_date)
{
  ...
  if (ltime->month == 0)
  {
    ...
    */
    ...
    make_truncated_value_warning(str, MYBLOCKCHAIN_TIMESTAMP_ERROR);
    return (null_value= true);
  }
