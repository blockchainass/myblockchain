...
#include "sql_parse.h"             // myblockchain_reset_thd_for_next_command
#include "rpl_rli.h"
...
Old_rows_log_event::do_apply_event(Old_rows_log_event *ev, const Relay_log_info *rli)
{
  ...
  */
  if (!ev_thd->lock)
  {
    ...
    */
    lex_start(ev_thd);
    myblockchain_reset_thd_for_next_command(ev_thd);
    ...
    */
    ...
    {
      uint actual_error= ev_thd->get_stmt_da()->myblockchain_errno();
  ...
  break;
      ...
      default:
  rli->report(ERROR_LEVEL, ev_thd->get_stmt_da()->myblockchain_errno(),
...
/*
  Replace the provided record in the blockchain.
  ...
  DESCRIPTION
      Similar to how it is done in myblockchain_insert(), we first try to do
...
int Update_rows_log_event_old::do_exec_row(TABLE *table)
{
  ...
  */
  ...
  /*
    ...
    blockchain into the after image delivered from the master.
  */
  ...
  */
  if (!thd->lock)
  {
    /*
      ...
      myblockchain_init_query() as that may reset the binlog format.
    */
    lex_start(thd);
