...
#include "my_time.h"                            // MYBLOCKCHAIN_TIME_NOTE_TRUNCATED
...
#include "myblockchaind_error.h"                       // ER_*
#include "sql_error.h"                          // Sql_condition
#include "sql_string.h"                         // String
#include "table.h"                              // TABLE
#include "myblockchain_version.h"                      // FRM_VER
...
{
  if (warn & MYBLOCKCHAIN_TIME_NOTE_TRUNCATED)
    return TYPE_NOTE_TIME_TRUNCATED;

  if (warn & MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE)
    return TYPE_WARN_OUT_OF_RANGE;

  if (warn & MYBLOCKCHAIN_TIME_WARN_TRUNCATED)
    return TYPE_NOTE_TRUNCATED;

  if (warn & (MYBLOCKCHAIN_TIME_WARN_ZERO_DATE | MYBLOCKCHAIN_TIME_WARN_ZERO_IN_DATE))
    return TYPE_ERR_BAD_VALUE;

  if (warn & MYBLOCKCHAIN_TIME_WARN_INVALID_TIMESTAMP)
...
{
  return type == MYBLOCKCHAIN_TYPE_TIMESTAMP || type == MYBLOCKCHAIN_TYPE_TIMESTAMP2 ||
    type == MYBLOCKCHAIN_TYPE_DATETIME || type == MYBLOCKCHAIN_TYPE_DATETIME2;
}
...
inline enum_field_types real_type_to_type(enum_field_types real_type)
{
  ...
  {
  case MYBLOCKCHAIN_TYPE_TIME2:
    return MYBLOCKCHAIN_TYPE_TIME;
  case MYBLOCKCHAIN_TYPE_DATETIME2:
    return MYBLOCKCHAIN_TYPE_DATETIME;
  case MYBLOCKCHAIN_TYPE_TIMESTAMP2:
    return MYBLOCKCHAIN_TYPE_TIMESTAMP;
  case MYBLOCKCHAIN_TYPE_NEWDATE:
    return MYBLOCKCHAIN_TYPE_DATE;
...
public:
  ...
  */
  ...
  Generated_column()
    ...
    field_type(MYBLOCKCHAIN_TYPE_LONG), stored_in_db(false)
  {
    expr_str.str= NULL;
    expr_str.length= 0;
  };
...
private:
  ...
  */
  ...
  bool stored_in_db;             /* Indication that the field is 
                                    phisically stored in the blockchain*/
};
...
public:
  ...
  */
  ...
  /**
    Store MYBLOCKCHAIN_TIME value with the given amount of decimal digits
    into a field.

    Note, the "dec" parameter represents number of digits of the Item
    that previously created the MYBLOCKCHAIN_TIME value. It's needed when we
    ...
    @retval true   on error
  */
  virtual type_conversion_status store_time(MYBLOCKCHAIN_TIME *ltime, uint8 dec);
  /**
    Store MYBLOCKCHAIN_TYPE value into a field when the number of fractional
    ...
    @retval true   on error
  */
  type_conversion_status store_time(MYBLOCKCHAIN_TIME *ltime)
  {
  ...
  */
  longlong val_temporal_by_field_type()
  {
    ...
    if (type() == MYBLOCKCHAIN_TYPE_TIME)
  ...
  */
  ...
  /**
    ...
    to avoid conversion from timestamp to MYBLOCKCHAIN_TIME and back.
  */
  ...
  /**
   ...
   store_time(MYBLOCKCHAIN_TIME*, uint8) does not fail.
  ...
  */
  ...
  virtual enum_field_types binlog_type() const
  {
    /*
      ...
      This puts MYBLOCKCHAIN_TYPE_STRING in case of CHAR, VARCHAR, SET and ENUM,
      with extra data type details put into metadata.

      We cannot store field->type() in case of temporal types with
      fractional seconds: TIME(n), DATETIME(n) and TIMESTAMP(n),
      because binlog records with MYBLOCKCHAIN_TYPE_TIME, MYBLOCKCHAIN_TYPE_DATETIME
      ...
      MYBLOCKCHAIN_TYPE_TIME2, MYBLOCKCHAIN_TYPE_DATETIME2, MYBLOCKCHAIN_TYPE_TIMESTAMP2,
  ...
  */
  ...
  type_conversion_status check_constraints(int myblockchain_errno);
  ...
  */
  ...
  virtual bool get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate);
  virtual bool get_time(MYBLOCKCHAIN_TIME *ltime);
  ...
  */
  ...
  /**
    Get the upper limit of the MyBlockchain integral and floating-point type.
...
public:
  ...
  type_conversion_status store_time(MYBLOCKCHAIN_TIME *ltime, uint8 dec);
...
public:
  Field_decimal(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg,
    ...
    {}
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_DECIMAL;}
  ...
  Field_decimal *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_DECIMAL);
...
public:
  ...
  */
  ...
  Field_new_decimal(uint32 len_arg, bool maybe_null_arg,
                    ...
                    bool unsigned_arg);
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_NEWDECIMAL;}
  ...
  Field_new_decimal *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_NEWDECIMAL);
...
public:
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_TINY;}
  enum ha_base_keytype key_type() const
  ...
  Field_tiny *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_TINY);
...
public:
  ...
  Field_short(uint32 len_arg,bool maybe_null_arg, const char *field_name_arg,
    ...
    {}
  enum Item_result result_type () const { return INT_RESULT; }
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_SHORT;}
  enum ha_base_keytype key_type() const
  ...
  Field_short *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_SHORT);
...
public:
  Field_medium(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg,
    ...
    {}
  enum Item_result result_type () const { return INT_RESULT; }
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_INT24;}
  enum ha_base_keytype key_type() const
  ...
  Field_medium *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_INT24);
...
public:
  ...
  Field_long(uint32 len_arg,bool maybe_null_arg, const char *field_name_arg,
    ...
    {}
  enum Item_result result_type () const { return INT_RESULT; }
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_LONG;}
  enum ha_base_keytype key_type() const
  ...
  Field_long *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_LONG);
...
public:
  ...
  Field_longlong(uint32 len_arg,bool maybe_null_arg,
    ...
    {}
  enum Item_result result_type () const { return INT_RESULT; }
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_LONGLONG;}
  enum ha_base_keytype key_type() const
  ...
  Field_longlong *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_LONGLONG);
...
public:
  ...
  Field_float(uint32 len_arg, bool maybe_null_arg, const char *field_name_arg,
    ...
    {}
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_FLOAT;}
  ...
  Field_float *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_FLOAT);
...
public:
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_DOUBLE;}
  ...
  Field_double *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_DOUBLE);
...
public:
  Field_null(uchar *ptr_arg, uint32 len_arg,
    ...
    {}
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_NULL;}
  type_conversion_status store(const char *to, size_t length,
                               const CHARSET_INFO *cs)
  {
    null[0]= 1;
  ...
  Field_null *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_NULL);
...
protected:
  ...
  */
  ...
  /**
    ...
    @retval     true    In case of error.    
  */
  virtual type_conversion_status store_internal_with_round(MYBLOCKCHAIN_TIME *ltime,
                                                           int *warnings)= 0;
  ...
  */
  ...
  /**
    Convert a string to MYBLOCKCHAIN_TIME, according to the field type.
  ...
  */
  virtual bool convert_str_to_TIME(const char *str, size_t len,
                                   ...
                                   MYBLOCKCHAIN_TIME_STATUS *status)= 0;
  /**
    Convert a number with fractional part with nanosecond precision
    into MYBLOCKCHAIN_TIME, according to the field type. Nanoseconds
  ...
  */
  ...
  /**
    Convert an integer number into MYBLOCKCHAIN_TIME, according to the field type.
  ...
  */
  longlong convert_number_to_datetime(longlong nr, bool unsigned_val,
                                      MYBLOCKCHAIN_TIME *ltime, int *warning);
...
protected:
  /**
    Low level function to get value into MYBLOCKCHAIN_TIME,
    without checking for being valid.
  */
  virtual bool get_date_internal(MYBLOCKCHAIN_TIME *ltime)= 0;

  /**
    Get value into MYBLOCKCHAIN_TIME and check TIME_NO_ZERO_DATE flag.
    @retval   True on error: we get a zero value but flags disallow zero dates.
    @retval   False on success.
  */
  bool get_internal_check_zero(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate);
  ...
  bool convert_str_to_TIME(const char *str, size_t len, const CHARSET_INFO *cs,
                           MYBLOCKCHAIN_TIME *ltime, MYBLOCKCHAIN_TIME_STATUS *status);
  type_conversion_status store_internal_with_round(MYBLOCKCHAIN_TIME *ltime,
                                                   int *warnings);
public:
  /**
    Constructor for Field_temporal
...
protected:
  ...
  */
  ...
  bool convert_TIME_to_timestamp(THD *thd, const MYBLOCKCHAIN_TIME *ltime,
...
protected:
  ...
  type_conversion_status store_internal(const MYBLOCKCHAIN_TIME *ltime, int *error);
  bool get_date_internal(MYBLOCKCHAIN_TIME *ltime);
...
public:
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_TIMESTAMP;}
  ...
  Field_timestamp *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_TIMESTAMP);
...
public:
  ...
  */
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_TIMESTAMP; }
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_TIMESTAMP2; }
  enum_field_types binlog_type() const { return MYBLOCKCHAIN_TYPE_TIMESTAMP2; }
  bool zero_pack() const { return 0; }
...
public:
  Field_year(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg,
    ...
    {}
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_YEAR;}
  type_conversion_status store(const char *to, size_t length,
                               const CHARSET_INFO *charset);
  type_conversion_status store(double nr);
  type_conversion_status store(longlong nr, bool unsigned_val);
  ...
  Field_year *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_YEAR);
...
public:
  ...
  Field_newdate(bool maybe_null_arg, const char *field_name_arg)
    ...
    { }
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_DATE;}
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_NEWDATE; }
  ...
  {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_DATE);
    DBUG_ASSERT(real_type() == MYBLOCKCHAIN_TYPE_NEWDATE);
...
protected:
  ...
  type_conversion_status convert_number_to_TIME(longlong nr, bool unsigned_val,
                                                ...
                                                int *warning);
  /**
    Low-level function to store MYBLOCKCHAIN_TIME value.
...
public:
  ...
  Field_time(bool maybe_null_arg, const char *field_name_arg)
    ...
    { }
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_TIME;}
  ...
  Field_time *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_TIME);
...
public:
  ...
  */
  ...
  uint decimals() const { return dec; }
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_TIME;}
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_TIME2; }
  enum_field_types binlog_type() const { return MYBLOCKCHAIN_TYPE_TIME2; }
  type_conversion_status store_packed(longlong nr);
  type_conversion_status reset();
  double val_real();
  longlong val_int();
...
public:
  ...
  Field_datetime(bool maybe_null_arg, const char *field_name_arg)
    ...
    {}
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_DATETIME;}
  ...
  {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_DATETIME);
...
public:
  ...
  */
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_DATETIME;}
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_DATETIME2; }
  enum_field_types binlog_type() const { return MYBLOCKCHAIN_TYPE_DATETIME2; }
  uint32 pack_length() const
  {
...
public:
  ...
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_STRING; }
  bool has_charset(void) const
  { return charset() == &my_charset_bin ? FALSE : TRUE; }
  Field *new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type);
  Field_string *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(real_type() == MYBLOCKCHAIN_TYPE_STRING);
...
public:
  ...
  */
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_VARCHAR; }
  bool match_collation_to_optimize_range() const { return true; }
  enum ha_base_keytype key_type() const;
  uint row_pack_length() const { return field_length; }
  bool zero_pack() const { return 0; }
  ...
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_VARCHAR; }
  bool has_charset(void) const
  { return charset() == &my_charset_bin ? FALSE : TRUE; }
  Field *new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type);
  ...
  Field_varstring *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_VARCHAR);
    DBUG_ASSERT(real_type() == MYBLOCKCHAIN_TYPE_VARCHAR);
...
public:
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_BLOB;}
  bool match_collation_to_optimize_range() const { return true; }
  enum ha_base_keytype key_type() const
  ...
  */
  ...
  Field_blob *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_BLOB);
...
public:
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_GEOMETRY; }
  ...
  Field_geom *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_GEOMETRY);
...
public:
  ...
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_JSON; }
  void sql_type(String &str) const;
  /**
  ...
  */
  ...
  type_conversion_status store_time(MYBLOCKCHAIN_TIME *ltime, uint8 dec_arg);
...
public:
  ...
  Field *new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type);
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_STRING; }
  bool match_collation_to_optimize_range() const { return false; }
  enum Item_result cmp_type () const { return INT_RESULT; }
  ...
  void sql_type(String &str) const;
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_ENUM; }
  uint pack_length_from_metadata(uint field_metadata)
  ...
  Field_enum *clone(MEM_ROOT *mem_root) const {
    DBUG_ASSERT(real_type() == MYBLOCKCHAIN_TYPE_ENUM);
...
public:
  ...
  String *val_str(String*,String *);
  void sql_type(String &str) const;
  enum_field_types real_type() const { return MYBLOCKCHAIN_TYPE_SET; }
  bool has_charset(void) const { return TRUE; }
  Field_set *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(real_type() == MYBLOCKCHAIN_TYPE_SET);
...
public:
  ...
  Field_bit(uchar *ptr_arg, uint32 len_arg, uchar *null_ptr_arg,
            ...
            enum utype unireg_check_arg, const char *field_name_arg);
  enum_field_types type() const { return MYBLOCKCHAIN_TYPE_BIT; }
  ...
  Field_bit *clone(MEM_ROOT *mem_root) const { 
    DBUG_ASSERT(type() == MYBLOCKCHAIN_TYPE_BIT);
