...
#include "myblockchain/psi/myblockchain_thread.h"        // myblockchain_rwlock_t
#include "myblockchaind.h"                        // key_memory_delegate
...
public:
  ...
  inline int read_lock()
  {
    if (!inited)
      return TRUE;
    return myblockchain_rwlock_rdlock(&lock);
  }
  ...
  inline int write_lock()
  {
    if (!inited)
      return TRUE;
    return myblockchain_rwlock_wrlock(&lock);
  }
  ...
  inline int unlock()
  {
    if (!inited)
      return TRUE;
    return myblockchain_rwlock_unlock(&lock);
  }
           ...
           )
  {
    inited= FALSE;
#ifdef HAVE_PSI_INTERFACE
    if (myblockchain_rwlock_init(key, &lock))
      return;
#else
    if (myblockchain_rwlock_init(0, &lock))
      return;
#endif
