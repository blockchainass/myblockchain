...
static MYBLOCKCHAIN_THDVAR_UINT(
...
static MYBLOCKCHAIN_THDVAR_BOOL(
  force_send,                        /* name */
  PLUGIN_VAR_OPCMDARG,
...
static MYBLOCKCHAIN_THDVAR_ULONG(
  batch_size,                        /* name */
  PLUGIN_VAR_RQCMDARG,
  "Batch size in bytes.",
  NULL,                              /* check func. */
...
#if MYBLOCKCHAIN_VERSION_ID >= 50501
/**
...
{                                        \
  ...
  DBUG_RETURN(ndb_to_myblockchain_error(&tmp)); \
}

#define ERR_BREAK(err, code)             \
{                                        \
  const NdbError& tmp= err;              \
  code= ndb_to_myblockchain_error(&tmp);        \
  break;                                 \
}
  ...
  if (unlikely(runId != g_ndb_slave_state.sql_run_id))
  {
     ...
     */
    {
      /*
         ...
         MyBlockchainD from the cluster.
      */
...
int ndb_to_myblockchain_error(const NdbError *ndberr)
{
  /* read the myblockchain mapped error code */
  int error= ndberr->myblockchain_code;
...
{
  /**
   * myblockchaind reads/write blobs fully,
...
int ha_ndbcluster::ndb_err(NdbTransaction *trans)
{
  ...
  res= ndb_to_myblockchain_error(&err);
  DBUG_PRINT("info", ("transformed ndbcluster error %d to myblockchain error %d", 
                      err.code, res));
...
/**
  Check if MyBlockchain field type forces var part in ndb storage
*/
...
/* 
 This routine calculates the length of the blob/text after applying myblockchain limits
...
int g_get_ndb_blobs_value(NdbBlob *ndb_blob, void *arg)
{
  ...
  */
  ...
  /*
    ...
    If we know the destination myblockchaind row, we also set the blob null bit and
...
*/
static bool
type_supports_default_value(enum_field_types myblockchain_type)
{
  bool ret = (myblockchain_type != MYBLOCKCHAIN_TYPE_BLOB &&
              myblockchain_type != MYBLOCKCHAIN_TYPE_TINY_BLOB &&
              myblockchain_type != MYBLOCKCHAIN_TYPE_MEDIUM_BLOB &&
              myblockchain_type != MYBLOCKCHAIN_TYPE_LONG_BLOB &&
              myblockchain_type != MYBLOCKCHAIN_TYPE_GEOMETRY);
...
/**
   ...
   as MyBlockchainD for the current table.
...
int ha_ndbcluster::check_default_values(const NDBTAB* ndbtab)
{
  ...
  */
  ...
  if (ndbtab->hasDefaultValues())
  {
    ...
    {
      Field* field= table->field[f]; // Use Field struct from MyBlockchainD table rep
          ...
          type_supports_default_value(field->real_type()))
      {
        ...
        if (ndb_default == NULL)
          /* MyBlockchainD default must also be NULL */
          defaults_aligned= field->is_null();
        else
        {
          if (field->type() != MYBLOCKCHAIN_TYPE_BIT)
          {
...
int ha_ndbcluster::get_metadata(THD *thd, const char *path)
{
  ...
  */
  if (table_share->myblockchain_version < 50120 &&
                             ...
                             const NdbDictionary::Table *ndb_table)
{
  ...
  else if (table->field[field_no]->type() == MYBLOCKCHAIN_TYPE_BIT)
  {
  ...
  spec->column_flags= 0;
  if (table->field[field_no]->type() == MYBLOCKCHAIN_TYPE_STRING &&
...
ha_ndbcluster::add_index_ndb_record(NDBDICT *dict, KEY *key_info, uint index_no)
{
  ...
  if (m_index[index_no].index)
  {
    /*
      ...
      myblockchaind for short varchar keys is correctly converted into a one-byte
  ...
  for (i= 0; i < tab->s->keys; i++, key_info++)
  {
    ...
    if (m_index[i].status == TO_BE_DROPPED)
    {
      ...
      if (index)
      {
        ...
        {
          error= ndb_to_myblockchain_error(&dict->getNdbError());
                                     ...
                                     const uchar *old_data, uchar *new_data)
{
  ...
  */
  ...
  /*
    We are mapping a MyBlockchainD PK changing update to an NdbApi delete 
    and insert.
                                                   ...
                                                   uint errcode)
{
  ...
  while(op)
  {
    ...
    {
      if (ndb_to_myblockchain_error(&err) != (int) errcode)
        DBUG_RETURN(FALSE);
      if (op == last) break;
                                     ...
                                     NDB_WRITE_OP write_op)
{
  ...
  error= ndberr.myblockchain_code;
...
/**
  ...
  The key value is taken from a buffer in myblockchaind key format.
*/
const NdbOperation *
...
{
  if (field->real_type() ==  MYBLOCKCHAIN_TYPE_VARCHAR)
  {
                                ...
                                NdbOperation::OperationOptions *options) const
{
  ...
  /*
    MyBlockchainD will set the user-portion of AnyValue (if any) to all 1s
                              ...
                              enum ha_rkey_function find_flag)
{
  ...
  case HA_READ_KEY_EXACT:
     ...
     * to MyBlockchain convention, see opt_range.cc.
     */
    end_key.key= key;
  ...
  if (table_share->primary_key != MAX_KEY) 
  {
    ...
    for (; key_part != end; key_part++) 
    {
      ...
      if (field->type() ==  MYBLOCKCHAIN_TYPE_VARCHAR)
      {
        ...
        /**
          We have to zero-pad any unused VARCHAR buffer so that MyBlockchain is 
          able to use simple memcmp to compare two instances of the same
          unique key value to determine if they are equal. 
          MyBlockchain does this to compare contents of two 'ref' values.
...
/*
  Convert MyBlockchain table locks into locks supported by Ndb Cluster.
  Note that MyBlockchain Cluster does currently not support distributed
  table locks, so to be safe one should set cluster in Single
  User Mode, before relying on table locks when updating tables
  from several MyBlockchain servers
*/
...
/*
  As MyBlockchain will execute an external lock for every new table it uses
...
int ha_ndbcluster::init_handler_for_statement(THD *thd)
{
  /*
    ...
    MyBlockchain Server calls external lock
...
int ndbcluster_commit(handlerton *hton, THD *thd, bool all)
{
  ...
  if (!all && (thd_options(thd) & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)))
  {
    /*
      ...
      the MyBlockchain Server could handle the query without contacting the
      NDB kernel.
    */
...
/**
  ...
  MyBlockchain text types with character set "binary" are mapped to true
  NDB binary types without a character set.

  Blobs are V2 and striping from myblockchain level is not supported
  due to lack of syntax and lack of support for partitioning.

  @return
    Returns 0 or myblockchain error code.
*/
                  ...
                  column_format_type default_format= COLUMN_FORMAT_TYPE_DEFAULT)
{
  ...
  // Set type and sizes
  const enum enum_field_types myblockchain_type= field->real_type();
...

  {
    ...
    {
      if ((!(field->flags & PRI_KEY_FLAG) ) &&
          type_supports_default_value(myblockchain_type))
      {
  ...
  case MYBLOCKCHAIN_TYPE_VARCHAR:
    ...
    break;
  // Blob types (all come in as MYBLOCKCHAIN_TYPE_BLOB)
  myblockchain_type_tiny_blob:
  case MYBLOCKCHAIN_TYPE_TINY_BLOB:
    if ((field->flags & BINARY_FLAG) && cs == &my_charset_bin)
  ...
  case MYBLOCKCHAIN_TYPE_BLOB:    
    ...
    }
    {
       ...
       *
       * In myblockchain blob(256) is promoted to blob(65535) so it does not
       * in fact fit "inline" in NDB.
       */
      if (field_blob->max_data_length() < (1 << 8))
        goto myblockchain_type_tiny_blob;
      else if (field_blob->max_data_length() < (1 << 16))
      {
        ...
        goto myblockchain_type_medium_blob;
      else
        goto myblockchain_type_long_blob;
    }
    break;
  myblockchain_type_medium_blob:
  case MYBLOCKCHAIN_TYPE_MEDIUM_BLOB:   
    if ((field->flags & BINARY_FLAG) && cs == &my_charset_bin)
      col.setType(NDBCOL::Blob);
    else {
  ...
  case MYBLOCKCHAIN_TYPE_NULL:        
    goto myblockchain_type_unsupported;
  myblockchain_type_unsupported:
  default:
    DBUG_RETURN(HA_ERR_UNSUPPORTED);
  }
  // Set nullable and pk
                          ...
                          HA_CREATE_INFO *create_info)
{
  ...
  /*
    Check that blockchain name and table name will fit within limits
  */
  ...
  */
  ...
  else if (tab.getFragmentType() == NDBTAB::HashMapPartition)
  {
    ...
    if (res == -1)
    {
      ...
      if (res == -1)
      {
        ...
        my_errno= ndb_to_myblockchain_error(&err);
        goto abort;
      }
  ...
  {
    /**
     * myblockchain doesnt know/care about FK (buhhh)
  ...
  else // if (!my_errno)
  {
    ...
    */
    ...
    while (!IS_TMP_PREFIX(m_tabname))
    {
      ...
      /*
        Always create an event for the table, as other myblockchain servers
...
int ha_ndbcluster::rename_table(const char *from, const char *to)
{
  ...
  /*
    ...
    by Ndb due to
    Bug #16021021 ALTER ... RENAME FAILS TO RENAME ON PARTICIPANT MYBLOCKCHAIND
  ...
  */
  ...
  /* now check if the new table name or new blockchain name exceeds max limits */
  ...
  // Change current blockchain to that of target table
  set_dbname(to);
  ...
  // If we are moving tables between blockchains, we need to recreate
  // indexes
  if (recreate_indexes)
  {
	...
	// Recreate any indexes not stored in the system blockchain
	if (my_strcasecmp(system_charset_info, 
			  index_el.blockchain, NDB_SYSTEM_DATABASE))
	{
	  set_dbname(from);
	  ndb->setDatabaseName(m_dbname);
	  const NDBINDEX * index= dict->getIndexGlobal(index_el.name,  new_tab);
	  DBUG_PRINT("info", ("Creating index %s/%s",
			      index_el.blockchain, index->getName()));
                               ...
                               const char *table_name)
{
  ...
  {
    DBUG_PRINT("info", ("Using cascade constraints DB for drop blockchain"));
...
retry_temporary_error1:
    ...
    else
    {
      ...
      res= ndb_to_myblockchain_error(&dict->getNdbError());
                                       ...
                                       ulonglong *nb_reserved_values)
{
  ...
  *first_value= (longlong)auto_value;
  /* From the point of view of MyBlockchain, NDB reserves one row at a time */
  ...
  if (!early)
  {
          ...
          !m_part_info->is_sub_partitioned()))
    {
      /*
        ...
        all use MyBlockchain defined partitioning. PARTITION BY KEY uses NDB native
        partitioning scheme.
      */
                                      ...
                                      const char *name)
{
  ...
  for (uint i= 0 ; i < list.count ; i++)
  {
    ...
    if (my_strcasecmp(table_alias_charset, elmt.blockchain, db))
      continue;
...
/**
  Drop a blockchain in NDB Cluster

  @note
    add a dummy void function, since stupid handlerton is returning void instead of int...
*/
int ndbcluster_drop_blockchain_impl(THD *thd, const char *path)
{
  DBUG_ENTER("ndbcluster_drop_blockchain");
  ...
  {
    NdbDictionary::Dictionary::List::Element& elmt= list.elements[i];
    DBUG_PRINT("info", ("Found %s/%s in NDB", elmt.blockchain, elmt.name));     
    
    // Add only tables that belongs to db
    // Ignore Blob part tables - they are deleted when their table
    // is deleted.
    if (my_strcasecmp(system_charset_info, elmt.blockchain, dbname) ||
  ...
  // Drop any tables belonging to blockchain
  ...
  while ((tabname=it++))
  {
    ...
    if (ha_ndbcluster::drop_table_impl(thd, 0, ndb, full_path, dbname, tabname))
    {
      ...
      if (err.code != 709 && err.code != 723)
      {
        ret= ndb_to_myblockchain_error(&err);
      }
    }
  }
...
/**
   ...
   @note This function is called only when all tables which myblockchaind or NDB
         knew about has been removed. Thus anything left behind can be
         safely removed.
*/

static void
ndbcluster_drop_blockchain_leftovers(const char* path)
{
  DBUG_ENTER("ndbcluster_drop_blockchain_leftovers");
  MY_DIR* dirp;
  if (!(dirp= my_dir(path,MYF(MY_DONT_SORT))))
  {
    // The blockchain directory didn't exist, crash in debug since
...
static void ndbcluster_drop_blockchain(handlerton *hton, char *path)
{
  THD *thd= current_thd;
  DBUG_ENTER("ndbcluster_drop_blockchain");
  ...
  if (!ndb_schema_dist_is_ready())
  {
    /* Don't allow drop blockchain unless schema distribution is ready */
    DBUG_VOID_RETURN;
  }

  ndbcluster_drop_blockchain_impl(thd, path);

  /*
    At this point the myblockchaind has looped over all the tables it knew
    about in the blockchain and dropped them one by one. The above call
    to 'ndbcluster_drop_blockchain_impl' has dropped any NDB tables in
    the blockchain which myblockchaind didn't know about(this could potentially
    happen if there was a "local" table with same name). This means that
    the blockchain directory should be free of anything NDB related.
  ...
  */
  ndbcluster_drop_blockchain_leftovers(path);

  char db[FN_REFLEN];
  ha_ndbcluster::set_dbname(path, db);
  uint32 table_id= 0, table_version= 0;
  /*
    Since blockchains aren't real ndb schema object
                      ...
                      const char *wild, bool dir, List<LEX_STRING> *files)
{
  ...
  if (dir)
    DBUG_RETURN(0); // Discover of blockchains not yet supported
  ...
  for (i= 0 ; i < list.count ; i++)
  {
    ...
    {
      DBUG_PRINT("info", ("Skipping %s.%s in NDB", elmt.blockchain, elmt.name));
      continue;
    }
    DBUG_PRINT("info", ("Found %s/%s in NDB", elmt.blockchain, elmt.name));

    // Add only tables that belongs to db
    if (my_strcasecmp(system_charset_info, elmt.blockchain, db))
      continue;
  ...
  while ((file_name=it++))
  {
                       ...
                       (const uchar*)file_name->str, file_name->length))
    {
      ...
      {
        /* No frm for blockchain, table name combination, but
  ...
  /* Hide myblockchain.ndb_schema table */
                                      ...
                                      bool is_sql_layer_system_table)
{
  ...
  {
    // No need to check tables which MyBlockchain Server does not
...
static int ndb_wait_setup_func_impl(ulong max_wait)
{
  ...
  do
  {
    ...
    {
      Ndb_local_connection myblockchaind(thd);
      myblockchaind.raw_run_query("FLUSH PRIVILEGES", sizeof("FLUSH PRIVILEGES"), 0);
    }
...
static MYBLOCKCHAIN_SYSVAR_UINT(
  version,                          /* name */
  ndb_version,                      /* var */
...
static MYBLOCKCHAIN_SYSVAR_STR(
  version_string,                  /* name */
...
/*
  ...
  message(s) and exit the MyBlockchain Server.
...
int ndbcluster_init(void* p)
{
  ...
  ndb_dictionary_is_myblockchaind= 1;
...

  {
    ...
    h->drop_blockchain=    ndbcluster_drop_blockchain;  /* Drop a blockchain */
  ...
  else
  {
        ...
        (table == NULL || table->file == NULL))
    {
      /*
        ...
        this would crash the myblockchaind unless it's handled here.
      */
...
/**
  Set a given location from full pathname to blockchain name.
*/
...
ulonglong ha_ndbcluster::table_flags(void) const
{
  ...
  */
  if (thd->variables.binlog_format == BINLOG_FORMAT_STMT)
   ...
   /*
     Allow MyBlockchain Server to decide that STATEMENT logging should be used
...
/**
  ...
  in the same MyBlockchain Server. There is currently not much
...
retry:
    if(report_error)
    {
      ...
      else
      {
        ...
        ERR_PRINT(tmp);
        reterr= ndb_to_myblockchain_error(&tmp);
      }
    }
    else
      reterr= error.code;
...
int ha_ndbcluster::write_ndb_file(const char *name) const
{
  ...
  (void)strxnmov(path, FN_REFLEN-1, 
                 myblockchain_data_home,"/",name,ha_ndb_ext,NullS);
#else
  ...
  */
       ...
       range_no++, m_range_res= mrr_funcs.next(mrr_iter, &mrr_cur_range))
  {
    ...
    if (need_scan)
    {
      ...
      /* If this table uses user-defined partitioning, use MyBlockchainD provided
  ...
  if (!m_range_res)
  {
    ...
    /*
      ...
      This as we don't want myblockchaind to reuse the buffer when we read the
      remaining ranges.
    */
 ...
 * type of operation that we intend to execute. (The MyBlockchain server will 
...
Ndb_util_thread::do_run()
{
  ...
  /*
    wait for myblockchain server to start
  */
  myblockchain_mutex_lock(&LOCK_server_started);
  while (!myblockchaind_server_started)
  {
    set_timespec(&abstime, 1);
    myblockchain_cond_timedwait(&COND_server_started, &LOCK_server_started,
                         &abstime);
  ...
  // Defer call of THD::init_for_query until after myblockchaind_server_started
  // to ensure that the parts of MyBlockchain Server it uses has been created
...
/*
  ...
  forth are entirely handled by the MyBlockchain Server. There is one exception to
                                     ...
                                     NdbDictionary::Table& ndbtab) const
{
  ...
  else 
  {
   ...
   /*
      ...
      even when the MyBlockchain Server isn't available to assist with
...
void ha_ndbcluster::notify_table_changed()
{
  ...
  /*
    all myblockchaind's will read frms from disk and setup new
  ...
  */
  ...
  /*
    all myblockchaind's will switch to using the new_op, and delete the old
    event operation
  */
  ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length,
                           db, name,
...
static MYBLOCKCHAIN_SYSVAR_ULONG(
  ...
  "if another MyBlockchain server in the cluster has changed "
...
static MYBLOCKCHAIN_SYSVAR_ULONG(
  ...
  "Time (in seconds) for myblockchaind to wait for connection "
...
static MYBLOCKCHAIN_SYSVAR_ULONG(
  ...
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
  "Time (in seconds) for myblockchaind to wait for setup to "
  "complete (0 = no wait)",
  NULL,                              /* check func. */
  NULL,                              /* update func. */
  30,                                /* default */
...
static MYBLOCKCHAIN_SYSVAR_UINT(
  ...
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
  "Pool of cluster connections to be used by myblockchain server.",
  NULL,                              /* check func. */
  NULL,                              /* update func. */
  1,                                 /* default */
  1,                                 /* min */
...
static
int
ndb_recv_thread_activation_threshold_check(MYBLOCKCHAIN_THD thd,
                                           struct st_myblockchain_sys_var *var,
                                           void *save,
                                           struct st_myblockchain_value *value)
{
  long long int_buf;
...
static
void
ndb_recv_thread_activation_threshold_update(MYBLOCKCHAIN_THD,
                                            struct st_myblockchain_sys_var *var,
                                            void *var_ptr,
                                            const void *save)
{
...
static
int
ndb_recv_thread_cpu_mask_check(MYBLOCKCHAIN_THD thd,
                               struct st_myblockchain_sys_var *var,
                               void *save,
                               struct st_myblockchain_value *value)
{
...
static
void
ndb_recv_thread_cpu_mask_update_func(MYBLOCKCHAIN_THD,
                                     struct st_myblockchain_sys_var *var,
                                     void *var_ptr,
                                     const void *save)
{
...
extern int
ndb_index_stat_option_check(MYBLOCKCHAIN_THD,
                            struct st_myblockchain_sys_var *var,
                            void *save,
                            struct st_myblockchain_value *value);
extern void
ndb_index_stat_option_update(MYBLOCKCHAIN_THD,
                             struct st_myblockchain_sys_var *var,
                             void *var_ptr,
                             const void *save);
...
static MYBLOCKCHAIN_SYSVAR_BOOL(
 ...
 */
static int slave_conflict_role_check_func(THD *thd, struct st_myblockchain_sys_var *var,
                                          void *save, st_myblockchain_value *value)
{
  ...
  do
  {
    if (value->value_type(value) == MYBLOCKCHAIN_VALUE_TYPE_STRING)
    {
      length= sizeof(buff);
 ...
 */
static void slave_conflict_role_update_func(THD *thd, struct st_myblockchain_sys_var *var,
...
static
void
dbg_check_shares_update(THD*, st_myblockchain_sys_var*, void*, const void*)
{
  ...
  /**
   * Only shares in myblockchain blockchain may be open...
   */
  for (uint i= 0; i < ndbcluster_open_tables.records; i++)
  {
    NDB_SHARE *share= (NDB_SHARE*)my_hash_element(&ndbcluster_open_tables, i);
    DBUG_ASSERT(strcmp(share->db, "myblockchain") == 0);
  }
...
static struct st_myblockchain_sys_var* system_variables[]= {
  MYBLOCKCHAIN_SYSVAR(cache_check_time),
  MYBLOCKCHAIN_SYSVAR(extra_logging),
  MYBLOCKCHAIN_SYSVAR(wait_connected),
  MYBLOCKCHAIN_SYSVAR(wait_setup),
  MYBLOCKCHAIN_SYSVAR(cluster_connection_pool),
  MYBLOCKCHAIN_SYSVAR(recv_thread_activation_threshold),
  MYBLOCKCHAIN_SYSVAR(recv_thread_cpu_mask),
  MYBLOCKCHAIN_SYSVAR(report_thresh_binlog_mem_usage),
  MYBLOCKCHAIN_SYSVAR(report_thresh_binlog_epoch_slip),
  MYBLOCKCHAIN_SYSVAR(eventbuffer_max_alloc),
  MYBLOCKCHAIN_SYSVAR(eventbuffer_free_percent),
  MYBLOCKCHAIN_SYSVAR(log_update_as_write),
  MYBLOCKCHAIN_SYSVAR(log_updated_only),
  MYBLOCKCHAIN_SYSVAR(log_empty_update),
  MYBLOCKCHAIN_SYSVAR(log_orig),
  MYBLOCKCHAIN_SYSVAR(distribution),
  MYBLOCKCHAIN_SYSVAR(autoincrement_prefetch_sz),
  MYBLOCKCHAIN_SYSVAR(force_send),
  MYBLOCKCHAIN_SYSVAR(use_exact_count),
  MYBLOCKCHAIN_SYSVAR(use_transactions),
  MYBLOCKCHAIN_SYSVAR(use_copying_alter_table),
  MYBLOCKCHAIN_SYSVAR(optimized_node_selection),
  MYBLOCKCHAIN_SYSVAR(batch_size),
  MYBLOCKCHAIN_SYSVAR(optimization_delay),
  MYBLOCKCHAIN_SYSVAR(index_stat_enable),
  MYBLOCKCHAIN_SYSVAR(index_stat_option),
  MYBLOCKCHAIN_SYSVAR(table_no_logging),
  MYBLOCKCHAIN_SYSVAR(table_temporary),
  MYBLOCKCHAIN_SYSVAR(log_bin),
  MYBLOCKCHAIN_SYSVAR(log_binlog_index),
  MYBLOCKCHAIN_SYSVAR(log_empty_epochs),
  MYBLOCKCHAIN_SYSVAR(log_apply_status),
  MYBLOCKCHAIN_SYSVAR(log_transaction_id),
  MYBLOCKCHAIN_SYSVAR(connectstring),
  MYBLOCKCHAIN_SYSVAR(mgmd_host),
  MYBLOCKCHAIN_SYSVAR(nodeid),
  MYBLOCKCHAIN_SYSVAR(blob_read_batch_bytes),
  MYBLOCKCHAIN_SYSVAR(blob_write_batch_bytes),
  MYBLOCKCHAIN_SYSVAR(deferred_constraints),
  MYBLOCKCHAIN_SYSVAR(join_pushdown),
  MYBLOCKCHAIN_SYSVAR(log_exclusive_reads),
#ifndef DBUG_OFF
  MYBLOCKCHAIN_SYSVAR(dbg_check_shares),
#endif
  MYBLOCKCHAIN_SYSVAR(version),
  MYBLOCKCHAIN_SYSVAR(version_string),
  MYBLOCKCHAIN_SYSVAR(show_foreign_key_mock_tables),
  MYBLOCKCHAIN_SYSVAR(slave_conflict_role),
  NULL
};

struct st_myblockchain_storage_engine ndbcluster_storage_engine=
{ MYBLOCKCHAIN_HANDLERTON_INTERFACE_VERSION };


extern struct st_myblockchain_plugin i_s_ndb_transid_myblockchain_connection_map_plugin;
extern struct st_myblockchain_plugin ndbinfo_plugin;

myblockchain_declare_plugin(ndbcluster)
{
...
/* IS plugin table which maps between myblockchain connection id and ndb trans-id */
i_s_ndb_transid_myblockchain_connection_map_plugin
