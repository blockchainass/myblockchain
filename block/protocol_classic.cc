...
Low level functions for storing data to be send to the MyBlockchain client.
...
/**
  ...
  because column, table, blockchain names fit into this limit.
*/
...
bool net_send_error(THD *thd, uint sql_errno, const char *err)
{
  ...
  */
  ...
  error= net_send_error_packet(thd, sql_errno, err,
    myblockchain_errno_to_sqlstate(sql_errno));
...
bool net_send_error(NET *net, uint sql_errno, const char *err)
{
  ...
  bool error=
    net_send_error_packet(net, sql_errno, err,
                          myblockchain_errno_to_sqlstate(sql_errno), false, 0,
            ...
            bool eof_identifier)
{
  ...
  uchar buff[MYBLOCKCHAIN_ERRMSG_SIZE + 10];
  uchar *pos, *start;
  ...
  */
  ...
  if (protocol->has_client_capability(CLIENT_SESSION_TRACK))
  {
    ...
    {
      store.set_charset(thd->variables.collation_blockchain);

      /*
        First append the fields collected so far. In case of malloc, memory
        for message is also allocated here.
      */
      store.append((const char *)start, (pos - start), MYBLOCKCHAIN_ERRMSG_SIZE);
                           ...
                           const CHARSET_INFO* character_set_results)
{
  uint length;
  /*
    buff[]: sql_errno:2 + ('#':1 + SQLSTATE_LENGTH:5) + MYBLOCKCHAIN_ERRMSG_SIZE:512
  */
  uint error;
  char converted_err[MYBLOCKCHAIN_ERRMSG_SIZE];
  char buff[2+1+SQLSTATE_LENGTH+MYBLOCKCHAIN_ERRMSG_SIZE], *pos;
  ...
  length= (uint) (strmake(pos, converted_err, MYBLOCKCHAIN_ERRMSG_SIZE - 1) - buff);
...
/**
  ...
  uint is used as agrument type because of MyBlockchain type conventions:
    - uint for 0..65536
                                    ...
                                    enum_server_command cmd)
{
  ...
  case COM_SHUTDOWN:
  {
    data->com_shutdown.level= packet_length == 0 ?
      SHUTDOWN_DEFAULT : (enum myblockchain_enum_shutdown_level) raw_packet[0];
    break;
  }
  ...
  {
#ifndef EMBEDDED_LIBRARY
    if (packet_length < MYBLOCKCHAIN_LONG_DATA_HEADER)
      goto malformed;
#endif
                                           ...
                                           const CHARSET_INFO *item_charset)
{
  ...
  if (has_client_capability(CLIENT_PROTOCOL_41))
  {
    ...
    *pos++= 12;        // Length of packed fields
    ...
    else
    {
      ...
      */
      max_length= (field->type >= MYBLOCKCHAIN_TYPE_TINY_BLOB &&
  ...
  field_types[count++]= field->field ? MYBLOCKCHAIN_TYPE_VAR_STRING : field->type;
#endif
  DBUG_RETURN(false);
}
...
/****************************************************************************
  ...
  This protocol is the one that is used by default between the MyBlockchain server
                          ...
                          const CHARSET_INFO *tocs)
{
  ...
  DBUG_ASSERT(send_metadata || field_types == 0 ||
      field_types[field_pos] == MYBLOCKCHAIN_TYPE_DECIMAL ||
      field_types[field_pos] == MYBLOCKCHAIN_TYPE_BIT ||
      field_types[field_pos] == MYBLOCKCHAIN_TYPE_NEWDECIMAL ||
      field_types[field_pos] == MYBLOCKCHAIN_TYPE_NEWDATE ||
      field_types[field_pos] == MYBLOCKCHAIN_TYPE_JSON ||
      (field_types[field_pos] >= MYBLOCKCHAIN_TYPE_ENUM &&
           field_types[field_pos] <= MYBLOCKCHAIN_TYPE_GEOMETRY));
...
bool Protocol_text::store_tiny(longlong from)
{
  ...
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
              field_types[field_pos] == MYBLOCKCHAIN_TYPE_TINY);
  field_pos++;
#endif
  char buff[20];
...
bool Protocol_text::store_short(longlong from)
{
  ...
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_YEAR ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_SHORT);
  field_pos++;
#endif
  char buff[20];
  return net_store_data((uchar *) buff,
...
bool Protocol_text::store_long(longlong from)
{
  ...
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_INT24 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_LONG);
  field_pos++;
#endif
  char buff[20];
  return net_store_data((uchar *) buff,
...
bool Protocol_text::store_longlong(longlong from, bool unsigned_flag)
{
  ...
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_LONGLONG);
  field_pos++;
#endif
  char buff[22];
  return net_store_data((uchar *) buff,
...
bool Protocol_text::store(float from, uint32 decimals, String *buffer)
{
  ...
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_FLOAT);
  field_pos++;
#endif
...
bool Protocol_text::store(double from, uint32 decimals, String *buffer)
{
  ...
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_DOUBLE);
  field_pos++;
#endif
...
bool Protocol_text::store(MYBLOCKCHAIN_TIME *tm, uint decimals)
{
#ifndef DBUG_OFF
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
...
bool Protocol_text::store_date(MYBLOCKCHAIN_TIME *tm)
{
#ifndef DBUG_OFF
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_DATE);
  field_pos++;
#endif
  char buff[MAX_DATE_STRING_REP_LENGTH];
...
bool Protocol_text::store_time(MYBLOCKCHAIN_TIME *tm, uint decimals)
{
#ifndef DBUG_OFF
  // field_types check is needed because of the embedded protocol
  DBUG_ASSERT(send_metadata || field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_TIME);
  field_pos++;
#endif
  char buff[MAX_DATE_STRING_REP_LENGTH];
...
bool Protocol_binary::store_tiny(longlong from)
{
  ...
  DBUG_ASSERT(field_types == 0 ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_TINY ||
    field_types[field_pos] == MYBLOCKCHAIN_TYPE_VAR_STRING);
#endif
  field_pos++;
  buff[0]= (uchar) from;
  return packet->append(buff, sizeof(buff), PACKET_BUFFER_EXTRA_ALLOC);
...
bool Protocol_binary::store(MYBLOCKCHAIN_TIME *tm, uint precision)
{
  if(send_metadata)
...
bool Protocol_binary::store_date(MYBLOCKCHAIN_TIME *tm)
{
  if(send_metadata)
...
bool Protocol_binary::store_time(MYBLOCKCHAIN_TIME *tm, uint precision)
{
  if(send_metadata)
...
{
  return get_vio()->myblockchain_socket.fd;
}
