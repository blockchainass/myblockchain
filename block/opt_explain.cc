...
static bool myblockchain_explain_unit(THD *thd, SELECT_LEX_UNIT *unit);
...
bool Explain::explain_subqueries()
{
       ...
       unit= unit->next_unit())
  {
    ...
    if (myblockchain_explain_unit(thd, unit))
      return true;

    /*
      This must be after myblockchain_explain_unit() so that JOIN::optimize() has run
...
bool explain_query(THD *ethd, SELECT_LEX_UNIT *unit)
{
  ...
  const bool res= myblockchain_explain_unit(ethd, unit);
  /*
  ...
  */
      ...
      query_thd->query_plan.get_command() == SQLCOM_SELECT) // (3)
  {
    ...
    /*
      The warnings system requires input in utf8, see myblockchaind_show_warnings().
    */
...
bool myblockchain_explain_unit(THD *ethd, SELECT_LEX_UNIT *unit)
{
  DBUG_ENTER("myblockchain_explain_unit");
  bool res= false;
  if (unit->is_union())
...
/**
  Callback function used by myblockchain_explain_other() to find thd based
  on the thread id.
...
*/
void myblockchain_explain_other(THD *thd)
{
  bool res= false;
  THD *query_thd= NULL;
  bool send_ok= false;
      ...
      qp->get_command() != SQLCOM_END)
  {
    ...
    */
        ...
        qp->get_lex()->sphead == NULL)                               // (3)
    {
      ...
      myblockchain_mutex_unlock(&query_thd->LOCK_thd_data);
