...
#include "myblockchaind.h"      // select_errors
  ...
  if ((options & REFRESH_BINARY_LOG) || (options & REFRESH_RELAY_LOG ))
  {
    ...
    if (options & REFRESH_BINARY_LOG)
    {
      /*
        ...
        when doing myblockchainbinlog|myblockchain, and anyway it does not really make
      ...
      {
        if (myblockchain_bin_log.rotate_and_purge(thd, true))
          *write_to_binlog= -1;
      }
    }
...
/**
  ...
  myblockchain_execute_command(), it is not reusable in arbitrary
  execution context.
...
bool flush_tables_with_read_lock(THD *thd, TABLE_LIST *all_tables)
{
  ...
  */
  ...
  /*
    ...
    IX and blockchain-scope IX locks on the tables as this will make
  ...
  */
  ...
  /*
    ...
    MYBLOCKCHAIN_OPEN_IGNORE_FLUSH flag to it is important.
    Also we don't pass MYBLOCKCHAIN_OPEN_HAS_MDL_LOCK flag as we want
  ...
  */
  if (open_and_lock_tables(thd, all_tables, MYBLOCKCHAIN_OPEN_SKIP_SCOPED_MDL_LOCK,
