...
#include "sql_partition.h"               // myblockchain_unpack_partition
...
/* MYBLOCKCHAIN_SCHEMA name */
LEX_STRING MYBLOCKCHAIN_SCHEMA_NAME= {C_STRING_WITH_LEN("myblockchain")};
                                         ...
                                         const LEX_STRING &name)
{
  ...
  if ((db.length == MYBLOCKCHAIN_SCHEMA_NAME.length) &&
      (my_strcasecmp(system_charset_info,
                     MYBLOCKCHAIN_SCHEMA_NAME.str,
                     db.str) == 0))
  {
...
/**
  ...
  @param table_list  structure from which blockchain and table 
  ...
  */
                       ...
                       NULL))
  {
    ...
    */
    ...
    myblockchain_mutex_init(key_TABLE_SHARE_LOCK_ha_data,
...
void TABLE_SHARE::destroy()
{
  ...
  if (tmp_table == NO_TMP_TABLE)
    myblockchain_mutex_destroy(&LOCK_ha_data);
...
/**
  ...
  positives. Presumably myblockchain.* blockchain is for internal purposes only
...
inline bool is_system_table_name(const char *name, size_t length)
{
  ...
  return (
           ...
           (length > 4 &&
             (
               /* one of myblockchain.help* tables */
               (my_tolower(ci, name[0]) == 'h' &&
                 my_tolower(ci, name[1]) == 'e' &&
                 my_tolower(ci, name[2]) == 'l' &&
                 my_tolower(ci, name[3]) == 'p') ||

               /* one of myblockchain.time_zone* tables */
               (my_tolower(ci, name[0]) == 't' &&
                 my_tolower(ci, name[1]) == 'i' &&
                 my_tolower(ci, name[2]) == 'm' &&
                 my_tolower(ci, name[3]) == 'e') ||

               /* myblockchain.event table */
...
int open_table_def(THD *thd, TABLE_SHARE *share, uint db_flags)
{
                             ...
                             path, O_RDONLY | O_SHARE, MYF(0))) < 0)
  {
    /*
      ...
      - non-encoded db or table name contain "#myblockchain50#" prefix.
        This kind of tables must have been opened only by the
        myblockchain_file_open() above.
    */
    if (has_disabled_path_chars(share->table_name.str) ||
        has_disabled_path_chars(share->db.str) ||
        !strncmp(share->db.str, MYBLOCKCHAIN50_TABLE_NAME_PREFIX,
                 MYBLOCKCHAIN50_TABLE_NAME_PREFIX_LENGTH) ||
        !strncmp(share->table_name.str, MYBLOCKCHAIN50_TABLE_NAME_PREFIX,
                 MYBLOCKCHAIN50_TABLE_NAME_PREFIX_LENGTH))
      goto err_not_open;
    ...
    */
    ...
    strxnmov(path, sizeof(path)-1,
             myblockchain_data_home, "/", share->db.str, "/",
    ...
    */
    if (length == share->normalized_path.length ||
        ((file= myblockchain_file_open(key_file_frm,
                                path, O_RDONLY | O_SHARE, MYF(0))) < 0))
      goto err_not_open;
  ...
  error= 4;
  if (myblockchain_file_read(file, head, 64, MYF(MY_NABP)))
    goto err;
...
void KEY_PART_INFO::init_flags()
{
  ...
  else if (field->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR)
...
void KEY_PART_INFO::init_from_field(Field *fld)
{
  ...
  if (field->type() == MYBLOCKCHAIN_TYPE_BLOB ||
      field->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR ||
      field->type() == MYBLOCKCHAIN_TYPE_GEOMETRY)
  {
                               ...
                               uint *usable_parts)
{
  ...
  */
  for (uint pk_part= 0; pk_part < pk->user_defined_key_parts; pk_part++)
  {
    ...
    /* MyBlockchain does not supports more key parts than MAX_REF_LENGTH */
    ...
    {
      /* MyBlockchain does not supports keys longer than MAX_KEY_LENGTH */
...
const char *get_tablespace_name(THD *thd, const TABLE_LIST *table)
{
  ...
  File file= myblockchain_file_open(key_file_frm, path, O_RDONLY | O_SHARE, MYF(0));
  if (file < 0)
    return NULL;
  ...
  // header fields.
  uchar head[64];
  if (myblockchain_file_read(file, head, sizeof(head), MYF(MY_NABP)) ||
  ...
  myblockchain_file_seek(file, uint2korr(head + 6), MY_SEEK_SET, MYF(0));
  ...
  myblockchain_file_seek(file, pos, MY_SEEK_SET,MYF(0));
  uchar forminfo[288];
  ...
  if (!myblockchain_file_read(file, forminfo, sizeof(forminfo), MYF(MY_NABP)) &&
      extra_segment_buff &&
      !myblockchain_file_pread(file, extra_segment_buff, n_length,
                               ...
                               int *errarg)
{
  ...
  if (new_frm_ver >= 3)
  {
    ...
    if (field_type == MYBLOCKCHAIN_TYPE_GEOMETRY)
    {
  ...
  if (field_type == MYBLOCKCHAIN_TYPE_NEWDECIMAL && !share->myblockchain_version)
  {
    /*
  ...
  if (field_type == MYBLOCKCHAIN_TYPE_YEAR && field_length != 4)
  {
  ...
  if (field_type == MYBLOCKCHAIN_TYPE_BIT && !f_bit_as_char(pack_flag))
  {
...
/*
  Read data from a binary .frm file from MyBlockchain 3.23 - 5.0 into TABLE_SHARE
                           ...
                           File file)
{
  ...
  myblockchain_file_seek(file,pos,MY_SEEK_SET,MYF(0));
  if (myblockchain_file_read(file, forminfo,288,MYF(MY_NABP)))
    goto err;
  share->frm_version= head[2];
  /*
    Check if .frm file created by MyBlockchain 5.0. In this case we want to
    display CHAR fields as CHAR and not as VARCHAR.
    We do it this way as we want to keep the old frm version to enable
    MyBlockchain 4.1 to read these files.
  */
  ...
  */
  ...
  share->myblockchain_version= uint4korr(head+51);
  ...
  myblockchain_file_seek(file, (ulong) uint2korr(head+6), MY_SEEK_SET, MYF(0));
  if (read_string(file, &disk_buff,key_info_length))
    goto err;                                   /* purecov: inspected */
  if (disk_buff[0] & 0x80)
  {
  ...
  */
  ...
  if (*(head+26) == 1)
    share->system= 1;				/* one-record-blockchain */
  ...
  if ((n_length= uint4korr(head+55)))
  {
    ...
    if (myblockchain_file_pread(file, extra_segment_buff,
	...
	DBUG_ASSERT(is_ha_partition_handlerton(plugin_data<handlerton*>(
               ...
               !strncmp(name.str, "PERFORMANCE_SCHEMA", name.length))
      {
        /*
          ...
          This can happen:
          - (a) during tests with myblockchain-test-run,
            because the same blockchain installed image is used
            for regular builds (with P_S) and embedded builds (without P_S)
          - (b) in production, when random binaries (without P_S) are thrown
            on top of random installed blockchain instances on disk (with P_S).
    ...
    if (share->myblockchain_version >= 50110 && next_chunk < buff_end)
    {
    ...
    if (next_chunk + format_section_header_size < buff_end)
    {
      /*
        ...
        table and column properties introduced by MyBlockchain Cluster
        based on 5.1.20
  ...
  if (myblockchain_file_pread(file, record, (size_t) share->reclength,
                       record_offset, MYF(MY_NABP)))
    goto err;                                   /* purecov: inspected */

  myblockchain_file_seek(file, pos+288, MY_SEEK_SET, MYF(0));
	...
	for (i=0 ; i < keyinfo->user_defined_key_parts ;i++)
	{
          ...
          */
          if (!table_field->real_maybe_null() &&
              table_field->type() == MYBLOCKCHAIN_TYPE_BLOB &&
          ...
          */
          if (!table_field->real_maybe_null() &&
              table_field->type() == MYBLOCKCHAIN_TYPE_GEOMETRY &&
...
bool fix_fields_gcol_func(THD *thd, Field *field)
{
  ...
  /*
    ...
    and real name to table name and get blockchain name from file name.
  */
                          ...
                          TABLE *outparam, bool is_create_table)
{
  ...
  */
    ...
    tmp= myblockchain_unpack_partition(thd, share->partition_info_str,
...
static ulong get_form_pos(File file, uchar *head)
{
  ...
  myblockchain_file_seek(file, 64L, MY_SEEK_SET, MYF(0));

  if (!(buf= (uchar*) my_malloc(key_memory_frm_form_pos,
                                length+names*4, MYF(MY_WME))))
    DBUG_RETURN(0);

  if (myblockchain_file_read(file, buf, length+names*4, MYF(MY_NABP)))
  {
    my_free(buf);
    DBUG_RETURN(0);
  }
...
int read_string(File file, uchar**to, size_t length)
{
  ...
  if (!(*to= (uchar*) my_malloc(key_memory_frm_string,
                                length+1,MYF(MY_WME))) ||
      myblockchain_file_read(file, *to, length, MYF(MY_NABP)))
  {
		     ...
		     const char *newname)
{
  ...
  {						/* Expand file */
    ...
    /* Copy from file-end */
    endpos= (ulong) myblockchain_file_seek(file, 0L, MY_SEEK_END, MYF(0));
    bufflength= (uint) (endpos & (IO_SIZE-1));	/* IO_SIZE is a power of 2 */

    while (endpos > maxlength)
    {
      myblockchain_file_seek(file, (ulong) (endpos-bufflength), MY_SEEK_SET, MYF(0));
      if (myblockchain_file_read(file, buff, bufflength, MYF(MY_NABP+MY_WME)))
	DBUG_RETURN(0L);
      myblockchain_file_seek(file, (ulong) (endpos-bufflength+IO_SIZE), MY_SEEK_SET,
                      MYF(0));
      if ((myblockchain_file_write(file, buff, bufflength, MYF(MY_NABP+MY_WME))))
	DBUG_RETURN(0);
      endpos-=bufflength; bufflength=IO_SIZE;
    }
    memset(buff, 0, IO_SIZE);			/* Null new block */
    myblockchain_file_seek(file, (ulong) maxlength, MY_SEEK_SET, MYF(0));
    if (myblockchain_file_write(file, buff, bufflength, MYF(MY_NABP+MY_WME)))
	DBUG_RETURN(0L);
  ...
  myblockchain_file_seek(file, 63L+(ulong) n_length, MY_SEEK_SET, MYF(0));
  if (myblockchain_file_write(file, buff, (size_t) length+1, MYF(MY_NABP+MY_WME)) ||
      (names && myblockchain_file_write(file,
                                 (uchar*) (*formnames->type_names+n_length-1),
                                 names*4, MYF(MY_NABP+MY_WME))) ||
      myblockchain_file_write(file, fileinfo+10, 4, MYF(MY_NABP+MY_WME)))
    DBUG_RETURN(0L); /* purecov: inspected */

  int2store(fileinfo+8,names+1);
  int2store(fileinfo+4,n_length+length);
  (void) myblockchain_file_chsize(file, newpos, 0, MYF(MY_WME));/* Append file with '\0' */
  DBUG_RETURN(newpos);
} /* make_new_entry */
...
void open_table_error(TABLE_SHARE *share, int error, int db_errno, int errarg)
{
  ...
  case 6:
    ...
    my_printf_error(ER_NOT_FORM_FILE,
                    ...
                    "of MyBlockchain and cannot be read", 
                    MYF(0), buff);
    break;
  case 8:
    break;
  		...
  		HA_CREATE_INFO *create_info, uint keys, KEY *key_info)
{
  ...
  if ((file= myblockchain_file_create(key_file_frm,
                               name, CREATE_MODE, create_flags, MYF(0))) >= 0)
  {
    ...
    */
    ...
    /* fileinfo[26] is set in myblockchain_create_frm() */
    fileinfo[27]=2;				// Use long pack-fields
    /* fileinfo[28 & 29] is set to key_info_length in myblockchain_create_frm() */
    ...
    */
    ...
    tmp= MYBLOCKCHAIN_VERSION_ID;          // Store to avoid warning from int4store
    ...
    */
    ...
    {
      if (myblockchain_file_write(file, fill, IO_SIZE, MYF(MY_WME | MY_NABP)))
      {
        (void) myblockchain_file_close(file, MYF(0));
        (void) myblockchain_file_delete(key_file_frm, name, MYF(0));
	return(-1);
      }
    }
  }
...
rename_file_ext(const char * from,const char * to,const char * ext)
{
  ...
  return (myblockchain_file_rename(key_file_frm, from_b, to_b, MYF(MY_WME)));
}
...
/**
  Check if blockchain name is valid

  @param org_name             Name of blockchain and length
  @param preserve_lettercase  Preserve lettercase if true

  @note If lower_case_table_names is true and preserve_lettercase
  is false then blockchain is converted to lower case
                                                ...
                                                bool preserve_lettercase)
{
  ...
  if ((check_for_path_chars= check_myblockchain50_prefix(name)))
  {
    name+= MYBLOCKCHAIN50_TABLE_NAME_PREFIX_LENGTH;
    name_length-= MYBLOCKCHAIN50_TABLE_NAME_PREFIX_LENGTH;
  }
...
Table_check_intact::check(TABLE *table, const TABLE_FIELD_DEF *table_def)
{
  ...
  if (table->s->fields != table_def->count)
  {
    ...
    /* previous MyBlockchain version */
    if (MYBLOCKCHAIN_VERSION_ID > table->s->myblockchain_version)
    {
      report_error(ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE_V2,
                   ER(ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE_V2),
                   table->s->db.str, table->alias,
                   table_def->count, table->s->fields,
                   static_cast<int>(table->s->myblockchain_version),
                   MYBLOCKCHAIN_VERSION_ID);
      DBUG_RETURN(TRUE);
    }
    else if (MYBLOCKCHAIN_VERSION_ID == table->s->myblockchain_version)
    {
    ...
    /*
      ...
      version of MyBlockchain with new system tables.
                               ...
                               Name_resolution_context *context)
{
  ...
  if (view->schema_table_reformed)
  {
    /*
      ...
      ('myblockchain_schema_table' function). So we can return directly the
  ...
  if (file->ha_table_flags() & HA_PRIMARY_KEY_REQUIRED_FOR_DELETE)
  {
    ...
    */
    if (s->primary_key == MAX_KEY)
    {
      ...
      */
      if (!(myblockchain_bin_log.is_open() && in_use &&
      ...
      case BINLOG_ROW_IMAGE_NOBLOB:
        ...
        for (Field **ptr=field ; *ptr ; ptr++)
        {
          ...
          if ((s->primary_key < MAX_KEY) && 
              ...
              (my_field->type() != MYBLOCKCHAIN_TYPE_BLOB)))
            bitmap_set_bit(read_set, my_field->field_index);

          if (my_field->type() != MYBLOCKCHAIN_TYPE_BLOB)
...
/**
  ...
  @note that this ignores the number of key parts used by MyBlockchain: if the index
  is on (other, vgcol) and MyBlockchain chooses to use it but only its first
  ...
  vgcol's value. So the number of key parts used by MyBlockchain is irrelevant here.
...
bool update_generated_write_fields(TABLE *table)
{
  ...
  for (vfield_ptr= table->vfield; *vfield_ptr; vfield_ptr++)
  {
    ...
    if (bitmap_is_set(table->write_set, vfield->field_index))
    {
      ...
      */
      if (vfield->type() == MYBLOCKCHAIN_TYPE_BLOB && vfield->is_virtual_gcol())
...
void TABLE::mark_gcol_in_maps(Field *field)
{
  ...
  /*
    ...
    properly ignores read_set when MyBlockchain asks for "index only" reads
