...
#include <myblockchain/plugin_validate_password.h>
...
#include "sql_audit.h"         // myblockchain_audit_acquire_plugins
#include "sql_base.h"          // close_myblockchain_tables
...
static PSI_memory_key key_memory_myblockchain_plugin;
static PSI_memory_key key_memory_myblockchain_plugin_dl;
static PSI_memory_key key_memory_plugin_bookmark;

extern st_myblockchain_plugin *myblockchain_optional_plugins[];
extern st_myblockchain_plugin *myblockchain_mandatory_plugins[];
...
*/
const LEX_STRING plugin_type_names[MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM]=
{
...
/*
  ...
  defined.
*/
plugin_type_init plugin_type_initialize[MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM]=
{
  0,ha_initialize_handlerton,0,0,initialize_schema_table,
  initialize_audit_plugin,0,0,0
};

plugin_type_init plugin_type_deinitialize[MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM]=
{
...
static const char *plugin_interface_version_sym=
                   "_myblockchain_plugin_interface_version_";
static const char *sizeof_st_plugin_sym=
                   "_myblockchain_sizeof_struct_st_plugin_";
static const char *plugin_declarations_sym= "_myblockchain_plugin_declarations_";
static int min_plugin_interface_version= MYBLOCKCHAIN_PLUGIN_INTERFACE_VERSION & ~0xFF;
#endif
...
*/
static int min_plugin_info_interface_version[MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM]=
{
  0x0000,
  ...
  MYBLOCKCHAIN_AUDIT_INTERFACE_VERSION,
  MYBLOCKCHAIN_REPLICATION_INTERFACE_VERSION,
  MYBLOCKCHAIN_AUTHENTICATION_INTERFACE_VERSION,
  MYBLOCKCHAIN_VALIDATE_PASSWORD_INTERFACE_VERSION,
  MYBLOCKCHAIN_GROUP_REPLICATION_INTERFACE_VERSION
};
static int cur_plugin_info_interface_version[MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM]=
{
...
/*
  ...
  function. 
*/
myblockchain_mutex_t LOCK_plugin_delete;
...
*/
struct st_item_value_holder : public st_myblockchain_value
{
  Item *item;
};
...
static SHOW_TYPE pluginvar_show_type(st_myblockchain_sys_var *plugin_var);
...
public:
  ...
  st_myblockchain_sys_var *plugin_var;
  /**
  ...
  */
  ...
  sys_var_pluginvar(sys_var_chain *chain, const char *name_arg,
                    st_myblockchain_sys_var *plugin_var_arg)
...
static int test_plugin_options(MEM_ROOT *, st_plugin_int *,
                               int *, char **);
static bool register_builtin(st_myblockchain_plugin *, st_plugin_int *,
                             st_plugin_int **);
static void unlock_variables(THD *thd, struct system_variables *vars);
static void cleanup_variables(THD *thd, struct system_variables *vars);
static void plugin_vars_free_values(sys_var *vars);
static bool plugin_var_memalloc_session_update(THD *thd,
                                               st_myblockchain_sys_var *var,
...
static int item_value_type(st_myblockchain_value *value)
{
  switch (((st_item_value_holder*)value)->item->result_type()) {
  case INT_RESULT:
    return MYBLOCKCHAIN_VALUE_TYPE_INT;
  case REAL_RESULT:
    return MYBLOCKCHAIN_VALUE_TYPE_REAL;
  default:
    return MYBLOCKCHAIN_VALUE_TYPE_STRING;
  }
}

static const char *item_val_str(st_myblockchain_value *value,
...
static int item_val_int(st_myblockchain_value *value, long long *buf)
{
...
static int item_is_unsigned(st_myblockchain_value *value)
{
  Item *item= ((st_item_value_holder*)value)->item;
  return item->unsigned_flag;
}

static int item_val_real(st_myblockchain_value *value, double *buf)
{
  Item *item= ((st_item_value_holder*)value)->item;
  *buf= item->val_real();
  if (item->is_null())
...
#endif
  my_free(p->dl.str);
  if (p->version != MYBLOCKCHAIN_PLUGIN_INTERFACE_VERSION)
    my_free(p->plugins);
}
...
static st_plugin_dl *plugin_dl_add(const LEX_STRING *dl, int report)
{
  ...
  */
  ...
  /* Open new dll handle */
  myblockchain_mutex_assert_owner(&LOCK_plugin);
  ...
  if (plugin_dl.version < min_plugin_interface_version ||
      (plugin_dl.version >> 8) > (MYBLOCKCHAIN_PLUGIN_INTERFACE_VERSION >> 8))
  {
    free_plugin_mem(&plugin_dl);
  ...
  if (plugin_dl.version != MYBLOCKCHAIN_PLUGIN_INTERFACE_VERSION)
  {
    uint sizeof_st_plugin;
    st_myblockchain_plugin *old, *cur;
    char *ptr= (char *)sym;
    ...
    else
    {
      ...
      */
      ...
      sizeof_st_plugin= (int)offsetof(st_myblockchain_plugin, version);
    }
     ...
     */
    for (i= 0;
         ((st_myblockchain_plugin *)(ptr+i*sizeof_st_plugin))->info;
         i++)
      /* no op */;

    cur= (st_myblockchain_plugin*)
      my_malloc(key_memory_myblockchain_plugin,
                (i+1)*sizeof(st_myblockchain_plugin), MYF(MY_ZEROFILL|MY_WME));
    if (!cur)
    {
      free_plugin_mem(&plugin_dl);
    ...
    */
    for (i=0;
         (old=(st_myblockchain_plugin *)(ptr+i*sizeof_st_plugin))->info;
         i++)
      memcpy(cur+i, old, min<size_t>(sizeof(cur[i]), sizeof_st_plugin));

    sym= cur;
  }
  plugin_dl.plugins= (st_myblockchain_plugin *)sym;

  /*
    If report is REPORT_TO_USER, we were called from
    myblockchain_install_plugin. Otherwise, we are called directly or
    indirectly from plugin_init.
   */
  if (report == REPORT_TO_USER)
  {
    st_myblockchain_plugin *plugin= plugin_dl.plugins;
  ...
  if (! (plugin_dl.dl.str= (char*) my_malloc(key_memory_myblockchain_plugin_dl,
                                                  ...
                                                  int type)
{
  ...
  if (type == MYBLOCKCHAIN_ANY_PLUGIN)
  {
    for (i= 0; i < MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM; i++)
    {
...
static SHOW_COMP_OPTION plugin_status(const LEX_CSTRING &name, int type)
{
  ...
  myblockchain_mutex_lock(&LOCK_plugin);
  ...
  myblockchain_mutex_unlock(&LOCK_plugin);
  DBUG_RETURN(rc);
}
                       ...
                       int *argc, char **argv, int report)
{
  ...
  if (plugin_find_internal(name_cstr, MYBLOCKCHAIN_ANY_PLUGIN))
  {
  ...
  for (plugin= tmp.plugin_dl->plugins; plugin->info; plugin++)
  {
    ...
    if (plugin->type >= 0 && plugin->type < MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM &&
                       ...
                       name_len))
    {
      ...
      myblockchain_del_sys_var_chain(tmp.system_vars);
...
static void plugin_deinitialize(st_plugin_int *plugin, bool ref_check)
{
  /*
    ...
    with plugin locks
  */
  myblockchain_mutex_assert_not_owner(&LOCK_plugin);
...
static void plugin_del(st_plugin_int *plugin)
{
  ...
  myblockchain_mutex_assert_owner(&LOCK_plugin);
  myblockchain_mutex_assert_owner(&LOCK_plugin_delete);
  /* Free allocated strings before deleting the plugin. */
  myblockchain_rwlock_wrlock(&LOCK_system_variables_hash);
  myblockchain_del_sys_var_chain(plugin->system_vars);
...
static PSI_memory_info all_plugin_memory[]=
{
  ...
  { &key_memory_myblockchain_plugin_dl, "myblockchain_plugin_dl", 0},
  { &key_memory_myblockchain_plugin, "myblockchain_plugin", 0},
...
static void init_plugin_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, all_plugin_mutexes, count);

  count= array_elements(all_plugin_memory);
  myblockchain_memory_register(category, all_plugin_memory, count);
}
...
{
  uint i;
  st_myblockchain_plugin **builtins;
  st_myblockchain_plugin *plugin;
  ...
  myblockchain_mutex_init(key_LOCK_plugin, &LOCK_plugin, MY_MUTEX_INIT_FAST);
  myblockchain_mutex_init(key_LOCK_plugin_delete, &LOCK_plugin_delete, MY_MUTEX_INIT_FAST);

  plugin_dl_array= new (std::nothrow)
    Prealloced_array<st_plugin_dl*, 16>(key_memory_myblockchain_plugin_dl);
  plugin_array= new (std::nothrow)
    Prealloced_array<st_plugin_int*, 16>(key_memory_myblockchain_plugin);
  ...
  */
  for (builtins= myblockchain_mandatory_plugins; *builtins || mandatory; builtins++)
  {
    if (!*builtins)
    {
      builtins= myblockchain_optional_plugins;
      mandatory= false;
      if (!*builtins)
        break;
    }
    for (plugin= *builtins; plugin->info; plugin++)
    {
      ...
      /*
        ...
        See myblockchaind.cc, set_vars.cc.
...
static bool register_builtin(st_myblockchain_plugin *plugin,
                             st_plugin_int *tmp,
                             st_plugin_int **ptr)
{
...
static void plugin_load(MEM_ROOT *tmp_root, int *argc, char **argv)
{
  ...
  LEX_CSTRING db_lex_cstr= { STRING_WITH_LEN("myblockchain") };
  new_thd->set_db(db_lex_cstr);
  thd.get_protocol_classic()->wipe_net();
  tables.init_one_table("myblockchain", 5, "plugin", 6, "plugin", TL_READ);

#ifdef EMBEDDED_LIBRARY
  /*
    When building an embedded library, if the myblockchain.plugin table
  ...
  */
  ...
  if (result)
  {
    ...
    sql_print_error("Can't open the myblockchain.plugin table. Please "
                    "run myblockchain_upgrade to create it.");
    DBUG_VOID_RETURN;
  }
  table= tables.table;
  ...
  /*
    ...
    environment, and it uses myblockchain_mutex_assert_owner(), so we lock
  ...
  if (initialized)
  {
    ...
    */
    ...
    /*
      ...
      here use myblockchain_mutex_assert_owner(), so we lock the mutex to satisfy it
    */
    myblockchain_mutex_lock(&LOCK_plugin_delete);
    myblockchain_mutex_lock(&LOCK_plugin);
    ...
    */
    ...
    initialized= 0;
    myblockchain_mutex_destroy(&LOCK_plugin);
    myblockchain_mutex_destroy(&LOCK_plugin_delete);
  }
...
static bool myblockchain_install_plugin(THD *thd, const LEX_STRING *name,
                                 const LEX_STRING *dl)
{
  TABLE_LIST tables;
  TABLE *table;
  ...
  DBUG_ENTER("myblockchain_install_plugin");
  ...
  tables.init_one_table("myblockchain", 5, "plugin", 6, "plugin", TL_WRITE);
  if (check_table_access(thd, INSERT_ACL, &tables, FALSE, 1, FALSE))
    DBUG_RETURN(true);

  /* need to open before acquiring LOCK_plugin or it will deadlock */
  if (! (table = open_ltable(thd, &tables, TL_WRITE,
                             MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT)))
    DBUG_RETURN(true);
  ...
  */
  myblockchain_audit_acquire_plugins(thd, MYBLOCKCHAIN_AUDIT_GENERAL_CLASS);

  myblockchain_mutex_lock(&LOCK_plugin);
  DEBUG_SYNC(thd, "acquired_LOCK_plugin");
  myblockchain_rwlock_wrlock(&LOCK_system_variables_hash);

  if (my_load_defaults(MYBLOCKCHAIN_CONFIG_NAME, load_default_groups, &argc, &argv, NULL))
  {
    myblockchain_rwlock_unlock(&LOCK_system_variables_hash);
  ...
  if (error || !(tmp= plugin_find_internal(name_cstr, MYBLOCKCHAIN_ANY_PLUGIN)))
    goto err;
...
static bool myblockchain_uninstall_plugin(THD *thd, const LEX_STRING *name)
{
  TABLE *table;
  TABLE_LIST tables;
  st_plugin_int *plugin;
  ...
  DBUG_ENTER("myblockchain_uninstall_plugin");
  ...
  /* need to open before acquiring LOCK_plugin or it will deadlock */
  if (! (table= open_ltable(thd, &tables, TL_WRITE, MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT)))
    DBUG_RETURN(true);
  ...
  */
  ...
  myblockchain_mutex_lock(&LOCK_plugin);
  if (!(plugin= plugin_find_internal(name_cstr, MYBLOCKCHAIN_ANY_PLUGIN)) ||
                              ...
                              int type, uint state_mask, void *arg)
{
  ...
  myblockchain_mutex_lock(&LOCK_plugin);
  total= type == MYBLOCKCHAIN_ANY_PLUGIN ? plugin_array->size()
...
#undef MYBLOCKCHAIN_SYSVAR_NAME
#define MYBLOCKCHAIN_SYSVAR_NAME(name) name
#define PLUGIN_VAR_TYPEMASK 0x007f

#define EXTRA_OPTIONS 3 /* options for: 'foo', 'plugin-foo' and NULL */

typedef DECLARE_MYBLOCKCHAIN_SYSVAR_BASIC(sysvar_bool_t, my_bool);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_BASIC(thdvar_bool_t, my_bool);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_BASIC(sysvar_str_t, char *);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_BASIC(thdvar_str_t, char *);

typedef DECLARE_MYBLOCKCHAIN_SYSVAR_TYPELIB(sysvar_enum_t, unsigned long);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_TYPELIB(thdvar_enum_t, unsigned long);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_TYPELIB(sysvar_set_t, ulonglong);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_TYPELIB(thdvar_set_t, ulonglong);

typedef DECLARE_MYBLOCKCHAIN_SYSVAR_SIMPLE(sysvar_int_t, int);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_SIMPLE(sysvar_long_t, long);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_SIMPLE(sysvar_longlong_t, longlong);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_SIMPLE(sysvar_uint_t, uint);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_SIMPLE(sysvar_ulong_t, ulong);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_SIMPLE(sysvar_ulonglong_t, ulonglong);
typedef DECLARE_MYBLOCKCHAIN_SYSVAR_SIMPLE(sysvar_double_t, double);

typedef DECLARE_MYBLOCKCHAIN_THDVAR_SIMPLE(thdvar_int_t, int);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_SIMPLE(thdvar_long_t, long);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_SIMPLE(thdvar_longlong_t, longlong);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_SIMPLE(thdvar_uint_t, uint);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_SIMPLE(thdvar_ulong_t, ulong);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_SIMPLE(thdvar_ulonglong_t, ulonglong);
typedef DECLARE_MYBLOCKCHAIN_THDVAR_SIMPLE(thdvar_double_t, double);
...
static int check_func_bool(THD *thd, st_myblockchain_sys_var *var,
...
static int check_func_int(THD *thd, st_myblockchain_sys_var *var,
                          void *save, st_myblockchain_value *value)
{
...
static int check_func_long(THD *thd, st_myblockchain_sys_var *var,
                          void *save, st_myblockchain_value *value)
{
  my_bool fixed1, fixed2;
  long long orig, val;
...
static int check_func_longlong(THD *thd, st_myblockchain_sys_var *var,
                               void *save, st_myblockchain_value *value)
{
  my_bool fixed1, fixed2;
  long long orig, val;
...
static int check_func_str(THD *thd, st_myblockchain_sys_var *var,
                          void *save, st_myblockchain_value *value)
{
  char buff[STRING_BUFFER_USUAL_SIZE];
  const char *str;
...
static int check_func_enum(THD *thd, st_myblockchain_sys_var *var,
                           void *save, st_myblockchain_value *value)
{
  char buff[STRING_BUFFER_USUAL_SIZE];
  const char *str;
...
static int check_func_set(THD *thd, st_myblockchain_sys_var *var,
                          void *save, st_myblockchain_value *value)
{
...
static int check_func_double(THD *thd, st_myblockchain_sys_var *var,
                             void *save, st_myblockchain_value *value)
{
  double v;
  my_bool fixed;
...
static void update_func_bool(THD *thd, st_myblockchain_sys_var *var,
...
static void update_func_int(THD *thd, st_myblockchain_sys_var *var,
                             void *tgt, const void *save)
{
  *(int *)tgt= *(int *) save;
}
...
static void update_func_long(THD *thd, st_myblockchain_sys_var *var,
                             void *tgt, const void *save)
{
  *(long *)tgt= *(long *) save;
}
...
static void update_func_longlong(THD *thd, st_myblockchain_sys_var *var,
                             void *tgt, const void *save)
{
...
static void update_func_str(THD *thd, st_myblockchain_sys_var *var,
                             void *tgt, const void *save)
{
  *(char **) tgt= *(char **) save;
}

static void update_func_double(THD *thd, st_myblockchain_sys_var *var,
                               void *tgt, const void *save)
{
...
void alloc_and_copy_thd_dynamic_variables(THD *thd, bool global_lock)
{
  ...
  myblockchain_mutex_assert_owner(&LOCK_global_system_variables);
  /*
...
static char *myblockchain_sys_var_char(THD* thd, int offset)
{
  return (char *) intern_sys_var_ptr(thd, offset, true);
}

static int *myblockchain_sys_var_int(THD* thd, int offset)
{
  return (int *) intern_sys_var_ptr(thd, offset, true);
}

static long *myblockchain_sys_var_long(THD* thd, int offset)
{
  return (long *) intern_sys_var_ptr(thd, offset, true);
}

static unsigned long *myblockchain_sys_var_ulong(THD* thd, int offset)
{
  return (unsigned long *) intern_sys_var_ptr(thd, offset, true);
}

static long long *myblockchain_sys_var_longlong(THD* thd, int offset)
{
  return (long long *) intern_sys_var_ptr(thd, offset, true);
}

static unsigned long long *myblockchain_sys_var_ulonglong(THD* thd, int offset)
{
  return (unsigned long long *) intern_sys_var_ptr(thd, offset, true);
}

static char **myblockchain_sys_var_str(THD* thd, int offset)
{
  return (char **) intern_sys_var_ptr(thd, offset, true);
}

static double *myblockchain_sys_var_double(THD* thd, int offset)
{
...
static void plugin_opt_set_limits(struct my_option *options,
                                  const st_myblockchain_sys_var *opt)
{
  options->sub_size= 0;
                             ...
                             my_option *options)
{
  ...
  st_myblockchain_sys_var *opt, **plugin_option;
  st_bookmark *v;
  ...
  */
       ...
       plugin_option && *plugin_option; plugin_option++, index++)
  {
    ...
    switch (opt->flags & PLUGIN_VAR_TYPEMASK) {
    case PLUGIN_VAR_BOOL:
      ((thdvar_bool_t *) opt)->resolve= myblockchain_sys_var_char;
      break;
    case PLUGIN_VAR_INT:
      ((thdvar_int_t *) opt)->resolve= myblockchain_sys_var_int;
      break;
    case PLUGIN_VAR_LONG:
      ((thdvar_long_t *) opt)->resolve= myblockchain_sys_var_long;
      break;
    case PLUGIN_VAR_LONGLONG:
      ((thdvar_longlong_t *) opt)->resolve= myblockchain_sys_var_longlong;
      break;
    case PLUGIN_VAR_STR:
      ((thdvar_str_t *) opt)->resolve= myblockchain_sys_var_str;
      break;
    case PLUGIN_VAR_ENUM:
      ((thdvar_enum_t *) opt)->resolve= myblockchain_sys_var_ulong;
      break;
    case PLUGIN_VAR_SET:
      ((thdvar_set_t *) opt)->resolve= myblockchain_sys_var_ulonglong;
      break;
    case PLUGIN_VAR_DOUBLE:
      ((thdvar_double_t *) opt)->resolve= myblockchain_sys_var_double;
      break;
    default:
                               ...
                               int *argc, char **argv)
{
  ...
  */
  ...
  if (chain.first)
  {
    ...
    if (myblockchain_add_sys_var_chain(chain.first))
    {
...
/** 
  ...
  @param type     type of the plugin (0-MYBLOCKCHAIN_MAX_PLUGIN_TYPE_NUM)
...
int lock_plugin_data()
{
  ...
  DBUG_RETURN(myblockchain_mutex_lock(&LOCK_plugin));
}
...
int unlock_plugin_data()
{
  ...
  DBUG_RETURN(myblockchain_mutex_unlock(&LOCK_plugin));
}


bool Sql_cmd_install_plugin::execute(THD *thd)
{
  bool st= myblockchain_install_plugin(thd, &m_comment, &m_ident);
  if (!st)
    my_ok(thd);
  return st;
}
...
{
  bool st= myblockchain_uninstall_plugin(thd, &m_comment);
  if (!st)
    my_ok(thd);
  return st;
}
