...
/* 
 todo: 
   There are no dangerous charsets in myblockchain for function 
...
/*
  ...
  @note
  MYBLOCKCHAINlex remember the following states from the following MYBLOCKCHAINlex():
...
int MYBLOCKCHAINlex(YYSTYPE *yylval, YYLTYPE *yylloc, THD *thd)
{
      ...
      if (lip->yyPeekn(2) == '!')
      {
        ...
        */
           ...
           )
        {
          ...
          if (version <= MYBLOCKCHAIN_VERSION_ID)
          {
            /* Accept 'M' 'm' 'm' 'd' 'd' */
            lip->yySkipn(5);
                              ...
                              TABLE_LIST **end, enum_query_type query_type)
{
  ...
  for (TABLE_LIST **tbl= table + 1; tbl < end; tbl++)
  {
    ...
    if (curr->outer_join)
    {
      /* MyBlockchain converts right to left joins */
...
/**
  @returns whether a blockchain is equal to the connection's default blockchain
*/
...
/**
  ...
  parser before returning an error from MYBLOCKCHAINparse. If your
...
/**
  ...
  blockchain of the stored routine, rather than the default
  blockchain of the connection it is parsed in.
  E.g. if one has no current blockchain selected, or current blockchain 
...
void LEX::set_trg_event_type_for_tables()
{
  ...
  /*
    ...
    refer to myblockchain.* system tables. It is not allowed to have
