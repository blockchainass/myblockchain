...
/*
  ...
  MyBlockchain Server without affecting the current THD's
  settings and status.
...
private:
  bool execute_query(MYBLOCKCHAIN_LEX_STRING sql_text,
                     const unsigned int* ignore_myblockchain_errors,
                     const class Suppressor* suppressor = NULL);
  bool execute_query_iso(MYBLOCKCHAIN_LEX_STRING sql_text,
                         const unsigned int* ignore_myblockchain_errors,
                         const class Suppressor* suppressor = NULL);
private:
  THD* m_thd;
