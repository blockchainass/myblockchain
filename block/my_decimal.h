...
class String;
typedef struct st_myblockchain_time MYBLOCKCHAIN_TIME;
...
/**
  my_decimal class limits 'decimal_t' type to what we need in MyBlockchain.
...
my_decimal *date2my_decimal(const MYBLOCKCHAIN_TIME *ltime, my_decimal *dec);
my_decimal *time2my_decimal(const MYBLOCKCHAIN_TIME *ltime, my_decimal *dec);
my_decimal *timeval2my_decimal(const struct timeval *tm, my_decimal *dec);

#endif /*defined(MYBLOCKCHAIN_SERVER) || defined(EMBEDDED_LIBRARY) */
