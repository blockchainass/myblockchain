...
NDB_SHARE::need_events(bool default_on) const
{
  ...
  if (default_on)
  {
    ...
    DBUG_PRINT("exit", ("need events(the default for this myblockchaind)"));
    DBUG_RETURN(true);
  }
  ...
  {
    // The table is in "myblockchain" blockchain
  ...
  DBUG_PRINT("exit", ("no events(the default for this myblockchaind)"));
  DBUG_RETURN(false);
}
