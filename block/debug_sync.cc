...
/**
  ...
  The facility is an optional part of the MyBlockchain server.
  It is enabled in a debug server by default.

  The Debug Sync Facility, when compiled in, is disabled by default. It
  can be enabled by a myblockchaind command line option:
  ...
  disabled with:
      ...
      myblockchain-test-run.pl ... --debug-sync-timeout=0 ...

  Likewise the default wait timeout can be set:

      myblockchain-test-run.pl ... --debug-sync-timeout=10 ...
  ...
  * Otherwise the value reads as "ON - current signal: " followed by the
  ...
  There are quite a few places in MyBlockchain, where we use a synchronization
  pattern like this:
  ...
  while (!thd->killed && !end_of_wait_condition)
    myblockchain_cond_wait(&condition_variable, &mutex);
  ...
  myblockchain_cond_wait(), the signaling happens at the right place. We
  have a safe synchronization.

  === Co-work with the DBUG facility ===

  When running the MyBlockchain test suite with the --debug command line
  ...
  egrep 'query:|debug_sync_exec:' myblockchain-test/var/log/myblockchaind.1.trace
  ...
  http://forge.myblockchain.com/wiki/MyBlockchain_Internals_Test_Synchronization
...
struct st_debug_sync_globals
{
  ...
  myblockchain_cond_t          ds_cond;                /* condition variable */
  myblockchain_mutex_t         ds_mutex;               /* mutex variable */
  ulonglong             dsp_hits;               /* statistics */
  ulonglong             dsp_executed;           /* statistics */
  ulonglong             dsp_max_active;         /* statistics */
...
/**
  ...
  @description
    ...
    of the MyBlockchain server anyway.
...
static void init_debug_sync_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, all_debug_sync_mutexes, count);

  count= array_elements(all_debug_sync_conds);
  myblockchain_cond_register(category, all_debug_sync_conds, count);

  count= array_elements(all_debug_sync_memory);
  myblockchain_memory_register(category, all_debug_sync_memory, count);
}
...
int debug_sync_init(void)
{
  ...
  if (opt_debug_sync_timeout)
  {
    ...
    if ((rc= myblockchain_cond_init(key_debug_sync_globals_ds_cond,
                             &debug_sync_global.ds_cond)) ||
        (rc= myblockchain_mutex_init(key_debug_sync_globals_ds_mutex,
...
void debug_sync_end(void)
{
  ...
  if (debug_sync_C_callback_ptr)
  {
    ...
    myblockchain_cond_destroy(&debug_sync_global.ds_cond);
    myblockchain_mutex_destroy(&debug_sync_global.ds_mutex);
...
void debug_sync_end_thread(THD *thd)
{
  ...
  if (thd->debug_sync_control)
  {
    ...
    */
    ...
    /* Statistics. */
    myblockchain_mutex_lock(&debug_sync_global.ds_mutex);
    ...
    myblockchain_mutex_unlock(&debug_sync_global.ds_mutex);
  ...
  if (action->execute)
  {
    ...
    */
    ...
    if (action->signal.length())
    {
      ...
      myblockchain_cond_broadcast(&debug_sync_global.ds_cond);
    ...
    if (action->wait_for.length())
    {
      myblockchain_mutex_t *old_mutex;
      myblockchain_cond_t  *old_cond= 0;
      int             error= 0;
      ...
      */
      ...
      {
        error= myblockchain_cond_timedwait(&debug_sync_global.ds_cond,
                                    &debug_sync_global.ds_mutex,
                                    &abstime);
      ...
      */
      ...
      if (old_mutex)
      {
        myblockchain_mutex_lock(&thd->LOCK_current_cond);
        thd->current_mutex= old_mutex;
        thd->current_cond= old_cond;
        myblockchain_mutex_unlock(&thd->LOCK_current_cond);
