...
/*
  ...
  time (my_time_t + offset). Used for local MYBLOCKCHAIN_TIME -> my_time_t conversion.
...
tz_load(const char *name, TIME_ZONE_INFO *sp, MEM_ROOT *storage)
{
  ...
  if (!(file= myblockchain_file_fopen(0, name, O_RDONLY|O_BINARY, MYF(MY_WME))))
    return 1;
  {
    union
    {
    ...
    read_from_file= myblockchain_file_fread(file, u.buf, sizeof(u.buf), MYF(MY_WME));

    if (myblockchain_file_fclose(file, MYF(MY_WME)) != 0)
      return 1;
...
/*
  ...
  DESCRIPTION
    ...
    MYBLOCKCHAIN_TIME -> my_time_t conversion.
    Note: See description of TIME_to_gmt_sec() function first.
    In order to perform MYBLOCKCHAIN_TIME -> my_time_t conversion we need to build table
...
/*
  ...
  DESCRIPTION
    Convert my_time_t with offset to MYBLOCKCHAIN_TIME struct. Differs from timesub
    (from elsie code) because doesn't contain any leap correction and
    TM_GMTOFF and is_dst setting and contains some MyBlockchain specific
    initialization. Funny but with removing of these we almost have
    glibc's offtime function.
*/
static void
sec_to_TIME(MYBLOCKCHAIN_TIME * tmp, my_time_t t, long offset)
{
  long days;
  long rem;
  int y;
  ...
  */
  ...
  /* filling MyBlockchain specific MYBLOCKCHAIN_TIME members */
  tmp->neg= 0; tmp->second_part= 0;
  tmp->time_type= MYBLOCKCHAIN_TIMESTAMP_DATETIME;
}
...
/*
  ...
  broken down MYBLOCKCHAIN_TIME representation in local time zone.
  ...
  TODO
    ...
    can be used fot MYBLOCKCHAIN_TIME -> my_time_t conversion. For this we need to
    implement tweaked binary search which will take into account that some
    MYBLOCKCHAIN_TIME has two matching my_time_t ranges and some of them have none.
*/
static void
gmt_sec_to_TIME(MYBLOCKCHAIN_TIME *tmp, my_time_t sec_in_utc, const TIME_ZONE_INFO *sp)
{
...
/*
  ...
  DESCRIPTION
    ...
    ignore this fact now since MyBlockchain will never pass such argument.
...
/*
  Converts local time in broken down MYBLOCKCHAIN_TIME representation to my_time_t
  representation.
  ...
  DESCRIPTION
    This is mktime analog for MyBlockchain. It is essentially different
    ...
    - It can give wrong results near the ends of my_time_t due to
      overflows, but we are safe since in MyBlockchain we will never
    ...
    look at my_time_t to MYBLOCKCHAIN_TIME conversion then you'll find that it consist
    of two steps:
    ...
    converison would be ambiguos but with MyBlockchain's restrictions we can use
...
*/
static my_time_t
TIME_to_gmt_sec(const MYBLOCKCHAIN_TIME *t, const TIME_ZONE_INFO *sp,
...
public:
  ...
  virtual my_time_t TIME_to_gmt_sec(const MYBLOCKCHAIN_TIME *t,
                                    my_bool *in_dst_time_gap) const;
  virtual void gmt_sec_to_TIME(MYBLOCKCHAIN_TIME *tmp, my_time_t t) const;
  virtual const String * get_name() const;
};


/*
  Converts local time in system time zone in MYBLOCKCHAIN_TIME representation
  to its my_time_t representation.

  SYNOPSIS
    TIME_to_gmt_sec()
      t               - pointer to MYBLOCKCHAIN_TIME structure with local time in
  ...
  DESCRIPTION
    ...
    local time in system time zone in MYBLOCKCHAIN_TIME structure to its my_time_t
...
*/
my_time_t
Time_zone_system::TIME_to_gmt_sec(const MYBLOCKCHAIN_TIME *t, my_bool *in_dst_time_gap) const
{
  long not_used;
...
/*
  ...
  SYNOPSIS
    gmt_sec_to_TIME()
      tmp - pointer to MYBLOCKCHAIN_TIME structure to fill-in
      t   - my_time_t value to be converted

  NOTE
    We assume that value passed to this function will fit into time_t range
    supported by localtime_r. This conversion is putting restriction on
    TIMESTAMP range in MyBlockchain. If we can get rid of SYSTEM time zone at least
    for interaction with client then we can extend TIMESTAMP range down to
    the 1902 easily.
*/
void
Time_zone_system::gmt_sec_to_TIME(MYBLOCKCHAIN_TIME *tmp, my_time_t t) const
{
  struct tm tmp_tm;
...
/*
  ...
  my_time_t -> MYBLOCKCHAIN_TIME conversions in various UTC_...  functions, it is not
  intended for MYBLOCKCHAIN_TIME -> my_time_t conversions and shouldn't be exposed to user.
*/
...
/*
  Convert UTC time from MYBLOCKCHAIN_TIME representation to its my_time_t representation.

  SYNOPSIS
    TIME_to_gmt_sec()
      t               - pointer to MYBLOCKCHAIN_TIME structure with local time
...
*/
my_time_t
Time_zone_utc::TIME_to_gmt_sec(const MYBLOCKCHAIN_TIME *t, my_bool *in_dst_time_gap) const
{
...
*/
void
Time_zone_utc::gmt_sec_to_TIME(MYBLOCKCHAIN_TIME *tmp, my_time_t t) const
{
  struct tm tmp_tm;
  time_t tmp_t= (time_t)t;
...
/*
  ...
  described in myblockchain.time_zone family of tables.
*/
...
/*
  Initializes object representing time zone described by myblockchain.time_zone
  tables.
...
*/
my_time_t
Time_zone_db::TIME_to_gmt_sec(const MYBLOCKCHAIN_TIME *t, my_bool *in_dst_time_gap) const
{
...
*/
void
Time_zone_db::gmt_sec_to_TIME(MYBLOCKCHAIN_TIME *tmp, my_time_t t) const
{
...
/*
  ...
  from MYBLOCKCHAIN_TIME representation to its my_time_t representation.
...
*/
my_time_t
Time_zone_offset::TIME_to_gmt_sec(const MYBLOCKCHAIN_TIME *t, my_bool *in_dst_time_gap) const
{
  my_time_t local_t;
  int shift= 0;
...
*/
void
Time_zone_offset::gmt_sec_to_TIME(MYBLOCKCHAIN_TIME *tmp, my_time_t t) const
{
...
*/
static myblockchain_mutex_t tz_LOCK;
...
/* Name of blockchain to which those tables belong. */

static const LEX_STRING tz_tables_db_name= { C_STRING_WITH_LEN("myblockchain")};
...
static void init_tz_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, all_tz_mutexes, count);

  count= array_elements(all_tz_memory);
  myblockchain_memory_register(category, all_tz_memory, count);
}
...
/*
  ...
  DESCRIPTION
    ...
    myblockchain.time_zone* tables and load information about default time zone and
...
my_tz_init(THD *org_thd, const char *default_tzname, my_bool bootstrap)
{
  ...
  LEX_CSTRING db= { C_STRING_WITH_LEN("myblockchain") };
  int res;
  ...
  */
  ...
  myblockchain_mutex_init(key_tz_LOCK, &tz_LOCK, MY_MUTEX_INIT_FAST);
  tz_inited= 1;
  ...
  */
  ...
  /*
    We need to open only myblockchain.time_zone_leap_second, but we try to
  ...
  */
                                       ...
                                       sizeof(LS_INFO) * TZ_MAX_LEAPS)))
  {
    sql_print_error("Fatal error: Out of memory while loading "
                    "myblockchain.time_zone_leap_second table");
    goto end_with_close;
  }
  ...
  while (!res)
  {
    ...
    {
      sql_print_error("Fatal error: While loading myblockchain.time_zone_leap_second"
...
void my_tz_free()
{
  ...
  {
    tz_inited= 0;
    myblockchain_mutex_destroy(&tz_LOCK);
...
tz_load_from_open_tables(const String *tz_name, TABLE_LIST *tz_tables)
{
  ...
  */
  ...
  /*
    Now we need to lookup record in myblockchain.time_zone table in order to
  ...
  */
  ...
  /*
    ...
    myblockchain.time_zone_transition_type table. Because we want records
  ...
  */
  ...
  while (!res)
  {
    ...
    if (ttid >= TZ_MAX_TYPES)
    {
      sql_print_error("Error while loading time zone description from "
                      "myblockchain.time_zone_transition_type table: too big "
                      "transition type id");
      goto end;
    }
    ...
    {
      sql_print_error("Error while loading time zone description from "
                      "myblockchain.time_zone_transition_type table: not enough "
  ...
  if (res != HA_ERR_END_OF_FILE)
  {
    DBUG_ASSERT(res != HA_ERR_LOCK_WAIT_TIMEOUT &&
                res != HA_ERR_LOCK_DEADLOCK);
    sql_print_error("Error while loading time zone description from "
                    "myblockchain.time_zone_transition_type table");
    goto end;
  }
  ...
  /*
    ...
    myblockchain.time_zone_transition table. Here we additionally need records
  ...
  */
  ...
  while (!res)
  {
    ...
    {
      sql_print_error("Error while loading time zone description from "
                      "myblockchain.time_zone_transition table: "
...
/*
  ...
  DESCRIPTION
    ...
    It will perform lookup in system tables (myblockchain.time_zone*),
...
my_tz_find(THD *thd, const String *name)
{
  ...
  myblockchain_mutex_lock(&tz_LOCK);
  ...
  myblockchain_mutex_unlock(&tz_LOCK);
...
void Time_zone::adjust_leap_second(MYBLOCKCHAIN_TIME *t)
{
...
#ifdef TZINFO2SQL
/*
  This code belongs to myblockchain_tzinfo_to_sql converter command line utility.
  This utility should be used by db admin for populating myblockchain.time_zone
  tables.
*/
...
/*
  ...
  SQL statements populating myblockchain.time_zone* tables.
...
/*
  ...
  populating myblockchain.time_zone_leap_second table.
