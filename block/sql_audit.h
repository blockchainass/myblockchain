...
extern unsigned long myblockchain_global_audit_mask[];


extern void myblockchain_audit_initialize();
extern void myblockchain_audit_finalize();


extern void myblockchain_audit_init_thd(THD *thd);
extern void myblockchain_audit_free_thd(THD *thd);
extern void myblockchain_audit_acquire_plugins(THD *thd, uint event_class);


#ifndef EMBEDDED_LIBRARY
extern void myblockchain_audit_notify(THD *thd, uint event_class,
                               uint event_subtype, ...);
bool is_any_audit_plugin_active(THD *thd);
bool is_audit_plugin_class_active(THD *thd, unsigned int event_class);
#else
#define myblockchain_audit_notify(...)
#endif
extern void myblockchain_audit_release(THD *thd);
...
/**
  Call audit plugins of GENERAL audit class, MYBLOCKCHAIN_AUDIT_GENERAL_LOG subtype.
...
static inline
void myblockchain_audit_general_log(THD *thd, const char *cmd, size_t cmdlen)
{
#ifndef EMBEDDED_LIBRARY
  if (myblockchain_global_audit_mask[0] & MYBLOCKCHAIN_AUDIT_GENERAL_CLASSMASK)
  {
    MYBLOCKCHAIN_LEX_STRING sql_command, ip, host, external_user;
    LEX_CSTRING query= EMPTY_CSTR;
    static MYBLOCKCHAIN_LEX_STRING empty= { C_STRING_WITH_LEN("") };
    ...
    myblockchain_audit_notify(thd, MYBLOCKCHAIN_AUDIT_GENERAL_CLASS, MYBLOCKCHAIN_AUDIT_GENERAL_LOG,
...
/**
  ...
  event_subtype should be set to one of:
    MYBLOCKCHAIN_AUDIT_GENERAL_ERROR
    MYBLOCKCHAIN_AUDIT_GENERAL_RESULT
    MYBLOCKCHAIN_AUDIT_GENERAL_STATUS
...
*/
static inline
void myblockchain_audit_general(THD *thd, uint event_subtype,
                         int error_code, const char *msg)
{
  ...
  if (myblockchain_global_audit_mask[0] & MYBLOCKCHAIN_AUDIT_GENERAL_CLASSMASK)
  {
    ...
    MYBLOCKCHAIN_LEX_STRING ip, host, external_user, sql_command;
    ha_rows rows;
    ...
    myblockchain_audit_notify(thd, MYBLOCKCHAIN_AUDIT_GENERAL_CLASS, event_subtype,
...
#define MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_CONNECT(thd) myblockchain_audit_notify(\
  (thd), MYBLOCKCHAIN_AUDIT_CONNECTION_CLASS, MYBLOCKCHAIN_AUDIT_CONNECTION_CONNECT,\
  (thd)->get_stmt_da()->is_error() ? (thd)->get_stmt_da()->myblockchain_errno() : 0,\
...
#define MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_DISCONNECT(thd, errcode)\
  myblockchain_audit_notify(\
  (thd), MYBLOCKCHAIN_AUDIT_CONNECTION_CLASS, MYBLOCKCHAIN_AUDIT_CONNECTION_DISCONNECT,\
...
#define MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_CHANGE_USER(thd) myblockchain_audit_notify(\
  (thd), MYBLOCKCHAIN_AUDIT_CONNECTION_CLASS, MYBLOCKCHAIN_AUDIT_CONNECTION_CHANGE_USER,\
  (thd)->get_stmt_da()->is_error() ? (thd)->get_stmt_da()->myblockchain_errno() : 0,\
  (thd)->thread_id(), (thd)->security_context()->user().str,\
  (thd)->security_context()->user().length,\
  (thd)->security_context()->priv_user().str,\
