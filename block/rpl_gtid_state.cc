  ...
  else
  {
    /*
      ...
      - There is also a corner case. If CREATE...SELECT is logged in
        ...
        MYBLOCKCHAIN_BIN_BINLOG::commit, if there were no matching rows so
        that there is nothing to commit, MYBLOCKCHAIN_BIN_LOG::commit does a
        direct call to gtid_state->update_on_[commit|rollback] since
        other functions called from MYBLOCKCHAIN_BIN_LOG::commit did not do
...
int Gtid_state::wait_for_gtid(THD *thd, const Gtid &gtid, struct timespec* timeout)
{
  ...
  myblockchain_mutex_unlock(thd->current_mutex);
