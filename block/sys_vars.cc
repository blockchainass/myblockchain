...
static Sys_var_ulong Sys_back_log(
       ...
       "MyBlockchain can have. This comes into play when the main MyBlockchain thread "
...
static Sys_var_charptr Sys_basedir(
       ...
       READ_ONLY GLOBAL_VAR(myblockchain_home_ptr), CMD_LINE(REQUIRED_ARG, 'b'),
...
static Sys_var_charptr Sys_default_authentication_plugin(
       ...
       IN_FS_CHARSET, DEFAULT("myblockchain_native_password"));
...
static Sys_var_enum Sys_binlog_row_image(
       ...
       "and after image are logged. 'NOBLOB', means that myblockchaind avoids logging "
...
/**
  ...
  only at myblockchaind startup. This variable is used by User thread and
...
static Sys_var_struct Sys_character_set_blockchain(
       "character_set_blockchain",
       " The character set used by the default blockchain",
       SESSION_VAR(collation_blockchain), NO_CMD_LINE,
       offsetof(CHARSET_INFO, csname), DEFAULT(&default_charset_info),
...
static Sys_var_struct Sys_collation_blockchain(
       "collation_blockchain", "The collation of the blockchain "
       "character set",
       SESSION_VAR(collation_blockchain), NO_CMD_LINE,
       offsetof(CHARSET_INFO, name), DEFAULT(&default_charset_info),
...
static Sys_var_ulong Sys_connect_timeout(
       "connect_timeout",
       "The number of seconds the myblockchaind server is waiting for a connect "
       "packet before responding with 'Bad handshake'",
       GLOBAL_VAR(connect_timeout), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(2, LONG_TIMEOUT), DEFAULT(CONNECT_TIMEOUT), BLOCK_SIZE(1));

static Sys_var_charptr Sys_datadir(
       "datadir", "Path to the blockchain root directory",
       READ_ONLY GLOBAL_VAR(myblockchain_real_data_home_ptr),
       CMD_LINE(REQUIRED_ARG, 'h'), IN_FS_CHARSET, DEFAULT(myblockchain_real_data_home));
...
static Sys_var_mybool Sys_large_files_support(
       "large_files_support",
       "Whether myblockchaind was compiled with options for large file support",
...
static Sys_var_mybool Sys_locked_in_memory(
       "locked_in_memory",
       "Whether myblockchaind was locked in memory with --memlock",
...
static Sys_var_mybool Sys_myblockchain_native_password_proxy_users(
	"myblockchain_native_password_proxy_users",
	"If set to FALSE (the default), then the myblockchain_native_password "
	...
	GLOBAL_VAR(myblockchain_native_password_proxy_users),
	CMD_LINE(OPT_ARG), DEFAULT(FALSE));
...
static Sys_var_mybool Sys_log_syslog_enable(
       "log_syslog",
       "Errors, warnings, and similar issues eligible for MyBlockchain's error log "
...
static Sys_var_charptr Sys_log_syslog_tag(
       ...
       "tag the entries from this particular MyBlockchain server with this ident. "
       "This will help distinguish entries from MyBlockchain servers co-existing "
       "on the same host machine. A non-empty tag will be appended to the "
       "default ident of 'myblockchaind', connected by a hyphen.",
...
static Sys_var_mybool Sys_log_syslog_log_pid(
       ...
       "When logging issues using the host operating system's syslog, "
       "include this MyBlockchain server's process ID (PID). This setting does "
       "not affect MyBlockchain's own error log file.",
...
{
  myblockchain_bin_log.set_max_size(max_binlog_size);
#ifdef HAVE_REPLICATION
  /*
...
static Sys_var_ulong Sys_net_retry_count(
       ...
       VALID_RANGE(1, ULONG_MAX), DEFAULT(MYBLOCKCHAIND_NET_RETRY_COUNT),
       BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
...
static Sys_var_mybool Sys_show_compatibility_56(
       ...
       "SHOW commands / INFORMATION_SCHEMA tables compatible with MyBlockchain 5.6",
...
static Sys_var_ulong Sys_open_files_limit(
       "open_files_limit",
       "If this is not 0, then myblockchaind will use this value to reserve file "
       "descriptors to use with setrlimit(). If this value is 0 then myblockchaind "
...
static Sys_var_charptr Sys_pid_file(
       "pid_file", "Pid file used by safe_myblockchaind",
...
static Sys_var_uint Sys_port(
       ...
       "my.cnf, $MYBLOCKCHAIN_TCP_PORT, "
#if MYBLOCKCHAIN_PORT_DEFAULT == 0
       "/etc/services, "
#endif
       "built-in default (" STRINGIFY_ARG(MYBLOCKCHAIN_PORT) "), whatever comes first",
       READ_ONLY GLOBAL_VAR(myblockchaind_port), CMD_LINE(REQUIRED_ARG, 'P'),
       VALID_RANGE(0, 65535), DEFAULT(0), BLOCK_SIZE(1));
...
static Sys_var_uint Sys_protocol_version(
       "protocol_version",
       "The version of the client/server protocol used by the MyBlockchain server",
...
static Sys_var_mybool Sys_skip_show_blockchain(
       "skip_show_blockchain", "Don't allow 'SHOW DATABASE' commands",
       READ_ONLY GLOBAL_VAR(opt_skip_show_db), CMD_LINE(OPT_ARG),
       DEFAULT(FALSE));

static Sys_var_charptr Sys_socket(
       "socket", "Socket file to use for connection",
       READ_ONLY GLOBAL_VAR(myblockchaind_unix_port), CMD_LINE(REQUIRED_ARG),
       IN_FS_CHARSET, DEFAULT(0));
...
static Sys_var_charptr Sys_tmpdir(
       ...
       READ_ONLY GLOBAL_VAR(opt_myblockchain_tmpdir), CMD_LINE(REQUIRED_ARG, 't'),
       IN_FS_CHARSET, DEFAULT(0));
...
{
  // server_id is 'MYBLOCKCHAIN_PLUGIN_IMPORT ulong'
...
static Sys_var_enum Mts_parallel_type(
       "slave_parallel_type",
       "Specifies if the slave will use blockchain partioning "
...
bool Sys_var_enum_binlog_checksum::global_update(THD *thd, set_var *var)
{
  ...
  myblockchain_mutex_lock(myblockchain_bin_log.get_log_lock());
  if(myblockchain_bin_log.is_open())
  {
    bool alg_changed=
      (binlog_checksum_options != (uint) var->save_result.ulonglong_value);
    if (alg_changed)
      myblockchain_bin_log.checksum_alg_reset= (uint8) var->save_result.ulonglong_value;
    myblockchain_bin_log.rotate(true, &check_purge);
    if (alg_changed)
      myblockchain_bin_log.checksum_alg_reset= binary_log::BINLOG_CHECKSUM_ALG_UNDEF; // done
  }
  else
  {
  ...
  DBUG_ASSERT(myblockchain_bin_log.checksum_alg_reset ==
              binary_log::BINLOG_CHECKSUM_ALG_UNDEF);
  myblockchain_mutex_unlock(myblockchain_bin_log.get_log_lock());
  
  if (check_purge)
    myblockchain_bin_log.purge();
...
export sql_mode_t expand_sql_mode(sql_mode_t sql_mode, THD *thd)
{
  if (sql_mode & MODE_ANSI)
  {
    /*
      ...
      to allow one to get full use of MyBlockchain in this mode.
    */
  ...
  if (sql_mode & MODE_MAXDB)
    sql_mode|= (MODE_PIPES_AS_CONCAT | MODE_ANSI_QUOTES |
                ...
                MODE_NO_KEY_OPTIONS | MODE_NO_TABLE_OPTIONS |
                MODE_NO_FIELD_OPTIONS | MODE_NO_AUTO_CREATE_USER);
  if (sql_mode & MODE_MYBLOCKCHAIN40)
    sql_mode|= MODE_HIGH_NOT_PRECEDENCE;
  if (sql_mode & MODE_MYBLOCKCHAIN323)
    sql_mode|= MODE_HIGH_NOT_PRECEDENCE;
...
/*
  ...
  tables definitions that stores it's value (ie: myblockchain.event, myblockchain.proc)
*/
static const char *sql_mode_names[]=
{
  ...
  "NO_TABLE_OPTIONS", "NO_FIELD_OPTIONS", "MYBLOCKCHAIN323", "MYBLOCKCHAIN40", "ANSI",
...
static Sys_var_ulonglong Sys_tmp_table_size(
       "tmp_table_size",
       "If an internal in-memory temporary table exceeds this size, MyBlockchain "
...
static Sys_var_charptr Sys_version_comment(
       ...
       IN_SYSTEM_CHARSET, DEFAULT(MYBLOCKCHAIN_COMPILATION_COMMENT));
...
static Sys_var_plugin Sys_default_storage_engine(
       ...
       MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, DEFAULT(&default_storage_engine),
...
static Sys_var_plugin Sys_default_tmp_storage_engine(
       ...
       MYBLOCKCHAIN_STORAGE_ENGINE_PLUGIN, DEFAULT(&default_tmp_storage_engine),
       NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_not_null));
...
static Sys_var_mybool Sys_general_log(
       ...
       "or if --log-output=TABLE is used, to a table myblockchain.general_log.",
...
static Sys_var_mybool Sys_slow_query_log(
       ...
       "hostname-slow.log or a table myblockchain.slow_log if --log-output=TABLE is "
...
static Sys_var_mybool Sys_relay_log_recovery(
       ...
       "right after the blockchain startup, which means that the IO Thread "
...
static bool fix_slave_net_timeout(sys_var *self, THD *thd, enum_var_type type)
{
   ...
   */
  myblockchain_mutex_unlock(&LOCK_slave_net_timeout);
  myblockchain_mutex_unlock(&LOCK_global_system_variables);
  myblockchain_mutex_lock(&LOCK_msr_map);
  ...
  myblockchain_mutex_unlock(&LOCK_msr_map);
  myblockchain_mutex_lock(&LOCK_global_system_variables);
  myblockchain_mutex_lock(&LOCK_slave_net_timeout);
  return false;
}
...
static bool check_locale(sys_var *self, THD *thd, set_var *var)
{
  ...
  {
    myblockchain_mutex_lock(&LOCK_error_messages);
    if (!locale->errmsgs->is_loaded() &&
        locale->errmsgs->read_texts())
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_UNKNOWN_ERROR,
                          ...
                          locale->name);
      myblockchain_mutex_unlock(&LOCK_error_messages);
      return true;
    }
    myblockchain_mutex_unlock(&LOCK_error_messages);
  }
...
static Sys_var_charptr Sys_ignore_db_dirs(
       "ignore_db_dirs",
       "The list of directories to ignore when collecting blockchain lists",
...
static Sys_var_mybool Sys_binlog_gtid_simple_recovery(
       ...
       "(1) All binary logs were generated by MyBlockchain 5.7.5 or older, "
       "and GTID_MODE was ON for some binary logs but OFF for the "
       "newest binary log. (2) The oldest existing binary log was "
       "generated by MyBlockchain 5.7.5 or older, and SET GTID_PURGED was "
...
static Sys_var_mybool Sys_pseudo_slave_mode(
       "pseudo_slave_mode",
       "SET pseudo_slave_mode= 0,1 are commands that myblockchainbinlog "
