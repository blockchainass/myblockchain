...
/**
  ...
  @param IN/OUT ltime       MYBLOCKCHAIN_TIME variable to add to.
  @param        nanosecons  Nanosecons value.
  @param IN/OUT warnings    Warning flag vector.
  @retval                   False on success, true on error.
*/
bool time_add_nanoseconds_with_round(MYBLOCKCHAIN_TIME *ltime,
...
/*
  Convert a timestamp string to a MYBLOCKCHAIN_TIME value and produce a warning 
...
*/
bool
str_to_datetime_with_warn(String *str, MYBLOCKCHAIN_TIME *l_time,
...
*/
static bool lldiv_t_to_datetime(lldiv_t lld, MYBLOCKCHAIN_TIME *ltime,
                                my_time_flags_t flags, int *warnings)
{
      ...
      number_to_datetime(lld.quot, ltime, flags, warnings) == -1LL)
  {
    ...
    set_zero_time(ltime, MYBLOCKCHAIN_TIMESTAMP_ERROR);
    if (!*warnings) /* Neither sets warnings in case of ZERO DATE */
      *warnings|= MYBLOCKCHAIN_TIME_WARN_TRUNCATED;
    return true;
  }
  else if (ltime->time_type == MYBLOCKCHAIN_TIMESTAMP_DATE)
  {
    /*
                                      ...
                                      MYBLOCKCHAIN_TIME *ltime, my_time_flags_t flags)
{
  ...
  {
    warnings|= MYBLOCKCHAIN_TIME_WARN_TRUNCATED;
    set_zero_time(ltime, MYBLOCKCHAIN_TIMESTAMP_NONE);
  }
  else
                                       ...
                                       my_time_flags_t flags)
{
  ...
  if (warnings)
    make_truncated_value_warning(ErrConvString(nr),  MYBLOCKCHAIN_TIMESTAMP_NONE);
  return rc;
}
...
/**
  ...
  @return                 False on success, true on error.
*/
static bool lldiv_t_to_time(lldiv_t lld, MYBLOCKCHAIN_TIME *ltime, int *warnings)
{
...
/**
  ...
  @return     False on success, true on error.
*/
bool my_decimal_to_time_with_warn(const my_decimal *decimal, MYBLOCKCHAIN_TIME *ltime)
{
  lldiv_t lld;
  int warnings= 0;
  bool rc;
  ...
  if (warnings)
    make_truncated_value_warning(ErrConvString(decimal), MYBLOCKCHAIN_TIMESTAMP_TIME);
  return rc;
}
...
bool my_double_to_time_with_warn(double nr, MYBLOCKCHAIN_TIME *ltime)
{
  ...
  if (warnings)
    make_truncated_value_warning(ErrConvString(nr), MYBLOCKCHAIN_TIMESTAMP_TIME);
  return rc;
}
...
/**
  Convert a datetime from broken-down MYBLOCKCHAIN_TIME representation
...
*/
my_time_t TIME_to_timestamp(THD *thd, const MYBLOCKCHAIN_TIME *t, my_bool *in_dst_time_gap)
{
  my_time_t timestamp;
...
/**
  Convert a datetime MYBLOCKCHAIN_TIME representation
                                              ...
                                              int *warnings)
{
  ...
  {
    /*
      Set MYBLOCKCHAIN_TIME_WARN_INVALID_TIMESTAMP warning to indicate
    ...
    */
    *warnings|= MYBLOCKCHAIN_TIME_WARN_INVALID_TIMESTAMP;
  }
...
/**
  ...
  @retval      true on error
*/
bool datetime_to_timeval(THD *thd, const MYBLOCKCHAIN_TIME *ltime,
...
/*
  Convert a time string to a MYBLOCKCHAIN_TIME struct and produce a warning
...
*/
bool
str_to_time_with_warn(String *str, MYBLOCKCHAIN_TIME *l_time)
{
  MYBLOCKCHAIN_TIME_STATUS status;
  bool ret_val= str_to_time(str, l_time, 0, &status);
  if (ret_val || status.warnings)
    make_truncated_value_warning(ErrConvString(str), MYBLOCKCHAIN_TIMESTAMP_TIME);
  return ret_val;
}
...
*/
void time_to_datetime(THD *thd, const MYBLOCKCHAIN_TIME *ltime, MYBLOCKCHAIN_TIME *ltime2)
{
  ...
  ltime2->time_type= MYBLOCKCHAIN_TIMESTAMP_DATE;
...
void calc_time_from_sec(MYBLOCKCHAIN_TIME *to, longlong seconds, long microseconds)
{
  ...
  to->time_type= MYBLOCKCHAIN_TIMESTAMP_TIME;
  to->year= 0;
  to->month= 0;
  to->day= 0;
			    ...
			    Date_time_format *date_time_format)
{
  ...
  */
  ...
  /*
    ...
    it's needed.
  */
  if ((format_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME &&
       !test_all_bits(part_map, (1 | 2 | 4 | 8 | 16 | 32))) ||
      (format_type == MYBLOCKCHAIN_TIMESTAMP_DATE && part_map != (1 | 2 | 4)) ||
      (format_type == MYBLOCKCHAIN_TIMESTAMP_TIME &&
  ...
  */
  ...
  case MYBLOCKCHAIN_TIMESTAMP_TIME:
    ...
    */
    ...
    {
      if (format_type == MYBLOCKCHAIN_TIMESTAMP_TIME)
      {
...
/****************************************************************************
  ...
  NOTE:
    ...
    MyBlockchain doesn't support comparing of date/time/datetime strings that
...
void make_time(const Date_time_format *format __attribute__((unused)),
               const MYBLOCKCHAIN_TIME *l_time, String *str, uint dec)
{
...
void make_date(const Date_time_format *format __attribute__((unused)),
               const MYBLOCKCHAIN_TIME *l_time, String *str)
{
                                  ...
                                  const char *field_name)
{
  ...
  switch (time_type) {
    ...
    case MYBLOCKCHAIN_TIMESTAMP_TIME:
      type_str= "time";
      break;
    case MYBLOCKCHAIN_TIMESTAMP_DATETIME:  // FALLTHROUGH
    default:
      type_str= "datetime";
      break;
  }
  ...
  {
    if (time_type > MYBLOCKCHAIN_TIMESTAMP_ERROR)
      cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff),
                       ...
                       Interval interval)
{
  ...
  case INTERVAL_DAY_HOUR:
  {
    ...
    ltime->time_type= MYBLOCKCHAIN_TIMESTAMP_DATETIME; // Return full date
...
/*
  ...
  NOTE
    ...
    signs into account (i.e. for MYBLOCKCHAIN_TIME values).
...
bool
calc_time_diff(const MYBLOCKCHAIN_TIME *l_time1, const MYBLOCKCHAIN_TIME *l_time2,
               int l_sign, longlong *seconds_out, long *microseconds_out)
{
  ...
  /*
    We suppose that if first argument is MYBLOCKCHAIN_TIMESTAMP_TIME
    the second argument should be TIMESTAMP_TIME also.
    We should check it before calc_time_diff call.
  */
  if (l_time1->time_type == MYBLOCKCHAIN_TIMESTAMP_TIME)  // Time value
  ...
  else
  {
    days= calc_daynr((uint) l_time1->year,
		     (uint) l_time1->month,
		     (uint) l_time1->day);
    if (l_time2->time_type == MYBLOCKCHAIN_TIMESTAMP_TIME)
...
/*
  Compares 2 MYBLOCKCHAIN_TIME structures
...
/**
  ...
  @return        False on success, true on error.
*/
bool my_time_round(MYBLOCKCHAIN_TIME *ltime, uint dec)
{
  int warnings= 0;
...
/**
  ...
  @return        False on success, true on error.
*/
bool my_datetime_round(MYBLOCKCHAIN_TIME *ltime, uint dec, int *warnings)
{
  DBUG_ASSERT(dec <= DATETIME_MAX_DECIMALS);
  /* Add half away from zero */
...
{
  DBUG_ASSERT(ldate->time_type == MYBLOCKCHAIN_TIMESTAMP_DATE ||
              ldate->time_type == MYBLOCKCHAIN_TIMESTAMP_DATETIME);
...
/**
  Convert MYBLOCKCHAIN_TIME value to its packed numeric representation,
  using field type.
  @param ltime  The value to convert.
  @param type   MyBlockchain field type.
