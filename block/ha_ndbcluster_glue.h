...
/* No myblockchain_rm_table_part2 anymore in 5.5.8 */
#define NDB_NO_MYBLOCKCHAIN_RM_TABLE_PART2
...
{
#if MYBLOCKCHAIN_VERSION_ID < 50500
  return thd->options;
#else
...
/*
  MyBlockchain Server has got its own mutex type in 5.5, add backwards
  compatibility support allowing to write code in 7.0 that works
  in future MyBlockchain Server
*/

typedef pthread_mutex_t myblockchain_mutex_t;

static inline
int myblockchain_mutex_lock(myblockchain_mutex_t* mutex)
{
  return pthread_mutex_lock(mutex);
}

static inline
int myblockchain_mutex_unlock(myblockchain_mutex_t* mutex)
{
  return pthread_mutex_unlock(mutex);
}

static inline
void myblockchain_mutex_assert_owner(myblockchain_mutex_t* mutex)
{
  return safe_mutex_assert_owner(mutex);
}

typedef pthread_cond_t myblockchain_cond_t;

static inline
int myblockchain_cond_wait(myblockchain_cond_t* cond, myblockchain_mutex_t* mutex)
{
  return pthread_cond_wait(cond, mutex);
}

static inline
int myblockchain_cond_timedwait(myblockchain_cond_t* cond, myblockchain_mutex_t* mutex,
