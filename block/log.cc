...
#include "sql_audit.h"    // myblockchain_audit_general_log
...
{
  char m_message[MYBLOCKCHAIN_ERRMSG_SIZE];
public:
...
bool log_syslog_update_settings()
{
  ...
  /*
    ...
    conventions -- "myblockchaind" on unix (via myblockchaind_safe), and "MyBlockchain" for the
    Win NT EventLog.
  */
  prefix= "myblockchaind";
#else
  prefix= "MyBlockchain";
#endif
...
bool File_query_log::open()
{
  ...
  fn_format(log_file_name, name, myblockchain_data_home, "", 4);
  ...
  if ((file= myblockchain_file_open(m_log_file_key,
                             log_file_name,
...

  {
    ...
    size_t len=my_snprintf(buff, sizeof(buff), "%s, Version: %s (%s). "
                        ...
                        "embedded library\n",
                        my_progname, server_version, MYBLOCKCHAIN_COMPILATION_COMMENT
#elif _WIN32
                        "started with:\nTCP Port: %d, Named Pipe: %s\n",
                        my_progname, server_version, MYBLOCKCHAIN_COMPILATION_COMMENT,
                        myblockchaind_port, myblockchaind_unix_port
#else
...
err:
  ...
  sql_print_error("Could not use %s for logging (error %d - %s). "
                  ...
                  "then %s restart the MyBlockchain server.", name, errno,
...
void File_query_log::close()
{
  ...
  if (myblockchain_file_close(log_file.file, MYF(MY_WME)))
    check_and_print_write_error();
                                           ...
                                           const CHARSET_INFO *client_cs)
{
  ...
  */
  ...
  table_list.init_one_table(MYBLOCKCHAIN_SCHEMA_NAME.str, MYBLOCKCHAIN_SCHEMA_NAME.length,
  ...
  */
  ...
  DBUG_ASSERT(table->field[GLT_FIELD_EVENT_TIME]->type() == MYBLOCKCHAIN_TYPE_TIMESTAMP);
...
err:
  ...
  if (result && !thd->killed)
    sql_print_error("Failed to write to myblockchain.general_log: %s",
                                        ...
                                        size_t sql_text_len)
{
  ...
  */
  ...
  DBUG_ASSERT(table->field[SQLT_FIELD_START_TIME]->type() == MYBLOCKCHAIN_TYPE_TIMESTAMP);
...
err:
  ...
  if (result && !thd->killed)
    sql_print_error("Failed to write to myblockchain.slow_log: %s",
                    error_handler.message());
...
bool Log_to_file_event_handler::log_slow(THD *thd, ulonglong current_utime,
                                         ...
                                         size_t sql_text_len)
{
  if (!myblockchain_slow_log.is_open())
    return false;

  Silence_log_table_errors error_handler;
  thd->push_internal_handler(&error_handler);
  bool retval= myblockchain_slow_log.write_slow(thd, current_utime, query_start_arg,
...
bool Log_to_file_event_handler::log_general(THD *thd, ulonglong event_utime,
                                            ...
                                            const CHARSET_INFO *client_cs)
{
  if (!myblockchain_general_log.is_open())
    return false;

  Silence_log_table_errors error_handler;
  thd->push_internal_handler(&error_handler);
  bool retval= myblockchain_general_log.write_general(event_utime, user_host,
...
{
  myblockchain_rwlock_destroy(&LOCK_logger);
                                  ...
                                  size_t query_length)
{
  ...
  myblockchain_rwlock_rdlock(&LOCK_logger);
  ...
  myblockchain_rwlock_unlock(&LOCK_logger);
                                     ...
                                     const char *query, size_t query_length)
{
  ...
  myblockchain_audit_general_log(thd, command_name[(uint) command].str,
...
{
  myblockchain_rwlock_wrlock(&LOCK_logger);
...
{
  if (table_list->db_length == MYBLOCKCHAIN_SCHEMA_NAME.length &&
      !my_strcasecmp(system_charset_info,
                     table_list->db, MYBLOCKCHAIN_SCHEMA_NAME.str))
  {
...
char *make_query_log_name(char *buff, enum_log_table_type log_type)
{
  ...
  return fn_format(buff, buff, myblockchain_real_data_home, log_ext,
...
Slow_log_throttle::Slow_log_throttle(ulong *threshold, myblockchain_mutex_t *lock,
                                     ulong window_usecs,
...
bool Slow_log_throttle::flush(THD *thd)
{
  ...
  myblockchain_mutex_lock(LOCK_log_throttle);
  ...
  myblockchain_mutex_unlock(LOCK_log_throttle);
...
static myblockchain_mutex_t LOCK_error_log;
...
void init_error_log()
{
  ...
  myblockchain_mutex_init(key_LOCK_error_log, &LOCK_error_log, MY_MUTEX_INIT_FAST);
...
void destroy_error_log()
{
  if (error_log_initialized)
  {
    ...
    error_log_file= NULL;
    myblockchain_mutex_destroy(&LOCK_error_log);
  }
}
...
bool reopen_error_log()
{
  if (!error_log_file)
    return false;
  myblockchain_mutex_lock(&LOCK_error_log);
  bool result= open_error_log(error_log_file);
  myblockchain_mutex_unlock(&LOCK_error_log);
  if (result)
...
extern "C"
int my_plugin_log_message(MYBLOCKCHAIN_PLUGIN *plugin_ptr, plugin_log_level level,
                          const char *format, ...)
{
  char format2[MYBLOCKCHAIN_ERRMSG_SIZE];
  loglevel lvl;
