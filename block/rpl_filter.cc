...
/*
  ...
  TODO
    ...
    (I could not find an equivalent in the regex library MyBlockchain uses).
...
Rpl_filter::db_ok(const char* db)
{
  ...
  /*
    ...
    blockchains to replicate and db was not selected, do not replicate" has
...
/*
  ...
  NOTES
    ...
    We advise users who want to exclude a blockchain 'db1' safely to do it
...
void Sql_cmd_change_repl_filter::set_filter_value(List<Item>* item_list,
                                                  options_myblockchaind filter_type)
{
...
bool Sql_cmd_change_repl_filter::change_rpl_filter(THD* thd)
{
  ...
  */
  ...
  for (mi_map::iterator it= msr_map.begin(); it!= msr_map.end(); it++)
  {
    mi= it->second;
    if (mi)
      myblockchain_mutex_lock(&mi->rli->run_lock); /* lock slave_sql_thread */
  }
