...
/*
  ...
  to partitioning introduced in MyBlockchain version 5.1. It contains functionality
  ...
  called KEY partitioning), where the MyBlockchain server will use an internal
  hash function.
...
/*
  ...
  SYNOPSIS
    ...
    normalized_path               Normalized path name of table and blockchain
...
/*
  ...
  DESCRIPTION
    ...
    blockchain name of the table which is used to set-up a correct
...
{
  size_t ret_code= myblockchain_file_write(fptr, (const uchar*)buf, len, MYF(MY_FNABP));
...
/*
   ...
   parsing it later with myblockchain_unpack_partition will fail otherwise.
*/
...
static int check_part_field(enum_field_types sql_type,
                            ...
                            bool *need_cs_check)
{
  if (sql_type >= MYBLOCKCHAIN_TYPE_TINY_BLOB &&
      sql_type <= MYBLOCKCHAIN_TYPE_BLOB)
  {
    my_error(ER_BLOB_FIELD_IN_PART_FUNC_ERROR, MYF(0));
    return TRUE;
  }
                                ...
                                const char *current_comment_start)
{
  ...
  if (unlikely(((fptr= create_temp_file(path,myblockchain_tmpdir,"psy",
  ...
  if (err)
    goto close_file;
  buffer_length= myblockchain_file_seek(fptr, 0L, MY_SEEK_END, MYF(0));
  if (unlikely(buffer_length == MY_FILEPOS_ERROR))
    goto close_file;
  if (unlikely(myblockchain_file_seek(fptr, 0L, MY_SEEK_SET, MYF(0))
               == MY_FILEPOS_ERROR))
    goto close_file;
  *buf_length= (uint)buffer_length;
  ...
  if (unlikely(myblockchain_file_read(fptr, (uchar*)buf, *buf_length, MYF(MY_FNABP))))
  {
    if (!use_sql_alloc)
      my_free(buf);
    else
...
close_file:
  ...
  myblockchain_file_close(fptr, MYF(0));
  DBUG_RETURN(buf);
}
...
bool myblockchain_unpack_partition(THD *thd,
                            ...
                            bool *work_part_info_used)
{
  ...
  Partition_handler *part_handler;
  DBUG_ENTER("myblockchain_unpack_partition");
  ...
  *work_part_info_used= FALSE;
  lex.part_info= new partition_info();/* Indicates MYBLOCKCHAINparse from this place */
  if (!lex.part_info)
  {
  ...
  /*
    ...
    MyBlockchain Server and used in backup and restore of clusters or partitioned
    ...
    part_info we already built up during myblockchain_create_table if we are
                           ...
                           partition_info **new_part_info)
{
  ...
  /* ALTER_ADMIN_PARTITION is handled in myblockchain_admin_table */
...
also involve moving them to new disks or new node groups (MyBlockchain Cluster).
  ...
  else
  {
    /*
     ...
     Case IIa:
       ...
       open_table to the partition_info struct used by myblockchain_create_table
...
/*
  ...
  SYNOPSIS
    myblockchain_change_partitions()
...
static bool myblockchain_change_partitions(ALTER_PARTITION_PARAM_TYPE *lpt)
{
  ...
  DBUG_ENTER("myblockchain_change_partitions");
  ...
  if (alter_info->flags & Alter_info::ALTER_DROP_PARTITION)
  {
    /*
      ...
      3) Write the ddl log to ensure that the operation is completed
         even in the presence of a MyBlockchain Server crash (the log is executed
    ...
    */
    if (write_log_drop_shadow_frm(lpt) ||
        ...
        myblockchain_write_frm(lpt, WFRM_WRITE_SHADOW) ||
            ...
            part_info->part_type == LIST_PARTITION))
  {
    ...
    */
    if (write_log_drop_shadow_frm(lpt) ||
        ...
        myblockchain_change_partitions(lpt) ||
  ...
  else
  {
    ...
    else
    {
      ...
      */
      ...
      {
        if (!(flags & NO_MAX_RANGE) &&
            (field->type() == MYBLOCKCHAIN_TYPE_DATE ||
             field->type() == MYBLOCKCHAIN_TYPE_DATETIME))
        {
