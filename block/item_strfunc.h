...
class Item_func_blockchain :public Item_func_sysconst
{
  typedef Item_func_sysconst super;

public:
  explicit Item_func_blockchain(const POS &pos) :Item_func_sysconst(pos) {}
  ...
  const char *func_name() const { return "blockchain"; }
  const Name_string fully_qualified_func_name() const
  { return NAME_STRING("blockchain()"); }
};
