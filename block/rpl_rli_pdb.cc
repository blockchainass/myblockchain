                              ...
                              Slave_job_item *job_item)
{
  ...
  myblockchain_mutex_lock(&rli->exit_count_lock);
  /*
  ...
  */
  if (!job_item->data)
  {
    ...
    myblockchain_cond_signal(&worker->jobs_cond);
    myblockchain_mutex_unlock(&rli->exit_count_lock);
    return(true);
  }
    ...
    checkpoint_seqno(0), running_status(NOT_RUNNING), exit_incremented(false)
{
  ...
  */
  ...
  myblockchain_mutex_init(key_mutex_slave_parallel_worker, &jobs_lock,
                   MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_cond_slave_parallel_worker, &jobs_cond);
  myblockchain_cond_init(key_cond_mts_gaq, &logical_clock_cond);
}
...
Slave_worker::~Slave_worker()
{
  ...
  myblockchain_mutex_destroy(&jobs_lock);
  myblockchain_cond_destroy(&jobs_cond);
  myblockchain_cond_destroy(&logical_clock_cond);
  myblockchain_mutex_lock(&info_thd_lock);
  info_thd= NULL;
  myblockchain_mutex_unlock(&info_thd_lock);
...
int Slave_worker::init_worker(Relay_log_info * rli, ulong i)
{
  ...
  current_mts_submode=
    (mts_parallel_option == MTS_PARALLEL_TYPE_DB_NAME)?
       (Mts_submode*) new Mts_submode_blockchain():
...
bool init_hash_workers(Relay_log_info *rli)
{
  ...
  {
    myblockchain_mutex_init(key_mutex_slave_worker_hash, &rli->slave_worker_hash_lock,
                     MY_MUTEX_INIT_FAST);
    myblockchain_cond_init(key_cond_slave_worker_hash, &rli->slave_worker_hash_cond);
  }
...
void destroy_hash_workers(Relay_log_info *rli)
{
  ...
  if (rli->inited_hash_workers)
  {
    ...
    myblockchain_mutex_destroy(&rli->slave_worker_hash_lock);
    myblockchain_cond_destroy(&rli->slave_worker_hash_cond);
...
/**
   Relocating references of temporary tables of a blockchain
   ...
   @param entry  a pointer to db_worker_hash_entry record
                 containing blockchain descriptor and temporary_tables list.
...
/**
   ...
   @param  dbname      pointer to c-string containing blockchain name
                               ...
                               bool need_temp_tables, Slave_worker *last_worker)
{
  ...
  myblockchain_mutex_lock(&rli->slave_worker_hash_lock);
  ...
  {
    /*
      The blockchain name was not found which means that a worker never
      processed events from that blockchain. In such case, we need to
      map the blockchain to a worker my inserting an entry into the
      hash map.
    */
    my_bool ret;
    char *db= NULL;

    myblockchain_mutex_unlock(&rli->slave_worker_hash_lock);
    ...
    else
    {
      ...
      do
      {
        thd->ENTER_COND(&rli->slave_worker_hash_cond,
                                   ...
                                   &old_stage);
        myblockchain_cond_wait(&rli->slave_worker_hash_cond, &rli->slave_worker_hash_lock);
...
void Slave_worker::slave_worker_ends_group(Log_event* ev, int error)
{
  ...
  else
  {
    if (running_status != STOP_ACCEPTED)
    {
      ...
      myblockchain_mutex_lock(&jobs_lock);
      running_status= ERROR_LEAVING;
      myblockchain_mutex_unlock(&jobs_lock);
      ...
      myblockchain_mutex_lock(&c_rli->info_thd->LOCK_thd_data);
      c_rli->info_thd->awake(THD::KILL_QUERY);
      myblockchain_mutex_unlock(&c_rli->info_thd->LOCK_thd_data);
    }
  }
  ...
  */
  ...
  for (size_t i= 0; i < curr_group_exec_parts.size(); i++)
  {
    ...
    myblockchain_mutex_lock(&c_rli->slave_worker_hash_lock);
    ...
    if (entry->usage == 0)
    {
      ...
      */
      ...
      if (entry->worker != this) // Coordinator is waiting
      {
        ...
        myblockchain_cond_signal(&c_rli->slave_worker_hash_cond);
      }
    }
    else
      DBUG_ASSERT(usage_partition != 0);

    myblockchain_mutex_unlock(&c_rli->slave_worker_hash_lock);
  }
  ...
  else // not DB-type scheduler
  {
    ...
    */
    ...
    if (error)
    {
      myblockchain_mutex_lock(&c_rli->mts_gaq_LOCK);
      mts_submode->is_error= true;
      if (mts_submode->min_waited_timestamp != SEQ_UNINIT)
        myblockchain_cond_signal(&c_rli->logical_clock_cond);
      myblockchain_mutex_unlock(&c_rli->mts_gaq_LOCK);
    }
...
int Slave_worker::slave_worker_exec_event(Log_event *ev)
{
  ...
  // Address partioning only in blockchain mode
...
bool Slave_worker::worker_sleep(ulong seconds)
{
  ...
  myblockchain_mutex_t *lock= &jobs_lock;
  myblockchain_cond_t *cond= &jobs_cond;
                                     ...
                                     my_off_t end_relay_pos)
{
  ...
  do
  {
    ...
    if (trans_retries >= slave_trans_retries)
    {
      ...
      c_rli->report(ERROR_LEVEL, thd->get_stmt_da()->myblockchain_errno(),
    ...
    myblockchain_mutex_lock(&c_rli->data_lock);
    c_rli->retried_trans++;
    myblockchain_mutex_unlock(&c_rli->data_lock);
  ...
  while (!arrive_end)
  {
    ...
    else
    {
      ...
      */
      ...
      myblockchain_file_close(relay_io.file, MYF(0));
...
/*
  Find blockchain entry from map_db_to_worker hash table.
 */
                         ...
                         Slave_worker *worker, Relay_log_info *rli)
{
  ...
  while (new_pend_size > rli->mts_pending_jobs_size_max)
  {
    ...
    myblockchain_cond_wait(&rli->pending_jobs_cond, &rli->pending_jobs_lock);
  ...
  */
  ...
  myblockchain_mutex_lock(&worker->jobs_lock);
         ...
         (ret= en_queue(&worker->jobs, job_item)) == -1)
  {
    ...
    myblockchain_cond_wait(&worker->jobs_cond, &worker->jobs_lock);
    myblockchain_mutex_unlock(&worker->jobs_lock);
    thd->EXIT_COND(&old_stage);
                                  ...
                                  Slave_worker *worker, Relay_log_info *rli)
{
  ...
  */
      ...
      rli->mts_wq_oversize)  // TODO: unit/general test wq_oversize
  {
    ...
    myblockchain_cond_signal(&rli->pending_jobs_cond);
  }
