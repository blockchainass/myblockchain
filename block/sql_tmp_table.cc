                                         ...
                                         bool modify_item)
{
  ...
  case STRING_RESULT:
    ...
    */
    if (item->is_temporal() ||
        item->field_type() == MYBLOCKCHAIN_TYPE_GEOMETRY ||
        item->field_type() == MYBLOCKCHAIN_TYPE_JSON)
    {
...
{
  if (item->field_type() == MYBLOCKCHAIN_TYPE_VARCHAR)
  {
    Field *field;
...
/**
  ...
  @remark myblockchain_create_view() checks that views have less than
		 ...
		 const char *table_alias)
{
  ...
  */
  fn_format(path, path, myblockchain_tmpdir, "", MY_REPLACE_EXT|MY_UNPACK_FILENAME);
  ...
  */
  ...
  while ((item=li++))
  {
    ...
    {						/* Can't calc group yet */
      ...
      for (i=0 ; i < sum_item->get_arg_count() ; i++)
      {
        ...
        if (!arg->const_item())
        {
          ...
          if (new_field->type() == MYBLOCKCHAIN_TYPE_BIT)
            total_uneven_bit_length+= new_field->field_length & 7;
          *(reg_field++)= new_field;
          if (new_field->real_type() == MYBLOCKCHAIN_TYPE_STRING ||
              new_field->real_type() == MYBLOCKCHAIN_TYPE_VARCHAR)
          {
            string_count++;
	...
	blob_count++;
      ...
      if (new_field->type() == MYBLOCKCHAIN_TYPE_VAR_STRING ||
          new_field->type() == MYBLOCKCHAIN_TYPE_VARCHAR)
        table->s->db_create_options|= HA_OPTION_PACK_RECORD;
    ...
    */
    ...
    if (field->flags & BLOB_FLAG)
      recinfo->type= (int) FIELD_BLOB;
             ...
             field->real_type() == MYBLOCKCHAIN_TYPE_STRING &&
  ...
  *field= NULL;                             /* mark the end of the list */
  ...
  table->in_use= thd;           /* field->reset() may access table->in_use */
  {
    ...
    for (field= table->field; *field; ++field)
    {
      ...
      if (cur_field->type() == MYBLOCKCHAIN_TYPE_BIT &&
                             ...
                             ulonglong options, my_bool big_tables)
{
  ...
  {						// Get keys for ni_create
	...
	if (field->real_type() == MYBLOCKCHAIN_TYPE_STRING &&
