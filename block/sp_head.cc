...
#include "sql_db.h"            // myblockchain_opt_change_db, myblockchain_change_db
...
/**
  ...
  @param key         MDL key containing blockchain and routine name.
...
void sp_name::init_qname(THD *thd)
{
  ...
  /* m_qname format: [blockchain + dot] + name + '\0' */
...
bool sp_head::execute(THD *thd, bool merge_da_on_success)
{
  ...
  */
  ...
  /*
    ...
    decided that current blockchain should be preserved.
  */
  if (m_db.length &&
      (err_status= myblockchain_opt_change_db(thd, to_lex_cstring(m_db),
  ...
  */
  ...
  do
  {
    ...
    */
    ...
    thd->m_statement_psi= MYBLOCKCHAIN_START_STATEMENT(&psi_state, psi_info->m_key,
                                                thd->db().str,
                                                thd->db().length,
                                                thd->charset(),
                                                this->m_sp_share);
  ...
  if (err_status && thd->is_error() && !caller_da->is_error())
  {
    /*
      ...
      It will be set later by myblockchain_execute_command() / execute_trigger().
    ...
    */
    caller_da->set_error_status(thd->get_stmt_da()->myblockchain_errno(),
  ...
  */
  ...
  {
    /*
      Force switching back to the saved current blockchain, because it may be
      NULL. In this case, myblockchain_change_db() would generate an error.
    */

    err_status|= myblockchain_change_db(thd, to_lex_cstring(saved_cur_db_name), true);
  }
  m_flags&= ~IS_INVOKED;
  DBUG_PRINT("info",
             ("first free for 0x%lx --: 0x%lx->0x%lx, level: %lu, flags %x",
                              ...
                              GRANT_INFO *grant_info)
{
  ...
  */
  ...
  locker= MYBLOCKCHAIN_START_SP(&psi_state, m_sp_share);
#endif
  err_status= execute(thd, FALSE);
#ifdef HAVE_PSI_SP_INTERFACE
  MYBLOCKCHAIN_END_SP(locker);
#endif
                               ...
                               Field *return_value_fld)
{
  ...
  */
  need_binlog_call= myblockchain_bin_log.is_open() &&
  ...
  */
  ...
  if (need_binlog_call)
  {
    ...
    */
    ...
    myblockchain_bin_log.start_union_events(thd, q + 1);
  ...
  */
  ...
  if (need_binlog_call)
  {
    myblockchain_bin_log.stop_union_events(thd);
    ...
    if (thd->binlog_evt_union.unioned_events)
    {
          ...
          thd->binlog_evt_union.unioned_events_trans)
      {
        push_warning(thd, Sql_condition::SL_WARNING, ER_UNKNOWN_ERROR,
                     "Invoked ROUTINE modified a transactional table but MyBlockchain "
...
bool sp_head::execute_procedure(THD *thd, List<Item> *args)
{
  ...
  */
  ...
  /*
    ...
    messages.
  */ 
  bool need_binlog_call= myblockchain_bin_log.is_open() &&
                         (thd->variables.option_bits & OPTION_BIN_LOG) &&
                         !thd->is_current_stmt_binlog_format_row();
                                            ...
                                            TABLE_LIST *belong_to_view)
{
  /*
    ...
    Note that we also have to copy blockchain/table names and alias to PS/SP
