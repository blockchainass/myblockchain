...
extern "C" void handle_fatal_signal(int sig)
{
  if (segfaulted)
  {
    ...
    _exit(MYBLOCKCHAIND_FAILURE_EXIT); /* Quit without running destructors */
  }
  ...
  my_safe_printf_stderr("%s:%s:%s UTC - myblockchaind got " SIGNAL_FMT " ;\n",
  ...
  my_safe_printf_stderr("It is possible that myblockchaind could use up to \n"
                        "key_buffer_size + "
  ...
  if (!(test_flags & TEST_NO_STACKTRACE))
  {
    ...
    my_safe_printf_stderr("%s",
      ...
      "where myblockchaind died. If you see no messages after this, something went\n"
      "terribly wrong...\n");
  ...
  my_safe_printf_stderr("%s",
    "The manual page at "
    "http://dev.myblockchain.com/doc/myblockchain/en/crashing.html contains\n"
  ...
  */
  _exit(MYBLOCKCHAIND_FAILURE_EXIT);  // Using _exit(), since exit() is not async signal safe
#endif
}
