...
/* Classes in myblockchain */

#include "my_global.h"

#include "dur_prop.h"                     // durability_properties
#include "myblockchain/myblockchain_lex_string.h"       // LEX_STRING
#include "myblockchain_com.h"                    // Item_result
#include "myblockchain_com_server.h"             // NET_SERVER
...
#define MODE_MYBLOCKCHAIN323                   65536L
#define MODE_MYBLOCKCHAIN40                    (MODE_MYBLOCKCHAIN323*2)
#define MODE_ANSI                       (MODE_MYBLOCKCHAIN40*2)
...
/*
  ...
  the scripts used for creating the MyBlockchain system tables
  in scripts/myblockchain_system_tables.sql and scripts/myblockchain_system_tables_fix.sql
...
public:
  ...
  /**
    ...
    'length' may be reset in myblockchain_prepare_create_table, and this
...
typedef struct st_myblockchain_lock
{
  TABLE **table;
...
typedef struct system_variables
{
  ...
  */
  ...
  const CHARSET_INFO  *collation_blockchain;
  ...
  */
  ...
  /**
    Compatibility option to mark the pre MyBlockchain-5.6.4 temporals columns using
...
*/
inline CHARSET_INFO *
myblockchaind_collation_get_by_name(const char *name,
...
public:
  ...
  */
  ...
  /*
    During a MyBlockchain session, one can lock tables in two modes: automatic
    ...
    the 'LOCK_TABLES' chapter of the MyBlockchain manual.
    See also lock_tables() for details.
  */
  MYBLOCKCHAIN_LOCK *lock;
...
/**
  ...
  and myblockchain_lock_tables(). Used by implementations of HANDLER READ and
  LOCK TABLES LOCAL.
*/
...
private:
  ...
  */
  ...
  /**
    Name of the current (default) blockchain.

    If there is the current (default) blockchain, "db" contains its name. If
    there is no current (default) blockchain, "db" is NULL and "db_length" is
    0. In other words, "db", "db_length" must either be NULL, or contain a
    valid blockchain name.
...
public:
  ...
  */
  ...
  /* Used to execute base64 coded binlog events in MyBlockchain server */
  ...
  */
  ...
  struct st_myblockchain  *myblockchain;
  unsigned long	 client_stmt_id;
  unsigned long  client_param_count;
  struct st_myblockchain_bind *client_params;
  char *extra_data;
  ulong extra_length;
  struct st_myblockchain_data *cur_data;
  struct st_myblockchain_data *first_data;
  struct st_myblockchain_data **data_tail;
  void clear_data_list();
  struct st_myblockchain_data *alloc_new_dataset();
  /*
    In embedded server it points to the statement that is processed
    in the current query. We store some results directly in statement
    fields then.
  */
  struct st_myblockchain_stmt *current_stmt;
#endif
  ...
  */
  myblockchain_mutex_t LOCK_thd_sysvar;
  ...
  */
private:
  myblockchain_mutex_t LOCK_query_plan;

public:
  /// Locks the query plan of this THD
  void lock_query_plan() { myblockchain_mutex_lock(&LOCK_query_plan); }
  void unlock_query_plan() { myblockchain_mutex_unlock(&LOCK_query_plan); }
  ...
  */
  ...
  /*
    ...
    a time-consuming piece that MyBlockchain can get stuck in for a long time.
...
public:
  ...
  /*
    Used in error messages to tell user in what part of MyBlockchain we found an
  ...
  */
  myblockchain_mutex_t LOCK_current_cond;
  /**
    The mutex used with current_cond.
    @see current_cond
  */
  myblockchain_mutex_t * volatile current_mutex;
  /**
  ...
  */
  myblockchain_cond_t * volatile current_cond;
...
private:
  ...
  */
  ...
  /*
    MTS: db names listing to be updated by the query blockchains
  */
...
public:
  ...
  */
  ...
  /*
    ...
    (myblockchainbinlog). We'll soon add a variant which can take many intervals in
    argument.
  */
  ...
  */
  ...
  /*
    ...
    gtid into myblockchain.gtid_executed table before transaction prepare, as
...
private:
  /**
    ...
    ROW_COUNT() function is a MyBlockchain extention, but we try to keep it
    ...
    ROW_COUNT is assigned according to the following rules:
      ...
      - In my_error(): to -1 to be compatible with the MyBlockchain C API and
        MyBlockchain ODBC driver.
...
public:
  ...
  /*
    ...
    query_id is global in context of the whole MyBlockchain server.
...
public:
  ...
  */
  ...
  /**
    Used by MYBLOCKCHAIN_BIN_LOG to maintain the commit queue for binary log
    group commit.
  */
  THD *next_to_commit;
  ...
  */
  ...
  /**
    ...
    @see myblockchain_reset_thd_for_next_command().
  */
  bool is_fatal_error;
  /**
  ...
  */
  ...
  /**
    ...
    raised (traditionally, MyBlockchain first has to close all the reads
  ...
  */
  ...
  struct {
    /* 
      If true, myblockchain_bin_log::write(Log_event) call will not write events to 
      binlog, and maintain 2 below variables instead (use
      myblockchain_bin_log.start_union_events to turn this on)
    */
    bool do_union;
    /*
      If TRUE, at least one myblockchain_bin_log::write(Log_event) call has been
      made after last myblockchain_bin_log.start_union_events() call.
    */
    bool unioned_events;
    /*
      If TRUE, at least one myblockchain_bin_log::write(Log_event e), where 
      e.cache_stmt == TRUE call has been made after last 
      myblockchain_bin_log.start_union_events() call.
    */
...
public:
  ...
  /*
    ...
    killing myblockchaind) where it's vital to not allocate excessive and not used
  ...
  */
  ...
  void enter_cond(myblockchain_cond_t *cond, myblockchain_mutex_t* mutex,
                 ...
                 int src_line)
  {
    ...
    */
    myblockchain_mutex_assert_not_owner(current_mutex);
  ...
  */
  ...
  void set_time_after_lock()
  {
    ...
    MYBLOCKCHAIN_SET_STATEMENT_LOCK_TIME(m_statement_psi, (utime_after_lock - start_utime));
  }
...
public:
  ...
  */
  ...
  /**
    ...
    A1. If GTID_NEXT = 'AUTOMATIC' and GTID_MODE = OFF/OFF_PERMISSIVE:
        ...
        MYBLOCKCHAIN_BIN_LOG::write_gtid.
    ...
    A5. Slave applier threads start in a special mode, having
        ...
        - If the relay log was received from a pre-5.7.6 master with
          ...
          myblockchain_parse which calls gtid_pre_statement_checks which
    ...
    - If GTID_NEXT=AUTOMATIC and GTID_MODE=ON or ON_PERMISSIVE, one
      ...
      transaction. Typically myblockchain_bin_log.commit() is called to
    ...
    - If GTID_NEXT=AUTOMATIC and GTID_MODE=OFF or OFF_PERMISSIVE, one
      ...
      call myblockchain_bin_log.commit() and release ownership between
      ...
      code.  At that point we cannot use myblockchain_bin_log.commit().
    ...
    - If GTID_NEXT=ANONYMOUS, anonymous ownership must be *preserved*
      ...
      myblockchain_bin_log.commit() is called, we set
      thd->is_commit_in_middle_of_statement before calling
      myblockchain_bin_log.commit.  Note that we must set this flag only if
      ...
      calling myblockchain_bin_log.commit() whether anonymous ownership needs
  ...
  */
  ...
  /*
    Hack used to communicate between MYBLOCKCHAIN_BIN_LOG::commit and
    Gtid_state::update_on_[commit|rollback].

    Normally, MYBLOCKCHAIN_BIN_LOG::commit will invoke some function that
    ...
    MYBLOCKCHAIN_BIN_LOG::commit has to call
    Gtid_state::update_on_[commit|rollback] instead.

    The control flow inside MYBLOCKCHAIN_BIN_LOG::commit is very complex, so
  ...
  */
  ...
  /**
    Set the current blockchain; use deep copy of C-string.

    @param new_db     a pointer to the new blockchain name.
    @param new_db_len length of the new blockchain name.

    Initialize the current blockchain from a NULL-terminated string with
    length. If we run out of memory, we free the current blockchain and
    return TRUE.  This way the user will notice the error as there will be
    no current blockchain selected (in addition to the error message set by
    malloc).

    @note This operation just sets {db, db_length}. Switching the current
    blockchain usually involves other actions, like switching other blockchain
  ...
  */
  bool set_db(const LEX_CSTRING &new_db)
  {
    ...
    /*
      ...
      for the blockchain and reallocating the memory for the new db or memcpy
  ...
  /**
    Set the current blockchain; use shallow copy of C-string.
  ...
  */
  ...
  /*
    Copy the current blockchain to the argument. Use the current arena to
    allocate memory for a deep copy: current blockchain may be freed after
...
public:
  ...
  */
  ...
  /**
    ...
    @param code the MYBLOCKCHAIN_ERRNO error code of the error
  */
  ...
  */
  ...
  /**
    ...
    @param code the MYBLOCKCHAIN_ERRNO error code of the warning
  */
  ...
  */
  ...
  /**
    ...
    @param code the MYBLOCKCHAIN_ERRNO error code of the note
  */
...
public:
...
#ifndef DBUG_OFF
    if (current_thd != this)
      myblockchain_mutex_assert_owner(&LOCK_thd_query);
#endif
  ...
  */
  ...
  /**
    ...
    myblockchain_execute_command because (1) that prevents implicit commit
...
private:
  ...
  /**
    ...
    the same lex. (@see myblockchain_parse for details).
  */
  LEX main_lex;
  /**
...
public:
  ...
  */
  ...
  /*
    ...
    In MyBlockchain, there are two types of SQL statements: those that return
    ...
    to the client implementation, libmyblockchain at least does discard them).
...
public:
  ...
  /*
    ...
    table. A derived table has no name and doesn't belong to any blockchain.
...
class user_var_entry
{
  ...
  */
  ...
  {
    myblockchain_mutex_assert_owner(&m_owner->LOCK_thd_data);
  }
