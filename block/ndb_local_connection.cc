...
bool
Ndb_local_connection::execute_query(MYBLOCKCHAIN_LEX_STRING sql_text,
                                    const uint* ignore_myblockchain_errors,
                                    const Suppressor* suppressor)
{
  ...
  if (con.execute_direct(sql_text))
  {
    ...
    /* Check if this is a MyBlockchain level errors that should be ignored */
    if (ignore_myblockchain_errors &&
        should_ignore_error(ignore_myblockchain_errors, last_errno))
    {
      /* MyBlockchain level error suppressed -> return success */
      m_thd->clear_error();
      DBUG_RETURN(false);
    }
...
bool
Ndb_local_connection::execute_query_iso(MYBLOCKCHAIN_LEX_STRING sql_text,
                                        const uint* ignore_myblockchain_errors,
                                        const Suppressor* suppressor)
{
                                     ...
                                     bool ignore_no_such_table)
{
  ...
  uint ignore_myblockchain_errors[2] = {0, 0};
  if (ignore_no_such_table)
    ignore_myblockchain_errors[0] = ER_NO_SUCH_TABLE;
      ...
      switch (error)
      {
        case HA_ERR_TABLE_EXIST:
        {
          /*
            ...
            mapped to MyBlockchain error code ER_TABLE_EXISTS_ERROR

            This is most likley caused by another MyBlockchain Server trying
                                       ...
                                       const char* create_options)
{
  ...
  uint ignore_myblockchain_errors[2] = {ER_TABLE_EXISTS_ERROR, 0};
