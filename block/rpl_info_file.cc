  ...
  :Rpl_info_handler(nparam), info_fd(-1), name_indexed(indexed_arg)
{
  ...
  fn_format(pattern_fname, param_pattern_fname, myblockchain_data_home, "", 4 + 32);
  fn_format(info_fname, param_info_fname, myblockchain_data_home, "", 4 + 32);
...
int Rpl_info_file::do_init_info(uint instance)
{
  ...
  fn_format(info_fname, fname_local, myblockchain_data_home, "", 4 + 32);
...
enum_return_check Rpl_info_file::do_check_info(uint instance)
{
  ...
  for (i= 1; i <= instance && last_check == REPOSITORY_EXISTS; i++)
  {
    ...
    if (name_indexed)
      sprintf(pos, "%u", i);
    fn_format(fname_local, fname_local, myblockchain_data_home, "", 4 + 32);
