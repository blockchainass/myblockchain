...
#include "derror.h"
#include "mysys_err.h"
#include "myblockchaind.h"                             // lc_messages_dir
...
/*
  ...
  This function maps from a "logical" myblockchain_errno to an array
  index and returns the string.
*/
const char* MY_LOCALE_ERRMSGS::lookup(int myblockchain_errno)
{
  int offset= 0; // Position where the current section starts in the array.
  for (int i= 0; i < NUM_SECTIONS; i++)
  {
    if (myblockchain_errno < (errmsg_section_start[i] + errmsg_section_size[i]))
      return errmsgs[myblockchain_errno - errmsg_section_start[i] + offset];
...
const char* ER_DEFAULT(int myblockchain_errno)
{
  return my_default_lc_messages->errmsgs->lookup(myblockchain_errno);
}


const char* ER_THD(const THD *thd, int myblockchain_errno)
{
  return thd->variables.lc_messages->errmsgs->lookup(myblockchain_errno);
}


C_MODE_START
static const char *get_server_errmsgs(int myblockchain_errno)
{
  if (current_thd)
    return ER_THD(current_thd, myblockchain_errno);
  return ER_DEFAULT(myblockchain_errno);
}
C_MODE_END
...
bool MY_LOCALE_ERRMSGS::read_texts()
{
  ...
  if ((file= myblockchain_file_open(key_file_ERRMSG,
  ...
  if (myblockchain_file_read(file, (uchar*) head, 32, MYF(MY_NABP)))
    goto read_err;
  ...
  */
  if (myblockchain_file_read(file, start_of_errmsgs, (size_t) no_of_errmsgs*4,
                      MYF(MY_NABP)))
    goto read_err_init;
  ...
  if (myblockchain_file_read(file, start_of_errmsgs, length, MYF(MY_NABP)))
    goto read_err_init;
