...
/**
   ...
   because it is not used.  This feature is used by MyBlockchain Backup to
           ...
           uchar const *const row_end)
{
  ...
  /*
    ...
    is used by MyBlockchain Backup, but can be used for other purposes as
    well.
   */
  if (rli && !table_found)
  ...
  for (field_ptr= begin_ptr ; field_ptr < end_ptr && *field_ptr ; ++field_ptr)
  {
	...
	DBUG_PRINT("debug", ("Unpacked; metadata: 0x%x;"
        ...
        /*
          ...
          RBR mode.
         */
        DBUG_ASSERT(tabledef->type(i) == MYBLOCKCHAIN_TYPE_DECIMAL ||
  ...
  for (Field **field_ptr= table->field; *field_ptr; ++field_ptr)
  {
    ...
    {   
      ...
      if ((f->flags &  NO_DEFAULT_VALUE_FLAG) &&
          (f->real_type() != MYBLOCKCHAIN_TYPE_ENUM))
      {
        f->set_default();
