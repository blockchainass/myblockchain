...
typedef struct st_myblockchain MYBLOCKCHAIN;
...
/*
  ...
  MASTER, RESET SLAVE, end_slave() (when myblockchaind stops) [init_slave() does not
  ...
  position) and relay log (MYBLOCKCHAIN_BIN_LOG object).
  ...
  In MYBLOCKCHAIN_BIN_LOG: LOCK_log, LOCK_index of the binlog and the relay log
  ...
  then we write F2's name in parentheses in the list of locks for F1.
    ...
    MYBLOCKCHAIN_BIN_LOG::new_file_impl:
      .LOCK_log, .LOCK_index,
...
*/
#define MYBLOCKCHAIN_SLAVE_NOT_RUN         0
#define MYBLOCKCHAIN_SLAVE_RUN_NOT_CONNECT 1
#define MYBLOCKCHAIN_SLAVE_RUN_CONNECT     2
...
int register_slave_on_master(MYBLOCKCHAIN* myblockchain);
...
/*
  ...
  myblockchain_cond_wait() on start_cond, start_lock
*/
int start_slave_thread(
                       ...
                       myblockchain_mutex_t *start_lock,
...
int fetch_master_table(THD* thd, const char* db_name, const char* table_name,
		       Master_info* mi, MYBLOCKCHAIN* myblockchain, bool overwrite);
