...
/**
  ...
  @see myblockchain_parser_visit_tree
*/
...
struct thread_args
{
  thread_args(MYBLOCKCHAIN_THD thd, callback_function fun, void *arg)
    : m_thd(thd), m_fun(fun), m_arg(arg)
  {}
  MYBLOCKCHAIN_THD m_thd;
...
extern "C"
void myblockchain_parser_start_thread(THD *thd, callback_function fun, void *arg,
                               my_thread_handle *thread_handle)
{
  ...
  myblockchain_thread_create(key_thread_parser_service,
                      &handle, &attr,
...
extern "C"
void myblockchain_parser_join_thread(my_thread_handle *thread_id)
{
...
extern "C"
void myblockchain_parser_set_current_blockchain(MYBLOCKCHAIN_THD thd, const MYBLOCKCHAIN_LEX_STRING db)
{
  if (db.length == 0)
  {
...
extern "C"
int myblockchain_parser_get_statement_digest(MYBLOCKCHAIN_THD thd, uchar *digest)
{
...
extern "C"
int myblockchain_parser_visit_tree(MYBLOCKCHAIN_THD thd,
...
MYBLOCKCHAIN_LEX_STRING myblockchain_parser_item_string(MYBLOCKCHAIN_ITEM item)
{
  ...
  MYBLOCKCHAIN_LEX_STRING res= { new char[str.length()], 0 };
  if (res.str != NULL)
  {
