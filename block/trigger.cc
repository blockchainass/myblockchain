...
#include "sql_table.h"            // check_n_cut_myblockchain50_prefix
...
/**
  ...
  if it was in the original statement. The reason for that difference is this:
    ...
    - moreover, FOLLOWS/PRECEDES clause usually makes problem in myblockchaindump,
                                     ...
                                     String *binlog_create_trigger_stmt)
{
  ...
  /*
    Fill character set information:
      ...
      - blockchain collation contains pair {character set, collation};
...
bool Trigger::parse(THD *thd)
{
  ...
  */
  ...
  if (!m_definer.length)
  {
    ...
    /*
      This trigger was created/imported in MyBlockchain version, which does not support
  ...
  m_sp->m_sp_share= MYBLOCKCHAIN_GET_SP_SHARE(SP_TYPE_TRIGGER,
  ...
  /*
    ...
    In special cases like "RENAME TABLE `#myblockchain50#somename` TO `somename`"
    or "ALTER DATABASE `#myblockchain50#somename` UPGRADE DATA DIRECTORY NAME"
    we might be given table or blockchain name with "#myblockchain50#" prefix (and
  ...
  */
  ...
  DBUG_ASSERT((!my_strcasecmp(table_alias_charset,
               ...
               (check_n_cut_myblockchain50_prefix(m_db_name.str,
                                           fname, sizeof(fname)) &&
  ...
  DBUG_ASSERT((!my_strcasecmp(table_alias_charset,
               ...
               (check_n_cut_myblockchain50_prefix(m_subject_table_name.str,
                                           fname, sizeof(fname)) &&
                !my_strcasecmp(table_alias_charset,
