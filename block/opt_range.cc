...
static bool fields_ok_for_partition_index(Field **pfield)
{
  ...
  for (; (*pfield); pfield++)
  {
    ...
    if (ftype == MYBLOCKCHAIN_TYPE_ENUM || ftype == MYBLOCKCHAIN_TYPE_GEOMETRY)
      return FALSE;
  }
  return TRUE;
}
                                ...
                                const Item *value)
{
  ...
  */
  if (value->result_type() == STRING_RESULT &&
      value->field_type() == MYBLOCKCHAIN_TYPE_JSON)
    return false;
                                             ...
                                             MEM_ROOT *memroot)
{
  ...
  */
  ...
  case TYPE_ERR_BAD_VALUE:
    /*
      In the case of incompatible values, MyBlockchain's SQL dialect has some
  ...
  case TYPE_NOTE_TIME_TRUNCATED:
         ...
         comp_op == Item_func::LT_FUNC || comp_op == Item_func::LE_FUNC))
    {
      /*
        ...
        In MyBlockchain's SQL dialect, DATE and DATETIME are compared as datetime
  ...
  */
  {
    const Field::geometry_type save_geom_type=
      ...
      Field::GEOM_GEOMETRY;
    if (field->type() == MYBLOCKCHAIN_TYPE_GEOMETRY)
    {
