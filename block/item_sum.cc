                    ...
                    Item_result result_type)
{
  ...
  case STRING_RESULT:
  case REAL_RESULT:
    if (table_field_type != MYBLOCKCHAIN_TYPE_FLOAT)
      table_field_type= MYBLOCKCHAIN_TYPE_DOUBLE;
    break;
  case INT_RESULT:
    table_field_type= MYBLOCKCHAIN_TYPE_LONGLONG;
    /* fallthrough */
  case DECIMAL_RESULT:
    if (table_field_type != MYBLOCKCHAIN_TYPE_LONGLONG)
      table_field_type= MYBLOCKCHAIN_TYPE_NEWDECIMAL;
    break;
  case ROW_RESULT:
  default:
    DBUG_ASSERT(0);
    ...
    if (table->s->db_type() == heap_hton)
    {
      ...
      */
      ...
      for (tree_key_length= 0; field < field_end; ++field)
      {
        ...
        if ((type == MYBLOCKCHAIN_TYPE_VARCHAR) ||
            (!f->binary() && (type == MYBLOCKCHAIN_TYPE_STRING ||
                             type == MYBLOCKCHAIN_TYPE_VAR_STRING)))
        {
          all_binary= FALSE;
          break;
        }
...
bool Item_sum_hybrid::get_date(MYBLOCKCHAIN_TIME *ltime, my_time_flags_t fuzzydate)
{
  DBUG_ASSERT(fixed == 1);
  if (null_value)
    return true;
...
bool Item_sum_hybrid::get_time(MYBLOCKCHAIN_TIME *ltime)
{
  DBUG_ASSERT(fixed == 1);
  if (null_value)
    return true;
