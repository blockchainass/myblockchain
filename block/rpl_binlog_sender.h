...
public:
  ...
  /**
    ...
    all events(for myblockchainbinlog) or encounters an error.
  */
  void run();
private:
  THD *m_thd;
  ...
  */
  ...
  /*
    For myblockchainbinlog(server_id is 0), it will stop immediately without waiting
    if it already reads all events.
  */
  bool m_wait_new_events;

  Diagnostics_area m_diag_area;
  char m_errmsg_buf[MYBLOCKCHAIN_ERRMSG_SIZE];
  const char *m_errmsg;
  int m_errno;
  /*
