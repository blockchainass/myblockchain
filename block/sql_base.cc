...
#include "lock.h"        // myblockchain_lock_remove,
                         // myblockchain_unlock_tables,
                         // myblockchain_lock_have_duplicate
#include "sql_show.h"    // append_identifier
#include "strfunc.h"     // find_type
#include "sql_view.h"    // myblockchain_make_view, VIEW_ANY_ACL
#include "sql_parse.h"   // check_table_access
#include "auth_common.h" // *_ACL, check_grant_all_columns,
                         // check_column_grant_in_table_ref,
                         // get_column_grant
#include "sql_handler.h" // myblockchain_ha_flush
...
static void init_tdc_psi_keys(void)
{
  ...
  myblockchain_mutex_register(category, all_tdc_mutexes, count);

  count= array_elements(all_tdc_conds);
  myblockchain_cond_register(category, all_tdc_conds, count);
}
...
bool table_def_init(void)
{
  ...
  init_tdc_psi_keys();
#endif
  myblockchain_mutex_init(key_LOCK_open, &LOCK_open, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(key_COND_open, &COND_open);
  oldest_unused_share= &end_of_unused_share;
  end_of_unused_share.prev= &oldest_unused_share;

  if (table_cache_manager.init())
  {
    myblockchain_cond_destroy(&COND_open);
    myblockchain_mutex_destroy(&LOCK_open);
    return true;
  }
                             ...
                             my_hash_value_type hash_value)
{
  ...
  */
                       ...
                       key_length))))
  {
    ...
    myblockchain_cond_wait(&COND_open, &LOCK_open);
  }
  ...
  */
  ...
  myblockchain_cond_broadcast(&COND_open);
                         ...
                         bool wait_for_refresh, ulong timeout)
{
  ...
  if (thd->locked_tables_mode)
  {
    ...
    */
    ...
    myblockchain_ha_flush_tables(thd, tables_to_reopen);
...
bool close_temporary_tables(THD *thd)
{
  ...
  */
  myblockchain_ha_rm_temporary_tables(thd);
  if (!myblockchain_bin_log.is_open())
  {
    ...
    {
      tmp_next= t->next;
      myblockchain_lock_remove(thd, thd->lock, t);
  ...
  */
  ...
  for (table= thd->temporary_tables; table; table= next)
  {
    if (is_user_table(table))
    {
      ...
      /* Loop forward through all tables that belong to a common blockchain
...
/**
  Find temporary table specified by blockchain and table names in the
...
/**
  ...
  @param  db_name     Name of blockchain for this table
...
/**
  ...
  @param[in]    flags flags MYBLOCKCHAIN_OPEN_FORCE_SHARED_MDL,
                          MYBLOCKCHAIN_OPEN_FORCE_SHARED_HIGH_PRIO_MDL or
                          MYBLOCKCHAIN_OPEN_FAIL_ON_MDL_CONFLICT
                          @sa open_table().
                        ...
                        MDL_ticket **mdl_ticket)
{
  ...
  if (flags & (MYBLOCKCHAIN_OPEN_FORCE_SHARED_MDL |
               MYBLOCKCHAIN_OPEN_FORCE_SHARED_HIGH_PRIO_MDL))
  {
    /*
      MYBLOCKCHAIN_OPEN_FORCE_SHARED_MDL flag means that we are executing
      ...
      MYBLOCKCHAIN_OPEN_FORCE_SHARED_HIGH_PRIO_MDL flag means that we open
    ...
    */
    DBUG_ASSERT(!(flags & MYBLOCKCHAIN_OPEN_FORCE_SHARED_MDL) ||
                !(flags & MYBLOCKCHAIN_OPEN_FORCE_SHARED_HIGH_PRIO_MDL));

    MDL_REQUEST_INIT_BY_KEY(&new_mdl_request,
                            &mdl_request->key,
                            (flags & MYBLOCKCHAIN_OPEN_FORCE_SHARED_MDL) ?
  ...
  if (flags & MYBLOCKCHAIN_OPEN_FAIL_ON_MDL_CONFLICT)
  {
    /*
...
/**
  ...
  @param ot_ctx         Context with flags which modify how open works
                        ...
                        MYBLOCKCHAIN_OPEN_IGNORE_FLUSH - Open table even if
                        someone has done a flush. No version number
                        checking is done.
                        MYBLOCKCHAIN_OPEN_HAS_MDL_LOCK - instead of acquiring
...
bool open_table(THD *thd, TABLE_LIST *table_list, Open_table_context *ot_ctx)
{
  ...
  */
  ...
  if (!(flags & MYBLOCKCHAIN_OPEN_IGNORE_KILLED) && thd->killed)
    DBUG_RETURN(TRUE);
  ...
  */
  if (table_list->mdl_request.is_write_lock_request() &&
      thd->tx_read_only &&
      !(flags & (MYBLOCKCHAIN_LOCK_LOG_TABLE | MYBLOCKCHAIN_OPEN_HAS_MDL_LOCK)))
  {
  ...
  */
  if (thd->locked_tables_mode &&
      ! (flags & MYBLOCKCHAIN_OPEN_GET_NEW_TABLE))
  {						// Using table locks
    TABLE *best_table= 0;
  ...
  if (! (flags & MYBLOCKCHAIN_OPEN_HAS_MDL_LOCK))
  {
    /*
    ...
    */
    if (table_list->mdl_request.is_write_lock_request() &&
        ! (flags & (MYBLOCKCHAIN_OPEN_IGNORE_GLOBAL_READ_LOCK |
                    MYBLOCKCHAIN_OPEN_FORCE_SHARED_MDL |
                    MYBLOCKCHAIN_OPEN_FORCE_SHARED_HIGH_PRIO_MDL |
                    MYBLOCKCHAIN_OPEN_SKIP_SCOPED_MDL_LOCK)) &&
...
retry_share:
  {
    ...
    */
    ...
    if (table)
    {
      ...
      if (!(flags & MYBLOCKCHAIN_OPEN_IGNORE_FLUSH))
      {
        /*
  ...
  */
  if (share->is_view)
  {
    ...
    */
    ...
    /* Open view */
    if (myblockchain_make_view(thd, share, table_list, false))
      goto err_unlock;
...
Locked_tables_list::init_locked_tables(THD *thd)
{
  ...
  {
    /**
      Allocate an auxiliary array to pass to myblockchain_lock_tables()
...
unlink_all_closed_tables(THD *thd, MYBLOCKCHAIN_LOCK *lock, size_t reopen_count)
{
  /* If we managed to take a lock, unlock tables and free the lock. */
  if (lock)
    myblockchain_unlock_tables(thd, lock);
  /*
    If a failure happened in reopen_tables(), we may have succeeded
    reopening some tables, but not all.
    This works when the connection was killed in myblockchain_lock_tables().
  */
  if (reopen_count)
  {
    while (reopen_count--)
...
Locked_tables_list::reopen_tables(THD *thd)
{
  ...
  if (reopen_count)
  {
    ...
    /*
      We re-lock all tables with myblockchain_lock_tables() at once rather
    ...
    */
    lock= myblockchain_lock_tables(thd, m_reopen_array, reopen_count,
                            MYBLOCKCHAIN_OPEN_REOPEN);
    thd->in_lock_tables= 0;
    if (lock == NULL || (merged_lock=
                         myblockchain_lock_merge(thd->lock, lock)) == NULL)
    {
                   ...
                   const char *cache_key, size_t cache_key_length, uint flags)
{
  ...
  if (share->is_view &&
      !myblockchain_make_view(thd, share, table_list, (flags & OPEN_VIEW_NO_PARSE)))
  {
    release_table_share(share);
    myblockchain_mutex_unlock(&LOCK_open);
    return FALSE;
...
Open_table_context::Open_table_context(THD *thd, uint flags)
  :m_thd(thd),
   ...
   m_timeout(flags & MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT ?
                       ...
                       TABLE_LIST *table)
{
  /*
    ...
    * We met a broken table that needs repair, or a table that
      is not present on this MyBlockchain server and needs re-discovery.
                                       ...
                                       bool routine_modifies_data)
{
  ...
  */
  bool log_on= myblockchain_bin_log.is_open() && thd->variables.sql_log_bin;
  ...
  */
  if (tables->schema_table)
  {
    ...
    */
    ...
    if (!myblockchain_schema_table(thd, lex, tables) &&
                 ...
                 ulong lock_wait_timeout, uint flags)
{
       ...
       table= table->next_global)
  {
    ...
    if (table->mdl_request.type != MDL_SHARED_READ_ONLY)
    {
      ...
      if (! (flags & MYBLOCKCHAIN_OPEN_SKIP_SCOPED_MDL_LOCK) &&
  ...
  //          statement, indicated by the MYBLOCKCHAIN_OPEN_SKIP_SCOPED_MDL_LOCK flag.
  if (!(flags & MYBLOCKCHAIN_OPEN_SKIP_SCOPED_MDL_LOCK) && !thd->tablespace_op)
  {
      ...
      */
           ...
           sroutine_to_open= &rt->next, rt= rt->next)
      {
        ...
        if (error)
        {
          ...
          /*
            Serious error during reading stored routines from myblockchain.proc table.
...
/**
  ...
  @param[in]    flags           options to be used while opening and locking
                                table (see open_table(), myblockchain_lock_tables())
  @param[in]    prelocking_strategy  Strategy which specifies how prelocking
                                     algorithm should work for this statement.
...
/*
  ...
  SYNOPSIS
    ...
    lock_flags          Flags passed to myblockchain_lock_table
                   ...
                   uint lock_flags)
{
  ...
  if (!error)
  {
    ...
    */
	...
	if (! (thd->lock= myblockchain_lock_tables(thd, &table_list->table, 1,
                                            lock_flags)))
        {
          table= 0;
        }
...
/**
  ...
  @param flags                Bitmap of options to be used to open and lock
                              tables (see open_tables() and myblockchain_lock_tables()
                              for details).
...
/**
  ...
  @param       flags    bitmap of flags to modify how the tables will be open:
                        MYBLOCKCHAIN_LOCK_IGNORE_FLUSH - open table even if someone has
                        done a flush on it.
...
/**
  ...
  @param  flags         Options (see myblockchain_lock_tables() for details)
  ...
  */
  if (! thd->locked_tables_mode)
  {
    ...
    */
    ...
    if (! (thd->lock= myblockchain_lock_tables(thd, start, (uint) (ptr - start),
                                        flags)))
      DBUG_RETURN(TRUE);
...
bool rm_temporary_table(handlerton *base, const char *path)
{
  ...
  if (myblockchain_file_delete(key_file_frm, frm_path, MYF(0)))
  ...
  if (nj_col->view_field)
  {
    ...
    if (nj_col->table_ref->schema_table_reformed)
    {
      /*
        ...
        already('myblockchain_schema_table' function). So we can return
...
/*
  ...
  SYNOPSIS
    ...
    db_name                [in]  optional blockchain name that qualifies the
                        ...
                        bool register_tree_change, TABLE_LIST **actual_table)
{
  ...
  /*
    Check that the table and blockchain that qualify the current field name
      ...
      */ 
      ...
      if (table_name)
      {
        /*
          ...
          We use strcmp for table names and blockchain names as these may be
...
/*
  ...
  SYNOPSIS
    ...
    db_name		Database name in case of 'blockchain_name.table_name.*'
...
my_bool myblockchain_rm_tmp_tables(void)
{
  uint i, idx;
  ...
  TABLE_SHARE share;
  THD *thd;
  DBUG_ENTER("myblockchain_rm_tmp_tables");
  ...
  {
    tmpdir=myblockchain_tmpdir_list.list[i];
    ...
    for (idx=0 ; idx < dirp->number_off_files ; idx++)
    {
          ...
          !memcmp(file->name, tmp_file_prefix, tmp_file_prefix_length))
      {
        ...
        /*
          ...
          files(MYF(0)).
        */
        (void) myblockchain_file_delete(key_file_misc, filePath, MYF(0));
      }
    }
    my_dirend(dirp);
  }
...
{
  uint counter;
  uint flags= MYBLOCKCHAIN_OPEN_IGNORE_FLUSH | MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT;
...
/**
  A helper function to close a myblockchain.* table opened
  ...
  itself, since myblockchain.* tables are not transactional
...
void
close_myblockchain_tables(THD *thd)
{
...
{
  uint flags= ( MYBLOCKCHAIN_OPEN_IGNORE_GLOBAL_READ_LOCK |
                MYBLOCKCHAIN_LOCK_IGNORE_GLOBAL_READ_ONLY |
                MYBLOCKCHAIN_OPEN_IGNORE_FLUSH |
                MYBLOCKCHAIN_LOCK_IGNORE_TIMEOUT |
                MYBLOCKCHAIN_LOCK_LOG_TABLE);
  TABLE *table;
  /* Save value that is changed in myblockchain_lock_tables() */
