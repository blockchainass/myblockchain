...
#include "lock.h"                // myblockchain_unlock_some_tables
...
JOIN::optimize()
{
  ...
  */
      ...
      !(select_lex->active_options() & SELECT_NO_UNLOCK))
  {
    ...
    myblockchain_unlock_some_tables(thd, ct, const_tables);
  }
      ...
      rollup.state == ROLLUP::STATE_NONE)
  {
    ...
    */
	...
	if (all_order_fields_used)
	{
	  ...
	  {
	    /*
	      Force MyBlockchain to read the table in sorted order to get result in
	      ORDER BY order.
	    */
                                  ...
                                  void *table_join_idx)
{
  ...
  /*
    ...
    but from e.g. myblockchain_delete() or myblockchain_update(). In these cases
...
static uint get_tmp_table_rec_length(List<Item> &items)
{
  ...
  while ((item= it++))
  {
    ...
    case STRING_RESULT:
      ...
      if (item->is_temporal() || item->field_type() == MYBLOCKCHAIN_TYPE_GEOMETRY)
        len += 8;
      else
        len += item->max_length;
      break;
  ...
  case Item::FIELD_ITEM:
    {
      ...
      */
      return item_field->field->part_of_key.is_set(keyno) &&
             item_field->field->type() != MYBLOCKCHAIN_TYPE_GEOMETRY &&
             item_field->field->type() != MYBLOCKCHAIN_TYPE_BLOB;
    }
  case Item::REF_ITEM:
    ...
    else if (tl->table->reginfo.join_tab)
    {
      ...
      */
      if (value[0]->result_type() == STRING_RESULT &&
          value[0]->field_type() == MYBLOCKCHAIN_TYPE_JSON)
      {
        warn_index_not_applicable(stat->join()->thd, field, possible_keys);
        return;
      }
      ...
      (join_tab->type() != JT_REF_OR_NULL))
  {
	...
	return (field->eq_def(((Item_field *) right_item)->field));
      ...
      if (right_item->const_item() && !(right_item->is_null()))
      {
        ...
        */
        if (!((field->type() == MYBLOCKCHAIN_TYPE_STRING ||                       // 1
               field->type() == MYBLOCKCHAIN_TYPE_VARCHAR) && field->binary()) &&
            !(field->type() == MYBLOCKCHAIN_TYPE_FLOAT && field->decimals() > 0)) // 2
        {
...
internal_remove_eq_conds(THD *thd, Item *cond, Item::cond_result *cond_value)
{
	   ...
	   ((Item_func*) cond)->functype() == Item_func::ISNULL_FUNC)
  {
    ...
    if (args[0]->type() == Item::FIELD_ITEM)
    {
      ...
      */
      if (((field->type() == MYBLOCKCHAIN_TYPE_DATE) ||
  ...
  *all_order_by_fields_used= 1;
  ...
  while ((item=li++))
  {
	...
	return 0;
      ...
      if (item->type() == Item::FIELD_ITEM &&
          item->field_type() == MYBLOCKCHAIN_TYPE_BIT)
      {
        /*
...
static uint32 get_key_length_tmp_table(Item *item)
{
  ...
  if (type == MYBLOCKCHAIN_TYPE_BLOB ||
      type == MYBLOCKCHAIN_TYPE_VARCHAR ||
      type == MYBLOCKCHAIN_TYPE_GEOMETRY)
