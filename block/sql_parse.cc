...
#include "opt_explain.h"      // myblockchain_explain_other
#include "opt_trace.h"        // Opt_trace_start
#include "partition_info.h"   // partition_info
#include "probes_myblockchain.h"     // MYBLOCKCHAIN_COMMAND_START
#include "rpl_filter.h"       // rpl_filter
#include "rpl_master.h"       // register_slave
#include "rpl_rli.h"          // myblockchain_show_relaylog_events
#include "rpl_slave.h"        // change_master_cmd
#include "sp.h"               // sp_create_routine
#include "sp_cache.h"         // sp_cache_enforce_limit
#include "sp_head.h"          // sp_head
#include "sql_admin.h"        // myblockchain_assign_to_keycache
#include "sql_analyse.h"      // Query_result_analyse
#include "sql_audit.h"        // MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_CHANGE_USER
#include "sql_base.h"         // find_temporary_table
#include "sql_binlog.h"       // myblockchain_client_binlog_statement
#include "sql_cache.h"        // query_cache
#include "sql_connect.h"      // decrease_user_connections
#include "sql_db.h"           // myblockchain_change_db
#include "sql_delete.h"       // myblockchain_delete
#include "sql_do.h"           // myblockchain_do
#include "sql_handler.h"      // myblockchain_ha_rm_tables
#include "sql_help.h"         // myblockchaind_help
#include "sql_insert.h"       // Query_result_create
#include "sql_load.h"         // myblockchain_load
#include "sql_prepare.h"      // myblockchain_stmt_execute
...
#include "sql_table.h"        // myblockchain_create_table
#include "sql_tablespace.h"   // myblockchain_alter_tablespace
#include "sql_test.h"         // myblockchain_print_status
#include "sql_select.h"       // handle_query
#include "sql_load.h"         // myblockchain_load
#include "sql_servers.h"      // create_servers, alter_servers,
                              // drop_servers, servers_reload
#include "sql_handler.h"      // myblockchain_ha_open, myblockchain_ha_close,
                              // myblockchain_ha_read
#include "sql_binlog.h"       // myblockchain_client_binlog_statement
#include "sql_do.h"           // myblockchain_do
#include "sql_help.h"         // myblockchaind_help
...
#include "sp_cache.h"
#include "events.h"
#include "sql_trigger.h"      // myblockchain_create_or_drop_trigger
...
#include "sql_update.h"       // myblockchain_update
#include "sql_view.h"         // myblockchain_create_view
#include "table_cache.h"      // table_cache_manager
...
/**
  Checks whether the event for the given blockchain, db, should
...
void execute_init_command(THD *thd, LEX_STRING *init_command,
                          myblockchain_rwlock_t *var_lock)
{
  ...
  myblockchain_rwlock_rdlock(var_lock);
  if (!init_command->length)
  {
    myblockchain_rwlock_unlock(var_lock);
    return;
  }
...
bool do_command(THD *thd)
{
  ...
  */
  ...
  if (rc)
  {
    ...
    thd->m_statement_psi= MYBLOCKCHAIN_REFINE_STATEMENT(thd->m_statement_psi,
    ...
    MYBLOCKCHAIN_END_STATEMENT(thd->m_statement_psi, thd->get_stmt_da());
...
/**
  ...
  This is a helper function to myblockchain_execute_command.

  @note SQLCOM_UPDATE_MULTI is an exception and delt with elsewhere.

  @see myblockchain_execute_command
  @returns Status code
                                                ...
                                                TABLE_LIST *all_tables)
{
  ...
  const my_bool create_or_drop_blockchains=
    (lex->sql_command == SQLCOM_CREATE_DB) ||
    (lex->sql_command == SQLCOM_DROP_DB);

  if (update_real_tables || create_or_drop_blockchains)
  {
      /*
                      ...
                      enum enum_server_command command)
{
  ...
  MYBLOCKCHAIN_COMMAND_START(thd->thread_id(), command,
  ...
  */
  ...
  case COM_INIT_DB:
  {
    ...
    if (!myblockchain_change_db(thd, tmp_cstr, FALSE))
    {
  ...
  case COM_CHANGE_USER:
  {
    ...
    MYBLOCKCHAIN_AUDIT_NOTIFY_CONNECTION_CHANGE_USER(thd);
    if (auth_rc)
    {
  ...
  {
    myblockchaind_stmt_execute(thd, com_data->com_stmt_execute.stmt_id,
  ...
  case COM_STMT_FETCH:
  {
    myblockchaind_stmt_fetch(thd, com_data->com_stmt_fetch.stmt_id,
                      com_data->com_stmt_fetch.num_rows);
    break;
  }
  case COM_STMT_SEND_LONG_DATA:
  {
    myblockchain_stmt_get_longdata(thd, com_data->com_stmt_send_long_data.stmt_id,
  ...
  {
    myblockchaind_stmt_prepare(thd, com_data->com_stmt_prepare.query,
                        com_data->com_stmt_prepare.length);
    break;
  }
  case COM_STMT_CLOSE:
  {
    myblockchaind_stmt_close(thd, com_data->com_stmt_close.stmt_id);
    break;
  }
  case COM_STMT_RESET:
  {
    myblockchaind_stmt_reset(thd, com_data->com_stmt_reset.stmt_id);
    break;
  }
  case COM_QUERY:
  {
    ...
    MYBLOCKCHAIN_QUERY_START(const_cast<char*>(thd->query().str), thd->thread_id(),
    ...
    MYBLOCKCHAIN_SET_STATEMENT_TEXT(thd->m_statement_psi, thd->query().str,
                             thd->query().length);
    ...
    myblockchain_parse(thd, &parser_state);
           ...
           ! thd->is_error())
    {
      ...
      */
      ...
      myblockchain_audit_general(thd, MYBLOCKCHAIN_AUDIT_GENERAL_STATUS,
                          thd->get_stmt_da()->is_error() ?
                          thd->get_stmt_da()->myblockchain_errno() : 0,
                          command_name[command].str);
...
/* DTRACE end */
      if (MYBLOCKCHAIN_QUERY_DONE_ENABLED())
      {
        MYBLOCKCHAIN_QUERY_DONE(thd->is_error());
      }
...
/* DTRACE begin */
      MYBLOCKCHAIN_QUERY_START(const_cast<char*>(beginning_of_next_stmt),
                        thd->thread_id(),
                        (char *) (thd->db().str ? thd->db().str : ""),
                        (char *) thd->security_context()->priv_user().str,
                        (char *) thd->security_context()->host_or_ip().str);
...
/* PSI begin */
      ...
      thd->m_statement_psi= MYBLOCKCHAIN_START_STATEMENT(&thd->m_statement_state,
                                          com_statement_info[command].m_key,
                                          thd->db().str, thd->db().length,
                                          thd->charset(), NULL);
      THD_STAGE_INFO(thd, stage_starting);
      MYBLOCKCHAIN_SET_STATEMENT_TEXT(thd->m_statement_psi, beginning_of_next_stmt, length);
  ...
  case COM_FIELD_LIST:				// This isn't actually needed
  {
    ...
    */
    ...
    myblockchaind_list_fields(thd,&table_list,fields);
  ...
  case COM_SHUTDOWN:
  {
    ...
    /*
      ...
      packet[0].
    */
    enum myblockchain_enum_shutdown_level level;
    if (!thd->is_valid_time())
  ...
  case COM_PROCESS_KILL:
  {
    if (thd_manager->get_thread_id() & (~0xfffffffful))
      my_error(ER_DATA_OUT_OF_RANGE, MYF(0), "thread_id", "myblockchain_kill()");
    else
    {
  ...
  case COM_SET_OPTION:
  {
    ...
    case (int) MYBLOCKCHAIN_OPTION_MULTI_STATEMENTS_ON:
      //TODO: access of protocol_classic should be removed
      thd->get_protocol_classic()->add_client_capability(
          CLIENT_MULTI_STATEMENTS);
      my_eof(thd);
      break;
    case (int) MYBLOCKCHAIN_OPTION_MULTI_STATEMENTS_OFF:
  ...
  if (!thd->is_error() && !thd->killed_errno())
    myblockchain_audit_general(thd, MYBLOCKCHAIN_AUDIT_GENERAL_RESULT, 0, 0);
  ...
  if (MYBLOCKCHAIN_QUERY_DONE_ENABLED() && command == COM_QUERY)
  {
    MYBLOCKCHAIN_QUERY_DONE(thd->is_error());
  }
  if (MYBLOCKCHAIN_COMMAND_DONE_ENABLED())
  {
    MYBLOCKCHAIN_COMMAND_DONE(thd->is_error());
  }
...
/**
  This is a wrapper for MYBLOCKCHAIN_BIN_LOG::gtid_end_transaction. For normal
...
static inline void binlog_gtid_end_transaction(THD *thd)
{
  ...
  */
  if ((thd->lex->sql_command == SQLCOM_COMMIT ||
    ...
    (void) myblockchain_bin_log.gtid_end_transaction(thd);
...
int
myblockchain_execute_command(THD *thd)
{
  int res= FALSE;
  ...
  DBUG_ENTER("myblockchain_execute_command");
  ...
  */
  if (sql_command_flags[lex->sql_command] & CF_HA_CLOSE)
    myblockchain_ha_rm_tables(thd, all_tables);
...
case SQLCOM_PREPARE:
  {
    myblockchain_sql_stmt_prepare(thd);
    break;
  }
  case SQLCOM_EXECUTE:
  {
    myblockchain_sql_stmt_execute(thd);
    break;
  }
  case SQLCOM_DEALLOCATE_PREPARE:
  {
    myblockchain_sql_stmt_close(thd);
    break;
  }
  case SQLCOM_DO:
    res= myblockchain_do(thd, lex);
    break;
  ...
  case SQLCOM_HELP:
    res= myblockchaind_help(thd,lex->help_arg);
    break;
  ...
  {
    res= myblockchaind_show_warnings(thd, (ulong)
  ...
  case SQLCOM_SHOW_RELAYLOG_EVENTS:
  {
    if (check_global_access(thd, REPL_SLAVE_ACL))
      goto error;
    res = myblockchain_show_relaylog_events(thd);
    break;
  }
  case SQLCOM_SHOW_BINLOG_EVENTS:
  {
    if (check_global_access(thd, REPL_SLAVE_ACL))
      goto error;
    res = myblockchain_show_binlog_events(thd);
    break;
  }
#endif
  ...
  case SQLCOM_ASSIGN_TO_KEYCACHE:
  {
    ...
    if (check_access(thd, INDEX_ACL, first_table->db,
                     ...
                     0, 0))
      goto error;
    res= myblockchain_assign_to_keycache(thd, first_table, &lex->ident);
    break;
  }
  case SQLCOM_PRELOAD_KEYS:
  {
    ...
    if (check_access(thd, INDEX_ACL, first_table->db,
                     ...
                     0, 0))
      goto error;
    res = myblockchain_preload_keys(thd, first_table);
    break;
  }
#ifdef HAVE_REPLICATION
  ...
  case SQLCOM_CREATE_TABLE:
  {
    ...
    /*
      Code below (especially in myblockchain_create_table() and Query_result_create
    ...
    if (select_lex->item_list.elements)		// With select
    {
      ...
      */
      ...
      if (thd->query_name_consts && 
          ...
          !myblockchain_bin_log.is_query_in_union(thd, thd->query_id))
      {
...
"section of the manual.");
    ...
    else
    {
      ...
      {
        /* CREATE TABLE ... LIKE ... */
        res= myblockchain_create_like_table(thd, create_table, select_tables,
                                     &create_info);
      }
      else
      {
        /* Regular CREATE TABLE */
        res= myblockchain_create_table(thd, create_table,
...
end_with_restore_list:
  ...
  */
  {
    ...
    */
    ...
    create_info.default_table_charset= thd->variables.collation_blockchain;
    ...
    res= myblockchain_alter_table(thd, first_table->db, first_table->table_name,
  ...
  */
  ...
  case SQLCOM_RENAME_TABLE:
  {
    ...
    if (myblockchain_rename_tables(thd, first_table, 0))
      goto error;
    break;
  }
#ifndef EMBEDDED_LIBRARY
  ...
  case SQLCOM_SHOW_CREATE:
    DBUG_ASSERT(first_table == all_tables && first_table != 0);
    {
      ...
      */
      ...
      res= myblockchaind_show_create(thd, first_table);
      break;
    }
  case SQLCOM_CHECKSUM:
  {
    ...
    res = myblockchain_checksum_table(thd, first_table, &lex->check_opt);
    break;
  }
  case SQLCOM_REPLACE:
#ifndef DBUG_OFF
    if (myblockchain_bin_log.is_open())
    {
      ...
      */
      ...
      DBUG_EXECUTE_IF("incident_blockchain_resync_on_replace",
      ...
      if (incident)
      {
        ...
        if (myblockchain_bin_log.write_incident(&ev, true/*need_lock_log=true*/,
                                         err_msg))
        {
          res= 1;
          break;
  ...
  case SQLCOM_DROP_TABLE:
  {
    ...
    res= myblockchain_rm_table(thd, first_table, lex->drop_if_exists,
			lex->drop_temporary);
  ...
  case SQLCOM_SHOW_PRIVILEGES:
    res= myblockchaind_show_privileges(thd);
    break;
  ...
  case SQLCOM_CHANGE_DB:
  {
    ...
    if (!myblockchain_change_db(thd, db_str, FALSE))
      my_ok(thd);
  ...
  case SQLCOM_LOAD:
  {
    ...
    res= myblockchain_load(thd, lex->exchange, first_table, lex->load_field_list,
  ...
  case SQLCOM_UNLOCK_TABLES:
    /*
      It is critical for myblockchaindump --single-transaction --master-data that
      UNLOCK TABLES does not implicitely commit a connection which has only
      done FLUSH TABLES WITH READ LOCK + BEGIN. If this assumption becomes
      false, myblockchaindump will not work.
    */
  ...
  {
    /*
      As myblockchain_create_db() may modify HA_CREATE_INFO structure passed to
    ...
    */
    ...
    if (check_access(thd, CREATE_ACL, lex->name.str, NULL, NULL, 1, 0))
      break;
    res= myblockchain_create_db(thd,(lower_case_table_names == 2 ? alias :
  ...
  case SQLCOM_DROP_DB:
  {
    ...
    */
    ...
    if (check_access(thd, DROP_ACL, lex->name.str, NULL, NULL, 1, 0))
      break;
    res= myblockchain_rm_db(thd, to_lex_cstring(lex->name), lex->drop_if_exists, 0);
    break;
  }
  case SQLCOM_ALTER_DB_UPGRADE:
  {
    ...
    res= myblockchain_upgrade_db(thd, db_name);
    if (!res)
      my_ok(thd);
    break;
  }
  case SQLCOM_ALTER_DB:
  {
    ...
    */
    ...
    if (check_access(thd, ALTER_ACL, db->str, NULL, NULL, 1, 0))
      break;
    res= myblockchain_alter_db(thd, db->str, &create_info);
    break;
  }
  case SQLCOM_SHOW_CREATE_DB:
  {
    ...
    if (check_and_convert_db_name(&lex->name, TRUE) != IDENT_NAME_OK)
      break;
    res= myblockchaind_show_create_db(thd, lex->name.str, &lex->create_info);
    break;
  }
  ...
  {
    if (check_access(thd, INSERT_ACL, "myblockchain", NULL, NULL, 1, 0))
      break;
#ifdef HAVE_DLOPEN
    if (!(res = myblockchain_create_function(thd, &lex->udf)))
      my_ok(thd);
#else
  ...
  {
    if (check_access(thd, INSERT_ACL, "myblockchain", NULL, NULL, 1, 1) &&
        check_global_access(thd,CREATE_USER_ACL))
      break;
    /* Conditionally writes to binlog */
    HA_CREATE_INFO create_info(lex->create_info);
    if (!(res = myblockchain_create_user(thd, lex->users_list, create_info.options & HA_LEX_CREATE_IF_NOT_EXISTS)))
      my_ok(thd);
    break;
  }
  case SQLCOM_DROP_USER:
  {
    if (check_access(thd, DELETE_ACL, "myblockchain", NULL, NULL, 1, 1) &&
        check_global_access(thd,CREATE_USER_ACL))
      break;
    /* Conditionally writes to binlog */
    if (!(res = myblockchain_drop_user(thd, lex->users_list, lex->drop_if_exists)))
      my_ok(thd);
    break;
  }
  case SQLCOM_RENAME_USER:
  {
    if (check_access(thd, UPDATE_ACL, "myblockchain", NULL, NULL, 1, 1) &&
        check_global_access(thd,CREATE_USER_ACL))
      break;
    /* Conditionally writes to binlog */
    if (!(res= myblockchain_rename_user(thd, lex->users_list)))
      my_ok(thd);
    break;
  }
  case SQLCOM_REVOKE_ALL:
  {
    ...
    /* Conditionally writes to binlog */
    if (!(res = myblockchain_revoke_all(thd, lex->users_list)))
      my_ok(thd);
    break;
  }
  case SQLCOM_REVOKE:
  case SQLCOM_GRANT:
  {
    ...
    if (first_table)
    {
	  ...
	  goto error;
        /* Conditionally writes to binlog */
        res= myblockchain_routine_grant(thd, all_tables,
	...
	if (check_grant(thd,(lex->grant | lex->grant_tot_col | GRANT_ACL),
	  ...
	  goto error;
        /* Conditionally writes to binlog */
        res= myblockchain_table_grant(thd, all_tables, lex->users_list,
	...
	my_message(ER_ILLEGAL_GRANT_FOR_TABLE, ER(ER_ILLEGAL_GRANT_FOR_TABLE),
      ...
      {
        /* Conditionally writes to binlog */
        res = myblockchain_grant(thd, select_lex->db, lex->users_list, lex->grant,
                          lex->sql_command == SQLCOM_REVOKE,
  ...
  case SQLCOM_SHOW_GRANTS:
  {
    ...
    if (!strcmp(thd->security_context()->priv_user().str,
                grant_user->user.str) ||
        !check_access(thd, SELECT_ACL, "myblockchain", NULL, NULL, 1, 0))
    {
      res = myblockchain_show_grants(thd, grant_user);
    }
    break;
  }
  case SQLCOM_SHOW_CREATE_USER:
  {
    ...
    if (!strcmp(thd->security_context()->priv_user().str, show_user->user.str) ||
        !check_access(thd, SELECT_ACL, "myblockchain", NULL, NULL, 1, 0))
      res= myblockchain_show_create_user(thd, show_user);
    break;
  }
#endif
  case SQLCOM_BEGIN:
  ...
  case SQLCOM_CREATE_SPFUNCTION:
  {
    ...
    /*
      Verify that the blockchain name is allowed, optionally
      lowercase it.
    */
    ...
    if (! (res= sp_create_routine(thd, lex->sphead)))
    {
      ...
      */
      ...
      /*
        ...
        insert routine privileges to myblockchain.procs_priv. If invoker is not
        available then consider using definer.

        Check if the definer exists on slave,
        then use definer privilege to insert routine privileges to myblockchain.procs_priv.

        For current user of SQL thread has GLOBAL_ACL privilege,
        which doesn't any check routine privileges,
        so no routine privilege record  will insert into myblockchain.procs_priv.
      */
  ...
  case SQLCOM_DROP_FUNCTION:
    {
          ...
          ! lex->spname->m_explicit_name)
      {
        ...
        if (udf)
        {
          if (check_access(thd, DELETE_ACL, "myblockchain", NULL, NULL, 1, 0))
            goto error;

          if (!(res = myblockchain_drop_function(thd, &lex->spname->m_name)))
          {
            my_ok(thd);
            break;
          }
  ...
  case SQLCOM_CREATE_VIEW:
    {
      ...
      */
      res= myblockchain_create_view(thd, first_table, thd->lex->create_view_mode);
      break;
    }
  case SQLCOM_DROP_VIEW:
    {
      if (check_table_access(thd, DROP_ACL, all_tables, FALSE, UINT_MAX, FALSE))
        goto error;
      /* Conditionally writes to binlog. */
      res= myblockchain_drop_view(thd, first_table, thd->lex->drop_mode);
      break;
    }
  case SQLCOM_CREATE_TRIGGER:
  {
    /* Conditionally writes to binlog. */
    res= myblockchain_create_or_drop_trigger(thd, all_tables, 1);
  ...
  {
    /* Conditionally writes to binlog. */
    res= myblockchain_create_or_drop_trigger(thd, all_tables, 0);
    break;
  }
  case SQLCOM_ALTER_TABLESPACE:
    if (check_global_access(thd, CREATE_TABLESPACE_ACL))
      break;
    if (!(res= myblockchain_alter_tablespace(thd, lex->alter_tablespace_info)))
      my_ok(thd);
    break;
  case SQLCOM_BINLOG_BASE64_EVENT:
  {
#ifndef EMBEDDED_LIBRARY
    myblockchain_client_binlog_statement(thd);
  ...
  case SQLCOM_EXPLAIN_OTHER:
  {
    ...
    myblockchain_explain_other(thd);
    break;
  }
  case SQLCOM_ANALYZE:
  case SQLCOM_CHECK:
  ...
  case SQLCOM_ALTER_USER:
  {
    ...
    /* Conditionally writes to binlog */
    if (!(res = myblockchain_alter_user(thd, lex->users_list, lex->drop_if_exists)))
      my_ok(thd);
    break;
  }
#endif
...
finish:
  ...
  // ./mtr --mem --myblockchaind='-T 4096' --valgrind-myblockchaind main.1st
...
static bool execute_sqlcom_select(THD *thd, TABLE_LIST *all_tables)
{
  ...
  if (!(res= open_tables_for_query(thd, all_tables, 0)))
  {
    MYBLOCKCHAIN_SELECT_START(const_cast<char*>(thd->query().str));
    if (lex->is_explain())
    {
      /*
    ...
    MYBLOCKCHAIN_SELECT_DONE((int) res, (ulong) thd->limit_found_rows);
  }
			 ...
			 uchar *buf __attribute__((unused)))
{
      ...
      (long) (my_thread_stack_size - margin))
  {
    ...
    */
    char* ebuff= new (std::nothrow) char[MYBLOCKCHAIN_ERRMSG_SIZE];
    if (ebuff) {
      my_snprintf(ebuff, MYBLOCKCHAIN_ERRMSG_SIZE, ER(ER_STACK_OVERRUN_NEED_MORE),
...
/**
  ...
  @todo Remove myblockchain_reset_thd_for_next_command and only use the
  member function.

  @todo Call it after we use THD for queries, not before.
*/
void myblockchain_reset_thd_for_next_command(THD *thd)
{
...
void THD::reset_for_next_command()
{
  ...
  THD *thd= this;
  DBUG_ENTER("myblockchain_reset_thd_for_next_command");
...
void myblockchain_init_multi_delete(LEX *lex)
{
...
/*
  When you modify myblockchain_parse(), you may need to mofify
  myblockchain_test_parse_for_slave() in this same file.
*/
...
void myblockchain_parse(THD *thd, Parser_state *parser_state)
{
  int error __attribute__((unused));
  DBUG_ENTER("myblockchain_parse");
  DBUG_PRINT("myblockchain_parse", ("query: '%s'", thd->query().str));
  ...
  /*
    ...
    - lex_start()
    - myblockchain_reset_thd_for_next_command()
  ...
  if (query_cache.send_result_to_client(thd, thd->query()) <= 0)
  {
    ...
    if (!err)
    {
      /*
        ...
        Sub-routines of myblockchain_rewrite_query() should try to only rewrite when
      ...
      */
      ...
      {
        myblockchain_rewrite_query(thd);
	...
	if (! thd->is_error())
	{
          ...
          */
                       ...
                       lex->sql_command != SQLCOM_ALTER_USER))
          {
            ...
            error= myblockchain_execute_command(thd);

          MYBLOCKCHAIN_QUERY_EXEC_DONE(error);
	}
      }
    }
    else
...
bool myblockchain_test_parse_for_slave(THD *thd)
{
  LEX *lex= thd->lex;
  bool ignorable= false;
  sql_digest_state *parent_digest= thd->m_digest;
  PSI_statement_locker *parent_locker= thd->m_statement_psi;
  DBUG_ENTER("myblockchain_test_parse_for_slave");
...
static uint kill_one_thread(THD *thd, my_thread_id id, bool only_kill_query)
{
  ...
  if (tmp)
  {
    ...
    */
    ...
    myblockchain_mutex_unlock(&tmp->LOCK_thd_data);
  }
...
public:
  ...
  {
    myblockchain_mutex_lock(&thd_to_kill->LOCK_thd_data);
  ...
  && !(thd_to_kill->security_context()->check_access(SUPER_ACL))
    ...
    myblockchain_mutex_unlock(&thd_to_kill->LOCK_thd_data);
  }
};
...
/*
  Check if path does not contain myblockchain data home directory
  SYNOPSIS
...
int test_if_data_home_dir(const char *dir)
{
  ...
  dir_len= strlen(path);
  if (myblockchain_unpacked_real_data_home_len<= dir_len)
  {
    if (dir_len > myblockchain_unpacked_real_data_home_len &&
        path[myblockchain_unpacked_real_data_home_len] != FN_LIBCHAR)
      DBUG_RETURN(0);
    ...
    if (lower_case_file_system)
    {
      if (!my_strnncoll(default_charset_info, (const uchar*) path,
                        ...
                        (const uchar*) myblockchain_unpacked_real_data_home,
                        myblockchain_unpacked_real_data_home_len))
        DBUG_RETURN(1);
    }
    else if (!memcmp(path, myblockchain_unpacked_real_data_home,
                     myblockchain_unpacked_real_data_home_len))
      DBUG_RETURN(1);
  }
  DBUG_RETURN(0);
...
extern int MYBLOCKCHAINparse(class THD *thd); // from sql_yacc.cc


/**
  This is a wrapper of MYBLOCKCHAINparse(). All the code should call parse_sql()
  instead of MYBLOCKCHAINparse().
...
//  DBUG_ASSERT(thd->lex->m_sql_cmd == NULL);
  ...
  MYBLOCKCHAIN_QUERY_PARSE_START(const_cast<char*>(thd->query().str));
  ...
  {
    /* Start Digest */
    parser_state->m_digest_psi= MYBLOCKCHAIN_DIGEST_START(thd->m_statement_psi);
  ...
  */
  ...
  bool myblockchain_parse_status= MYBLOCKCHAINparse(thd) != 0;
  ...
  */
  ...
  if (parser_da->current_statement_cond_count() != 0)
  {
    ...
    */
    ...
    {
      da->set_error_status(parser_da->myblockchain_errno(),
  ...
  /*
    Check that if MYBLOCKCHAINparse() failed either thd->is_error() is set, or an
  ...
  */
  ...
  DBUG_ASSERT(!myblockchain_parse_status ||
              (myblockchain_parse_status && thd->is_error()) ||
              (myblockchain_parse_status && thd->get_internal_handler()));
  ...
  ret_value= myblockchain_parse_status || thd->is_fatal_error;
      ...
      (parser_state->m_digest_psi != NULL))
  {
    ...
    */
    DBUG_ASSERT(thd->m_digest != NULL);
    MYBLOCKCHAIN_DIGEST_END(parser_state->m_digest_psi,
                     & thd->m_digest->m_digest_storage);
  }

  MYBLOCKCHAIN_QUERY_PARSE_DONE(ret_value);
