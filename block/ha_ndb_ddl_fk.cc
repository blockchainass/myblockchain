 ...
 * myblockchain frequently uses LEX-string...(ptr + len)
 *
...
// copied from unused table_case_convert() in myblockchaind.h
static void
...
class Fk_util
{
  ...
  {
    va_list args;
    char msg[MYBLOCKCHAIN_ERRMSG_SIZE];
    va_start(args,fmt);
  ...
  error(const NdbDictionary::Dictionary* dict, const char* fmt, ...) const
  {
    ...
    char ndb_msg[MYBLOCKCHAIN_ERRMSG_SIZE] = {0};
    if (dict)
    {
                      ...
                      const char* new_parent_name) const
  {
    ...
    */
    ...
    for (unsigned i = 0; i < table_list.count; i++)
    {
      ...
      // Check if table is in same blockchain as the potential new parent
      if (strcmp(new_parent_db, el.blockchain) != 0)
      {
        DBUG_PRINT("info", ("Skip, '%s.%s' is in different blockchain",
                            el.blockchain, el.name));
        continue;
      }
...
ha_ndbcluster::create_fks(THD *thd, Ndb *ndb)
{
  ...
  // return real myblockchain error to avoid total randomness..
...
cmp_fk_name(const void * _e0, const void * _e1)
{
  ...
  if ((res= strcmp(e0->blockchain, e1->blockchain)) != 0)
    return res;
  ...
  for (unsigned i = 0; i < obj_list.count; i++)
  {
    if (obj_list.elements[i].type == NdbDictionary::Object::ForeignKey)
    {
          ...
          strcmp(db_and_name, src_db) == 0)
      {
         ...
         *   it should remain consistent since myblockchain does not
