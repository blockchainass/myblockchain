...
/**
  ...
  @retval
    TRUE  error
*/
bool myblockchain_create_or_drop_trigger(THD *thd, TABLE_LIST *tables, bool create)
{
  /*
    ...
    'myblockchain_create_or_drop_trigger', compared to two separate functions
    'myblockchain_create_trigger' and 'myblockchain_drop_trigger' is not apparent.
  ...
  */
  ...
  DBUG_ENTER("myblockchain_create_or_drop_trigger");

  /* Charset of the buffer for statement must be system one. */
  stmt_query.set_charset(system_charset_info);

  /*
    QQ: This function could be merged in myblockchain_alter_table() function
    But do we want this ?
  */
  ...
  */
  ...
  /*
    We don't allow creating triggers on tables in the 'myblockchain' schema
  */
  if (create && !my_strcasecmp(system_charset_info, "myblockchain", tables->db))
  {
  ...
  /*
    ...
    applies to them too.
  */
  if (!trust_function_creators && myblockchain_bin_log.is_open() &&
  ...
  */
  ...
  if (!result)
  {
    ...
    MYBLOCKCHAIN_DROP_SP(SP_TYPE_TRIGGER,
...
/**
  ...
  @param[in] db_name        Current blockchain of subject table
  @param[in] table_alias    Current alias of subject table
  @param[in] table_name     Current name of subject table
  @param[in] new_db_name    New blockchain for subject table
                               ...
                               const char *new_table_name)
{
  ...
  /*
    ...
    In case of "ALTER DATABASE `#myblockchain50#db1` UPGRADE DATA DIRECTORY NAME"
    we will be given table name with "#myblockchain50#" prefix
    To remove this prefix we use check_n_cut_myblockchain50_prefix().
  */
  ...
  if (my_strcasecmp(table_alias_charset, db_name, new_db_name))
  {
    ...
    if (check_n_cut_myblockchain50_prefix(db_name, dbname, sizeof(dbname)) &&
  ...
  /*
    This method interfaces the myblockchain server code protected by
