...
#include "myblockchain.h"                             // MYBLOCKCHAIN
...
// Sic: Must be after myblockchaind.h to get the right ER macro.
...
static int safe_connect(THD* thd, MYBLOCKCHAIN* myblockchain, Master_info* mi);
static int safe_reconnect(THD* thd, MYBLOCKCHAIN* myblockchain, Master_info* mi,
                          bool suppress_warnings);
static int connect_to_master(THD* thd, MYBLOCKCHAIN* myblockchain, Master_info* mi,
                             bool reconnect, bool suppress_warnings);
static int get_master_version_and_clock(MYBLOCKCHAIN* myblockchain, Master_info* mi);
static int get_master_uuid(MYBLOCKCHAIN *myblockchain, Master_info *mi);
int io_thread_init_commands(MYBLOCKCHAIN *myblockchain, Master_info *mi);
static Log_event* next_event(Relay_log_info* rli);
int queue_event(Master_info* mi,const char* buf,ulong event_len);
static int terminate_slave_thread(THD *thd,
                                  myblockchain_mutex_t *term_lock,
                                  myblockchain_cond_t *term_cond,
...
/*
  ...
  of slave_max_allowed_packet.
    ...
    @in_param    myblockchain  MyBlockchain connection handle
*/

static void set_slave_max_allowed_packet(THD *thd, MYBLOCKCHAIN *myblockchain)
{
  DBUG_ENTER("set_slave_max_allowed_packet");
  // thd and myblockchain must be valid
  DBUG_ASSERT(thd && myblockchain);

  thd->variables.max_allowed_packet= slave_max_allowed_packet;
  /*
    Adding MAX_LOG_EVENT_HEADER_LEN to the max_packet_size on the I/O
    thread and the myblockchain->option max_allowed_packet, since a
  ...
  */
  ...
  /*
    Skipping the setting of myblockchain->net.max_packet size to slave
    max_allowed_packet since this is done during myblockchain_real_connect.
  */
  myblockchain->options.max_allowed_packet=
    slave_max_allowed_packet+MAX_LOG_EVENT_HEADER;
  DBUG_VOID_RETURN;
}
...
void lock_slave_threads(Master_info* mi)
{
  ...
  myblockchain_mutex_lock(&mi->run_lock);
  myblockchain_mutex_lock(&mi->rli->run_lock);
  DBUG_VOID_RETURN;
}
...
void unlock_slave_threads(Master_info* mi)
{
  ...
  //TODO: see if we can do this without dual mutex
  myblockchain_mutex_unlock(&mi->rli->run_lock);
  myblockchain_mutex_unlock(&mi->run_lock);
  DBUG_VOID_RETURN;
}
...
static void init_slave_psi_keys(void)
{
  ...
  myblockchain_thread_register(category, all_slave_threads, count);

  count= array_elements(all_slave_memory);
  myblockchain_memory_register(category, all_slave_memory, count);
}
...
int init_slave()
{
  ...
  /*
    This is called when myblockchaind starts. Before client connections are
...
int start_slave(THD *thd)
{
  ...
  else
  {
    ...
    */
    myblockchain_mutex_lock(&LOCK_sql_slave_skip_counter);
    ...
    if (sql_slave_skip_counter > 0 && !(thd->lex->slave_thd_opt & SLAVE_IO))
    {
      ...
      myblockchain_mutex_unlock(&LOCK_sql_slave_skip_counter);
      goto err;
    }
    myblockchain_mutex_unlock(&LOCK_sql_slave_skip_counter);
                           ...
                           my_off_t *master_log_pos)
{
  ...
  if (myblockchain_file_close(file, MYF(MY_WME)))
    DBUG_RETURN(ERROR);
  if (end_io_cache(&log))
  {
...
int flush_master_info(Master_info* mi, bool force)
{
  ...
  */
  myblockchain_mutex_t *log_lock= mi->rli->relay_log.get_log_lock();
...
/*
  ...
  NOTES
    Called from get_options() in myblockchaind.cc on start-up
*/
                            ...
                            ulong stop_wait_timeout, bool need_lock_term)
{
  ...
  myblockchain_mutex_t *sql_lock = &mi->rli->run_lock, *io_lock = &mi->run_lock;
  myblockchain_mutex_t *log_lock= mi->rli->relay_log.get_log_lock();
  /*
                       ...
                       bool need_lock_term)
{
  ...
  if (need_lock_term)
  {
    myblockchain_mutex_lock(term_lock);
  }
  else
  {
    myblockchain_mutex_assert_owner(term_lock);
  }
  if (!*slave_running)
  {
    if (need_lock_term)
    {
      ...
      */
      myblockchain_mutex_unlock(term_lock);
      DBUG_RETURN(0);
    }
    else
    {
  ...
  */
  ...
  while (*slave_running)                        // Should always be true
  {
    ...
    */
    ...
    int error=
#endif
      myblockchain_cond_timedwait(term_cond, term_lock, &abstime);
...
#endif
                       my_start_routine h_func, myblockchain_mutex_t *start_lock,
                       myblockchain_mutex_t *cond_lock,
                       myblockchain_cond_t *start_cond,
                       volatile uint *slave_running,
                       ...
                       Master_info* mi)
{
  ...
  if (start_lock)
    myblockchain_mutex_lock(start_lock);
  if (!server_id)
  {
    if (start_cond)
      myblockchain_cond_broadcast(start_cond);
    if (start_lock)
      myblockchain_mutex_unlock(start_lock);
  ...
  if ((error= myblockchain_thread_create(thread_key,
  ...
  if (start_cond && cond_lock) // caller has cond_lock
  {
    ...
    while (start_id == *slave_run_id && thd != NULL)
    {
      ...
      */
      if (!thd->killed)
        myblockchain_cond_wait(start_cond, cond_lock);
      myblockchain_mutex_unlock(cond_lock);
      thd->EXIT_COND(& saved_stage);
      myblockchain_mutex_lock(cond_lock); // re-acquire it
      if (thd->killed)
      {
        if (start_lock)
          myblockchain_mutex_unlock(start_lock);
...
{
  myblockchain_mutex_t *lock_io=0, *lock_sql=0, *lock_cond_io=0, *lock_cond_sql=0;
  myblockchain_cond_t* cond_io=0, *cond_sql=0;
  int error=0;
...
/**
  ...
  If there is an error, then this function calls myblockchain_free_result;
  otherwise the MYBLOCKCHAIN object holds the result after this call.  If
  there is an error other than allowed_error, then this function
  prints a message and returns -1.

  @param myblockchain MYBLOCKCHAIN object.
  ...
  myblockchain_store_result() will be called and the result stored in this pointer.
  @param[out] master_row If this is not NULL and there is no error, then
  myblockchain_fetch_row() will be called and the result stored in this pointer.
...
io_thread_init_command(Master_info *mi, const char *query, int allowed_error,
                       MYBLOCKCHAIN_RES **master_res= NULL,
                       MYBLOCKCHAIN_ROW *master_row= NULL)
{
  DBUG_ENTER("io_thread_init_command");
  DBUG_PRINT("info", ("IO thread initialization command: '%s'", query));
  MYBLOCKCHAIN *myblockchain= mi->myblockchain;
  int ret= myblockchain_real_query(myblockchain, query, static_cast<ulong>(strlen(query)));
  ...
  if (ret != 0)
  {
    int err= myblockchain_errno(myblockchain);
    myblockchain_free_result(myblockchain_store_result(myblockchain));
    if (!err || err != allowed_error)
    {
      mi->report(is_network_error(err) ? WARNING_LEVEL : ERROR_LEVEL, err,
                 ...
                 query, myblockchain_error(myblockchain));
      DBUG_RETURN(COMMAND_STATUS_ERROR);
    }
  ...
  if (master_res != NULL)
  {
    if ((*master_res= myblockchain_store_result(myblockchain)) == NULL)
    {
      mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
                 "The slave IO thread stops because the initialization query "
    ...
    if (master_row != NULL)
    {
      if ((*master_row= myblockchain_fetch_row(*master_res)) == NULL)
      {
        myblockchain_free_result(*master_res);
        mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
                   "The slave IO thread stops because the initialization query "
...
/**
  ...
  @param  myblockchain MYBLOCKCHAIN to request uuid from master.
...
err:
  if (myblockchain_errno(myblockchain) && is_network_error(myblockchain_errno(myblockchain)))
  {
    mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
               "The initialization command '%s' failed with the following"
               " error: '%s'.", query, myblockchain_error(myblockchain));
    ret= 2;
  }
  else
  {
...
static int get_master_uuid(MYBLOCKCHAIN *myblockchain, Master_info *mi)
{
  ...
  if (!myblockchain_real_query(myblockchain,
                        STRING_WITH_LEN("SHOW GLOBAL VARIABLES LIKE 'SERVER_UUID'")) &&
      (master_res= myblockchain_store_result(myblockchain)) &&
      (master_row= myblockchain_fetch_row(master_res)))
  {
    if (!strcmp(::server_uuid, master_row[1]) &&
        !mi->rli->replicate_same_server_id)
    {
      errmsg= "The slave I/O thread stops because master and slave have equal "
              "MyBlockchain server UUIDs; these UUIDs must be different for "
              "replication to work.";
      mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER(ER_SLAVE_FATAL_ERROR),
                 errmsg);
      // Fatal error
  ...
  else if (myblockchain_errno(myblockchain))
  {
    if (is_network_error(myblockchain_errno(myblockchain)))
    {
      mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
...
static int get_master_version_and_clock(MYBLOCKCHAIN* myblockchain, Master_info* mi)
{
  ...
  version_number= atoi(myblockchain->server_version);

  MYBLOCKCHAIN_RES *master_res= 0;
  MYBLOCKCHAIN_ROW master_row;
  ...
  */
  ...
  if (!my_isdigit(&my_charset_bin,*myblockchain->server_version))
  {
    errmsg = "Master reported unrecognized MyBlockchain version";
  ...
  {
    /*
      Note the following switch will bug when we have MyBlockchain branch 30 ;)
    */
    ...
    case 3:
      mi->set_mi_description_event(new
        Format_description_log_event(1, myblockchain->server_version));
      break;
    case 4:
      mi->set_mi_description_event(new
        Format_description_log_event(3, myblockchain->server_version));
      break;
    default:
      /*
        Master is MyBlockchain >=5.0. Give a default Format_desc event, so that we can
      ...
      */
      mi->set_mi_description_event(new
        Format_description_log_event(4, myblockchain->server_version));
      break;
    }
  }
  ...
  */
  ...
  {
    sql_print_warning("Found a master with MyBlockchain server version older than "
  ...
  /*
    ...
    See legends definition on MYBLOCKCHAIN_BIN_LOG::relay_log_checksum_alg
    docs lines (binlog.h).
  ...
  */
  ...
  master_res= NULL;
  if (!myblockchain_real_query(myblockchain, STRING_WITH_LEN("SELECT UNIX_TIMESTAMP()")) &&
      (master_res= myblockchain_store_result(myblockchain)) &&
      (master_row= myblockchain_fetch_row(master_res)))
  {
    myblockchain_mutex_lock(&mi->data_lock);
  ...
  else if (is_network_error(myblockchain_errno(myblockchain)))
  {
    mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
               "Get master clock failed with error: %s", myblockchain_error(myblockchain));
    goto network_err;
  }
  else 
  {
    ...
    sql_print_warning("\"SELECT UNIX_TIMESTAMP()\" failed on master, "
                      ...
                      myblockchain_error(myblockchain), myblockchain_errno(myblockchain));
  }
  if (master_res)
  {
    myblockchain_free_result(master_res);
  ...
  */
      ...
      (master_row= myblockchain_fetch_row(master_res)))
  {
    ...
    {
      errmsg= "The slave I/O thread stops because master and slave have equal \
MyBlockchain server ids; these ids must be different for replication to work (or \
...
not always make sense; please check the manual before using it).";
  ...
  else if (myblockchain_errno(myblockchain))
  {
    ...
    {
      mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
                 "Get master SERVER_ID failed with error: %s", myblockchain_error(myblockchain));
      goto network_err;
    }
    /* Fatal error */
    errmsg= "The slave I/O thread stops because a fatal error is encountered \
when it try to get the value of SERVER_ID variable from master.";
    err_code= myblockchain_errno(myblockchain);
    sprintf(err_buff, "%s Error: %s", errmsg, myblockchain_error(myblockchain));
    goto err;
  }
  else if (!master_row && master_res)
  {
...
maybe it is a *VERY OLD MASTER*.");
  ...
  /*
    ...
    We don't test equality of global collation_blockchain either as it's is
  ...
  */
  ...
  if (*myblockchain->server_version == '3')
    goto err;

  if (*myblockchain->server_version == '4')
  {
    master_res= NULL;
    if (!myblockchain_real_query(myblockchain,
...
be equal for the Statement-format replication to work";
    ...
    {
      mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
                 "Get master COLLATION_SERVER failed with error: %s", myblockchain_error(myblockchain));
      goto network_err;
    }
    else if (myblockchain_errno(myblockchain) != ER_UNKNOWN_SYSTEM_VARIABLE)
    {
      /* Fatal error */
      errmsg= "The slave I/O thread stops because a fatal error is encountered \
...
inconsistency if replicated data deals with collation.");
  ...
  */
  ...
  {
    master_res= NULL;
    if (!myblockchain_real_query(myblockchain, STRING_WITH_LEN("SELECT @@GLOBAL.TIME_ZONE")) &&
        (master_res= myblockchain_store_result(myblockchain)) &&
        (master_row= myblockchain_fetch_row(master_res)))
    {
...
be equal for the Statement-format replication to work";
    ...
    {
      mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
                 "Get master TIME_ZONE failed with error: %s", myblockchain_error(myblockchain));
      goto network_err;
    } 
    else
    {
...
when it try to get the value of TIME_ZONE global variable from master.";
  ...
  */
  if (DBUG_EVALUATE_IF("simulate_slave_unaware_checksum", 0, 1))
  {
    ...
    */
    rc= myblockchain_real_query(myblockchain, query, static_cast<ulong>(strlen(query)));
    if (rc != 0)
    {
      ...
      if (myblockchain_errno(myblockchain) == ER_UNKNOWN_SYSTEM_VARIABLE)
      {
    ...
    else
    {
      ...
      {
        mi->report(WARNING_LEVEL, myblockchain_errno(myblockchain),
                   "Get master BINLOG_CHECKSUM failed with error: %s", myblockchain_error(myblockchain));
        goto network_err;
      }
      else
      {
...
static bool wait_for_relay_log_space(Relay_log_info* rli)
{
  ...
  while (rli->log_space_limit < rli->log_space_total &&
    ...
    myblockchain_cond_wait(&rli->log_space_cond, &rli->log_space_lock);
...
{
  Relay_log_info *rli= mi->rli;
  myblockchain_mutex_t *log_lock= rli->relay_log.get_log_lock();
  int error= 0;
...
int register_slave_on_master(MYBLOCKCHAIN* myblockchain, Master_info *mi,
                             bool *suppress_warnings)
{
  ...
  *suppress_warnings= FALSE;
  ...
  if (simple_command(myblockchain, COM_REGISTER_SLAVE, buf, (size_t) (pos- buf), 0))
  {
    if (myblockchain_errno(myblockchain) == ER_NET_READ_INTERRUPTED)
    {
    ...
    {
      char buf[256];
      my_snprintf(buf, sizeof(buf), "%s (Errno: %d)", myblockchain_error(myblockchain), 
                                ...
                                int io_gtid_set_size, int sql_gtid_set_size)
{
  ...
  field_list.push_back(new Item_empty_string("Master_User",
                                             USERNAME_LENGTH+1));
  field_list.push_back(new Item_return_int("Master_Port", 7,MYBLOCKCHAIN_TYPE_LONG));
  field_list.push_back(new Item_return_int("Connect_Retry", 10,
                                           MYBLOCKCHAIN_TYPE_LONG));
  field_list.push_back(new Item_empty_string("Master_Log_File", FN_REFLEN));
  field_list.push_back(new Item_return_int("Read_Master_Log_Pos", 10,
                                           MYBLOCKCHAIN_TYPE_LONGLONG));
  ...
  field_list.push_back(new Item_empty_string("Replicate_Wild_Ignore_Table",
                                             28));
  field_list.push_back(new Item_return_int("Last_Errno", 4, MYBLOCKCHAIN_TYPE_LONG));
  ...
  field_list.push_back(new Item_empty_string("Master_SSL_Verify_Server_Cert",
                                             3));
  field_list.push_back(new Item_return_int("Last_IO_Errno", 4, MYBLOCKCHAIN_TYPE_LONG));
  field_list.push_back(new Item_empty_string("Last_IO_Error", 20));
  field_list.push_back(new Item_return_int("Last_SQL_Errno", 4, MYBLOCKCHAIN_TYPE_LONG));
  ...
  field_list.push_back(new Item_empty_string("Master_Info_File",
                                             2 * FN_REFLEN));
  field_list.push_back(new Item_return_int("SQL_Delay", 10, MYBLOCKCHAIN_TYPE_LONG));
  field_list.push_back(new Item_return_int("SQL_Remaining_Delay", 8, MYBLOCKCHAIN_TYPE_LONG));
                                 ...
                                 char* sql_gtid_set_buffer)
{
  ...
  */
  myblockchain_mutex_lock(&mi->info_thd_lock);
  protocol->store(mi->info_thd ? mi->info_thd->get_proc_info() : "",
                  &my_charset_bin);
  myblockchain_mutex_unlock(&mi->info_thd_lock);

  myblockchain_mutex_lock(&mi->rli->info_thd_lock);
  slave_sql_running_state= const_cast<char *>(mi->rli->info_thd ? mi->rli->info_thd->get_proc_info() : "");
  myblockchain_mutex_unlock(&mi->rli->info_thd_lock);
  ...
  protocol->store(mi->slave_running == MYBLOCKCHAIN_SLAVE_RUN_CONNECT ?
                  "Yes" : (mi->slave_running == MYBLOCKCHAIN_SLAVE_RUN_NOT_CONNECT ?
    ...
    */
    ...
    else
    {
      ...
      /*
        ...
        reasons related to MyBlockchain:
        - the master is itself a slave of another master whose time is ahead.
        - somebody used an explicit SET TIMESTAMP on the master.
        Possible reason related to granularity-to-second of time functions
        (nothing to do with MyBlockchain), which can explain a value of -1:
...
bool show_slave_status(THD *thd)
{
  ...
  */
  ...
  myblockchain_mutex_assert_owner(&LOCK_msr_map);
...
void set_slave_thread_options(THD* thd)
{
  ...
  */
  ...
  /*
    ...
    avoid trigger asserts on myblockchain_execute_command(THD *thd) caused by
                               ...
                               killed_func func, rpl_info info)
{
  bool ret;
  struct timespec abstime;
  myblockchain_mutex_t *lock= &info->sleep_lock;
  myblockchain_cond_t *cond= &info->sleep_cond;

  /* Absolute system time at which the sleep time expires. */
  set_timespec(&abstime, seconds);

  myblockchain_mutex_lock(lock);
  thd->ENTER_COND(cond, lock, NULL, NULL);

  while (! (ret= func(thd, info)))
  {
    int error= myblockchain_cond_timedwait(cond, lock, &abstime);
    if (error == ETIMEDOUT || error == ETIME)
      break;
  }

  myblockchain_mutex_unlock(lock);
  thd->EXIT_COND(NULL);

  return ret;
}

static int request_dump(THD *thd, MYBLOCKCHAIN* myblockchain, Master_info* mi,
                        bool *suppress_warnings)
{
...
static ulong read_event(MYBLOCKCHAIN* myblockchain, Master_info *mi, bool* suppress_warnings)
{
  ulong len;
  ...
  */
    ...
    else
    {
      if (!mi->rli->abort_slave) 
      {
        sql_print_error("Error reading packet from server%s: %s (server_errno=%d)",
                        mi->get_for_channel_str(), myblockchain_error(myblockchain),
                        myblockchain_errno(myblockchain));
      }
    }
    DBUG_RETURN(packet_error);
  ...
  /* Check if eof packet */
  if (len < 8 && myblockchain->net.read_pos[0] == 254)
  {
  ...
  DBUG_PRINT("exit", ("len: %lu  net->read_pos[4]: %d",
                      len, myblockchain->net.read_pos[4]));
  DBUG_RETURN(len - 1);
}
...
static int sql_delay_event(Log_event *ev, THD *thd, Relay_log_info *rli)
{
      ...
      type != binary_log::START_EVENT_V3)
  {
    ...
    if (sql_delay_end > now)
    {
      ...
      myblockchain_mutex_unlock(&rli->data_lock);
...
apply_event_and_update_pos(Log_event** ptr_ev, THD* thd, Relay_log_info* rli)
{
  ...
  /*
    Execute the event to change the blockchain and update the binary
  ...
  */
  ...
  if (!rli->curr_group_seen_begin)
  {
    ...
    */
    ...
    myblockchain_mutex_lock(&rli->data_lock);
  }
	  ...
	  there is no rollback since 5.0.13 (ref: manual).
        ...
        */
        if (rli->trans_retries < slave_trans_retries)
        {
          ...
          */
          ...
          else
          {
            ...
            myblockchain_mutex_lock(&rli->data_lock); // because of SHOW STATUS
            if (!silent)
              rli->trans_retries++;
        ...
        else
        {
          ...
          rli->report(ERROR_LEVEL, thd->get_stmt_da()->myblockchain_errno(),
...
binary log is corrupted (you can check this by running 'myblockchainbinlog' on the \
binary log), the slave's relay log is corrupted (you can check this by running \
'myblockchainbinlog' on the relay log), a network problem, or a bug in the master's \
or slave's MyBlockchain code. If you want to check the master's binary log or slave's \
...
/**
  ...
  @param[in]     myblockchain               MyBlockchain connection.
...
static int try_to_reconnect(THD *thd, MYBLOCKCHAIN *myblockchain, Master_info *mi,
                            uint *retry_count, bool suppress_warnings,
                            const char *messages[SLAVE_RECON_MSG_MAX])
{
  mi->slave_running= MYBLOCKCHAIN_SLAVE_RUN_NOT_CONNECT;
  ...
  if (safe_reconnect(thd, myblockchain, mi, 1) || io_slave_killed(thd, mi))
  {
...
extern "C" void *handle_slave_io(void *arg)
{
  ...
  {
    myblockchain_cond_broadcast(&mi->start_cond);
    myblockchain_mutex_unlock(&mi->run_lock);
    mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR,
  ...
  if (!(mi->myblockchain = myblockchain = myblockchain_init(NULL)))
  {
    mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR,
               ER(ER_SLAVE_FATAL_ERROR), "error in myblockchain_init()");
    goto err;
  }

  THD_STAGE_INFO(thd, stage_connecting_to_master);
  // we can get killed during safe_connect
  if (!safe_connect(thd, myblockchain, mi))
  {
...
connected:
  ...
  */
  ...
  thd->slave_net = &myblockchain->net;
  THD_STAGE_INFO(thd, stage_checking_master_version);
  ret= get_master_version_and_clock(myblockchain, mi);
  if (!ret)
    ret= get_master_uuid(myblockchain, mi);
  if (!ret)
    ret= io_thread_init_commands(myblockchain, mi);
  ...
  { 
    ...
    if (try_to_reconnect(thd, myblockchain, mi, &retry_count, suppress_warnings,
  ...
  if (binlog_version > 1)
  {
    ...
    */
    ...
    if (register_slave_on_master(myblockchain, mi, &suppress_warnings))
    {
  ...
  while (!io_slave_killed(thd,mi))
  {
    ...
    if (request_dump(thd, myblockchain, mi, &suppress_warnings))
    {
      sql_print_error("Failed on request_dump()%s", mi->get_for_channel_str());
      if (check_io_slave_killed(thd, mi, "Slave I/O thread killed while \
requesting master dump") ||
          try_to_reconnect(thd, myblockchain, mi, &retry_count, suppress_warnings,
    ...
    while (!io_slave_killed(thd,mi))
    {
      ...
      */
      ...
      event_len= read_event(myblockchain, mi, &suppress_warnings);
      if (check_io_slave_killed(thd, mi, "Slave I/O thread killed while \
reading event"))
        goto err;
      ...
      {
        uint myblockchain_error_number= myblockchain_errno(myblockchain);
        switch (myblockchain_error_number) {
...
slave_max_allowed_packet",
        ...
        case ER_MASTER_FATAL_ERROR_READING_BINLOG:
          mi->report(ERROR_LEVEL, ER_MASTER_FATAL_ERROR_READING_BINLOG,
                     ...
                     myblockchain_error_number, myblockchain_error(myblockchain));
          goto err;
...
err:
  ...
  if (myblockchain)
  {
    /*
      ...
      the 'myblockchain' object, causing a crash.
    */
    thd->clear_active_vio();
  ...
  */
   ...
   */ 
  myblockchain_cond_broadcast(&mi->stop_cond);       // tell the world we are done
...
int check_temp_dir(char* tmp_file, const char *channel_name)
{
  ...
  */
  ...
  if ((fd= myblockchain_file_create(key_file_misc,
  ...
  myblockchain_file_delete(key_file_misc, unique_tmp_file_name, MYF(0));
...
extern "C" void *handle_slave_worker(void *arg)
{
  ...
  myblockchain_mutex_lock(&w->info_thd_lock);
  w->info_thd= thd;
  myblockchain_mutex_unlock(&w->info_thd_lock);
  ...
  myblockchain_cond_signal(&w->jobs_cond);
  ...
  */
  ...
  myblockchain_mutex_lock(&rli->pending_jobs_lock);
  ...
  myblockchain_mutex_unlock(&rli->pending_jobs_lock);
  ...
  */
  ...
  myblockchain_cond_signal(&w->jobs_cond);  // famous last goodbye
      ...
      */
      if (!checksum_detected)
      {
        ...
        while (i < 4 && (ev= Log_event::read_log_event(&log,
               (myblockchain_mutex_t*) 0, p_fdle, 0)))
        {
          if (ev->get_type_code() == binary_log::FORMAT_DESCRIPTION_EVENT)
          {
                            ...
                            bool force, bool need_data_lock)
{
 ...
 do
  {
    if (!is_mts_db_partitioned(rli))
      myblockchain_mutex_lock(&rli->mts_gaq_LOCK);

    cnt= rli->gaq->move_queue_head(&rli->workers);

    if (!is_mts_db_partitioned(rli))
      myblockchain_mutex_unlock(&rli->mts_gaq_LOCK);
#ifndef DBUG_OFF
    if (DBUG_EVALUATE_IF("check_slave_debug_group", 1, 0) &&
  ...
  */
  ...
  myblockchain_cond_broadcast(&rli->data_cond);
  if (need_data_lock)
    myblockchain_mutex_unlock(&rli->data_lock);
...
int slave_start_single_worker(Relay_log_info *rli, ulong i)
{
  ...
  myblockchain_mutex_assert_owner(&rli->run_lock);
  ...
  if (DBUG_EVALUATE_IF("mts_worker_thread_fails", i == 1, 0) ||
      (error= myblockchain_thread_create(key_thread_slave_worker, &th,
  ...
  if (w->running_status == Slave_worker::NOT_RUNNING)
    myblockchain_cond_wait(&w->jobs_cond, &w->jobs_lock);
  myblockchain_mutex_unlock(&w->jobs_lock);
...
extern "C" void *handle_slave_sql(void *arg)
{
  ...
  myblockchain_mutex_lock(&rli->run_lock);
  ...
  thd = new THD; // note that contructor of THD uses DBUG_ !
  thd->thread_stack = (char*)&thd; // remember where our stack is
  myblockchain_mutex_lock(&rli->info_thd_lock);
  rli->info_thd= thd;
 ...
 if (rli->channel_mts_submode != MTS_PARALLEL_TYPE_DB_NAME)
   ...
   rli->current_mts_submode= new Mts_submode_blockchain();
  ...
  myblockchain_mutex_unlock(&rli->info_thd_lock);
  ...
  if (init_slave_thread(thd, SLAVE_THD_SQL))
  {
    ...
    */
    myblockchain_cond_broadcast(&rli->start_cond);
    myblockchain_mutex_unlock(&rli->run_lock);
    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER(ER_SLAVE_FATAL_ERROR),
      ...
      if (!sql_slave_killed(thd,rli))
      {
        ...
        */
        ...
        if (thd->is_error())
        {
          ...
          DBUG_PRINT("info",
                     ("thd->get_stmt_da()->get_myblockchain_errno()=%d; "
                      "rli->last_error.number=%d",
                      thd->get_stmt_da()->myblockchain_errno(), last_errno));
          if (last_errno == 0)
          {
            /*
 	    ...
 	    */ 
          ...
          else if (last_errno != thd->get_stmt_da()->myblockchain_errno())
          {
            /*
            ...
            sql_print_error("Slave (additional info): %s Error_code: %d",
                            errmsg, thd->get_stmt_da()->myblockchain_errno());
          }
        }
        ...
        */
        ...
        while ((err= it++))
        {
          if (err->myblockchain_errno() == ER_CANT_OPEN_LIBRARY)
            udf_error = true;
          sql_print_warning("Slave: %s Error_code: %d",
                            err->message_text(), err->myblockchain_errno());
        }
        if (udf_error)
...
llstr(rli->get_group_master_log_pos(), llbuff));
 ...
 /*
  ...
  delete the mi structure leading to a crash! (see BUG#25306 for details)
 */ 
  myblockchain_cond_broadcast(&rli->stop_cond);
  DBUG_EXECUTE_IF("simulate_slave_delay_at_terminate_bug38694", sleep(5););
  myblockchain_mutex_unlock(&rli->run_lock);  // tell the world we are done
  ...
  */
  ...
  case binary_log::ANONYMOUS_GTID_LOG_EVENT:
    /*
      ...
      myblockchaind, which could contain bugs that we have no control over.
...
static int safe_connect(THD* thd, MYBLOCKCHAIN* myblockchain, Master_info* mi)
{
  ...
  DBUG_RETURN(connect_to_master(thd, myblockchain, mi, 0, 0));
}
                             ...
                             bool reconnect, bool suppress_warnings)
{
  ...
  set_slave_max_allowed_packet(thd, myblockchain);
#ifndef DBUG_OFF
  mi->events_until_exit = disconnect_slave_event_count;
#endif
  ...
  myblockchain_options(myblockchain, MYBLOCKCHAIN_OPT_CONNECT_TIMEOUT, (char *) &slave_net_timeout);
  myblockchain_options(myblockchain, MYBLOCKCHAIN_OPT_READ_TIMEOUT, (char *) &slave_net_timeout);

  if (mi->bind_addr[0])
  {
    DBUG_PRINT("info",("bind_addr: %s", mi->bind_addr));
    myblockchain_options(myblockchain, MYBLOCKCHAIN_OPT_BIND, mi->bind_addr);
  }
  ...
  if (mi->ssl)
  {
    ...
    myblockchain_ssl_set(myblockchain,
    ...
    mi->ssl_crlpath[0]= '\0';
#endif
    myblockchain_options(myblockchain, MYBLOCKCHAIN_OPT_SSL_CRL,
                  mi->ssl_crl[0] ? mi->ssl_crl : 0);
    myblockchain_options(myblockchain, MYBLOCKCHAIN_OPT_SSL_CRLPATH,
    ...
    myblockchain_options(myblockchain, MYBLOCKCHAIN_OPT_SSL_ENFORCE, &ssl_enforce_true);
  }
#endif
  ...
  */
  if (is_supported_parser_charset(default_charset_info))
    myblockchain_options(myblockchain, MYBLOCKCHAIN_SET_CHARSET_NAME, default_charset_info->csname);
  else
  {
  ...
  myblockchain_options(myblockchain, MYBLOCKCHAIN_SET_CHARSET_DIR, (char *) charsets_dir);

  if (mi->is_start_plugin_auth_configured())
  {
    DBUG_PRINT("info", ("Slaving is using MYBLOCKCHAIN_DEFAULT_AUTH %s",
                        mi->get_start_plugin_auth()));
    myblockchain_options(myblockchain, MYBLOCKCHAIN_DEFAULT_AUTH, mi->get_start_plugin_auth());
  }
  
  if (mi->is_start_plugin_dir_configured())
  {
    DBUG_PRINT("info", ("Slaving is using MYBLOCKCHAIN_PLUGIN_DIR %s",
                        mi->get_start_plugin_dir()));
    myblockchain_options(myblockchain, MYBLOCKCHAIN_PLUGIN_DIR, mi->get_start_plugin_dir());
  }
  /* Set MYBLOCKCHAIN_PLUGIN_DIR in case master asks for an external authentication plugin */
  else if (opt_plugin_dir_ptr && *opt_plugin_dir_ptr)
    myblockchain_options(myblockchain, MYBLOCKCHAIN_PLUGIN_DIR, opt_plugin_dir_ptr);
  ...
  while (!(slave_was_killed = io_slave_killed(thd,mi))
         && (reconnect ? myblockchain_reconnect(myblockchain) != 0 :
             myblockchain_real_connect(myblockchain, mi->host, user,
  ...
  if (!slave_was_killed)
  {
    ...
    thd->set_active_vio(myblockchain->net.vio);
  }
                          ...
                          bool suppress_warnings)
{
  ...
  DBUG_RETURN(connect_to_master(thd, myblockchain, mi, 1, suppress_warnings));
}
...
static Log_event* next_event(Relay_log_info* rli)
{
  ...
  /*
    ...
    myblockchain_cond_wait() with the non-data_lock mutex
  */
  myblockchain_mutex_assert_owner(&rli->data_lock);
  ...
  while (!sql_slave_killed(thd,rli))
  {
    ...
    */
    ...
    /*
      ...
      MYBLOCKCHAIN_BIN_LOG::open() will write the buffered description event.
    */
    ...
    DBUG_ASSERT(thd==rli->info_thd);
    if (opt_reckless_slave)                     // For myblockchain-test
      cur_log->error = 0;
    if (cur_log->error < 0)
    {
    ...
    if (!cur_log->error) /* EOF */
    {
      ...
      */
      ...
      myblockchain_file_close(rli->cur_log_fd, MYF(MY_WME));
      rli->cur_log_fd = -1;
...
/*
  ...
  Returns nothing as we cannot catch any error (MYBLOCKCHAIN_BIN_LOG::new_file()
  is void).
*/
...
/**
   ...
   @param bug_id Number of the bug as found in bugs.myblockchain.com
       ...
       i < sizeof(versions_for_all_bugs)/sizeof(*versions_for_all_bugs);i++)
  {
	...
	return TRUE;
      ...
      my_printf_error(ER_UNKNOWN_ERROR, "master may suffer from"
                      " http://bugs.myblockchain.com/bug.php?id=%u"
...
/**
  ...
  - Delete worker info in myblockchain.slave_worker_info table if applier not running.
  ...
  if ((thread_mask & SLAVE_SQL) == 0) // If execute threads are stopped
  {
    ...
    else
    {
      /*
        ...
        in myblockchain.slave_worker_info table.
      */
  ...
  if ((thread_mask & SLAVE_SQL) == 0) /* Applier module is not executing */
  {
    ...
    */
    ...
    /*
      ...
      relay-log.info until START SLAVE is issued; but if myblockchaind is shutdown
    ...
    if (mi)
    {
      if ((thread_mask & SLAVE_IO) != 0)
      {
        ...
        */
        ...
        {
          myblockchain_mutex_assert_owner(&mi->run_lock);
        }
      ...
      if ((thread_mask & SLAVE_SQL) != 0)
      {
        ...
        */
        ...
        {
          myblockchain_mutex_assert_owner(&mi->rli->run_lock);
        }
