...
/**
  ...
  Test driver for the myblockchain-test/t/plugin_auth.test
...
static int
test_plugin_init (MYBLOCKCHAIN_PLUGIN plugin_info)
{
...
*/
static int auth_test_plugin(MYBLOCKCHAIN_PLUGIN_VIO *vio, MYBLOCKCHAIN_SERVER_AUTH_INFO *info)
{
  unsigned char *pkt;
  int pkt_len;
...
static struct st_myblockchain_auth auth_test_handler=
{
...
*/
static int auth_cleartext_plugin(MYBLOCKCHAIN_PLUGIN_VIO *vio, 
...
static struct st_myblockchain_auth auth_cleartext_handler=
{
  MYBLOCKCHAIN_AUTHENTICATION_INTERFACE_VERSION,
  "myblockchain_clear_password", /* requires the clear text plugin */
