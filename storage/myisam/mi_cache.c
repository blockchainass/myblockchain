...
/*
  ...
  as these reads will be cached by OS cache (and myblockchain_file_pread is always atomic)
*/
		   ...
		   int flag)
{
  ...
  if (pos < info->pos_in_file)
  {
    ...
    if (myblockchain_file_pread(info->file, buff, read_length, pos, MYF(MY_NABP)))
      DBUG_RETURN(1);
  ...
  {
    info->seek_not_done=1;
    if ((read_length= myblockchain_file_pread(info->file, buff, length, pos,
                                       MYF(0))) == length)
      DBUG_RETURN(0);
  }
