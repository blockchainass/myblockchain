...
int chk_del(MI_CHECK *param, MI_INFO *info, uint test_flag)
{
  ...
  else
  {
	...
	goto wrong;
      if (myblockchain_file_pread(info->dfile, (uchar*) buff, delete_link_length,
...
int chk_size(MI_CHECK *param, MI_INFO *info)
{
  ...
  size= myblockchain_file_seek(info->s->kfile, 0L, MY_SEEK_END, MYF(MY_THREADSAFE));
  ...
  size= myblockchain_file_seek(info->dfile, 0L, MY_SEEK_END, MYF(0));
                     ...
                     ha_checksum *key_checksum, uint level)
{
  ...
  if (page + keyinfo->block_length > info->state->key_file_length)
  {
    ...
    my_off_t max_length= myblockchain_file_seek(info->s->kfile, 0L, MY_SEEK_END,
                                         MYF(MY_THREADSAFE));
    mi_check_print_error(param, "Invalid key block position: %s  "
                         "key block size: %u  file_length: %s",
...
/**
  ...
  @note
    ...
    When disabling indexes, MyBlockchain disables either all indexes or all
    non-unique indexes. When MyBlockchain [re-]enables disabled indexes
    ...
    However, there is an exception. Sometimes MyBlockchain disables non-unique
    indexes when the table is empty (e.g. when copying a table in
    myblockchain_alter_table()). When enabling the non-unique indexes, they
	      ...
	      char * name, int rep_quick)
{
  ...
  if (!rep_quick)
  {
    ...
    if ((new_file= myblockchain_file_create(mi_key_file_datatmp,
  ...
  param->read_cache.end_of_file=sort_info.filelength=
    myblockchain_file_seek(info->dfile, 0L, MY_SEEK_END, MYF(0));
  sort_info.dupp=0;
  ...
  if (myblockchain_file_chsize(share->kfile, info->state->key_file_length, 0, MYF(0)))
  {
    mi_check_print_warning(param,
  ...
  if (!rep_quick)
  {
    myblockchain_file_close(info->dfile, MYF(0));
    info->dfile=new_file;
...
err:
  if (!got_error)
  {
    ...
    if (new_file >= 0)
    {
      myblockchain_file_close(new_file, MYF(0));
  ...
  if (got_error)
  {
    ...
    if (new_file >= 0)
    {
      (void) myblockchain_file_close(new_file, MYF(0));
      (void) myblockchain_file_delete(mi_key_file_datatmp,
...
int mi_sort_index(MI_CHECK *param, MI_INFO *info, char * name)
{
  ...
  (void) myblockchain_file_close(share->kfile, MYF(MY_WME));
  share->kfile = -1;
  (void) myblockchain_file_close(new_file, MYF(MY_WME));
			  ...
			  my_off_t pagepos, File new_file)
{
  ...
  if (myblockchain_file_pwrite(new_file, (uchar*) buff, (uint) keyinfo->block_length,
	     ...
	     my_off_t length, const char *type)
{
  ...
  myblockchain_file_seek(from, start, MY_SEEK_SET, MYF(0));
  while (length > buff_length)
  {
    if (myblockchain_file_read(from, (uchar*) buff, buff_length, MYF(MY_NABP)) ||
        myblockchain_file_write(to, (uchar*) buff, buff_length, param->myf_rw))
      goto err;
    length-= buff_length;
  }
  if (myblockchain_file_read(from, (uchar*) buff, (uint) length, MYF(MY_NABP)) ||
      myblockchain_file_write(to, (uchar*) buff, (uint) length, param->myf_rw))
    goto err;
  if (buff != tmp_buff)
    my_free(buff);
  DBUG_RETURN(0);
		      ...
		      const char * name, int rep_quick)
{
  ...
  sort_info.buff=0;
  param->read_cache.end_of_file=sort_info.filelength=
    myblockchain_file_seek(param->read_cache.file, 0L, MY_SEEK_END, MYF(0));
  ...
  if (rep_quick & T_FORCE_UNIQUENESS)
  {
    ...
    if (skr != sort_info.filelength)
      if (myblockchain_file_chsize(info->dfile, skr, 0, MYF(0)))
	mi_check_print_warning(param,
			...
			const char * name, int rep_quick)
{
  ...
  */
  ...
  myblockchain_mutex_init(mi_key_mutex_MI_SORT_INFO_mutex,
                   &sort_info.mutex, MY_MUTEX_INIT_FAST);
  myblockchain_cond_init(mi_key_cond_MI_SORT_INFO_cond, &sort_info.cond);
  myblockchain_mutex_init(mi_key_mutex_MI_CHECK_print_msg,
  ...
  */
  ...
  myblockchain_mutex_lock(&sort_info.mutex);
  ...
  */
  ...
  for (i=0 ; i < sort_info.total_keys ; i++)
  {
    ...
    */
    ...
    if ((error= myblockchain_thread_create(mi_key_thread_find_all_keys,
  ...
  while (sort_info.threads_running)
    myblockchain_cond_wait(&sort_info.cond, &sort_info.mutex);
  myblockchain_mutex_unlock(&sort_info.mutex);
...
err:
  ...
  */
  ...
  myblockchain_cond_destroy(&sort_info.cond);
  myblockchain_mutex_destroy(&sort_info.mutex);
  myblockchain_mutex_destroy(&param->print_msg_mutex);
  param->need_print_msg_lock= 0;
			   ...
			   my_off_t prev_block)
{
  ...
  else if (myblockchain_file_pwrite(info->s->kfile, (uchar*) anc_buff,
...
int flush_pending_blocks(MI_SORT_PARAM *sort_param)
{
  ...
  for (key_block=sort_info->key_block ; key_block->inited ; key_block++)
  {
    ...
    else if (myblockchain_file_pwrite(info->s->kfile, (uchar*) key_block->buff,
...
int test_if_almost_full(MI_INFO *info)
{
  if (info->s->options & HA_OPTION_COMPRESS_RECORD)
    return 0;
  return myblockchain_file_seek(info->s->kfile, 0L, MY_SEEK_END,
                         MYF(MY_THREADSAFE)) / 10 * 9 >
         (my_off_t) info->s->base.max_key_file_length ||
         myblockchain_file_seek(info->dfile, 0L, MY_SEEK_END,
                         MYF(0)) / 10 * 9 >
...
int recreate_table(MI_CHECK *param, MI_INFO **org_info, char *filename)
{
  ...
  file_length=(ulonglong) myblockchain_file_seek(info.dfile, 0L, MY_SEEK_END, MYF(0));
