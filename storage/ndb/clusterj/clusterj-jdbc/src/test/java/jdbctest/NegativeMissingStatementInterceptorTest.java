...
public class NegativeMissingStatementInterceptorTest extends AbstractClusterJTest {
    public void test() {
        try {
            getConnection("target/test-classes/bad-connection-no-statement-interceptor.properties");
            connection.prepareStatement("SELECT 1 FROM DUAL");
            fail("Missing statement interceptor should fail with ClusterJUserException.");
            ...
            if (sqlException instanceof SQLException) {
                ...
                if (clusterJException instanceof ClusterJUserException) {
                    if (clusterJException.getMessage().contains("statementInterceptors")) {
                    // good catch
                    } else {
