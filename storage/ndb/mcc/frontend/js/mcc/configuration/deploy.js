...
function verifyConfiguration() {
    ...
    for (var h in hosts) {
        ...
        for (var p in processesOnHost[hostId]) {
            ...
            // Only myblockchaind processes have socket and port
...
function distributeConfigurationFiles() {
    ...
    // Get all ndb_mgmds and all myblockchainds
    var cfg_procs = processTypeInstances("ndb_mgmd").concat(processTypeInstances("myblockchaind"));
...
function getConfigurationFile(process) {
    ...
    if (ptype == "ndb_mgmd") {
            ...
            if (true) {
                ...
                for (var p in mcc.configuration.getAllPara(family)) {
                    ...
                    if (val !== undefined) {
                        ...
                        // TCP buffers and myblockchaind options treated separately
        ...
        addln(configFile, "#");
        addln(configFile, "[myblockchaind]");
        addln(configFile, "log-error=myblockchaind."+process.getValue("NodeId")+".err")
...
function getCreateCommands() {
    ...
    for (var i in processes) {
        ...
        if (ptype.getValue("name") != "api") {
            ...
            // Push the create datadir command unless myblockchaind
            if (ptype.getValue("name") != "myblockchaind") {
                ...
                // All myblockchainds also need data/test, data/myblockchain and socket dir
                ...
                createDirCommands.push({
                    host: host.getValue("name"),
                    path: datadir + "myblockchain" + dirSep,
                    name: null
                });
                ...
                if (mcc.util.isWin(host.getValue("uname"))) {
                    ...
                    createDirCommands.push({
                        ...
                        sourceName: "myblockchain_system_tables.sql",
                    ...
                    createDirCommands.push({
                        ...
                        host: host.getValue("name"),
                        sourcePath: installDir + "share" + installSep,
                        sourceName: "myblockchain_system_tables_data.sql",
                        destinationPath: datadir + "tmp" + dirSep,
                        destinationName: "install.sql"
                    });
                    createDirCommands.push({
					...
					createDirCommands.push({
                        host: host.getValue("name"),
                        path: datadir + "tmp" + dirSep,
                        name: "myblockchain_install_db.bat",
                        msg: "\""+installDir+installSep+"bin"+installSep+"myblockchaind.exe\" --lc-messages-dir=\""+installDir+installSep+"share\" --bootstrap --basedir=\""+
                    ...
                    // Non-win myblockchainds also need data/tmp
                    createDirCommands.push({
                        host: host.getValue("name"),
                        path: datadir + "tmp" + dirSep,
                        name: null
...
function getStartProcessCommands(process) {
    ...
    if (ptype == "myblockchaind") {
        ...
        if (isWin) {
			...
			var midb = new ProcessCommand(host, tmpdir, "myblockchain_install_db.bat");
			midb.progTitle = "Running myblockchain_install_db.bat for node "+nodeid;
			scmds.unshift(midb);
			
            var ic = new ProcessCommand(host, "C:\\Windows\\System32", "cmd.exe");
            delete ic.msg.file.autoComplete; // Don't want ac for cmd.exe
            ic.addopt("/C");
            ic.addopt(basedir+"bin\\myblockchaind.exe");
            ...
            ic.progTitle = "Running myblockchaind --bootstrap for node "+nodeid;
            //scmds.unshift(ic);
            ...
            var ic = new ProcessCommand(host, basedir, "myblockchain_install_db");
            ic.addopt("--no-defaults");
            ic.addopt("--datadir", datadir);
            ic.addopt("--basedir", basedir);
            ic.progTitle = "Running myblockchain_install_db for node "+nodeid;
            scmds.unshift(ic);

            sc.addopt("--defaults-file", datadir+"my.cnf");
            // Invoking myblockchaind does not return
