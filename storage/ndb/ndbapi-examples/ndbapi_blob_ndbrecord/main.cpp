...
*/
void drop_table(MYBLOCKCHAIN &myblockchain)
{
  if (myblockchain_query(&myblockchain, "DROP TABLE api_blob_ndbrecord"))
...
*/
int try_create_table(MYBLOCKCHAIN &myblockchain)
{
  return myblockchain_query(&myblockchain,
                     "CREATE TABLE"
                     "  api_blob_ndbrecord"
...
void create_table(MYBLOCKCHAIN &myblockchain)
{
  if (try_create_table(myblockchain))
  {
    if (myblockchain_errno(&myblockchain) != ER_TABLE_EXISTS_ERROR)
      MYBLOCKCHAINERROR(myblockchain);
    std::cout << "MyBlockchain Cluster already has example table: api_blob_ndbrecord. "
