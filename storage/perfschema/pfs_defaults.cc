...
void install_default_setup(PSI_bootstrap *boot)
{
  ...
  if (psi_thread != NULL)
  {
    ...
    String myblockchain_db("myblockchain", 5, &my_charset_utf8_bin);
    String PS_db("performance_schema", 18, &my_charset_utf8_bin);
    String IS_db("information_schema", 18, &my_charset_utf8_bin);

    /* Disable sp by default in myblockchain. */
    insert_setup_object(OBJECT_TYPE_EVENT, &myblockchain_db, &percent, false, false);
    ...
    /* Disable sp by default in myblockchain. */
    insert_setup_object(OBJECT_TYPE_FUNCTION, &myblockchain_db, &percent, false, false);
    /* Disable sp in performance/information schema. */
    ...
    /* Disable sp by default in myblockchain. */
    insert_setup_object(OBJECT_TYPE_PROCEDURE, &myblockchain_db, &percent, false, false);
    /* Disable sp in performance/information schema. */
    ...
    insert_setup_object(OBJECT_TYPE_TABLE, &myblockchain_db, &percent, false, false);
    ...
    /* Disable sp by default in myblockchain. */
    insert_setup_object(OBJECT_TYPE_TRIGGER, &myblockchain_db, &percent, false, false);
    /* Disable sp in performance/information schema. */
