...
uint emb_count_querycache_size(THD *thd)
{
  ...
  *data->embedded_info->prev_ptr= NULL; // this marks the last record
  ...
  else
  {
    ...
    for (; cur_row; cur_row=cur_row->next)
    {
      MYBLOCKCHAIN_ROW col= cur_row->data;
      MYBLOCKCHAIN_ROW col_end= col + data->fields;
      for (; col < col_end; col++)
        if (*col)
...
{
  MYBLOCKCHAIN_DATA *data= thd->alloc_new_dataset();
  MYBLOCKCHAIN_FIELD *field;
  MYBLOCKCHAIN_FIELD *field_end;
  MEM_ROOT *f_alloc;
  MYBLOCKCHAIN_ROWS *row, *end_row;
  MYBLOCKCHAIN_ROWS **prev_row;
  ulonglong rows;
  MYBLOCKCHAIN_ROW columns;
  ...
  if (!(field= (MYBLOCKCHAIN_FIELD *)
        alloc_root(f_alloc,data->fields*sizeof(MYBLOCKCHAIN_FIELD))))
    goto err;
  ...
  {
    uint length;
    row= (MYBLOCKCHAIN_ROWS *)alloc_root(&data->alloc,
                                  (size_t) (rows * sizeof(MYBLOCKCHAIN_ROWS)));
    end_row= row + rows;
    data->data= row;
    ...
    {
      *prev_row= row;
      row->data= (MYBLOCKCHAIN_ROW) src->load_str(&data->alloc, &length);
      row->length= length;
    }
  }
  else
  {
    row= (MYBLOCKCHAIN_ROWS *)alloc_root(&data->alloc,
        (uint) (rows * sizeof(MYBLOCKCHAIN_ROWS) +
          rows*(data->fields+1)*sizeof(char*)));
    end_row= row + rows;
    columns= (MYBLOCKCHAIN_ROW)end_row;
    ...
    for (prev_row= &row->next; row < end_row; prev_row= &row->next, row++)
    {
      *prev_row= row;
      row->data= columns;
      MYBLOCKCHAIN_ROW col_end= columns + data->fields;
