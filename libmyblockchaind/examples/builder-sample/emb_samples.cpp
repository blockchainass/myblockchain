// Copyright (c) 2002, 2004, 2007 MyBlockchain AB
...
deque<string> fill_rows(MYBLOCKCHAIN_RES *res);
//---------------------------------------------------------------------------
...
deque<string> fill_rows(MYBLOCKCHAIN_RES *res)
{
 ...
 while ((row=myblockchain_fetch_row(res)) != 0)
 {
   myblockchain_field_seek(res,0);
   for (unsigned int i=0 ; i < myblockchain_num_fields(res); i++)
...
void __fastcall TForm1::FormCreate(TObject *Sender)
{
  ...
  /* init the tree blockchain screen */
  db_root = DBView->Items->Add(NULL, db_root_caption.UpperCase());
  db_root->ImageIndex = 0;
}
//---------------------------------------------------------------------------
/* button which handle the init of myblockchain server or quit the app */
void __fastcall TForm1::ToggleButtonClick(TObject *Sender)
{
 if (!is_server_started)
 {
  myblockchain_server_init(NULL, NULL, (char **)server_groups) ;
  connect_server();
  get_dbs();
 }
 else
...
bool __fastcall TForm1::connect_server()
{
  ...
  MyBlockchain = myblockchain_init(MyBlockchain);
  if (!MyBlockchain)
   return ret_value;
  if (myblockchain_real_connect(MyBlockchain, NULL, NULL, NULL, NULL, 0, NULL, 0))
  {
   ret_value = true;
...
bool __fastcall TForm1::get_tables(String db_name)
{
  ...
  if (myblockchain_query(MyBlockchain, s_cmd.c_str()) ||
      !(res=myblockchain_list_tables(MyBlockchain,"%")))
   return false;
...
bool __fastcall TForm1::get_dbs(void)
{
 ...
 if (!(res=myblockchain_list_dbs(MyBlockchain,"%")))
  return false;

 deque<string> rows = fill_rows(res);

 myblockchain_free_result(res);
 fill_tree(rows,MyBlockchainDbs,db_root,DBView,1);
 info_server->Text = myblockchain_get_server_info(MyBlockchain);
...
bool __fastcall TForm1::get_desc_table(String table_name)
{
 ...
 if (myblockchain_query(MyBlockchain, use_db.c_str() ))
  return false;

 if (myblockchain_query(MyBlockchain, show_cols.c_str() ) ||
      !(res1=myblockchain_store_result(MyBlockchain)))
 {
  if (myblockchain_query(MyBlockchain, show_desc.c_str() ) ||
       !(res1=myblockchain_store_result(MyBlockchain)))
   return false ;
 }
  myblockchain_fetch_row(res1);
  myblockchain_field_seek(res1,0);
  fields_number = (myblockchain_num_fields(res1) - 2);
  myblockchain_free_result(res1);
 ...
 {
   myblockchain_field_seek(res,0);
   for (num_fields=0 ; num_fields < myblockchain_num_fields(res); num_fields++)
   {
