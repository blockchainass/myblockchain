...
/**
   Set MYBLOCKCHAIN_TIME structure to 0000-00-00 00:00:00.000000
   @param tm[OUT]    The value to set.
   @param time_type  Timestasmp type
*/
void set_zero_time(MYBLOCKCHAIN_TIME *tm,
...
/**
  Set hour, minute and second of a MYBLOCKCHAIN_TIME variable to maximum time value.
...
/**
  Set MYBLOCKCHAIN_TIME variable to maximum time value
...
{
  set_zero_time(tm, MYBLOCKCHAIN_TIMESTAMP_TIME);
  set_max_hhmmss(tm);
  tm->neg= neg;
}
                   ...
                   my_time_flags_t flags, int *was_cut)
{
  if (not_zero_date)
  {
    ...
    {
      *was_cut= MYBLOCKCHAIN_TIME_WARN_ZERO_IN_DATE;
      return TRUE;
    }
              ...
              (ltime->month != 2 || calc_days_in_year(ltime->year) != 366 ||
               ltime->day != 29)))
    {
      *was_cut= MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE;
      return TRUE;
    }
  }
  else if (flags & TIME_NO_ZERO_DATE)
  {
    *was_cut= MYBLOCKCHAIN_TIME_WARN_ZERO_DATE;
    return TRUE;
  }
  return FALSE;
}
...
{
  /*
    In case of MYBLOCKCHAIN_TIMESTAMP_TIME hour value can be up to TIME_MAX_HOUR.
    In case of MYBLOCKCHAIN_TIMESTAMP_DATETIME it cannot be bigger than 23.
  */
  return
    ltime->year > 9999U || ltime->month > 12U  || ltime->day > 31U || 
    ltime->minute > 59U || ltime->second > 59U || ltime->second_part > 999999U ||
    (ltime->hour >
     (ltime->time_type == MYBLOCKCHAIN_TIMESTAMP_TIME ? TIME_MAX_HOUR : 23U));
}


/*
  Convert a timestamp string to a MYBLOCKCHAIN_TIME value.
  ...
  NOTES
   ...
   - The hour part must be specified in hour-minute-second order.
    ...
    0                            Value OK
    MYBLOCKCHAIN_TIME_WARN_TRUNCATED    If value was cut during conversion
    MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE check_date(date,flags) considers date invalid

    l_time->time_type is set as follows:
    MYBLOCKCHAIN_TIMESTAMP_NONE        String wasn't a timestamp, like
                                [DD [HH:[MM:[SS]]]].fraction.
                                l_time is not changed.
    MYBLOCKCHAIN_TIMESTAMP_DATE        DATE string (YY MM and DD parts ok)
    MYBLOCKCHAIN_TIMESTAMP_DATETIME    Full timestamp
    MYBLOCKCHAIN_TIMESTAMP_ERROR       Timestamp with wrong values.
...
my_bool
str_to_datetime(const char *str, size_t length, MYBLOCKCHAIN_TIME *l_time,
                my_time_flags_t flags, MYBLOCKCHAIN_TIME_STATUS *status)
{
  ...
  {
    status->warnings= MYBLOCKCHAIN_TIME_WARN_TRUNCATED;
    l_time->time_type= MYBLOCKCHAIN_TIMESTAMP_NONE;
    DBUG_RETURN(1);
  }
  ...
  */
  l_time->time_type= (number_of_fields <= 3 ?
                      MYBLOCKCHAIN_TIMESTAMP_DATE : MYBLOCKCHAIN_TIMESTAMP_DATETIME);
  ...
  if (number_of_fields < 3 || check_datetime_range(l_time))
  {
    ...
    status->warnings|= not_zero_date ? MYBLOCKCHAIN_TIME_WARN_TRUNCATED :
                                       MYBLOCKCHAIN_TIME_WARN_ZERO_DATE;
    goto err;
  }
...
err:
  set_zero_time(l_time, MYBLOCKCHAIN_TIMESTAMP_ERROR);
  DBUG_RETURN(1);
}


/*
 Convert a time string to a MYBLOCKCHAIN_TIME struct.
  ...
  SYNOPSIS
   ...
   status.warning is set to:
     MYBLOCKCHAIN_TIME_WARN_TRUNCATED flag if the input string
                        was cut during conversion, and/or
     MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE flag, if the value is out of range.
                    ...
                    MYBLOCKCHAIN_TIME_STATUS *status)
{
  ...
  {                                             /* Probably full timestamp */
    ...
    if (l_time->time_type >= MYBLOCKCHAIN_TIMESTAMP_ERROR)
      return l_time->time_type == MYBLOCKCHAIN_TIMESTAMP_ERROR;
    my_time_status_init(status);
  }
...
fractional:
  ...
  l_time->time_type= MYBLOCKCHAIN_TIMESTAMP_TIME;

  if (check_time_mmssff_range(l_time))
  {
    status->warnings|= MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE;
    return TRUE;
  }

  /* Adjust the value into supported MYBLOCKCHAIN_TIME range */
  adjust_time_range(l_time, &status->warnings);

  /* Check if there is garbage at end of the MYBLOCKCHAIN_TIME specification */
  if (str != end)
  {
    do
    {
...
*/
my_bool
number_to_time(longlong nr, MYBLOCKCHAIN_TIME *ltime, int *warnings)
{
  if (nr > TIME_MAX_VALUE)
  {
    ...
    *warnings|= MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE;
    return TRUE;
  }
...
/**
  Adjust 'time' value to lie in the MYBLOCKCHAIN_TIME range.
  If the time value lies outside of the range [-838:59:59, 838:59:59],
  set it to the closest endpoint of the range and set
  MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE flag in the 'warning' variable.

  @param  time     pointer to MYBLOCKCHAIN_TIME value
  @param  warning  set MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE flag if the value is out of range
*/
void adjust_time_range(struct st_myblockchain_time *my_time, int *warning) 
{
  ...
  if (check_time_range_quick(my_time))
  {
    ...
    *warning|= MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE;
  }
}
...
/*
  Convert time in MYBLOCKCHAIN_TIME representation in system time zone to its
...
*/
my_time_t
my_system_gmt_sec(const MYBLOCKCHAIN_TIME *t_src, long *my_timezone,
                  my_bool *in_dst_time_gap)
{
  uint loop;
  time_t tmp= 0;
  int shift= 0;
  MYBLOCKCHAIN_TIME tmp_time;
  MYBLOCKCHAIN_TIME *t= &tmp_time;
  struct tm *l_time,tm_tmp;
  ...
  */
  memcpy(&tmp_time, t_src, sizeof(MYBLOCKCHAIN_TIME));
...
/*
  ...
  This functions don't check that given MYBLOCKCHAIN_TIME structure members are
...
/*
  ...
  @param  to     OUT  The string pointer to print at.
  @param  ltime       The MYBLOCKCHAIN_TIME value.
  @return             The length of the result string.
*/
static inline int
TIME_to_datetime_str(char *to, const MYBLOCKCHAIN_TIME *ltime)
{
  uint32 temp, temp2;
  /* Year */
  temp= ltime->year / 100;
...
/**
  ...
  @l_time       The MYBLOCKCHAIN_TIME value to print.
...
/*
  ...
  MyBlockchain time conversion formats.
...
int my_TIME_to_str(const MYBLOCKCHAIN_TIME *l_time, char *to, uint dec)
{
  ...
  case MYBLOCKCHAIN_TIMESTAMP_NONE:
  case MYBLOCKCHAIN_TIMESTAMP_ERROR:
    to[0]='\0';
    return 0;
  default:
    DBUG_ASSERT(0);
...
/*
  ...
  DESCRIPTION
    ...
    YYYYMMDDHHMMSS to broken-down MYBLOCKCHAIN_TIME representation. Return value in
  ...
  RETURN VALUE
    ...
    was_cut         if return value -1: one of
                      - MYBLOCKCHAIN_TIME_WARN_OUT_OF_RANGE
                      - MYBLOCKCHAIN_TIME_WARN_ZERO_DATE
                      - MYBLOCKCHAIN_TIME_WARN_TRUNCATED
                    otherwise 0.
*/
                            ...
                            my_time_flags_t flags, int *was_cut)
{
  ...
  *was_cut= 0;
  ...
  time_res->time_type=MYBLOCKCHAIN_TIMESTAMP_DATE;

  if (nr == 0LL || nr >= 10000101000000LL)
  {
    time_res->time_type=MYBLOCKCHAIN_TIMESTAMP_DATETIME;
 ...
 err:
  *was_cut= MYBLOCKCHAIN_TIME_WARN_TRUNCATED;
  return -1LL;
}
...
/**
  ...
  @param  my_time  The MYBLOCKCHAIN_TIME value to convert.
  @return          A number in format YYYYMMDDHHMMSS.
*/
ulonglong TIME_to_ulonglong_datetime(const MYBLOCKCHAIN_TIME *my_time)
{
...
/**
  Convert MYBLOCKCHAIN_TIME value to integer in YYYYMMDD format
  @param my_time  The MYBLOCKCHAIN_TIME value to convert.
  @return         A number in format YYYYMMDD.
*/
ulonglong TIME_to_ulonglong_date(const MYBLOCKCHAIN_TIME *my_time)
{
...
/**
  Convert MYBLOCKCHAIN_TIME value to integer in HHMMSS format.
  This function doesn't take into account time->day member:
  it's assumed that days have been converted to hours already.
  @param my_time  The TIME value to convert.
  @return         The number in HHMMSS format.
*/
ulonglong TIME_to_ulonglong_time(const MYBLOCKCHAIN_TIME *my_time)
{
...
/**
  ...
  @param ltime    MYBLOCKCHAIN_TIME variable
...
/*
  Convert struct MYBLOCKCHAIN_TIME (date and time split into year/month/day/hour/...
  ...
  NOTE
    This function doesn't check that given MYBLOCKCHAIN_TIME structure members are
    in valid range. If they are not, return value won't reflect any
    valid date either.
*/

ulonglong TIME_to_ulonglong(const MYBLOCKCHAIN_TIME *my_time)
{
...
*/
longlong TIME_to_longlong_time_packed(const MYBLOCKCHAIN_TIME *ltime)
{
...
/**
  ...
  @param  OUT ltime  The MYBLOCKCHAIN_TIME variable to set.
  @param      tmp    The packed numeric representation.
*/
void TIME_from_longlong_time_packed(MYBLOCKCHAIN_TIME *ltime, longlong tmp)
{
  longlong hms;
  if ((ltime->neg= (tmp < 0)))
    tmp= -tmp;
...
*/
longlong TIME_to_longlong_datetime_packed(const MYBLOCKCHAIN_TIME *ltime)
{
...
/**
  ...
  @return      Packed numeric representation of ltime.
*/
longlong TIME_to_longlong_date_packed(const MYBLOCKCHAIN_TIME *ltime)
{
  longlong ymd= ((ltime->year * 13 + ltime->month) << 5) | ltime->day;
...
/**
  Convert packed numeric datetime representation to MYBLOCKCHAIN_TIME.
  @param OUT  ltime The datetime variable to convert to.
  @param      tmp   The packed numeric datetime value.
*/
void TIME_from_longlong_datetime_packed(MYBLOCKCHAIN_TIME *ltime, longlong tmp)
{
  longlong ymd, hms;
  longlong ymdhms, ym;
  if ((ltime->neg= (tmp < 0)))
...
/**
  Convert packed numeric date representation to MYBLOCKCHAIN_TIME.
  @param OUT  ltime The date variable to convert to.
  @param      tmp   The packed numeric date value.
*/
void TIME_from_longlong_date_packed(MYBLOCKCHAIN_TIME *ltime, longlong tmp)
{
...
*/
longlong TIME_to_longlong_packed(const MYBLOCKCHAIN_TIME *ltime)
{
