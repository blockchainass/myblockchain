...
#if !defined(HAVE_YASSL)
myblockchain_mutex_t g_public_key_mutex;
#endif

int sha256_password_init(char *a, size_t b, int c, va_list d)
{
#if !defined(HAVE_YASSL)
  myblockchain_mutex_init(0,&g_public_key_mutex, MY_MUTEX_INIT_SLOW);
#endif
  return 0;
}
...
{
#if !defined(HAVE_YASSL)
  myblockchain_mutex_destroy(&g_public_key_mutex);
#endif
  return 0;
}
...
/**
  ...
  @param myblockchain connection handle with file path data
 
  @return Pointer to the RSA public key storage buffer
*/

RSA *rsa_init(MYBLOCKCHAIN *myblockchain)
{
  static RSA *g_public_key= NULL;
  RSA *key= NULL;

  myblockchain_mutex_lock(&g_public_key_mutex);
  key= g_public_key;
  myblockchain_mutex_unlock(&g_public_key_mutex);
  ...
  if (myblockchain->options.extension != NULL &&
      myblockchain->options.extension->server_public_key_path != NULL &&
      myblockchain->options.extension->server_public_key_path != '\0')
  {
    pub_key_file= fopen(myblockchain->options.extension->server_public_key_path,
                        "r");
  }
  ...
  if (pub_key_file == NULL)
  {
    ...
    */
    my_message_local(WARNING_LEVEL, "Can't locate server public key '%s'",
                     myblockchain->options.extension->server_public_key_path);
...
/**
  ...
  @param myblockchain Client connection handler
...
extern "C"
int sha256_password_auth_client(MYBLOCKCHAIN_PLUGIN_VIO *vio, MYBLOCKCHAIN *myblockchain)
{
  bool uses_password= myblockchain->passwd[0] != 0;
#if !defined(HAVE_YASSL)
  ...
  */
  ...
  if (myblockchain_get_ssl_cipher(myblockchain) != NULL)
  ...
  {
 #if !defined(HAVE_YASSL)
    public_key= rsa_init(myblockchain);
#endif
  }
  ...
  else
  {
    ...
    unsigned int passwd_len= static_cast<unsigned int>(strlen(myblockchain->passwd) + 1);
    if (!connection_is_secure)
    {
#if !defined(HAVE_YASSL)
      /*
      ...
      */
      ...
      memmove(passwd_scramble, myblockchain->passwd, passwd_len);
      ...
      */
      ...
      if (vio->write_packet(vio, (uchar*) encrypted_password, cipher_length))
        DBUG_RETURN(CR_ERROR);
#else
      set_myblockchain_extended_error(myblockchain, CR_AUTH_PLUGIN_ERR, unknown_sqlstate,
    ...
    else
    {
      ...
      if (vio->write_packet(vio, (uchar*) myblockchain->passwd, passwd_len))
        DBUG_RETURN(CR_ERROR);
    }
  }
