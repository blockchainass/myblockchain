...
BEGIN {
  # Check that myblockchain-test-run.pl is started from myblockchain-test/
  unless ( -f "myblockchain-test-run.pl" )
  {
    print "**** ERROR **** ",
      "You must start myblockchain-test-run from the myblockchain-test/ directory\n";
    exit(1);
  }
...
BEGIN {
  ...
  {
    print "=======================================================\n";
    print "  WARNING: Using myblockchain-test-run.pl version 1!  \n";
    print "=======================================================\n";
    # Should use exec() here on *nix but this appears not to work on Windows
    exit(system($^X, "lib/v1/myblockchain-test-run.pl", @ARGV) >> 8);
  }
  elsif ( $version == 2 )
  {
  ...
  {
    print "ERROR: Version $version of myblockchain-test-run does not exist!\n";
    exit(1);
  }
}
...
my $myblockchain_version_extra;
our $glob_myblockchain_test_dir;
our $basedir;
our $bindir;
...
our $exe_myblockchain_plugin;
our $exe_myblockchainadmin;
our $exe_myblockchaintest;
our $exe_libtool;
our $exe_myblockchain_embedded;
our $exe_myblockchain_ssl_rsa_setup;
...
our $opt_gcov_err= "myblockchain-test-gcov.err";
our $opt_gcov_msg= "myblockchain-test-gcov.msg";
...
sub run_test_server ($$$) {
  ...
  my %running;
  my $result;
  my $exe_myblockchaind= find_myblockchaind($basedir) || ""; # Used as hint to CoreDump
	...
	if ($line eq 'TESTRESULT'){
	  ...
	  if ( $result->is_failed() ) {
		...
		$num_saved_datadir >= $opt_max_save_datadir)
	    {
	      ...
	      # Move any core files from e.g. myblockchaintest
		...
		find({ no_chdir => 1,
			 ...
			 if (($core_name =~ /^core/ and $core_name !~ /\.gz$/)
			   ...
			   My::CoreDump->show($core_file, $exe_myblockchaind, $opt_parallel);
...
sub run_worker ($) {
  ...
  while (my $line= <$server>){
	...
	print $server "VALGREP\n" if $valgrind_reports;
      ...
      if ( $opt_gprof ) {
	gprof_collect (find_myblockchaind($basedir), keys %gprof_dirs);
      }
      mark_time_used('admin');
...
sub set_vardir {
  ...
  $path_testlog=         "$opt_vardir/log/myblockchaintest.log";
...
sub print_global_resfile {
  ...
  resfile_global("myblockchaind", \@opt_extra_myblockchaind_opt);
...
#  resfile_global("default-myisam", $opt_default_myisam ? 1 : 0);
  resfile_global("product", "MyBlockchain");
  # Somewhat hacky code to convert numeric version back to dot notation
  my $v1= int($myblockchain_version_id / 10000);
  my $v2= int(($myblockchain_version_id % 10000)/100);
  my $v3= $myblockchain_version_id % 100;
...
sub command_line_setup {
  ...
  my %options=(
	     ...
	     'port-base|mtr-port-base=i'       => \$opt_port_base,
             ...
             'myblockchaind-env=s'             => \@opt_myblockchaind_envs,

             # Extra options used when bootstrapping myblockchaind
  ...
  if ($glob_myblockchain_test_dir =~ / /)
  {
    die("Working directory \"$glob_myblockchain_test_dir\" contains space\n".
	"Bailing out, cannot function properly with space in path");
  }
  if (IS_CYGWIN)
  {
    # Use mixed path format i.e c:/path/to/
    $glob_myblockchain_test_dir= mixed_path($glob_myblockchain_test_dir);
  }

  # In most cases, the base directory we find everything relative to,
  # is the parent directory of the "myblockchain-test" directory. For source
  # distributions, TAR binary distributions and some other packages.
  $basedir= dirname($glob_myblockchain_test_dir);
  ...
  $path_language=   mtr_path_exists("$bindir/share/myblockchain",
                                    "$bindir/sql/share",
                                    "$bindir/share");
  my $path_share= $path_language;
  $path_charsetsdir =   mtr_path_exists("$basedir/share/myblockchain/charsets",
  ...
  if (using_extern())
  {
    # Connect to the running myblockchaind and find out what it supports
    collect_myblockchaind_features_from_running_server();
  }
  else
  {
    # Run the myblockchaind to find out what features are available
  ...
  push(@opt_extra_myblockchaind_opt, "--log-syslog=0");
  ...
  if ( ! $opt_tmpdir )
  {
    ...
    if (check_socket_path_length("$opt_tmpdir/myblockchain_testsocket.sock"))
    {
  ...
  if ( $opt_embedded_server )
  {
    if ( IS_WINDOWS )
    {
      ...
      my $lib_myblockchaind=
        mtr_path_exists("$bindir/lib", vs_config_dirs('libmyblockchaind',''));
      if ( IS_CYGWIN )
      {
	$lib_myblockchaind= posix_path($lib_myblockchaind);
	$separator= ":";
      }
      $ENV{'PATH'}= "$ENV{'PATH'}".$separator.$lib_myblockchaind;
    }
  ...
  {
    push(@opt_extra_myblockchaind_opt, "--optimizer_trace=enabled=on,one_line=off");
    # some queries yield big traces:
    push(@opt_extra_myblockchaind_opt, "--optimizer-trace-max-mem-size=1000000");
  }
...
sub collect_myblockchaind_features {
  ...
  #
  # Execute "myblockchaind --no-defaults --help --verbose" to get a
  ...
  my $exe_myblockchaind= find_myblockchaind($basedir);
  my $cmd= join(" ", $exe_myblockchaind, @$args);
  my $list= `$cmd`;
  ...
  $myblockchaind_variables{'innodb'}= "ON";
...
{
  my $myblockchain= mtr_exe_exists("$path_client_bindir/myblockchain");
  ...
  mtr_add_arg($args, "-e '%s'", "use myblockchain; SHOW GLOBAL VARIABLES");
  my $cmd= "$myblockchain " . join(' ', @$args);
  mtr_verbose("cmd: $cmd");
  ...
  # Parse version
  my $version_str= $myblockchaind_variables{'version'};
...
sub find_myblockchaind {

  my ($myblockchaind_basedir)= $ENV{MTR_BINDIR}|| @_;

  my @myblockchaind_names= ("myblockchaind", "myblockchaind-max-nt", "myblockchaind-max",
		     "myblockchaind-nt");

  if ( $opt_debug_server ){
    # Put myblockchaind-debug first in the list of binaries to look for
    mtr_verbose("Adding myblockchaind-debug first in list of binaries to look for");
    unshift(@myblockchaind_names, "myblockchaind-debug");
  }

  return my_find_bin($myblockchaind_basedir,
...
sub executable_setup () {
  ...
  $exe_myblockchain_plugin=   mtr_exe_exists("$path_client_bindir/myblockchain_plugin");
  $exe_myblockchain_ssl_rsa_setup= mtr_exe_exists("$path_client_bindir/myblockchain_ssl_rsa_setup");

  $exe_myblockchain_embedded=
    mtr_exe_maybe_exists(vs_config_dirs('libmyblockchaind/examples','myblockchain_embedded'),
                         "$bindir/libmyblockchaind/examples/myblockchain_embedded",
                         "$bindir/bin/myblockchain_embedded");
  ...
  if ( $opt_embedded_server )
  {
    $exe_myblockchaintest=
      mtr_exe_exists(vs_config_dirs('libmyblockchaind/examples','myblockchaintest_embedded'),
                     "$basedir/libmyblockchaind/examples/myblockchaintest_embedded",
                     "$path_client_bindir/myblockchaintest_embedded");
  }
  else
  {
    if ( defined $ENV{'MYBLOCKCHAIN_TEST'} )
    {
      $exe_myblockchaintest=$ENV{'MYBLOCKCHAIN_TEST'};
      print "===========================================================\n";
      print "WARNING:The myblockchaintest binary is fetched from $exe_myblockchaintest\n";
      print "===========================================================\n";
    }
    else
    {
...
sub client_debug_arg($$) {
  ...
  return if $client_name =~ /^myblockchainbinlog/;
...
sub myblockchainslap_arguments () {
  my $exe= mtr_exe_maybe_exists("$path_client_bindir/myblockchainslap");
  if ( $exe eq "" ) {
    # myblockchainap was not found

    if (defined $myblockchain_version_id and $myblockchain_version_id >= 50100 ) {
      mtr_error("Could not find the myblockchainslap binary");
    }
    return ""; # Don't care about myblockchainslap
  }

  my $args;
  mtr_init_args(\$args);
  mtr_add_arg($args, "--defaults-file=%s", $path_config_file);
  client_debug_arg($args, "myblockchainslap");
  return mtr_args2str($exe, @$args);
}


sub myblockchaindump_arguments ($) {
  my($group_suffix) = @_;
  my $exe= mtr_exe_exists("$path_client_bindir/myblockchaindump");

  my $args;
  mtr_init_args(\$args);
  mtr_add_arg($args, "--defaults-file=%s", $path_config_file);
  mtr_add_arg($args, "--defaults-group-suffix=%s", $group_suffix);
  client_debug_arg($args, "myblockchaindump-$group_suffix");
  return mtr_args2str($exe, @$args);
}
...
sub myblockchain_client_test_arguments(){
  my $exe;
  # myblockchain_client_test executable may _not_ exist
  $exe= mtr_exe_maybe_exists(vs_config_dirs('tests', 'myblockchain_client_test'),
			     "$basedir/tests/myblockchain_client_test",
			     "$basedir/bin/myblockchain_client_test");
  return "" unless $exe;
  my $args;
  mtr_init_args(\$args);
  ...
  client_debug_arg($args,"myblockchain_client_test");

  return mtr_args2str($exe, @$args);
}

sub myblockchainpump_arguments ($) {
  my($group_suffix) = @_;
  my $exe= mtr_exe_exists(vs_config_dirs('client/dump','myblockchainpump'),
                          "$basedir/client/myblockchainpump",
                          "$path_client_bindir/myblockchainpump");

  my $args;
  mtr_init_args(\$args);
  mtr_add_arg($args, "--defaults-file=%s", $path_config_file);
  mtr_add_arg($args, "--defaults-group-suffix=%s", $group_suffix);
  client_debug_arg($args, "myblockchainpump-$group_suffix");
  return mtr_args2str($exe, @$args);
}
#
# Set environment to be used by childs of this process for
# things that are constant during the whole lifetime of myblockchain-test-run
#
...
sub find_plugin($$)
{
  ...
  my $lib_plugin=
    mtr_file_exists(vs_config_dirs($location,$plugin_filename),
                    ...
                    "$basedir/lib/myblockchain/plugin/".$plugin_filename,
                    "$basedir/lib64/myblockchain/plugin/".$plugin_filename,
                    );
  return $lib_plugin;
}
...
sub read_plugin_defs($)
{
  ...
  # Need to check if we will be running myblockchaind-debug
  if ($opt_debug_server) {
    $running_debug= 1 if find_myblockchaind($basedir) =~ /myblockchaind-debug/;
  }
...
sub environment_setup {
  ...
  else
  {
    ...
    if ( $source_dist )
    {
      push(@ld_library_paths, "$basedir/libmyblockchain/.libs/",
	   "$basedir/libmyblockchain_r/.libs/",
	   "$basedir/zlib/.libs/");
    }
    else
    {
      push(@ld_library_paths, "$basedir/lib", "$basedir/lib/myblockchain");
    }
  }
  ...
  $ENV{'DEFAULT_MASTER_PORT'}= $myblockchaind_variables{'port'};
  $ENV{'MYBLOCKCHAIN_TMP_DIR'}=      $opt_tmpdir;
  $ENV{'MYBLOCKCHAINTEST_VARDIR'}=   $opt_vardir;
  $ENV{'MYBLOCKCHAIN_BINDIR'}=       "$bindir";
  $ENV{'MYBLOCKCHAIN_SHAREDIR'}=     $path_language;
  $ENV{'MYBLOCKCHAIN_CHARSETSDIR'}=  $path_charsetsdir;
  
  if (IS_WINDOWS)
  {
    $ENV{'SECURE_LOAD_PATH'}= $glob_myblockchain_test_dir."\\std_data";
    $ENV{'MYBLOCKCHAIN_TEST_LOGIN_FILE'}=
  ...
  # ----------------------------------------------------
  # myblockchain clients
  # ----------------------------------------------------
  $ENV{'MYBLOCKCHAIN_CHECK'}=                 client_arguments("myblockchaincheck");
  $ENV{'MYBLOCKCHAIN_DUMP'}=                  myblockchaindump_arguments(".1");
  $ENV{'MYBLOCKCHAIN_DUMP_SLAVE'}=            myblockchaindump_arguments(".2");
  $ENV{'MYBLOCKCHAIN_SLAP'}=                  myblockchainslap_arguments();
  $ENV{'MYBLOCKCHAIN_IMPORT'}=                client_arguments("myblockchainimport");
  $ENV{'MYBLOCKCHAIN_SHOW'}=                  client_arguments("myblockchainshow");
  $ENV{'MYBLOCKCHAIN_CONFIG_EDITOR'}=         client_arguments_no_grp_suffix("myblockchain_config_editor");
  $ENV{'MYBLOCKCHAIN_PUMP'}=                  myblockchainpump_arguments(".1");

  if (!IS_WINDOWS)
  {
    $ENV{'MYBLOCKCHAIN_INSTALL_DB'}=         client_arguments_no_grp_suffix("myblockchain_install_db");
  }
  $ENV{'MYBLOCKCHAIN_BINLOG'}=                client_arguments("myblockchainbinlog");
  $ENV{'MYBLOCKCHAIN'}=                       client_arguments("myblockchain");
  $ENV{'MYBLOCKCHAIN_SLAVE'}=                 client_arguments("myblockchain", ".2");
  $ENV{'MYBLOCKCHAIN_UPGRADE'}=               client_arguments("myblockchain_upgrade");
  $ENV{'MYBLOCKCHAIN_SECURE_INSTALLATION'}=   "$path_client_bindir/myblockchain_secure_installation";
  $ENV{'MYBLOCKCHAINADMIN'}=                  native_path($exe_myblockchainadmin);
  $ENV{'MYBLOCKCHAIN_CLIENT_TEST'}=           myblockchain_client_test_arguments();
  $ENV{'EXE_MYBLOCKCHAIN'}=                   $exe_myblockchain;
  $ENV{'MYBLOCKCHAIN_PLUGIN'}=                $exe_myblockchain_plugin;
  $ENV{'MYBLOCKCHAIN_EMBEDDED'}=              $exe_myblockchain_embedded;
  $ENV{'PATH_CONFIG_FILE'}=            $path_config_file;
  $ENV{'MYBLOCKCHAIN_SSL_RSA_SETUP'}=         $exe_myblockchain_ssl_rsa_setup;

  my $exe_myblockchaind= find_myblockchaind($basedir);
  $ENV{'MYBLOCKCHAIND'}= $exe_myblockchaind;
  my $extra_opts= join (" ", @opt_extra_myblockchaind_opt);
  $ENV{'MYBLOCKCHAIND_CMD'}= "$exe_myblockchaind --defaults-group-suffix=.1 ".
  ...
  $ENV{'MYBLOCKCHAIN_BUG25714'}=  native_path($exe_bug25714);

  # ----------------------------------------------------
  # Get the bin dir
  # ----------------------------------------------------
  $ENV{'MYBLOCKCHAIN_BIN_PATH'}=  native_path($bindir);

  # ----------------------------------------------------
  # myblockchain_fix_privilege_tables.sql
  # ----------------------------------------------------
  my $file_myblockchain_fix_privilege_tables=
    mtr_file_exists("$basedir/scripts/myblockchain_fix_privilege_tables.sql",
		    "$basedir/share/myblockchain_fix_privilege_tables.sql",
		    "$basedir/share/myblockchain/myblockchain_fix_privilege_tables.sql",
                    "$bindir/scripts/myblockchain_fix_privilege_tables.sql",
		    "$bindir/share/myblockchain_fix_privilege_tables.sql",
		    "$bindir/share/myblockchain/myblockchain_fix_privilege_tables.sql");
  ...
  $ENV{'MYBLOCKCHAIN_MY_PRINT_DEFAULTS'}= native_path($exe_my_print_defaults);
  ...
  # myblockchaind_safe
  # ----------------------------------------------------
  my $myblockchaind_safe=
    mtr_pl_maybe_exists("$bindir/scripts/myblockchaind_safe") ||
    mtr_pl_maybe_exists("$path_client_bindir/myblockchaind_safe");
  if ($myblockchaind_safe)
  {
    $ENV{'MYBLOCKCHAIND_SAFE'}= $myblockchaind_safe;
  }
  ...
  # Add dir of this perl to aid myblockchaintest in finding perl
...
sub setup_vardir() {
  ...
  if (check_socket_path_length("$opt_tmpdir/myblockchain_testsocket.sock")){
    mtr_error("Socket path '$opt_tmpdir' too long, it would be ",
	      ...
	      "MyBlockchain Server. Set a shorter with --tmpdir=<path> option");
  }

  # copy all files from std_data into var/std_data
  # and make them world readable
  copytree("$glob_myblockchain_test_dir/std_data", "$opt_vardir/std_data", "0022");
...
sub check_ndbcluster_support ($) {
  ...
  # Check if this is MyBlockchain Cluster, ie. myblockchain version string ends
  # with -ndb-Y.Y.Y[-status]
  if ( defined $myblockchain_version_extra &&
       $myblockchain_version_extra =~ /-ndb-([0-9]*)\.([0-9]*)\.([0-9]*)/ )
  {
    # MyBlockchain Cluster tree
    mtr_report(" - MyBlockchain Cluster detected");
    ...
    {
      # MyBlockchain Cluster tree, but myblockchaind was not compiled with
      # ndbcluster -> fail unless --skip-ndbcluster was used
      mtr_error("This is MyBlockchain Cluster but myblockchaind does not " .
		"support ndbcluster. Use --skip-ndbcluster to " .
		"force mtr to run without it.");
    }

    # myblockchaind was compiled with ndbcluster -> auto enable
  }
  else
  {
    # Not a MyBlockchain Cluster tree
    if (!$ndbcluster_supported)
    {
	...
	mtr_error("Could not detect ndbcluster support ".
      ...
      # Silently skip, myblockchaind was compiled without ndbcluster
  ...
  $ndbcluster_enabled= 1;
  # Add MyBlockchain Cluster test suites
...
sub memcached_start {
  ...
  my $found_perl_source = my_find_file($bindir,
     ["storage/ndb/memcache",        # source
      "myblockchain-test/lib",              # install
      "share/myblockchain-test/lib"],       # install
      "memcached_path.pl");
...
sub memcached_load_metadata($) {
  ...
  foreach my $myblockchaind (myblockchainds())
  {
    if(-d $myblockchaind->value('datadir') . "/" . "ndbmemcache")
    {
  ...
  my $sql_script= my_find_file($bindir,
                              ["share/myblockchain/memcache-api", # RPM install
...
# binlog reads from [client] and [myblockchainbinlog]
[myblockchainbinlog]
...
sub initialize_servers {
  ...
  else
  {
    ...
    if ( ! $opt_start_dirty )
    {
      ...
      myblockchain_install_db(default_myblockchaind(), "$opt_vardir/install.db");
    }
  }
}
...
sub default_myblockchaind {
  # Generate new config file from template
  my $config= My::ConfigFactory->new_config
    ( {
       basedir         => $basedir,
       testdir         => $glob_myblockchain_test_dir,
       template_path   => "include/default_my.cnf",
       vardir          => $opt_vardir,
       tmpdir          => $opt_tmpdir,
       baseport        => 0,
  ...
  my $myblockchaind= $config->group('myblockchaind.1')
    or mtr_error("Couldn't find myblockchaind.1 in default config");
  return $myblockchaind;
}


sub myblockchain_install_db {
  my ($myblockchaind, $datadir)= @_;

  my $install_datadir= $datadir || $myblockchaind->value('datadir');
  my $install_basedir= $myblockchaind->value('basedir');
  my $install_lang= $myblockchaind->value('lc-messages-dir');
  my $install_chsdir= $myblockchaind->value('character-sets-dir');

  mtr_report("Installing system blockchain...");
  ...
  my $exe_myblockchaind_bootstrap =
    $ENV{'MYBLOCKCHAIND_BOOTSTRAP'} || find_myblockchaind($install_basedir);
  ...
  if ($opt_boot_gdb) {
    gdb_arguments(\$args, \$exe_myblockchaind_bootstrap, $myblockchaind->name(),
		  $bootstrap_sql_file);
  }
  if ($opt_boot_dbx) {
    dbx_arguments(\$args, \$exe_myblockchaind_bootstrap, $myblockchaind->name(),
		  $bootstrap_sql_file);
  }
  if ($opt_boot_ddd) {
    ddd_arguments(\$args, \$exe_myblockchaind_bootstrap, $myblockchaind->name(),
		  $bootstrap_sql_file);
  }

  my $path_sql= my_find_file($install_basedir,
			     ["myblockchain", "sql/share", "share/myblockchain",
			      "share", "scripts"],
			      "myblockchain_system_tables.sql",
			     NOT_REQUIRED);
  ...
  # Create directories myblockchain and test
  mkpath("$install_datadir/myblockchain");
  mkpath("$install_datadir/test");

  if ( My::SafeProcess->run
       (
	name          => "bootstrap",
	path          => $exe_myblockchaind_bootstrap,
	args          => \$args,
...
sub run_query {
  my ($tinfo, $myblockchaind, $query)= @_;

  my $args;
  mtr_init_args(\$args);
  mtr_add_arg($args, "--defaults-file=%s", $path_config_file);
  mtr_add_arg($args, "--defaults-group-suffix=%s", $myblockchaind->after('myblockchaind'));

  mtr_add_arg($args, "-e %s", $query);

  my $res= My::SafeProcess->run
    (
     name          => "run_query -> ".$myblockchaind->name(),
     path          => $exe_myblockchain,
     args          => \$args,
     output        => '/dev/null',
     error         => '/dev/null'
...
sub check_testcase($$)
{
  ...
  # Start the myblockchaintest processes in parallel to save time
  # also makes it possible to wait for any process to exit during the check
  my %started;
  foreach my $myblockchaind ( myblockchainds() )
  {
    # Skip if server has been restarted with additional options
    if ( defined $myblockchaind->{'proc'} && ! exists $myblockchaind->{'restart_opts'} )
    {
      my $proc= start_check_testcase($tinfo, $mode, $myblockchaind);
  ...
  while (1){
	...
	{
	  # Test failed, grab the report myblockchaintest has created
...
# Start run myblockchaintest on one server
#
# RETURN VALUE
#  0 OK
#  1 Check failed
#
sub start_run_one ($$) {
  my ($myblockchaind, $run)= @_;

  my $name= "$run-".$myblockchaind->name();
...
sub run_on_all($$)
{
  ...
  foreach my $myblockchaind ( myblockchainds() )
  {
    if ( defined $myblockchaind->{'proc'} )
    {
      my $proc= start_run_one($myblockchaind, $run);
      $started{$proc->pid()}= $proc;
    }
  }
  ...
  while (1){
    ...
    if ( delete $started{$proc->pid()} ) {
      ...
      # One myblockchaintest process returned
      my $err_file= $proc->user_data();
      my $res= $proc->exit_status();
...
sub restart_forced_by_test($)
{
  ...
  foreach my $myblockchaind ( myblockchainds() )
  {
    my $datadir = $myblockchaind->value('datadir');
...
sub run_testcase ($) {
  ...
  if ( $start_only )
  {
    ...
    foreach my $myblockchaind ( myblockchainds() )
    {
      mtr_print ($myblockchaind->name() . "  " . $myblockchaind->value('port') .
	      "  " . $myblockchaind->value('socket'));
    }
    if ( $opt_start_exit )
    {
  ...
  my $test= start_myblockchaintest($tinfo);
  ...
  while (1)
  {
	...
	$tinfo->{comment}=
	  "myblockchaintest failed with unexpected return code $res\n";
	report_failure_and_restart($tinfo);
      }
  ...
  SRVDIED:
	...
	get_log_from_proc($proc, $tinfo->{name});
      ...
      # ----------------------------------------------------
      # It's not myblockchaintest that has exited, kill it
      # ----------------------------------------------------
      $test->kill();
    ...
    # Try to dump core for myblockchaintest and all servers
...
sub get_log_from_proc ($$) {
  ...
  foreach my $myblockchaind (myblockchainds()) {
    if ($myblockchaind->{proc} eq $proc) {
      my @srv_lines= extract_server_log($myblockchaind->value('#log-error'), $name);
...
# Write all suspicious lines to $error_log.warnings file
  ...
  my @patterns =
    (
     qr/^Warning:|myblockchaind: Warning|\[Warning\]/,
     qr/^Error:|\[ERROR\]/,
...
sub start_check_warnings ($$) {
  ...
  my $name= "warnings-".$myblockchaind->name();

  my $log_error= $myblockchaind->value('#log-error');
  ...
  if ( $opt_embedded_server )
  {
    ...
    my $myblockchaind=  $config->group('embedded')
      or mtr_error("Could not get [embedded] section");

    my $myblockchaind_args;
    mtr_init_args(\$myblockchaind_args);
    my $extra_opts= get_extra_opts($myblockchaind, $tinfo);
    myblockchaind_arguments($myblockchaind_args, $myblockchaind, $extra_opts);
    mtr_add_arg($args, "--server-arg=%s", $_) for @$myblockchaind_args;
  }
...
sub check_warnings ($) {
  ...
  foreach my $myblockchaind ( myblockchainds() )
  {
    if ( defined $myblockchaind->{'proc'} )
    {
      my $proc= start_check_warnings($tinfo, $myblockchaind);
      $started{$proc->pid()}= $proc;
    }
  }
...
sub check_expected_crash_and_restart {
  ...
  foreach my $myblockchaind ( myblockchainds() )
  {
    next unless ( $myblockchaind->{proc} and $myblockchaind->{proc} eq $proc );

    # Check if crash expected by looking at the .expect file
    # in var/tmp
    my $expect_file= "$opt_vardir/tmp/".$myblockchaind->name().".expect";
    if ( -f $expect_file )
    {
	...
	# If last line begins "restart:", the rest of the line is read as
        # extra command line options to add to the restarted myblockchaind.
        # Anything other than 'wait' or 'restart:' (with a colon) will
        # result in a restart with original myblockchaind options.
	if ($last_line =~ /restart:(.+)/) {
	  my @rest_opt= split(' ', $1);
	  $myblockchaind->{'restart_opts'}= \@rest_opt;
	} else {
	  delete $myblockchaind->{'restart_opts'};
	}
	unlink($expect_file);

	# Start server with same settings as last time
	myblockchaind_start($myblockchaind, $myblockchaind->{'started_opts'});
...
sub clean_datadir {
  ...
  foreach my $myblockchaind ( myblockchainds() )
  {
    my $myblockchaind_dir= dirname($myblockchaind->value('datadir'));
    if (-d $myblockchaind_dir ) {
      mtr_verbose(" - removing '$myblockchaind_dir'");
      rmtree($myblockchaind_dir);
    }
  }
...
sub after_failure ($) {
  ...
  if ( clusters() ) {
    ...
    foreach my $myblockchaind ( myblockchainds() ) {
      my $data_dir= $myblockchaind->value('datadir');
...
sub report_failure_and_restart ($) {
  ...
  if ($opt_valgrind_myblockchaind && ($tinfo->{'warnings'} || $tinfo->{'timeout'})) {
  ...
  if ( !defined $tinfo->{logfile} )
  {
	...
	if ($tinfo->{logfile} !~ /\n/)
	{
	  ...
	  $tinfo->{comment}.= "myblockchaintest failed but provided no output\n";
	  my $log_file_name= $opt_vardir."/log/".$tinfo->{shortname}.".log";
	  if (-e $log_file_name) {
	    $tinfo->{comment}.=
...
sub myblockchaind_stop {
  my $myblockchaind= shift or die "usage: myblockchaind_stop(<myblockchaind>)";

  my $args;
  mtr_init_args(\$args);

  mtr_add_arg($args, "--no-defaults");
  mtr_add_arg($args, "--character-sets-dir=%s", $myblockchaind->value('character-sets-dir'));
  mtr_add_arg($args, "--user=%s", $opt_user);
  mtr_add_arg($args, "--password=");
  mtr_add_arg($args, "--port=%d", $myblockchaind->value('port'));
  mtr_add_arg($args, "--host=%s", $myblockchaind->value('#host'));
  ...
  My::SafeProcess->run
    (
     name          => "myblockchainadmin shutdown ".$myblockchaind->name(),
     path          => $exe_myblockchainadmin,
     args          => \$args,
     error         => "/dev/null",
...
sub myblockchaind_arguments ($$$) {
  ...
  if ( $myblockchain_version_id >= 50106 && !$opt_user_args)
  {
  ...
  foreach my $arg ( @$extra_opts )
  {
    ...
    elsif ($skip_binlog and mtr_match_prefix($arg, "--binlog-format"))
    {
      ; # Dont add --binlog-format when running without binlog
           ...
           $myblockchaind->option("log-slave-updates"))
    {
      ; # Dont add --skip-log-bin when myblockchaind have --log-slave-updates in config
    }
    elsif ($arg eq "")
    {
...
sub myblockchaind_start ($$) {
  ...
  mtr_verbose(My::Options::toStr("myblockchaind_start", @$extra_opts));

  my $exe= find_myblockchaind($myblockchaind->value('basedir'));
...
# implementation for strace-server
  ...
  {
    strace_server_arguments($args, \$exe, $myblockchaind->name());
  }
  ...
  if (exists $myblockchaind->{'restart_opts'}) {
    push (@all_opts, @{$myblockchaind->{'restart_opts'}});
    mtr_verbose(My::Options::toStr("myblockchaind_start restart",
				   @{$myblockchaind->{'restart_opts'}}));
  }
  myblockchaind_arguments($args,$myblockchaind,\@all_opts);

  if ( $opt_debug )
  {
    mtr_add_arg($args, "--debug=$debug_d:t:i:A,%s/log/%s.trace",
		$path_vardir_trace, $myblockchaind->name());
  }

  if ( $opt_gdb || $opt_manual_gdb )
  {
    gdb_arguments(\$args, \$exe, $myblockchaind->name());
  }
  elsif ( $opt_manual_lldb )
  {
    lldb_arguments(\$args, \$exe, $myblockchaind->name());
  }
  elsif ( $opt_ddd || $opt_manual_ddd )
  {
    ddd_arguments(\$args, \$exe, $myblockchaind->name());
  }
  if ( $opt_dbx || $opt_manual_dbx ) {
    dbx_arguments(\$args, \$exe, $myblockchaind->name());
  }
  elsif ( $opt_debugger )
  {
    debugger_arguments(\$args, \$exe, $myblockchaind->name());
  }
  elsif ( $opt_manual_debug )
  {
     print "\nStart " .$myblockchaind->name()." in your debugger\n" .
  ...
  unlink($myblockchaind->value('pid-file'));

  my $output= $myblockchaind->value('#log-error');
  # Remember this log file for valgrind error report search
  $myblockchaind_logs{$output}= 1 if $opt_valgrind;
  # Remember data dir for gmon.out files if using gprof
  $gprof_dirs{$myblockchaind->value('datadir')}= 1 if $opt_gprof;

  if ( defined $exe )
  {
    $myblockchaind->{'proc'}= My::SafeProcess->new
      (
       name          => $myblockchaind->name(),
       path          => $exe,
       args          => \$args,
       output        => $output,
       error         => $output,
       ...
       host          => undef,
       shutdown      => sub { myblockchaind_stop($myblockchaind) },
       envs          => \@opt_myblockchaind_envs,
       pid_file      => $myblockchaind->value('pid-file'),
       daemon_mode   => $myblockchaind->{'daemonize'}
      );
    mtr_verbose("Started $myblockchaind->{proc}");
  }

  if ( $wait_for_pid_file &&
       !sleep_until_file_created($myblockchaind->value('pid-file'),
				 $opt_start_timeout,
				 $myblockchaind->{'proc'}))
  {
    my $mname= $myblockchaind->name();
    mtr_error("Failed to start myblockchaind $mname with command $exe");
  }

  # Remember options used when starting
  $myblockchaind->{'started_opts'}= $extra_opts;
...
sub is_slave {
  ...
  # a myblockchaind is master or slave. Best guess is to treat all which haven't
...
sub server_need_restart {
  ...
  my $is_myblockchaind= grep ($server eq $_, myblockchainds());
  if ($is_myblockchaind)
  {
...
sub servers_need_restart($) {
  ...
  # Build list of master and slave myblockchainds to be able to restart
  # all slaves whenever a master restarts.
  my @masters;
  my @slaves;
  foreach my $server (myblockchainds())
  {
    if (is_slave($server))
    {
      push(@slaves, $server);
...
sub myblockchainds { return _like('myblockchaind.'); }
sub ndbds   { return _like('cluster_config.ndbd.');}
sub ndb_mgmds { return _like('cluster_config.ndb_mgmd.'); }
sub clusters  { return _like('myblockchain_cluster.'); }
sub memcacheds { return _like('memcached.'); }
sub all_servers { return ( myblockchainds(), ndb_mgmds(), ndbds(), memcacheds() ); }
...
sub get_extra_opts {
  ...
  return \@opt_extra_myblockchaind_opt if $opt_user_args;

  my ($myblockchaind, $tinfo)= @_;

  my $opts=
    $myblockchaind->option("#!use-slave-opt") ?
...
sub stop_servers($$) {
  ...
  if ( join('|', @servers) eq join('|', all_servers()) )
  {
   ...
   mtr_report("Restarting all servers");
    ...
    #  myblockchaind processes
    My::SafeProcess::shutdown( $opt_shutdown_timeout, started(myblockchainds()) );
...
sub start_servers($) {
  ...
  # Start myblockchainds
  foreach my $myblockchaind ( myblockchainds() )
  {
    if ( $myblockchaind->{proc} )
    {
      # Already started

      # Write start of testcase to log file
      mark_log($myblockchaind->value('#log-error'), $tinfo);
	...
	next unless $myblockchaind->option($option_name);

	my $file_name= $myblockchaind->value($option_name);
	next unless
    ...
    my $myblockchaind_basedir= $myblockchaind->value('basedir');
    if ( $basedir eq $myblockchaind_basedir )
    {
    ...
    {
      myblockchain_install_db($myblockchaind); # For versional testing
    ...
    my $tmpdir= $myblockchaind->value('tmpdir');
    ...
    # Run <tname>-master.sh
    if ($myblockchaind->option('#!run-master-sh') and
    ...
    # Run <tname>-slave.sh
    if ($myblockchaind->option('#!run-slave-sh') and
    ...
    {
      my $extra_opts= get_extra_opts($myblockchaind, $tinfo);
      myblockchaind_start($myblockchaind,$extra_opts);

      # Save this test case information, so next can examine it
      $myblockchaind->{'started_tinfo'}= $tinfo;
  ...
  # Wait for myblockchainds to start
  foreach my $myblockchaind ( myblockchainds() )
  {
    next if !started($myblockchaind);

    if (sleep_until_file_created($myblockchaind->value('pid-file'),
				 $opt_start_timeout,
				 $myblockchaind->{'proc'}) == 0) {
      $tinfo->{comment}=
	"Failed to start ".$myblockchaind->name();

      my $logfile= $myblockchaind->value('#log-error');
...
sub start_check_testcase ($$$) {
  ...
  my $name= "check-".$myblockchaind->name();
  # Replace dots in name with underscore to avoid that myblockchaintest
...
sub run_myblockchaintest ($) {
  my $proc= start_myblockchaintest(@_);
  $proc->wait();
}


sub start_myblockchaintest ($) {
  my ($tinfo)= @_;
  ...
  # ----------------------------------------------------------------------
  $ENV{'MYBLOCKCHAIN_TEST'}= mtr_args2str($exe_myblockchaintest, @$args);
  ...
  client_debug_arg($args, "myblockchaintest");
...
sub gdb_arguments {
  ...
  # write init file for myblockchaind or client
...
sub lldb_arguments {
  ...
  mtr_tofile($lldb_init_file,
    ...
    print "cd $glob_myblockchain_test_dir && lldb -s $lldb_init_file $$exe\n";
...
sub dbx_arguments {
  ...
  if ( $opt_manual_dbx ) {
    ...
    print "cd $glob_myblockchain_test_dir; dbx -c \"stop in main; " .
          "$runline\" $$exe\n";
...
#
# Search server logs for valgrind reports printed at myblockchaind termination
#

sub valgrind_exit_reports() {
  my $found_err= 0;

  foreach my $log_file (keys %myblockchaind_logs)
  {
    my @culprits= ();
    my $valgrind_rep= "";
    my $found_report= 0;
...
Options to control what test suites or cases to run
  ...
  skip-sys-schema       Skip loading of the sys schema, and running the
                        sysschema test suite. An empty sys blockchain is
                        still created
...
Options that pass on options (these may be repeated)
  ...
  myblockchaind-env=VAR=VAL    Specify additional environment settings for "myblockchaind"
...
Options to run test on running server
  ...
  extern option=value   Run only the tests against an already started server
                        ...
                        For example:
                         ./$0 --extern socket=/tmp/myblockchaind.sock
...
Options for debugging the product
  ...
  client-dbx            Start myblockchaintest client in dbx
  ...
  dbx                   Start the myblockchaind(s) in dbx
  ddd                   Start the myblockchaind(s) in ddd
  ...
  manual-ddd            Let user manually start myblockchaind in ddd, before running
                        test(s)
  manual-dbx            Let user manually start myblockchaind in dbx, before running
                        test(s)
  manual-lldb           Let user manually start myblockchaind in lldb, before running 
                        test(s)
  strace-client         Create strace output for myblockchaintest client, 
  strace-server         Create strace output for myblockchaintest server, 
...
Misc options
  user=USER             User for connecting to myblockchaind(default: $opt_user)
  ...
  start                 Only initialize and start the servers, using the
                        ...
                        Example:
                         $0 --start alias &
  start-and-exit        Same as --start, but myblockchain-test-run terminates and
                        leaves just the server running
  start-dirty           Only start the servers (without initialization) for
                        the first specified test case
  user-args             In combination with start* and no test name, drops
                        arguments to myblockchaind except those specified with
                        --myblockchaind (if any)
  ...
  report-features       First run a "test" that reports myblockchain features
  timestamp             Print timestamp before each test report line
  timediff              With --timestamp, also print time passed since
                        *previous* test started
  max-connections=N     Max number of open connection to server in myblockchaintest
  ...
  stress=ARGS           Run stress test, providing options to
                        myblockchain-stress-test.pl. Options are separated by comma.
