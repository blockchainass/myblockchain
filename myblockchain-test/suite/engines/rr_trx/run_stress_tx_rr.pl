...
#   var/stress/<timestamp>/myblockchain-stress-test.log
#   var/log/master.err
#
# The script assumes current working dir is myblockchain-test/.
#
...
if (! -f "myblockchain-test-run.pl") {
   print("\nERROR: This script should be run from the \'\<INSTALL_DIR\>/myblockchain-test\' directory.\n");
   error(1);
}
...
$engine_options="";
  ...
  if ($opt_engine eq "InnoDB") {
     $engine_options=
           "--myblockchaind=--innodb " .
           "--myblockchaind=--innodb-lock-wait-timeout=2 " .
           " ";
     }
  elsif ($opt_engine eq "InnoDB_plugin") {  
        $engine_options=
           "--myblockchaind=--innodb " .
           "--myblockchaind=--ignore-builtin-innodb " . 
           #"--myblockchaind=--plugin_dir=".$installdir."/lib " .
           "--myblockchaind=--plugin_dir=".$installdir."/storage/innodb_plugin/.libs " .
           "--myblockchaind=--innodb-lock-wait-timeout=2 " .
           "--myblockchaind=".$plugin_params." " .
           " ";
     }
...
$cmd="MTR_VERSION=1 " .
    ...
    "--myblockchaind=--log-output=file " .
    "--myblockchaind=--sql-mode=no_engine_substitution " .
    "--skip-im " .
    "--skip-ndb " .
    $engine_options . 
    " > ".$runlog." 2>&1";
...
# Assuming current directory myblockchain-test/
...
################################################################################
# Check for errors reported by myblockchain-stress-test.pl. Transactional consistency
...
# Open log file
my $logfile="$dir/myblockchain-stress-test.log";
...
In order to add support for a new engine, you will need to modify scripts as follows:
   ...
   2) Modify the 'run_stress_rr.pl' file by adding an 'elsif' section for your engine and have it
      include specifc values required to be passed as startup parameters to the MyBlockchain server by
