...
class PopulateWorker(threading.Thread):
  ...
  def run(self):
    try:
      ...
      except MyBlockchaindb.Error, e2:
...
def populate_table(con, num_records_before, do_blob, log):
  ...
  start_id = 0
  for i in xrange(10):
     w = PopulateWorker(MyBlockchaindb.connect(user=user, host=host, port=port, db=db),
...
class Worker(threading.Thread):
  ...
  def runme(self):
    ...
    while not self.num_xactions or (self.loop_num < self.num_xactions):
      ...
      except MyBlockchaindb.Error, e:
        if e.args[0] == 2006:  # server is killed
          print >> self.log, "myblockchaind down, transaction %d" % self.xid
          return
        else:
          print >> self.log, "myblockchain error for stmt(%s) %s" % (stmt, e)
...
#" db = ", db, " server_pid = ", server_pid
  ...
  if num_records_before:
    ...
    con = MyBlockchaindb.connect(user=user, host=host, port=port, db=db)
  ...
  if checksum:
    ...
    checksum_worker = ChecksumWorker(MyBlockchaindb.connect(user=user, host=host, port=port, db=db), checksum)
    workers.append(checksum_worker)

  print >> log, "start %d threads" % num_workers
  for i in xrange(num_workers):
    worker = Worker(num_xactions_per_worker, i,
                    MyBlockchaindb.connect(user=user, host=host, port=port, db=db),
                    server_pid, do_blob, max_id, fake_changes, secondary_checks)
    workers.append(worker)

  if kill_db_after:
    print >> log, "kill myblockchaind"
