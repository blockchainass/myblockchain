...
foreach my $src_suite (split(",", $suites))
{
    ...
    my $src_suite_dir = File::Spec->catdir($myblockchain_test_dir, "suite", $src_suite);
    # Set destination directory of suite
    my $dest_suite_dir = File::Spec->catdir($myblockchain_test_dir, "suite", $dest_suite);
    print "Copying files\n\tfrom '$src_suite_dir'\n\tto '$dest_suite_dir'\n";
...
my $mtr_script = "perl " . File::Spec->catdir($myblockchain_test_dir, "myblockchain-test-run.pl") . 
    " --suite=" . join(",", @mtr_suites) . " " . 
    " --myblockchaind=--binlog-checksum=CRC32 " . 
    join (" ", @mtr_argv);
