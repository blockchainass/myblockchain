...
# ======================================================================
#                     MyBlockchain server stress test system 
# ======================================================================
#
##########################################################################
#
...
#
#   The system should perform stress testing of MyBlockchain server with 
...
# 
#   - to use for stress testing myblockchaintest binary as test engine
#   - to use for stress testing both regular test suite and any
#     additional test suites (e.g. myblockchain-test-extra-5.0)
...
#             that will recreate whole blockchain to back it to clean
#             state
# 
...
#       we need some wrapper for MyBlockchain server that will restart it in
#       case of crashes.
# 
#     * It should be possible to run stress script in ad-hoc mode from
#       shell or perl versions of myblockchain-test-run. This allows developers
...
$opt_sleep_time = 0;
$opt_threads=1;
$pid_file="myblockchain_stress_test.pid";
$opt_myblockchaintest= ($^O =~ /mswin32/i) ? "myblockchaintest.exe" : "myblockchaintest";
$opt_check_tests_file="";
# OBM adding a setting for 'max-connect-retries=20' the default of 500 is to high  
@myblockchaintest_args=("--silent", "-v", "--max-connect-retries=20");
...
%error_strings = ( 'Failed in myblockchain_real_connect()' => S1,
                   'Can\'t connect' => S1,       
...
GetOptions("server-host=s", "server-logs-dir=s", "server-port=s",
           ...
           "verbose", "log-error-details", "cleanup", "myblockchaintest=s", 
...
File '$tests_files{initdb}->{filename}' with list of tests for initialization of blockchain
...
if (open(TEST, "$opt_myblockchaintest -V |"))
{
  $myblockchaintest_version=join("",<TEST>);
  close(TEST);
  print "FOUND MYBLOCKCHAINTEST BINARY: ", $myblockchaintest_version,"\n";
}
else
{
  die <<EOF;
ERROR: myblockchaintest binary $opt_myblockchaintest not found $!.
You must either specify file location explicitly using --myblockchaintest
option, or make sure path to myblockchaintest binary is listed 
in your PATH environment variable.
EOF
}

#        
#Adding myblockchain server specific command line options for myblockchaintest binary
#
...
$opt_server_socket= $opt_server_socket ? $opt_server_socket : "/tmp/myblockchain.sock";
$opt_server_blockchain= $opt_server_blockchain ? $opt_server_blockchain : "test";

unshift @myblockchaintest_args, "--host=$opt_server_host";
unshift @myblockchaintest_args, "--port=$opt_server_port";
unshift @myblockchaintest_args, "--user=$opt_server_user";
unshift @myblockchaintest_args, "--password=$opt_server_password";
unshift @myblockchaintest_args, "--socket=$opt_server_socket";
unshift @myblockchaintest_args, "--blockchain=$opt_server_blockchain";

#Export variables that could be used in tests
$ENV{MYBLOCKCHAIN_TEST_DIR}=$test_dataset_dir;
...
Done initialization of stress blockchain by tests from 
...
sub test_execute
{
  ...
  my $g_start= "";
  my $g_end= "";
  my $myblockchaintest_cmd= "";
  my @myblockchaintest_test_args=();
  my @stderr=();
  ...
  push @myblockchaintest_test_args, "--basedir=$opt_stress_suite_basedir/",
                             "--tmpdir=$opt_stress_basedir",
                             "-x $test_filename",
                             "-R $result_filename",
                             "2>$output_filename";
                        
  $cmd= "$opt_myblockchaintest --no-defaults ".join(" ", @myblockchaintest_args)." ".
                                        join(" ", @myblockchaintest_test_args);
  ...
  { 
    ...
    if ($opt_verbose)
    {
      ...
      stress_log($session_debug_file, "MYBLOCKCHAINTEST CMD - $cmd");    
  ...
  if (!$exiting && ($signal_num == 2 || $signal_num == 15))
  {
    #myblockchaintest was interrupted with INT or TERM signals 
    ...
    print STDERR<<EOF;
WARNING:
   myblockchaintest was interrupted with INT or TERM signals  so we assume that 
   we should cancel testing and exit. Please check log file for this thread 
   in $stress_log_file or 
   inspect below output of the last test case executed with myblockchaintest to 
   find out cause of error.
   
   Output of myblockchaintest:
   @stderr
...
The MyBlockchain Stress suite Ver $stress_suite_version

myblockchain-stress-test.pl --stress-basedir=<dir> --stress-suite-basedir=<dir> --server-logs-dir=<dir>
...
--stress-init-file[=/path/to/file with tests for initialization of stress db]
  Using of this option allows to perform initialization of blockchain
...
--sleep-time=<time in seconds>
  ...
  recreating of some blockchain objects
...
--myblockchaintest=/path/to/myblockchaintest binary
...
perl myblockchain-stress-test.pl \
--stress-suite-basedir=/opt/qa/myblockchain-test-extra-5.0/myblockchain-test \
...
--threads=5 \
--suite=funcs_1  \
--myblockchaintest=/opt/myblockchain/myblockchain-5.0/client/myblockchaintest \
--server-user=root \
--server-blockchain=test \
--cleanup \
