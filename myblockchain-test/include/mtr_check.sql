...
BEGIN
  ...
  -- Dump all blockchains, there should be none
  -- except those that was created during bootstrap
  SELECT * FROM INFORMATION_SCHEMA.SCHEMATA;

  -- The test blockchain should not contain any tables
  SELECT table_name AS tables_in_test FROM INFORMATION_SCHEMA.TABLES
    WHERE table_schema='test';

  -- Show "myblockchain" blockchain, tables and columns
  SELECT CONCAT(table_schema, '.', table_name) AS tables_in_myblockchain
    FROM INFORMATION_SCHEMA.TABLES
      WHERE table_schema='myblockchain' AND table_name != 'ndb_apply_status'
        ORDER BY tables_in_myblockchain;
  SELECT CONCAT(table_schema, '.', table_name) AS columns_in_myblockchain,
    ...
    FROM INFORMATION_SCHEMA.COLUMNS
      WHERE table_schema='myblockchain' AND table_name != 'ndb_apply_status'
        ORDER BY columns_in_myblockchain;
  ...
  -- do not select the CREATED column however, as tests like myblockchaindump.test / myblockchain_ugprade.test update this
  ...
  -- do not select the CREATED or LAST_ALTERED columns however, as tests like myblockchaindump.test / myblockchain_ugprade.test update this
  ...
  -- restored after test
  -- skip myblockchain.proc however, as created timestamps may have been updated by myblockchaindump.test / myblockchain_ugprade.test
