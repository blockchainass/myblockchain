  ...
  $lib_innodb_plugin=
    my_find_file($::basedir,
		 ["storage/innodb_plugin", "storage/innodb_plugin/.libs",
		  "lib/myblockchain/plugin", "lib/plugin"],
		 ["ha_innodb_plugin.dll", "ha_innodb_plugin.so",
		  "ha_innodb_plugin.sl"],
		 NOT_REQUIRED);
  $do_innodb_plugin= ($::myblockchain_version_id >= 50100 &&
...
sub collect_one_suite($)
{
  ...
  my $suitedir= "$::glob_myblockchain_test_dir"; # Default
  if ( $suite ne "main" )
  {
    ...
    else
    {
      $suitedir= my_find_dir($::basedir,
			     ["share/myblockchain-test/suite",
			      "myblockchain-test/suite",
			      "internal/myblockchain-test/suite",
			      "myblockchain-test",
...
sub optimize_cases {
  ...
  foreach my $tinfo ( @$cases )
  {
    ...
    # --myblockchaind=--binlog-format=x, skip all test that does not
    # support it
    # =======================================================
	...
	#my $engine_value= $::myblockchaind_variables{$default_engine};
	#print " - The myblockchaind_variables says '$engine_value'\n";

	if ( ! exists $::myblockchaind_variables{$default_engine} and
	...
	#my $engine_value= $::myblockchaind_variables{$default_tmp_engine};
	#print " - The myblockchaind_variables says '$engine_value'\n";

	if ( ! exists $::myblockchaind_variables{$default_tmp_engine} and
...
sub process_opts_file {
  ...
  if ( -f $opt_file )
  {
    ...
    foreach my $opt ( @$opts )
    {
      ...
      # The opt file is used both to send special options to the myblockchaind
	...
	next;
      ...
      if ( defined $value )
      {
	# Specifies the file myblockchaintest should compare
	# output against
...
sub collect_one_test_case {
  ...
  {
    if (grep(/^--skip-log-bin/,  @::opt_extra_myblockchaind_opt) )
    {
      $tinfo->{'skip'}= 1;
...
#storage engine for myblockchaind in embedded mode is still MyISAM.
#To be removed after completion of WL #6911.
  ...
  # ----------------------------------------------------------------------
  # Append myblockchaind extra options to both master and slave
  # ----------------------------------------------------------------------
  push(@{$tinfo->{'master_opt'}}, @::opt_extra_myblockchaind_opt);
  push(@{$tinfo->{'slave_opt'}}, @::opt_extra_myblockchaind_opt);
  ...
  while ( my $line= <$F> )
  {
	 ...
	 $line =~ /^([[:space:]]*)source(.*);$/ )
    {
      ...
      foreach my $sourced_file (dirname($file). "/$value",
				"$::glob_myblockchain_test_dir/$value")
      {
	if ( -f $sourced_file )
	{
	  # Only source the file if it exists, we may get
	  # false positives in the regexes above if someone
	  # writes "source nnnn;" in a test case(such as myblockchaintest.test)
