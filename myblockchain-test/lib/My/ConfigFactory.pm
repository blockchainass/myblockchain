...
my @share_locations= ("share/myblockchain", "sql/share", "share");
...
sub fix_server_id {
  my ($self, $config, $group_name, $group)= @_;
#define in the order that myblockchainds are listed in my.cnf 
...
sub fix_socket {
  ...
  if (length($socket) > length("$dir/myblockchain_testsocket.sock"))
  {
    # Too long socket path, generate shorter based on port
    my $port = $group->value('port');
    $socket = "$dir/myblockchaind-$port.sock"; 
  }
  return $socket;
}
...
sub fix_log {
  ...
  return "$dir/myblockchaind.log";
}

sub fix_log_slow_queries {
  my ($self, $config, $group_name, $group)= @_;
  my $dir= dirname($group->value('datadir'));
  return "$dir/myblockchaind-slow.log";
}
...
sub fix_skip_ssl {
  ...
  # that myblockchaintest connects with SSL by default
  return 1;
}
...
#
# Rules to run for each myblockchaind in the config
#  - will be run in order listed here
#
my @myblockchaind_rules=
  (
...
if (IS_WINDOWS)
{
  ...
  # named pipes.
  push(@myblockchaind_rules, {'shared-memory-base-name' => \&fix_socket});
}
...
sub fix_cluster_dir {
  ...
  return "$vardir/myblockchain_cluster.$suffix/$process_type.$idx";
}
...
sub fix_cluster_backup_dir {
  ...
  my $vardir= $self->{ARGS}->{vardir};
  my (undef, $process_type, $idx, $suffix)= split(/\./, $group_name);
  return "$vardir/myblockchain_cluster.$suffix/";
}
...
#
# Rules to run for [myblockchaintest] section
#  - will be run in order listed here
#
my @myblockchaintest_rules=
(
 { 'ssl-ca' => \&fix_ssl_ca },
...
#
# Rules to run for [myblockchainbinlog] section
#  - will be run in order listed here
#
my @myblockchainbinlog_rules=
(
 { 'character-sets-dir' => \&fix_charset_dir },
);
...
#
# Rules to run for [myblockchain_upgrade] section
#  - will be run in order listed here
#
my @myblockchain_upgrade_rules=
(
);
...
# used for connecting to [myblockchaind.<suffix>]
#
sub post_check_client_group {
  my ($self, $config, $client_group_name, $myblockchaind_group_name)= @_;


  #  Settings needed for client, copied from its "myblockchaind"
  my %client_needs=
    (
     port       => 'port',
     socket     => 'socket',
  ...
  my $group_to_copy_from= $config->group($myblockchaind_group_name);
...
sub post_check_client_groups {
 ...
 my $first_myblockchaind= $config->first_like('myblockchaind.');

 return unless $first_myblockchaind;

 # Always generate [client] pointing to the first
 # [myblockchaind.<suffix>]
 $self->post_check_client_group($config,
				'client',
				$first_myblockchaind->name());

 # Then generate [client.<suffix>] for each [myblockchaind.<suffix>]
 foreach my $myblockchaind ( $config->like('myblockchaind.') ) {
   $self->post_check_client_group($config,
				  'client'.$myblockchaind->after('myblockchaind'),
				  $myblockchaind->name())
 }
...
# needed from the default [myblockchaind] section
# and from first [myblockchaind.<suffix>]
#
sub post_check_embedded_group {
  ...
  my $myblockchaind= $config->group('myblockchaind') or
    croak "Can't run with embedded, config has no default myblockchaind section";

  my $first_myblockchaind= $config->first_like('myblockchaind.') or
    croak "Can't run with embedded, config has no myblockchaind";

  my @no_copy =
    (
     '#log-error', # Embedded server writes stderr to myblockchaintest's log file
     'slave-net-timeout', # Embedded server are not build with replication
     'shared-memory-base-name', # No shared memory for embedded
    );

  foreach my $option ( $myblockchaind->options(), $first_myblockchaind->options() ) {
...
sub post_fix_myblockchain_cluster_section {
  my ($self, $config)= @_;
  ...
  foreach my $group ( $config->like('cluster_config\.\w*$') )
  {
    ...
    # Add ndb_connectstring to [myblockchain_cluster.<suffix>]
    $config->insert('myblockchain_cluster'.$group->suffix(),
		    'ndb_connectstring', $ndb_connectstring);

    # Add ndb_connectstring to each myblockchaind connected to this
    # cluster
    foreach my $myblockchaind ( $config->like('cluster_config.myblockchaind.')) {
      if ($myblockchaind->suffix() eq $group->suffix()) {
	my $after= $myblockchaind->after('cluster_config.myblockchaind');
	$config->insert("myblockchaind$after",
			'ndb_connectstring', $ndb_connectstring);
      }
    }
  }
...
(
 ...
 \&post_fix_myblockchain_cluster_section,
...
sub run_generate_sections_from_cluster_config {
  ...
  foreach my $group ( $config->like('cluster_config\.\w*$') ) {
	...
	if ($option_name eq 'myblockchaind'){
	  my $datadir=
	    $self->fix_cluster_dir($config,
				   "cluster_config.myblockchaind.$idx$suffix",
				   $group);
	  $config->insert("myblockchaind.$idx$suffix",
...
sub new_config {
  ...
  # [myblockchainbinlog] need additional settings
  $self->run_rules_for_group($config,
			     $config->insert('myblockchainbinlog'),
			     @myblockchainbinlog_rules);

  # [myblockchain_upgrade] need additional settings
  $self->run_rules_for_group($config,
			     $config->insert('myblockchain_upgrade'),
			     @myblockchain_upgrade_rules);
  ...
  # Additional rules required for [myblockchaintest]
  $self->run_rules_for_group($config,
			     $config->insert('myblockchaintest'),
			     @myblockchaintest_rules);
