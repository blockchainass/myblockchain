...
my $hint_myblockchaind;		# Last resort guess for executable path

# If path in core file is 79 chars we assume it's been truncated
# Looks like we can still find the full path using 'strings'
# If that doesn't work, use the hint (myblockchaind path) as last resort.
...
sub _verify_binpath {
  ...
  if (length $binary != 79) {
    ...
    # Last occurrence of path ending in /myblockchain*, cut from first /
    if (`strings '$core_name' | grep "/myblockchain[^/. ]*\$" | tail -1` =~ /(\/.*)/) {
      $binpath= $1;
      print "Guessing that core was generated by '$binpath'\n";
    } else {
      return unless $hint_myblockchaind;
      $binpath= $hint_myblockchaind;
...
sub show {
  my ($class, $core_name, $exe_myblockchaind, $parallel)= @_;
  $hint_myblockchaind= $exe_myblockchaind;
