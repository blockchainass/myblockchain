...
sub mtr_report_stats ($$;$) {
  ...
  foreach my $tinfo (@$tests)
  {
    ...
    # Look for warnings produced by myblockchaintest
  ...
  if ( $tot_failed != 0 )
  {
    ...
    print
      ...
      "the documentation\n",
      "at http://dev.myblockchain.com/doc/myblockchain/en/myblockchain-test-suite.html\n\n";
...
sub mtr_warning (@) {
  print STDERR _name(). _timestamp().
    "myblockchain-test-run: WARNING: ". join(" ", @_). "\n";
}
...
sub mtr_error (@) {
  ...
  print STDERR _name(). _timestamp().
    "myblockchain-test-run: *** ERROR: ". join(" ", @_). "\n";
  if (IS_WINDOWS)
  {
    POSIX::_exit(1);
  }
