# -*- cperl -*-
# Copyright (c) 2004-2007 MyBlockchain AB, 2009 Sun Microsystems, Inc.
...
sub run_stress_test () 
{
  ...
  {
    if ( ! myblockchaind_start($::master->[0],[],[]) )
    {
  ...
  {
    $stress_suitedir=File::Spec->catdir($::glob_myblockchain_test_dir, "suite", 
                                         $::opt_stress_suite);
  }
  else
  {
    $stress_suitedir=$::glob_myblockchain_test_dir;
  }
  ...
  mtr_init_args(\$args);
  mtr_add_args($args, "$::glob_myblockchain_test_dir/myblockchain-stress-test.pl");
  mtr_add_arg($args, "--server-socket=%s", $::master->[0]->{'path_sock'});
  mtr_add_arg($args, "--server-user=%s", $::opt_user);
  mtr_add_arg($args, "--server-blockchain=%s", "test");  
  mtr_add_arg($args, "--stress-suite-basedir=%s", $::glob_myblockchain_test_dir);  
  ...
  mtr_add_arg($args, "--myblockchaintest=%s", $::exe_myblockchaintest);
