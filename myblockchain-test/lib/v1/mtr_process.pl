# -*- cperl -*-
# Copyright (c) 2004-2006 MyBlockchain AB, 2008 Sun Microsystems, Inc.
...
sub mtr_myblockchainadmin_start($$$);
sub mtr_exit ($);
...
sub spawn_parent_impl {
  ...
  if ( $mode eq 'run' or $mode eq 'test' )
  {
    ...
    {
      # We run myblockchaintest and wait for it to return. But we try to
      # catch dying myblockchaind processes as well.
      #
      # We do blocking waitpid() until we get the return from the
      # "myblockchaintest" call. But if a myblockchaind process dies that we
      # started, we take this as an error, and kill myblockchaintest.
      ...
      while ( ($ret_pid= waitpid(-1,0)) != -1 )
      {
        ...
        if ( $timer_name )
        {
          if ( $timer_name eq "suite" )
          {
            # We give up here
            print STDERR "\n";
            kill(9, $pid); # Kill myblockchaintest
        ...
        if ( $ret_pid == $pid )
        {
          # We got termination of myblockchaintest, we are done
          $exit_value= mtr_process_exit_status($?);
          last;
        }

        # One of the child processes died, unless this was expected
	# myblockchaintest should be killed and test aborted
	...
	check_expected_crash_and_restart($ret_pid);
      ...
      if ( $ret_pid != $pid )
      {
        # We terminated the waiting because a "myblockchaind" process died.
        # Kill the myblockchaintest process.
	mtr_verbose("Kill myblockchaintest because another process died");
        kill(9,$pid);
...
# Kill all processes(myblockchaind, ndbd, ndb_mgmd and im) that would conflict with
# this run
...
sub mtr_kill_leftovers () {
  ...
  {
    mtr_debug("  - myblockchaind " .
              "(pid: $srv->{pid}; " .
              "pid file: '$srv->{path_pid}'; " .
              "socket: '$srv->{path_sock}'; ".
              "port: $srv->{port})");

    my $pid= mtr_myblockchainadmin_start($srv, "shutdown", 20);

    # Save the pid of the myblockchainadmin process
    $admin_pids{$pid}= 1;
  ...
  # If we trusted "myblockchainadmin --shutdown_timeout= ..." we could just
  # terminate now, but we don't (FIXME should be debugged).
  # So we try again to ping and at least wait the same amount of time
  # myblockchainadmin would for all to die.

  mtr_ping_with_timeout(\@kill_pids);

  # We now have tried to terminate nice. We have waited for the listen
  # port to be free, but can't really tell if the myblockchaind process died
  ...
  mtr_debug("Checking known myblockchaind servers...");
...
sub mtr_wait_blocking($) {
  ...
  # As myblockchainadmin is such a simple program, we trust it to terminate itself.
...
# Start "myblockchainadmin <command>" for a specific myblockchaind
sub mtr_myblockchainadmin_start($$$) {
  my $srv= shift;
  my $command= shift;
  ...
  my $pid= mtr_spawn($::exe_myblockchainadmin, $args,
		     "", "", "", "",
		     { append_log_file => 1 });
  mtr_verbose("mtr_myblockchainadmin_start, pid: $pid");
  return $pid;
...
sub mtr_ping_with_timeout($) {
  ...
  mtr_debug("Waiting for myblockchaind servers to stop...");
 ...
 TIME:
  ...
  if ($res)
  {
    mtr_debug("mtr_ping_with_timeout(): All myblockchaind instances are down.");
  }
  else
  {
...
sub mark_process_dead($)
{
  ...
  {
    if ( $myblockchaind->{'pid'} eq $ret_pid )
    {
      mtr_verbose("$myblockchaind->{'type'} $myblockchaind->{'idx'} exited, pid: $ret_pid");
      $myblockchaind->{'pid'}= 0;
      return;
    }
  }
...
sub check_expected_crash_and_restart($)
{
  ...
  foreach my $myblockchaind (@{$::master}, @{$::slave})
  {
    if ( $myblockchaind->{'pid'} eq $ret_pid )
    {
      ...
      my $expect_file= "$::opt_vardir/tmp/" . "$myblockchaind->{'type'}" .
	"$myblockchaind->{'idx'}" . ".expect";
      if ( -f $expect_file )
      {
	mtr_verbose("Crash was expected, file $expect_file exists");
	myblockchaind_start($myblockchaind, $myblockchaind->{'start_opts'},
		     $myblockchaind->{'start_slave_master_info'});
	unlink($expect_file);
      }
