# -*- cperl -*-
# Copyright (c) 2005, 2006 MyBlockchain AB, 2008, 2009 Sun Microsystems, Inc.
...
sub collect_one_test_case($$$$$$$$$) {
  ...
  {
    # Default, federated uses the first slave as it's federated blockchain
    $tinfo->{'slave_num'}= 1;
  }
  ...
  else
  {
	...
	return;
      ...
      # Ndb tests run with two myblockchaind masters
      $tinfo->{'master_num'}= 2;
    }
    else
    {
    ...
    if ( $tinfo->{'innodb_test'} )
    {
      ...
      if ( $::myblockchaind_variables{'innodb'} eq "OFF" )
      {
...
sub mtr_options_from_test_file($$) {
  ...
  while ( my $line= <$F> )
  {
	 ...
	 $line =~ /^([[:space:]]*)source(.*);$/ )
    {
      ...
      my $sourced_file= "$::glob_myblockchain_test_dir/$value";
      if ( -f $sourced_file )
      {
