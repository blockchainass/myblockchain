# -*- cperl -*-
# Copyright (c) 2006 MyBlockchain AB, 2008 Sun Microsystems, Inc.
...
sub mtr_im_check_myblockchainds_alive ($);
sub mtr_im_check_myblockchaind_alive ($);
sub mtr_im_cleanup ($);
sub mtr_im_rm_file ($);
sub mtr_im_errlog ($);
sub mtr_im_kill ($);
sub mtr_im_wait_for_connection ($$$);
sub mtr_im_wait_for_myblockchaind($$$);
...
sub mtr_im_load_pids($) {
  ...
  # Obtain myblockchaind-process pids.

  my $instances = $im->{'instances'};

  for ( my $idx= 0; $idx < 2; ++$idx )
  {
    mtr_debug("IM-guarded myblockchaind[$idx] PID file: '" .
              $instances->[$idx]->{'path_pid'} . "'.");

    my $myblockchaind_pid;

    if ( -r $instances->[$idx]->{'path_pid'} )
    {
      $myblockchaind_pid= mtr_get_pid_from_file($instances->[$idx]->{'path_pid'});
      mtr_debug("IM-guarded myblockchaind[$idx] PID: $myblockchaind_pid.");
    }
    else
    {
      $myblockchaind_pid= undef;
      mtr_debug("IM-guarded myblockchaind[$idx]: no PID file.");
    }

    $instances->[$idx]->{'pid'}= $myblockchaind_pid;
  }
...
sub mtr_im_check_alive($) {
  ...
  return 1 if mtr_im_check_myblockchainds_alive($im);
...
sub mtr_im_check_myblockchainds_alive($) {
  ...
  mtr_debug("Checking for IM-guarded myblockchaind instances...");

  my $instances = $im->{'instances'};

  for ( my $idx= 0; $idx < 2; ++$idx )
  {
    mtr_debug("Checking myblockchaind[$idx]...");

    return 1
      if mtr_im_check_myblockchaind_alive($instances->[$idx]);
  }
}
...
sub mtr_im_check_myblockchaind_alive($) {
  my $myblockchaind_instance= shift;

  # Check that the process is dead.

  if ( defined $myblockchaind_instance->{'pid'} )
  {
    if ( kill (0, $myblockchaind_instance->{'pid'}) )
    {
      mtr_debug("Mysqld instance (PID: $myblockchaind_instance->{pid}) is alive.");
      return 1;
    }
    else
    {
      mtr_debug("Mysqld instance (PID: $myblockchaind_instance->{pid}) is dead.");
    }
  }
  else
  {
    mtr_debug("No PID file for myblockchaind instance.");
  }

  # Check that myblockchaind does not accept client connections.

  if ( mtr_ping_port($myblockchaind_instance->{'port'}) )
  {
    mtr_debug("Mysqld instance (port: $myblockchaind_instance->{port}) " .
              "is accepting connections.");

    mtr_im_errlog("Mysqld is accepting connections on port " .
                  "$myblockchaind_instance->{port}, but there is no " .
                  "process information.");
    return 1;
  }
  else
...
sub mtr_im_errlog($) {
  ...
  # my $errlog= "$opt_vardir/log/myblockchain-test-run.pl.err";
  # 
  # open (ERRLOG, ">>$errlog") ||
...
sub mtr_im_kill($) {
  ...
  # Kill guarded myblockchaind instances.

  my @myblockchaind_pids;

  mtr_debug("Collecting PIDs of myblockchaind instances to kill...");
  ...
  for ( my $idx= 0; $idx < 2; ++$idx )
  {
    ...
    mtr_debug("  - IM-guarded myblockchaind[$idx] PID: $pid.");

    push (@myblockchaind_pids, $pid);
  }

  if ( scalar @myblockchaind_pids > 0 )
  {
    mtr_debug("Killing IM-guarded myblockchaind instances...");
    mtr_im_kill_process(\@myblockchaind_pids, 'KILL', 10, 1);
  }
...
sub mtr_im_wait_for_myblockchaind($$$) {
  ...
  mtr_debug("Waiting for IM-guarded myblockchaind on port $myblockchaind->{port} " .
            "to start accepting connections...");

  for ( my $cur_attempt= 1; $cur_attempt <= $total_attempts; ++$cur_attempt )
  {
    mtr_debug("Trying to connect to myblockchaind " .
              "($cur_attempt of $total_attempts)...");

    if ( mtr_ping_port($myblockchaind->{'port'}) )
    {
      mtr_debug("Mysqld is accepting connections " .
                "on port $myblockchaind->{port}.");
      return 1;
    }
  ...
  mtr_debug("Mysqld does not accept connections " .
            "on port $myblockchaind->{port} after " .
            ($total_attempts * $connect_timeout) . " seconds.");
...
sub mtr_im_start($$) {
  ...
  mtr_debug("Waiting for guarded myblockchainds instances to create PID files...");

  for ( my $idx= 0; $idx < 2; ++$idx )
  {
    my $myblockchaind= $im->{'instances'}->[$idx];

    if ( exists $myblockchaind->{'nonguarded'} )
    {
      next;
    }

    mtr_debug("Waiting for myblockchaind[$idx] to create PID file (" .
              "path: '$myblockchaind->{path_pid}'; " .
              "timeout: $myblockchaind->{start_timeout})...");

    unless ( sleep_until_file_created($myblockchaind->{'path_pid'},
                                      $myblockchaind->{'start_timeout'},
                                      -1) ) # real PID is still unknown
    {
      mtr_debug("myblockchaind[$idx] has not created PID file in " .
                 "$myblockchaind->{start_timeout} secs.");
      mtr_debug("Aborting test suite...");

      mtr_kill_leftovers();

      mtr_report("myblockchaind[$idx] has not created PID file in " .
                 "$myblockchaind->{start_timeout} secs.");
      return 0;
    }

    mtr_debug("PID file for myblockchaind[$idx] ($myblockchaind->{path_pid} created.");
  }

  # Wait until we can connect to guarded myblockchaind-instances
  # (in other words -- wait for IM to start guarded instances).

  mtr_debug("Waiting for guarded myblockchainds to start accepting connections...");
  ...
  for ( my $idx= 0; $idx < 2; ++$idx )
  {
    ...
    mtr_debug("Waiting for myblockchaind[$idx] to accept connection...");

    unless ( mtr_im_wait_for_myblockchaind($myblockchaind, 30, 1) )
    {
      mtr_debug("Can not connect to myblockchaind[$idx] " .
                "in $IM_CONNECT_TIMEOUT seconds after start.");
      mtr_debug("Aborting test suite...");

      mtr_kill_leftovers();

      mtr_report("Can not connect to myblockchaind[$idx] " .
                 "in $IM_CONNECT_TIMEOUT seconds after start.");
      return 0;
    }

    mtr_debug("myblockchaind[$idx] started.");
  }
