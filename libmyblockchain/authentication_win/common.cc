...
/**
  Create connection out of an active MYBLOCKCHAIN_PLUGIN_VIO object.

  @param[in] vio  pointer to a @c MYBLOCKCHAIN_PLUGIN_VIO object used for
                  connection - it can not be NULL
*/

Connection::Connection(MYBLOCKCHAIN_PLUGIN_VIO *vio): m_vio(vio), m_error(0)
{
  DBUG_ASSERT(vio);
}
