...
/**
  ...
  If WITH_TEST_TRACE_PLUGIN build option was checked when building libmyblockchain
  ...
  it. If MYBLOCKCHAIN_TEST_TRACE_DEBUG environment variable is non-zero then it logs
  information about received trace events and reports if a particular event
  is invalid.

  If MYBLOCKCHAIN_TEST_TRACE_CRASH environment variable is set to something else than 0
...
#include "myblockchain_trace.h"
#include <myblockchain/service_my_snprintf.h>          /* my_snprintf() */
...
struct st_myblockchain_client_plugin_TRACE test_trace_plugin=
{
  MYBLOCKCHAIN_CLIENT_TRACE_PLUGIN,
...
/*
  ...
  {
    LOGGER(myblockchain);   // declare logger for given connection - without this
  ...
  MYBLOCKCHAIN_TRACE_DEBUG env. variable (see plugin_init() below).
*/
...
public:
  ...
  Logger(MYBLOCKCHAIN *conn);
  void log(const char *format, ...);
  void dump(const char *key, const void *data, size_t len);
};


Logger::Logger(MYBLOCKCHAIN *conn): end(buffer)
{
  connection_id= myblockchain_thread_id(conn);
};
...
/**
  ...
  MYBLOCKCHAIN_TEST_TRACE_CRASH upon plugin initialization. By default it
  is disabled.
*/
...
{
  const char *opt= getenv("MYBLOCKCHAIN_TEST_TRACE_CRASH");

  if (opt && '0' != *opt)
    opt_crash= true;

  opt= getenv("MYBLOCKCHAIN_TEST_TRACE_DEBUG");
...
void*
trace_start(struct st_myblockchain_client_plugin_TRACE *self,
            MYBLOCKCHAIN *conn, enum protocol_stage stage)
{
  LOGGER(conn);
...
void
trace_stop(struct st_myblockchain_client_plugin_TRACE *self,
           MYBLOCKCHAIN *conn, void *data)
{
  LOGGER(conn);
...
#define protocol_stage_chk_ev_declare(S)                  \
  static                                                  \
  int check_event_ ## S(MYBLOCKCHAIN *conn,                      \
...
int
trace_event(struct st_myblockchain_client_plugin_TRACE *self,
            void *data_ptr, MYBLOCKCHAIN *conn,
...
int check_event_CONNECTING(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_WAIT_FOR_INIT_PACKET(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_AUTHENTICATE(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_SSL_NEGOTIATION(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_READY_FOR_COMMAND(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_WAIT_FOR_PACKET(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_WAIT_FOR_RESULT(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_WAIT_FOR_FIELD_DEF(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_WAIT_FOR_ROW(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_WAIT_FOR_PS_DESCRIPTION(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_WAIT_FOR_PARAM_DEF(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_FILE_REQUEST(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
...
int check_event_DISCONNECTED(MYBLOCKCHAIN *conn,
                           struct st_trace_data *data,
                           enum trace_event ev,
                           struct st_trace_event_args args,
                           enum protocol_stage *next_stage)
